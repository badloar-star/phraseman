...
### Edge-cases & offline
> **⚠️ Офлайн-контракт — см. `specs/ai-gen-offline-policy.md` (обязателен).**
10.96a. **ГАРАНТИЯ (не риск): генеративный урок ОФЛАЙН-ПОЛНЫЙ на бандле.** Движок собирает урок из 1600 прошитых фраз + словарей БЕЗ сети; офлайн теряется только ИИ-новизна (свежие примеры), не сам урок. Это самая устойчивая офлайн-секция суита (§2 offline-policy). Спец-резерв уроку НЕ нужен.
10.96b. Голос офлайн = **device-TTS (expo-speech)** по бандл-фразам; приоритет: кешированное серверное аудио → device-TTS → текст без принудительной тишины. НЕ блокировать поток ожиданием сетевого аудио (§3 offline-policy).
...
10.99. Пул пуст + нет сети → сборка урока из бандла (1600 фраз) + device-TTS; при `net_status=offline` сетевой запрос НЕ отправлять; честная плашка «офлайн-урок из твоего набора». Kill-switch НИКОГДА не пустой batch (C3).
10.100. Аудио createAsync подвис → watchdog → шаг работает через device-TTS/без аудио (не блокировать).
...
13.2. ✅ **РЕШЕНО владельцем 2026-07-05: теория подаётся ЧЕРЕЗ ЗАДАНИЯ** (task-embedded подсказки/поле `rule` на каждом шаге), отдельный текстовый раздел теории на язык НЕ пишется — новый язык требует ноль ручного авторинга теории. «Большой» генеративный справочник теории (§7.1) — НЕ в v1; вернуться к нему позже по метрикам спроса, отдельным этапом. Это снимает отдельную ось кеша под теорию и снижает офлайн-вес.
