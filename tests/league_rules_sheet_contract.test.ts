...
describe('объяснялка лиги (владелец, 2026-08-24)', () => {
  ...
  it('объясняет недельный счёт и Супервоскресенье без старых двух часов', () => {
    const code = stripComments(sheet);
    expect(code).toContain('Супервоскресенье');
    // зачем (владелец, 2026-09-14): две фразы про подарочные и видео-руны сняты
    // по прямому решению владельца — «убери это, бред и не надо такое писать».
    // Сторож сторожил ИХ ДОСЛОВНО, поэтому он и правится вместе с текстом: он
    // обязан охранять действующее правило, а не отменённое (тот же класс, что в
    // project_paywall_guard_protected_a_lie_2026-09-14 — тест требовал лжи).
    // Остаётся суть раздела: недельный счёт складывается из занятий и игр.
    expect(code).toContain('собирай руны');
    expect(code).toContain('руны за занятия, игры и просмотр видео удваиваются');
    expect(code).not.toMatch(/последние два часа|зона вылета|last two hours/i);
  });
