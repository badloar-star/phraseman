...
// зачем: Lang расширен до 9 локалей (добавлен en) — T() без en ломал типы
// на каждом из ~60 вызовов ниже; сигнатура и порядок ключей (ru, uk, en, es, ...)
// зеркалят остальной проект (см. вызовы triLang в этом же файле).
const T = (ru: string, uk: string, en: string, es: string, ptBR: string, vi: string, id: string, tr: string, pl: string): Tri =>
  ({ ru, uk, en, es, 'pt-BR': ptBR, vi, id, tr, pl });
...
export const SEASON_MODAL_COPY: Record<SeasonReward['kind'], { title: Tri; desc: Tri; applied: Tri }> = {
  pearls: {
    title: T('Жемчужины!', 'Перлини!', 'Pearls!', '¡Perlas!', 'Pérolas!', 'Ngọc trai!', 'Mutiara!', 'İnciler!', 'Perły!'),
    ...
    desc: T('Звенят в кошельке, ждут своего магазина или буста.', 'Дзвенять у гаманці, чекають на свій магазин чи буст.', 'They\'ll jingle in your wallet, ready for the shop or a boost.', 'Sonarán en tu monedero, listas para la tienda o un impulso.', 'Vão tilintar no seu bolso, prontas para a loja ou um impulso.', 'Sẽ kêu leng keng trong ví, chờ cửa hàng hoặc tăng tốc.', 'Akan berdenting di dompet, siap buat toko atau boost.', 'Cüzdanında şıngırdayacak, mağaza ya da destek için hazır.', 'Zabrzęczą w portfelu, gotowe na sklep albo boost.'),
    applied: T('Жемчужины у вас на счету. Проценты не начисляем — но и не забираем обратно.', 'Перлини на вашому рахунку.', 'Pearls are on your balance. No interest paid — but none taken back either.', 'Perlas en tu saldo. Sin intereses, pero tampoco se pierden.', 'Pérolas no saldo. Sem juros, mas sem perdas.', 'Ngọc trai đã vào tài khoản.', 'Mutiara sudah masuk saldo.', 'İnciler hesabında.', 'Perły na koncie.'),
  },
  battery: {
    title: T('Запасная батарея', 'Запасна батарея', 'Spare battery', 'Batería de reserva', 'Bateria reserva', 'Pin dự phòng', 'Baterai cadangan', 'Yedek batarya', 'Zapasowa bateria'),
    desc: T('Заряжает энергию под завязку одним нажатием. Держите до момента «энергия кончилась в самый неподходящий раз».', 'Заряджає енергію по вінця одним натисканням. Тримайте до моменту «енергія скінчилась у найгірший момент».', 'Tops off your energy with one tap. Save it for that "ran out at the worst possible moment" moment.', 'Recarga la energía al máximo con un toque. Guárdala para cuando se agote en el peor momento.', 'Recarrega a energia ao máximo com um toque. Guarde para quando acabar na pior hora.', 'Sạc đầy năng lượng chỉ bằng một chạm. Cất dành lúc năng lượng cạn vào thời điểm tệ nhất.', 'Isi penuh energi dengan sekali tap. Simpan untuk saat energi habis di waktu terburuk.', 'Tek dokunuşla enerjiyi doldurur. En kötü anda bitmesi için sakla.', 'Ładuje energię do pełna jednym dotknięciem. Trzymaj na moment, gdy zabraknie jej najgorzej.'),
    applied: T('Энергия заряжена под завязку. Отговорки закончились раньше энергии.', 'Енергія по вінця. Виправдання скінчилися раніше за енергію.', 'Energy topped off. Your excuses ran out before your energy did.', 'Energía al máximo. Ya no hay excusas.', 'Energia no máximo. Sem mais desculpas.', 'Năng lượng đầy. Hết cớ rồi nhé.', 'Energi penuh. Tak ada alasan lagi.', 'Enerji tam dolu. Bahane kalmadı.', 'Energia po brzegi. Koniec wymówek.'),
  },
  ...
  league_boost: {
    title: T('Буст лиги ×2', 'Буст ліги ×2', 'League boost ×2', 'Impulso de liga ×2', 'Impulso de liga ×2', 'Tăng tốc giải ×2', 'Dorongan liga ×2', 'Lig desteği ×2', 'Boost ligi ×2'),
    desc: T('Удваивает очки лиги до конца сегодняшнего дня. Соперники узнают об этом позже вас.', 'Подвоює очки ліги до кінця сьогоднішнього дня. Суперники дізнаються про це пізніше за вас.', 'Doubles your league points until the end of today. Your rivals will find out later than you did.', 'Duplica los puntos de liga hasta el final del día. Tus rivales se enterarán después que tú.', 'Dobra os pontos da liga até o fim do dia. Seus rivais vão saber depois de você.', 'Nhân đôi điểm giải đấu đến hết ngày hôm nay. Đối thủ biết tin sau bạn.', 'Melipatgandakan poin liga sampai akhir hari ini. Rival tahu belakangan.', 'Bugünün sonuna kadar lig puanlarını ikiye katlar. Rakiplerin bunu senden sonra öğrenecek.', 'Podwaja punkty ligi do końca dzisiejszego dnia. Rywale dowiedzą się później niż Ty.'),
    applied: T('Очки лиги идут вдвойне до конца сегодняшнего дня. Соперники по таблице ещё не в курсе.', 'Очки ліги йдуть удвічі до кінця сьогоднішнього дня.', 'League points count double until the end of today. Your rivals on the table have no idea yet.', 'Puntos de liga al doble hasta el final del día.', 'Pontos de liga em dobro até o fim do dia.', 'Điểm giải đấu nhân đôi đến hết ngày hôm nay.', 'Poin liga dua kali lipat sampai akhir hari ini.', 'Lig puanları bugünün sonuna kadar iki katı.', 'Punkty ligi podwójnie do końca dzisiejszego dnia.'),
  },
  club_totem: {
    title: T('Тотем клуба', 'Тотем клубу', 'Club totem', 'Tótem del club', 'Totem do clube', 'Vật tổ câu lạc bộ', 'Totem klub', 'Kulüp totemi', 'Totem klubu'),
    desc: T('Бесплатный буст на всю вашу группу лиги. Активируется в клубе — и все узнают, кому сказать спасибо.', 'Безплатний буст на всю вашу групу ліги. Активується в клубі — і всі дізнаються, кому дякувати.', 'A free boost for your whole league group. Activate it in the club — and everyone will know who to thank.', 'Impulso gratis para todo tu grupo de liga. Se activa en el club, y todos sabrán a quién agradecer.', 'Impulso grátis para todo o seu grupo de liga. Ativado no clube — todos vão saber a quem agradecer.', 'Tăng tốc miễn phí cho cả nhóm giải đấu. Kích hoạt trong câu lạc bộ — mọi người sẽ biết cảm ơn ai.', 'Boost gratis untuk seluruh grup ligamu. Diaktifkan di klub — semua tahu harus berterima kasih ke siapa.', 'Tüm lig grubuna ücretsiz destek. Kulüpte etkinleştirilir — kime teşekkür edeceklerini herkes bilecek.', 'Darmowy boost dla całej Twojej grupy ligi. Aktywowany w klubie — wszyscy będą wiedzieć, komu dziękować.'),
    applied: T('Тотем ждёт в клубе — активируйте его там. Клуб станет заметнее, а вы — причиной этого.', 'Тотем чекає в клубі — активуйте його там.', 'The totem is waiting in the club — activate it there.', 'Tótem listo en el club: actívalo allí.', 'Totem pronto no clube: ative-o lá.', 'Vật tổ đang chờ trong câu lạc bộ — kích hoạt tại đó.', 'Totem menunggu di klub — aktifkan di sana.', 'Totem kulüpte hazır — orada etkinleştirin.', 'Totem czeka w klubie — aktywujcie go tam.'),
  },
  golden_lesson: {
    title: T('Золотой урок', 'Золотий урок', 'Golden lesson', 'Lección dorada', 'Lição dourada', 'Bài học vàng', 'Pelajaran emas', 'Altın ders', 'Złota lekcja'),
    desc: T('Утраивает опыт за один пройденный урок — какой именно, выбираете сами. Заряд ждёт своего часа.', 'Потроює досвід за один пройдений урок — який саме, обираєте самі. Заряд чекає свого часу.', 'Triples the XP from one completed lesson — you pick which one. The charge waits for its moment.', 'Triplica la XP de una lección: tú eliges cuál. La carga espera su momento.', 'Triplica o XP de uma lição: você escolhe qual. A carga espera o momento certo.', 'Nhân ba XP cho một bài học — bạn chọn bài nào. Điện tích chờ đúng lúc.', 'Melipatgandakan XP satu pelajaran ×3 — kamu yang pilih yang mana. Muatan menunggu saatnya.', 'Bir dersin XP\'sini üçe katlar — hangisi olduğuna sen karar verirsin. Şarj sırasını bekliyor.', 'Potraja XP z jednej lekcji — Ty wybierasz, której. Ładunek czeka na swój moment.'),
    applied: T('Заряд активен. Следующий пройденный урок принесёт золотой опыт — один-единственный раз.', 'Заряд активний. Наступний урок принесе золотий досвід — лише один раз.', 'Charge active. The next lesson you finish will pay out golden XP — just this once.', 'Cargado. La próxima lección dará XP dorada, una sola vez.', 'Carregado. A próxima lição dará XP dourada, uma única vez.', 'Đã sẵn sàng. Bài học tiếp theo cho XP vàng — chỉ một lần.', 'Aktif. Pelajaran berikutnya memberi XP emas — sekali saja.', 'Yüklü. Bir sonraki ders altın XP verir — sadece bir kez.', 'Naładowane. Następna lekcja da złote XP — tylko raz.'),
  },
  ...
  collection_magnet: {
    title: T('Магнит коллекции', 'Магніт колекції', 'Collection magnet', 'Imán de colección', 'Ímã de coleção', 'Nam châm bộ sưu tập', 'Magnet koleksi', 'Koleksiyon mıknatısı', 'Magnes kolekcji'),
    desc: T('24 часа с момента активации карточки сокровищницы будут находиться вдвое чаще — притягиваются, как магнитом.', '24 години з моменту активації картки скарбниці траплятимуться вдвічі частіше — притягуються, мов магнітом.', 'For 24 hours from activation, treasure cards will show up twice as often — pulled in like a magnet.', '24 horas desde la activación, las cartas del tesoro aparecerán el doble de veces: como imantadas.', '24 horas a partir da ativação, as cartas do tesouro vão aparecer o dobro das vezes — como se fossem imantadas.', '24 giờ kể từ khi kích hoạt, thẻ kho báu xuất hiện nhiều gấp đôi — như bị nam châm hút.', '24 jam sejak diaktifkan, kartu harta akan muncul dua kali lebih sering — seolah tertarik magnet.', 'Etkinleştirmeden itibaren 24 saat boyunca hazine kartları iki katı sıklıkla çıkar — mıknatıs gibi çeker.', '24 godziny od aktywacji karty skarbca będą trafiać się dwa razy częściej — jakby przyciągał je magnes.'),
    applied: T('Магнит включён на 24 часа. Карточки сами липнут к вам — физика бессильна.', 'Магніт увімкнено на 24 години. Картки самі липнуть до вас.', 'Magnet active for 24 hours. Cards stick to you on their own — physics can\'t stop it.', 'Imán activo 24 h. Las cartas se pegan solas.', 'Ímã ativo por 24 h. As cartas grudam sozinhas.', 'Nam châm hoạt động 24 giờ. Thẻ tự dính vào bạn.', 'Magnet aktif 24 jam. Kartu menempel sendiri.', 'Mıknatıs 24 saat aktif. Kartlar kendiliğinden yapışıyor.', 'Magnes aktywny 24 h. Karty same się kleją.'),
  },
  ...
  nick_color: {
    title: T('Цвет ника', 'Колір ніка', 'Name color', 'Color del nombre', 'Cor do nome', 'Màu biệt danh', 'Warna nama', 'Takma ad rengi', 'Kolor nicku'),
    desc: T('Красит ник в фирменную бирюзу сезона и вешает рядом титул «Сезон 1». В таблице вас теперь видно ещё до того, как прочитают имя.', 'Фарбує нік у фірмову бірюзу сезону і додає поруч титул «Сезон 1». У таблиці вас тепер видно ще до того, як прочитають ім’я.', 'Paints your name in the season\'s signature turquoise and adds the "Season 1" title next to it. People spot you on the table before they even read your name.', 'Pinta tu nombre con el turquesa de temporada y añade el título «Temporada 1». En la tabla te verán antes de leer tu nombre.', 'Pinta seu nome com o turquesa da temporada e adiciona o título «Temporada 1». Na tabela, vão te ver antes de ler seu nome.', 'Tô tên bạn màu ngọc lam của mùa và gắn thêm danh hiệu «Mùa 1». Trong bảng xếp hạng, người ta thấy bạn trước cả khi đọc tên.', 'Mewarnai namamu dengan toska musim dan menambahkan gelar «Musim 1». Di peringkat, kamu terlihat sebelum namamu terbaca.', 'Adını sezonun turkuazına boyar ve yanına «1. Sezon» unvanını ekler. Sıralamada, adın okunmadan önce sen görünürsün.', 'Maluje Twój nick na sezonowy turkus i dokłada tytuł «Sezon 1». W tabeli widać Cię, zanim ktoś przeczyta imię.'),
    applied: T('Ник перекрашен, титул «Сезон 1» присвоен. В таблице теперь видно издалека.', 'Нік перефарбовано, титул «Сезон 1» присвоєно. У таблиці видно здалеку.', 'Name repainted, "Season 1" title assigned. Visible from across the table now.', 'Nombre repintado, título «Temporada 1» asignado. Se ve desde lejos.', 'Nome repintado, título «Temporada 1» atribuído. Dá para ver de longe.', 'Đã đổi màu tên, gắn danh hiệu «Mùa 1». Nhìn từ xa cũng thấy.', 'Nama diwarnai, gelar «Musim 1» diberikan. Terlihat dari jauh.', 'Ad rengi değişti, «1. Sezon» unvanı verildi. Uzaktan görünüyor.', 'Nick pomalowany, przyznano tytuł «Sezon 1». Widać z daleka.'),
  },
  custom_avatar: {
    title: T('Кастомный аватар', 'Кастомний аватар', 'Custom avatar', 'Avatar personalizado', 'Avatar personalizado', 'Ảnh đại diện riêng', 'Avatar kustom', 'Özel avatar', 'Własny awatar'),
    desc: T('Добавляет новый стиль в вашу коллекцию аватаров — какой именно, решит случай, а носить его или нет, решаете вы.', 'Додає новий стиль до вашої колекції аватарів — який саме, вирішить випадок, а носити його чи ні — вирішуєте ви.', 'Adds a new style to your avatar collection — chance decides which one, you decide whether to wear it.', 'Añade un nuevo estilo a tu colección de avatares — cuál, lo decide el azar; si lo usas o no, lo decides tú.', 'Adiciona um novo estilo à sua coleção de avatares — qual, o acaso decide; usar ou não, é você quem decide.', 'Thêm một kiểu mới vào bộ sưu tập avatar — kiểu nào thì để may rủi quyết định, còn có đeo hay không là tùy bạn.', 'Menambah gaya baru ke koleksi avatarmu — yang mana, biar keberuntungan yang memilih, dipakai atau tidak terserah kamu.', 'Avatar koleksiyonuna yeni bir stil ekler — hangisi olduğuna şans karar verir, takıp takmamaya ise sen.', 'Dodaje nowy styl do kolekcji awatarów — który, decyduje przypadek, a czy go nosić — Ty.'),
    applied: T('Аватар ваш. Кот или закат — решать вам.', 'Аватар ваш. Кіт чи захід сонця — вирішувати вам.', 'Avatar\'s yours. Cat or sunset, you decide.', 'Avatar tuyo. Gato o atardecer, tú decides.', 'Avatar é seu. Gato ou pôr do sol, você decide.', 'Ảnh đại diện của bạn. Tùy bạn chọn.', 'Avatar milikmu. Terserah kamu.', 'Avatar senin. Sen karar ver.', 'Awatar Twój. Ty decydujesz.'),
  },
  card_pack: {
    title: T('Набор карточек', 'Набір карток', 'Card set', 'Set de tarjetas', 'Pacote de cartões', 'Bộ thẻ', 'Paket kartu', 'Kart paketi', 'Zestaw fiszek'),
    desc: T('Открывает фирменный набор «Peaky Blinders» навсегда — без пересдачи, без срока годности.', 'Відкриває фірмовий набір «Peaky Blinders» назавжди — без перескладання, без терміну придатності.', 'Unlocks the signature "Peaky Blinders" set for good — no retakes, no expiry date.', 'Desbloquea el set «Peaky Blinders» para siempre: sin repetir el examen, sin fecha de caducidad.', 'Desbloqueia o pacote «Peaky Blinders» para sempre — sem refazer prova, sem prazo de validade.', 'Mở khóa bộ «Peaky Blinders» vĩnh viễn — không cần thi lại, không có hạn dùng.', 'Membuka paket «Peaky Blinders» selamanya — tanpa ujian ulang, tanpa kedaluwarsa.', '«Peaky Blinders» paketini sonsuza dek açar — tekrar sınav yok, son kullanma tarihi yok.', 'Odblokowuje zestaw «Peaky Blinders» na zawsze — bez podejścia do egzaminu, bez terminu ważności.'),
    applied: T('Набор «Peaky Blinders» открыт навсегда — карточки уже в коллекции. Разбирайте, пока не остыли.', 'Набір «Peaky Blinders» відкрито назавжди — картки вже в колекції.', 'The "Peaky Blinders" set is unlocked for good — the cards are already in your collection. Go check them out.', 'Set «Peaky Blinders» desbloqueado para siempre. A revisarlas.', 'Pacote «Peaky Blinders» desbloqueado para sempre. Vá conferir.', 'Bộ «Peaky Blinders» đã mở khóa vĩnh viễn.', 'Paket «Peaky Blinders» terbuka selamanya.', '«Peaky Blinders» paketi sonsuza dek açıldı.', 'Zestaw «Peaky Blinders» odblokowany na zawsze.'),
  },
  season_finale: {
    title: T('Финал сезона!', 'Фінал сезону!', 'Season finale!', '¡Final de temporada!', 'Final da temporada!', 'Chung kết mùa!', 'Final musim!', 'Sezon finali!', 'Finał sezonu!'),
    desc: T('Переливающийся ник, легендарный титул и финальный вихрь ауры — весь набор для тех, кто дошёл до конца, а не сдался на полпути.', 'Переливчастий нік, легендарний титул і фінальний вихор аури — весь набір для тих, хто дійшов до кінця, а не здався на півдорозі.', 'A shimmering name, a legendary title, and the final aura vortex — the whole set for those who made it to the end instead of quitting halfway.', 'Nombre iridiscente, título legendario y el vórtice final del aura: el paquete completo para quien llegó al final y no tiró la toalla a mitad de camino.', 'Nome iridescente, título lendário e o vórtice final da aura — o pacote completo para quem chegou ao fim e não desistiu no meio do caminho.', 'Biệt danh lung linh, danh hiệu huyền thoại và xoáy hào quang cuối cùng — trọn bộ dành cho ai đi đến đích, không bỏ cuộc giữa chừng.', 'Nama berkilau, gelar legendaris, dan pusaran aura final — paket lengkap untuk yang sampai ke ujung, bukan yang menyerah di tengah jalan.', 'Yanardöner ad, efsanevi unvan ve final aura girdabı — yarı yolda pes etmeyip sonuna kadar gidenler için tam paket.', 'Mieniący się nick, legendarny tytuł i finałowy wir aury — pełny zestaw dla tych, co dotarli do końca, a nie poddali się w połowie.'),
    applied: T('Сезон закрыт полностью. Путь был долгим, и вы дошли до конца. Уважение.', 'Сезон закрито повністю. Шлях був довгим, і ви дійшли до кінця.', 'Season fully closed out. It was a long road, and you made it to the end. Respect.', 'Temporada completada. Fue un largo camino y llegaste. Respeto.', 'Temporada concluída. Foi um longo caminho e você chegou. Respeito.', 'Hoàn thành cả mùa. Một chặng đường dài và bạn đã tới đích.', 'Musim selesai penuh. Perjalanan panjang, dan kamu berhasil.', 'Sezon tamamen bitti. Uzun bir yoldu ve başardınız.', 'Sezon zamknięty. To była długa droga i dotarliście do końca.'),
  },
};
