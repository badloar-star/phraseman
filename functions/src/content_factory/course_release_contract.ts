...
export interface CourseReleaseArtifact {
...
export function validateCourseRelease(value: unknown): { ok: boolean; errors: string[] } {
  const errors: string[] = [];
  if (!isRecord(value)) return { ok: false, errors: ['release_required'] };
  ...
  if (!hasIsoDate(value.createdAt)) errors.push('created_at_invalid');
...
export function assertCourseRelease(value: unknown): CourseRelease {
  const result = validateCourseRelease(value);
  if (!result.ok) throw new Error(`course_release_invalid:${result.errors.join(',')}`);
