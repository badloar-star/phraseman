...
describe('admin permission matrix', () => {
    ...
    it('keeps the owner permission list in sync with the AdminPermission type', () => {
        ...
        Object.keys(exhaustive).forEach((permission) => {
            expect((0, permissions_1.hasPermission)('owner', permission)).toBe(true);
        });
    });
    ...
    it('restricts auth repair and personal messaging to owner/admin only', () => {
        for (const role of ['owner', 'admin']) {
            expect((0, permissions_1.hasPermission)(role, 'users.auth_repair')).toBe(true);
            expect((0, permissions_1.hasPermission)(role, 'users.message.write')).toBe(true);
        }
        // support видит юзеров, но НЕ перепривязывает вход и не пишет им лично.
        for (const role of ['support', 'moderator', 'analyst', 'developer', 'content_editor']) {
            expect((0, permissions_1.hasPermission)(role, 'users.auth_repair')).toBe(false);
            expect((0, permissions_1.hasPermission)(role, 'users.message.write')).toBe(false);
        }
    });
    it('grants the ideas workflow to owner, admin and moderator only', () => {
        for (const role of ['owner', 'admin', 'moderator']) {
            expect((0, permissions_1.hasPermission)(role, 'ideas.read')).toBe(true);
            expect((0, permissions_1.hasPermission)(role, 'ideas.decide')).toBe(true);
        }
        for (const role of ['support', 'analyst', 'developer', 'content_editor']) {
            expect((0, permissions_1.hasPermission)(role, 'ideas.read')).toBe(false);
            expect((0, permissions_1.hasPermission)(role, 'ideas.decide')).toBe(false);
        }
    });
});
describe('claimed admin permissions', () => {
    it('allows money analytics only to claimed roles with money.read', () => {
        ...
        expect((0, permissions_1.hasClaimedPermission)({ admin: true }, 'money.read')).toBe(true);
        expect((0, permissions_1.hasClaimedPermission)({}, 'money.read')).toBe(false);
        expect((0, permissions_1.hasClaimedPermission)(null, 'money.read')).toBe(false);
    });
});
