...
import {
  ...
  LIVE_THEMES,
  NON_THEME_ASSET_PREFIXES,
  RETIRED_RASTER_PREFIXES,
  SOURCE_EXTENSIONS,
  SOURCE_ROOTS,
} from "./theme-asset-config.mjs";

const STATIC_REQUIRE_PATTERN =
  /require\(\s*['"](?<assetPath>[^'"]*assets[\\/]images[\\/][^'"]+)['"]\s*\)/g;
const THEME_REQUIRE_PATTERN =
  /(?:['"])?(?<theme>[A-Za-z][A-Za-z0-9]*)(?:['"])?\s*:\s*require\(\s*['"](?<assetPath>[^'"]*assets[\\/]images[\\/][^'"]+)['"]\s*\)/g;
const BRANCH_THEME_PATTERN =
  /if\s*\(\s*themeMode\s*===\s*['"](?<theme>[^'"]+)['"]\s*\)/;
const BRANCH_REQUIRE_PATTERN =
  /(?<slot>[A-Za-z][A-Za-z0-9_]*)\s*:\s*require\(\s*['"](?<assetPath>[^'"]*assets[\\/]images[\\/][^'"]+)['"]\s*\)/g;
  ...
  return segments
    .filter((segment) => {
      return !ALL_THEME_TOKENS.some((theme) => {
        ...
        return (
          lowerSegment === lowerTheme ||
          lowerSegment.startsWith(`${lowerTheme}_`)
        );
      });
    })
    .join("/");
...
function slotIdFromAssetPath(assetPath, explicitSlot = null) {
  ...
  const normalizedSegments = segments.filter((segment) => {
    return !ALL_THEME_TOKENS.some((theme) => {
      ...
      const lowerTheme = theme.toLowerCase();
      return (
        lowerSegment === lowerTheme || lowerSegment.startsWith(`${lowerTheme}_`)
      );
    });
  });
  ...
  const baseName =
    stripThemeToken(fileName.slice(0, -extension.length)) || "asset";
...
function collectNestedThemeEntries(source) {
  ...
  const nestedRequirePattern =
    /(?:['"])?(?<slot>[A-Za-z0-9_-]+)(?:['"])?\s*:\s*require\(\s*['"](?<assetPath>[^'"]*assets[\\/]images[\\/][^'"]+)['"]\s*\)/g;

  for (const outerMatch of source.matchAll(outerPattern)) {
    const openBraceIndex = source.indexOf(
      "{",
      outerMatch.index + outerMatch[0].length - 1,
    );
    if (openBraceIndex < 0) continue;
    const closeBraceIndex = findMatchingBrace(source, openBraceIndex);
    if (closeBraceIndex < 0) continue;
    const block = source.slice(openBraceIndex + 1, closeBraceIndex);
    const innerMatches = [...block.matchAll(nestedRequirePattern)];
    const slotCounts = new Map();
    for (const innerMatch of innerMatches) {
      const slot = innerMatch.groups.slot;
    ...
    for (const innerMatch of innerMatches) {
      entries.push({
        ...
        explicitSlot: slotCounts.get(innerMatch.groups.slot) === 1 ? innerMatch.groups.slot : null,
      });
    }
  }
  return entries;
...
export function auditThemeAssets({ rootDir, sourceRoots = SOURCE_ROOTS }) {
  ...
  for (const sourceFile of sourceFiles) {
    ...
    const relativeSource = path
      .relative(resolvedRoot, sourceFile)
      .replace(/\\/g, "/");
    ...
    for (const match of source.matchAll(STATIC_REQUIRE_PATTERN)) {
      ...
      if (
        RETIRED_RASTER_PREFIXES.some((prefix) => assetPath.startsWith(prefix))
      ) {
        addViolation(violations, {
    ...
    for (const entry of collectThemeEntries(source)) {
      if (NON_THEME_ASSET_PREFIXES.some((prefix) => entry.assetPath.startsWith(prefix))) continue;
  ...
  return {
    ...
    slots: Object.fromEntries(
      Object.entries(slots).sort(([left], [right]) =>
...
function markdownSummary(report) {
  ...
  for (const violation of report.violations) {
    lines.push(
      `- \`${violation.code}\`: ${violation.path ?? violation.slotId ?? "unknown"}`,
    );
  }
  lines.push("");
  return lines.join("\n");
