import {
  MAP_RADIUS,
  STAR_COUNT,
  allMapHexes,
  connectedGroups,
  generateMap,
  hexDistance,
  hexKey,
  hexNeighbors,
  isOnMap,
  neighborsInMap,
  parseHexKey,
  ringOf,
  validateMapSymmetry,
} from './hex';

describe('constellations/hex — базовая математика', () => {
  test('карта радиуса 3 содержит ровно 37 звёзд', () => {
    expect(MAP_RADIUS).toBe(3);
    expect(allMapHexes()).toHaveLength(STAR_COUNT);
    expect(STAR_COUNT).toBe(37);
  });

  test('кольца: полярная 1 / внутреннее 6 / среднее 12 / внешнее 18', () => {
    const byRing = { polar: 0, inner: 0, middle: 0, outer: 0 };
    for (const h of allMapHexes()) byRing[ringOf(h)] += 1;
    expect(byRing).toEqual({ polar: 1, inner: 6, middle: 12, outer: 18 });
  });

  test('ringOf бросает для клетки вне карты', () => {
    expect(() => ringOf({ q: 4, r: 0 })).toThrow();
  });

  test('hexKey/parseHexKey — обратимы, мусор отклоняется', () => {
    const h = { q: -2, r: 3 };
    expect(parseHexKey(hexKey(h))).toEqual(h);
    expect(() => parseHexKey('garbage')).toThrow();
    expect(() => parseHexKey('1,NaN')).toThrow();
  });

  test('дистанция: симметрична, 0 к себе, диаметр карты = 6', () => {
    const a = { q: 3, r: 0 };
    const b = { q: -3, r: 0 };
    expect(hexDistance(a, a)).toBe(0);
    expect(hexDistance(a, b)).toBe(hexDistance(b, a));
    expect(hexDistance(a, b)).toBe(6);
  });

  test('соседи: всегда 6 сырых; в карте у угла внешнего кольца только 3', () => {
    expect(hexNeighbors({ q: 0, r: 0 })).toHaveLength(6);
    const cornerNeighbors = neighborsInMap({ q: 3, r: 0 });
    expect(cornerNeighbors).toHaveLength(3);
    for (const n of cornerNeighbors) expect(isOnMap(n)).toBe(true);
  });
});

describe('constellations/hex — генерация карты от сида', () => {
  test('детерминизм: один сид → одна карта', () => {
    const a = generateMap('match-123');
    const b = generateMap('match-123');
    expect(a).toEqual(b);
  });

  test('разные сиды дают разные раскладки хотя бы иногда', () => {
    const homes = new Set<string>();
    for (let i = 0; i < 20; i += 1) homes.add(generateMap(`seed-${i}`).homes.join('|'));
    expect(homes.size).toBeGreaterThan(1);
  });

  test('родные звёзды: 4 разных, все на внешнем кольце', () => {
    const map = generateMap('any-seed');
    expect(new Set(map.homes).size).toBe(4);
    for (const key of map.homes) {
      expect(ringOf(parseHexKey(key))).toBe('outer');
    }
  });

  test('симметрия: каждый игрок эквидистантен центру и имеет одинаковый набор дистанций до врагов {3,6,6}', () => {
    const map = generateMap('sym-check');
    expect(validateMapSymmetry(map)).toBe(true);
    for (const key of map.homes) {
      const home = parseHexKey(key);
      expect(hexDistance(home, { q: 0, r: 0 })).toBe(MAP_RADIUS);
      const dists = map.homes
        .filter((k) => k !== key)
        .map((k) => hexDistance(home, parseHexKey(k)))
        .sort((x, y) => x - y);
      expect(dists).toEqual([3, 6, 6]);
    }
  });

  test('validateMapSymmetry ловит битые раскладки', () => {
    expect(validateMapSymmetry({ seed: 'x', homes: ['0,0', '3,0', '-3,0', '0,3'] })).toBe(false); // дом в центре
    expect(validateMapSymmetry({ seed: 'x', homes: ['3,0', '3,0', '-3,0', '0,3'] })).toBe(false); // дубль
    expect(validateMapSymmetry({ seed: 'x', homes: ['3,0', '-3,0'] })).toBe(false); // не 4 дома
    expect(validateMapSymmetry({ seed: 'x', homes: ['3,0', '2,-3', '-3,0', '-2,3'] })).toBe(false); // кривые дистанции
  });
});

describe('constellations/hex — связные группы (созвездия)', () => {
  test('пустой список → нет групп', () => {
    expect(connectedGroups([])).toEqual([]);
  });

  test('3 смежные звезды образуют одну группу, отдельная звезда — свою', () => {
    const groups = connectedGroups(['0,0', '1,0', '1,-1', '3,0']);
    const sizes = groups.map((g) => g.length).sort((a, b) => a - b);
    expect(sizes).toEqual([1, 3]);
  });

  test('диагональные (несмежные) звезды не склеиваются', () => {
    const groups = connectedGroups(['0,0', '2,0']);
    expect(groups).toHaveLength(2);
  });
});
