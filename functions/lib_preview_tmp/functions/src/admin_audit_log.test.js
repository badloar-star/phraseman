...
const admin_audit_log_1 = require("./admin_audit_log");
function cursorFor(id, timestampMs) {
    return Buffer.from(JSON.stringify({ id, timestampMs }), 'utf8').toString('base64url');
}
function fakeAuditDb(rows) {
    const byId = new Map(rows.map((row) => [row.id, row]));
    class Query {
        constructor(field, whereOp, whereValue, startAfterId, limitCount) {
            this.field = field;
    ...
    return { collection: () => new Query() };
}
describe('admin audit log list contract', () => {
    test('bounds filters and rejects unsafe cursor combinations', () => {
        expect((0, admin_audit_log_1.parseAuditListRequest)({ limit: 9999, sinceDays: 90, action: ' report.status.update ', query: ' user@example.com ' })).toMatchObject({
            limit: 100,
            sinceDays: 90,
        ...
        expect(() => (0, admin_audit_log_1.parseAuditListRequest)({ cursor: 'abc', action: 'x' })).toThrow('cursor cannot be combined with audit filters');
        expect(() => (0, admin_audit_log_1.parseAuditListRequest)({ cursor: 'abc', query: 'x' })).toThrow('cursor cannot be combined with audit filters');
        expect((0, admin_audit_log_1.parseAuditListRequest)({ cursor: 'abc', sinceDays: 30 })).toMatchObject({ cursor: 'abc', sinceDays: 30 });
        expect(() => (0, admin_audit_log_1.parseAuditListRequest)({ cursor: '../../bad' })).toThrow('cursor is invalid');
    });
    test('projects bounded read-only rows without leaking message body fields', () => {
        expect((0, admin_audit_log_1.projectAuditRow)('audit-1', {
    ...
    test('merges mixed timestamp schemas without dropping structured audit records', () => {
        const input = (0, admin_audit_log_1.parseAuditListRequest)({ limit: 10, sinceDays: 90 });
        const rows = (0, admin_audit_log_1.mergeAuditRowsForList)([
        ...
        const nextRows = (0, admin_audit_log_1.mergeAuditRowsForList)(rows, input, Date.parse('2033-05-01T00:00:00.000Z'), { id: 'structured-timestamp', timestampMs: 2000034000000 });
    ...
    test('collects the next page through a per-stream cursor instead of rereading the first page', async () => {
        const rows = Array.from({ length: 250 }, (_, index) => {
            ...
            const ts = new Date(Date.UTC(2033, 4, 18, 12, 0, 0) + (250 - index) * 60000).toISOString();
            return { id: `ts-${sequence}`, ts, action: 'grant_reward' };
        });
        const firstPageInput = (0, admin_audit_log_1.parseAuditListRequest)({ limit: 100, sinceDays: 90 });
        const firstRaw = await (0, admin_audit_log_1.collectAuditRawRows)(fakeAuditDb(rows), firstPageInput, 101);
        expect(firstRaw.saturated).toBe(true);
        const firstPage = (0, admin_audit_log_1.mergeAuditRowsForList)(firstRaw.rows, firstPageInput, 0, null).slice(0, 100);
        expect(firstPage).toHaveLength(100);
        const last = firstPage[firstPage.length - 1];
        const secondPageInput = (0, admin_audit_log_1.parseAuditListRequest)({ limit: 100, sinceDays: 90, cursor: cursorFor(String(last.id), Number(last.timestampMs)) });
        const secondRaw = await (0, admin_audit_log_1.collectAuditRawRows)(fakeAuditDb(rows), secondPageInput, 101);
        const secondPage = (0, admin_audit_log_1.mergeAuditRowsForList)(secondRaw.rows, secondPageInput, 0, { id: String(last.id), timestampMs: Number(last.timestampMs) }).slice(0, 100);
    ...
    test('keeps equal-timestamp rows from another timestamp schema after a cursor', async () => {
        ...
        const input = (0, admin_audit_log_1.parseAuditListRequest)({ limit: 10, sinceDays: 90, cursor: cursorFor('ts-a', Date.parse(sameTimestamp)) });
        const raw = await (0, admin_audit_log_1.collectAuditRawRows)(fakeAuditDb([
            { id: 'ts-a', ts: sameTimestamp, action: 'grant_reward' },
            { id: 'zz-timestamp', timestamp: sameTimestamp, action: 'remote_config.publish' },
        ]), input, 10);
        const rows = (0, admin_audit_log_1.mergeAuditRowsForList)(raw.rows, input, 0, { id: 'ts-a', timestampMs: Date.parse(sameTimestamp) });
    ...
    test('audit source health reports partial timestamp-index failures instead of always ready', () => {
        expect((0, admin_audit_log_1.summarizeAuditSourceHealth)({
            rows: [{ id: 'a1' }],
            scanned: 2,
            saturated: false,
            health: [
...
//# sourceMappingURL=admin_audit_log.test.js.map