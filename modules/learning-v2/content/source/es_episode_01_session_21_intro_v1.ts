...
const CONCEPT_BODY = L({
  ru: 'Libro называет предмет — книгу — и у этого слова есть свой род, мужской, точно так же, как у прилагательных caro или bonito уже есть свои формы. Артикль el стоит перед libro всегда, потому что род книги не зависит от того, кто о ней говорит: El libro es caro — Книга дорогая. Этот род не связан с полом — книга не «мужчина», просто испанское слово libro исторически мужского рода, и это нужно запомнить вместе со словом. Ответ прост: у каждого испанского предмета есть свой род, и его показывает артикль el или la рядом со словом.',
  uk: 'Libro називає предмет — книгу — і в цього слова є свій рід, чоловічий, точнісінько як у прикметників caro чи bonito вже є свої форми. Артикль el стоїть перед libro завжди, бо рід книги не залежить від того, хто про неї говорить: El libro es caro — Книга дорога. Цей рід не пов’язаний зі статтю — книга не «чоловік», просто іспанське слово libro історично чоловічого роду, і це треба запам’ятати разом зі словом. Відповідь проста: у кожного іспанського предмета є свій рід, і його показує артикль el або la поряд зі словом.',
  es: 'Libro names a thing — a book — and this word has its own gender, masculine, just as the adjectives caro or bonito already have their own forms. The article el stands before libro always, because the book\'s gender does not depend on who talks about it: El libro es caro — The book is expensive. This gender has nothing to do with sex — a book is not "male," the Spanish word libro is simply historically masculine, and this must be learned along with the word. The answer is simple: every Spanish thing has its own gender, shown by the article el or la next to the word.',
  'pt-BR': 'Libro nomeia uma coisa — um livro — e essa palavra tem seu próprio gênero, masculino, assim como os adjetivos caro ou bonito já têm suas próprias formas. O artigo el fica antes de libro sempre, porque o gênero do livro não depende de quem fala dele: El libro es caro — O livro é caro. Esse gênero não tem relação com sexo — um livro não é "homem," a palavra espanhola libro é simplesmente historicamente masculina, e isso precisa ser aprendido junto com a palavra. A resposta é simples: cada coisa em espanhol tem seu próprio gênero, mostrado pelo artigo el ou la ao lado da palavra.',
  vi: 'Libro gọi tên một vật — một cuốn sách — và từ này có giống riêng, giống đực, y hệt như các tính từ caro hay bonito đã có dạng riêng của chúng. Mạo từ el luôn đứng trước libro, vì giống của cuốn sách không phụ thuộc vào ai đang nói về nó: El libro es caro — Cuốn sách đắt. Giống này không liên quan đến giới tính — cuốn sách không phải là "đàn ông," từ tiếng Tây Ban Nha libro đơn giản là về mặt lịch sử thuộc giống đực, và điều này cần được học cùng với từ đó. Câu trả lời rất đơn giản: mỗi vật trong tiếng Tây Ban Nha có giống riêng, được thể hiện bằng mạo từ el hoặc la bên cạnh từ đó.',
  id: 'Libro menyebut suatu benda — sebuah buku — dan kata ini punya gendernya sendiri, maskulin, persis seperti kata sifat caro atau bonito yang sudah punya bentuknya sendiri. Artikel el selalu berdiri sebelum libro, karena gender buku tidak bergantung pada siapa yang membicarakannya: El libro es caro — Bukunya mahal. Gender ini tidak ada hubungannya dengan jenis kelamin — buku bukan "laki-laki," kata Spanyol libro secara historis memang maskulin, dan ini harus dihafal bersama katanya. Jawabannya sederhana: setiap benda dalam bahasa Spanyol punya gendernya sendiri, ditunjukkan oleh artikel el atau la di samping kata itu.',
  tr: 'Libro bir şeyi — bir kitabı — adlandırır ve bu kelimenin kendi cinsiyeti vardır, eril, tıpkı caro ya da bonito sıfatlarının zaten kendi biçimleri olduğu gibi. El tanımlığı libro’dan önce her zaman durur, çünkü kitabın cinsiyeti kim ondan bahsettiğine bağlı değildir: El libro es caro — Kitap pahalı. Bu cinsiyetin cinsiyetle (biyolojik) ilgisi yoktur — kitap "erkek" değildir, İspanyolca libro kelimesi sadece tarihsel olarak erildir, ve bu kelimeyle birlikte ezberlenmelidir. Cevap basittir: her İspanyolca şeyin kendi cinsiyeti vardır ve bunu kelimenin yanındaki el ya da la tanımlığı gösterir.',
  pl: 'Libro nazywa rzecz — książkę — i to słowo ma własny rodzaj, męski, dokładnie tak jak przymiotniki caro czy bonito mają już swoje formy. Rodzajnik el stoi przed libro zawsze, bo rodzaj książki nie zależy od tego, kto o niej mówi: El libro es caro — Książka jest droga. Ten rodzaj nie ma nic wspólnego z płcią — książka nie jest „mężczyzną”, hiszpańskie słowo libro jest po prostu historycznie rodzaju męskiego, i trzeba to zapamiętać razem ze słowem. Odpowiedź jest prosta: każda hiszpańska rzecz ma swój rodzaj, pokazywany przez rodzajnik el lub la obok słowa.',
});

const FORMULA_BODY = L({
  ru: 'Формула та же, что и для caro/cara или bonito/bonita: концовка -o для мужского рода, концовка -a для женского. У libro концовка -o, и это фиксировано навсегда — libro никогда не станет libra, в отличие от прилагательного caro, которое умеет меняться на cara. Артикль перед словом всегда совпадает с этим родом: el libro, никогда la libro. А признак, который описывает libro, тоже берёт форму на -o, потому что подстраивается под род предмета, а не наоборот: El libro es caro, El libro es bonito. Ответ прост: артикль и признак меняются под род существительного, а само существительное libro — нет.',
  uk: 'Формула та сама, що й для caro/cara чи bonito/bonita: закінчення -o для чоловічого роду, закінчення -a для жіночого. У libro закінчення -o, і воно зафіксоване назавжди — libro ніколи не стане libra, на відміну від прикметника caro, який уміє змінюватися на cara. Артикль перед словом завжди збігається з цим родом: el libro, ніколи la libro. А ознака, що описує libro, теж бере форму на -o, бо підлаштовується під рід предмета, а не навпаки: El libro es caro, El libro es bonito. Відповідь проста: артикль і ознака змінюються під рід іменника, а сам іменник libro — ні.',
  es: 'The formula is the same as for caro/cara or bonito/bonita: the ending -o for masculine, the ending -a for feminine. Libro has the ending -o, and it is fixed forever — libro will never become libra, unlike the adjective caro, which can change to cara. The article before the word always matches this gender: el libro, never la libro. And the quality describing libro also takes the -o form, because it adapts to the thing\'s gender, not the other way around: El libro es caro, El libro es bonito. The answer is simple: the article and the quality change to match the noun\'s gender, but the noun libro itself does not.',
  'pt-BR': 'A fórmula é a mesma de caro/cara ou bonito/bonita: a terminação -o para masculino, a terminação -a para feminino. Libro tem a terminação -o, e ela é fixa para sempre — libro nunca vira libra, diferente do adjetivo caro, que consegue virar cara. O artigo antes da palavra sempre corresponde a esse gênero: el libro, nunca la libro. E a qualidade que descreve libro também toma a forma -o, porque se adapta ao gênero da coisa, não o contrário: El libro es caro, El libro es bonito. A resposta é simples: o artigo e a qualidade mudam para corresponder ao gênero do substantivo, mas o substantivo libro em si não muda.',
  vi: 'Công thức giống như caro/cara hay bonito/bonita: đuôi -o cho giống đực, đuôi -a cho giống cái. Libro có đuôi -o, và nó cố định mãi mãi — libro sẽ không bao giờ trở thành libra, khác với tính từ caro, có thể đổi thành cara. Mạo từ trước từ luôn khớp với giống này: el libro, không bao giờ la libro. Và đặc điểm mô tả libro cũng lấy dạng -o, vì nó thích nghi theo giống của vật, chứ không phải ngược lại: El libro es caro, El libro es bonito. Câu trả lời rất đơn giản: mạo từ và đặc điểm thay đổi để khớp với giống của danh từ, nhưng bản thân danh từ libro thì không.',
  id: 'Rumusnya sama seperti caro/cara atau bonito/bonita: akhiran -o untuk maskulin, akhiran -a untuk feminin. Libro memiliki akhiran -o, dan itu tetap selamanya — libro tidak akan pernah menjadi libra, berbeda dengan kata sifat caro, yang bisa berubah menjadi cara. Artikel sebelum kata selalu cocok dengan gender ini: el libro, tidak pernah la libro. Dan sifat yang menggambarkan libro juga mengambil bentuk -o, karena menyesuaikan dengan gender bendanya, bukan sebaliknya: El libro es caro, El libro es bonito. Jawabannya sederhana: artikel dan sifat berubah mengikuti gender kata benda, tapi kata benda libro itu sendiri tidak.',
  tr: 'Formül caro/cara ya da bonito/bonita ile aynıdır: eril için -o son eki, dişil için -a son eki. Libro’nun -o son eki vardır, ve bu sonsuza dek sabittir — libro asla libra olmaz, cara’ya dönüşebilen caro sıfatının aksine. Kelimeden önceki tanımlık her zaman bu cinsiyetle eşleşir: el libro, asla la libro değil. Ve libro’yu tanımlayan nitelik de -o biçimini alır, çünkü tersi değil, şeyin cinsiyetine uyum sağlar: El libro es caro, El libro es bonito. Cevap basittir: tanımlık ve nitelik ismin cinsiyetine göre değişir, ama libro isminin kendisi değişmez.',
  pl: 'Formuła jest taka sama jak dla caro/cara czy bonito/bonita: końcówka -o dla rodzaju męskiego, końcówka -a dla żeńskiego. Libro ma końcówkę -o, i jest ona ustalona na zawsze — libro nigdy nie stanie się libra, w przeciwieństwie do przymiotnika caro, który potrafi zmienić się w cara. Rodzajnik przed słowem zawsze pasuje do tego rodzaju: el libro, nigdy la libro. A cecha opisująca libro też przyjmuje formę -o, bo dostosowuje się do rodzaju rzeczy, a nie odwrotnie: El libro es caro, El libro es bonito. Odpowiedź jest prosta: rodzajnik i cecha zmieniają się zgodnie z rodzajem rzeczownika, ale sam rzeczownik libro się nie zmienia.',
});

const TRAP_BODY = L({
  ru: 'Легко ошибиться и сказать la libro, думая, что -o всегда значит мужской род. Это верно только для libro, а не всеобщее правило: род запоминают вместе со словом. Вторая ловушка — согласовать признак с полом говорящего, а не с родом предмета: если о книге говорит женщина, признак всё равно остаётся caro, а не cara, потому что род принадлежит libro, а не человеку, который о нём говорит. Проверка простая: libro — всегда libro, el — перед ним всегда, а признак берёт -o, потому что таков род книги.',
  uk: 'Легко помилитися і сказати la libro, думаючи, що -o завжди означає чоловічий рід. Це вірно лише для libro, а не загальне правило: рід запам’ятовують разом зі словом. Друга пастка — узгодити ознаку зі статтю мовця, а не з родом предмета: якщо про книгу говорить жінка, ознака все одно лишається caro, а не cara, бо рід належить libro, а не людині, яка про нього говорить. Перевірка проста: libro — завжди libro, el — перед ним завжди, а ознака бере -o, бо такий рід книги.',
  es: 'It is easy to make the mistake of saying la libro, thinking that -o always means masculine. This holds only for libro, not as a universal rule: gender is memorized along with the word. The second trap is agreeing the quality with the speaker\'s sex instead of the thing\'s gender: if a woman is talking about the book, the quality still stays caro, not cara, because the gender belongs to libro, not to the person talking about it. The check is simple: libro is always libro, el always stands before it, and the quality takes -o because that is the book\'s gender.',
  'pt-BR': 'É fácil errar e dizer la libro, pensando que -o sempre significa masculino. Isso vale só para libro, não como regra universal: o gênero se memoriza junto com a palavra. A segunda armadilha é concordar a qualidade com o sexo de quem fala em vez do gênero da coisa: se uma mulher fala sobre o livro, a qualidade continua caro, não cara, porque o gênero pertence a libro, não à pessoa que fala dele. A checagem é simples: libro é sempre libro, el sempre fica antes dele, e a qualidade toma -o porque esse é o gênero do livro.',
  vi: 'Dễ mắc lỗi nói la libro, nghĩ rằng -o luôn có nghĩa là giống đực. Điều này chỉ đúng với libro, không phải quy tắc chung: giống được ghi nhớ cùng với từ. Cái bẫy thứ hai là hòa hợp đặc điểm theo giới tính của người nói thay vì giống của vật: nếu một phụ nữ nói về cuốn sách, đặc điểm vẫn là caro, không phải cara, vì giống thuộc về libro, không phải người nói về nó. Cách kiểm tra đơn giản: libro luôn là libro, el luôn đứng trước nó, và đặc điểm lấy -o vì đó là giống của cuốn sách.',
  id: 'Mudah salah mengatakan la libro, mengira bahwa -o selalu berarti maskulin. Ini hanya berlaku untuk libro, bukan aturan umum: gender dihafal bersama katanya. Jebakan kedua adalah menyesuaikan sifat dengan jenis kelamin pembicara, bukan gender bendanya: jika seorang wanita berbicara tentang buku, sifatnya tetap caro, bukan cara, karena gendernya milik libro, bukan orang yang membicarakannya. Pengecekannya sederhana: libro selalu libro, el selalu berdiri sebelumnya, dan sifatnya mengambil -o karena itulah gender buku.',
  tr: 'Kelime -o ile bittiği için her zaman eril olduğunu düşünüp la libro demek kolay bir hatadır. Bu yalnızca libro için geçerlidir, evrensel bir kural değil: cinsiyet kelimeyle birlikte ezberlenir. İkinci tuzak, niteliği şeyin cinsiyeti yerine konuşanın cinsiyetiyle uyumlu hale getirmektir: eğer bir kadın kitap hakkında konuşuyorsa, nitelik yine de caro kalır, cara değil, çünkü cinsiyet libro’ya aittir, ondan bahseden kişiye değil. Kontrol basittir: libro her zaman libro’dur, el her zaman ondan önce durur, ve nitelik -o alır çünkü bu kitabın cinsiyetidir.',
  pl: 'Łatwo popełnić błąd i powiedzieć la libro, myśląc, że -o zawsze oznacza rodzaj męski. To prawda tylko dla libro, nie jako uniwersalna zasada: rodzaj zapamiętuje się razem ze słowem. Druga pułapka to dopasowanie cechy do płci mówiącego zamiast do rodzaju rzeczy: jeśli o książce mówi kobieta, cecha nadal zostaje caro, nie cara, bo rodzaj należy do libro, nie do osoby, która o niej mówi. Sprawdzenie jest proste: libro to zawsze libro, el zawsze stoi przed nim, a cecha przyjmuje -o, bo taki jest rodzaj książki.',
});
