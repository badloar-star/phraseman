...
describe('challenge studio artifacts', () => {
  const topic = (stage: 'challenge_topic') => ({ stage, result: { topicId: 'travel-basics', title: 'Travel basics', learningPromise: 'Choose natural phrases for common travel situations.', skillTags: ['travel', 'requests'], inclusions: ['transport', 'directions'], exclusions: ['rare aviation terms'], difficultyDistribution: { easy: 3, medium: 4, hard: 3 } } });

  it.each(['challenge_topic'] as const)('accepts an editable %s proposal', (stage) => {
  ...
  it.each(['challenge_questions'] as const)('accepts exactly ten fully explained %s', (stage) => {
    const grounding = { topic: topic('challenge_topic').result };
  ...
  it('rejects duplicate choices, bad correct index, missing per-option explanations and repeated templates', () => {
    ...
    const grounding = { topic: topic('challenge_topic').result };
    expect(validateQuestionBatchArtifact({ stage: 'challenge_questions', items }, { kind: 'challenge_questions', count: 10, grounding })).toEqual(expect.arrayContaining(['question_choices_unique', 'question_correct_index_invalid', 'question_option_explanations_expected_4', 'question_semantic_duplicate', 'question_skill_tag_unapproved']));
  });
  ...
  it('rejects a distractor that is a truncated fragment of the correct answer', () => {
    ...
    const grounding = { topic: topic('challenge_topic').result };
    expect(validateQuestionBatchArtifact({ stage: 'challenge_questions', items }, { kind: 'challenge_questions', count: 10, grounding }))
  ...
  it('rejects a truncation conflict in a single-question replacement too', () => {
    ...
    const grounding = { replacementForQuestionId: 'q-1', originalQuestion: original, topic: topic('challenge_topic').result, previousQuestionKeys: [] };
    expect(validateQuestionReplacementArtifact({ stage: 'challenge_question_replacement', result: { replacementForQuestionId: 'q-1', item: replacement } }, { kind: 'challenge_question_replacement', grounding }))
  ...
  it('accepts one independently grounded replacement and rejects a semantic clone', () => {
    ...
    const grounding = { replacementForQuestionId: 'q-1', originalQuestion: original, topic: topic('challenge_topic').result, previousQuestionKeys: [] };
    expect(validateQuestionReplacementArtifact({ stage: 'challenge_question_replacement', result: { replacementForQuestionId: 'q-1', item: replacement } }, { kind: 'challenge_question_replacement', grounding })).toEqual([]);
    expect(validateQuestionReplacementArtifact({ stage: 'challenge_question_replacement', result: { replacementForQuestionId: 'q-1', item: { ...original, id: 'clone' } } }, { kind: 'challenge_question_replacement', grounding })).toContain('question_replacement_duplicate');
  });
});
