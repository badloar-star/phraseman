...
function giftLabelForPushLang(gift, lang) {
    switch (lang) {
...
function giftPushBody(lang, senderName, giftLabel) {
    switch (lang) {
...
function parseProgressInt(value) {
    const n = typeof value === 'number' ? value : Number(value);
    return Number.isFinite(n) && n > 0 ? Math.floor(n) : 0;
}
...
function userMatchesAuth(stableId, data, authUid) {
...
function questIdFor(senderStableId, friendStableId, weekKey) {
...
function isActiveQuestMeta(data, now) {
...
exports.friendSendGift = (0, https_1.onCall)({ region: REGION, enforceAppCheck: callable_options_1.ENFORCE_APP_CHECK }, async (request) => {
    ...
    const linkedSender = await (0, auth_identity_1.ensureStableLinkForAuth)(db, request.auth.uid, senderStableId, signInProvider);
    if (linkedSender.stableUid !== senderStableId) {
        throw new https_1.HttpsError('failed-precondition', 'sender_stable_id_changed');
    }
    ...
    const result = await db.runTransaction(async (tx) => {
        ...
        tx.set((0, user_notifications_1.userNotificationRef)(db, friendStableId, `gift_${sentGiftRef.id}`), (0, user_notifications_1.buildUserNotification)({
            type: 'friend_gift_received',
            fromUid: senderStableId,
            fromName: senderName,
            text: gift.labelRu,
...
function buildQuestStatus(quest, userDataByUid) {
    const participantUids = Array.isArray(quest.participantUids)
        ? quest.participantUids.map((uid) => String(uid)).filter(Boolean)
        : [];
...
const FRIEND_GET_ACTIVE_QUEST_SERVER_CACHE_TTL_MS = 30000;
const friendGetActiveQuestServerCache = new Map();
function cacheFriendGetActiveQuest(stableId, data) {
    const now = Date.now();
...
exports.friendGetActiveQuest = (0, https_1.onCall)({ region: REGION, enforceAppCheck: callable_options_1.ENFORCE_APP_CHECK }, async (request) => {
    ...
    return db.runTransaction(async (tx) => {
        ...
        if (!userMatchesAuth(stableId, userSnap.data(), request.auth.uid)) {
            throw new https_1.HttpsError('permission-denied', 'User does not match auth');
        }
        if (!currentSnap.exists) {
            const data = { ok: true, quest: null };
        ...
        if (!participantUids.includes(stableId)) {
            throw new https_1.HttpsError('permission-denied', 'Not a quest participant');
        }
        ...
        if (expiresAtMs > 0 && expiresAtMs <= now && quest.status === 'active') {
            ...
            const data = { ok: true, quest: { ...buildQuestStatus({ ...quest, status: 'expired' }, { [stableId]: userSnap.data(), [otherUid]: otherSnap?.data() }), status: 'expired' } };
            cacheFriendGetActiveQuest(stableId, data);
            if (otherUid)
                cacheFriendGetActiveQuest(otherUid, data);
            return data;
        }
        const data = {
            ok: true,
...
exports.friendClaimQuestReward = (0, https_1.onCall)({ region: REGION, enforceAppCheck: callable_options_1.ENFORCE_APP_CHECK }, async (request) => {
    ...
    if (!stableId || !questId) {
        throw new https_1.HttpsError('invalid-argument', 'stableId and questId required');
    }
    const db = admin.firestore();
    const questRef = db.collection('friend_quests').doc(questId);
    const now = Date.now();
...
exports.friendThankGift = (0, https_1.onCall)({ region: REGION, enforceAppCheck: callable_options_1.ENFORCE_APP_CHECK }, async (request) => {
    ...
    const result = await db.runTransaction(async (tx) => {
        ...
        tx.set((0, user_notifications_1.userNotificationRef)(db, friendStableId, `gift_thanks_${senderStableId}_${idempotencyKey || now}`), (0, user_notifications_1.buildUserNotification)({
            type: 'friend_gift_thanks',
            fromUid: senderStableId,
            fromName: senderName,
            text: gift.labelRu,
