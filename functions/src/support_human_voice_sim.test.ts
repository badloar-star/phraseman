...
describe('Симулятор: как Джарвис отвечает живым людям', () => {
  ...
  test('болтовня без утверждений о продукте доходит до настоящего ответа', () => {
    const chatty = [
      ['Спасибо', 'Спасибо большое за тёплые слова! Очень приятно это слышать — такие письма правда поддерживают нас в работе. Рады, что у вас всё складывается хорошо. Если появятся вопросы или предложения, пишите нам сюда, всегда рады помочь и подсказать.'],
      ['Пожелание', 'Спасибо за идею! Мы собираем такие пожелания и обсуждаем их всей командой — именно так обычно и появляется что-то новое. Обещать сроки не станем, чтобы не подвести вас, но идея точно не потеряется. Если придумаете что-то ещё, пишите нам сюда.'],
    ] as const;
