...
describe("Tournament per-theme visual kits", () => {
  it("wires backdrop, podium, weekly-bank, and season-rewards assets for every selectable theme", () => {
    ...
    expect(source).toContain("Record<ThemeMode, TournamentThemeAssetKit>");
    expect(source).not.toMatch(
      /\b(?:minimalDark|candyBlue|business|businessLight)\s*:/,
    );
  ...
  it("keeps the round loading state themed and bundles every kit for first launch", () => {
    ...
    const playBackdrops =
      roundSource.match(/<TournamentBackdrop variant="play" \/>/g) ?? [];
  ...
  it("ships a compact, correctly sized WebP set with alpha on overlay slots", async () => {
    ...
    for (const theme of SELECTABLE_THEMES) {
      for (const slot of SLOTS) {
        const file = path.join(
          ROOT,
          "assets/images/tournament/themes",
          theme,
          `${slot}.webp`,
        );
        ...
        if (slot !== "backdrop") {
          ...
          for (const alphaIndex of corners)
            expect(data[alphaIndex]).toBeLessThanOrEqual(8);
        }
      }
    }
    ...
    const shippedWebps = fs
      .readdirSync(path.join(ROOT, "assets/images/tournament/themes"), {
        recursive: true,
      })
