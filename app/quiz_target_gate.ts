...
export function frenchQuizGateCopy(sourceLocale?: string) {
  ...
  return {
    title: uk ? 'Французькі вызовы ще готуються' : 'Французские вызовы ещё готовятся',
    body: uk
      ? 'Англійський банк вызовов приховано в режимі French. Вызовы відкриються тільки після окремого французького банку завдань, перевірених за джерелами, дистракторами й підказками українською/російською.'
