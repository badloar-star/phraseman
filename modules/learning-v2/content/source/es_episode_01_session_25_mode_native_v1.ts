...
// открывает Главу 4 "Мы и они"): испанская сессия 25 переписывается в
// mode-native формат, зеркаля КОД (не данные) сессий 17/19/20 — kind:
// 'phrases', новых слов нет. Все 15 фраз сохранены из легаси-черновика
// 2026-08-25 без изменений содержания — каждая учит somos (первое лицо
// множественного числа) через контраст с уже известными soy/eres/es,
// признаки así/de acuerdo не меняются ни по роду, ни по числу.
//
// зачем НЕ все practice-фразы идут через phrase_builder/listen_build_dictation
// (владелец, 2026-08-28, тот же класс находки, что и сессии 21/22): фразы
// этой сессии — двойные реплики (recall-реплика + somos-реакция), у 7 из 15
// (индексы 4,5,8,10,11,13,14) больше 8 уникальных дистракторов, что
// превышает предел responseFeedbackById в course_session_client_children_v1.ts.
// Эти 7 идут только через listen_choose/context_gap_grammar/speed_match.
// Индексы 3,6,7,9,12 (≤8 уникальных дистракторов) используют
// phrase_builder/listen_build_dictation.
function asciiId(value: string): string {
  return value.normalize('NFD').replace(/[̀-ͯ]/gu, '');
}

const phrases = ES_EPISODE_01_SESSION_25_PHRASES;
if (phrases.length !== 15) {
  throw new Error('es_session_25_mode_native_phrase_count_invalid');
}
...
// зачем 'en' вместо 'es' здесь (как в испанских сессиях 1-24): в испанском
...
function localizedPhraseField(
  ...
  if (!phrase?.localizedDetails) throw new Error(`es_session_25_phrase_details_missing:${phraseIndex}`);
  return Object.fromEntries(LEARNING_V2_INTERFACE_LOCALES.map((locale) => {
    ...
    if (!detail) throw new Error(`es_session_25_phrase_locale_missing:${phraseIndex}:${locale}`);
...
function localizedPhraseDistractorFeedback(
  ...
  return Object.fromEntries(LEARNING_V2_INTERFACE_LOCALES.map((locale) => {
    ...
    if (!reason) throw new Error(`es_session_25_phrase_distractor_missing:${phraseIndex}:${locale}:${value}`);
...
function phraseAudio(phraseIndex: number): LearningV2ModeAudioReferenceV1 {
  const phrase = phrases[phraseIndex]!;
  return authoredAudio(`es-e01-s25-phrase-${phraseIndex + 1}`, phrase.english);
}
function phraseSlowAudio(phraseIndex: number): LearningV2ModeAudioReferenceV1 {
  const phrase = phrases[phraseIndex]!;
  return authoredAudio(`es-e01-s25-phrase-${phraseIndex + 1}-slow`, phrase.english);
}
...
// 2026-08-28): эта сессия проверяет именно выбор связки (soy/eres/es/somos),
// а не последнее слово — именно связка несёт грамматическую тему сессии
// (сдвиг числа при сохранении лица). Дистракторы берутся из
// authored-словаря первого слова фразы.
function contextGapFirstWord(phraseIndex: number): LearningV2ModeNativePayloadV1 {
  ...
  return Object.freeze({
    ...
    testedDimension: `first_person_plural:${asciiId(phrase.id)}`,
    choiceFeedback: Object.freeze([
      ...
      ...firstWord.distractors.map((entry) => feedback(
        ...
        false,
        `first_person_plural:${asciiId(entry.reasonCode)}`,
...
// 2026-08-28, тот же приём, что сессии 11/17): широкий срез уже
// отработанных somos-фраз, а не смежный блок — индексы 3/6/8/10/13.
function speedMatchPhrases(pairIndices: readonly number[]): LearningV2ModeNativePayloadV1 {
  ...
  const ids = pairIndices.map((index) => `es-e01-s25-pair-${index + 1}`);
  return Object.freeze({
    family: 'speed_match',
...
// общей форме kind: 'phrases' — три первые фразы 0-2 уже отработаны в
// интро-carousel, introSteps() использует sourcePhraseIndex 0/1/2):
// собственное распределение семей из-за 8-дистракторного предела —
// индексы 4/5/8/10/11/13/14 (>8 уникальных дистракторов) идут только через
// context_gap_grammar/listen_choose/speed_match; индексы 3/6/7/9/12
// (≤8 уникальных дистракторов) используют phrase_builder/
// listen_build_dictation. context_gap_grammar проверяет ПЕРВОЕ слово
// (связку) — ядро темы сессии (сдвиг числа при сохранении лица). Каждый
// индекс 3-14 используется как target.sourceIndex ровно один раз.
export const ES_EPISODE_01_SESSION_25_MODE_NATIVE_PRACTICE_V1 = Object.freeze<readonly SessionModeNativePracticeSourceV1[]>([
  ...
  { family: 'context_gap_grammar', purpose: 'guided_practice', learningStage: 'apply_in_phrase', target: { kind: 'phrase', sourceIndex: 5 }, modePayload: contextGapFirstWord(5) },
  { family: 'listen_build_dictation', purpose: 'guided_practice', learningStage: 'apply_in_phrase', target: { kind: 'phrase', sourceIndex: 6 }, modePayload: listenBuildPhrase(6) },
  { family: 'listen_choose', purpose: 'retrieval_practice', learningStage: 'apply_in_phrase', target: { kind: 'phrase', sourceIndex: 7 }, modePayload: listenChoosePhrase(7, 9) },
  ...
  { family: 'speed_match', purpose: 'retrieval_practice', learningStage: 'apply_in_phrase', target: { kind: 'phrase', sourceIndex: 9 }, modePayload: speedMatchPhrases([3, 6, 8, 10, 13]) },
  { family: 'phrase_builder', purpose: 'near_transfer', learningStage: 'apply_in_phrase', target: { kind: 'phrase', sourceIndex: 12 }, modePayload: fullPhraseBuilder(12) },
  { family: 'context_gap_grammar', purpose: 'near_transfer', learningStage: 'apply_in_phrase', target: { kind: 'phrase', sourceIndex: 10 }, modePayload: contextGapFirstWord(10) },
  { family: 'context_gap_grammar', purpose: 'near_transfer', learningStage: 'apply_in_phrase', target: { kind: 'phrase', sourceIndex: 11 }, modePayload: contextGapFirstWord(11) },
  { family: 'listen_choose', purpose: 'independent_check', learningStage: 'apply_in_phrase', target: { kind: 'phrase', sourceIndex: 13 }, modePayload: listenChoosePhrase(13, 14) },
...
if (ES_EPISODE_01_SESSION_25_MODE_NATIVE_PRACTICE_V1.length !== 12) {
  throw new Error('es_session_25_mode_native_practice_count_invalid');
}
