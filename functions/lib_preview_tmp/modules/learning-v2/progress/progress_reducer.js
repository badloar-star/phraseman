...
exports.deriveRequiredLoopsComplete = exports.deriveAccessStarsEarned = exports.reduceProgress = exports.assertProgressSnapshot = exports.createInitialProgressState = void 0;
const stars_1 = require("../contracts/stars");
const isRecord = (value) => typeof value === "object" && value !== null && !Array.isArray(value);
const isNonEmpty = (value) => typeof value === "string" && value.trim().length > 0;
const isStars = (value) => Number.isInteger(value) && Number(value) >= 0 && Number(value) <= 3;
const isCanonicalRef = (value) => isRecord(value) &&
...
const isMaterializationRef = (value, assessed) => {
    if (!isRecord(value) ||
    ...
    return (isNonEmpty(id) && typeof hash === "string" && /^[a-f0-9]{64}$/.test(hash));
};
const isStrictAttemptRef = (value) => isCanonicalRef(value) && Object.keys(value).length === 3;
const MAX_INDEX_ENTRIES = 2048;
const isAcceptedOutcome = (value) => [
    "CORRECT",
    "COMPLETED",
    "PASS_CONFIDENT",
...
const createInitialProgressState = (options = {}) => ({
...
const assertProgressSnapshot = (state) => {
    ...
    const slotStars = slotIds.map((slotId) => {
        ...
        if (slot.bestPerformanceAttemptRef &&
    ...
    if (Object.keys(state.learningEvidenceIndex).length > MAX_INDEX_ENTRIES ||
    ...
    for (const [key, entry] of Object.entries(state.checkpointEvidenceIndex)) {
        if (!isRecord(entry) ||
            ...
            !["passed", "repair_required", "not_assessed", "incomplete"].includes(String(entry.state)))
    ...
    for (const [tupleKey, entry] of Object.entries(state.learningEvidenceIndex)) {
        ...
        if (typedEntry.bestAssessedEvidenceRef &&
            !isMaterializationRef(typedEntry.bestAssessedEvidenceRef, true))
            throw new Error("learning_evidence_index_invalid");
        if (typedEntry.latestNonAssessmentRef &&
...
exports.assertProgressSnapshot = assertProgressSnapshot;
const outcomeRank = (outcome) => {
    if (outcome.resultCode === "PASS_CONFIDENT" ||
        outcome.resultCode === "CORRECT" ||
        outcome.resultCode === "COMPLETED")
        return 2;
    if (outcome.resultCode === "NEEDS_WORK_CONFIDENT" ||
        outcome.resultCode === "WRONG")
        return 1;
    return 0;
};
const reduceProgress = (state, event, context) => {
    (0, exports.assertProgressSnapshot)(state);
    if (state.accountScopeHash !== context.expectedAccountScopeHash ||
    ...
    const previousAttemptHash = context.processedAttemptIndex[event.attemptRef.opId];
    if (previousAttemptHash &&
    ...
    if (!catalog)
        throw new Error("star_slot_unknown");
    const episodeSlotCount = Object.values(context.slotCatalog).filter((slot) => slot.episodeId === catalog.episodeId).length;
    if (episodeSlotCount > 8)
        throw new Error("star_slots_invalid");
    ...
    if (event.activityId && event.activityId !== catalog.activityId)
        throw new Error("star_slot_catalog_mismatch");
    if (event.progressCompatibilityKey &&
        event.progressCompatibilityKey !== catalog.progressCompatibilityKey)
        throw new Error("star_slot_catalog_mismatch");
    if (context.declaredEvidenceTupleKeys &&
    ...
    if (current?.bestPerformanceAttemptRef?.opId === event.attemptRef.opId &&
    ...
    const projection = (0, stars_1.applyBestPerformanceStars)({
        previous,
        candidate: event.candidateStars,
    });
    const activityId = event.activityId ?? current?.activityId ?? catalog.activityId;
    const compatibilityKey = event.progressCompatibilityKey ??
    ...
    if (!current || projection.next > previous) {
        nextSlots[event.starSlotId] = {
            ...
            bestPerformanceStars: projection.next,
            ...(event.attemptRef
    ...
    if (event.learningEvidence) {
        ...
        const bodyHash = "assessedRef" in input
        ...
        if ("assessedRef" in input) {
            if (existing.bestAssessedEvidenceRef?.observationId ===
            ...
            const currentHash = existing.bestAssessedEvidenceRef?.evidenceBodyHash ?? "";
            nextEvidence[input.tupleKey] = {
                ...existing,
                bestAssessedEvidenceRef: input.assessedRef.evidenceBodyHash >= currentHash
                    ? input.assessedRef
                    : existing.bestAssessedEvidenceRef,
            };
        }
        else {
            if (existing.latestNonAssessmentRef?.nonAssessmentId ===
            ...
            nextEvidence[input.tupleKey] = {
                ...existing,
                latestNonAssessmentRef: input.nonAssessmentRef.nonAssessmentId >= currentId
    ...
    if (event.checkpointEvidence) {
        ...
        if (checkpoint.state === "passed" &&
            !context.checkpointContract.criticalTupleKeys.includes(checkpoint.tupleKey))
            throw new Error("checkpoint_critical_tuple_required");
        if (checkpoint.state === "repair_required" &&
            !context.checkpointContract.repairTupleKeys.includes(checkpoint.tupleKey))
            throw new Error("checkpoint_repair_route_missing");
        if (checkpoint.state === "not_assessed" &&
            !context.checkpointContract.alternateTupleKeys.includes(checkpoint.tupleKey))
    ...
    const changed = projection.performanceStarsDelta > 0 ||
    ...
    return {
        state: {
            ...
            updatedAt: event.occurredAtMs === undefined ||
...
exports.reduceProgress = reduceProgress;
const deriveAccessStarsEarned = (state) => {
    (0, exports.assertProgressSnapshot)(state);
    return Object.values(state.slots).reduce((sum, slot) => sum + slot.bestPerformanceStars, 0);
};
exports.deriveAccessStarsEarned = deriveAccessStarsEarned;
const deriveRequiredLoopsComplete = (state, context) => {
    (0, exports.assertProgressSnapshot)(state);
    ...
    const accepted = (nodeId) => {
        ...
        if (!entry ||
            ...
            return false;
        if (context.processedAttemptIndex[entry.bestAcceptedAttemptRef.opId] !==
            entry.bestAcceptedAttemptRef.attemptBodyHash)
            return false;
        if (!isRecord(entry.bestAcceptedOutcome) ||
        ...
        return (resultCode === "CORRECT" ||
            resultCode === "COMPLETED" ||
            resultCode === "PASS_CONFIDENT");
    };
    return (loops.encounterBuild.length > 0 &&
        loops.nearTransfer.length > 0 &&
        loops.encounterBuild.every(accepted) &&
        loops.nearTransfer.every(accepted));
};
exports.deriveRequiredLoopsComplete = deriveRequiredLoopsComplete;
//# sourceMappingURL=progress_reducer.js.map