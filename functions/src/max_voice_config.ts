...
export const MAX_VOICE_CONFIG_DEFAULTS: MaxVoiceConfig = {
  ...
  // зачем (владелец 2026-08-29, «он перебивает сам себя, отвечает 2-3 раза»):
  // max_output_tokens — ЖЁСТКИЙ обрыв посреди слова. У A1 учитель говорит
  // БОЛЬШЕ всех (объяснение на родном + английская фраза + просьба повторить),
  // а лимит стоял МЕНЬШЕ всех — 500. Ответ обрывался, следом шла автоматическая
  // «договорка» отдельным ответом, и для человека это звучало как вторая
  // реплика на ту же фразу. Потолок выровнен по реальной длине речи учителя;
  // краткость держат инструкции промпта, а не обрыв на полуслове.
  maxResponseOutputTokens: { A1: 1200, A2: 1200, B1: 1200, B2: 1200, injected: 800 },
  ...
  // зачем (владелец 2026-09-04, живой звонок): «какого хуя он говорит сам,
  // когда я молчу». 8 секунд — меньше, чем новичку нужно, чтобы собрать первую
  // фразу на чужом языке: человек ещё думает, а учитель уже подсказывает, и
  // разговор превращается в монолог. Новичку даём больше времени на подумать,
  // и подсказок за звонок меньше (4 → 2): помощь должна быть редкой и ценной.
  hintDelaySec: { A1: 20, A2: 16, B1: 13, B2: 11 },
  hintMaxPerSession: 2,
