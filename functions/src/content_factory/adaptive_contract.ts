...
export function validateAdaptiveGenerationRequest(input: AdaptiveGenerationRequest): void {
  ...
  if (!['practice', 'quiz', 'cards'].includes(input.surface) || !['repeated_mistakes', 'spaced_repetition', 'arena_review'].includes(input.reason)) throw new Error('surface_or_reason_invalid');
}
