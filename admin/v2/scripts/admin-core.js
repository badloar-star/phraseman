import { capabilitiesForRoute, capabilityById, capabilityHubHash } from './admin-capabilities.js';
...
import { createContentOperationsState } from './admin-content-operations-state.js';
import { createContentOperationsController } from './admin-content-operations-controller.js';
import { renderContentOperationsWorkspace } from './admin-content-operations-view.js';
import { createCommunityOperationsState } from './admin-community-operations-state.js';
import { createCommunityOperationsController } from './admin-community-operations-controller.js';
import { renderCommunityOperationsWorkspace } from './admin-community-operations-view.js';
...
const PAGES = Object.freeze({
  ...
  'money-operations': { title: 'Операции с оплатами', description: 'Покупки UGC, возвраты провайдеров, рефералы, Telegram и веб-оплаты через серверный предпросмотр и одобрение.' },
  content: { title: 'Контент', description: 'Уроки, языковые пакеты и безопасная фабрика новых языков.' },
  'content-operations': { title: 'Операции с контентом', description: 'Пакеты, карточки, фразы, French HOLD и репорты через ограниченные серверные процессы.' },
  community: { title: 'Комьюнити', description: 'Жалобы, пользовательский контент, чат и состояние Арены.' },
  'community-operations': { title: 'Сообщество и Арена', description: 'Очереди, помощь, лиги и Арена через ограниченные серверные процессы.' },
...
const state = {
  ...
  contentOperations: createContentOperationsState(),
  communityOperations: createCommunityOperationsState(),
};

let actions = null;
let voiceResearchController = null;
let moneyOperationsController = null;
let contentOperationsController = null;
let communityOperationsController = null;
...
function renderAuditLogPanel() {
  ...
  const filterBar = `<div class="toolbar-grid">
    ...
    <div class="actions end"><button class="button primary" data-action="load-audit-log" type="button"${disabledWhenUnauthorized('diagnostics.read')} title="Загрузить журнал действий через серверную проверку прав" data-tooltip="Загрузить журнал действий через серверную проверку прав">Обновить журнал</button></div>
  </div>`;
  let body = '';
...
function renderDiagnostics() {
  ...
  const headerActions = `<button class="button primary" data-action="load-daily-briefing" type="button"${disabledWhenUnauthorized('briefing.read')} title="Прочитать последний сохранённый снимок без запуска генерации" data-tooltip="Прочитать последний сохранённый снимок без запуска генерации">Обновить диагностику</button><a class="button" href="./migration.html" title="Открыть полный реестр переноса функций" data-tooltip="Открыть полный реестр переноса функций">Реестр переноса</a>`;
...
function renderCurrentPage() {
  ...
  if (!target) return;
  const renderers = { overview: renderOverview, 'control-panel': renderControlPanel, application: renderApplication, campaigns: renderCampaigns, users: renderUsers, money: renderMoney, 'money-operations': () => renderMoneyOperationsWorkspace(state.moneyOperations, { escapeHtml, can }), content: renderContent, 'content-operations': () => renderContentOperationsWorkspace(state.contentOperations, { escapeHtml, can }), community: renderCommunity, 'community-operations': () => renderCommunityOperationsWorkspace(state.communityOperations, { escapeHtml, can }), diagnostics: renderDiagnostics, support: renderSupport, emails: renderEmails, analytics: renderAnalytics, 'daily-briefing': renderDailyBriefing, 'report-center': renderReportQueue, 'asset-studio': renderAssetStudio, 'explain-cache': renderExplainCache, compass: renderCompass, 'review-promo': renderReviewPromo, alerts: renderAlerts, 'voice-research': () => renderVoiceResearchCenter(state.voiceResearch, { escapeHtml, can }), 'safety-moderation': () => renderSafetyModerationCenter(state.safetyModeration, { escapeHtml, can }) };
...
function getContentOperationsController() {
  if (!contentOperationsController) {
    contentOperationsController = createContentOperationsController({
      getModel: () => state.contentOperations,
      setModel: (value) => { state.contentOperations = value; },
      actions: () => actions,
      render: renderCurrentPage,
      route: () => state.route,
      authorized: () => state.authorized && can('content.read'),
      can,
      message: setMessage,
      errorMessage,
    });
  }
  return contentOperationsController;
}

function getCommunityOperationsController() {
  if (!communityOperationsController) {
    communityOperationsController = createCommunityOperationsController({
      getModel: () => state.communityOperations,
      setModel: (value) => { state.communityOperations = value; },
      actions: () => actions,
      render: renderCurrentPage,
      route: () => state.route,
      authorized: (capabilityId) => state.authorized && (['help-board', 'helpers-board'].includes(capabilityId) ? can('community.help.read') : can('community.read')),
      can,
      message: setMessage,
      errorMessage,
    });
  }
  return communityOperationsController;
}
...
async function handleClick(event) {
  ...
  const contentAction = target.getAttribute('data-content-action');
  if (contentAction) {
    event.preventDefault();
    await getContentOperationsController().handleAction(contentAction, target);
    return;
  }
  const communityAction = target.getAttribute('data-community-action');
  if (communityAction) {
    event.preventDefault();
    await getCommunityOperationsController().handleAction(communityAction, target);
    return;
  }
  ...
  if (capabilityId) {
    ...
    if (capability) globalThis.location.hash = capabilityHubHash(capability);
    return;
  }
...
export function setAdminActions(nextActions) {
  ...
  getContentOperationsController().maybeLoad();
  getCommunityOperationsController().maybeLoad();
...
export function setAuthState(auth) {
  ...
  if (!state.authorized || !can('content.read')) state.contentOperations = createContentOperationsState();
  if (!state.authorized || (!can('community.read') && !can('community.help.read'))) state.communityOperations = createCommunityOperationsState();
...
export function renderRoute(route, capabilityId = '') {
  ...
  if (state.route === 'content-operations') getContentOperationsController().selectCapability(capability?.id || capabilityId || 'community-packs');
  if (state.route === 'community-operations') getCommunityOperationsController().selectCapability(capability?.id || capabilityId || 'mod-queue');
...
export function initAdminUi() {
  ...
  document.addEventListener('change', (event) => {
    ...
    if (event.target instanceof HTMLSelectElement && event.target.hasAttribute('data-content-mobile-view')) {
      void getContentOperationsController().selectCapability(event.target.value);
      return;
    }
    if (event.target instanceof HTMLSelectElement && event.target.hasAttribute('data-community-mobile-view')) {
      void getCommunityOperationsController().selectCapability(event.target.value);
      return;
    }
