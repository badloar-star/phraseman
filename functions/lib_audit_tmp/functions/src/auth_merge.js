...
exports.authMergeStableAccounts = exports.accountMergeOutboxRetryCron = exports.accountMergeOutboxWorker = void 0;
...
exports.processAccountMergeOutboxJob = processAccountMergeOutboxJob;
...
function revenueCatProjectionStrength(progress) {
    const plan = cleanStr(progress.premium_plan).toLowerCase();
    if (plan === 'lifetime')
        return Number.MAX_SAFE_INTEGER;
    if (!isStorePremiumPlan(plan))
        return 0;
...
function accountMergeOutboxId(winnerStableId, loserStableId) {
    return `merge_${(0, crypto_1.createHash)('sha256').update(`${winnerStableId}\0${loserStableId}`).digest('hex')}`;
}
async function mergeStableAccountsTransactionally(db, authUid, rawA, rawB, now) {
    ...
    const outcome = await db.runTransaction(async (tx) => {
        ...
        if (loserLineages.docs.length > MAX_MERGE_PREMIUM_LINEAGES
            ...
            throw new https_1.HttpsError('resource-exhausted', 'premium_lineage_limit');
        }
        const canonicalLineages = new Map();
        for (const lineage of [...winnerLineages.docs, ...loserLineages.docs]) {
            ...
            canonicalLineages.set(state.lineageHash, {
                targetRef,
                state: existing
                    ? (0, revenuecat_premium_lineage_1.chooseAuthoritativePremiumLineageState)(existing.state, state)
                    : state,
                obsoleteRefs: [
        ...
        const canonicalAggregate = canonicalLineages.size > 0
            ? (0, revenuecat_premium_lineage_1.aggregatePremiumLineages)([...canonicalLineages.values()].map(({ state }) => state), mergedProgress, now)
            : null;
        ...
        for (const key of PREMIUM_KEYS)
            loserUpdate[`progress.${key}`] = admin.firestore.FieldValue.delete();
...
const ACCOUNT_MERGE_LEASE_MS = 5 * 60000;
const ACCOUNT_MERGE_RECEIPT_PAGE_SIZE = 100;
const ACCOUNT_MERGE_RECEIPT_FIELDS = [
...
async function processAccountMergeOutboxJob(db, outboxId, now = Date.now()) {
    ...
    const claim = await db.runTransaction(async (tx) => {
        ...
        if (data.status === 'completed')
            return { alreadyCompleted: true, winner: '', loser: '' };
        ...
        if (!winner || !loser || winner === loser)
            throw new Error('account_merge_outbox_invalid');
        const deletionGuards = await Promise.all([
            tx.get(db.collection(account_delete_job_1.ACCOUNT_DELETE_TOMBSTONES).doc(winner)),
            tx.get(db.collection(account_delete_job_1.ACCOUNT_DELETE_TOMBSTONES).doc(loser)),
            tx.get(db.collection(account_delete_job_1.ACCOUNT_DELETE_PERMANENT_DENIALS).doc((0, account_delete_job_1.accountDeletePermanentDenialId)(winner))),
            tx.get(db.collection(account_delete_job_1.ACCOUNT_DELETE_PERMANENT_DENIALS).doc((0, account_delete_job_1.accountDeletePermanentDenialId)(loser))),
        ]);
        if (deletionGuards.some((guard) => guard.exists)) {
            tx.set(ref, { status: 'cancelled_by_deletion', leaseUntilMs: 0, updatedAt: now }, { merge: true });
            return { cancelled: true, winner, loser };
        }
        tx.set(ref, {
            status: 'running',
            leaseToken,
    ...
    if (!claim)
        return { status: 'already_completed', updated: 0 };
    if ('alreadyCompleted' in claim)
        return { status: 'already_completed', updated: 0 };
    if ('cancelled' in claim)
        return { status: 'cancelled', updated: 0 };
    let updated = 0;
    try {
            ...
            for (;;) {
                ...
                const pageResult = await db.runTransaction(async (tx) => {
                    ...
                    for (let depth = 0; depth <= revenuecat_premium_lineage_1.MAX_OWNER_LINEAGES; depth += 1) {
                        if (seenOwners.has(canonicalWinner))
                            throw new Error('account_merge_owner_map_cycle');
                        ...
                        if (!ownerMap.exists || !nextOwner || nextOwner === canonicalWinner)
                            break;
                        if (depth === revenuecat_premium_lineage_1.MAX_OWNER_LINEAGES)
                            throw new Error('account_merge_owner_map_overflow');
                        canonicalWinner = nextOwner;
                    }
                    const guardedIdentities = [...new Set([claim.loser, claim.winner, canonicalWinner])];
                    const deletionGuards = [];
                    for (const identity of guardedIdentities) {
                        deletionGuards.push(await tx.get(db.collection(account_delete_job_1.ACCOUNT_DELETE_TOMBSTONES).doc(identity)));
                        deletionGuards.push(await tx.get(db.collection(account_delete_job_1.ACCOUNT_DELETE_PERMANENT_DENIALS).doc((0, account_delete_job_1.accountDeletePermanentDenialId)(identity))));
                    }
                    const currentDocs = [];
                    for (const doc of snapshot.docs)
                        currentDocs.push(await tx.get(doc.ref));
                    if (deletionGuards.some((guard) => guard.exists)) {
                        tx.set(ref, {
                        ...
                        return { cancelled: true, changed: 0 };
                    }
                    let changed = 0;
                    for (const currentDoc of currentDocs) {
                        ...
                        if (!stillOwnedByLoser)
                            continue;
                        const replacement = spec.array
                            ? [...new Set(raw.map((value) => (cleanStr(value) === claim.loser ? canonicalWinner : cleanStr(value))).filter(Boolean))]
                            : canonicalWinner;
                        tx.update(currentDoc.ref, {
                            [spec.field]: replacement,
                            canonicalOwnerUid: canonicalWinner,
                            previousOwnerUidHash: (0, crypto_1.createHash)('sha256').update(claim.loser).digest('hex'),
                ...
                if (snapshot.size < ACCOUNT_MERGE_RECEIPT_PAGE_SIZE)
                    break;
            }
        }
        ...
        await db.runTransaction(async (tx) => {
            const snap = await tx.get(ref);
            if (!snap.exists || snap.data()?.status !== 'running' || snap.data()?.leaseToken !== leaseToken)
                return;
            tx.set(ref, {
                status: 'pending',
...
exports.accountMergeOutboxWorker = (0, firestore_1.onDocumentCreated)({
...
exports.accountMergeOutboxRetryCron = (0, scheduler_1.onSchedule)({
    ...
    for (const doc of [...pending.docs, ...running.docs]) {
        if (doc.data()?.status === 'running' && Number(doc.data()?.leaseUntilMs ?? 0) > now)
            continue;
