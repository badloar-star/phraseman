...
exports.tournamentClaimReward = exports.tournamentFinalize = exports.tournamentSubmitAnswers = exports.tournamentFillBots = exports.tournamentJoin = exports.tournamentCreateRooms = void 0;
...
function readRoom(snap) {
    ...
    return {
        ...
        state: sanitizeString(d.state, 20),
...
async function loadTasksByIds(db, taskIds) {
    const out = new Map();
    const snaps = await Promise.all(taskIds.map((id) => db.collection(tournament_core_1.TOURNAMENT_TASKS_COLLECTION).doc(id).get()));
...
function scorePlayerRound(answers, streakStart) {
    let roundScore = 0;
    let correct = 0;
    let streak = Math.max(0, streakStart);
    for (const a of answers) {
        roundScore += (0, tournament_core_1.scoreAnswer)({ ...a, streakBefore: streak });
        if (a.correct) {
            correct += 1;
            streak += 1;
        }
...
function advanceState(room, target) {
    let guard = 0;
    while (room.state !== target && guard < 16) {
        const next = (0, tournament_core_1.nextTournamentState)(room.state);
        if (!next || !(0, tournament_core_1.canTransitionTournament)(room.state, next)) {
            throw new https_1.HttpsError('failed-precondition', `bad_state_transition_${room.state}_to_${target}`);
        }
        room.state = next;
        guard += 1;
    }
...
exports.tournamentCreateRooms = (0, scheduler_1.onSchedule)({ schedule: '*/5 * * * *', timeZone: 'UTC', region: REGION }, async () => {
    ...
    for (const slot of cfg.slots) {
        ...
        const createAt = startsAt - tournament_core_1.TOURNAMENT_CREATE_AHEAD_MS;
        if (nowMs < createAt || nowMs >= startsAt)
            continue;
...
exports.tournamentJoin = (0, https_1.onCall)(callable_options_1.HOT_CALLABLE_OPTIONS, async (request) => {
    ...
    const cfg = await loadScheduleConfig(db);
    const weekId = (0, tournament_core_1.tournamentWeekId)();
    const bankRef = db.collection(tournament_core_1.TOURNAMENT_BANK_COLLECTION).doc(weekId);
    return db.runTransaction(async (tx) => {
        ...
        if (room.state !== 'scheduled' && room.state !== 'lobby') {
            throw new https_1.HttpsError('failed-precondition', 'room_already_started');
        }
        if (room.players.some((p) => !p.isBot && p.id === stableUid)) {
            return { ok: true, joined: true, alreadyJoined: true, roomId };
        }
        if (room.players.length >= tournament_core_1.TOURNAMENT_ROOM_SIZE) {
        ...
        if (useFreeEntry) {
            ...
            const contribution = (0, tournament_core_1.bankContributionGems)(ticketsRequired, cfg.ticketGemValue);
            if (contribution > 0) {
                tx.set(bankRef, {
                    weekId,
...
async function cancelRoomWithRefunds(db, roomRef, room, reason) {
    const nowMs = Date.now();
    const batch = db.bulkWriter();
    for (const p of room.players) {
        if (p.isBot || p.refunded)
            continue;
        ...
        batch.set(userRef, {
            shards: admin.firestore.FieldValue.increment(tournament_core_1.TOURNAMENT_CANCEL_COMPENSATION_GEMS),
    ...
    batch.update(roomRef, {
        state: tournament_core_1.TOURNAMENT_STATE_CANCELLED,
        cancelReason: reason,
        cancelledAtMs: nowMs,
...
exports.tournamentFillBots = (0, scheduler_1.onSchedule)({ schedule: '* * * * *', timeZone: 'UTC', region: REGION }, async () => {
    const db = admin.firestore();
    const nowMs = Date.now();
    const horizon = nowMs + tournament_core_1.TOURNAMENT_FILL_BOTS_AHEAD_MS;
    const snap = await db.collection(tournament_core_1.TOURNAMENT_ROOMS_COLLECTION)
    ...
    for (const doc of snap.docs) {
        ...
        if (realCount < tournament_core_1.TOURNAMENT_MIN_REAL_PLAYERS) {
            if (!(0, tournament_core_1.canCancelTournament)(room.state))
                continue;
        ...
        const botsSnap = await db.collection(tournament_core_1.BOT_PROFILES_COLLECTION).limit(tournament_core_1.TOURNAMENT_ROOM_SIZE * 2).get();
        const allBots = botsSnap.docs.map((d) => ({ botId: d.id, ...d.data() }));
        const picked = (0, tournament_core_1.seededShuffle)(allBots, `${room.seed}:botpick`);
        const needed = tournament_core_1.TOURNAMENT_ROOM_SIZE - room.players.length;
        const botPlayers = picked.slice(0, Math.max(0, needed)).map((bot, i) => ({
            id: bot.botId,
            isBot: true,
            name: bot.name,
        ...
        const tasksSnap = await db.collection(tournament_core_1.TOURNAMENT_TASKS_COLLECTION)
            .where('verified', '==', true)
            .limit(200)
            .get();
        const pool = tasksSnap.docs.map((d) => ({
            taskId: d.id,
            mode: sanitizeString(d.data().mode, 40),
            isVoice: d.data().isVoice === true,
            difficulty: readInt(d.data().difficulty, 1),
            payload: d.data().payload && typeof d.data().payload === 'object' ? d.data().payload : {},
            tags: Array.isArray(d.data().tags) ? d.data().tags : [],
            verified: true,
        }));
        const rounds = tournament_core_1.TOURNAMENT_ROUND_MODE_KINDS.map((modeKind, idx) => {
            const roundNo = idx + 1;
            const tasks = (0, tournament_core_1.selectRoundTasks)({
                pool,
                roomId: room.roomId,
                roundNo,
...
exports.tournamentSubmitAnswers = (0, https_1.onCall)(callable_options_1.HOT_CALLABLE_OPTIONS, async (request) => {
    ...
    if (room.state !== (0, tournament_core_1.roundStateFor)(roundNo)) {
        throw new https_1.HttpsError('failed-precondition', `round_not_active_${room.state}`);
    }
    ...
    for (const a of rawAnswers.slice(0, round.taskIds.length)) {
        ...
        if (taskId)
            answersByTask.set(taskId, a);
    }
    const maxMs = DEFAULT_MAX_MS_PER_TASK;
    const inputs = round.taskIds.map((taskId) => {
    ...
    if (allRealSubmitted) {
        const roomCursor = { ...room, state: room.state };
        if (roundNo < 4) {
            advanceState(roomCursor, (0, tournament_core_1.roundStateFor)(roundNo + 1));
        }
        else {
...
async function finalizeTournamentRoom(db, roomId) {
    const roomRef = db.collection(tournament_core_1.TOURNAMENT_ROOMS_COLLECTION).doc(roomId);
    ...
    if (room.state !== 'results' && room.state !== 'final') {
        throw new https_1.HttpsError('failed-precondition', `cannot_finalize_from_${room.state}`);
    }
    ...
    const profilesSnap = await db.collection(tournament_core_1.BOT_PROFILES_COLLECTION)
    ...
    profilesSnap?.forEach((d) => {
        ...
        if (existing)
            existing.winRate = typeof data.winRate === 'number' ? data.winRate : existing.winRate;
    });
    for (const round of rounds) {
        ...
        for (const bp of missingBots) {
            ...
            const inputs = (0, tournament_core_1.simulateBotAnswers)(profile, {
                roomId,
                roundNo: round.roundNo,
                tasks: round.taskIds.map((id) => tasksById.get(id)).filter((t) => !!t),
    ...
    const { realPlacements } = (0, tournament_core_1.computePlacements)(players);
    const weekId = (0, tournament_core_1.tournamentWeekId)(room.startsAt || Date.now());
    const nowMs = Date.now();
    const winnerPlace1 = realPlacements.find((p) => p.place === 1);
    const batch = db.batch();
    for (const { player, place } of realPlacements) {
        const prize = (0, tournament_core_1.prizeForPlace)(place);
        const seasonPoints = (0, tournament_core_1.seasonPointsForPlace)(place);
        const seasonRef = db.collection(tournament_core_1.TOURNAMENT_SEASONS_COLLECTION)
            .doc(weekId)
            .collection(tournament_core_1.TOURNAMENT_SEASON_ENTRIES_SUBCOLLECTION)
            .doc(player.id);
        batch.set(seasonRef, {
            uid: player.id,
            name: player.name,
        ...
        if (prize) {
            const claimRef = userRef.collection(tournament_core_1.TOURNAMENT_REWARD_CLAIMS_SUBCOLLECTION).doc(`tournament_${roomId}`);
            batch.set(claimRef, {
                uid: player.id,
                roomId,
                place,
    ...
    const roomCursor = { ...room, players, rounds };
...
exports.tournamentClaimReward = (0, https_1.onCall)(callable_options_1.HOT_CALLABLE_OPTIONS, async (request) => {
    ...
    const { realPlacements } = (0, tournament_core_1.computePlacements)(room.players);
    const myPlacement = realPlacements.find((p) => p.player.id === stableUid);
    const prize = myPlacement ? (0, tournament_core_1.prizeForPlace)(myPlacement.place) : null;
    if (!myPlacement || !prize)
