...
export interface MaxVoiceMintRequest {
  ...
  /**
   * Учебный план урока (фразы курса + грамматика). Отдельно от снимка ученика:
   * зависит только от НОМЕРА урока, поэтому одинаков у всех на этом уроке и
  ...
  function handleDcMessage(raw: unknown): void {
    ...
    switch (type) {
      ...
      case 'conversation.item.input_audio_transcription.completed': {
        ...
        // Пустой транскрипт = VAD сработал на шум или эхо.
        //
        // НО отменяем ТОЛЬКО ответ, который реально ждал проверки на эхо, то есть
        // поставленный речью ПОВЕРХ звучащего MAX. Раньше здесь стояло
        // безусловное defaultResponseQueued = false, и пустой транскрипт гасил
        // в том числе ответ на настоящую реплику в тишине — MAX замолкал
        // навсегда («он может замолчать и не говорить ничего», живой звонок).
        // Гасим только ответ, который ЖДАЛ проверки, то есть поставленный речью
        // поверх звучащего MAX. Ответ на реплику в тишине пустой транскрипт не
        // трогает: раньше здесь стояло безусловное снятие, и MAX замолкал
        // навсегда («он может замолчать и не говорить ничего», живой звонок).
        if (responseAwaitsTranscript) {
          clearEchoVerdict();
