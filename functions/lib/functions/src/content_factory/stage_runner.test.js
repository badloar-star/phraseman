...
describe('generation stage runner', () => {
    const packet = (0, prompt_registry_1.buildStagePromptPacket)('challenge_questions', 'v1', (0, prompt_context_1.buildPromptContext)({ studyTarget: 'fr', sourceLocale: 'ru', cefr: 'A1', objective: 'Identity', count: 2, approvedArtifactIds: [], exemplarIds: [], previousContentFingerprints: [] }));
    it('accepts structured output with exact identity and count', async () => {
        const provider = { generate: async () => JSON.stringify({ stage: 'challenge_questions', items: [{ id: 'q1' }, { id: 'q2' }] }) };
    ...
    it('negotiates json_schema with fallback and still runs the server validator', async () => {
        ...
        formats.length = 0;
        await (0, stage_runner_1.runGenerationStage)({ provider: { generate: async (input) => { formats.push(input.responseFormat); return JSON.stringify({ stage: 'challenge_questions', items: [{ id: 'q1' }, { id: 'q2' }] }); } }, model: 'unknown-model', packet, maxRepairs: 0 });
