...
export function planTaskDestinationLabel(destination: PlanTaskDestination): string {
  ...
  if (destination.type === 'mistake_practice') return `Ошибки · ${destination.length}`;
...
/** True only when tapping the task immediately enters an activity that charges on start. */
export function personalPlanTaskStartsPaidExercise(task: PlanDailyTask): boolean {
  return [
    'plan_phrase_lesson',
    'recall',
    'plan_phrase_recall',
    ...
    'mistake_practice',
    'flashcards',
  ].includes(task.destination.type);
}
...
export function openPersonalPlanTask(
  ...
  if (destination.type === 'mistake_practice') {
    go({
      pathname: '/mistake_practice_session',
      params: {
        length: String(destination.length),
        planTaskId: task.id,
        ...(planInstanceId ? { planInstanceId } : {}),
        planId: plan.id,
        planDayIndex: String(day.dayIndex),
