 ...
 *
 * Требование плана: ноль живых функций без исходников, кроме осознанно
 * удалённых (чаты). Арена удалена через tombstone-экспорты
 * (functions/src/quiz_arena_decommission.ts) — они ОСТАЮТСЯ в сборке и потому
 * сиротами не считаются, это правильное поведение.
 *
 * Использование:
