...
отдельный plan pool не блокирует готовность генератора. Существующая функция не
удаляется и остаётся отдельной необязательной специализацией второй очереди.
Старые противоположные формулировки о plan cards считаются superseded этим
решением.
