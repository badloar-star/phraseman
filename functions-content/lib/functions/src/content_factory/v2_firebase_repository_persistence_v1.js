...
exports.V2_REPOSITORY_IMMUTABLE_STORAGE_MAX_BYTES_V1 = exports.V2_REPOSITORY_IMMUTABLE_AUDIO_CONTENT_TYPE_V1 = exports.V2_REPOSITORY_IMMUTABLE_BINARY_CONTENT_TYPE_V1 = exports.V2_REPOSITORY_IMMUTABLE_JSON_CONTENT_TYPE_V1 = void 0;
exports.executeV2RepositorySingleflightClaimTransactionV1 = executeV2RepositorySingleflightClaimTransactionV1;
exports.executeV2RepositorySingleflightFinalizeTransactionV1 = executeV2RepositorySingleflightFinalizeTransactionV1;
exports.persistV2ImmutableRepositoryObjectV1 = persistV2ImmutableRepositoryObjectV1;
const node_crypto_1 = require("node:crypto");
const v2_repository_singleflight_state_v1_1 = require("./v2_repository_singleflight_state_v1");
exports.V2_REPOSITORY_IMMUTABLE_JSON_CONTENT_TYPE_V1 = "application/json; charset=utf-8";
exports.V2_REPOSITORY_IMMUTABLE_BINARY_CONTENT_TYPE_V1 = "application/octet-stream";
exports.V2_REPOSITORY_IMMUTABLE_AUDIO_CONTENT_TYPE_V1 = "audio/mpeg";
exports.V2_REPOSITORY_IMMUTABLE_STORAGE_MAX_BYTES_V1 = 24 * 1024 * 1024;
...
function exactObjectPath(value) {
...
function exactContentType(value) {
    if (value !== exports.V2_REPOSITORY_IMMUTABLE_JSON_CONTENT_TYPE_V1 &&
        value !== exports.V2_REPOSITORY_IMMUTABLE_BINARY_CONTENT_TYPE_V1 &&
        value !== exports.V2_REPOSITORY_IMMUTABLE_AUDIO_CONTENT_TYPE_V1) {
        fail("v2_repository_immutable_content_type_invalid");
    }
    return value;
}
function parseExactRead(value) {
    ...
    if (!hasExactKeys(value, ["exists", "raw"]) ||
        typeof value.raw !== "string") {
        fail("v2_repository_firestore_exact_read_invalid");
    }
    return Object.freeze({ exists: true, raw: value.raw });
}
async function applyClaimWrite(transaction, decision) {
    if (decision.kind === "create") {
        await transaction.createExact(decision.command.documentPath, (0, v2_repository_singleflight_state_v1_1.serializeV2RepositorySingleflightStateV1)(decision.command.next));
        return;
    }
    if (decision.kind === "stale_takeover") {
        if (decision.command.expectedOperationRevision === null ||
            decision.command.expectedOperationFingerprint === null) {
            fail("v2_repository_firestore_cas_command_invalid");
        }
        await transaction.compareAndSetExact(decision.command.documentPath, Object.freeze({
        ...
        }), (0, v2_repository_singleflight_state_v1_1.serializeV2RepositorySingleflightStateV1)(decision.command.next));
    }
}
async function executeV2RepositorySingleflightClaimTransactionV1(input) {
    if (!isRecord(input) ||
        !hasExactKeys(input, [
            "firestore",
            "identity",
    ...
    const documentPath = (0, v2_repository_singleflight_state_v1_1.v2RepositorySingleflightDocumentPathV1)(input.identity.planFingerprint);
    return input.firestore.runTransaction(async (transaction) => {
        ...
        const decision = (0, v2_repository_singleflight_state_v1_1.decideV2RepositorySingleflightClaimV1)({
...
async function executeV2RepositorySingleflightFinalizeTransactionV1(input) {
    if (!isRecord(input) ||
        !hasExactKeys(input, [
            "firestore",
            "identity",
    ...
    return input.firestore.runTransaction(async (transaction) => {
        ...
        const current = parseExactRead(await transaction.readExact(documentPath));
        if (!current.exists)
            fail("v2_repository_firestore_claim_missing");
        const decision = (0, v2_repository_singleflight_state_v1_1.decideV2RepositorySingleflightFinalizeV1)({
...
function parseMetadata(value) {
    if (!isRecord(value) ||
        ...
        Number(value.byteSize) > exports.V2_REPOSITORY_IMMUTABLE_STORAGE_MAX_BYTES_V1 ||
...
async function quarantineBestEffort(storage, conflict) {
    try {
...
async function persistV2ImmutableRepositoryObjectV1(input) {
    ...
    if (!Number.isSafeInteger(input.maximumBytes) ||
        ...
        Number(input.maximumBytes) > exports.V2_REPOSITORY_IMMUTABLE_STORAGE_MAX_BYTES_V1 ||
    ...
    if (download.kind !== "downloaded") {
        if ((download.kind !== "generation_mismatch" &&
...
//# sourceMappingURL=v2_firebase_repository_persistence_v1.js.map