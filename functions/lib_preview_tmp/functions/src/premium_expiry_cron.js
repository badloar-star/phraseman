...
exports.samePremiumAuthority = samePremiumAuthority;
exports.reconcilePremiumExpiry = reconcilePremiumExpiry;
...
function samePremiumAuthority(scanned, current) {
    const left = scanned ?? {};
    const right = current ?? {};
    return PREMIUM_AUTHORITY_FIELDS.every((field) => (cleanStr(left[field]) === cleanStr(right[field])));
}
function reconcilePremiumExpiry(currentProgress, currentLineages, now) {
    const aggregate = currentLineages.length > 0
        ? (0, revenuecat_premium_lineage_1.aggregatePremiumLineages)(currentLineages, currentProgress, now)
        : null;
    ...
    if (aggregate) {
        for (const [key, value] of Object.entries(aggregate.progressPatch)) {
            if (cleanStr(currentProgress[key]) !== cleanStr(value))
                patch[key] = cleanStr(value);
        }
    }
    if (decision)
...
async function sweepExpiredPremium(now = Date.now()) {
    ...
    for (;;) {
        ...
        for (const doc of page.docs) {
            stats.scanned += 1;
            const scannedProgress = (doc.data()?.progress ?? {});
            if (!planExpiryDeactivation(scannedProgress, now))
                continue;
            writes.push(db.runTransaction(async (tx) => {
                const current = await tx.get(doc.ref);
                const tombstone = await tx.get(db.collection(account_delete_job_1.ACCOUNT_DELETE_TOMBSTONES).doc(doc.id));
                const permanentDenial = await tx.get(db.collection(account_delete_job_1.ACCOUNT_DELETE_PERMANENT_DENIALS).doc((0, account_delete_job_1.accountDeletePermanentDenialId)(doc.id)));
                const lineageSnapshot = await tx.get(db.collection('revenuecat_premium_lineages')
                    .where('ownerUid', '==', doc.id)
                    .limit(revenuecat_premium_lineage_1.MAX_OWNER_LINEAGES + 1));
                if (!current.exists || tombstone.exists || permanentDenial.exists)
                    return null;
                if (lineageSnapshot.docs.length > revenuecat_premium_lineage_1.MAX_OWNER_LINEAGES) {
                    throw new Error('premium_lineage_limit');
                }
                const currentProgress = (current.data()?.progress ?? {});
                if (!samePremiumAuthority(scannedProgress, currentProgress))
                    return null;
                const currentLineages = lineageSnapshot.docs
                    .map((lineage) => lineage.data());
