...
const firestore_repository_1 = require("../agent_manager/firestore_repository");
const ledger_1 = require("../agent_manager/ledger");
const telegram_approvals_1 = require("../agent_manager/telegram_approvals");
const telegram_contracts_1 = require("../agent_manager/telegram_contracts");
const firestore_repository_2 = require("./firestore_repository");
const ledger_2 = require("./ledger");
const telegram_approvals_2 = require("./telegram_approvals");
...
const handler = (0, telegram_transport_1.createAgentOfficeTelegramWebhookHandler)({
    ...
    loadTokenByHash: async (callbackNamespace, tokenIdHash) => {
        const tokenPath = callbackNamespace === 'ao1'
            ? (0, telegram_contracts_2.telegramApprovalTokenPath)(tokenIdHash)
            : (0, telegram_contracts_1.managerTelegramTokenPath)(tokenIdHash);
    ...
    handleApproval: async (update, state) => {
        if (update.callbackNamespace === 'am1') {
            const repository = new firestore_repository_1.FirestoreAgentManagerRepository(admin.firestore());
            const core = new telegram_approvals_1.AgentManagerTelegramApprovalCore(new ledger_1.AgentManagerLedger(repository));
            return core.handle(update, state);
        }
        const repository = new firestore_repository_2.FirestoreAgentOfficeRepository(admin.firestore());
        const core = new telegram_approvals_2.TelegramApprovalCore(new ledger_2.AgentOfficeLedger(repository));
        return core.handle(update, state);
    },
