...
const { accountDeleteQueryPlan, accountDeleteCollectionGroupPlan, accountDeleteCollectionGroupDocumentIdPlan, accountDeleteDirectDocumentPlan, resolveStableUidForDelete, enqueueForAuthenticatedAccount, removeFromFriendGiftDailyLimits, deleteCrossUserDocumentIdMatches, deleteArenaSeasonEntries, resolveAccountDeleteIdentityClosure, } = account_delete_1.__accountDeleteTestHooks;
...
describe('accountDelete query plan', () => {
    it('exposes the idempotent deletion executor for the durable worker', () => {
        expect(typeof account_delete_1.executeAccountDeletion).toBe('function');
    });
    it('resolves stable identity on the server before enqueueing', async () => {
        ...
        const result = await enqueueForAuthenticatedAccount(db, 'auth-456', 'stable-123', enqueue);
...
describe('accountDelete stable id resolver', () => {
    it('keeps merged loser aliases in the deletion closure after hidden user docs are gone', async () => {
        ...
        const identities = await resolveAccountDeleteIdentityClosure(db, 'winner', 'auth-1');
    ...
    it('does not authorize an existing unowned legacy user document', async () => {
        ...
        await expect(resolveStableUidForDelete(db, 'auth456', 'victimStable')).resolves.toBe('auth456');
    });
    ...
    it('prefers the authoritative auth-link anchor over stale direct and firebaseAuthUid user docs', async () => {
        ...
        await expect(resolveStableUidForDelete(db, 'auth456', 'missingLocal')).resolves.toBe('serverStable');
        await expect(resolveStableUidForDelete(db, 'auth456', 'staleByAuth')).resolves.toBe('serverStable');
        await expect(resolveStableUidForDelete(db, 'auth456', 'auth456')).resolves.toBe('serverStable');
    });
    ...
    it('deletes each historical Arena season entry by stable and auth document id', async () => {
        const seasonRefs = ['2025-Q4', '2026-Q1'].map((seasonId) => ({
            ...
            collection: jest.fn((collection) => ({
                doc: (id) => ({ path: `arena_season_leaderboard/${seasonId}/${collection}/${id}` }),
            })),
        }));
        const listDocuments = jest.fn(async () => seasonRefs);
        const recursiveDelete = jest.fn(async (_ref) => { });
        const db = {
        ...
        await deleteArenaSeasonEntries(db, 'stable-123', 'auth-456', ctx, stats);
