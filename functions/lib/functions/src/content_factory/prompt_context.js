...
function normalizedIds(values) {
    if (values.some((value) => typeof value !== 'string' || !value.trim() || value.length > 260))
        throw new Error('prompt_context_invalid');
    return Object.freeze([...new Set(values.map((value) => value.trim()))].sort());
}
function buildPromptContext(input) {
    const objective = String(input.objective ?? '').trim();
    if (INSTRUCTION_RE.test(objective))
        throw new Error('prompt_context_instruction_injection');
...
//# sourceMappingURL=prompt_context.js.map