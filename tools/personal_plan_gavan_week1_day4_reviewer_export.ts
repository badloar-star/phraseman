...
} from './personal_plan_gavan_week1_day4_blueprint_candidate';

export const GAVAN_WEEK1_DAY4_REVIEWER_EXPORT_PATH = path.join(
  process.cwd(),
  '.codex-tmp',
  'personal-plans',
...
export type GavanWeek1Day4ReviewerStatus = 'needs_manual_review';

export type GavanWeek1Day4ReviewerContentUnitRow = {
  id: string;
  kind: 'content_unit';
  ...
  reviewStatus: GavanWeek1Day4ReviewerStatus;
};

export type GavanWeek1Day4ReviewerExplanationRow = {
  id: string;
...
export type GavanWeek1Day4ReviewerExerciseRow = {
  id: string;
...
export type GavanWeek1Day4ReviewerExport = {
  kind: 'gavan_week1_day4_reviewer_export';
  generatedAt: string;
  ...
  reviewStatus: GavanWeek1Day4ReviewerStatus;
  contentUnitRows: GavanWeek1Day4ReviewerContentUnitRow[];
  explanationRows: GavanWeek1Day4ReviewerExplanationRow[];
  exerciseRows: GavanWeek1Day4ReviewerExerciseRow[];
  exerciseCoverage: {
  ...
  mediaClaims: GavanWeek1Day4BlueprintCandidate['mediaClaims'];
  writePolicy: {
    dryRunOnly: true;
...
export type GavanWeek1Day4ReviewerExportOptions = {
  generatedAt: string;
};

export type GavanWeek1Day4ReviewerExportWriteOptions =
  GavanWeek1Day4ReviewerExportOptions & {
    targetPath: string;
  };

export type GavanWeek1Day4ReviewerExportIssueCode =
  | 'wrong_export_kind'
  | 'wrong_day_id'
...
export type GavanWeek1Day4ReviewerExportIssue = {
  code: GavanWeek1Day4ReviewerExportIssueCode;
  detail: string;
  target?: string;
};

export type GavanWeek1Day4ReviewerExportValidationResult = {
  valid: boolean;
  issues: GavanWeek1Day4ReviewerExportIssue[];
};

export type GavanWeek1Day4ReviewerExportWriteResult = {
  valid: boolean;
  issues: GavanWeek1Day4ReviewerExportIssue[];
  targetPath?: string;
  bytesWritten?: number;
  export?: GavanWeek1Day4ReviewerExport;
};
...
function issue(
  ...
  detail: string,
  target?: string,
): GavanWeek1Day4ReviewerExportIssue {
...
export function isGavanWeek1Day4ReviewerExportTargetAllowed(
  targetPath: string,
  cwd = process.cwd(),
): boolean {
...
function contentUnitRow(
  unit: GavanWeek1Day4ContentUnit,
...
function explanationRow(
  contentUnitId: string,
  card: GavanWeek1Day4ExplanationCard,
...
function exerciseRow(
  exercise: GavanWeek1Day4ExerciseBlueprint,
...
function visibleCopy(exportValue: GavanWeek1Day4ReviewerExport): string {
...
function forbiddenAnchors(exportValue: GavanWeek1Day4ReviewerExport): string[] {
...
function presentExerciseTypes(
  rows: GavanWeek1Day4ReviewerExerciseRow[],
): GavanWeek1ExpansionExerciseType[] {
  return rows.map((row) => row.exerciseType);
}

export function buildGavanWeek1Day4ReviewerExport(
  candidate: GavanWeek1Day4BlueprintCandidate,
  options: GavanWeek1Day4ReviewerExportOptions,
  ...
  const exportValue: GavanWeek1Day4ReviewerExport = {
    kind: 'gavan_week1_day4_reviewer_export',
...
function everyRowNeedsReview(exportValue: GavanWeek1Day4ReviewerExport): boolean {
  return [
...
export function validateGavanWeek1Day4ReviewerExport(
  exportValue: unknown,
  candidate: GavanWeek1Day4BlueprintCandidate,
): GavanWeek1Day4ReviewerExportValidationResult {
  const value = exportValue as GavanWeek1Day4ReviewerExport;
  const issues: GavanWeek1Day4ReviewerExportIssue[] = [];

  if (value.kind !== 'gavan_week1_day4_reviewer_export') {
  ...
  if (value.dayId !== 'gavan-week1-day4' || value.dayIndex !== 4) {
    issues.push(issue('wrong_day_id', 'Reviewer export must describe Gavan week 1 day 4 only.'));
  }
  ...
  candidate.contentUnits.forEach((unit) => {
    if (!exportedContentIds.has(unit.id)) {
      issues.push(issue(
        'missing_content_unit_export',
        'Every day 4 content unit must appear in the reviewer export.',
        unit.id,
      ));
    }
  });
  ...
  candidate.contentUnits.flatMap((unit) => unit.explanationCards).forEach((card) => {
    if (!exportedExplanationIds.has(card.id)) {
      issues.push(issue(
        'missing_explanation_card_export',
        'Every day 4 explanation card must appear in the reviewer export.',
        card.id,
      ));
    }
  });
  ...
  candidate.exerciseBlueprints.forEach((exercise) => {
    if (!exportedExerciseIds.has(exercise.id)) {
      issues.push(issue(
        'missing_exercise_blueprint_export',
        'Every day 4 exercise blueprint must appear in the reviewer export.',
        exercise.id,
      ));
    }
  });
  ...
  if (
    ...
    issues.push(issue(
      'fake_final_audio_claim',
      'Reviewer export cannot claim generated final audio for day 4.',
    ));
  }
  ...
  if (
    ...
    issues.push(issue(
      'fake_final_pronunciation_claim',
      'Reviewer export cannot claim built pronunciation scoring for day 4.',
    ));
  }
...
export function serializeGavanWeek1Day4ReviewerExport(
  exportValue: GavanWeek1Day4ReviewerExport,
): string {
  return `${JSON.stringify(exportValue, null, 2)}\n`;
}

export function writeGavanWeek1Day4ReviewerExport(
  candidate: GavanWeek1Day4BlueprintCandidate,
  options: GavanWeek1Day4ReviewerExportWriteOptions,
): GavanWeek1Day4ReviewerExportWriteResult {
  const exportValue = buildGavanWeek1Day4ReviewerExport(candidate, options);
  const validation = validateGavanWeek1Day4ReviewerExport(exportValue, candidate);
  ...
  if (!isGavanWeek1Day4ReviewerExportTargetAllowed(resolvedTargetPath)) {
    return {
      valid: false,
      issues: [
        issue(
          'target_path_not_allowed',
          'Gavan week 1 day 4 reviewer export can only write under .codex-tmp or docs/reports.',
        ),
      ],
    };
  }

  const serialized = serializeGavanWeek1Day4ReviewerExport(exportValue);
