...
describe('R4 quiz/challenge fake-provider smoke', () => {
    it.each([['quiz_topic', 'quiz_questions'], ['challenge_topic', 'challenge_questions']])('runs accepted %s through two independently deduplicated batches', async (topicKind, questionKind) => {
        ...
        expect(replaced.ledger.batches[`${questionKind}-batch-1`].items[0].semanticKey).toBe((0, quiz_challenge_artifacts_1.questionSemanticKey)(replacementItem));
