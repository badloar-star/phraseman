...
describe('extractPaywallFailureBreakdown', () => {
  test('returns only bounded failure groups from conversion failure rows', () => {
    const result = extractPaywallFailureBreakdown([
      {
  ...
  test('sorts deterministically, stays bounded, and freezes the public result', () => {
    ...
    const result = extractPaywallFailureBreakdown(allowedReasons.map((reason) => ({
      row_kind: 'conversion_failure',
...
describe('normalizeTrendRequest', () => {
  test('defaults to the inclusive 28-day preset with safe query defaults', () => {
    const result = normalizeTrendRequest({ scope: 'overview' }, NOW);
  ...
  test.each([{ presetDays: 7 }, { presetDays: 28 }, { presetDays: 90 }])(
    'accepts the bounded $presetDays-day preset',
    ({ presetDays }) => {
      const result = normalizeTrendRequest({ scope: 'paywall', presetDays }, NOW);

      expect(result.presetDays).toBe(presetDays);
      expect(Date.parse(`${result.fromDate}T00:00:00.000Z`))
        .toBe(Date.UTC(2026, 6, 13) - (presetDays - 1) * DAY_MS);
      expect(result.toDate).toBe('2026-07-13');
    },
  );

  test('requires a supported scope and a supported preset', () => {
    expect(() => normalizeTrendRequest({}, NOW)).toThrow(TrendValidationError);
    expect(() => normalizeTrendRequest({ scope: 'users' }, NOW)).toThrow(TrendValidationError);
    expect(() => normalizeTrendRequest({ scope: 'overview', presetDays: 14 }, NOW)).toThrow(TrendValidationError);
  });

  test('accepts strict inclusive UTC custom dates and explicit comparison settings', () => {
    const result = normalizeTrendRequest({
      scope: 'paywall',
      fromDate: '2026-07-01',
      toDate: '2026-07-13',
      granularity: 'week',
    ...
    expect(buildTrendWindow(result).current).toEqual({
  ...
  test.each([
    ...
    expect(() => normalizeTrendRequest({ scope: 'overview', ...invalid }, NOW))
      .toThrow(TrendValidationError);
  });
  ...
  test.each([
    ...
    expect(() => normalizeTrendRequest({ scope: 'overview', ...range }, NOW))
      .toThrow(TrendValidationError);
  });
  ...
  test.each([
    ...
    try {
      normalizeTrendRequest({ scope: 'overview', fromDate, toDate }, NOW);
      throw new Error('expected validation to fail');
    } catch (error) {
      expect(error).toBeInstanceOf(TrendValidationError);
  ...
  test('accepts every filter enum from the public allowlist', () => {
    for (const variant of ['A', 'B', 'C']) {
      expect(normalizeTrendRequest({ scope: 'paywall', filters: { variant } }, NOW).filters)
        .toEqual({ variant });
    }
    for (const plan of ['monthly', 'yearly', 'lifetime']) {
      expect(normalizeTrendRequest({ scope: 'paywall', filters: { plan } }, NOW).filters)
        .toEqual({ plan });
    }
    for (const store of ['APP_STORE', 'PLAY_STORE', 'STRIPE', 'AMAZON', 'PROMOTIONAL']) {
      expect(normalizeTrendRequest({ scope: 'paywall', filters: { store } }, NOW).filters)
        .toEqual({ store });
    }
    for (const platform of ['ios', 'android']) {
      expect(normalizeTrendRequest({ scope: 'paywall', filters: { platform } }, NOW).filters)
        .toEqual({ platform });
    }
  });
  ...
  test.each([
    ...
    try {
      normalizeTrendRequest({ scope: 'overview', filters }, NOW);
      throw new Error('expected validation to fail');
    } catch (error) {
      expect(error).toBeInstanceOf(TrendValidationError);
      expect(error).toMatchObject({ code: 'invalid-argument', field });
      expect((error as Error).name).toBe('TrendValidationError');
    }
  });
...
describe('UTC trend periods and buckets', () => {
  test('normalizes timestamps to UTC day starts', () => {
    expect(startOfUtcDay(Date.UTC(2026, 6, 13, 23, 59, 59, 999)))
      .toBe(Date.UTC(2026, 6, 13));
    expect(startOfUtcDay(Date.parse('0006-07-13T23:59:59.999Z')))
  ...
  test.each([
    ...
    expect(() => startOfUtcDay(timestamp)).toThrow(TrendValidationError);
    expect(() => startOfUtcWeek(timestamp)).toThrow(TrendValidationError);
  });
  ...
  test.each([
    ...
    expect(startOfUtcWeek(input)).toBe(expected);
  });
  ...
  test('returns ordered daily bucket starts intersecting the current window', () => {
    expect(bucketStarts(
      { fromMs: Date.UTC(2026, 6, 11), toMs: Date.UTC(2026, 6, 13) },
      'day',
    )).toEqual([
      Date.UTC(2026, 6, 11),
      Date.UTC(2026, 6, 12),
  ...
  test('includes the Monday bucket that intersects a mid-week range', () => {
    ...
    expect(bucketStarts(window.current, 'week')).toEqual([
      Date.UTC(2026, 5, 29),
      Date.UTC(2026, 6, 6),
      Date.UTC(2026, 6, 13),
    ]);
  ...
  test('rejects invalid range timestamps instead of returning empty buckets', () => {
    ...
    }, 'day')).toThrow(TrendValidationError);
  });
});
...
function pointValues(series: { points: readonly { value: number | null }[] }) {
  return series.points.map((point) => point.value);
}

function metric<T extends { metricId: string }>(series: readonly T[], metricId: string): T {
  const found = series.find((candidate) => candidate.metricId === metricId);
  if (!found) throw new Error(`Missing metric ${metricId}`);
  return found;
}
...
describe('honest paywall trends', () => {
  ...
  test('counts every stored step independently and excludes dev rows', () => {
    ...
    const result = aggregatePaywallTrends(rows, customRequest(), sourceHealth());
  ...
  test('returns deterministic capped context/variant/plan breakdowns with an other tail only', () => {
    ...
    expect(result.breakdowns.context.slice(0, 19).map((row) => row.value)).toEqual(
      Array.from({ length: 19 }, (_, index) => `context-${String(index).padStart(2, '0')}`),
    );
...
describe('honest RevenueCat and shard trends', () => {
  test('deduplicates production store events and keeps lifecycle outcomes separate', () => {
    ...
    const result = aggregateRevenueCatTrends([
      ...
      { eventId: 'refund-secret', eventType: 'REFUND', environment: 'PRODUCTION', eventTimestampMs: at, store: 'APP_STORE', productId: 'premium.monthly', grossUsdMicros: -2_000_000 },
  ...
  test('keeps signed gross money field-complete and outside all count series', () => {
    const result = aggregateRevenueCatTrends([
      { eventId: 'd1-buy', eventType: 'INITIAL_PURCHASE', environment: 'PRODUCTION', eventTimestampMs: Date.UTC(2026, 6, 10, 1), grossUsdMicros: 10_000_000, financialCoverage: 'partial' },
      { eventId: 'd1-refund', eventType: 'REFUND', environment: 'PRODUCTION', eventTimestampMs: Date.UTC(2026, 6, 10, 2), grossUsdMicros: -2_000_000, financialCoverage: 'complete' },
      { eventId: 'd2-buy', eventType: 'INITIAL_PURCHASE', environment: 'PRODUCTION', eventTimestampMs: Date.UTC(2026, 6, 11, 1), estimatedProceedsUsdMicros: 7_000_000, financialCoverage: 'partial' },
      { eventId: 'd3-buy', eventType: 'RENEWAL', environment: 'PRODUCTION', eventTimestampMs: Date.UTC(2026, 6, 12, 1), grossUsdMicros: 5_000_000, financialCoverage: 'complete' },
    ...
    expect(pointValues(gross)).toEqual([8_000_000, null, null, 0]);
  ...
  test('does not publish a TRANSFER series for premium or writer-shaped shard rows', () => {
    const premium = aggregateRevenueCatTrends([], customRequest(), sourceHealth({
      state: 'empty', latestAtMs: null,
    }));
    const shards = aggregateShardTrends([
  ...
  test('aggregates shard transactions as a separate production-only deduplicated source', () => {
    const at = Date.UTC(2026, 6, 13, 1);
    const result = aggregateShardTrends([
...
describe('trend completeness, freshness, and immutability', () => {
  ...
  test('distinguishes a complete empty read from an unavailable source', () => {
    const empty = aggregatePaywallTrends([], customRequest(), sourceHealth({
      state: 'empty', latestAtMs: null,
    }));
    const failed = aggregatePaywallTrends([], customRequest(), sourceHealth({
  ...
  test('reports stale event watermark as a hint without changing known zero buckets', () => {
    const latestAtMs = NOW - TREND_FRESHNESS_STALE_AFTER_MS - 1;
    const result = aggregatePaywallTrends([], customRequest(), sourceHealth({ latestAtMs }));

    expect(result.health).toMatchObject({
      latestAtMs,
      dataAgeMs: TREND_FRESHNESS_STALE_AFTER_MS + 1,
  ...
  test('freezes public results deeply enough for the immutable response contract', () => {
    const result = aggregateRevenueCatTrends([], customRequest(), sourceHealth({ state: 'empty', latestAtMs: null }));
