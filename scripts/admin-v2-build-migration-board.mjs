...
const capabilityCoverage = ADMIN_CAPABILITY_REGISTRY.map((capability) => ({
  ...
  notes: capability.nativeRoute
    ...
    : 'Функция временно открывается отдельной ссылкой на старый интерфейс до пофункционального переноса.',
}));
...
const board = {
  ...
  source: { buttons: buttons.length, functions: functions.length, links: links.length, legacyFile: 'admin/legacy.html' },
  routes,
  statusDefinitions: {
    ...
    fallback: 'Временно доступно отдельной ссылкой на старый интерфейс, пока перенос не завершён',
