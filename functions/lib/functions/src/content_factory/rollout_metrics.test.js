...
const rollout_metrics_1 = require("./rollout_metrics");
describe('content factory rollout metrics', () => {
    it('aggregates normalized quality, latency, correction and budget-proxy metrics', () => {
        const metrics = (0, rollout_metrics_1.aggregateContentFactoryRolloutMetrics)({
    ...
    it('reports operator correction as unavailable instead of a misleading zero', () => {
        const metrics = (0, rollout_metrics_1.aggregateContentFactoryRolloutMetrics)({ window: { fromMs: 1, toMs: 2 }, attempts: [{ artifactId: 'a1', accepted: true }], budgetCapUnits: 0 });
    ...
    it('defines zero-denominator behavior and rejects invalid or unbounded input', () => {
        expect((0, rollout_metrics_1.aggregateContentFactoryRolloutMetrics)({ window: { fromMs: 1, toMs: 2 }, attempts: [], budgetCapUnits: 0 })).toMatchObject({ attemptsPerAcceptedArtifact: 0, operatorCorrectionRate: 0, latencyMs: { p50: 0, p95: 0 } });
        expect(() => (0, rollout_metrics_1.aggregateContentFactoryRolloutMetrics)({ window: { fromMs: 2, toMs: 1 }, attempts: [], budgetCapUnits: 0 })).toThrow('rollout_metrics_window_invalid');
        expect(() => (0, rollout_metrics_1.aggregateContentFactoryRolloutMetrics)({ window: { fromMs: 1, toMs: 2 }, attempts: Array.from({ length: 5001 }, () => ({})), budgetCapUnits: 0 })).toThrow('rollout_metrics_input_too_large');
        expect(() => (0, rollout_metrics_1.aggregateContentFactoryRolloutMetrics)({ window: { fromMs: 1, toMs: 2 }, attempts: [{ latencyMs: -1 }], budgetCapUnits: 0 })).toThrow('rollout_metrics_attempt_invalid');
    });
    it('derives canonical bounded metrics from sanitized stage and job documents', () => {
        const metrics = (0, rollout_metrics_1.deriveContentFactoryRolloutMetricsFromDocuments)({
            nowMs: 10000,
            stageDocs: [
    ...
    it('fails closed above the 100+100 document read boundary', () => {
        expect(() => (0, rollout_metrics_1.deriveContentFactoryRolloutMetricsFromDocuments)({ nowMs: 1000, stageDocs: Array.from({ length: 101 }, (_, id) => ({ id: String(id) })), unitDocs: [], jobDocs: [], budgetCapUnits: 0 })).toThrow('rollout_metrics_document_limit_exceeded');
        expect(() => (0, rollout_metrics_1.deriveContentFactoryRolloutMetricsFromDocuments)({ nowMs: 1000, stageDocs: [], unitDocs: Array.from({ length: 101 }, (_, id) => ({ id: String(id) })), jobDocs: [], budgetCapUnits: 0 })).toThrow('rollout_metrics_document_limit_exceeded');
    });
});
//# sourceMappingURL=rollout_metrics.test.js.map