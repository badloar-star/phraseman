...
# For each file, simulate applying edits and find what's missing
missing_by_file = defaultdict(list)
for fp, edits in sorted(edits_per_file.items()):
    if not os.path.exists(fp):
        missing_by_file[fp] = edits
        continue
    with open(fp, encoding='utf-8', errors='ignore') as f:
        content = f.read()
    for old, new, sess in edits:
        if old in content:
            # This edit is NOT yet applied - it's missing
            missing_by_file[fp].append((old, new, sess))
        # if new in content - already applied, skip
        # if neither - may have been superseded by later edit, skip for now

print(f"Files with unapplied edits: {len(missing_by_file)}")
for fp, ops in sorted(missing_by_file.items(), key=lambda x: -len(x[1])):
    print(f"\n{'='*60}")
    print(f"FILE: {fp}")
    print(f"UNAPPLIED OPS: {len(ops)}")
    for i, (old, new, sess) in enumerate(ops):
        print(f"\n  [{i+1}] session={sess}")
        print(f"  OLD: {repr(old[:200])}")
        print(f"  NEW: {repr(new[:200])}")
