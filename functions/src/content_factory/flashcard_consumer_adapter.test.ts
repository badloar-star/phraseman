...
describe('flashcard consumer adapter', () => {
  ...
  it('maps the rich English/Russian draft into the existing CardItem shape without losing details', () => {
    expect(buildEnglishFlashcardDraft({ studyTarget: 'en', sourceLocale: 'ru', idea, items: [item] })).toEqual({
  ...
  it('fails closed for unsupported target/source mappings and enables the gated rich consumer', () => {
    expect(() => buildEnglishFlashcardDraft({ studyTarget: 'fr', sourceLocale: 'ru', idea, items: [item] })).toThrow('flashcard_runtime_target_not_supported');
    expect(() => buildEnglishFlashcardDraft({ studyTarget: 'en', sourceLocale: 'uk', idea, items: [item] })).toThrow('flashcard_runtime_source_not_supported');
