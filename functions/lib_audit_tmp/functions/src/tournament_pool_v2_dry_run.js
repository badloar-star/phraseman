...
const tournament_pool_v2_factory_1 = require("./tournament_pool_v2_factory");
...
function preflightRooms(tasks) {
    let taskSlots = 0;
    for (let roomIndex = 0; roomIndex < PREFLIGHT_ROOM_SEEDS; roomIndex += 1) {
        const used = new Set();
        for (let roundIndex = 0; roundIndex < tournament_core_1.TOURNAMENT_ROUND_MODE_PLAN.length; roundIndex += 1) {
            const roundNo = roundIndex + 1;
            for (const mode of tournament_core_1.TOURNAMENT_ROUND_MODE_PLAN[roundIndex]) {
                const selected = (0, tournament_core_1.selectRoundTasks)({
                ...
                if (!selected)
                    throw new Error(`new_pool_preflight_unavailable:${roomIndex}:${roundNo}:${mode}`);
                used.add(selected.taskId);
                taskSlots += 1;
            }
        }
        if (used.size !== 16)
            throw new Error(`new_pool_preflight_duplicate:${roomIndex}:${used.size}`);
    }
    return { roomSeeds: PREFLIGHT_ROOM_SEEDS, taskSlots };
}
function payloadShape(task) {
    const shape = {
        keys: Object.keys(task.payload).sort(),
        explanationReasons: task.explanation?.wrongOptionReasons?.length ?? 0,
    };
    if (Array.isArray(task.payload.options))
        shape.options = task.payload.options.length;
    if (Array.isArray(task.payload.items)) {
        shape.items = task.payload.items.length;
        shape.itemOptions = task.payload.items
            .map((item) => Array.isArray(item.options) ? item.options.length : 0);
    }
    if (Array.isArray(task.payload.rightOptions))
        shape.rightOptions = task.payload.rightOptions.length;
    if (Array.isArray(task.payload.wordBank))
        shape.wordBank = task.payload.wordBank.length;
    return shape;
}
...
function normalizedSemanticSignature(task) {
    const normalized = semanticNormalize;
    if (task.mode === 'speed_match') {
        const rightOptions = task.payload.rightOptions;
        const pairs = task.payload.items.map((item) => (`${normalized(item.prompt)}=${normalized(rightOptions[Number(item.correctIndex)])}`)).sort();
...
function selectCompleteTaskIds(roomId, tasks) {
    const used = new Set();
    for (let roundIndex = 0; roundIndex < tournament_core_1.TOURNAMENT_ROUND_MODE_PLAN.length; roundIndex += 1) {
        const roundNo = roundIndex + 1;
        for (const mode of tournament_core_1.TOURNAMENT_ROUND_MODE_PLAN[roundIndex]) {
            const task = (0, tournament_core_1.selectRoundTasks)({
            ...
            if (!task)
                throw new Error(`exposure_preflight_unavailable:${roomId}:${roundNo}:${mode}`);
            used.add(task.taskId);
        }
    }
    if (used.size !== 16)
        throw new Error(`exposure_preflight_duplicate:${roomId}:${used.size}`);
    return [...used];
}
function auditExposure(tasks, modeBucketCounts) {
    const token = {
        generation: tournament_pool_v2_factory_1.NEW_TOURNAMENT_POOL_VERSION,
        revision: 7,
    ...
    for (let dayOffset = 0; dayOffset < 730; dayOffset += 1) {
        ...
        const dayOrdinal = Math.floor(Date.parse(`${date}T00:00:00.000Z`) / 86400000);
        const slice = tournament_pool_v2_factory_1.NEW_TOURNAMENT_POOL_MODES.flatMap((mode) => (byBucket.get((0, tournaments_1.tournamentExposureBucketId)(token, mode, dayOrdinal)) ?? []));
        if (slice.length > 200)
            throw new Error(`exposure_slice_oversized:${date}:${slice.length}`);
        const ids = new Set(selectCompleteTaskIds(`daily_1200_Europe-Moscow_${date}`, slice));
        if ([...ids].some((id) => previous.has(id)))
            throw new Error(`exposure_adjacent_repeat:${date}`);
        ids.forEach((id) => exposed.add(id));
        previous = ids;
    }
    if (exposed.size !== tasks.length)
        throw new Error(`exposure_unreachable:${exposed.size}/${tasks.length}`);
...
async function initializeAdmin(serviceAccountPath) {
    if (admin.apps.length > 0)
        return;
    const raw = JSON.parse(await (0, promises_1.readFile)(serviceAccountPath, 'utf8'));
    admin.initializeApp({
...
async function main() {
    ...
    const serviceAccountPath = node_path_1.default.resolve(argValue('service-account') ?? 'service-account.json');
    const requestedOut = argValue('out');
    const runId = `dry-run-${tournament_pool_v2_factory_1.NEW_TOURNAMENT_POOL_VERSION}-${new Date().toISOString().replace(/[:.]/g, '-')}`;
    const outDir = node_path_1.default.resolve(requestedOut ?? node_path_1.default.join('.codex-tmp', 'tournament-pool-v2', runId));
    await (0, promises_1.mkdir)(outDir, { recursive: true });
    const generated = (0, tournament_pool_v2_factory_1.buildNewTournamentPool)((0, tournament_content_source_1.loadTournamentSourceDays)(tournament_content_source_1.TOURNAMENT_SOURCE_PLANS));
    const invalid = generated.tasks
        .map((task) => ({ task, validation: (0, tournament_core_1.validateTournamentTaskForNewRoom)(task) }))
        .filter((entry) => !entry.validation.ok);
    if (invalid.length > 0)
        throw new Error(`new_pool_invalid:${invalid.length}`);
    ...
    const oneTrapBuilders = builderTasks.filter((task) => (task.payload.wordBank.length - task.payload.correctTokens.length === 1)).length;
    const speedPairs = generated.tasks.filter((task) => task.mode === 'speed_match').flatMap((task) => {
        const rightOptions = task.payload.rightOptions;
        return task.payload.items.map((item) => (`${semanticNormalize(item.prompt)}\u0000${semanticNormalize(rightOptions[Number(item.correctIndex)])}`));
    });
    ...
    const sourcePoolBarrier = barrierSnapshot.exists
        ? encodeFirestoreValue(barrierSnapshot.data())
        : undefined;
    ...
    if (collisions.length > 0)
        throw new Error(`new_pool_id_collision:${collisions.length}`);
    ...
    const backupPath = node_path_1.default.join(outDir, 'old-pool-backup.ndjson');
    const newPoolPath = node_path_1.default.join(outDir, 'new-pool.ndjson');
    await (0, promises_1.writeFile)(backupPath, backupNdjson, 'utf8');
    await (0, promises_1.writeFile)(newPoolPath, newPoolNdjson, 'utf8');
    ...
    const redactedSamples = generated.tasks
        ...
        validForNewRoom: (0, tournament_core_1.validateTournamentTaskForNewRoom)(task).ok,
    }));
    const manifest = {
        ...
        artifacts: {
            backupFile: node_path_1.default.basename(backupPath),
            backupSha256: sha256(backupNdjson),
            newPoolFile: node_path_1.default.basename(newPoolPath),
    ...
    await (0, promises_1.writeFile)(node_path_1.default.join(outDir, 'manifest.json'), `${JSON.stringify(manifest, null, 2)}\n`, 'utf8');
    const qualityReport = [
        `poolVersion=${tournament_pool_v2_factory_1.NEW_TOURNAMENT_POOL_VERSION}`,
    ...
    await (0, promises_1.writeFile)(node_path_1.default.join(outDir, 'quality-report.txt'), qualityReport, 'utf8');
...
//# sourceMappingURL=tournament_pool_v2_dry_run.js.map