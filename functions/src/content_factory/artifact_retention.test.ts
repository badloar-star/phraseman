import { artifactOrphanCandidateId, buildArtifactOrphanCandidate, buildArtifactRetentionManifest } from './artifact_retention';
...
describe('artifact retention manifest', () => {
  ...
  test('rejects unbounded scans and non-dry-run planning without an explicit guard', () => {
    expect(() => buildArtifactRetentionManifest({ nowMs, candidates: Array.from({ length: 501 }, (_, index) => ({ objectPath: `x/${index}.json`, objectGeneration: '1', createdAtMs: 0 })), references: [] })).toThrow('artifact_retention_scan_limit_exceeded');
    expect(() => buildArtifactRetentionManifest({ nowMs, mode: 'delete', candidates: [], references: [] })).toThrow('artifact_retention_delete_not_authorized');
  });
  ...
  test('builds a deterministic candidate only after a lost terminal race', () => {
    ...
    expect(artifactOrphanCandidateId(receipt)).toBe(artifactOrphanCandidateId(receipt));
    expect(buildArtifactOrphanCandidate(receipt, { entityCollection: 'content_factory_stages', entityId: 'stage-1', attempt: 2, detectedAtMs: 1000 })).toMatchObject({ state: 'orphan_candidate', reason: 'terminal_lease_lost', objectPath: receipt.objectPath, objectGeneration: '9' });
  });
});
