...
exports.speakingClubReview = exports.speakingClubSend = void 0;
exports.sanitizeTargetPhrases = sanitizeTargetPhrases;
exports.buildMissionPhraseBlock = buildMissionPhraseBlock;
exports.parseClubReviewEnvelope = parseClubReviewEnvelope;
 ...
 */
function sanitizeTargetPhrases(value) {
    if (!Array.isArray(value))
        return [];
    const out = [];
 ...
 */
function buildMissionPhraseBlock(targetPhrases, studyTarget) {
 ...
 */
async function enforceClubQuota(authUid, stableUid, opts) {
    ...
    return db.runTransaction(async (tx) => {
        ...
        if (replies >= opts.dailyReplyCap) {
            ...
            throw new https_1.HttpsError('resource-exhausted', opts.isPremiumTier ? 'club_premium_cap' : 'club_free_limit');
        }
        ...
        if (opts.isNewMission && opts.dailyMissionCap != null) {
            if (missions >= opts.dailyMissionCap) {
                ...
                throw new https_1.HttpsError('resource-exhausted', 'club_mission_limit');
            }
...
async function releaseClubQuota(authUid, stableUid, charge) {
    if (!charge.replyCharged && !charge.missionMarkedNow)
        return;
    const db = admin.firestore();
    const ref = db.collection(QUOTA_COLLECTION).doc(docId('club', authUid, stableUid));
    await db.runTransaction(async (tx) => {
        ...
        if (charge.replyCharged && replies > 0)
            patch.dailyReplies = replies - 1;
        const missions = Number(data.dailyMissions ?? 0);
        if (charge.missionMarkedNow && missions > 0)
            patch.dailyMissions = missions - 1;
 ...
 */
function assertClubReplyMatchesTarget(reply, studyTarget) {
    const stripped = reply.replace(/\[\[|\]\]/g, ' ').trim();
    if (!stripped)
        return;
    try {
        (0, ai_language_contract_1.assertAiStudyLanguage)({ text: stripped, studyTarget, feature: 'premium_dialog' });
    }
    catch (e) {
...
exports.speakingClubSend = (0, https_1.onCall)({
    region: REGION,
    ...
    const [dialogModel, stableUid, clubGatedByPremium] = await Promise.all([
        ...
        (0, remote_gates_1.resolveRemoteBool)(db, 'gate_speaking_club_premium', true),
    ]);
    ...
    const [isPremium] = await Promise.all([
        ...
        (0, premium_dialog_1.enforceRateLimit)(authUid, stableUid),
    ]);
    ...
    const baseSystemPrompt = (0, premium_dialog_1.buildScenarioSystemPrompt)(cefr, {
        ...data,
    ...
    const gameMode = objectives.length > 0 && (0, openai_dialog_model_config_1.modelSupportsJsonObject)(dialogModel);
    ...
    try {
        ...
        if (gameMode) {
            const env = (0, premium_dialog_1.parseGameEnvelope)(rawContent, objectives.map((o) => o.id));
            if (env && env.reply) {
...
function sanitizeReviewHistory(value) {
    ...
    for (const raw of value.slice(-MAX_REVIEW_TURNS)) {
        ...
        if ((role === 'user' || role === 'assistant') && content) {
            result.push({ role: role, content });
        }
    }
    return result;
}
function buildClubReviewSystemPrompt(cefr, learnerLangName, goalEn, studyTarget, targetPhrases) {
...
function parseClubReviewEnvelope(raw) {
...
exports.speakingClubReview = (0, https_1.onCall)({
    region: REGION,
    enforceAppCheck: callable_options_1.ENFORCE_APP_CHECK_OPENAI,
    timeoutSeconds: 30,
    memory: '256MiB',
...
//# sourceMappingURL=speaking_club.js.map