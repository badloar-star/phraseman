...
function fakeDb(seed: Record<string, Data> = {}) {
  const docs = new Map(Object.entries(seed)); let writes = 0;
  const ref = (path: string): any => ({ path, id: path.split('/').at(-1), collection: (name: string) => ({ doc: (id = 'audit-1') => ref(`${path}/${name}/${id}`) }) });
  const db: any = {
    collection: (name: string) => ({ doc: (id = 'audit-1') => ref(`${name}/${id}`) }),
    runTransaction: async (handler: any) => handler({
      get: async (document: any) => ({ exists: docs.has(document.path), data: () => docs.get(document.path), ref: document }),
      set: (document: any, data: Data) => { writes += 1; docs.set(document.path, data); },
      create: (document: any, data: Data) => { if (docs.has(document.path)) throw new Error('already_exists'); writes += 1; docs.set(document.path, data); },
    }),
  };
...
describe('surface convergence repository', () => {
  it('updates config with CAS and an audit in one transaction', async () => {
    ...
    const result = await updateArenaConvergenceConfig(db, { expectedRevision: 4, mode: 'legacy', actorUid: 'admin-1', role: 'owner', disabledReason: 'operator_stop', requestId: 'r1', nowIso: '2026-07-13T00:00:00.000Z', serverTimestamp: 'SERVER_TIME' });
  ...
  it('persists the same comparison receipt once without touching unit state', async () => {
    ...
    const receipt = { documentId: 'a'.repeat(64), unitId: 'u1', comparatorVersion: 'arena-parity-v1', eligible: true } as any;
    expect(await persistArenaComparisonReceipt(db, receipt, 'SERVER_TIME')).toEqual({ created: true, documentId: receipt.documentId });
    expect(await persistArenaComparisonReceipt(db, receipt, 'SERVER_TIME')).toEqual({ created: false, documentId: receipt.documentId });
