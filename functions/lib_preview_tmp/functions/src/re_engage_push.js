...
function parseReEngageUser(uid, data) {
    ...
    const pushPrefs = prefsRaw && typeof prefsRaw === 'object'
        ? {
            streak: typeof prefsRaw.streak === 'boolean'
                ? prefsRaw.streak
                : undefined,
            offers: typeof prefsRaw.offers === 'boolean'
                ? prefsRaw.offers
                : undefined,
        }
        : null;
...
async function runReEngagePush(now = Date.now()) {
    ...
    while (true) {
        ...
        let query = db.collection('users')
            .orderBy('__name__')
            .limit(PAGE_SIZE)
            .select('expoPushToken', 'pushTokenLang', 'pushTokenTimezone', 'last_active_at', 'lastReEngagePushAt', 'progress.streak_count', 'progress.user_name', 'pushPrefs');
        if (lastDoc)
