...
export const E1_DEMO_EPISODE_ID = 'ep-01';
export const E1_DEMO_CAN_DO_OUTCOME_ID = 'obj-introduce-self';
...
export function buildE1DemoProfile(): V2LanguageProfileBody {
  ...
  if (!result.ok) throw new Error(`e1_demo_profile_invalid: ${result.issues.join(',')}`);
  cachedProfile = result.value;
  return result.value;
}

export function buildE1DemoItems(): readonly V2ContentItem[] {
  ...
  const items = DEMO_SEEDS.map((seed, index) => {
    const result = validateV2ContentItem({
      ...
      objectiveIds: [E1_DEMO_CAN_DO_OUTCOME_ID],
      compatibleFamilies: [...FAMILY_ROTATION[index % FAMILY_ROTATION.length]],
    });
    if (!result.ok) throw new Error(`e1_demo_item_invalid: ${result.issues.join(',')}`);
    return result.value;
  });
