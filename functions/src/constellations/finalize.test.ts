...
type FakeRef = { path: string; collection: (name: string) => FakeCol };
type FakeCol = { doc: (id?: string) => FakeRef };
...
function makeCol(path: string): FakeCol {
  return {
    doc: (id?: string) => makeRef(`${path}/${id ?? `auto_${autoId += 1}`}`),
  };
}

function mergeDeep(existing: DocData, patch: DocData): DocData {
  ...
  for (const [k, v] of Object.entries(patch)) {
    if (v && typeof v === 'object' && !Array.isArray(v) && existing[k]
      ...
      next[k] = mergeDeep(existing[k] as DocData, v as DocData);
    } else {
      next[k] = v;
    }
  }
          ...
          writes.push(() => {
            ...
            for (const [k, v] of Object.entries(data)) {
              if (k.includes('.')) {
                ...
                cur[head] = { ...((cur[head] as DocData) ?? {}), [tail]: v };
              } else {
                cur[k] = v;
              }
            }
...
function shardLogEntries(): DocData[] {
  const out: DocData[] = [];
  for (const [path, data] of docs.entries()) {
    if (path.includes(`users/${HUMAN}/shard_log/`)) out.push(data);
  }
  return out;
}
...
describe('finalizeConstellationMatch — идемпотентность (7.1)', () => {
  ...
  test('двойной finalize начисляет осколки РОВНО раз', async () => {
    ...
    const afterFirst = Number((docs.get(`users/${HUMAN}`) as DocData).shards);
    const xpAfterFirst = Number((docs.get(`arena_profiles/${HUMAN}`) as DocData).xp);
    const logAfterFirst = shardLogEntries().length;
    const processedAt = (docs.get(`constellation_matches/${MATCH_ID}`) as DocData).resultProcessedAt;
    ...
    await finalizeConstellationMatch(MATCH_ID);
    const afterSecond = Number((docs.get(`users/${HUMAN}`) as DocData).shards);
    const xpAfterSecond = Number((docs.get(`arena_profiles/${HUMAN}`) as DocData).xp);
    ...
    expect((docs.get(`constellation_matches/${MATCH_ID}`) as DocData).resultProcessedAt).toBe(processedAt);
  });
  ...
  test('тройной finalize так же безопасен (watchdog дёргает многократно)', async () => {
    ...
    const { finalizeConstellationMatch } = require('./match_service');
    await finalizeConstellationMatch(MATCH_ID);
    const shards1 = Number((docs.get(`users/${HUMAN}`) as DocData).shards);
    const log1 = shardLogEntries().length;
    await finalizeConstellationMatch(MATCH_ID);
    await finalizeConstellationMatch(MATCH_ID);
    expect(Number((docs.get(`users/${HUMAN}`) as DocData).shards)).toBe(shards1);
  ...
  test('finalize на НЕ finished матче ничего не начисляет', async () => {
    ...
    const m = docs.get(`constellation_matches/${MATCH_ID}`) as DocData;
    docs.set(`constellation_matches/${MATCH_ID}`, { ...m, stage: 'active' });
    const { finalizeConstellationMatch } = require('./match_service');
    await finalizeConstellationMatch(MATCH_ID);
    expect(Number((docs.get(`users/${HUMAN}`) as DocData).shards)).toBe(50);
    expect(shardLogEntries().length).toBe(0);
    expect((docs.get(`constellation_matches/${MATCH_ID}`) as DocData).resultProcessedAt).toBeUndefined();
  });
});
