...
describe('content factory generation error taxonomy', () => {
    ...
    ])('classifies %s as %s with retryable=%s', (message, code, retryable) => {
        expect((0, generation_errors_1.classifyGenerationError)(new Error(message))).toEqual({ code, retryable });
    });
    it('removes secrets and control characters and caps stored detail', () => {
        ...
        const detail = (0, generation_errors_1.sanitizeGenerationErrorDetail)(raw);
    ...
    it('builds a stable attempt record with normalized retry semantics', () => {
        expect((0, generation_errors_1.buildGenerationFailureRecord)(new Error('provider_rate_limit: retry later'), 2, '2026-07-12T12:00:00.000Z')).toEqual({
            attempt: 2,
...
//# sourceMappingURL=generation_errors.test.js.map