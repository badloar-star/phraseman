...
describe('help_board contract helpers', () => {
    it('scopes boards by study target and interface language', () => {
        ...
        expect((0, help_board_1.normalizeHelpBoardScope)('fr', 'PT-br')).toEqual({
            targetLang: 'fr',
            uiLang: 'pt-BR',
            boardKey: 'fr:pt-BR',
        });
        expect(() => (0, help_board_1.normalizeHelpBoardScope)('en', 'xx')).toThrow('help_board_unsupported_language');
    });
    it('keeps ranking responsive to votes, comments, reports and age', () => {
        ...
        expect((0, help_board_1.helpBoardHotScore)({ ...base, helpfulScore: 4 }, now)).toBeGreaterThan((0, help_board_1.helpBoardHotScore)(base, now));
        expect((0, help_board_1.helpBoardHotScore)({ ...base, commentCount: 5 }, now)).toBeGreaterThan((0, help_board_1.helpBoardHotScore)(base, now));
        expect((0, help_board_1.helpBoardHotScore)({ ...base, reportCount: 2 }, now)).toBeLessThan((0, help_board_1.helpBoardHotScore)(base, now));
        expect((0, help_board_1.helpBoardHotScore)(base, now + 72 * 3600000)).toBeLessThan((0, help_board_1.helpBoardHotScore)(base, now));
        expect((0, help_board_1.helpBoardBestScore)({ helpfulScore: 5, commentCount: 3, reportCount: 0 }))
            .toBeGreaterThan((0, help_board_1.helpBoardBestScore)({ helpfulScore: 5, commentCount: 3, reportCount: 2 }));
    });
    it('blocks links, private contact patterns and obvious spam before publication', () => {
        expect((0, help_board_1.moderateHelpBoardText)('Please check https://example.com', 200).reasons).toContain('external_link');
        expect((0, help_board_1.moderateHelpBoardText)('Write me at person@example.com', 200).reasons).toContain('external_contact');
    ...
    it('validates Compass output deterministically (replaces the off_topic LLM judge)', () => {
        ...
        expect((0, help_board_1.validateCompassAnswer)(ru, 'ru')).toEqual({ ok: true });
        // Огрызок.
        expect((0, help_board_1.validateCompassAnswer)('Ок.', 'ru')).toEqual({ ok: false, reason: 'too_short' });
        // Ссылки/контакты в ответе запрещены.
        expect((0, help_board_1.validateCompassAnswer)(`${ru} Подробнее: https://example.com`, 'ru'))
            .toEqual({ ok: false, reason: 'unsafe_output' });
        // Ответ не на языке сообщества (для кириллических языков).
        expect((0, help_board_1.validateCompassAnswer)('This is a long answer written entirely in English for a Russian board.', 'ru'))
            .toEqual({ ok: false, reason: 'non_target_language' });
        // Для латинских языков сообщества языковой порог не применяется.
        expect((0, help_board_1.validateCompassAnswer)('Great question! Use Past Simple with yesterday.', 'en')).toEqual({ ok: true });
    });
    it('exports the policy version for client and legal consent gates', () => {
        expect(help_board_1.__helpBoardTestHooks.HELP_BOARD_POLICY_VERSION).toBe(1);
        expect(help_board_1.__helpBoardTestHooks.helpBoardBoardKey('en', 'ru')).toBe('en:ru');
    });
});
//# sourceMappingURL=help_board.test.js.map