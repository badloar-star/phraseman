# Update russian/ukrainian in LESSON_29_PHRASES from lesson_29_user.md translation lines
import pathlib
import re

ROOT = pathlib.Path(__file__).resolve().parent.parent
md_path = ROOT / "scripts" / "lesson_29_user.md"
...
# Replace within LESSON_29_PHRASES[ ... ] block only
m_start = re.search(r"export const LESSON_29_PHRASES", text)
m_end = re.search(r"^\];", text[m_start.end() :], re.M) if m_start else None
if not m_start or m_end is None:
    raise SystemExit("Could not find LESSON_29_PHRASES block")
...
print("Updated LESSON_29_PHRASES russian/ukrainian for 50 items")
