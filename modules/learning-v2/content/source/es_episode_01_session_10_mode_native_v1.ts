...
// + СТАРТ ES §0/§10, Глава 2 "Ты: вопрос"): испанская сессия 10 переписывается
// в mode-native формат. Как и сессия 7 (voice), сессия 10 НЕ вводит новых слов
// и не имеет newVocabulary — все 15 фраз применения оборачивают уже известную
// лексику (eres из сессии 9, es/soy/fácil/verdad из сессии 1, bonito/bonita
// из сессии 3, rápido/rápida из сессии 5, único/única из сессии 6) в
// испанский вопрос ¿...? без инверсии порядка слов.
//
// зачем именно этот набор families и порядок (владелец, 2026-08-27, зеркало
// легаси generic phraseSteps() по составу и индексам — единственная разница
// в том, что каждый шаг здесь получает РЕАЛЬНЫЙ mode-native payload вместо
// generic-карточки): phraseSteps() уже использует только утверждённые
...
// сохранён 1:1, вместо переписывания принципиально другим образом.
// 12 шагов на индексах 3-14 из 15 фраз (0-2 уже отработаны в introSteps()).
const phrases = ES_EPISODE_01_SESSION_10_PHRASES;
if (phrases.length !== 15) {
  throw new Error('es_session_10_mode_native_phrase_count_invalid');
}

const L = (source: LocalizedSource): LearningV2Localized<string> => expandLocalized(source);

// зачем эта функция (владелец, 2026-08-27, тот же класс бага сессий 5/6/8/9):
...
// difícil), context_gap_grammar передаёт responseId напрямую в runtime.
...
// зачем 'en' вместо 'es' здесь (как в испанских сессиях 1-9): в испанском
...
function localizedPhraseField(
  ...
  if (!phrase?.localizedDetails) throw new Error(`es_session_10_phrase_details_missing:${phraseIndex}`);
  return Object.fromEntries(LEARNING_V2_INTERFACE_LOCALES.map((locale) => {
    ...
    if (!detail) throw new Error(`es_session_10_phrase_locale_missing:${phraseIndex}:${locale}`);
...
function localizedPhraseDistractorFeedback(
  ...
  return Object.fromEntries(LEARNING_V2_INTERFACE_LOCALES.map((locale) => {
    ...
    if (!reason) throw new Error(`es_session_10_phrase_distractor_missing:${phraseIndex}:${locale}:${value}`);
...
function phraseAudio(phraseIndex: number): LearningV2ModeAudioReferenceV1 {
  const phrase = phrases[phraseIndex]!;
  return authoredAudio(`es-e01-s10-phrase-${phraseIndex + 1}`, phrase.english);
}
function phraseSlowAudio(phraseIndex: number): LearningV2ModeAudioReferenceV1 {
  const phrase = phrases[phraseIndex]!;
  return authoredAudio(`es-e01-s10-phrase-${phraseIndex + 1}-slow`, phrase.english);
}
...
function contextGapLastWord(phraseIndex: number): LearningV2ModeNativePayloadV1 {
  ...
  return Object.freeze({
    ...
    testedDimension: `question_form:${asciiId(phrase.id)}`,
    choiceFeedback: Object.freeze([
      ...
      ...lastWord.distractors.map((entry) => feedback(
        ...
        false,
        `question_form:${asciiId(entry.reasonCode)}`,
...
// вопросов, разбросанных по всей сессии (индексы 3/6/9/12/14) — широкий
// срез уже изученных вопросительных фраз, а не смежный блок.
function speedMatchPhrases(pairIndices: readonly number[]): LearningV2ModeNativePayloadV1 {
  ...
  const ids = pairIndices.map((index) => `es-e01-s10-pair-${index + 1}`);
  return Object.freeze({
    family: 'speed_match',
...
// легаси generic phraseSteps()): те же 12 families/purposes/индексов
// (3-14), что и в phraseSteps(), но с реальным mode-native payload вместо
// generic-карточки для каждого шага.
export const ES_EPISODE_01_SESSION_10_MODE_NATIVE_PRACTICE_V1 = Object.freeze<readonly SessionModeNativePracticeSourceV1[]>([
...
if (ES_EPISODE_01_SESSION_10_MODE_NATIVE_PRACTICE_V1.length !== 12) {
  throw new Error('es_session_10_mode_native_practice_count_invalid');
}
