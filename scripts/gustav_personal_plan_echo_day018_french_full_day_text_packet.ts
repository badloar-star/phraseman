...
const DAY_INDEX = 18 as const;
const GENERATED_BY = 'gustav_personal_plan_echo_day018_french_full_day_text_packet' as const;

const EXPLANATIONS = [
  {
    phraseId: 'echo_d18_p1',
    title: 'Comment commencer à parler d’une nouvelle',
    rule: 'I think est le début d’une opinion. Ensuite, tu dis ce que tu penses.',
    why: 'C’est la façon la plus simple et la plus claire de dire « je pense » en anglais.',
    commonMistake: 'Ne dis pas « I am think » : am est en trop ici. Dis seulement « I think ».',
  },
  {
    phraseId: 'echo_d18_p2',
    title: 'Dire « je crois » avec un autre mot',
    rule: 'I believe est proche de I think, mais un peu plus formel. Les deux signifient « je crois ».',
    why: 'Can montre une possibilité : quelque chose est capable d’arriver ou d’aider.',
    commonMistake: 'Ne dis pas « can to help » : après can, il n’y a pas to. Dis seulement « can help ».',
  },
  {
    phraseId: 'echo_d18_p3',
    title: 'Parler de l’impact d’une nouvelle',
    rule: 'Can change signifie « peut changer ». Can + verbe : toujours sans terminaison.',
    why: 'Ainsi, tu montres que la nouvelle est importante et peut avoir un effet sur la vie.',
    commonMistake: 'Ne dis pas « can changes » : après can, le verbe ne prend jamais -s.',
  },
  {
    phraseId: 'echo_d18_p4',
    title: 'Dire honnêtement que tu n’as pas compris',
    rule: 'I do not signifie « je ne ... pas ». C’est ainsi que l’on fait la négation avec I. Ensuite vient le verbe.',
    why: 'Dire « je ne comprends pas » est utile et honnête. C’est une partie normale de la conversation.',
    commonMistake: 'Ne dis pas « I not understand » : il faut do not. C’est la règle pour I, you, we et they.',
  },
  {
    phraseId: 'echo_d18_p5',
    title: 'Dire que quelque chose est important pour tous',
    rule: 'Should signifie « devrait » ou « devraient ». Tu dis ce qu’il serait bon ou juste de faire.',
    why: 'C’est poli et doux : ce n’est pas un ordre, mais ton opinion sur ce qui compte.',
    commonMistake: 'Ne dis pas « should knows » : après should, le verbe ne prend jamais de terminaison.',
  },
  {
    phraseId: 'echo_d18_p6',
    title: 'Dire que le sujet t’intéresse',
    rule: 'I want to signifie « je veux ». Après, on met le verbe d’action. La structure reste toujours la même.',
    why: 'More about this signifie « plus à ce sujet ». Cela montre ton intérêt de façon courte et claire.',
    commonMistake: 'Ne dis pas « I want know » : il faut « want to know ». Ici, to est obligatoire.',
  },
] as const;

const VOCAB_TRANSLATIONS = [
  { word: 'important', frenchText: 'important' },
  { word: 'believe', frenchText: 'croire' },
  { word: 'change', frenchText: 'changer' },
  { word: 'understand', frenchText: 'comprendre' },
  { word: 'problem', frenchText: 'problème' },
  { word: 'more', frenchText: 'plus' },
] as const;
...
const FIELD_SPECS: FieldSpec[] = [
  {
    fieldPath: 'topic',
    fieldKind: 'day_topic',
    frenchText: 'Exprimer une opinion sur une nouvelle',
    source: (day) => day.topic,
  },
  {
    fieldPath: 'outcome',
    fieldKind: 'day_outcome',
    frenchText: 'Tu peux dire ce que tu penses d’une nouvelle en anglais, de façon simple et sûre.',
    source: (day) => day.outcome,
  },
  {
    fieldPath: 'intro[0].title',
    fieldKind: 'intro_title',
    frenchText: 'Comment exprimer ton opinion',
    source: (day) => day.intro?.[0]?.title,
  },
  {
    fieldPath: 'intro[0].body',
    fieldKind: 'intro_body',
    frenchText: 'Pour dire ce que tu penses, commence par « I think » (je pense) ou « I believe » (je crois). Ensuite, dis simplement ce que tu penses. Cela fonctionne pour n’importe quelle nouvelle.',
    source: (day) => day.intro?.[0]?.body,
  },
  {
    fieldPath: 'intro[0].examples[0].gloss',
    fieldKind: 'intro_example_gloss',
    frenchText: 'Je pense que c’est une nouvelle importante.',
  ...
  },
  {
    ...
    fieldKind: 'intro_example_gloss',
    frenchText: 'Je crois que cela peut aider beaucoup de personnes.',
  ...
  },
  {
    ...
    fieldKind: 'intro_title',
    frenchText: 'Le mot « can » est ton allié',
    source: (day) => day.intro?.[1]?.title,
  },
  {
    fieldPath: 'intro[1].body',
    fieldKind: 'intro_body',
    frenchText: 'Can signifie « peut » ou « on peut ». Il aide à parler de possibilités : quelque chose peut arriver ou être fait. Après can, il y a toujours un autre verbe sans terminaison.',
    source: (day) => day.intro?.[1]?.body,
  },
  {
    fieldPath: 'intro[1].examples[0].gloss',
    fieldKind: 'intro_example_gloss',
    frenchText: 'Cette nouvelle peut changer beaucoup de choses.',
  ...
  },
  {
    ...
    fieldKind: 'intro_example_gloss',
    frenchText: 'Les gens peuvent apprendre de cette histoire.',
  ...
  },
  {
    ...
    fieldKind: 'intro_title',
    frenchText: 'Parler du sujet : « about this »',
    source: (day) => day.intro?.[2]?.title,
  },
  {
    fieldPath: 'intro[2].body',
    fieldKind: 'intro_body',
    frenchText: 'About signifie « sur » ou « à propos de ». About this veut dire « à propos de cela ». C’est très utile quand tu parles déjà d’une nouvelle et que tu veux ajouter quelque chose sans répéter le nom.',
    source: (day) => day.intro?.[2]?.body,
  },
  {
    fieldPath: 'intro[2].examples[0].gloss',
    fieldKind: 'intro_example_gloss',
    frenchText: 'Je veux en savoir plus à ce sujet.',
  ...
  },
  {
    ...
    fieldKind: 'intro_example_gloss',
    frenchText: 'Que penses-tu de cette nouvelle ?',
...
function rowIdFromPath(fieldPath: string): string {
  return `fr_echo_d018_${fieldPath.replace(/[^a-zA-Z0-9]+/g, '_').replace(/^_+|_+$/g, '').toLowerCase()}`;
}

function renderMarkdown(report: any): string {
  return [
    '# GUSTAV Personal Plan Echo Day 018 French Full-Day Text Packet',
    '',
    `Run: \`${report.runId}\``,
    '',
...
async function main(): Promise<void> {
  ...
  if (!runArg) {
    throw new Error('Usage: npx tsx scripts/gustav_personal_plan_echo_day018_french_full_day_text_packet.ts --run <run-dir>');
  }
  ...
  const rowsPath = path.join(outDir, 'echo_day_018_full_day_fr_rows.jsonl');
  const reviewerQueuePath = path.join(outDir, 'echo_day_018_full_day_fr_reviewer_queue.tsv');
  const packetJsonPath = path.join(auditsDir, 'personal_plan_echo_day018_french_full_day_text_packet.json');
  const packetMdPath = path.join(auditsDir, 'personal_plan_echo_day018_french_full_day_text_packet.md');

  const findings: Finding[] = [];
  const day = await readEchoDay(repoRoot);
  if (!day) addFinding(findings, 'blocker', 'echo_day_018_missing', 'Echo day 18 is missing from source content.', rel(repoRoot, sourcePath));

  let existingPhraseMeaningRowsForDay = 0;
  if (!fs.existsSync(phraseMeaningRowsPath)) {
    addFinding(findings, 'blocker', 'phrase_meaning_rows_missing', 'Echo day 18 phrase meaning rows are missing.', rel(repoRoot, phraseMeaningRowsPath));
  } else {
    ...
    if (existingPhraseMeaningRowsForDay !== 6) addFinding(findings, 'blocker', 'phrase_meaning_rows_day_018_incomplete', `Expected 6 reviewer-needed phrase meaning rows for Echo day 18 but found ${existingPhraseMeaningRowsForDay}.`, rel(repoRoot, phraseMeaningRowsPath));
  }
  ...
  const report = {
    schemaVersion: 'gustav-personal-plan-echo-day018-french-full-day-text-packet-v0',
    ...
    fieldKindCounts,
    generationPolicy: [
      'This packet translates Echo day 18 full-day text fields except phrase meanings, which are already covered by the phrase-meaning layer.',
  ...
  console.log(`GUSTAV Echo day 018 French full-day text packet: ${report.status}`);
