...
import { getPaywallCopy, getHeroPlannedCopy, makeLP, applyWinBackCopy, applyWinBackPlannedCopy } from './paywall_copy';
...
export function PaywallCView({ entry }: { readonly entry: PaywallEntry }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      // некритично
      DebugLogger.error('paywall_c:tags', e instanceof Error ? e : new Error(String(e)), 'warning');
    }
      try {
      ...
      // некритично
      DebugLogger.error('paywall_c:m', e instanceof Error ? e : new Error(String(e)), 'warning');
    }
      try {
      ...
      // некритично
      DebugLogger.error('paywall_c:prof', e instanceof Error ? e : new Error(String(e)), 'warning');
    }
    })();
    ...
    try {
      ...
      // некритично
      DebugLogger.error('paywall_c:dayHash', e instanceof Error ? e : new Error(String(e)), 'warning');
    }
    return () => { dead = true; };
  }, [ctx, lang, source]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      // нет данных — молчим
      DebugLogger.error('paywall_c:line', e instanceof Error ? e : new Error(String(e)), 'warning');
    }
    })();
    return () => { dead = true; };
      ...
      <SafeAreaView style={S.safe}>
        <View style={S.wrap}>
          ...
          >
            ...
            <PaywallEntrance index={10} style={S.repeatCta}>
              <PaywallCtaBlock
                lang={lang as Lang}
                chrome={chrome}
                label={ctaLabel}
...
export default function PaywallCRoute() {
  return <DirectPaywallRoute render={(entry) => <PaywallCView entry={entry} />} />;
}
