...
function qaV2EpisodeContent(compiled, contentItems, profile, activityBindings, optionalPracticeTemplates = [], languageProfileRef = null) {
    const blocking = [];
    const warnings = [];
    ...
    const approvedActivityCoordinates = new Set(activityBindings.flatMap((binding) => binding.contentUnitIds.map((contentUnitId) => `${binding.activityId}\u0000${binding.family}\u0000${contentUnitId}`)));
