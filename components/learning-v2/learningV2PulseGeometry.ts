 ...
 * Шаг строки карты.
 *
 * зачем: владелец 21.09 — «названия уроков убирай, вместо названий
 * нумерация». Подписи под кружками удалены, номер занятия написан ВНУТРИ
 * кружка. Вместе с подписью ушёл и блок высоты под неё (был 40px): строка
 * стала ниже, на экран влезает больше кружков, и при быстрой прокрутке карта
 * успевает рисоваться («экран при быстром скролле не успевает рисоваться»).
 *
 * Запас 12px оставляем как воздух между кружками — без него соседние узлы
 * смыкаются и тропа читается как сплошная колбаса.
 * Базовую раскладку (learningV2OwnerLayout) не трогаем: mapStep там общий.
 */
const MAP_STEP_WITH_LABEL = Math.max(mapStep, nodeSize + 12);
/**
