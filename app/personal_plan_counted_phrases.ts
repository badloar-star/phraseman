  ...
  const run = writeChain.then(async (): Promise<number> => {
    try {
      const isCurrent = () => {
        ...
        return current.generation === operationAccount.generation &&
          current.stableId === operationAccount.stableId && current.phase === operationAccount.phase;
      };
      ...
      if (!isCurrent()) {
        await AsyncStorage.removeItem(COUNTED_PHRASES_KEY).catch(() => {});
        return 0;
      }
      return fresh.length;
    } catch {
