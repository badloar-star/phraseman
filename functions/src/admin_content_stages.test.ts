...
describe('admin independent content stage callables', () => {
  ...
  it('rejects client-owned prompt, model, schema and QA fields', () => {
    for (const forbidden of ['model', 'promptVersion', 'schemaVersion', 'qaPolicy', 'qaReceipt', 'artifactId']) {
      expect(() => parseContentStageCreateRequest({ requestId: 'request-1', kind: 'quiz_topic', studyTarget: 'fr', sourceLocale: 'ru', cefr: 'A1', objective: 'Travel', scopeId: 'topic-1', count: 1, revision: 1, prerequisiteStageIds: [], [forbidden]: 'attacker-value' })).toThrow(HttpsError);
    }
  });

  it('builds a plan only from approved server prerequisite documents', () => {
    const plan = stagePlanFromStoredPrerequisites(parseContentStageCreateRequest({ requestId: 'request-1', kind: 'quiz_questions', studyTarget: 'fr', sourceLocale: 'ru', cefr: 'A1', objective: 'Identity', scopeId: 'topic-1', count: 10, revision: 1, prerequisiteStageIds: ['topic-stage'] }), [{ stageId: 'topic-stage', requestId: 'request-1', kind: 'quiz_topic', artifactId: 'topic-artifact', state: 'approved', studyTarget: 'fr', sourceLocale: 'ru', scopeId: 'topic-1' }]);
  ...
  it.each([1, 9, 11, 100])('rejects client question batch count %i before planning', (count) => {
    expect(() => parseContentStageCreateRequest({ requestId: 'request-quiz', kind: 'quiz_questions', studyTarget: 'en', sourceLocale: 'ru', cefr: 'A2', objective: 'Travel', scopeId: 'travel', count, revision: 1, prerequisiteStageIds: ['topic-stage'] })).toThrow('stage_capability_count_unsupported');
  });

  it('accepts a bounded one-question replacement identity only for replacement stages', () => {
    const input = parseContentStageCreateRequest({ requestId: 'request-quiz', kind: 'quiz_question_replacement', studyTarget: 'en', sourceLocale: 'ru', cefr: 'A2', objective: 'Travel', scopeId: 'travel.replace.q-1', count: 1, revision: 1, prerequisiteStageIds: ['batch-stage'], replacementForQuestionId: 'q-1' });
    expect(input.replacementForQuestionId).toBe('q-1');
    expect(() => parseContentStageCreateRequest({ requestId: 'request-quiz', kind: 'quiz_questions', studyTarget: 'en', sourceLocale: 'ru', cefr: 'A2', objective: 'Travel', scopeId: 'travel', count: 10, revision: 1, prerequisiteStageIds: ['topic-stage'], replacementForQuestionId: 'q-1' })).toThrow('question_replacement_identity_invalid');
  });
  ...
  it('selects Arena topic v2 and question quality v4, and rejects unsupported C1 or non-ten batches', () => {
    const topic = stagePlanFromStoredPrerequisites(parseContentStageCreateRequest({ requestId: 'request-arena', kind: 'arena_topic', studyTarget: 'en', sourceLocale: 'ru', cefr: 'A2', objective: 'Fast city', scopeId: 'city-a2', count: 1, revision: 1, prerequisiteStageIds: [] }), []);
    expect(topic.unit).toMatchObject({ schemaVersion: 2, promptVersion: 'v2', qaPolicy: 'arena-studio-quality-v2' });
    const questions = stagePlanFromStoredPrerequisites(parseContentStageCreateRequest({ requestId: 'request-arena', kind: 'arena_questions', studyTarget: 'en', sourceLocale: 'ru', cefr: 'A2', objective: 'Fast city', scopeId: 'city-a2', count: 10, revision: 1, prerequisiteStageIds: ['topic-stage'] }), [
    ...
    expect(() => parseContentStageCreateRequest({ requestId: 'request-arena', kind: 'arena_topic', studyTarget: 'en', sourceLocale: 'ru', cefr: 'C1', objective: 'Fast city', scopeId: 'city-c1', count: 1, revision: 1, prerequisiteStageIds: [] })).toThrow('stage_capability_cefr_unsupported');
  ...
  it('rejects an approved prerequisite from another language or scope', () => {
    ...
    expect(() => stagePlanFromStoredPrerequisites(input, [{ stageId: 'topic-stage', requestId: 'request-1', kind: 'quiz_topic', artifactId: 'topic-artifact', state: 'approved', studyTarget: 'fr', sourceLocale: 'ru', scopeId: 'topic-2' }])).toThrow('generation_stage_prerequisite_identity_mismatch');
  });

  it('fingerprints every generation input and sorted prerequisite identity', () => {
    const input = parseContentStageCreateRequest({ requestId: 'request-1', kind: 'quiz_questions', studyTarget: 'fr', sourceLocale: 'ru', cefr: 'A1', objective: 'Identity', scopeId: 'topic-1', count: 10, revision: 1, prerequisiteStageIds: ['b', 'a'] });
  ...
  it('parses only explicit pause/resume/cancel controls', () => {
    expect(parseContentStageControlRequest({ stageId: 'request-1:quiz_topic:topic-1:r1', action: 'pause' })).toEqual({ stageId: 'request-1:quiz_topic:topic-1:r1', action: 'pause' });
