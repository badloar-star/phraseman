...
- одну новую микрограмматическую операцию в каждой сессии: она заранее
  объявлена, полностью объяснена тремя интро и не появляется скрыто в practice;
...
Размер словарной сетки обновлён владельцем 2026-08-28: один экран использует
не более четырёх разных уже введённых word-first пар. Для текущих сессий 1–3
используются ровно четыре слова; delayed return из предыдущих сессий допустим,
но не расширяет экран до пятой пары. Незнакомый target или повтор пары ради
заполнения сетки считается curriculum leakage.
...
Builder modes работают только с целыми словами/чанками. Буквенная сборка и
смешение букв с цельными словами запрещены независимо от того, насколько
«похожими» кажутся дистракторы. Для формы одного слова применяется whole-word
choice/audio/voice, а не раскладывание орфографии на буквы.
