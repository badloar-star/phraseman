...
describe('DEV center bottom sheet', () => {
  ...
  test('renders a deterministic typed registry that can grow by category', () => {
    ...
    expect(ordered.map((section: { id: string }) => section.id)).toEqual([
      // Празднование ПЕРВОЙ покупки — самая первая секция (владелец,
      // 2026-09-21): «в девхаб в самом вверху добавь обе анимации, чтобы я мог
      // запустить и проверить их». Выше празднования Plus, потому что это
      // свежая правка, которую владелец проверяет прямо сейчас; сам праздник
      // виден лишь один раз за товар, иначе его не вызвать.
      'first-purchase-celebration',
      // Празднование Plus/Pro (владелец, 2026-09-20):
