export type DirectorNarrativeState = 'ready' | 'empty' | 'partial' | 'unavailable';
export type DirectorNarrativeMetricAvailability = 'ready' | 'empty' | 'partial' | 'unavailable';
export type DirectorNarrativeMetricDirection = 'up' | 'down' | 'flat' | 'new' | 'unavailable';

export interface DirectorDigestNarrativeMetric {
  readonly id: string;
  readonly source: 'paywall_funnel' | 'revenuecat_premium_events';
  readonly unit: 'count' | 'usd_micros';
  readonly availability: DirectorNarrativeMetricAvailability;
  readonly current: number | null;
  readonly previous: number | null;
  readonly absoluteDelta: number | null;
  readonly percentDelta: number | null;
  readonly direction: DirectorNarrativeMetricDirection;
}

export interface DirectorDigestNarrativeSourceHealth {
  readonly source: 'paywall' | 'premium_event_time' | 'premium_created_at';
  readonly state: 'ready' | 'empty' | 'partial' | 'error' | 'unavailable';
  readonly truncated: boolean;
  readonly freshness: 'recent' | 'stale_event_watermark' | 'no_events' | 'unknown';
}

export interface DirectorDigestNarrativeInput {
  readonly generatedAtMs: number;
  readonly rangeDays: 1 | 3 | 7 | 28 | 90;
  readonly state: DirectorNarrativeState;
  readonly period: Readonly<{ startMs: number; endExclusiveMs: number }>;
  readonly previousPeriod: Readonly<{ startMs: number; endExclusiveMs: number }>;
  readonly sourceHealth: readonly DirectorDigestNarrativeSourceHealth[];
  readonly metrics: readonly DirectorDigestNarrativeMetric[];
}

export interface DirectorDigestProductManagerInsight {
...
export interface DirectorDigestNarrative {
  readonly ownerMonologue: string;
  readonly executiveSummary: string;
  readonly productManager: readonly DirectorDigestProductManagerInsight[];
  readonly growthAndRevenue: readonly string[];
  readonly qualityAndRisks: readonly string[];
  readonly userVoice: readonly string[];
...
export interface ParsedDirectorDigestNarrative {
  readonly state: 'generated' | 'fallback';
  readonly narrative: DirectorDigestNarrative;
}

const PROMPT_VERSION = 4 as const;
const MAX_TEXT_LENGTH = 600;
const MAX_SUMMARY_LENGTH = 1_000;
const MAX_MONOLOGUE_LENGTH = 8_000;
...
const SOURCE_LABELS: Readonly<Record<DirectorDigestNarrativeSourceHealth['source'], string>> =
  Object.freeze({
...
function sanitizeText(value: unknown, max = MAX_TEXT_LENGTH): string {
...
function safeStringArray(value: unknown, maxItems = 12): readonly string[] {
...
function normalizedNumberToken(value: number): string {
  return String(value).replace(',', '.');
}

function validatedOwnerMonologue(
  value: unknown,
  input: DirectorDigestNarrativeInput,
): string {
  if (typeof value !== 'string') return '';
  if (
  ...
  const allowedNumbers = new Set<string>([normalizedNumberToken(input.rangeDays)]);
  ...
  if (numberTokens.some((token) => !allowedNumbers.has(token.replace(',', '.')))) return '';
  return sanitizeText(value, MAX_MONOLOGUE_LENGTH);
}

function metricLabel(metric: Pick<DirectorDigestNarrativeMetric, 'id'>): string {
  return METRIC_LABELS[metric.id] ?? 'Агрегированный показатель';
}

function metricFact(metric: DirectorDigestNarrativeMetric): string {
...
function metricComparison(metric: DirectorDigestNarrativeMetric): string {
  const label = metricLabel(metric);
  if (
  ...
  if (metric.absoluteDelta === null) return `${label}: сравнение недоступно`;
  const percent = metric.percentDelta === null ? '' : ` (${Math.abs(metric.percentDelta)}%)`;
  return `${label}: ${partial}было ${metric.previous}, ${
    metric.absoluteDelta > 0 ? 'рост' : 'снижение'
  } на ${Math.abs(metric.absoluteDelta)}${percent}`;
}

function deterministicSourceWarnings(
  input: DirectorDigestNarrativeInput,
): readonly string[] {
...
function metricWhyItMatters(metric: DirectorDigestNarrativeMetric): string {
...
function metricHypothesis(metric: DirectorDigestNarrativeMetric): string {
...
function metricAction(metric: DirectorDigestNarrativeMetric): string {
  const label = metricLabel(metric);
  if (
...
function metricSuccessMetric(metric: DirectorDigestNarrativeMetric): string {
  if (
    metric.availability === 'partial'
    || metric.availability === 'unavailable'
    || metric.current === null
...
function metricConfidence(
  metric: DirectorDigestNarrativeMetric,
): DirectorDigestProductManagerInsight['confidence'] {
...
function deterministicProductManager(
  input: DirectorDigestNarrativeInput,
): readonly DirectorDigestProductManagerInsight[] {
...
function deterministicMetricStatements(
  metrics: readonly DirectorDigestNarrativeMetric[],
  fallback: string,
): readonly string[] {
  if (metrics.length === 0) return Object.freeze([fallback]);
  return Object.freeze(metrics.slice(0, 5).map(
    (metric) => `${metricFact(metric)}; ${metricComparison(metric)}.`,
  ));
}

function safePromptMetric(metric: DirectorDigestNarrativeMetric) {
...
export function buildDirectorDigestNarrativePrompt(
  input: DirectorDigestNarrativeInput,
): Readonly<{ promptVersion: typeof PROMPT_VERSION; system: string; user: string }> {
  const payload = {
...
function deterministicNarrative(input: DirectorDigestNarrativeInput): DirectorDigestNarrative {
  const changed = input.metrics.filter((metric) => (
    metric.current !== null
    && metric.previous !== null
    && metric.absoluteDelta !== null
    && metric.absoluteDelta !== 0
  ...
  const known = input.metrics.filter((metric) => (
    metric.current !== null && metric.availability !== 'unavailable'
  )).slice(0, 5);
  ...
  const executiveSummary = summaryMetrics.length > 0
    ? summaryMetrics.map((metric) => (
      metric.previous === null
        ? metricFact(metric)
        : `${metricFact(metric)} (${metricComparison(metric)})`
    )).join('; ')
  ...
  const growthAndRevenue = deterministicMetricStatements(
    input.metrics.filter((metric) => (
      metric.id.startsWith('store.')
      || metric.id.startsWith('revenue.')
      || metric.id === 'paywall.trial_started.v1'
      || metric.id === 'paywall.purchase_completed.v1'
    )),
    'Данных по агрегированным показателям роста и денег недостаточно для вывода.',
  );
  const qualityAndRisks = deterministicMetricStatements(
    input.metrics.filter((metric) => (
      metric.id === 'paywall.purchase_failed.v1'
      ...
      || metric.id === 'store.cancellation.v1'
    )),
    'Данных по агрегированным показателям качества и рисков недостаточно для вывода.',
  );
  ...
  const userVoice = behavioralMetrics.length > 0
    ? Object.freeze(behavioralMetrics.slice(0, 5).map(
      (metric) => `Агрегированный сигнал поведения: ${metricFact(metric)}; ${metricComparison(metric)}.`,
    ))
    : Object.freeze([
...
function safeTitle(value: unknown, metrics: readonly DirectorDigestNarrativeMetric[]): string {
...
export function parseDirectorDigestNarrative(
  raw: string,
  input: DirectorDigestNarrativeInput,
): ParsedDirectorDigestNarrative {
  try {
      ...
      ? parsed.productManager.slice(0, 5).flatMap((value) => {
        ...
        const metricIds = Array.isArray(row.metricIds)
          ? [...new Set(row.metricIds
            ...
            .slice(0, 3)
          : [];
        const metrics = metricIds.map((id) => metricById.get(id) as DirectorDigestNarrativeMetric);
        ...
        const confidence: DirectorDigestProductManagerInsight['confidence'] =
    ...
    const actions = Array.isArray(parsed.actions)
      ? parsed.actions.slice(0, 7).flatMap((value, index) => {
        ...
        const successMetric = sanitizeText(row.successMetric);
        if (!action || !reason || !successMetric) return [];
    ...
    if (!executiveSummary) throw new Error('director_digest_missing_summary');
    if (ownerMonologue.length < 160) throw new Error('director_digest_missing_owner_monologue');
    if (
      input.metrics.some((metric) => metric.current !== null)
