...
function makeDbStub(initial = {}, options) {
    ...
    const pathFor = (name, id) => `${name}/${id}`;
    const mutate = (name, id, data) => {
    ...
    const queryApi = (name, field, op, value, pageLimit = Number.POSITIVE_INFINITY, cursor = '') => ({
        orderBy: () => queryApi(name, field, op, value, pageLimit, cursor),
        startAfter: (nextCursor) => queryApi(name, field, op, value, pageLimit, String(nextCursor?.id ?? nextCursor ?? '')),
        limit: (nextLimit) => queryApi(name, field, op, value, nextLimit, cursor),
        get: async () => {
            ...
            options?.afterQueryGet?.(name, docs.map((doc) => doc.id), (id, data) => mutate(name, id, data));
...
describe('mergeStableAccounts', () => {
    ...
    it('leases and idempotently completes merge outbox receipt repointing', async () => {
        ...
        await expect((0, auth_merge_1.processAccountMergeOutboxJob)(db, 'merge1', NOW)).resolves.toEqual({
            status: 'completed',
            updated: 205,
        });
        ...
        const candidateOnlyPages = queryLog.filter(({ ids }) => (ids.some((id) => id.startsWith('candidate-only-'))));
        ...
        expect(finalCandidateOnlyReceipt).toMatchObject({
            ...
            canonicalOwnerUid: 'winner-next',
            previousOwnerUidHash: (0, crypto_1.createHash)('sha256').update('loser').digest('hex'),
        });
        ...
        await expect((0, auth_merge_1.processAccountMergeOutboxJob)(db, 'merge1', NOW + 1)).resolves.toEqual({
        ...
        const { db, store } = makeDbStub({
            ...
            afterQueryGetAny: (collection, ids, mutate) => {
                if (!deletionStarted && collection === 'revenuecat_premium_events' && ids.length > 0) {
                    ...
                    mutate('account_deletion_permanent_denials', (0, account_delete_job_1.accountDeletePermanentDenialId)('winner'), {
                        status: 'denied',
                    });
                }
            },
        });
        await expect((0, auth_merge_1.processAccountMergeOutboxJob)(db, 'mergeDelete', NOW)).resolves.toEqual({
            status: 'cancelled',
            updated: 0,
        });
    ...
    it('fails closed when the latest canonical owner chain exceeds the bounded hop limit', async () => {
        ...
        await expect((0, auth_merge_1.processAccountMergeOutboxJob)(db, 'mergeOverflow', NOW))
    ...
    it('repoints through a valid long canonical owner chain within the system cap', async () => {
        ...
        await expect((0, auth_merge_1.processAccountMergeOutboxJob)(db, 'mergeLong', NOW)).resolves.toEqual({
    ...
    it('does not let a stale failed worker overwrite a reclaimed terminal outbox state', async () => {
        ...
        await expect((0, auth_merge_1.processAccountMergeOutboxJob)(db, 'mergeReclaimed', NOW))
    ...
    it('atomically rekeys loser RevenueCat lineages and records durable owner mapping/outbox', async () => {
        ...
        await (0, auth_merge_1.mergeStableAccounts)(db, 'auth-rc', 'winner', 'loser', NOW);
        const ownerHash = (0, crypto_1.createHash)('sha256').update('winner').digest('hex').slice(0, 32);
        ...
        expect(store.users.loser.progress.premium_plan).toBeUndefined();
        expect(store.users.loser.progress.premium_rc_active_lineage).toBeUndefined();
    ...
    it('chooses the winner and merge payload from transaction snapshots', async () => {
        ...
        const result = await (0, auth_merge_1.mergeStableAccounts)(db, 'google-transaction', 'stable-a', 'stable-b', NOW);
        ...
        expect((store.users['stable-b']?.progress).streak_count).toBe('99');
    ...
    it('fails closed when winner ownership changes before the transaction reads it', async () => {
        ...
        await expect((0, auth_merge_1.mergeStableAccounts)(db, 'google-owner-race', 'stable-owned', 'stable-loser', NOW)).rejects.toMatchObject({ code: 'permission-denied' });
    ...
    it('fails closed when an account-deletion marker appears before the transaction reads', async () => {
        ...
        await expect((0, auth_merge_1.mergeStableAccounts)(db, 'google-delete-race', 'stable-a', 'stable-b', NOW)).rejects.toMatchObject({ code: 'failed-precondition', message: 'account_delete_pending' });
    ...
    it.each([
        ...
        await expect((0, auth_merge_1.mergeStableAccounts)(db, 'google-tombstone-race', 'stable-raw', 'stable-b', NOW)).rejects.toMatchObject({ code: 'failed-precondition', message: 'account_delete_pending' });
        expect(store.users['stable-b']?.identityHidden).not.toBe(true);
    });
    it('does not recreate a user that disappears before the transaction reads it', async () => {
        ...
        await expect((0, auth_merge_1.mergeStableAccounts)(db, 'google-missing-race', 'stable-a', 'stable-b', NOW)).rejects.toMatchObject({ code: 'failed-precondition', message: 'stable_id_missing' });
    ...
    it('does not overwrite an auth_links anchor that moves to a protected raw id after preflight', async () => {
        ...
        await expect((0, auth_merge_1.mergeStableAccounts)(db, 'google-protected-link-race', 'stable-raw-a', 'stable-b', NOW)).rejects.toMatchObject({ code: 'failed-precondition', message: 'stable_identity_changed' });
    ...
    it('rejects auth_links drift after resolver reads but before the post-resolution baseline', async () => {
        ...
        await expect((0, auth_merge_1.mergeStableAccounts)(db, 'google-resolver-link-race', 'stable-raw-a', 'stable-b', NOW)).rejects.toMatchObject({ code: 'failed-precondition', message: 'stable_identity_changed' });
    ...
    it('rejects an absent-to-anchor-to-absent ABA during resolution with zero stale writes', async () => {
        ...
        const { db, store } = makeDbStub({
            ...
            afterDocGet: (collection, id, _count, mutate) => {
                if (collection !== 'auth_links' || id !== 'google-resolver-aba')
                    return;
                authLinkReads += 1;
                if (authLinkReads === 1) {
                    mutate({ stable_id: 'stable-raw-a', updatedAt: NOW + 1 });
        ...
        await expect((0, auth_merge_1.mergeStableAccounts)(db, 'google-resolver-aba', 'stable-raw-a', 'stable-b', NOW)).rejects.toMatchObject({ code: 'failed-precondition', message: 'stable_identity_changed' });
        expect(store.auth_links['google-resolver-aba']).toBeUndefined();
        expect(store.users['stable-b']?.identityHidden).not.toBe(true);
        expect(store.users['stable-canonical-a']?.identityMergedAt).toBeUndefined();
    });
    it.each(['changed', 'cycle'])('fails closed when canonical targets become %s after preflight', async (mode) => {
        let changed = false;
        let storeRef;
        const { db, store } = makeDbStub({
            users: {
            ...
            afterDocGet: (collection, id, _count, mutate) => {
                if (changed || collection !== 'users' || id !== 'stable-a')
                    return;
                changed = true;
                mutate({
        ...
        storeRef = store;
        await expect((0, auth_merge_1.mergeStableAccounts)(db, 'google-canonical-race', 'stable-a', 'stable-b', NOW)).rejects.toMatchObject({ code: 'failed-precondition', message: 'stable_identity_changed' });
    ...
    it('keeps auth_links unchanged when distinct inputs resolve to one winner before merge', async () => {
        ...
        const res = await (0, auth_merge_1.mergeStableAccounts)(db, 'google-converged', 'stable-input-a', 'stable-input-b', NOW);
        expect(res).toEqual({
            canonicalStableId: 'stable-canonical',
            mergedFromStableId: null,
            alreadyMerged: true,
...
describe('repointReferralOnMerge — перенос реферальных данных loser → winner', () => {
    ...
    it('deterministically paginates 501 attributions and 51 codes without truncation', async () => {
        const { db, store, queryLog } = makeDbStub();
        store.referral_attributions = Object.fromEntries(Array.from({ length: 501 }, (_, index) => [
            `attr-${String(index).padStart(4, '0')}`,
            { referrerStableId: 'loser', status: index % 2 ? 'pending' : 'qualified' },
        ]));
        store.referral_codes = Object.fromEntries(Array.from({ length: 51 }, (_, index) => [
            `code-${String(index).padStart(4, '0')}`,
            { ownerStableId: 'loser', normalized: `CODE${index}` },
        ]));
        const result = await (0, auth_merge_1.repointReferralOnMerge)(db, 'winner', 'loser');
        ...
        expect(Object.values(store.referral_attributions))
            .toHaveLength(501);
        expect(Object.values(store.referral_attributions).every((data) => data?.referrerStableId === 'winner')).toBe(true);
        expect(Object.values(store.referral_codes).every((data) => data?.ownerStableId === 'winner')).toBe(true);
    ...
    it('is idempotent on rerun after all referral pages and owner docs are complete', async () => {
        ...
        const second = await (0, auth_merge_1.repointReferralOnMerge)(db, 'winner', 'loser');
        ...
        const { db, store } = makeDbStub({}, {
            afterDocGet: (collection, id, _count, mutate) => {
                if (collection === 'account_deletion_tombstones' && id === 'loser') {
                    ...
                    if (loserTombstoneReads === 3)
                        mutate({ status: 'pending' });
                }
            },
        });
        store.referral_attributions = Object.fromEntries(Array.from({ length: 20 }, (_, index) => [
