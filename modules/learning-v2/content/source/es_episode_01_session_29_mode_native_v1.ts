...
// продолжает Главу 4 "Мы и они"): испанская сессия 29 переписывается в
// mode-native формат, зеркаля КОД (не данные) сессии 25/27/28 — kind:
// 'phrases', новых слов нет. Все 15 фраз сохранены из легаси-черновика без
// изменений содержания — каждая учит no + somos/son + признак с полным
// согласованием по роду и числу (rápidos/rápidas, bonitos/bonitas,
// únicos/únicas, caros/caras), завершая парадигму отрицания связки ser.
//
// зачем НЕ все фразы идут через phrase_builder/listen_build_dictation:
// только индекс 13 даёт больше 8 уникальных дистракторов, превышая предел
// responseFeedbackById в course_session_client_children_v1.ts — этот
// индекс идёт только через listen_choose/context_gap_grammar/speed_match.
function asciiId(value: string): string {
  return value.normalize('NFD').replace(/[̀-ͯ]/gu, '');
}

const phrases = ES_EPISODE_01_SESSION_29_PHRASES;
if (phrases.length !== 15) {
  throw new Error('es_session_29_mode_native_phrase_count_invalid');
}
...
// зачем 'en' вместо 'es' здесь (как в испанских сессиях 1-28): в испанском
...
function localizedPhraseField(
  ...
  if (!phrase?.localizedDetails) throw new Error(`es_session_29_phrase_details_missing:${phraseIndex}`);
  return Object.fromEntries(LEARNING_V2_INTERFACE_LOCALES.map((locale) => {
    ...
    if (!detail) throw new Error(`es_session_29_phrase_locale_missing:${phraseIndex}:${locale}`);
...
function localizedPhraseDistractorFeedback(
  ...
  return Object.fromEntries(LEARNING_V2_INTERFACE_LOCALES.map((locale) => {
    ...
    if (!reason) throw new Error(`es_session_29_phrase_distractor_missing:${phraseIndex}:${locale}:${value}`);
...
function phraseAudio(phraseIndex: number): LearningV2ModeAudioReferenceV1 {
  const phrase = phrases[phraseIndex]!;
  return authoredAudio(`es-e01-s29-phrase-${phraseIndex + 1}`, phrase.english);
}
function phraseSlowAudio(phraseIndex: number): LearningV2ModeAudioReferenceV1 {
  const phrase = phrases[phraseIndex]!;
  return authoredAudio(`es-e01-s29-phrase-${phraseIndex + 1}-slow`, phrase.english);
}
...
// 2026-08-28): эта сессия проверяет согласование признака по роду/числу
// после отрицания — последнее содержательное слово почти всегда несёт эту
// форму.
function contextGapLastWord(phraseIndex: number): LearningV2ModeNativePayloadV1 {
  ...
  return Object.freeze({
    ...
    testedDimension: `negation:${asciiId(phrase.id)}`,
    choiceFeedback: Object.freeze([
      ...
      ...lastWord.distractors.map((entry) => feedback(
        ...
        false,
        `negation:${asciiId(entry.reasonCode)}`,
...
// 2026-08-28, тот же приём, что сессии 11/17/25/27/28): широкий срез уже
// отработанных no somos/son + признак фраз, а не смежный блок.
function speedMatchPhrases(pairIndices: readonly number[]): LearningV2ModeNativePayloadV1 {
  ...
  const ids = pairIndices.map((index) => `es-e01-s29-pair-${index + 1}`);
  return Object.freeze({
    family: 'speed_match',
...
// phrase_builder/listen_build_dictation. Каждый индекс 3-14 используется
// как target.sourceIndex ровно один раз.
export const ES_EPISODE_01_SESSION_29_MODE_NATIVE_PRACTICE_V1 = Object.freeze<readonly SessionModeNativePracticeSourceV1[]>([
  ...
  { family: 'context_gap_grammar', purpose: 'supported_practice', learningStage: 'apply_in_phrase', target: { kind: 'phrase', sourceIndex: 4 }, modePayload: contextGapLastWord(4) },
  ...
  { family: 'speed_match', purpose: 'retrieval_practice', learningStage: 'apply_in_phrase', target: { kind: 'phrase', sourceIndex: 9 }, modePayload: speedMatchPhrases([3, 6, 9, 11, 14]) },
...
if (ES_EPISODE_01_SESSION_29_MODE_NATIVE_PRACTICE_V1.length !== 12) {
  throw new Error('es_session_29_mode_native_practice_count_invalid');
}
