...
// + СТАРТ ES §0/§10): испанская сессия 4 переписывается в mode-native формат,
// повторяя КОД (не данные) es_episode_01_session_03_mode_native_v1.ts — тот же
// класс сессии, ровно ОДНО новое слово (verdadero), полностью авторенное в
// es_episode_01_session_04_vocabulary_v1.ts тремя контактами (recognize/
// retrieve_meaning/build_form). Освободившийся бюджет 17 шагов уходит на
// более глубокую отработку "verdadero" (доп. интеракции на recognize и
...
// сессии (Es verdadero / Es verdadera), каждая из которых использует только
// уже изученные слова: es (сессия 1) + verdadero/verdadera (эта сессия). Ни
// одно новое слово не вводится внутри фраз — word-first правило соблюдено.
//
// зачем combined review-словарь берёт И сессию 1, И сессию 3 (владелец,
// 2026-08-27, карта es_episode_01_session_map_v1.ts: builtOn: [1, 3],
// recalls: [1, 3]): в отличие от сессий 2/3 (review только из сессии 1),
// сессия 4 явно recalls обе — verdad (существительное, сессия 1) и
// gender_agreement_full (формула -o/-a, впервые показанная в сессии 3 через
// bonito/bonita). speed_match поэтому сочетает оба набора + новое слово этой
// сессии — девять слов, честно изученных к этому моменту, без выдумывания.
const vocabulary = ES_EPISODE_01_SESSION_04_VOCABULARY_V1;
const reviewVocabularySession01 = ES_EPISODE_01_SESSION_01_VOCABULARY_V1;
const reviewVocabularySession03 = ES_EPISODE_01_SESSION_03_VOCABULARY_V1;
const phrases = ES_EPISODE_01_SESSION_04_PHRASES;
if (phrases.length !== 2) {
  throw new Error('es_session_04_mode_native_phrase_count_invalid');
}
...
// зачем 'en' вместо 'es' здесь (как в испанских сессиях 1/2/3): в испанском
...
function localizedPhraseField(
  ...
  if (!phrase?.localizedDetails) throw new Error(`es_session_04_phrase_details_missing:${phraseIndex}`);
  return Object.fromEntries(LEARNING_V2_INTERFACE_LOCALES.map((locale) => {
    ...
    if (!detail) throw new Error(`es_session_04_phrase_locale_missing:${phraseIndex}:${locale}`);
...
function localizedPhraseDistractorFeedback(
  ...
  return Object.fromEntries(LEARNING_V2_INTERFACE_LOCALES.map((locale) => {
    ...
    if (!reason) throw new Error(`es_session_04_phrase_distractor_missing:${phraseIndex}:${locale}:${value}`);
...
const VERDADERO_AUDIO = authoredAudio('es-e01-s04-word-verdadero', 'verdadero');
const VERDADERO_SLOW_AUDIO = authoredAudio('es-e01-s04-word-verdadero-slow', 'verdadero');
const ES_VERDADERO_PHRASE_AUDIO = authoredAudio('es-e01-s04-phrase-es-verdadero', 'Es verdadero');
const ES_VERDADERO_PHRASE_SLOW_AUDIO = authoredAudio('es-e01-s04-phrase-es-verdadero-slow', 'Es verdadero');
const ES_VERDADERA_PHRASE_AUDIO = authoredAudio('es-e01-s04-phrase-es-verdadera', 'Es verdadera');
const ES_VERDADERA_PHRASE_SLOW_AUDIO = authoredAudio('es-e01-s04-phrase-es-verdadera-slow', 'Es verdadera');

// зачем именно эта ловушка (владелец, 2026-08-27): третья фонетическая
// ловушка для recognize/build_form "verdadero", не входящая в исходный
// словарный файл (там уже две — verdad, verdadera), нужна как самостоятельный
// дополнительный контакт для listen_build_dictation-повтора recognize.
// "bonito" уже authored в испанской сессии 3 как честный испанский сосед той
// же морфологической формы (-o, мужской род прилагательного), не выдумываем
// новое слово.
const VERDADERO_EXTRA_RECOGNIZE_TRAP = Object.freeze({
  value: 'bonito',
  reasonCode: 'verdadero_recognize_bonito_different_word',
  feedback: L({
    ru: 'Bonito означает «красивый» — совсем другое слово и признак, хоть и с той же концовкой -o. Нужное слово — verdadero, «истинный».',
    uk: 'Bonito означає «красивий» — зовсім інше слово й ознака, хоч і з тим самим закінченням -o. Потрібне слово — verdadero, «істинний».',
    es: 'Bonito means "pretty" — a completely different word and quality, even though it shares the -o ending. The word here is verdadero, "true".',
    'pt-BR': 'Bonito significa "bonito" — uma palavra e qualidade completamente diferentes, mesmo com a mesma terminação -o. A palavra aqui é verdadero, "verdadeiro".',
    vi: 'Bonito nghĩa là "đẹp" — một từ và đặc điểm hoàn toàn khác, dù có cùng đuôi -o. Từ cần dùng là verdadero, "đúng".',
    id: 'Bonito berarti "cantik" — kata dan sifat yang sama sekali berbeda, meskipun berbagi akhiran -o yang sama. Kata di sini adalah verdadero, "benar".',
    tr: 'Bonito "güzel" demektir — aynı -o son ekini paylaşsa da tamamen farklı bir kelime ve nitelik. Buradaki kelime verdadero, "doğru"dur.',
    pl: 'Bonito znaczy „ładny” — zupełnie inne słowo i cecha, mimo tej samej końcówki -o. Potrzebne słowo to verdadero, „prawdziwy”.',
  }),
});

// Meaning-choice labels are authored for the learner's own language, exactly
// as the English precedent and Spanish sessions 1/2/3 do — showing the raw
...
// including session-1/session-3 review words pulled in as distractors.
const VOCABULARY_TRAP_MEANINGS = Object.freeze({
  verdad: L({ ru: 'правда — предмет, не признак', uk: 'правда — предмет, не ознака', es: 'truth (a thing, not a quality)', 'pt-BR': 'verdade (uma coisa, não uma qualidade)', vi: 'sự thật (một sự vật, không phải đặc điểm)', id: 'kebenaran (benda, bukan sifat)', tr: 'doğruluk (bir şey, nitelik değil)', pl: 'prawda (rzecz, nie cecha)' }),
  verdadera: L({ ru: 'истинная — женский род признака', uk: 'істинна — жіночий рід ознаки', es: 'true (feminine form)', 'pt-BR': 'verdadeira (forma feminina)', vi: 'đúng (dạng giống cái)', id: 'benar (bentuk feminin)', tr: 'gerçek (dişil biçim)', pl: 'prawdziwa (forma żeńska)' }),
  mentira: L({ ru: 'ложь', uk: 'брехня', es: 'lie', 'pt-BR': 'mentira', vi: 'lời nói dối', id: 'kebohongan', tr: 'yalan', pl: 'kłamstwo' }),
  bonito: L({ ru: 'красивый', uk: 'красивий', es: 'pretty', 'pt-BR': 'bonito', vi: 'đẹp', id: 'cantik', tr: 'güzel', pl: 'ładny' }),
...
function vocabularyTrapMeaning(value: string): LearningV2Localized<string> {
  ...
  if (!localized) throw new Error(`es_session_04_trap_meaning_missing:${value}`);
  return localized;
}
...
function repeatWord(): LearningV2ModeNativePayloadV1 {
  ...
  return Object.freeze({
    ...
    referenceAudio: VERDADERO_AUDIO,
    slowReferenceAudio: VERDADERO_SLOW_AUDIO,
...
function listenBuildWord(): LearningV2ModeNativePayloadV1 {
  ...
  return Object.freeze({
    ...
    authoredDistractorTokens: Object.freeze([
      ...
      VERDADERO_EXTRA_RECOGNIZE_TRAP.value,
    ]),
    slotFeedback: Object.freeze([
      ...vocabularyFeedback(item, 'recognize'),
      feedback(
        `${item.id}:recognize:${VERDADERO_EXTRA_RECOGNIZE_TRAP.reasonCode}`,
        false,
        `phonetic:${VERDADERO_EXTRA_RECOGNIZE_TRAP.reasonCode}`,
        VERDADERO_EXTRA_RECOGNIZE_TRAP.feedback,
      ),
    ]),
  });
}
...
// mentira уже используется как встроенный word-level дистрактор внутри
// словарного файла (es_episode_01_session_04_vocabulary_v1.ts). Второй
// retrieve_meaning-контакт нужен под другой family (context_gap_grammar
// вместо listen_choose) с честной новой ловушкой — берём "bonito", реальное
// review-прилагательное из сессии 3, чтобы контакт заодно тренировал
// различение двух признаков предмета — истинности от внешности.
function contextMeaningVerdadero(): LearningV2ModeNativePayloadV1 {
  const item = vocabulary[0]!;
  const contact = item.contacts.retrieve_meaning;
  const extraValue = 'bonito';
  ...
  return Object.freeze({
    ...
    testedDimension: `meaning:${item.target}_describes_truthfulness`,
    choiceFeedback: Object.freeze([
      ...
      feedback(
        ...
        L({
          ru: 'Bonito — про внешность, «красивый». Про истинность нужно другое слово — verdadero.',
          uk: 'Bonito — про зовнішність, «красивий». Про істинність потрібне інше слово — verdadero.',
          es: 'Bonito is about appearance, "pretty". For truthfulness, a different word is needed — verdadero.',
          'pt-BR': 'Bonito é sobre aparência, "bonito". Para veracidade, é preciso outra palavra — verdadero.',
          vi: 'Bonito nói về vẻ ngoài, "đẹp". Về sự đúng đắn cần một từ khác — verdadero.',
          id: 'Bonito tentang penampilan, "cantik". Untuk kebenaran diperlukan kata lain — verdadero.',
          tr: 'Bonito görünümle ilgilidir, "güzel". Doğruluk için başka bir kelime gerekir — verdadero.',
          pl: 'Bonito dotyczy wyglądu, „ładny”. O prawdziwości mówi inne słowo — verdadero.',
        }),
      ),
    ]),
  });
...
function listenChooseMeaningVerdadero(): LearningV2ModeNativePayloadV1 {
  const item = vocabulary[0]!;
  const contact = item.contacts.retrieve_meaning;
  return Object.freeze({
    family: 'listen_choose',
...
function buildFormVerdaderoBuilder(): LearningV2ModeNativePayloadV1 {
  const item = vocabulary[0]!;
...
// 2026-08-27): та же орфографическая проверка "verdadero" (ровно концовка
// -o), но внутри видимого контекста фразы "Es ___" вместо изолированной
// плитки — держит правило "target+family не повторяется" и готовит к
// настоящему gender-agreement-применению фраз ниже.
function buildFormVerdaderoContext(): LearningV2ModeNativePayloadV1 {
  const item = vocabulary[0]!;
  const build = item.contacts.build_form;
  return Object.freeze({
    family: 'context_gap_grammar',
    ...
    testedDimension: 'orthography:verdadero_masculine_ending',
...
// + session-3 review (bonito) + session-4 "verdadero" — ровно словам,
// перечисленным картой как builtOn/recalls этой сессии ([1, 3]), плюс новое
// слово этой сессии. Choreography-шаг видит только sourceVocabularyIndices:
// [0] — позицию "verdadero" внутри ЭТОЙ сессии, как того требует
...
const SPEED_WORDS = Object.freeze([
  ...
  ...reviewVocabularySession03,
  ...vocabulary,
]);
const SPEED_IDS = SPEED_WORDS.map((word) => `es-e01-s04-pair-${word.target.toLowerCase()}`);
const speedMatchPayload: LearningV2ModeNativePayloadV1 = Object.freeze({
  ...
  rightColumn: Object.freeze([SPEED_IDS[1]!, SPEED_IDS[4]!, SPEED_IDS[2]!, SPEED_IDS[5]!, SPEED_IDS[0]!, SPEED_IDS[3]!]),
  pairingKey: 'pair_id',
...
function listenBuildPhrase(phraseIndex: number): LearningV2ModeNativePayloadV1 {
  ...
  return Object.freeze({
    family: 'listen_build_dictation',
    referenceAudio: phraseIndex === 0 ? ES_VERDADERO_PHRASE_AUDIO : ES_VERDADERA_PHRASE_AUDIO,
    slowReferenceAudio: phraseIndex === 0 ? ES_VERDADERO_PHRASE_SLOW_AUDIO : ES_VERDADERA_PHRASE_SLOW_AUDIO,
...
// приём, что и в испанской сессии 3): предмет согласования — сам признак
// (verdadero/verdadera), связка Es неизменна и уже видна в сцене. Гейт стоит
// на позиции признака: "Es ___", варианты — verdadero/verdadera/verdad
// (третий вариант из этой же фразы, wrong_word).
...
export const ES_EPISODE_01_SESSION_04_MODE_NATIVE_PRACTICE_V1 = Object.freeze<readonly SessionModeNativePracticeSourceV1[]>([
  ...
  { family: 'context_gap_grammar', instruction: INSTRUCTION.contextMeaning, purpose: 'retrieval_practice', learningStage: 'retrieve_meaning', target: { kind: 'vocabulary', sourceIndex: 0 }, modePayload: contextMeaningVerdadero() },
  { family: 'listen_choose', instruction: INSTRUCTION.listenMeaning, purpose: 'retrieval_practice', learningStage: 'retrieve_meaning', target: { kind: 'vocabulary', sourceIndex: 0 }, modePayload: listenChooseMeaningVerdadero() },
  { family: 'phrase_builder', instruction: INSTRUCTION.buildWord, purpose: 'guided_practice', learningStage: 'build_form', target: { kind: 'vocabulary', sourceIndex: 0 }, modePayload: buildFormVerdaderoBuilder() },
  { family: 'context_gap_grammar', instruction: INSTRUCTION.contextBuild, purpose: 'guided_practice', learningStage: 'build_form', target: { kind: 'vocabulary', sourceIndex: 0 }, modePayload: buildFormVerdaderoContext() },
...
if (ES_EPISODE_01_SESSION_04_MODE_NATIVE_PRACTICE_V1.length !== 17) {
  throw new Error('es_session_04_mode_native_practice_count_invalid');
}
