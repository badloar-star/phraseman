...
describe('RevenueCat client projection sync', () => {
  ...
  it('has no retired MAX activation polling or intent contract', () => {
    const source = fs.readFileSync(
      path.join(process.cwd(), 'app', 'revenuecat_projection_sync.ts'),
      'utf8',
    );
    expect(source).not.toContain('confirmRevenueCatMaxProjectionForAccount');
    expect(source).not.toContain('MAX_ACTIVATION_DEFAULT');
    expect(source).not.toContain("intent: 'max_activation'");
    expect(source).not.toContain("plan: 'max_monthly'");
  });
});
