  ...
  return (
    <ScreenGradient>
      <SafeAreaView testID="max-call-session-screen" style={{ flex: 1 }}>
        ...
        {/* Провал соединения во время отсчёта: оверлей снимаем сразу, иначе
            он ещё несколько секунд закрывает экран ошибки с кнопкой «Повторить»
            (аудит 2026-09-01). Отсчёт при этом не «продолжает» урок — старт
            держит holdStart, а упавший клиент говорить уже не начнёт. */}
        {countdownLeft > 0 && phase !== 'failed' && phase !== 'ended' ? (
          <MaxLessonCountdown
