...
function breakdown(counts: Map<string, number>): BreakdownRow[] {
  return [...counts.entries()]
    .map(([id, events]) => ({ id, events }))
    .sort((a, b) => b.events - a.events || a.id.localeCompare(b.id));
}

export function aggregateSubscriptionAnalytics(
  ...
  const byPeriodType = new Map<string, number>();
  const byEventType = new Map<string, number>();
  const byCancellationReason = new Map<string, number>();
  const byExpirationReason = new Map<string, number>();
  let purchases = 0;
  ...
  rows.forEach((row, index) => {
    ...
    if (options.fromMs != null && effectiveEventAtMs < options.fromMs) return;
    if (eventAtMs == null) {
    ...
    if (eventType === 'INITIAL_PURCHASE') purchases += 1;
    if (eventType === 'NON_RENEWING_PURCHASE') lifetimePurchases += 1;
    if (eventType === 'RENEWAL') renewals += 1;
    if (periodType === 'TRIAL' && (eventType === 'INITIAL_PURCHASE' || eventType === 'NON_RENEWING_PURCHASE')) trialEvents += 1;
    if (eventType === 'CANCELLATION') {
      ...
      } else cancellationReasonUnavailable += 1;
    }
    if (eventType === 'UNCANCELLATION') uncancellations += 1;
    if (eventType === 'BILLING_ISSUE') billingIssues += 1;
    if (eventType === 'EXPIRATION') {
      ...
      } else expirationReasonUnavailable += 1;
    }
    if (eventType === 'REFUND') refunds += 1;
    if (eventType === 'PRODUCT_CHANGE') productChanges += 1;
    if (eventType === 'SUBSCRIPTION_EXTENDED') extensions += 1;
  });
