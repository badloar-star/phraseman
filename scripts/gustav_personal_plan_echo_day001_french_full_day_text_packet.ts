...
type GeneratedRow = {
  ...
  generatedBy: 'gustav_personal_plan_echo_day001_french_full_day_text_packet';
};

const PLAN_ID = 'echo' as const;
const DAY_INDEX = 1 as const;
const GENERATED_BY = 'gustav_personal_plan_echo_day001_french_full_day_text_packet' as const;

const FIELD_SPECS: FieldSpec[] = [
  {
    fieldPath: 'topic',
    fieldKind: 'day_topic',
    frenchText: 'Se présenter dans une conversation',
    source: (day) => day.topic,
  },
  {
    fieldPath: 'outcome',
    fieldKind: 'day_outcome',
    frenchText: "Tu peux dire qui tu es et d'où tu viens : le premier pas dans toute conversation en anglais.",
    source: (day) => day.outcome,
  },
  {
    fieldPath: 'intro[0].title',
    fieldKind: 'intro_title',
    frenchText: 'Le premier pas',
    source: (day) => day.intro?.[0]?.title,
  },
  {
    fieldPath: 'intro[0].body',
    fieldKind: 'intro_body',
    frenchText: "Tu ouvres une application d'actualités en anglais et tu vois le commentaire d'un lecteur d'un autre pays. Il a écrit quelques mots sur lui. Simple et clair. C'est comme ça que tout commence : dire ton nom, d'où tu viens, ce que tu fais. Aujourd'hui, tu fais ce premier pas.",
    source: (day) => day.intro?.[0]?.body,
  },
  {
    fieldPath: 'intro[0].examples[0].gloss',
    fieldKind: 'intro_example_gloss',
    frenchText: "Je m'appelle Anna.",
  ...
  },
  {
    ...
    fieldKind: 'intro_title',
    frenchText: 'Un seul mot fait tout le travail',
    source: (day) => day.intro?.[1]?.title,
  },
  {
    fieldPath: 'intro[1].body',
    fieldKind: 'intro_body',
    frenchText: "En anglais, on ne peut pas dire simplement « moi Anna » : il faut un petit mot entre les deux. Ce mot est « am ». C'est comme un pont : il relie la personne à ce qu'elle dit d'elle-même. Avec « you », « he » et « she », le pont change : are, is. Retiens ces trois paires, et tu parles déjà correctement.",
    source: (day) => day.intro?.[1]?.body,
  },
  {
    fieldPath: 'intro[1].examples[0].gloss',
    fieldKind: 'intro_example_gloss',
    frenchText: 'Je suis enseignant.',
  ...
  },
  {
    ...
    fieldKind: 'intro_example_gloss',
    frenchText: "Elle vient d'Espagne.",
  ...
  },
  {
    ...
    fieldKind: 'intro_title',
    frenchText: 'Dis-le à voix haute',
    source: (day) => day.intro?.[2]?.title,
  },
  {
    fieldPath: 'intro[2].body',
    fieldKind: 'intro_body',
    frenchText: "Le moyen le plus simple de retenir, c'est de le dire à voix haute. Dis ton nom en anglais. Puis ajoute d'où tu viens. Puis ton travail ou ce qui t'intéresse. Trois phrases, et tu as déjà une petite présentation de toi. C'est comme ça que tout le monde commence.",
    source: (day) => day.intro?.[2]?.body,
  },
  {
    fieldPath: 'intro[2].examples[0].gloss',
    fieldKind: 'intro_example_gloss',
    frenchText: 'Je suis prêt à commencer.',
  ...
  ...[
    {
      phraseId: 'echo_d1_p1',
      title: 'Dire son nom',
      rule: "« My name is » veut littéralement dire « mon nom est ». C'est la présentation la plus simple.",
      why: "En anglais, il faut le petit mot « is » entre le nom et ce qu'on dit de lui.",
      commonMistake: "Ne dis pas « My name Anna » : on ne peut pas enlever « is ».",
    },
    {
      phraseId: 'echo_d1_p2',
      title: "Dire d'où tu viens",
      rule: "« I am from » + pays. C'est une façon courte de dire d'où tu viens.",
      why: '« Am » est obligatoire après « I ». Sans lui, la phrase est incorrecte.',
      commonMistake: "Ne dis pas « I from Ukraine » : on ne peut pas enlever « am ».",
    },
    {
      phraseId: 'echo_d1_p3',
      title: 'Parler de son métier',
      rule: "« She is a » + métier. On utilise « is » parce qu'on parle d'elle.",
      why: 'Quand tu parles de quelqu\'un avec « he » ou « she », utilise « is ».',
      commonMistake: 'Ne dis pas « She are a teacher » : « are » va avec « you », « we » et « they ».',
    },
    {
      phraseId: 'echo_d1_p4',
      title: "Parler d'un groupe",
      rule: "« We are » s'utilise quand tu parles de toi avec d'autres personnes.",
      why: '« We », « you » et « they » vont avec « are ».',
      commonMistake: 'Ne dis pas « We is new » : « is » va avec « he », « she » et « it ».',
    },
    {
      phraseId: 'echo_d1_p5',
      title: 'Dire que tu es prêt',
      rule: '« I am ready » décrit ton état maintenant. « To start » dit à quoi tu es prêt.',
      why: 'Après « I », il faut « am », même devant un mot court comme « ready ».',
      commonMistake: 'Ne dis pas « I ready to start » : on ne peut pas enlever « am ».',
    },
    {
      phraseId: 'echo_d1_p6',
      title: "Présenter quelqu'un",
      rule: "« He is my » + la personne. C'est une façon simple de présenter quelqu'un.",
      why: '« My » veut dire « mon/ma » ; « his » son à lui ; « her » son à elle.',
      commonMistake: 'Ne dis pas « He is mine friend » : devant le nom, il faut « my », pas « mine ».',
    },
  ].flatMap((item) => [
    {
...
function rowIdFromPath(fieldPath: string): string {
  return `fr_echo_d001_${fieldPath.replace(/[^a-zA-Z0-9]+/g, '_').replace(/^_+|_+$/g, '').toLowerCase()}`;
}
...
function renderMarkdown(report: any): string {
  const lines = [
    '# GUSTAV Personal Plan Echo Day 001 French Full-Day Text Packet',
    '',
    `Run: \`${report.runId}\``,
    '',
...
async function main(): Promise<void> {
  ...
  if (!runArg) {
    throw new Error('Usage: npx tsx scripts/gustav_personal_plan_echo_day001_french_full_day_text_packet.ts --run <run-dir>');
  }
  ...
  const rowsPath = path.join(outDir, 'echo_day_001_full_day_fr_rows.jsonl');
  const reviewerQueuePath = path.join(outDir, 'echo_day_001_full_day_fr_reviewer_queue.tsv');
  const packetJsonPath = path.join(auditsDir, 'personal_plan_echo_day001_french_full_day_text_packet.json');
  const packetMdPath = path.join(auditsDir, 'personal_plan_echo_day001_french_full_day_text_packet.md');

  const findings: Finding[] = [];
  const day = await readEchoDay(repoRoot);
  if (!day) {
    addFinding(findings, 'blocker', 'echo_day_001_missing', 'Echo day 1 is missing from source content.', rel(repoRoot, sourcePath));
  }

  let existingPhraseMeaningRowsForDay = 0;
  if (!fs.existsSync(phraseMeaningRowsPath)) {
    addFinding(findings, 'blocker', 'phrase_meaning_rows_missing', 'Echo day 1 phrase meaning rows are missing.', rel(repoRoot, phraseMeaningRowsPath));
  } else {
    ...
    if (existingPhraseMeaningRowsForDay !== 6) {
      addFinding(findings, 'blocker', 'phrase_meaning_rows_day_001_incomplete', `Expected 6 reviewer-needed phrase meaning rows for Echo day 1 but found ${existingPhraseMeaningRowsForDay}.`, rel(repoRoot, phraseMeaningRowsPath));
    }
  }
  ...
  const report = {
    schemaVersion: 'gustav-personal-plan-echo-day001-french-full-day-text-packet-v0',
    ...
    fieldKindCounts,
    generationPolicy: [
      'This packet translates Echo day 1 full-day text fields except phrase meanings, which are already covered by the phrase-meaning layer.',
  ...
  console.log(`GUSTAV Echo day 001 French full-day text packet: ${report.status}`);
