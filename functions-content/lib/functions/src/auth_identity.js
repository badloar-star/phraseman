...
function throwIdentityRetired(subject) {
...
async function assertStableAccountMergeNotPending(db, stableIds) {
    for (const stableId of [...new Set(stableIds.map(normalizeStableId).filter(Boolean))]) {
        ...
        if (snapshot.exists && snapshot.data()?.mistakePracticeMergePending === true) {
            throw new https_1.HttpsError('failed-precondition', 'account_merge_pending');
        }
    }
}
...
async function findStableUidForProviderAuth(db, authUid, authoritativeOnly = false) {
    ...
    const candidateStableIds = new Set(docs.map((doc) => doc.id));
    for (const canonicalStableId of canonicalIdsToCheck)
        candidateStableIds.add(canonicalStableId);
    await Promise.all([...candidateStableIds].map((candidateStableId) => (assertStableDeletionNotPending(db, candidateStableId))));
...
async function repairIdentityDocumentsAtomically(db, authUid, stableId, options) {
    ...
    const authPermanentDenialRef = db.collection(account_delete_job_1.ACCOUNT_DELETE_PERMANENT_DENIALS)
        .doc((0, account_delete_job_1.accountDeletePermanentDenialId)(authUid));
    ...
    const permanentDenialRefs = protectedStableIds.map((id) => (db.collection(account_delete_job_1.ACCOUNT_DELETE_PERMANENT_DENIALS).doc((0, account_delete_job_1.accountDeletePermanentDenialId)(id))));
    let transactionBaseline = null;
    try {
        return await db.runTransaction(async (transaction) => {
            const [authMarkerSnap, authPermanentDenialSnap, tombstoneSnaps, permanentDenialSnaps, userSnap, authLinkSnap, existingOwnerSnap,] = await Promise.all([
            ...
            if (currentLinkStableId && !protectedStableIds.includes(currentLinkStableId)) {
                const [currentLinkTombstone, currentLinkPermanentDenial] = await Promise.all([
                    transaction.get(db.collection(account_delete_job_1.ACCOUNT_DELETE_TOMBSTONES).doc(currentLinkStableId)),
...
async function bootstrapMissingStableIdentity(db, authUid, stableId, provider, metadata) {
    ...
    const authPermanentDenialRef = db.collection(account_delete_job_1.ACCOUNT_DELETE_PERMANENT_DENIALS)
        .doc((0, account_delete_job_1.accountDeletePermanentDenialId)(authUid));
    const stablePermanentDenialRef = db.collection(account_delete_job_1.ACCOUNT_DELETE_PERMANENT_DENIALS)
        .doc((0, account_delete_job_1.accountDeletePermanentDenialId)(stableId));
    const existingIdentityQuery = db.collection(USERS).where('firebaseAuthUid', '==', authUid).limit(2);
    try {
        return await db.runTransaction(async (transaction) => {
            const [userSnap, authLinkSnap, authMarkerSnap, tombstoneSnap, authPermanentDenialSnap, stablePermanentDenialSnap, existingIdentitySnap,] = await Promise.all([
                transaction.get(userRef),
                transaction.get(authLinkRef),
                transaction.get(authMarkerRef),
            ...
            if (existingIdentityDocs.length === 1) {
                ...
                const [authoritativeTombstoneSnap, authoritativePermanentDenialSnap] = authoritativeStableId === stableId
                    ...
                    : await Promise.all([
                        transaction.get(db.collection(account_delete_job_1.ACCOUNT_DELETE_TOMBSTONES).doc(authoritativeStableId)),
                        transaction.get(db.collection(account_delete_job_1.ACCOUNT_DELETE_PERMANENT_DENIALS)
...
exports.authEnsureStableLink = (0, https_1.onCall)({
    ...
    // зачем (ревизия тёплых инстансов 2026-08-29, память 22.08): постоянный
    // тёплый инстанс стоит заметных денег каждый месяц, а вход и так делает
    // несколько сетевых раунд-трипов — лишняя секунда холодного старта функции
    // раз в десятки минут на этом фоне не читается. Тёплым остаётся только
    // maxVoiceMint: старт звонка MAX — премиум-момент, где секунда слышна.
    minInstances: 0,
}, async (request) => {
