...
function parsePersistedTask(value) {
    ...
    return Object.freeze({ ...draft, status: status(input.status), assignedAgentId: id(input.assignedAgentId, 'assignedAgentId'), createdByUid: id(input.createdByUid, 'createdByUid'), createdAtMs: integer(input.createdAtMs, 'createdAtMs'), updatedAtMs: integer(input.updatedAtMs, 'updatedAtMs'), revision: integer(input.revision, 'revision', 1), result: input.result === null ? null : (0, contracts_1.parseManagerTaskResult)(input.result) });
}
...
function parseCreateInput(value) { return (0, contracts_1.parseManagerTaskDraft)(value); }
function parseTransitionInput(value) {
    ...
    return Object.freeze({ taskId: id(input.taskId, 'taskId'), expectedRevision: integer(input.expectedRevision, 'expectedRevision', 1), status: status(input.status), result: input.result === undefined || input.result === null ? null : (0, contracts_1.parseManagerTaskResult)(input.result) });
}
...
class AgentManagerLedger {
    ...
    async transitionTask(auth, value) {
        ...
        return this.repository.runTransaction(async (transaction) => {
            ...
            (0, contracts_1.assertManagerTaskTransition)(current.status, input.status);
