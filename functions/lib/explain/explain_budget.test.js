...
describe('enforceGlobalBudget — product-wide daily breaker, atomic', () => {
    async function callGlobal(n, now = NOW) {
        ...
        for (let i = 0; i < n; i++) {
            try {
                await (0, explain_budget_1.enforceGlobalBudget)(now);
            }
            catch {
                thrown++;
            }
    ...
    it('resets on the next UTC day', async () => {
        ...
        let threw = false;
        try {
            await (0, explain_budget_1.enforceGlobalBudget)(nextDay);
        }
        catch {
            threw = true;
        }
...
describe('constants have explicit values (audit blocker)', () => {
    it('GLOBAL_DAILY_CAP and USER_DAILY_GEN_CAP are finite positive numbers', () => {
        expect(explain_budget_1.GLOBAL_DAILY_CAP).toBe(100);
        expect(explain_budget_1.USER_DAILY_GEN_CAP).toBe(20);
    });
});
