...
import {
  computeMatchRewards,
  resolveStarDeltaWithNewbieGuard,
  type RewardInput,
} from './rewards';

const CFG = CONSTELLATION_DEFAULTS;

function baseInput(over?: Partial<RewardInput>): RewardInput {
  return {
    place: 1,
    isBot: false,
    golden: false,
...
describe('constellations/rewards — расчёт наград за место', () => {
  test('XP по месту из конфига [60,40,25,15]', () => {
    expect(computeMatchRewards(baseInput({ place: 1 })).xp).toBe(60);
    expect(computeMatchRewards(baseInput({ place: 2 })).xp).toBe(40);
    expect(computeMatchRewards(baseInput({ place: 3 })).xp).toBe(25);
    expect(computeMatchRewards(baseInput({ place: 4 })).xp).toBe(15);
  });

  test('осколки за место [3,1,1,0] (обновлено этапом 1.6) + пыль + звездопад + wager-выигрыш', () => {
    const r = computeMatchRewards(baseInput({
  ...
  test('wager: 1 место ×3, 2 место возврат, 3-4 сгорает', () => {
    expect(computeMatchRewards(baseInput({ place: 1, wager: 2 })).shards).toBe(3 + 2 * 3);
    expect(computeMatchRewards(baseInput({ place: 2, wager: 2 })).shards).toBe(1 + 2); // возврат
    expect(computeMatchRewards(baseInput({ place: 3, wager: 2 })).shards).toBe(1 + 0); // сгорела
    expect(computeMatchRewards(baseInput({ place: 4, wager: 2 })).shards).toBe(0 + 0);
  });

  test('дельта звёзд по месту [1,0,0,-1]', () => {
    expect(computeMatchRewards(baseInput({ place: 1 })).starDelta).toBe(1);
    expect(computeMatchRewards(baseInput({ place: 2 })).starDelta).toBe(0);
    expect(computeMatchRewards(baseInput({ place: 4 })).starDelta).toBe(-1);
  });

  test('SR-дельта по месту [25,10,-5,-20]', () => {
    expect(computeMatchRewards(baseInput({ place: 1 })).srDelta).toBe(25);
    expect(computeMatchRewards(baseInput({ place: 4 })).srDelta).toBe(-20);
  });
});
...
describe('constellations/rewards — защита новичка', () => {
  test('первые 5 матчей: отрицательная дельта звёзд НЕ применяется (только вверх)', () => {
    ...
    expect(resolveStarDeltaWithNewbieGuard(-1, 3, CFG)).toBe(0);
    expect(resolveStarDeltaWithNewbieGuard(-1, 4, CFG)).toBe(0); // 5-й матч (индекс 4) ещё защищён
    // положительная дельта проходит всегда
    expect(resolveStarDeltaWithNewbieGuard(1, 0, CFG)).toBe(1);
  });

  test('после 5 матчей минус применяется', () => {
    expect(resolveStarDeltaWithNewbieGuard(-1, 5, CFG)).toBe(-1);
    expect(resolveStarDeltaWithNewbieGuard(-1, 20, CFG)).toBe(-1);
  });

  test('SR-минус тоже гасится защитой новичка', () => {
    const r = computeMatchRewards(baseInput({ place: 4, matchesPlayedBefore: 2 }));
...
describe('constellations/rewards — античит-фарм и боты', () => {
  test('боту награды не начисляются (все нули)', () => {
    const r = computeMatchRewards(baseInput({ isBot: true, place: 1, dustEarned: 2 }));
    expect(r).toEqual({
  ...
  test('коллекционный дроп положен только не-ботам не-туториалу', () => {
    expect(computeMatchRewards(baseInput({ place: 3 })).collectibleEligible).toBe(true);
    expect(computeMatchRewards(baseInput({ isBot: true })).collectibleEligible).toBe(false);
  });
});
