...
const handlers = {
  grantHelperShards: () => block('window.grantHelperShards = async function(uid) {', '\n  window.grantHelperPlus'),
  grantHelperPlus: () => block('window.grantHelperPlus = async function(uid) {', '\n  window.editShards'),
  grantAdminPremiumChoice: () => block('async function grantAdminPremiumChoice(uid, spec) {', '\n  window.revokeAdminVip'),
  revokeAdminVip: () => block('window.revokeAdminVip = async function(uid) {', '\n  window.warnUserDirect'),
};
...
describe('live admin Plus/helper protected callable contract', () => {
  it('has exactly one cached callable getter for access and reward mutations', () => {
    expect(source.match(/httpsCallable\(functionsUs, 'adminGrantAccess'\)/g) || []).toHaveLength(1);
    expect(source).toContain('function getAdminGrantAccessCallable()');
    expect(source.match(/httpsCallable\(functionsUs, 'adminGrantReward'\)/g) || []).toHaveLength(1);
    expect(source).toContain('function getAdminGrantFn()');

    const existingRewardConsumer = block('window.grantReward = async function grantReward', '\n  window.viewAchievements');
  ...
  ] as const)('%s requires a reason and sends an idempotent VIP access command', (name, active) => {
  ...
  it.each([
    ...
    'revokeAdminVip',
    'grantHelperPlus',
  ] as const)('%s trusts only an acknowledged canonical server result before refreshing UI', (name) => {
    const body = handlers[name]();
    ...
    const expiry = body.indexOf('const expiresAtMs = Number(response.data.expiresAtMs);', acknowledged);
    const cachedMutation = body.indexOf('applyVipPatchToCachedUser(writeUid,', acknowledged);
  ...
  it.each([
    ...
    'grantAdminPremiumChoice',
    'revokeAdminVip',
  ] as const)('%s has no direct user mutation, client audit, or canonical fallback', (name) => {
    const body = handlers[name]();
    ...
    expect(body).not.toMatch(/target\s*&&\s*target\.uid|target\.uid\s*\|\|\s*uid/);
  });
