...
exports.MAX_WRONG_SCRIPT_RATIO = exports.MIN_OUTPUT_LEN = exports.MAX_MEANING_LEN = exports.MAX_PHRASE_LEN = void 0;
exports.validateExplainInput = validateExplainInput;
exports.sanitizeExplanationOutput = sanitizeExplanationOutput;
exports.expectedScriptFor = expectedScriptFor;
exports.wrongScriptRatio = wrongScriptRatio;
exports.heuristicReject = heuristicReject;
/** Max length of the source English phrase the client may send. */
exports.MAX_PHRASE_LEN = 200;
/** Max length of the native-language gloss (feeds the fallback text). */
exports.MAX_MEANING_LEN = 500;
/** Below this many non-space chars an explanation is "too short" to be real. */
exports.MIN_OUTPUT_LEN = 5;
/** If more than this fraction of LETTERS are outside the expected script, reject as wrong-language. */
exports.MAX_WRONG_SCRIPT_RATIO = 0.4;
...
function validateExplainInput(input) {
    ...
    if (!phraseEn)
        return { ok: false, reason: 'phrase_empty' };
    if (phraseEn.length > exports.MAX_PHRASE_LEN)
        return { ok: false, reason: 'phrase_too_long' };
    const meaning = asString(input.phraseMeaning).trim();
    if (!meaning)
        return { ok: false, reason: 'meaning_empty' };
    if (meaning.length > exports.MAX_MEANING_LEN)
 ...
 */
function sanitizeExplanationOutput(raw) {
    let s = asString(raw);
...
function expectedScriptFor(lang) {
 ...
 */
function heuristicReject(text, lang) {
    const trimmed = asString(text).trim();
    if (trimmed.length === 0)
        return 'empty';
    if (trimmed.replace(/\s+/g, '').length < exports.MIN_OUTPUT_LEN)
        return 'too_short';
    if (wrongScriptRatio(trimmed, lang) > exports.MAX_WRONG_SCRIPT_RATIO)
        return 'non_target_language';
    return null;
}
//# sourceMappingURL=explain_gates.js.map