...
exports.resolveV2ProgressAppCheckEnforcement = resolveV2ProgressAppCheckEnforcement;
 ...
 */
function resolveV2ProgressAppCheckEnforcement(environment = process.env) {
...
function normalizeProgressStableUid(value) {
    ...
    if (!/^[A-Za-z0-9._-]{1,160}$/.test(stableUid) ||
        stableUid === '.' ||
        stableUid === '..') {
        throw new https_1.HttpsError('failed-precondition', 'stable_id_required');
    }
...
function createProgressEventHandler(authorize, execute) {
    return async (request) => {
        ...
        let input;
        try {
            input = (0, progress_event_1.parseProgressEventRequest)(request.data);
        }
        catch (error) {
            if (error instanceof https_1.HttpsError)
                throw error;
            throw new https_1.HttpsError('invalid-argument', error instanceof Error ? error.message : 'v2_progress_event_request_invalid');
        }
