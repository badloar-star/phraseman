...
const OUT_AUDIT = path.join(OUT_DIR, 'fr_personal_practice_native_bank_review_decision_schema_gate_v1.json');
const OUT_MD = path.join(OUT_DIR, 'fr_personal_practice_native_bank_review_decision_schema_gate_v1.md');
...
function validateDecision(row, request, findings) {
  ...
  if (!request) {
    fail('decision_has_no_matching_request', 'Decision has no matching native-bank review request.');
    return blockers;
  }
...
function runFixtureProbes(request) {
  ...
  add('accept_with_correction_payload_rejected', false, (row) => { row.correctedFrenchSkillName = 'correction'; });
  add('needs_review_without_payload_rejected', false, (row) => { row.reviewerDecision = 'needs_llm_regeneration_review'; row.gateReviewerDecisions.french_native_skill_fit_gate = 'fail'; });
  return probes;
}
...
function main() {
  ...
  if (!decisionFilePresent) addFinding(findings, 'info', 'decision_file_missing', 'No native-bank review decision file exists yet.');
  ...
  const report = {
    schemaVersion: 'gustav-fr-personal-practice-native-bank-review-decision-schema-gate-v1',
    generatedAt,
  ...
  fs.writeFileSync(OUT_MD, `# French Personal Practice Native Bank Decision Schema Gate\n\nStatus: ${report.status}\n\nDecision rows: ${decisions.length}/${requests.length}\n\nActivation approved: false\n`, 'utf8');
  console.log(`Gustav French personal-practice native-bank decision schema gate: ${report.status}`);
