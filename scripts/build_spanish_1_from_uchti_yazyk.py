"""Build SPANISH_1 as a Russian-first, Spanish-second native CapCut draft."""
...
def backup(project: Path, paths: list[Path], root: Path) -> Path:
    destination = root / f"SPANISH_1.before-build.{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
def write_mirrors(paths: list[Path], payload: bytes) -> None:
    for path in paths:
        temporary = path.with_name(f"{path.name}.spanish-new")
...
def main() -> int:
    ...
    parser.add_argument("--spanish-list", type=Path, required=True)
    parser.add_argument("--russian-dir", type=Path, required=True)
    parser.add_argument("--spanish-dir", type=Path, required=True)
    ...
    if any(path.read_bytes() != original for path in native_mirrors):
        fail("Native mirrors differ before the Spanish build")
    spanish = [line.strip() for line in args.spanish_list.read_text(encoding="utf-8-sig").splitlines() if line.strip()]
    if len(spanish) != ROWS:
        fail("Spanish source must contain exactly 300 rows")
    russian_files = numbered_audio(args.russian_dir.resolve())
    spanish_files = numbered_audio(args.spanish_dir.resolve())
    if not args.intro.is_file():
        fail("Russian intro file is missing")
    russian_durations = [duration_us(path) for path in russian_files]
    spanish_durations = [duration_us(path) for path in spanish_files]
    ...
    spanish_text_ids = ids(tracks[8], ROWS)
    ...
    spanish_audio_ids = ids(tracks[16], ROWS)
    ...
    for index, material_id in enumerate(spanish_text_ids):
        set_text(texts[material_id], wrap(spanish[index]))
    for material_id in phonetic_text_ids:
        # English IPA must never appear beneath Spanish text. Keep the native
        # layer for future Spanish IPA, but hide it rather than mislabel words.
    ...
    assets = project / "Resources" / "lingman_spanish_a1_20260813"
    intro_path = "##_draftpath_placeholder_0E685133-18CE-45ED-8CB8-2904A212EC80_##/Resources/lingman_spanish_a1_20260813/intro/intro_es.mp3"
    for material_id in intro_audio_ids:
        audios[material_id].update({"path": intro_path, "name": "intro_es.mp3", "duration": intro_duration})
    ...
    for index in range(ROWS):
        ...
        es_target = assets / "es" / f"es_{index + 1:03d}.mp3"
        audios[russian_audio_ids[index]].update({"path": f"##_draftpath_placeholder_0E685133-18CE-45ED-8CB8-2904A212EC80_##/Resources/lingman_spanish_a1_20260813/ru/{ru_target.name}", "name": ru_target.name, "duration": russian_durations[index]})
        audios[spanish_audio_ids[index]].update({"path": f"##_draftpath_placeholder_0E685133-18CE-45ED-8CB8-2904A212EC80_##/Resources/lingman_spanish_a1_20260813/es/{es_target.name}", "name": es_target.name, "duration": spanish_durations[index]})
        for segment, duration in ((tracks[15]["segments"][index], russian_durations[index]), (tracks[16]["segments"][index], spanish_durations[index])):
            segment["source_timerange"]["duration"] = duration
            segment["target_timerange"]["duration"] = duration

    # Every Russian phrase must finish before its Spanish answer starts, and
    # Spanish must end before the next Russian phrase / intro boundary.
    russian_starts = [int(segment["target_timerange"]["start"]) for segment in tracks[15]["segments"]]
    spanish_starts = [int(segment["target_timerange"]["start"]) for segment in tracks[16]["segments"]]
    ...
    for index in range(ROWS):
        ...
        latest = russian_windows[index] - spanish_durations[index]
        start = min(spanish_starts[index], latest)
        if start < earliest:
            errors.append(f"Spanish cannot fit after Russian at {index + 1}")
            continue
        tracks[16]["segments"][index]["target_timerange"]["start"] = start
        spanish_starts[index] = start
    if errors:
        fail("Voice timing gate failed: " + "; ".join(errors[:6]))
    payload = json.dumps(draft, ensure_ascii=False, separators=(",", ":")).encode("utf-8")
    report = {"status": "dry-run", "russian": len(russian_files), "spanish": len(spanish_files), "introDurationUs": intro_duration, "headline": HEADLINE, "phraseTextTimingsPreserved": True, "introTracks": [5, 6, len(draft["tracks"]) - 1]}
    if not args.apply:
        print(json.dumps(report, ensure_ascii=False))
        return 0
    saved = backup(project, native_mirrors, args.backup_root.resolve())
    copy_asset(args.intro, assets / "intro" / "intro_es.mp3")
    for index, source in enumerate(russian_files, start=1):
        copy_asset(source, assets / "ru" / f"ru_{index:03d}.mp3")
    for index, source in enumerate(spanish_files, start=1):
        copy_asset(source, assets / "es" / f"es_{index:03d}.mp3")
    write_mirrors(native_mirrors, payload)
    if any(path.read_bytes() != payload for path in native_mirrors):
        fail("Native mirrors differ after Spanish build")
