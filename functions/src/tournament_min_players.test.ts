...
describe('порог живых игроков', () => {
  ...
  it('дев-комната обходит порог — иначе владелец не сыграет один', () => {
    // devRoom:true снимает требование живых. Без этого дев-кнопка создавала бы
    // комнату, которую крон тут же отменял бы.
    expect(tournamentsSource).toMatch(/isDevRoom\s*&&/);
    expect(tournamentsSource).toMatch(/devRoom.*!==\s*true.*TOURNAMENT_MIN_REAL_PLAYERS/s);
  });
