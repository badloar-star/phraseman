...
// его рисует штатный AvatarView. Боты засеяны с эмодзи — на клиенте каждому
// боту детерминированно (по id) выдаём НАСТОЯЩИЙ аватар приложения:
// «рандомно — могут быть уровневые, могут быть кастомные» (слова владельца).
 ...
 * Бот — детерминированный аватар из наборов приложения:
 * каждый третий бот получает кастомный (градиент+лого), остальные — уровневый.
 */
export function tournamentAvatarValue(player: TournamentPlayerLike): string {
  ...
  if (!player.isBot && isAppValue) return raw;
