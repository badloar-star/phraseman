...
const { sanitizeBriefing, parseAndGuardResult, buildSystemPrompt, briefingHashForReplay, decideWeeklyReviewReplay, readStoredWeeklyReview, runWeeklyReviewPreflight, asOpenAIChatResponse, } = weekly_review_1.__weeklyReviewTestHooks;
...
describe('weekly_review Plus-only preflight ordering', () => {
    it('rejects Free before config, replay, rate, lease, budget, provider, or billing', async () => {
        ...
        await expect(runWeeklyReviewPreflight({ authUid: 'auth-free', rawBriefing: baseBriefing(), db: {} }, {
            requireAuth: (uid) => { trace.push('auth'); return uid; },
            sanitize: (raw) => { trace.push('sanitize'); return raw; },
