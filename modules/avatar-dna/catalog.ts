...
const parseAvatarCatalogUnsafe = (input: unknown): AvatarCatalogManifest => {
  ...
  const items = asArray(input.items, 'items').map((value): AvatarItemManifest => {
    ...
    const layers = asArray(value.layers, 'layers').map((layer): AvatarItemManifest['layers'][number] => {
      ...
      if (!privateAvatarId.test(layerId) || layerIds.has(layerId) || !slots.has(slot as AvatarSlot) || !safeFile(file) || !Number.isInteger(layer.z) || (layer.z as number) < 0 || (layer.z as number) > 179 || (layer.clip !== undefined && layer.clip !== 'face.safe' && layer.clip !== 'head.safe') || (layer.tintFrom !== undefined && layer.tintFrom !== 'skinTone' && layer.tintFrom !== 'hairColor' && layer.tintFrom !== 'irisColor') || (layer.bytes !== undefined && (!Number.isInteger(layer.bytes) || (layer.bytes as number) <= 0)) || (layer.sha256 !== undefined && (typeof layer.sha256 !== 'string' || !/^[a-f0-9]{64}$/.test(layer.sha256))) || ((layer.bytes === undefined) !== (layer.sha256 === undefined))) throw new TypeError('avatar_catalog_invalid: layer');
      layerIds.add(layerId); return { id: layerId, slot: slot as AvatarSlot, z: layer.z as number, file, ...(typeof layer.clip === 'string' ? { clip: layer.clip } : {}), ...(layer.tintFrom === 'skinTone' || layer.tintFrom === 'hairColor' || layer.tintFrom === 'irisColor' ? { tintFrom: layer.tintFrom } : {}) };
    });
