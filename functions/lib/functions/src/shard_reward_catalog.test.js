...
describe('generic shard earn compatibility catalog (zeroed by 2026-07-20 economy)', () => {
    it.each([
        ...
        ['survey_completed', 3],
    ])('rejects client-initiated earn %s=%d (coins are purchase-only now)', (reason, delta) => {
        expect((0, shard_reward_catalog_1.resolveShardEarnPolicy)(reason, delta)).toBeNull();
    });
    it.each([
        'toString',
        'constructor',
        ...
        expect((0, shard_reward_catalog_1.resolveShardEarnPolicy)(reason, 1)).toBeNull();
    });
    it('rejects non-positive amounts even for known reasons', () => {
        expect((0, shard_reward_catalog_1.resolveShardEarnPolicy)('lesson_first', 0)).toBeNull();
        expect((0, shard_reward_catalog_1.resolveShardEarnPolicy)('lesson_first', -5)).toBeNull();
        expect((0, shard_reward_catalog_1.resolveShardEarnPolicy)('lesson_first', 1.5)).toBeNull();
    });
});
describe('generic shard earn UTC-day budgets', () => {
    ...
    it('bounds earn operations by both source and total policy', () => {
        ...
        let counter = (0, shard_reward_catalog_1.emptyShardEarnDailyCounter)('2026-07-18');
        let granted = 0;
        for (let index = 0; index < 1000; index += 1) {
            const outcome = (0, shard_reward_catalog_1.applyShardEarnBudget)(counter, policy, false);
            if (!outcome.allowed)
                continue;
        ...
        expect(granted).toBeLessThanOrEqual(shard_reward_catalog_1.SHARD_EARN_DAILY_TOTAL_MAX);
    });
    it('does not consume source or total budget for an exact receipt retry', () => {
        const policy = syntheticPolicy;
        const full = {
            dayKey: '2026-07-18',
            totalEarned: shard_reward_catalog_1.SHARD_EARN_DAILY_TOTAL_MAX,
            bySource: { [policy.reasonKey]: policy.perSourceDailyMax },
        };
        expect((0, shard_reward_catalog_1.applyShardEarnBudget)(full, policy, true)).toEqual({
            allowed: true,
            counted: false,
            counter: full,
        });
    ...
    it('isolates counters across owners and UTC days', () => {
        const policy = syntheticPolicy;
        const ownerADay1 = (0, shard_reward_catalog_1.applyShardEarnBudget)((0, shard_reward_catalog_1.emptyShardEarnDailyCounter)('2026-07-18'), policy, false);
        const ownerBDay1 = (0, shard_reward_catalog_1.applyShardEarnBudget)((0, shard_reward_catalog_1.emptyShardEarnDailyCounter)('2026-07-18'), policy, false);
        const ownerADay2 = (0, shard_reward_catalog_1.applyShardEarnBudget)((0, shard_reward_catalog_1.emptyShardEarnDailyCounter)('2026-07-19'), policy, false);
        expect(ownerADay1.allowed).toBe(true);
        expect(ownerBDay1.allowed).toBe(true);
        expect(ownerADay2.allowed).toBe(true);
        expect((0, shard_reward_catalog_1.utcShardEarnDayKey)(Date.UTC(2026, 6, 18, 23, 59, 59))).toBe('2026-07-18');
        expect((0, shard_reward_catalog_1.utcShardEarnDayKey)(Date.UTC(2026, 6, 19, 0, 0, 0))).toBe('2026-07-19');
    });
});
//# sourceMappingURL=shard_reward_catalog.test.js.map