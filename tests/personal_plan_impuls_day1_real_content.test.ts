...
import {
  buildImpulsDay1GeneratedDayPacket,
...
describe('Impuls day 1 real content', () => {
  ...
  it('certifies the first short-story day with generator evidence', () => {
    expect(day).toEqual(expect.objectContaining({
      ...
      lifeOutcome: expect.stringContaining('пропустил автобус'),
    }));
  ...
  it('has complete material for every Impuls day-1 mode', () => {
    ...
    expect(lesson.phrases.map((phrase) => phrase.english)).toEqual([
      ...
      'First, I missed the bus.',
      'Then I called my friend.',
      'After that, I found another way.',
      'The story ends well.',
      'That is why I was late.',
    ]);
    expect(lesson.phrases.map((phrase) => phrase.russian)).toEqual([
      'Я могу рассказать короткую историю.',
      'Сначала я пропустил автобус.',
      'Потом я позвонил другу.',
      'После этого я нашел другой путь.',
    ...
    expect(getPersonalPlanQuizPhrases('impuls_day_1_quiz', 'Sam')).toHaveLength(10);
  });

  it('passes the generator pipeline before progress can mark the day certified', () => {
    const packet = buildImpulsDay1GeneratedDayPacket();
    const report = runPersonalPlanGenerationDryRunHarness({
      packet,
      reviewerId: 'impuls-day1-reviewer',
      reviewedAt: '2026-06-05T02:15:00.000Z',
    });
