...
describe('adaptive generation contract', () => {
  it('keeps generated practice user-scoped and bounded', () => {
    const input = { userId: 'u1', studyTarget: 'fr', surface: 'cards' as const, sourceLessonIds: [1, 2], maxItems: 10, reason: 'spaced_repetition' as const, activePackRevision: 4, expiresAt: new Date(Date.now() + 60_000).toISOString() };
    validateAdaptiveGenerationRequest(input);
    expect(adaptiveGenerationStoragePath(input)).toBe('users/u1/adaptive_content/fr/cards/r4');
