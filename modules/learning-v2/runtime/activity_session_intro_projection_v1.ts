...
export interface LearningV2ActivitySessionIntroProjectionV1
  extends LearningV2ActivitySessionIntroProjectionInputV1 {
...
const ROOT_KEYS = Object.freeze([
  "schemaVersion", "contentClass", "introId", "introFingerprint",
  "sourceSubjectFingerprint", "episodeId", "sessionId", "sessionOrdinal",
  "targetLanguage", "title", "pages", "pageCount", "embeddedQuestionCount",
  "practiceStartSlot", "slotPresentationPolicy", "learnerVisibleFieldPolicy",
  "evaluatorDataPolicy", "answerDataPolicy", "serverSidecarPolicy",
  "sourceContentAuthority", "languageAccuracyAuthority", "curriculumAuthority",
  "repositoryAuthority", "storageAuthority", "runtimeAuthority", "walletAuthority",
  "masteryAuthority", "evidenceAuthority", "completionAuthority",
  "publicationAuthority", "releaseAuthority", "projectionFingerprint",
] as const);
const PAGE_KEYS = Object.freeze([
  "pageOrdinal", "conceptId", "heading", "explanation", "question",
] as const);
const QUESTION_KEYS = Object.freeze([
  "taskId", "taskSlot", "questionId", "coveredConceptIds",
  "learnerSurfaceFingerprint", "promptId", "prompt", "responseOptions",
  "accessibilityLabel",
] as const);
...
function exactKeys(value: Record<string, unknown>, keys: readonly string[]) {
  ...
  if (actual.length !== expected.length || actual.some((key, i) => key !== expected[i])) fail();
}
function text(value: unknown, max: number): string {
  if (typeof value !== "string" || value.length < 1 || value.length > max ||
      value !== value.normalize("NFC") || CONTROL_RE.test(value)) fail();
  return value;
}
...
function parseValue(value: unknown): LearningV2ActivitySessionIntroProjectionV1 {
  if (!record(value)) fail();
  exactKeys(value, ROOT_KEYS);
  if (
    value.schemaVersion !== LEARNING_V2_ACTIVITY_SESSION_INTRO_PROJECTION_SCHEMA_V1 ||
    !["production_candidate", "neutral_test_fixture"].includes(String(value.contentClass)) ||
    !Number.isSafeInteger(value.sessionOrdinal) || Number(value.sessionOrdinal) < 1 ||
    Number(value.sessionOrdinal) > 12 || !Array.isArray(value.pages) ||
    value.pages.length !== 3 || value.pageCount !== 3 ||
    value.embeddedQuestionCount !== 3 || value.practiceStartSlot !== 4
  ) fail();
  ...
  const pages = Object.freeze(value.pages.map((entry, index) => {
    ...
    if (question.taskSlot !== index + 1 || !Array.isArray(question.coveredConceptIds) ||
        question.coveredConceptIds.length !== 1 || question.coveredConceptIds[0] !== conceptId ||
        !Array.isArray(question.responseOptions) || question.responseOptions.length < 2 ||
        question.responseOptions.length > 6) fail();
    ...
    taskIds.add(taskId); questionIds.add(questionId);
  ...
  const body = {
    ...
    contentClass: value.contentClass as "production_candidate" | "neutral_test_fixture",
    introId: id(value.introId), introFingerprint: hash(value.introFingerprint),
    sourceSubjectFingerprint: hash(value.sourceSubjectFingerprint),
    episodeId: token(value.episodeId), sessionId: id(value.sessionId),
    sessionOrdinal: Number(value.sessionOrdinal),
    targetLanguage: (() => { const result = text(value.targetLanguage, 255); if (!parseV2ExactLanguageTagV1(result)) fail(); return result; })(),
    title: text(value.title, 240), pages,
    pageCount: 3 as const, embeddedQuestionCount: 3 as const,
    practiceStartSlot: 4 as const,
    slotPresentationPolicy: "slots_1_2_3_are_embedded_in_intro_pages_and_must_not_repeat" as const,
    learnerVisibleFieldPolicy: "positive_allowlist_title_and_three_intro_pages_only" as const,
    evaluatorDataPolicy: "none" as const, answerDataPolicy: "none" as const,
    serverSidecarPolicy: "none" as const,
    sourceContentAuthority: "unverified_owner_input_claim" as const,
    languageAccuracyAuthority: "none" as const, curriculumAuthority: "none" as const,
    repositoryAuthority: "none" as const, storageAuthority: "none" as const,
    runtimeAuthority: "none_release_readback_required" as const,
    walletAuthority: "none" as const, masteryAuthority: "none" as const,
    evidenceAuthority: "none" as const, completionAuthority: "none" as const,
    publicationAuthority: "none" as const, releaseAuthority: false as const,
  };
  for (const [key, expected] of Object.entries({
    slotPresentationPolicy: body.slotPresentationPolicy,
    learnerVisibleFieldPolicy: body.learnerVisibleFieldPolicy,
    evaluatorDataPolicy: "none", answerDataPolicy: "none", serverSidecarPolicy: "none",
    sourceContentAuthority: body.sourceContentAuthority, languageAccuracyAuthority: "none",
    curriculumAuthority: "none", repositoryAuthority: "none", storageAuthority: "none",
    runtimeAuthority: body.runtimeAuthority, walletAuthority: "none", masteryAuthority: "none",
    evidenceAuthority: "none", completionAuthority: "none", publicationAuthority: "none",
    releaseAuthority: false,
  })) if (value[key] !== expected) fail();
  if (value.projectionFingerprint !== hashCanonicalBody(body)) fail();
  const result = Object.freeze({ ...body, projectionFingerprint: hash(value.projectionFingerprint) });
  handles.add(result);
  return result;
}
...
export function materializeLearningV2ActivitySessionIntroProjectionV1(
  ...
  const body = {
    schemaVersion: LEARNING_V2_ACTIVITY_SESSION_INTRO_PROJECTION_SCHEMA_V1,
    ...input, pageCount: 3 as const, embeddedQuestionCount: 3 as const,
    practiceStartSlot: 4 as const,
    slotPresentationPolicy: "slots_1_2_3_are_embedded_in_intro_pages_and_must_not_repeat" as const,
    learnerVisibleFieldPolicy: "positive_allowlist_title_and_three_intro_pages_only" as const,
    evaluatorDataPolicy: "none" as const, answerDataPolicy: "none" as const,
  ...
  return parseLearningV2ActivitySessionIntroProjectionV1(canonicalJsonV1({
    ...body, projectionFingerprint: hashCanonicalBody(body),
  }));
}
...
export function parseLearningV2ActivitySessionIntroProjectionV1(raw: string) {
  if (typeof raw !== "string" || raw.length < 2 ||
      ...
      utf8ByteLengthV1(raw) > LEARNING_V2_ACTIVITY_SESSION_INTRO_PROJECTION_MAX_BYTES_V1) fail();
  let value: unknown;
  try { value = JSON.parse(raw); } catch { fail(); }
  if (canonicalJsonV1(value) !== raw) fail();
  return parseValue(value);
}
export function encodeLearningV2ActivitySessionIntroProjectionV1(value: LearningV2ActivitySessionIntroProjectionV1) {
  if (!isLearningV2ActivitySessionIntroProjectionV1(value)) fail();
  return canonicalJsonV1(value);
}
export function isLearningV2ActivitySessionIntroProjectionV1(value: unknown): value is LearningV2ActivitySessionIntroProjectionV1 {
