...
const execution_outbox_1 = require("./execution_outbox");
...
describe('agent-manager execution outbox', () => {
    test('creates one strict PII-free job for the matching queued support task', async () => {
        ...
        const result = await (0, execution_outbox_1.issueExecutionJobForQueuedEvent)(repo, { taskId: 'task-001', taskRevision: 3, toStatus: 'queued' }, 2000000000000);
    ...
    test('is idempotent for an event replay and does not overwrite the original job', async () => {
        ...
        await (0, execution_outbox_1.issueExecutionJobForQueuedEvent)(repo, { taskId: 'task-001', taskRevision: 3, toStatus: 'queued' }, 2000000000000);
        const replay = await (0, execution_outbox_1.issueExecutionJobForQueuedEvent)(repo, { taskId: 'task-001', taskRevision: 3, toStatus: 'queued' }, 2000000000999);
        expect(replay).toEqual({ created: false, jobId: 'task-001__r3' });
        expect(repo.documents.size).toBe(2);
        expect(repo.documents.get('agent_manager_execution_jobs/task-001__r3')?.data.createdAtMs).toBe(2000000000000);
    });
    test('creates a job for the maximum-length valid task id without exceeding the execution key contract', async () => {
        ...
        const result = await (0, execution_outbox_1.issueExecutionJobForQueuedEvent)(repo, { taskId, taskRevision: 3, toStatus: 'queued' }, 2000000000000);
    ...
    test.each([
        ...
        [{ taskId: 'task-001', taskRevision: 3, toStatus: 'queued' }, queuedTask({ allowedScope: 'code_prepare' })],
        ...
        const result = await (0, execution_outbox_1.issueExecutionJobForQueuedEvent)(repo, event, 2000000000000);
...
//# sourceMappingURL=execution_outbox.test.js.map