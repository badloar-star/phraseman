...
async function validateCatalogUnsafe(catalog, rig, { fixture = false, root = process.cwd(), assetsRoot } = {}) {
  ...
  for (const item of catalog.items) {
    ...
    for (const layer of item.layers) {
      ...
      if (!exact(layer, layerKeys) || !ids.test(layer.id) || layerIds.has(layer.id) || !slots.has(layer.slot) || !Number.isInteger(layer.z) || layer.z < 0 || layer.z > 179 || typeof layer.file !== 'string' || !fileSafe.test(layer.file) || isAbsolute(layer.file) || layer.file.includes('\\') || layer.file.includes('..') || layer.file.includes('://') || (layer.clip !== undefined && !Object.hasOwn(rig.safePolygons, layer.clip)) || (layer.tintFrom !== undefined && layer.tintFrom !== 'skinTone' && layer.tintFrom !== 'hairColor') || (layer.bytes !== undefined && (!Number.isInteger(layer.bytes) || layer.bytes <= 0)) || (layer.sha256 !== undefined && (typeof layer.sha256 !== 'string' || !/^[a-f0-9]{64}$/.test(layer.sha256))) || ((layer.bytes === undefined) !== (layer.sha256 === undefined))) fail('unsafe or invalid layer'); layerIds.add(layer.id);
      if (!fixture) {
