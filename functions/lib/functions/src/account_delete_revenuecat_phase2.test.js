...
describe('account deletion RevenueCat permanent denial Phase 2 (RED)', () => {
    ...
    it.each([
        ...
        expect(handler).toContain('ACCOUNT_DELETE_PERMANENT_DENIALS');
        expect(handler.indexOf('ACCOUNT_DELETE_PERMANENT_DENIALS')).toBeLessThan(handler.indexOf("db.collection('users')"));
    });
});
