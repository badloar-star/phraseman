...
const source = (name) => node_fs_1.default.readFileSync(node_path_1.default.join(__dirname, '..', name), 'utf8');
const matrix = [
...
describe('R7 content factory permission and audit matrix', () => {
    it.each(matrix)('$action denies missing claims and permits only the least role or stronger', ({ permission, leastRole }) => {
        expect((0, permissions_1.hasClaimedPermission)(undefined, permission)).toBe(false);
        expect((0, permissions_1.hasClaimedPermission)({ admin: false, adminRole: leastRole }, permission)).toBe(false);
        expect((0, permissions_1.hasClaimedPermission)({ admin: true, adminRole: 'support' }, permission)).toBe(false);
        expect((0, permissions_1.hasPermission)(leastRole, permission)).toBe(true);
        expect((0, permissions_1.hasClaimedPermission)({ admin: true, adminRole: leastRole }, permission)).toBe(true);
    });
    it.each(matrix)('$action callable keeps App Check, permission and transactional audit evidence', ({ file, permission, auditAction, auditFile }) => {
        ...
        const auditSource = auditFile ? source(auditFile) : node_fs_1.default.readFileSync(node_path_1.default.join(__dirname, 'generation_audit.ts'), 'utf8');
        const text = `${worker}\n${auditSource}`;
        expect(text).toContain('enforceAppCheck: ENFORCE_APP_CHECK');
        expect(text).toContain(permission);
        expect(text).toContain(auditAction);
        for (const field of ['actorUid', 'role', 'entity', 'operationId', 'reason'])
...
//# sourceMappingURL=r7_permission_audit_contract.test.js.map