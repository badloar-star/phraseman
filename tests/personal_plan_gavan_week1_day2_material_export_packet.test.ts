...
import {
  ...
  GAVAN_WEEK1_DAY2_MATERIAL_EXPORT_PACKET_PATH,
  writeGavanWeek1Day2MaterialExportPacket,
} from '../tools/personal_plan_gavan_week1_day2_material_export_packet';

const GENERATED_AT = '2026-06-03T16:00:00.000Z';
...
function candidate() {
  return buildGavanWeek1Day2MaterialCandidate(seed(), day1Export(), {
    generatedAt: GENERATED_AT,
  });
}

describe('Gavan week 1 day 2 material quality export packet', () => {
  beforeEach(() => {
    if (existsSync(GAVAN_WEEK1_DAY2_MATERIAL_EXPORT_PACKET_PATH)) {
      rmSync(GAVAN_WEEK1_DAY2_MATERIAL_EXPORT_PACKET_PATH, { force: true });
    }
  });

  afterAll(() => {
    writeGavanWeek1Day2MaterialExportPacket(candidate(), seed(), day1Export(), {
      generatedAt: GENERATED_AT,
      targetPath: GAVAN_WEEK1_DAY2_MATERIAL_EXPORT_PACKET_PATH,
    });
  });

  it('builds a non-live reviewer export packet from the day 2 material candidate', () => {
    const packet = buildGavanWeek1Day2MaterialExportPacket(candidate(), seed(), day1Export(), {
      generatedAt: GENERATED_AT,
    });

    expect(packet).toEqual(expect.objectContaining({
      kind: 'gavan_week1_day2_material_export_packet',
      generatedAt: GENERATED_AT,
      planId: 'gavan',
      weekId: 'gavan-week1',
      dayId: 'gavan-week1-day2',
      status: 'day2_material_export_not_live',
      sourceCandidateStatus: 'day2_material_candidate_not_live',
  ...
  it('summarizes blocks phrases tiles listening placeholders recall and quiz intent', () => {
    const packet = buildGavanWeek1Day2MaterialExportPacket(candidate(), seed(), day1Export(), {
      generatedAt: GENERATED_AT,
    });
  ...
  it('includes pass quality gates for the valid day 2 candidate', () => {
    const packet = buildGavanWeek1Day2MaterialExportPacket(candidate(), seed(), day1Export(), {
      generatedAt: GENERATED_AT,
    });
  ...
  it('includes a compact human-readable preview of day 2', () => {
    const packet = buildGavanWeek1Day2MaterialExportPacket(candidate(), seed(), day1Export(), {
      generatedAt: GENERATED_AT,
    });
    ...
    expect(packet.preview.listeningLine).toContain('4');
  ...
  it('blocks export quality when candidate gates fail instead of approving it', () => {
    ...
    const packet = buildGavanWeek1Day2MaterialExportPacket(brokenCandidate as any, seed(), day1Export(), {
      generatedAt: GENERATED_AT,
    });
    const failedGates = packet.qualityGates.filter((gate) => gate.status === 'fail');

    expect(packet.status).toBe('day2_material_export_blocked');
  ...
  it('writes deterministic export JSON only under temp or report roots', () => {
    const result = writeGavanWeek1Day2MaterialExportPacket(candidate(), seed(), day1Export(), {
      generatedAt: GENERATED_AT,
      targetPath: GAVAN_WEEK1_DAY2_MATERIAL_EXPORT_PACKET_PATH,
    });
    ...
    expect(result.targetPath).toBe(path.resolve(
      ...
      '.codex-tmp',
      'personal-plans',
      'gavan-week1-day2-material-export-packet.json',
    ));
    expect(existsSync(GAVAN_WEEK1_DAY2_MATERIAL_EXPORT_PACKET_PATH)).toBe(true);

    const parsed = JSON.parse(readFileSync(GAVAN_WEEK1_DAY2_MATERIAL_EXPORT_PACKET_PATH, 'utf8'));
    expect(parsed.kind).toBe('gavan_week1_day2_material_export_packet');
    expect(parsed.status).toBe('day2_material_export_not_live');
  });

  it('rejects app tools tests and root config write targets', () => {
    const appResult = writeGavanWeek1Day2MaterialExportPacket(candidate(), seed(), day1Export(), {
      generatedAt: GENERATED_AT,
      targetPath: path.join(process.cwd(), 'app', 'gavan-week1-day2-export.json'),
    });
    const toolsResult = writeGavanWeek1Day2MaterialExportPacket(candidate(), seed(), day1Export(), {
      generatedAt: GENERATED_AT,
      targetPath: path.join(process.cwd(), 'tools', 'gavan-week1-day2-export.json'),
    });
    const testsResult = writeGavanWeek1Day2MaterialExportPacket(candidate(), seed(), day1Export(), {
      generatedAt: GENERATED_AT,
      targetPath: path.join(process.cwd(), 'tests', 'gavan-week1-day2-export.json'),
    });
  ...
  it('does not import or mutate live app routes catalog quiz UI storage audio scoring or navigation', () => {
    const source = readFileSync(
      path.join(process.cwd(), 'tools', 'personal_plan_gavan_week1_day2_material_export_packet.ts'),
      'utf8',
    );
