...
describe("Firestore V2 authoring storage paths", () => {
    it("pins draft and immutable revision namespaces", () => {
        ...
        expect((0, firestore_authoring_store_1.episodeRevisionDocumentPath)("draft-1", 3)).toBe("content_episode_revisions/draft-1__r3");
        ...
        expect((0, firestore_authoring_store_1.modeTemplateVersionDocumentPath)("template-1", 3)).toBe("content_mode_template_versions/template-1__v3");
    });
    it.each(["published", "deprecated"])("resolves a %s ModeTemplate and exposes only its override allowlist", async (status) => {
        const fixture = JSON.parse((0, node_fs_1.readFileSync)(node_path_1.default.resolve(__dirname, "../../../tests/fixtures/learning-v2/episode-01.valid.json"), "utf8"));
        ...
        const objectPath = `content-studio/mode-templates/${(0, decision_registry_1.sha256Utf8)(templateId)}/v${version}/${contentHash}.json`;
        const body = template.body;
        const byteSize = Buffer.byteLength((0, decision_registry_1.canonicalJsonV1)(body), "utf8");
        const db = {
            doc: (path) => ({
                get: async () => path === (0, firestore_authoring_store_1.modeTemplateVersionDocumentPath)(templateId, version)
                    ? {
                        exists: true,
                        data: () => ({
                    ...
                    : path === (0, firestore_authoring_store_1.modeTemplateLifecycleDocumentPath)(templateId, version)
                        ? {
                            exists: true,
                            data: () => ({
        ...
        await expect(resolver({ templateId, version, contentHash })).resolves.toEqual({
            templateRef: { templateId, version, contentHash },
            allowedOverridePaths: body.authoring
    ...
    it.each(["approved", "archived", "draft", "hidden", "deprecated"])("rejects a ModeTemplate lifecycle status that is not published/deprecated (%s)", async (status) => {
        const fixture = JSON.parse((0, node_fs_1.readFileSync)(node_path_1.default.resolve(__dirname, "../../../tests/fixtures/learning-v2/episode-01.valid.json"), "utf8"));
        const template = fixture.dependencies.templates[0];
        const { templateId, version, contentHash } = template.templateRef;
        const objectPath = `content-studio/mode-templates/${(0, decision_registry_1.sha256Utf8)(templateId)}/v${version}/${contentHash}.json`;
        const byteSize = Buffer.byteLength((0, decision_registry_1.canonicalJsonV1)(template.body), "utf8");
        const db = {
            doc: (requestedPath) => ({
                get: async () => requestedPath === (0, firestore_authoring_store_1.modeTemplateVersionDocumentPath)(templateId, version)
                    ? {
                        exists: true,
                        data: () => ({
                            schemaVersion: "v2-mode-template-record.v1",
    ...
    it("exposes the canonical shared ModeTemplate semantic validator", () => {
        expect((0, validation_1.validateModeTemplateArtifactBody)({
            schemaVersion: "v2-mode-template-body.v1",
    ...
    it("shares strict lifecycle XOR semantics with the resolver", () => {
        ...
        expect((0, validation_1.validateModeTemplateLifecycleHead)(base).ok).toBe(true);
        expect((0, validation_1.validateModeTemplateLifecycleHead)({ ...base, noReplacement: false }).ok).toBe(false);
        expect((0, validation_1.validateModeTemplateLifecycleHead)({
            ...base,
            status: "deprecated",
            noReplacement: true,
        }).ok).toBe(true);
    ...
    it("accepts the repository's complete normative ModeTemplate corpus", () => {
        ...
        for (const template of fixture.dependencies.templates) {
            expect((0, validation_1.validateModeTemplateArtifactBody)(template.body).ok).toBe(true);
        }
    });
    it("reads and verifies the exact immutable Storage generation", async () => {
        ...
        const path = (0, episode_revision_resolver_1.episodeRevisionObjectPath)(ref.draftId, ref.revision, ref.contentHash);
        const db = {
            doc: () => ({
                get: async () => ({
                    exists: true,
        ...
        const resolver = (0, firestore_authoring_store_1.createFirestoreEpisodeRevisionResolver)(db, {
            read: async (requestedPath, expectedHash, expectedGeneration, expectedByteSize) => {
    ...
    it.each([
        ...
        const ref = {
            ...
            revision: 1,
            revisionFingerprint: (0, episode_revision_resolver_1.episodeRevisionFingerprint)("draft-storage-failure", 1, contentHash),
            contentHash,
            ordinal: 1,
            chapterId: "chapter-1",
            approvalStatus: "approved",
        };
        const objectPath = (0, episode_revision_resolver_1.episodeRevisionObjectPath)(ref.draftId, ref.revision, ref.contentHash);
    ...
    it("rejects a resolver reader result with mismatched pinned generation", async () => {
        ...
        const ref = {
            ...
            revision: 1,
            revisionFingerprint: (0, episode_revision_resolver_1.episodeRevisionFingerprint)("draft-generation-mismatch", 1, contentHash),
            contentHash,
            ordinal: 1,
            chapterId: "chapter-1",
            approvalStatus: "approved",
        ...
        await expect((0, firestore_authoring_store_1.createFirestoreEpisodeRevisionResolver)(db, {
            read: async () => ({
                body,
                contentHash,
        ...
        }).resolve(ref)).rejects.toThrow("season_episode_object_generation_invalid");
    });
    it("rejects a malformed DecisionRegistry instead of raw-casting it", async () => {
        ...
        await expect((0, firestore_authoring_store_1.createFirestoreDecisionRegistryResolver)(db).resolve({
            id: "registry",
            version: 1,
            contentHash: "a".repeat(64),
    ...
    it("rejects DecisionRegistry Storage metadata that does not match the pinned object", async () => {
        ...
        const contentHash = (0, decision_registry_1.hashCanonicalBody)(body);
        const objectPath = (0, decision_registry_1.decisionRegistryObjectPath)(id, version, contentHash);
        const record = {
            ...
            object: {
                ...
                contentHash,
                objectGeneration: "g-1",
                byteSize: Buffer.byteLength((0, decision_registry_1.canonicalJsonV1)(body), "utf8"),
            },
            createdAt: "2026-07-16T00:00:00.000Z",
        };
        const db = {
        ...
        await expect((0, firestore_authoring_store_1.createFirestoreDecisionRegistryResolver)(db, {
            read: async () => ({
                body,
                contentHash,
        ...
        }).resolve({ id, version, contentHash })).rejects.toThrow("season_decision_registry_object_metadata_invalid");
    });
    it("rejects DecisionRegistry Storage bytes whose canonical body hash differs", async () => {
        ...
        }).resolve({ id, version, contentHash })).rejects.toThrow("season_decision_registry_object_invalid");
    });
});
