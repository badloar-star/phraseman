...
import type { V2DelayedProbeAssignmentRef, V2DelayedProbeLaunchReceiptRef, V2DelayedProbeRef } from '../../modules/learning-v2/contracts/delayed_receipts';
...
export interface DelayedReceiptRepository {
  runTransaction<T>(fn: (transaction: DelayedReceiptTransaction) => Promise<T>): Promise<T>;
}
...
const sameRef = (
  left: { readonly assignmentId?: string; readonly launchId?: string; readonly contentHash: string },
  right: { readonly assignmentId?: string; readonly launchId?: string; readonly contentHash: string },
): boolean =>
  (left.assignmentId ?? left.launchId) === (right.assignmentId ?? right.launchId) &&
  left.contentHash === right.contentHash;
const sameProbe = (left: V2DelayedProbeRef, right: V2DelayedProbeRef): boolean => left.probeId === right.probeId && left.contentHash === right.contentHash;
const operationKey = (input: FinalizeDelayedCandidateInput): string => `learning-v2:delayed:${input.stableId}:${input.operationId}`;
const receiptKey = (receipt: DelayedRuntimeReceipt): string => receipt.kind === 'timing' ? `learning_v2_timing_receipts:${receipt.ref.timingReceiptId}` : `learning_v2_failure_receipts:${receipt.ref.failureReceiptId}`;

export async function finalizeDelayedCandidate(
  repository: DelayedReceiptRepository,
  input: FinalizeDelayedCandidateInput,
): Promise<{ readonly replayed: boolean; readonly receipt: DelayedRuntimeReceipt }> {
  return repository.runTransaction(async (transaction) => {
    const operationDocument = await transaction.get<DelayedReceiptOperation>(operationKey(input));
    if (operationDocument.exists) {
      if (operationDocument.data?.fingerprint !== input.fingerprint) throw new Error('delayed_operation_replay_mismatch');
      if (!operationDocument.data?.receipt) throw new Error('delayed_operation_record_invalid');
      return { replayed: true, receipt: operationDocument.data.receipt };
    }
    const assignmentDocument = await transaction.get<DelayedAssignmentRecord>(`learning_v2_assignments:${input.assignmentRef.assignmentId}`);
    const launchDocument = await transaction.get<DelayedLaunchRecord>(`learning_v2_launches:${input.launchReceiptRef.launchId}`);
    ...
    if (!assignmentDocument.exists || !assignment) {
      ...
      decision = { kind: 'protocol_rejection', reasonCode: 'declaration_or_provenance_mismatch' };
    }
    ...
    transaction.create(operationKey(input), { operationId: input.operationId, fingerprint: input.fingerprint, receipt });
