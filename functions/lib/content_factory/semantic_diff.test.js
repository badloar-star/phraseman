...
describe('stage-aware semantic diff', () => {
    ...
    ])('reports semantic paths for %s', (kind, before, after, expectedPath) => {
        expect((0, semantic_diff_1.semanticDiff)(kind, before, after).details).toEqual(expect.arrayContaining([expect.objectContaining({ path: expectedPath, type: 'changed' })]));
    });
    test('reports added, removed and moved stable IDs', () => {
        ...
        const diff = (0, semantic_diff_1.semanticDiff)('flashcard_items', before, after);
    ...
    test('bounds details and retains complete summary counts', () => {
        ...
        const diff = (0, semantic_diff_1.semanticDiff)('lesson_vocabulary', before, after, 5);
        expect(diff.details).toHaveLength(5);
        expect(diff.isPartial).toBe(true);
        expect(diff.summary.changed).toBe(20);
    });
});
//# sourceMappingURL=semantic_diff.test.js.map