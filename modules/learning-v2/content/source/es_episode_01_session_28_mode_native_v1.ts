...
// продолжает Главу 4 "Мы и они"): испанская сессия 28 переписывается в
// mode-native формат, зеркаля КОД (не данные) сессии 25/27 — kind:
// 'phrases', новых слов формально нет (числительные dos/tres вводятся как
// обычные позиционные токены). Все 15 фраз сохранены из легаси-черновика
// без изменений содержания — каждая учит ser+число (Somos dos/Son tres) в
// разных контекстах, включая двухреплийные диалоги с recall eres/son.
//
// зачем ВСЕ 15 фраз могут идти через phrase_builder/listen_build_dictation
// (в отличие от сессий 25/26/27): каждая фраза здесь короткая (max 8
// уникальных дистракторов), в пределе responseFeedbackById в
// course_session_client_children_v1.ts — специальных ограничений нет.
function asciiId(value: string): string {
  return value.normalize('NFD').replace(/[̀-ͯ]/gu, '');
}

const phrases = ES_EPISODE_01_SESSION_28_PHRASES;
if (phrases.length !== 15) {
  throw new Error('es_session_28_mode_native_phrase_count_invalid');
}
...
// зачем 'en' вместо 'es' здесь (как в испанских сессиях 1-27): в испанском
...
function localizedPhraseField(
  ...
  if (!phrase?.localizedDetails) throw new Error(`es_session_28_phrase_details_missing:${phraseIndex}`);
  return Object.fromEntries(LEARNING_V2_INTERFACE_LOCALES.map((locale) => {
    ...
    if (!detail) throw new Error(`es_session_28_phrase_locale_missing:${phraseIndex}:${locale}`);
...
function localizedPhraseDistractorFeedback(
  ...
  return Object.fromEntries(LEARNING_V2_INTERFACE_LOCALES.map((locale) => {
    ...
    if (!reason) throw new Error(`es_session_28_phrase_distractor_missing:${phraseIndex}:${locale}:${value}`);
...
function phraseAudio(phraseIndex: number): LearningV2ModeAudioReferenceV1 {
  const phrase = phrases[phraseIndex]!;
  return authoredAudio(`es-e01-s28-phrase-${phraseIndex + 1}`, phrase.english);
}
function phraseSlowAudio(phraseIndex: number): LearningV2ModeAudioReferenceV1 {
  const phrase = phrases[phraseIndex]!;
  return authoredAudio(`es-e01-s28-phrase-${phraseIndex + 1}-slow`, phrase.english);
}
...
// зачем гейт стоит на ПОСЛЕДНЕМ содержательном слове (владелец,
// 2026-08-28): эта сессия проверяет числительное после связки — последнее
// содержательное слово почти всегда несёт число (dos/tres).
function contextGapLastWord(phraseIndex: number): LearningV2ModeNativePayloadV1 {
  ...
  return Object.freeze({
    ...
    testedDimension: `number_with_ser:${asciiId(phrase.id)}`,
    choiceFeedback: Object.freeze([
      ...
      ...lastWord.distractors.map((entry) => feedback(
        ...
        false,
        `number_with_ser:${asciiId(entry.reasonCode)}`,
...
// 2026-08-28, тот же приём, что сессии 11/17/25/27): широкий срез уже
// отработанных ser+число фраз, а не смежный блок.
function speedMatchPhrases(pairIndices: readonly number[]): LearningV2ModeNativePayloadV1 {
  ...
  const ids = pairIndices.map((index) => `es-e01-s28-pair-${index + 1}`);
  return Object.freeze({
    family: 'speed_match',
...
// интро-carousel. Все 15 фраз укладываются в 8-дистракторный предел, поэтому
// phrase_builder/listen_build_dictation используются свободно; context_gap
// проверяет число, listen_choose и speed_match добавляют разнообразие.
export const ES_EPISODE_01_SESSION_28_MODE_NATIVE_PRACTICE_V1 = Object.freeze<readonly SessionModeNativePracticeSourceV1[]>([
  ...
  { family: 'phrase_builder', purpose: 'supported_practice', learningStage: 'apply_in_phrase', target: { kind: 'phrase', sourceIndex: 4 }, modePayload: fullPhraseBuilder(4) },
  ...
  { family: 'listen_choose', purpose: 'guided_practice', learningStage: 'apply_in_phrase', target: { kind: 'phrase', sourceIndex: 6 }, modePayload: listenChoosePhrase(6, 9) },
  ...
  { family: 'phrase_builder', purpose: 'near_transfer', learningStage: 'apply_in_phrase', target: { kind: 'phrase', sourceIndex: 10 }, modePayload: fullPhraseBuilder(10) },
  ...
  { family: 'phrase_builder', purpose: 'independent_check', learningStage: 'apply_in_phrase', target: { kind: 'phrase', sourceIndex: 14 }, modePayload: fullPhraseBuilder(14) },
]);

if (ES_EPISODE_01_SESSION_28_MODE_NATIVE_PRACTICE_V1.length !== 12) {
  throw new Error('es_session_28_mode_native_practice_count_invalid');
}
