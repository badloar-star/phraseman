...
const inferential = (item) => { const proof = item.logicalInference; return proof?.domain === 'reading_pragmatics' && Array.isArray(proof.premises) && proof.premises.filter((premise) => String(premise).trim()).length >= 2 && String(proof.unstatedConclusion || '').trim() && String(proof.whyNotExplicit || '').trim(); };
...
export function auditQuestionBank(bank, { allowDraft = false } = {}) {
  ...
  for (const level of LEVELS) {
    ...
    for (const item of items) {
      ...
      if (/\b(?:obscure|trivia|which year|which decree|capital of|architect(?:ed)?|chapel)\b/iu.test(String(item.stimulus || ''))) errors.push(`${item.id}: external-knowledge/obscure-trivia proxy`);
