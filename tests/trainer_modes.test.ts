...
describe('trainer — free session limit', () => {
  // Дефолт = 1 бесплатная сессия/день (remote_flags, FREE_TRAINER_SESSIONS_PER_DAY_DEFAULT).
  it('starts with 1 session available', async () => {
    await expect(getFreeSessionsLeftToday()).resolves.toBe(1);
  });

  it('returns 0 after the session is marked used today', async () => {
  ...
  it('consumes the reserved free entry and blocks the next reservation', async () => {
    mockStorage.tester_no_premium = 'true';
    await expect(reserveTrainerSessionEntry('/trainer_words_session', false)).resolves.toBe(true);
    await expect(getFreeSessionsLeftToday()).resolves.toBe(1);
  ...
  it('rejects a reserved entry for a different trainer route', async () => {
    ...
    // Неверный маршрут не списывает сессию — остаётся полный дневной лимит (1).
    await expect(getFreeSessionsLeftToday()).resolves.toBe(1);
  });
});
...
describe('mistake_log analytics', () => {
  ...
  it('can shuffle personal-practice options even when content stores every correct answer first', () => {
    const training = getDiagnosisTraining('reported_speech_basic')!;

    expect(training.steps).toHaveLength(15);
    expect(training.steps.every((step) => step.answerOptions[0]?.id === step.correctAnswerId)).toBe(true);

    const uiOrders = training.steps.map((step) => shufflePracticeOptions(step.answerOptions, () => 0));

    expect(uiOrders.every((options, index) => options[0]?.id === training.steps[index].correctAnswerId)).toBe(false);
    for (const [index, options] of uiOrders.entries()) {
      expect(options).toHaveLength(training.steps[index].answerOptions.length);
      expect(options.map((option) => option.id).sort()).toEqual(
        training.steps[index].answerOptions.map((option) => option.id).sort(),
      );
      expect(options.some((option) => option.id === training.steps[index].correctAnswerId)).toBe(true);
    }
  });

  it('can shuffle arena-practice answer strings without dropping the correct answer', () => {
    const sourceOptions = ['went', 'go', 'gone', 'goes'];
    const shuffled = shufflePracticeOptions(sourceOptions, () => 0);

    expect(shuffled).not.toEqual(sourceOptions);
    expect(shuffled[0]).not.toBe('went');
    expect([...shuffled].sort()).toEqual([...sourceOptions].sort());
    expect(sourceOptions).toEqual(['went', 'go', 'gone', 'goes']);
  });
