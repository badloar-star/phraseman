...
exports.referralSpin = (0, https_1.onCall)(CALLABLE_BASE, async (request) => {
    ...
    const outcome = await db.runTransaction(async (tx) => {
        ...
        const isProLifetime = String(progress.premium_plan ?? '')
            .trim().toLowerCase() === 'lifetime';
        const prizeKind = isProLifetime ? 'pearls' : 'days';
        const prizePearls = isProLifetime
            ? Math.max(0, Math.floor(Number(referral_spin_logic_1.REFERRAL_SPIN_PRIZE_PEARLS[prizeIndex] ?? 0)))
            : 0;
        ...
        const vipUntil = prizeKind === 'pearls'
            ...
            : (0, referral_1.stackVipUntilMs)(currentUntil, nowMs, prizeDays);
