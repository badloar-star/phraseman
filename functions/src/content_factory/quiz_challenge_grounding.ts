...
import { validateQuestionBatchArtifact, validateQuestionReplacementArtifact } from './quiz_challenge_artifacts';
...
type TopicKind = 'quiz_topic' | 'challenge_topic';
function record(value: unknown): Record<string, unknown> | undefined { return typeof value === 'object' && value !== null && !Array.isArray(value) ? value as Record<string, unknown> : undefined; }

export async function loadApprovedTopicGrounding(bucket: StudioGroundingBucketLike, stage: { readonly stageId: string; readonly artifactId: string; readonly kind: TopicKind; readonly state: string; readonly cefr: string; readonly objectPath: string; readonly contentHash: string; readonly objectGeneration: string }) {
  if (stage.state !== 'approved') throw new Error('studio_topic_not_approved');
  if (stage.kind !== 'quiz_topic' && stage.kind !== 'challenge_topic') throw new Error('studio_topic_kind_invalid');
...
export async function loadQuestionBatchForReview(bucket: StudioGroundingBucketLike, stage: { readonly stageId: string; readonly artifactId: string; readonly kind: 'quiz_questions' | 'challenge_questions'; readonly state: string; readonly objectPath: string; readonly contentHash: string; readonly objectGeneration: string; readonly groundingReceipt: unknown }, options: { readonly allowNeedsReview?: boolean } = {}) {
