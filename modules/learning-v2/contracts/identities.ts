...
export const isCourseId = (value: unknown): value is CourseId => isValidV2Identity(value);
export const isSeasonId = (value: unknown): value is SeasonId => isValidV2Identity(value);
export const isEpisodeId = (value: unknown): value is EpisodeId => isValidV2Identity(value);
export const isNodeId = (value: unknown): value is NodeId => isValidV2Identity(value);
export const isActivityId = (value: unknown): value is ActivityId => isValidV2Identity(value);
export const isSkillId = (value: unknown): value is SkillId => isValidV2Identity(value);
export const isReleaseId = (value: unknown): value is ReleaseId => isValidV2Identity(value);
...
export const assertUniqueActivityIdsWithinRelease = (
  ...
  for (const candidate of ids) {
    ...
    if (seen.has(activityId)) {
      throw new V2IdentityError(V2_IDENTITY_ERROR_CODES.duplicateActivityWithinRelease);
    }
    seen.add(activityId);
  }
};
