...
describe('Learning V2 owner-current course topology v1', () => {
  ...
  test('groups every lesson into seven chapters of eight sessions with exact checks', () => {
    ...
    expect(lesson.sessions.filter((session) => session.role === 'chapter_checkpoint')
      .map((session) => session.sessionOrdinal)).toEqual([8, 16, 24, 32, 40, 48]);
    expect(lesson.sessions.slice(48, 55).every(
      (session) => session.role === 'transfer_practice',
    )).toBe(true);
  ...
  test('is deterministic without inventing any real lesson title or learning body', () => {
    ...
    expect(JSON.stringify(first)).not.toMatch(/hello|grammar|travel|interview/i);
  });
});
