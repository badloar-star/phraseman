...
describe('new paywalls activate Premium locally after RevenueCat success', () => {
  ...
  const v2Paywall = fs.readFileSync(path.join(process.cwd(), 'app', 'premium_modal_v2.tsx'), 'utf8');
  ...
  it('v2 paywall purchase and restore paths persist active RevenueCat state locally', () => {
    const purchaseStart = v2Paywall.indexOf('const handlePurchase = useCallback');
    const restoreStart = v2Paywall.indexOf('const handleRestore = useCallback');
    expect(purchaseStart).toBeGreaterThan(-1);
    expect(restoreStart).toBeGreaterThan(-1);

    const purchaseBody = v2Paywall.slice(purchaseStart, restoreStart);
    const restoreBody = v2Paywall.slice(restoreStart, v2Paywall.indexOf('// ── цвета', restoreStart));
