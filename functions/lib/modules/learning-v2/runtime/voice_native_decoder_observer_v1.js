...
exports.LEARNING_V2_NATIVE_DECODER_STATUS_MAX_COUNT_V1 = 4096;
const policyBody = Object.freeze({
    schemaVersion: "learning-v2-native-decoder-observer-policy.v1",
    policyId: "expo-audio-observed-playback-v1",
    policyVersion: 1,
    ...
    minimumProgressMs: 50,
    finishToleranceMs: 750,
    maximumStatusCount: exports.LEARNING_V2_NATIVE_DECODER_STATUS_MAX_COUNT_V1,
});
exports.LEARNING_V2_NATIVE_DECODER_OBSERVER_POLICY_V1 = Object.freeze({
    body: policyBody,
    ref: Object.freeze({
...
function observeLearningV2NativeDecoderPlaybackV1(input) {
    ...
    const allowedStates = identity.platform === "ios"
    ...
    const statuses = Object.freeze(input.statuses.map((status, index) => {
        if (!status ||
        ...
        if (errorState !== null && status.playbackState === errorState)
            fail();
        if (status.isLoaded) {
            if (status.durationMs < policyBody.minimumDurationMs ||
        ...
        if (status.playing) {
            if (!loaded || status.playbackState !== readyState)
                fail();
            if (!playing)
                firstPlayingPositionMs = status.currentTimeMs;
            playing = true;
        }
        if (loaded &&
            playing &&
            ...
            progressed = true;
        if (status.currentTimeMs >
            loadedDurationMs + policyBody.finishToleranceMs)
            fail();
        maximumObservedPositionMs = Math.max(maximumObservedPositionMs, status.currentTimeMs);
    ...
    if (!loaded || !playing || !progressed || !finished)
        fail();
    return Object.freeze({
        ...identity,
        nativeDecoderFamily: identity.platform === "ios"
            ? "avplayer"
            : "exoplayer",
        loadedDurationMs,
        ...
        statusSequenceFingerprint: (0, decision_registry_1.hashCanonicalBody)({
            schemaVersion: exports.LEARNING_V2_NATIVE_DECODER_OBSERVER_SCHEMA_V1,
            policyRef: exports.LEARNING_V2_NATIVE_DECODER_OBSERVER_POLICY_V1.ref,
            identity,
            statuses,
        }),
...
//# sourceMappingURL=voice_native_decoder_observer_v1.js.map