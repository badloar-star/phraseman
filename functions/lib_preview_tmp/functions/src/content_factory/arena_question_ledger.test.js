...
describe('arena question ledger', () => {
    ...
    it('parses pre-replacement ledgers and restores only the active replacement', () => {
        const base = (0, arena_question_ledger_1.approveArenaQuestionBatch)((0, arena_question_ledger_1.parseArenaQuestionLedger)(null, 'arena-topic-1'), { batchArtifactId: 'batch-1', items: batch('base') }).ledger;
        ...
        const parsed = (0, arena_question_ledger_1.parseArenaQuestionLedger)(legacy, 'arena-topic-1');
        const replacement = { ...batch('replacement')[0], id: 'base-0' };
        const approved = (0, arena_question_ledger_1.approveArenaQuestionReplacement)(parsed, { batchArtifactId: 'batch-1', questionId: 'base-0', replacementArtifactId: 'replacement-1', replacement });
        expect(approved.ledger.batches['batch-1'].items.slice(1)).toEqual(parsed.batches['batch-1'].items.slice(1));
        const rolledBack = (0, arena_question_ledger_1.rollbackArenaQuestionReplacement)(approved.ledger, { batchArtifactId: 'batch-1', questionId: 'base-0', replacementArtifactId: 'replacement-1' });
