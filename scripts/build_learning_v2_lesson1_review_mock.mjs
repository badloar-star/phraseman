...
<body>
  ...
  <script>
    ...
    const text=(el,value)=>{el.textContent=value??''};const loc=v=>v[locale];const key=()=>['l2-review',d.range.from+'-'+d.range.to,d.sessions[si].ordinal,locale].join(':');const termBoundary=(source,start,term)=>{const word=/[A-Za-z0-9]/;return !(word.test(source[start-1]||'')&&word.test(term[0]||''))&&!(word.test(source[start+term.length]||'')&&word.test(term[term.length-1]||''))};const semanticText=(el,value,terms=[])=>{el.replaceChildren();const source=String(value??''),ordered=[...new Set(terms)].filter(Boolean).sort((a,b)=>b.length-a.length||a.localeCompare(b));let cursor=0,plain='';const flush=()=>{if(!plain)return;el.append(document.createTextNode(plain));plain=''};while(cursor<source.length){const term=ordered.find(candidate=>source.startsWith(candidate,cursor)&&termBoundary(source,cursor,candidate));if(term){flush();const span=document.createElement('span');span.className='target-language';span.textContent=term;el.append(span);cursor+=term.length}else{plain+=source[cursor];cursor++}}flush()};
    ...
    function intro(page,index){const a=document.createElement('article');a.className='intro card';const eye=document.createElement('div');eye.className='eyebrow';text(eye,page.kind);const h=document.createElement('h3');text(h,loc(page.title));const body=document.createElement('div');body.className='body-copy';(page.bodyRuns[locale]||[{text:loc(page.body),semantic:'explanation'}]).forEach(run=>{const span=document.createElement('span');span.className=run.semantic;text(span,run.text);body.append(span)});const quiz=document.createElement('div');quiz.className='quiz';const prompt=document.createElement('strong');text(prompt,loc(page.question.prompt));const answer=document.createElement('div');answer.className='answer';const terms=page.question.choices.map(loc);page.question.choices.forEach((choice,ci)=>{const b=document.createElement('button');b.className='choice';text(b,loc(choice));b.onclick=()=>{quiz.querySelectorAll('.choice').forEach((node,j)=>node.classList.toggle(j===page.question.correctChoiceIndex?'correct':'wrong',j===ci));semanticText(answer,loc(page.question.explanation),terms)};quiz.append(b)});quiz.prepend(prompt);quiz.append(answer);a.append(eye,h,body,quiz);return a}
    function phrase(p,index){const hay=(p.english+' '+loc(p.localizedDetails).meaning+' '+loc(p.localizedDetails).explanation+' '+JSON.stringify(loc(p.localizedDetails))).toLocaleLowerCase();if(query&&!hay.includes(query))return null;const details=document.createElement('details');details.className='phrase card';const summary=document.createElement('summary');const n=document.createElement('span');n.className='num';text(n,String(index+1).padStart(2,'0'));const en=document.createElement('span');en.className='english';text(en,p.english);const localizedDetails=loc(p.localizedDetails);const terms=[p.english,...p.english.match(/[A-Za-z]+(?:[’'][A-Za-z]+)*/g)||[],...localizedDetails.words.flatMap(w=>[w.correct,...w.distractors.map(x=>x.value)])];const meaning=document.createElement('span');meaning.className='meaning';text(meaning,localizedDetails.meaning);summary.append(n,en,meaning);const content=document.createElement('div');content.className='phrase-content';const exp=document.createElement('p');semanticText(exp,localizedDetails.explanation,terms);const features=document.createElement('div');features.className='features';text(features,p.features.join(' · '));const words=document.createElement('div');words.className='words';localizedDetails.words.forEach(w=>{const box=document.createElement('section');box.className='word';const h=document.createElement('h4');h.className='target-language';text(h,w.correct);const prompt=document.createElement('p');semanticText(prompt,w.prompt,terms);const ul=document.createElement('ul');ul.className='wrong-list';w.distractors.forEach(x=>{const li=document.createElement('li');const token=document.createElement('span');token.className='wrong-token';text(token,x.value+' — ');li.append(token,document.createTextNode(x.reason));ul.append(li)});box.append(h,prompt,ul);words.append(box)});content.append(exp,features,words);details.append(summary,content);return details}
