...
import { addOrUpdateScore, pointsForAnswer, streakMultiplier } from '../hall_of_fame_utils';
import { checkAchievements } from '../achievements';
import { updateMultipleTaskProgress } from '../daily_tasks';
import { isCorrectAnswer } from '../../constants/contractions';
import { loadSettings, saveSettings, DEFAULT_SETTINGS, UserSettings as Settings } from '../settings_edu';
            ...
            <View key={lv} style={{ borderRadius: 20, overflow: 'hidden' }}>
              ...
              <TouchableOpacity
                onPress={() => {
                  ...
                  setSelected(isSelected ? null : lv);
                }}
                activeOpacity={0.85}
              >
                <LinearGradient
...
function QuizGame({ level, onBack }: { level:Level; onBack:()=>void }) {
      ...
      try {
        ...
        if (pct >= 70) {
          ...
          if (reward.hasBonusWon) {
            ...
            await AsyncStorage.setItem('user_total_xp', String(currentXP + reward.bonusXP));
            if (userNameRef.current) {
              addOrUpdateScore(userNameRef.current, reward.bonusXP, lang);
            }
          }
        }
      } catch {}
  ...
  const afterAnswer = (isRight: boolean) => {
    ...
    if (!reviewing) {
      if (isRight) {
        ...
        if (name) addOrUpdateScore(name, pts, lang);
