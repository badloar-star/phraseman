...
exports.adminListPromoCodes = exports.promoCodeDelete = exports.promoCodeBatchUpsert = exports.promoCodeUpsert = exports.promoCodeRedeem = void 0;
...
exports.buildPromoCodeDeletePlan = buildPromoCodeDeletePlan;
// ════════════════════════════════════════════════════════════════════════════
...
function buildPromoCodeDeletePlan(params) {
    ...
    if (!Number.isSafeInteger(currentUpdatedAtMs)
        ...
        throw new https_1.HttpsError('aborted', 'promo_code_delete_conflict');
    }
    ...
    if (giftBacked) {
        throw new https_1.HttpsError('failed-precondition', 'gift_backed_promo_delete_forbidden');
    }
    const checkoutBacked = String(params.promo.createdBy ?? '').trim() === 'web_checkout'
        || /^web_checkout\s+/.test(String(params.promo.note ?? '').trim());
    if (checkoutBacked) {
        throw new https_1.HttpsError('failed-precondition', 'paid_checkout_promo_delete_forbidden');
    }
    return {
        code,
        auditDoc: {
...
exports.promoCodeDelete = (0, https_1.onCall)({ region: REGION, enforceAppCheck: true }, async (request) => {
    assertAdminPermission(request, 'money.manual_access.write');
    const code = normalizePromoCode(request.data?.code);
    if (!CODE_RE.test(code))
        throw new https_1.HttpsError('invalid-argument', 'bad_code');
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (!codeSnapshot.exists)
            throw new https_1.HttpsError('not-found', 'promo_code_not_found');
