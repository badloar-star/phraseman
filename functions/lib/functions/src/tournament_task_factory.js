...
exports.TOURNAMENT_MODES = exports.GENERATABLE_KINDS = void 0;
exports.difficultyForDay = difficultyForDay;
exports.difficultyForTask = difficultyForTask;
exports.phraseTokens = phraseTokens;
exports.generateTournamentTasks = generateTournamentTasks;
const node_crypto_1 = require("node:crypto");
const tournament_core_1 = require("./tournament_core");
exports.GENERATABLE_KINDS = ['choice', 'translate', 'timeattack'];
/** Режимы пула. Сервер берёт список режимов отсюда — через поле task.mode. */
exports.TOURNAMENT_MODES = Object.freeze({
    choice: 'guess_phrase',
...
function stableHash(input) {
    const digest = (0, node_crypto_1.createHash)('sha1').update(input).digest();
 ...
 */
function difficultyForTask(day, kind, tokenCount) {
    ...
    if (kind === 'timeattack')
        score += 0.5;
    if (tokenCount >= VERY_LONG_PHRASE_TOKENS)
        score += 1;
    else if (tokenCount >= LONG_PHRASE_TOKENS)
        score += 0.5;
...
function tagsForDay(day) {
    const tags = [`plan:${day.planId}`, `day:${day.dayIndex}`];
    if (day.level)
        tags.push(`cefr:${String(day.level).toLowerCase()}`);
    const topic = typeof day.topic?.ru === 'string' ? day.topic.ru.trim() : '';
    if (topic) {
        // 'topic:' занимает 6 байт из бюджета тега.
        const room = tournament_core_1.TOURNAMENT_TASK_LIMITS.tagBytes - Buffer.byteLength('topic:', 'utf8');
        const trimmed = truncateToBytes(topic, room);
        if (trimmed)
            tags.push(`topic:${trimmed}`);
    }
    return tags;
}
// ── Нормализация ────────────────────────────────────────────────────────────
 ...
 */
function buildChoiceTask(day, phrase, pool, verified) {
    ...
    if (distractors.length < CHOICE_OPTIONS - 1)
        return null;
    ...
    return {
        ...
        mode: exports.TOURNAMENT_MODES.choice,
        isVoice: false,
 ...
 */
function buildTranslateTask(day, phrase, verified) {
    ...
    for (const candidate of seededShuffle(authored, `bank:${phrase.id}`)) {
        ...
        if (!token || lower.has(key))
            continue;
        lower.add(key);
        extras.push(token);
        // Банк заметно больше фразы превращает сборку в поиск, а не в грамматику.
        if (extras.length >= Math.min(4, correctTokens.length))
            break;
    }
    if (extras.length === 0)
        return null;
    ...
    return {
        ...
        mode: exports.TOURNAMENT_MODES.translate,
        isVoice: false,
 ...
 */
function buildTimeattackTask(day, phrases, itemCount, verified) {
    const usable = phrases.filter(isUsablePhrase);
    if (usable.length < Math.max(2, Math.min(itemCount, MIN_PHRASES_FOR_CHOICE)))
        return null;
    ...
    return {
        ...
        mode: exports.TOURNAMENT_MODES.timeattack,
        isVoice: false,
 ...
 */
function generateTournamentTasks(days, options = {}) {
    const kinds = options.kinds?.length ? options.kinds : exports.GENERATABLE_KINDS;
    ...
    const tasks = [];
    const byKind = { choice: 0, translate: 0, timeattack: 0 };
    let phrasesSeen = 0;
    let rejected = 0;
    const push = (task, kind) => {
        if (!task) {
            rejected += 1;
            return true;
        }
    ...
    outer: for (const day of days) {
        ...
        if (kinds.includes('timeattack')) {
            if (!push(buildTimeattackTask(day, usable, itemCount, verified), 'timeattack'))
                break outer;
        }
        for (const phrase of usable) {
            if (kinds.includes('choice') && usable.length >= MIN_PHRASES_FOR_CHOICE) {
                if (!push(buildChoiceTask(day, phrase, usable, verified), 'choice'))
                    break outer;
            }
            if (kinds.includes('translate')) {
                if (!push(buildTranslateTask(day, phrase, verified), 'translate'))
                    break outer;
            }
        }
    }
...
//# sourceMappingURL=tournament_task_factory.js.map