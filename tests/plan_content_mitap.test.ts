...
describe('mitap generated content', () => {
  it('every generated mitap day is valid and within its grammar gate', () => {
    for (const day of MITAP_CONTENT_DAYS) {
      expect(validatePlanContentDay(day)).toEqual([]);
      expect(checkPlanContentGate(day).withinGate).toBe(true);
    }
  });

  it('every mitap day maps cleanly to runtime phrases with all three locales', () => {
    for (const day of MITAP_CONTENT_DAYS) {
  ...
  it('every mitap day produces vocabulary cards with a real part of speech each', () => {
    for (const day of MITAP_CONTENT_DAYS) {
