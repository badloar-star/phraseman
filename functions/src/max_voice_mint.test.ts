...
describe('server-pinned session config (section 4)', () => {
  ...
  it('sends the full realtime session config; the client controls none of it', async () => {
    ...
    expect(body.session.audio.input.turn_detection).toEqual({
      ...
      // Ходами разговора управляет СЕРВЕР (штатная схема OpenAI Realtime):
      // он знает границу хода, сам отменяет свою речь при перебивании и не
      // может создать два ответа на одну реплику. Самодельная очередь на
      // клиенте давала «обрывает на полуслове / долго ждёт / отвечает дважды»
      // (владелец, живой тест 2026-08-23).
      create_response: true,
      // Перебивание голосом включено намеренно — так разговор живой. От эха
      // громкой связи защищает аудио-тракт (noise_reduction far_field + AEC),
      // а не запрет серверу отвечать.
