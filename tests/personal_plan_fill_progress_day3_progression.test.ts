...
describe('personal plan fill progress day 3 progression', () => {
  it('tracks certified Day 3 content for every plan', () => {
    ...
    expect(data.dayQuality).toEqual(expect.arrayContaining([
      expect.objectContaining({ label: 'Mitap Day 3', status: 'certified' }),
      expect.objectContaining({ label: 'Voyazh Day 3', status: 'certified' }),
      expect.objectContaining({ label: 'Gavan Day 3', status: 'certified' }),
      expect.objectContaining({ label: 'Impuls Day 3', status: 'certified' }),
      expect.objectContaining({ label: 'Echo Day 3', status: 'certified' }),
    ]));

    const day3Rows = data.dayQuality.filter((row: { label: string }) => row.label.endsWith('Day 3'));
    expect(day3Rows).toHaveLength(5);
    expect(day3Rows.filter((row: { status: string }) => row.status === 'certified')).toHaveLength(5);
    expect(day3Rows.map((row: { whatExists: string }) => row.whatExists).join(' ')).toContain('recall');
    expect(day3Rows.map((row: { notes: string }) => row.notes).join(' ').toLowerCase()).toContain('valid_non_live_dry_run');
  });
});
