...
- Админ-раздел: **админка → вкладка «🌐 Сайт»** (`admin/site.html`) — цены, флаг промокодов,
  все заказы (веб И Telegram Stars) с кодами, воронка. Деплой админки: `firebase deploy --only hosting:admin`.
- **Telegram Stars — тоже автоматика (с 2026-07-02)**: бот сам выдаёт код после оплаты,
  продления подписки продлевают доступ сами. Владелец только видит статусы в Telegram
  и в админке; ручная выдача (testers.html) — запасной путь при статусе
...
## Шаг 5. Цены (по желанию, 1 минута)

Дефолт: месяц $9.99 / год $49.99 / навсегда $99.99.
Меняются В ОДНОМ месте: **админка → «🌐 Сайт» → блок «Цены» → Сохранить**.
Витрина пейвола сама подтягивает цены с сервера (endpoint `webPrices`), поэтому
что видно на витрине — то и списывается. Обновление доезжает до открытых страниц
в течение ~5 минут (кэш). `site-config.js → webPrices` — только офлайн-фоллбек.
