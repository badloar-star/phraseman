...
// Перед выпуском: подстройте RELEASE_NOTES_NEW_USER_CUTOFF_MS (начало дня после выкладки в стор)
// и RELEASE_NOTES_INTRO_BUILD_ID = android.versionCode этой сборки.
...
/** Сборка, начиная с которой показываем этот текст (см. app.json android.versionCode). */
export const RELEASE_NOTES_INTRO_BUILD_ID = 55;

/**
 * Установки с первого запуска не раньше этого момента (UTC) считаются «новыми» — окно не показываем.
 * Рекомендация: дата на следующий календарный день после публикации в Google Play / App Store.
 */
export const RELEASE_NOTES_NEW_USER_CUTOFF_MS = Date.UTC(2026, 4, 3, 0, 0, 0, 0);

const DISMISS_KEY = 'release_notes_dismissed_2026_05_v1';

export async function shouldOfferReleaseNotesModal(): Promise<boolean> {
  if (!RELEASE_NOTES_MODAL_ENABLED) return false;
  if (IS_EXPO_GO) return false;
  const cur = getAppReleaseBuildId();
  if (cur < RELEASE_NOTES_INTRO_BUILD_ID) return false;
