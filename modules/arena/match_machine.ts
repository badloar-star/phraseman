...
/**
 * Владелец 2026-09-08: убрать 2,5 секунды заблокированных касаний.
 * Нулевая фаза чтения проходит в том же advance(), затем начинается полное
 * окно ARENA_ANSWER_MS. Первое касание уже видимого задания принимается.
 * Фаза сохранена для восстановления старых снимков матча.
 *
 * Серверные ARENA_DUEL_READING_MS и ARENA_V2_READING_MS НЕ обнулять:
 * первая сохраняет верхнюю границу длительности для старых клиентов,
 * вторая обслуживает отдельный старый пошаговый протокол. Их сокращение
 * не разблокирует старый UI, но может преждевременно завершить его матч.
 */
export const ARENA_LOCAL_READING_MS = 0;
