...
type GeneratedRow = {
  ...
  generatedBy: 'gustav_personal_plan_echo_day006_french_full_day_text_packet';
};

const PLAN_ID = 'echo' as const;
const DAY_INDEX = 6 as const;
const GENERATED_BY = 'gustav_personal_plan_echo_day006_french_full_day_text_packet' as const;

const EXPLANATIONS = [
  {
    phraseId: 'echo_d6_p1',
    title: 'What demande « quoi »',
    rule: 'What + do + you + verbe. L’ordre ne change pas.',
    why: 'Dans les questions avec « what », do est obligatoire ; sans lui, la question ne fonctionne pas.',
    commonMistake: 'Ne dis pas « What you usually read? » : il faut do après What.',
  },
  {
    phraseId: 'echo_d6_p2',
    title: 'Where demande « où »',
    rule: 'Where + do + sujet + verbe. Le lieu est au début de la question, pas à la fin.',
    why: 'Where met le lieu au premier plan : tu demandes exactement où.',
    commonMistake: 'Ne dis pas « You watch the news where? » : le mot interrogatif va en premier.',
  },
  {
    phraseId: 'echo_d6_p3',
    title: 'When demande « quand »',
    rule: 'When + do + sujet + verbe. Le moment va au début de la question.',
    why: 'When attire l’attention sur le temps : tu découvres à quel moment la personne le fait.',
    commonMistake: 'Ne dis pas « When you listen? » : tu ne peux pas omettre do avant you.',
  },
  {
    phraseId: 'echo_d6_p4',
    title: 'How demande « comment »',
    rule: 'How + do + sujet + verbe. Le schéma reste le même ; seul le mot interrogatif change.',
    why: 'How demande le moyen : par téléphone, à la télévision ou dans le journal.',
    commonMistake: 'Ne dis pas « How you get? » : il faut do, donc « How do you get ».',
  },
  {
    phraseId: 'echo_d6_p5',
    title: 'Who + does avec she/he',
    rule: 'Quand tu parles d’une autre personne, she ou he, tu utilises does au lieu de do.',
    why: 'She/he demande does : c’est la marque de la troisième personne du singulier.',
    commonMistake: 'Ne dis pas « Who do she follow? » : avec she, il faut does, pas do.',
  },
  {
    phraseId: 'echo_d6_p6',
    title: 'What + nom = « quel »',
    rule: 'What devant un nom signifie « quel » ou « quelle ». What channel = « quelle chaîne ».',
    why: 'Ainsi, tu précises l’objet : pas seulement « quoi », mais quelle chaîne exactement.',
    commonMistake: 'Ne dis pas « Which channel does he watches? » : après does, le verbe ne prend pas -s.',
  },
] as const;
...
const FIELD_SPECS: FieldSpec[] = [
  {
    fieldPath: 'topic',
    fieldKind: 'day_topic',
    frenchText: 'Demander quoi, où et quand',
    source: (day) => day.topic,
  },
  {
    fieldPath: 'outcome',
    fieldKind: 'day_outcome',
    frenchText: 'Tu peux poser des questions sur les nouvelles, les émissions et les médias : ce que tu regardes, où tu lis, quand tu allumes.',
    source: (day) => day.outcome,
  },
  {
    fieldPath: 'intro[0].title',
    fieldKind: 'intro_title',
    frenchText: 'Les questions avec un mot interrogatif',
    source: (day) => day.intro?.[0]?.title,
  },
  {
    fieldPath: 'intro[0].body',
    fieldKind: 'intro_body',
    frenchText: 'Pour connaître les détails, mets le mot interrogatif au début : What (quoi/quel), Where (où), When (quand), Who (qui), How (comment). Ensuite, construis la question normalement.',
  ...
  },
  {
    ...
    fieldKind: 'intro_example_gloss',
    frenchText: 'Où est-ce que tu regardes les nouvelles ?',
  ...
  },
  {
    ...
    fieldKind: 'intro_title',
    frenchText: 'Ordre des mots : interrogatif + do/does + sujet',
    source: (day) => day.intro?.[1]?.title,
  },
  {
    fieldPath: 'intro[1].body',
    fieldKind: 'intro_body',
    frenchText: 'Après le mot interrogatif vient do, ou does avec he/she/it, puis le sujet, puis le verbe. Exemple : What do you usually watch? Do vient avant you, et le verbe watch reste à la fin.',
    source: (day) => day.intro?.[1]?.body,
  },
  {
    fieldPath: 'intro[1].examples[0].gloss',
    fieldKind: 'intro_example_gloss',
    frenchText: 'Quand est-ce qu’elle écoute la radio ?',
  ...
  },
  {
    ...
    fieldKind: 'intro_example_gloss',
    frenchText: 'Qui est-ce que tu suis en ligne ?',
  ...
  },
  {
    ...
    fieldKind: 'intro_title',
    frenchText: 'Usually parle d’habitudes, pas d’une seule fois',
    source: (day) => day.intro?.[2]?.title,
  },
  {
    fieldPath: 'intro[2].body',
    fieldKind: 'intro_body',
    frenchText: 'Usually veut dire « normalement » et parle de quelque chose que tu fais régulièrement. Il va avant le verbe principal : I usually read the news in the morning. Ce n’est pas seulement aujourd’hui, c’est ton habitude.',
    source: (day) => day.intro?.[2]?.body,
  },
  {
    fieldPath: 'intro[2].examples[0].gloss',
    fieldKind: 'intro_example_gloss',
    frenchText: 'Qu’est-ce que tu lis normalement ?',
  ...
  },
  {
    ...
    fieldKind: 'intro_example_gloss',
    frenchText: 'Je regarde normalement les nouvelles le soir.',
...
function rowIdFromPath(fieldPath: string): string {
  return `fr_echo_d006_${fieldPath.replace(/[^a-zA-Z0-9]+/g, '_').replace(/^_+|_+$/g, '').toLowerCase()}`;
}
...
function renderMarkdown(report: any): string {
  const lines = [
    '# GUSTAV Personal Plan Echo Day 006 French Full-Day Text Packet',
    '',
    `Run: \`${report.runId}\``,
    '',
...
async function main(): Promise<void> {
  ...
  if (!runArg) {
    throw new Error('Usage: npx tsx scripts/gustav_personal_plan_echo_day006_french_full_day_text_packet.ts --run <run-dir>');
  }
  ...
  const rowsPath = path.join(outDir, 'echo_day_006_full_day_fr_rows.jsonl');
  const reviewerQueuePath = path.join(outDir, 'echo_day_006_full_day_fr_reviewer_queue.tsv');
  const packetJsonPath = path.join(auditsDir, 'personal_plan_echo_day006_french_full_day_text_packet.json');
  const packetMdPath = path.join(auditsDir, 'personal_plan_echo_day006_french_full_day_text_packet.md');

  const findings: Finding[] = [];
  const day = await readEchoDay(repoRoot);
  if (!day) {
    addFinding(findings, 'blocker', 'echo_day_006_missing', 'Echo day 6 is missing from source content.', rel(repoRoot, sourcePath));
  }

  let existingPhraseMeaningRowsForDay = 0;
  if (!fs.existsSync(phraseMeaningRowsPath)) {
    addFinding(findings, 'blocker', 'phrase_meaning_rows_missing', 'Echo day 6 phrase meaning rows are missing.', rel(repoRoot, phraseMeaningRowsPath));
  } else {
    ...
    if (existingPhraseMeaningRowsForDay !== 6) {
      addFinding(findings, 'blocker', 'phrase_meaning_rows_day_006_incomplete', `Expected 6 reviewer-needed phrase meaning rows for Echo day 6 but found ${existingPhraseMeaningRowsForDay}.`, rel(repoRoot, phraseMeaningRowsPath));
    }
  }
  ...
  const report = {
    schemaVersion: 'gustav-personal-plan-echo-day006-french-full-day-text-packet-v0',
    ...
    fieldKindCounts,
    generationPolicy: [
      'This packet translates Echo day 6 full-day text fields except phrase meanings, which are already covered by the phrase-meaning layer.',
  ...
  console.log(`GUSTAV Echo day 006 French full-day text packet: ${report.status}`);
