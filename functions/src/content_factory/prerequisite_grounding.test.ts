import { loadApprovedLessonGrounding, loadApprovedOutlineGrounding, prepareDerivedLessonGrounding, type GroundingBucketLike } from './prerequisite_grounding';
...
describe('approved prerequisite grounding loader', () => {
  ...
  const bucket: GroundingBucketLike = { file: () => ({ getMetadata: async () => [{ generation: '7' }], download: async () => [bytes] }) };

  it('loads verified approved phrases and deterministic candidate receipt', async () => {
    const grounding = await loadApprovedLessonGrounding(bucket, {
  ...
  it.each([
    ...
    await expect(loadApprovedLessonGrounding(bucket, { stageId: 's', artifactId: 'a', kind: 'lesson_phrases', state: 'approved', studyTarget: 'en', cefr: 'A1', promptVersion: 'v2', objectPath: 'safe.json', contentHash: hash, objectGeneration: '7', ...override })).rejects.toThrow(code);
  });

  it('builds a derived grounding receipt with explicit previous-lesson exclusions', async () => {
    const loaded = await loadApprovedLessonGrounding(bucket, { stageId: 's', artifactId: 'a2', kind: 'lesson_phrases', state: 'approved', studyTarget: 'en', cefr: 'A1', promptVersion: 'v2', objectPath: 'safe.json', contentHash: hash, objectGeneration: '7' });
    const grounding = prepareDerivedLessonGrounding({ kind: 'lesson_irregular_verbs', lessonId: 2, phraseArtifactId: 'a2', loaded, ledger: { studyTarget: 'en', revision: 1, lessons: { 1: { phraseArtifactId: 'a1', candidateKeys: ['irregular_verb\u0000go'], fingerprint: 'a'.repeat(64) } } } });
  ...
  it('loads an approved outline only after generation, hash and blueprint validation', async () => {
    ...
    const outlineBucket: GroundingBucketLike = { file: () => ({ getMetadata: async () => [{ generation: '11' }], download: async () => [outlineBytes] }) };
    ...
    await expect(loadApprovedOutlineGrounding(outlineBucket, metadata)).resolves.toMatchObject({ artifactId: 'outline-artifact', outline: { coverage: ['route', 'clarification'] } });
    await expect(loadApprovedOutlineGrounding(outlineBucket, { ...metadata, objectGeneration: '12' })).rejects.toThrow('outline_grounding_generation_mismatch');
    await expect(loadApprovedOutlineGrounding(outlineBucket, { ...metadata, contentHash: 'b'.repeat(64) })).rejects.toThrow('outline_grounding_content_hash_mismatch');
  });
});
