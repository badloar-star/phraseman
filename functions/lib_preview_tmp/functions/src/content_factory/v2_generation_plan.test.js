...
describe('V2 generation stage plan', () => {
    it('creates the exact pilot cardinality and terminal season QA', () => {
        const plan = (0, v2_generation_plan_1.buildV2SeasonPlan)({
            scope: 'full_season',
            seasonId: 'season-01',
    ...
    it('keeps optional dialogue and Speaking Club branches recipe-aware', () => {
        const withBranches = (0, v2_generation_plan_1.buildV2EpisodeSubgraph)({ episodeId: 'e1', dialogue: true, speakingClub: true }, 's1');
        const withoutBranches = (0, v2_generation_plan_1.buildV2EpisodeSubgraph)({ episodeId: 'e2' }, 's1');
    ...
    it('rejects wrong scope cardinality and duplicate episode IDs', () => {
        expect(() => (0, v2_generation_plan_1.buildV2SeasonPlan)({ scope: 'chapter_internal', seasonId: 's', episodeIds: ['e1'] })).toThrow('v2_episode_count_expected_8');
        expect(() => (0, v2_generation_plan_1.buildV2SeasonPlan)({ scope: 'vertical_slice', seasonId: 's', episodeIds: ['e1', 'e1'] })).toThrow('v2_episode_count_expected_1');
        expect(() => (0, v2_generation_plan_1.buildV2SeasonPlan)({ scope: 'vertical_slice', seasonId: 's', episodeIds: ['e1'], recipes: [{ episodeId: 'e1' }, { episodeId: 'e1' }] })).toThrow('v2_recipe_episode_ids_must_be_unique');
    });
...
//# sourceMappingURL=v2_generation_plan.test.js.map