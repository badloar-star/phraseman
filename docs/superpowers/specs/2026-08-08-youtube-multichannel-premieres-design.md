...
Правильная админка проекта — `admin/legacy.html`.

Это подтверждается текущим кодом:

- `admin/index.html` перенаправляет на `/legacy.html`;
- Firebase Hosting перенаправляет `/` и `/index.html` на `/legacy.html`;
- текущий редактор одного YouTube-канала и ручных видео уже находится в `admin/legacy.html`.

`admin/v2/**` забракован владельцем, не имеет отношения к этой функции и не должен изменяться.
...
Текущая карточка «YouTube-канал» в `admin/legacy.html` превращается в «YouTube-каталог».
...
Никакие файлы `admin/v2/**` в миграции не участвуют.
...
- контракт гарантирует отсутствие изменений `admin/v2/**`.
...
1. админ добавляет новый языковой канал в `admin/legacy.html`;
