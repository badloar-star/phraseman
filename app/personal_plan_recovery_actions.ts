...
function shouldSkipMistakeAnalytics(candidate: PlanRecoveryCandidate): boolean {
  return (
    candidate.target === 'mistake_analytics' &&
    candidate.grammarTags.length > 0 &&
    candidate.grammarTags.every((tag) => shouldSkipPlanGrammarAnalytics(tag))
  );
}
...
export function buildPlanRecoveryActions(
  ...
  return planRecoveryCandidatesForAttempt(block, event)
    .filter((candidate) => !shouldSkipMistakeAnalytics(candidate))
