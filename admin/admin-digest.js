(function initAdminDigestV2() {
  ...
  window.renderDigestV2 = function renderDigestV2(data) {
    ...
    const hasExactWindows = data.windows && data.windows.current && data.windows.previous;
    period.innerHTML = hasExactWindows
      ? `<div class="dd-period-main">Текущий период: ${escapeText(formatPeriod(data.windows.current))}</div>
        <div class="dd-period-compare">Сравнение: ${escapeText(formatPeriod(data.windows.previous))}</div>`
      : '<div class="dd-period-main">Старый формат дайджеста — точный период в нём не сохранялся.</div><div class="dd-period-compare">Нажмите «Сформировать дайджест»: период определится автоматически, вручную указывать его не нужно.</div>';
