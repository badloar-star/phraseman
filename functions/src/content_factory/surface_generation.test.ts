...
describe('generated quiz/card/arena surfaces', () => {
  ...
  it('validates all three surface item shapes and rejects wrong surface output', () => {
    ...
    expect(parseGeneratedSurfaceArtifact(JSON.stringify({ ...base, surface: 'arena', items: [{ id: 'a1', prompt: 'Как?', answer: 'Je suis', options: ['Je suis', 'Tu es'] }] }))).toMatchObject({ surface: 'arena' });
