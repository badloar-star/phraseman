...
// + СТАРТ ES §0/§10): испанская сессия 20 переписывается в mode-native
// формат, зеркаля код (не данные) сессий 10/11/13/17/19 — тот же класс
// сессии (kind: 'phrases', НЕТ новых слов). Все 15 фраз соединяют формулу
// проверки факта (¿Es verdad?, сессия 17) с формулой согласия (de acuerdo,
// сессия 14) — синтез двух прежних тем, разница между проверкой факта и
// согласием с выводом о нём. Фразовые данные и 8-локальные разборы взяты
// из легаси без изменений (MODE_NATIVE_AUTHORING_CONTRACT.ru.md §7).
//
// зачем именно этот набор families и порядок (владелец, 2026-08-28, зеркало
// легаси generic phraseSteps() по составу и индексам — та же логика, что и
// в сессиях 10/11/13/17/19): phraseSteps() уже использует только
// утверждённые families (listen_choose/phrase_builder/context_gap_grammar/
// speed_match/listen_build_dictation) — sound_contrast в нём нет, поэтому
// набор families сохранён 1:1, каждый шаг получает реальный mode-native
// payload.
const phrases = ES_EPISODE_01_SESSION_20_PHRASES;
if (phrases.length !== 15) {
  throw new Error('es_session_20_mode_native_phrase_count_invalid');
}

const L = (source: LocalizedSource): LearningV2Localized<string> => expandLocalized(source);

// зачем эта функция (владелец, 2026-08-28, тот же класс бага сессий 5/6/8-19):
...
// Фразы этой сессии переиспользуют reasonCode с диакритикой (fácil/único/
...
// зачем 'en' вместо 'es' здесь (как в испанских сессиях 1-19): в испанском
...
function localizedPhraseField(
  ...
  if (!phrase?.localizedDetails) throw new Error(`es_session_20_phrase_details_missing:${phraseIndex}`);
  return Object.fromEntries(LEARNING_V2_INTERFACE_LOCALES.map((locale) => {
    ...
    if (!detail) throw new Error(`es_session_20_phrase_locale_missing:${phraseIndex}:${locale}`);
...
function localizedPhraseDistractorFeedback(
  ...
  return Object.fromEntries(LEARNING_V2_INTERFACE_LOCALES.map((locale) => {
    ...
    if (!reason) throw new Error(`es_session_20_phrase_distractor_missing:${phraseIndex}:${locale}:${value}`);
...
function phraseAudio(phraseIndex: number): LearningV2ModeAudioReferenceV1 {
  const phrase = phrases[phraseIndex]!;
  return authoredAudio(`es-e01-s20-phrase-${phraseIndex + 1}`, phrase.english);
}
function phraseSlowAudio(phraseIndex: number): LearningV2ModeAudioReferenceV1 {
  const phrase = phrases[phraseIndex]!;
  return authoredAudio(`es-e01-s20-phrase-${phraseIndex + 1}-slow`, phrase.english);
}
...
function contextGapLastWord(phraseIndex: number): LearningV2ModeNativePayloadV1 {
  ...
  return Object.freeze({
    ...
    testedDimension: `fact_check_agreement:${asciiId(phrase.id)}`,
    choiceFeedback: Object.freeze([
      ...
      ...lastWord.distractors.map((entry) => feedback(
        ...
        false,
        `fact_check_agreement:${asciiId(entry.reasonCode)}`,
...
// зачем speed_match сочетает эти пять фраз (владелец, 2026-08-28, тот же
// приём, что сессии 11/17/19): пять фраз, разбросанных по всей сессии
// (индексы 3/6/9/12/14) — широкий срез проверки факта и согласия, а не
// смежный блок.
function speedMatchPhrases(pairIndices: readonly number[]): LearningV2ModeNativePayloadV1 {
  ...
  const ids = pairIndices.map((index) => `es-e01-s20-pair-${index + 1}`);
  return Object.freeze({
    family: 'speed_match',
...
// 2026-08-28, найдено при верификации мок-сборки сессии 20): в отличие от
// сессий 17/19 (все фразы = 2 слова связка+признак), фразы этой сессии —
// составные, многокомпонентные (De/de + acuerdo как отдельные токены,
// сдвоенные реплики "No es verdad, no de acuerdo"). Runtime-валидатор
// course_session_client_children_v1.ts (ID_RE, learning_v2_course_session_client_child_invalid)
// ограничивает responseFeedbackById максимум 8 записями на interaction.
// phraseBuilderFeedback() строит 1 (correct) + уникальные значения
// дистракторов; для индексов 6/7/9/10/11/13 (5-6 слов с 2 дистракторами
// каждое) это даёт 9-12 записей — выше потолка. listen_choose (только 2
// варианта) и context_gap_grammar (дистракторы только последнего слова)
// этому потолку не подвержены. Поэтому phrase_builder/listen_build_dictation
// здесь используются ТОЛЬКО на безопасных индексах (3/4/5/8/12/14 — ≤8
// уникальных дистракторов), а context_gap_grammar/listen_choose/speed_match
// покрывают "длинные" индексы (6/7/9/10/11/13). Полный охват индексов 3-14
// сохранён, изменился только выбор family на каждом индексе.
export const ES_EPISODE_01_SESSION_20_MODE_NATIVE_PRACTICE_V1 = Object.freeze<readonly SessionModeNativePracticeSourceV1[]>([
  ...
  { family: 'context_gap_grammar', purpose: 'retrieval_practice', learningStage: 'apply_in_phrase', target: { kind: 'phrase', sourceIndex: 7 }, modePayload: contextGapLastWord(7) },
  { family: 'phrase_builder', purpose: 'retrieval_practice', learningStage: 'apply_in_phrase', target: { kind: 'phrase', sourceIndex: 8 }, modePayload: phraseBuilder(8) },
  ...
  { family: 'context_gap_grammar', purpose: 'near_transfer', learningStage: 'apply_in_phrase', target: { kind: 'phrase', sourceIndex: 10 }, modePayload: contextGapLastWord(10) },
  { family: 'listen_choose', purpose: 'near_transfer', learningStage: 'apply_in_phrase', target: { kind: 'phrase', sourceIndex: 11 }, modePayload: listenChoosePhrase(11, 9) },
  { family: 'phrase_builder', purpose: 'near_transfer', learningStage: 'apply_in_phrase', target: { kind: 'phrase', sourceIndex: 12 }, modePayload: phraseBuilder(12) },
...
if (ES_EPISODE_01_SESSION_20_MODE_NATIVE_PRACTICE_V1.length !== 12) {
  throw new Error('es_session_20_mode_native_practice_count_invalid');
}
