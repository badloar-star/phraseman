import { runLessonQa } from './qa_service';
...
const validArtifact = {
  ...
  drills: [{ kind: 'irregular_verbs' as const, applicable: false, itemCount: 0 }],
};

describe('deterministic lesson QA service', () => {
  it('creates a stable passing receipt with artifact hash and source ids', () => {
    const result = runLessonQa({ artifact: validArtifact, blueprintHash: 'blueprint-hash', sourceEvidence: [{ evidenceId: 'src-1', kind: 'official_curriculum', authority: 'Authority', url: 'https://example.com/curriculum', retrievedAt: '2026-07-10', claim: 'Lesson order' }], now: '2026-07-10T00:00:00.000Z' });
  ...
  it('fails without evidence or with a duplicate phrase', () => {
    const result = runLessonQa({ artifact: { ...validArtifact, phrases: [...validArtifact.phrases.slice(0, 49), validArtifact.phrases[0]] }, blueprintHash: 'blueprint-hash', sourceEvidence: [] });
