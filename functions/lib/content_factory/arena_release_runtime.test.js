...
describe('canonical arena release runtime', () => {
    ...
    it('preserves strict Arena fields while serializing to the real string-answer runtime contract', () => {
        ...
        const [row] = (0, arena_release_runtime_1.arenaQuestionsFromCourseSurfaceEntries)({ studyTarget: 'en', learnerSourceLocale: 'ru', courseReleaseId: 'en-ru-r1' }, [{ lessonId: 9, payload: { lessonId: 9, surface: 'arena', items: [item] } }]);
    ...
    it('rejects a structured question whose index and string answer disagree', () => {
        ...
        expect(() => (0, arena_release_runtime_1.arenaQuestionsFromCourseSurfaceEntries)({ studyTarget: 'en', learnerSourceLocale: 'ru', courseReleaseId: 'en-ru-r1' }, [{ lessonId: 9, payload: { lessonId: 9, surface: 'arena', items: [item] } }])).toThrow('arena_release_payload_invalid');
    });
    it('resolves by stored unit provenance and never consults the current convergence mode', () => {
        ...
        expect((0, arena_release_runtime_1.arenaQuestionsFromCourseSurfaceEntries)(identity, [{ lessonId: 1, engineResolved: 'legacy', payload: legacyPayload }])).toHaveLength(1);
        expect(() => (0, arena_release_runtime_1.arenaQuestionsFromCourseSurfaceEntries)(identity, [{ lessonId: 1, engineResolved: 'stage', payload: legacyPayload }])).toThrow('arena_release_engine_payload_mismatch');
    });
    it.each([[8, 'A1'], [9, 'A2'], [16, 'A2'], [17, 'B1'], [24, 'B1'], [25, 'B2']])('keeps lesson %i on runtime level %s', (lessonId, level) => {
        const options = ['A', 'B', 'C', 'D'];
        const [row] = (0, arena_release_runtime_1.arenaQuestionsFromCourseSurfaceEntries)({ studyTarget: 'en', learnerSourceLocale: 'ru', courseReleaseId: `en-ru-r${lessonId}` }, [{ lessonId, payload: { lessonId, surface: 'arena', items: [{ id: `a${lessonId}`, prompt: 'Q?', answer: 'A', options }] } }]);
