...
for (const f of files) {
  ...
  for (let li = 0; li < lessonSections.length; li++) {
    ...
    for (let i = 0; i < ids.length; i++) {
      ...
      // extract all word texts via global regex
      const wordTexts = [];
      const wre = /text:\s*'((?:[^'\\]|\\.)*)'/g;
      let wm;
      while ((wm = wre.exec(block)) !== null) wordTexts.push(unescape(wm[1]));
...
fs.writeFileSync('docs/reports/_phrases_all.json', JSON.stringify(filtered, null, 2));
