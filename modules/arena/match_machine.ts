 ...
 *
 * зачем (владелец 2026-09-04, вместе с удлинением окон ответа): полторы
 * секунды человек тратил на то, чтобы вообще понять, что от него хотят —
 * таймер успевал побежать раньше, чем глаз дочитывал задание. Секунда сверху
 * снимает именно это ощущение спешки на старте КАЖДОГО задания.
 *
 * Копий три: здесь (клиент считает матч сам), `ARENA_DUEL_READING_MS`
 * в `functions/src/arena_duel_v3.ts` (бюджет матча в плане) и
 * `ARENA_V2_READING_MS` в `arena_v2_core.ts` (серверные фазы v2). Клиент
 * читает `readingMs` из плана, но фазу ведёт по СВОЕЙ константе — поэтому
 * правка только на сервере паузу не удлинит.
 */
export const ARENA_LOCAL_READING_MS = 2_500;
