...
import type { PromptContext } from './prompt_context';
...
export interface StagePromptPacket {
  readonly kind: GenerationStageKind;
  ...
  readonly grounding: Readonly<Record<string, unknown>> | null;
  readonly groundingHash: string | null;
}

const STAGE_TASKS: Readonly<Record<GenerationStageKind, string>> = Object.freeze({
...
const DEFINITIONS = new Map<string, PromptDefinition>(Object.entries(STAGE_TASKS).map(([kind, task]) => {
  const stageKind = kind as GenerationStageKind;
...
for (const kind of ['lesson_outline', 'lesson_phrases', 'lesson_vocabulary', 'lesson_irregular_verbs', 'lesson_prepositions', 'lesson_theory'] as const) {
  const previous = DEFINITIONS.get(`${kind}:v2`);
  if (!previous) throw new Error(`lesson_v2_prompt_missing:${kind}`);
...
}
{
  ...
  if (!current) throw new Error('lesson_phrases_v3_prompt_missing');
  const properties = (current.outputSchema.properties ?? {}) as Record<string, unknown>;
...
for (const kind of ['challenge_topic'] as const) {
  DEFINITIONS.set(`${kind}:v2`, Object.freeze({ kind, version: 'v2', task: 'Create one editable challenge topic proposal from the requested languages, CEFR and optional administrator guidance. Return title, learningPromise, skillTags, inclusions, exclusions and an exact ten-question difficultyDistribution.', outputSchema: Object.freeze({ type: 'object', additionalProperties: false, required: ['stage', 'result'], properties: { stage: { const: kind }, result: { type: 'object', additionalProperties: false, required: ['topicId', 'title', 'learningPromise', 'skillTags', 'inclusions', 'exclusions', 'difficultyDistribution'] } } }) }));
}
for (const kind of ['challenge_questions'] as const) {
  // зачем: правило про полноту/неусечённость добавлено В САМ v2, а не как v3 — валидация
  // в stage_runner.ts:81 привязана к promptVersion === 'v2', поэтому новая версия молча
  // ОТКЛЮЧИЛА бы проверки. Репорт: «Как по-английски "открывалка для банок"?» с вариантами
  // tongs/jar/corkscrew/masher — правильный ответ («jar opener») был усечён до дистрактора
  // «jar», и правильного варианта в списке не осталось вовсе.
  DEFINITIONS.set(`${kind}:v2`, Object.freeze({ kind, version: 'v2', task: 'Create exactly ten unique questions grounded in the approved topic. Each item needs a source-language prompt, exactly four unique target-language choices, one correctIndex, four source-language optionExplanations aligned by choice index, approved skillTag, difficulty and sourcePhraseIds when applicable. Distractors must be plausible but unambiguously wrong. The choice at correctIndex must be the complete, truthful answer to the prompt — never truncated, never a partial form. When the correct answer is a multi-word term (for example "jar opener"), no distractor may be a fragment of it (never "jar"): each distractor must be its own independent term, not a shortened piece of the correct answer. Before returning, verify for every item that the choice at correctIndex genuinely answers the prompt and that no other choice is contained inside it word-for-word.', outputSchema: Object.freeze({ type: 'object', additionalProperties: false, required: ['stage', 'items'], properties: { stage: { const: kind }, items: { type: 'array', minItems: 10, maxItems: 10, items: { type: 'object', additionalProperties: false, required: ['id', 'prompt', 'choices', 'correctIndex', 'optionExplanations', 'skillTag', 'difficulty', 'sourcePhraseIds'] } } } }) }));
}
for (const kind of ['challenge_question_replacement'] as const) {
...
for (const kind of ['flashcard_pack_idea', 'flashcard_items', 'flashcard_item_replacement'] as const) {
  const previous = DEFINITIONS.get(`${kind}:v2`);
  if (!previous) throw new Error(`flashcard_v2_prompt_missing:${kind}`);
...
export function promptDefinitionFor(kind: GenerationStageKind, version: string): PromptDefinition {
  const definition = DEFINITIONS.get(`${kind}:${version}`);
  if (!definition) throw new Error('prompt_definition_not_found');
  return definition;
}

export function buildStagePromptPacket(kind: GenerationStageKind, version: string, context: PromptContext, grounding: Readonly<Record<string, unknown>> | null = null): StagePromptPacket {
...
export function buildLessonPhraseChunkPromptPacket(base: StagePromptPacket, chunkIndex: number, acceptedChunkHashes: readonly string[], acceptedPhraseExclusions: { readonly ids?: readonly string[]; readonly meaningKeys?: readonly string[]; readonly pairs?: readonly string[] } = {}): StagePromptPacket {
  if (base.kind !== 'lesson_phrases' || !Number.isSafeInteger(chunkIndex) || chunkIndex < 0 || chunkIndex >= 5 || acceptedChunkHashes.some((value) => !/^[a-f0-9]{64}$/i.test(value))) throw new Error('lesson_phrase_chunk_packet_invalid');
  const context = Object.freeze({ ...base.context, count: 10, previousContentFingerprints: Object.freeze([...acceptedChunkHashes]) });
  const itemSchema = ((base.outputSchema.properties as Record<string, unknown> | undefined)?.items ?? {}) as Record<string, unknown>;
  const outputSchema = Object.freeze({ ...base.outputSchema, properties: { ...(base.outputSchema.properties as Record<string, unknown> ?? {}), items: { ...itemSchema, minItems: 10, maxItems: 10 } } });
