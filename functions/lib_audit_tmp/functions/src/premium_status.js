...
exports.isLifetimePlanActive = isLifetimePlanActive;
exports.resolveIsLifetimePlan = resolveIsLifetimePlan;
 ...
 */
async function resolveOwnedHiddenAliasAccess(db, canonicalStableUid, authUid, now, transaction) {
    ...
    return aliases?.docs?.some((doc) => {
        ...
        if (data.identityHidden !== true)
            return false;
        if (cleanStr(data.canonicalStableId) !== canonicalStableUid)
            return false;
        if (!userDocOwnedByAuth(data, authUid))
            return false;
        return isPremiumAccessActive((data.progress ?? {}), now);
    }) ?? false;
}
 ...
 */
async function resolvePremiumAccess(db, stableUid, now = Date.now(), authUid, transaction) {
    ...
    for (;;) {
        ...
        await Promise.all(ids.map(async (id) => {
            ...
            if (id === stableUid) {
                stableUidOwnedByAuth || (stableUidOwnedByAuth = Boolean(authUid && userDocOwnedByAuth(data, authUid)));
                stableUidIsCanonical = data.identityHidden !== true;
                stableUidHasVipEntitlementShape = hasVipEntitlementShape((data.progress ?? {}));
            }
    ...
    if (authUid
        ...
        if (observedOwnedHiddenAliasAccess)
            return true;
        if (!providerLookupMayBeTruncated)
            return false;
 ...
 */
function isLifetimePlanActive(progress, now = Date.now()) {
    const data = progress ?? {};
    const plan = cleanPlan(data.premium_plan);
 ...
 */
async function resolveIsLifetimePlan(db, stableUid, now = Date.now(), authUid) {
    const candidates = new Set();
    const add = (value) => {
        const id = cleanStr(value);
        if (id)
