...
// Реэкспорт намеренный: на сервере эти типы объявлены прямо в файле-копии,
// и всё, что импортирует движок, обязано видеть их с обеих сторон одинаково.
export type { ArenaEntryMode, ArenaTaskMode };

/**
 * Чистый движок звёзд Арены — единственный источник истины по начислению.
 *
 * Владелец (2026-08-12): отдельных очков матча не существует. Звёзды и есть
 * счёт: кто набрал больше, тот победил; при равенстве побеждает меньшее
 * суммарное время. Файл байт-в-байт переносится на сервер
 * (`functions/src/arena_stars_v3.ts`) и проверяется тестом на паритет — расчёт
 * на клиенте и на сервере обязан совпадать до последней звезды, иначе игрок
 * увидит одно число, а получит другое.
 *
 * Ни React, ни Firebase, ни Date.now(), ни ввода-вывода. Только арифметика.
 */
