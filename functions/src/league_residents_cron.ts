...
// 6 часов у них растёт опыт, а вместе с ним уровень и аватар. Опыт — чистая
...
// с лимитом, одна транзакция на комнату, 4 запуска в сутки. Комнаты, где живых
    ...
    for (const doc of snap.docs) {
      try {
        await db.runTransaction(async (tx) => {
          ...
          // Комната «переросла» порог: живых стало 15+. Жителей не добавляем.
          // Уже подселённых НЕ выгоняем — исчезновение соседей прямо на глазах
          // выглядит куда подозрительнее, чем их присутствие.
