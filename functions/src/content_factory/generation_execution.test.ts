import {
  canCommitGenerationExecution,
  generationPersistenceError,
  runGuardedGenerationTransaction,
  type GenerationExecutionLease,
} from './generation_execution';

const lease: GenerationExecutionLease = Object.freeze({ attempt: 2, leaseToken: 'lease-current' });

describe('generation execution guard', () => {
  test('allows only the matching lease and attempt in an allowed state', () => {
    expect(canCommitGenerationExecution(
      { state: 'running', attempts: 2, leaseToken: 'lease-current' },
      lease,
      ['running'],
    )).toBe(true);
    expect(canCommitGenerationExecution(
      { state: 'running', attempts: 1, leaseToken: 'lease-current' },
      lease,
      ['running'],
    )).toBe(false);
    expect(canCommitGenerationExecution(
      { state: 'running', attempts: 2, leaseToken: 'lease-stale' },
      lease,
      ['running'],
    )).toBe(false);
    expect(canCommitGenerationExecution(
      { state: 'cancelled', attempts: 2, leaseToken: 'lease-current' },
      lease,
      ['running'],
    )).toBe(false);
  });
  ...
  test('permits a guarded terminal commit from a generated checkpoint', () => {
    expect(canCommitGenerationExecution(
      { state: 'generated', attempts: 2, leaseToken: 'lease-current' },
      lease,
      ['running', 'generated'],
    )).toBe(true);
  });
...
describe('generation persistence errors', () => {
  test('returns a stable public error without leaking persistence details', () => {
    ...
    const error = generationPersistenceError(cause);
  ...
  test.each(['legacy-success', 'legacy-failure', 'stage-success', 'stage-failure'])(
    'normalizes rejected %s terminal transactions',
    async () => {
  ...
  test('runs the commit only for the matching production guard', async () => {
    ...
    const result = await runGuardedGenerationTransaction({
      lease,
      allowedStates: ['running'],
