...
type GeneratedRow = {
  ...
  generatedBy: 'gustav_personal_plan_echo_day004_french_full_day_text_packet';
};

const PLAN_ID = 'echo' as const;
const DAY_INDEX = 4 as const;
const GENERATED_BY = 'gustav_personal_plan_echo_day004_french_full_day_text_packet' as const;

const EXPLANATIONS = [
  {
    phraseId: 'echo_d4_p1',
    title: 'Nier une habitude',
    rule: "I + do not + mot d'action. « Don't » est la forme courte de « do not ».",
    why: "« Do not » indique que l'action ne se produit pas. Sans lui, le sens est l'inverse.",
    commonMistake: "Ne dis pas « I not watch » : il faut « do not » ou « don't » avant l'action.",
  },
  {
    phraseId: 'echo_d4_p2',
    title: "Quand tu parles d'elle",
    rule: "She/He + does not + mot d'action, sans -s. « Doesn't » est la forme courte.",
    why: "Avec « she/he », seul « does » change. Le mot d'action reste inchangé.",
    commonMistake: "Ne dis pas « She does not reads » : après « does not », l'action ne prend pas -s.",
  },
  {
    phraseId: 'echo_d4_p3',
    title: "Les habitudes d'une autre personne",
    rule: 'He + does not + listen to. « Listen to » veut dire écouter quelque chose. « to » est obligatoire.',
    why: '« Listen » va toujours avec « to » avant ce que tu écoutes.',
    commonMistake: "N'oublie pas « to » : dis « listen to the radio », pas « listen the radio ».",
  },
  {
    phraseId: 'echo_d4_p4',
    title: "Parler pour toi et quelqu'un d'autre",
    rule: "We + do not + mot d'action. « We » veut dire moi et quelqu'un d'autre.",
    why: '« We », « I », « you » et « they » utilisent « do not », pas « does not ».',
    commonMistake: 'Ne mélange pas : « We does not » est incorrect. Dis seulement « We do not ».',
  },
  {
    phraseId: 'echo_d4_p5',
    title: 'Les habitudes de plusieurs personnes',
    rule: "They + do not + mot d'action. « They » désigne plusieurs personnes ou choses.",
    why: '« They » fonctionne comme « we » : les deux utilisent « do not », sans changement.',
    commonMistake: "N'ajoute pas -s à l'action : « They do not buys » est incorrect.",
  },
  {
    phraseId: 'echo_d4_p6',
    title: 'Parler de ton ami',
    rule: "My friend équivaut à he/she. Donc : « does not » + mot d'action sans changement.",
    why: "Quand tu nommes quelqu'un par son nom ou son rôle, la même règle que pour « he/she » s'applique.",
    commonMistake: "Ne dis pas « My friend do not use » : avec une seule personne, c'est « does not ».",
  },
] as const;
...
const FIELD_SPECS: FieldSpec[] = [
  {
    fieldPath: 'topic',
    fieldKind: 'day_topic',
    frenchText: 'Dire ce que tu ne fais pas',
    source: (day) => day.topic,
  },
  {
    fieldPath: 'outcome',
    fieldKind: 'day_outcome',
    frenchText: 'Tu peux dire que tu ne fais pas quelque chose dans ta vie : regarder, écouter, lire.',
    source: (day) => day.outcome,
  },
  {
    fieldPath: 'intro[0].title',
    fieldKind: 'intro_title',
    frenchText: 'Comment dire « je ne fais pas »',
    source: (day) => day.intro?.[0]?.title,
  },
  {
    fieldPath: 'intro[0].body',
    fieldKind: 'intro_body',
    frenchText: "Tu veux dire que tu ne fais pas quelque chose ? Ajoute « do not » avant le mot d'action. La forme courte est « don't » : cela veut dire la même chose.",
    source: (day) => day.intro?.[0]?.body,
  },
  {
    fieldPath: 'intro[0].examples[0].gloss',
    fieldKind: 'intro_example_gloss',
    frenchText: 'Je ne regarde pas souvent la télé.',
  ...
  },
  {
    ...
    fieldKind: 'intro_example_gloss',
    frenchText: 'Elle ne lit pas les nouvelles.',
  ...
  },
  {
    ...
    fieldKind: 'intro_title',
    frenchText: "Avec « he/she/it », c'est un peu différent",
    source: (day) => day.intro?.[1]?.title,
  },
  {
    fieldPath: 'intro[1].body',
    fieldKind: 'intro_body',
    frenchText: "Quand tu parles d'une autre personne, « he » ou « she », tu écris « does not » ou « doesn't ». Le mot d'action ne change pas.",
    source: (day) => day.intro?.[1]?.body,
  },
  {
    fieldPath: 'intro[1].examples[0].gloss',
    fieldKind: 'intro_example_gloss',
    frenchText: "Il n'écoute pas la radio.",
  ...
  },
  {
    ...
    fieldKind: 'intro_example_gloss',
    frenchText: 'Ils ne vont pas au cinéma.',
  ...
  },
  {
    ...
    fieldKind: 'intro_title',
    frenchText: 'Jour 4 : médias et habitudes',
    source: (day) => day.intro?.[2]?.title,
  },
  {
    fieldPath: 'intro[2].body',
    fieldKind: 'intro_body',
    frenchText: "Aujourd'hui, on parle des nouvelles et des médias. Ce que les gens regardent, écoutent ou lisent, et ce qu'ils ne font pas. C'est utile dans toute conversation sur la vie et les habitudes.",
    source: (day) => day.intro?.[2]?.body,
  },
  {
    fieldPath: 'intro[2].examples[0].gloss',
    fieldKind: 'intro_example_gloss',
    frenchText: 'Je ne regarde pas les nouvelles tous les jours.',
  ...
  },
  {
    ...
    fieldKind: 'intro_example_gloss',
    frenchText: 'Nous ne lisons pas de journaux.',
...
function rowIdFromPath(fieldPath: string): string {
  return `fr_echo_d004_${fieldPath.replace(/[^a-zA-Z0-9]+/g, '_').replace(/^_+|_+$/g, '').toLowerCase()}`;
}
...
function renderMarkdown(report: any): string {
  const lines = [
    '# GUSTAV Personal Plan Echo Day 004 French Full-Day Text Packet',
    '',
    `Run: \`${report.runId}\``,
    '',
...
async function main(): Promise<void> {
  ...
  if (!runArg) {
    throw new Error('Usage: npx tsx scripts/gustav_personal_plan_echo_day004_french_full_day_text_packet.ts --run <run-dir>');
  }
  ...
  const rowsPath = path.join(outDir, 'echo_day_004_full_day_fr_rows.jsonl');
  const reviewerQueuePath = path.join(outDir, 'echo_day_004_full_day_fr_reviewer_queue.tsv');
  const packetJsonPath = path.join(auditsDir, 'personal_plan_echo_day004_french_full_day_text_packet.json');
  const packetMdPath = path.join(auditsDir, 'personal_plan_echo_day004_french_full_day_text_packet.md');

  const findings: Finding[] = [];
  const day = await readEchoDay(repoRoot);
  if (!day) {
    addFinding(findings, 'blocker', 'echo_day_004_missing', 'Echo day 4 is missing from source content.', rel(repoRoot, sourcePath));
  }

  let existingPhraseMeaningRowsForDay = 0;
  if (!fs.existsSync(phraseMeaningRowsPath)) {
    addFinding(findings, 'blocker', 'phrase_meaning_rows_missing', 'Echo day 4 phrase meaning rows are missing.', rel(repoRoot, phraseMeaningRowsPath));
  } else {
    ...
    if (existingPhraseMeaningRowsForDay !== 6) {
      addFinding(findings, 'blocker', 'phrase_meaning_rows_day_004_incomplete', `Expected 6 reviewer-needed phrase meaning rows for Echo day 4 but found ${existingPhraseMeaningRowsForDay}.`, rel(repoRoot, phraseMeaningRowsPath));
    }
  }
  ...
  const report = {
    schemaVersion: 'gustav-personal-plan-echo-day004-french-full-day-text-packet-v0',
    ...
    fieldKindCounts,
    generationPolicy: [
      'This packet translates Echo day 4 full-day text fields except phrase meanings, which are already covered by the phrase-meaning layer.',
  ...
  console.log(`GUSTAV Echo day 004 French full-day text packet: ${report.status}`);
