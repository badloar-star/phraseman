...
- **Bundled reserve (маленький, НЕ 130MB):** в бандл кладём небольшой резерв на каждую секцию: ~10–20 готовых элементов урока/объяснений на дефолтную пару (`en` target × `ru` ui) + ~30–50 коротких TTS-клипов ядра. Цель размера бандла-резерва: **< 5 MB** аудио (короткие mp3 самых частых фраз ядра), текстовый резерв — десятки КБ. Полные 130MB pre-rendered остаются как есть отдельно (не трогаем в этой спеке), bundled reserve — это аварийная подушка для генеративного пути.
- **Fallback order (тот же, что §5, но оффлайн):** primed buffer → локально закешированное (Firestore offline persistence через `nandorojo/expo-firestore-offline-persistence`, research) → bundled reserve → честный индикатор.
- **Честный UI-индикатор:** маленький ненавязчивый бейдж «оффлайн — показываем сохранённое», без блокирующего спиннера. Не притворяться, что онлайн.
- Админ-хук: раздел показывает, какие языковые пары ИМЕЮТ bundled reserve и его версию (`reserveVersion`), чтобы владелец знал, для каких языков оффлайн-путь пустой.
