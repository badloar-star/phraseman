...
exports.adminSetShardBalance = exports.adminGrantReward = exports.ADMIN_GRANT_REWARD_TYPES = void 0;
...
exports.normalizeAdminSetShardBalanceInput = normalizeAdminSetShardBalanceInput;
exports.adminSetShardBalanceFingerprint = adminSetShardBalanceFingerprint;
exports.buildAdminRewardMutation = buildAdminRewardMutation;
exports.buildAdminShardBalanceMutation = buildAdminShardBalanceMutation;
exports.assertAdminRewardReplay = assertAdminRewardReplay;
exports.assertAdminShardBalanceReplay = assertAdminShardBalanceReplay;
/**
...
const SHARD_BALANCE_MAX = 1000000;
...
function normalizeAdminSetShardBalanceInput(data) {
    if (!isRecord(data))
        throw new https_1.HttpsError('invalid-argument', 'shard balance command required');
    const uid = text(data.uid, 161);
    ...
    if (typeof newBalance !== 'number' || !Number.isSafeInteger(newBalance) || newBalance < 0 || newBalance > SHARD_BALANCE_MAX) {
        throw new https_1.HttpsError('invalid-argument', `newBalance must be an integer from 0 to ${SHARD_BALANCE_MAX}`);
    }
    if (!reason)
        throw new https_1.HttpsError('invalid-argument', 'reason is required');
    if (!TOKEN_RE.test(idempotencyKey) || !TOKEN_RE.test(requestId)) {
...
function adminSetShardBalanceFingerprint(input) {
...
function buildAdminShardBalanceMutation(user, newBalance, nowMs) {
...
function assertAdminShardBalanceReplay(operation, fingerprint, actorUid) {
    if (operation.action !== 'set_shard_balance') {
        throw new https_1.HttpsError('already-exists', 'idempotencyKey belongs to another admin action');
    }
    if (operation.requestFingerprint !== fingerprint) {
        throw new https_1.HttpsError('already-exists', 'idempotencyKey reused for another shard balance command');
    }
    if (operation.actorUid && operation.actorUid !== actorUid) {
        throw new https_1.HttpsError('permission-denied', 'admin operation belongs to another actor');
    }
...
exports.adminGrantReward = (0, https_1.onCall)(callable_options_1.ADMIN_SENSITIVE_WRITE_OPTIONS, async (request) => {
...
exports.adminSetShardBalance = (0, https_1.onCall)(callable_options_1.ADMIN_SENSITIVE_WRITE_OPTIONS, async (request) => {
    (0, callable_options_1.requireAdminAppCheck)(request);
    const actor = requireRewardWriter(request);
    ...
    return db.runTransaction(async (tx) => {
        ...
        const target = await (0, admin_access_controls_1.resolveCanonicalAdminAccessTarget)(tx, db, input.uid);
