...
const COURSE_STAGES = Object.freeze([
  {
    kind: 'learning_v2_research',
    title: 'Исследование',
    description:
      'Методика, источники и требования к реальному обучению с нуля.',
  },
  {
    kind: 'learning_v2_curriculum',
    title: 'Программа Pre-A1 → C2',
    description:
      'Уровни, навыки, повторение и последовательность роста сложности.',
  },
  {
    kind: 'learning_v2_lesson_outline',
    title: 'Карта курса',
    description:
      'Секторы, интро, уроки, практики, экзамены и контрольные точки.',
  },
  {
    kind: 'learning_v2_localized_course',
    title: 'Полный учебный контент',
    description: 'Объяснения, задания, подсказки и проверки сразу на 8 языках.',
  },
  {
    kind: 'learning_v2_audio',
    title: 'Аудиопакеты',
    description: 'Каждая фраза в голосах Ash, Onyx, Nova и Coral.',
  },
  {
    kind: 'learning_v2_quality_assurance',
    title: 'Проверка качества',
    description:
      'Педагогика, языки, доступность, зависимости и целостность курса.',
  },
  {
    kind: 'learning_v2_release',
    title: 'Кандидат в релиз',
    description:
      'Финальный пакет. Публикация возможна только после вашего решения.',
  },
]);

const COURSE_LOCALES = Object.freeze([
  'ru',
  'uk',
  'es',
  'pt-BR',
...
const visualMode =
  localHost && new URLSearchParams(location.search).get('live') !== '1';
...
function approvedByKind(kind) {
  return (
    state.stages
      .filter((stage) => stage.kind === kind && stage.state === 'approved')
      .sort((a, b) => Number(b.revision || 0) - Number(a.revision || 0))[0] ||
    null
  );
}
...
function waveApprovalId(stage) {
  return stage?.kind === 'learning_v2_localized_course' &&
    stage?.state === 'paused' &&
    stage?.learningV2PauseReason === 'owner_wave_approval_required' &&
...
function stageStatus(stage) {
  ...
  if (requiredWave)
    return {
      label: `Ждёт одобрения ${requiredWave}`,
      className: 'review',
      icon: 'eye',
    };
  if (
    stage.kind === 'learning_v2_localized_course' &&
    stage.state === 'paused' &&
    stage.learningV2PauseReason === 'root_manifest_materialization_pending'
  ) {
    return {
      label: 'Сессии готовы, собирается пакет',
      className: 'running',
      icon: 'clock',
    };
  }
  return (
    {
      ...
      label: String(stage.state || 'Неизвестно'),
      className: '',
      icon: 'clock',
    }
  );
...
function nextUnapprovedDefinition() {
  return (
    COURSE_STAGES.find((definition) => !approvedByKind(definition.kind)) || null
  );
}
...
function renderWorkflow() {
  ...
  const approvedCount = COURSE_STAGES.filter((definition) =>
    approvedByKind(definition.kind),
  ).length;
  byId('progress-label').textContent =
    `${approvedCount} из ${COURSE_STAGES.length} одобрено`;
  byId('progress-bar').style.width =
    `${(approvedCount / COURSE_STAGES.length) * 100}%`;
  ...
  COURSE_STAGES.forEach((definition, index) => {
    ...
    const requiredWave = waveLabel(stage);
    if (stage && requiredWave)
      meta.append(
        stageButton(`Проверить ${requiredWave}`, 'eye', () =>
          previewWave(stage, 1, 1),
        ),
      );
    else if (stage && ['queued', 'failed'].includes(stage.state))
      meta.append(stageButton('Запустить', 'play', () => runStage(stage.id)));
    else if (
      stage &&
      stage.state === 'paused' &&
      stage.learningV2PauseReason !== 'root_manifest_materialization_pending'
    )
      meta.append(stageButton('Продолжить', 'play', () => runStage(stage.id)));
    if (stage && ['needs_review', 'approved', 'rejected'].includes(stage.state))
      meta.append(stageButton('Открыть', 'eye', () => previewStage(stage.id)));
...
function renderPrimaryAction() {
  ...
  if (!stage || ['rejected', 'cancelled', 'superseded'].includes(stage.state))
    nextButton.textContent =
      stage?.state === 'rejected'
        ? 'Создать исправленную версию'
        : `Создать этап «${definition.title}»`;
  else if (waveLabel(stage))
    nextButton.textContent = `Проверить и решить по ${waveLabel(stage)}`;
  else if (
    stage.state === 'paused' &&
    stage.learningV2PauseReason === 'root_manifest_materialization_pending'
  ) {
    ...
    nextButton.disabled = true;
  } else if (['queued', 'failed', 'paused'].includes(stage.state))
    nextButton.textContent = `Запустить этап «${definition.title}»`;
  else if (stage.state === 'needs_review')
    nextButton.textContent = `Проверить этап «${definition.title}»`;
  else if (stage.state === 'running')
    nextButton.textContent = 'Генерация уже идёт';
  else nextButton.textContent = 'Продолжить';
}

function validateInput() {
  const input = courseInput();
  if (!/^[A-Za-z0-9._-]{1,160}$/.test(input.requestId))
    throw new Error(
      'Проверьте ID пакета: допустимы буквы, цифры, точка, дефис и подчёркивание.',
    );
  if (input.objective.length < 10 || input.objective.length > 1000)
    throw new Error('Опишите цель курса понятной фразой.');
  return input;
}
...
async function refreshStages() {
  ...
  try {
    const response = await callable('adminListContentStages')({
    ...
    state.stages = Array.isArray(response?.data?.stages)
      ? response.data.stages.filter((stage) =>
          String(stage?.kind || '').startsWith('learning_v2_'),
        )
      : [];
    renderWorkflow();
    setStatus(
      state.stages.length
        ? 'Очередь актуальна.'
        : 'Курс ещё не начат. Создайте исследование.',
      state.stages.length ? 'success' : '',
    );
  } catch (error) {
    setStatus(
      `Не удалось загрузить очередь: ${error?.message || error}`,
      'error',
    );
  } finally {
    setBusy(false);
...
async function createNextStage() {
  ...
  const index = COURSE_STAGES.findIndex(
    (definition) => !approvedByKind(definition.kind),
  );
  if (index < 0) return;
  const definition = COURSE_STAGES[index];
  const current = latestByKind(definition.kind);
  const prerequisite =
    index > 0 ? approvedByKind(COURSE_STAGES[index - 1].kind) : null;
  if (index > 0 && !prerequisite)
    throw new Error('Сначала одобрите предыдущий этап.');
  if (visualMode) {
  ...
  try {
    await callable('adminCreateContentStage')({
      ...input,
      kind: definition.kind,
      revision: current ? Number(current.revision || 1) + 1 : 1,
      prerequisiteStageIds: prerequisite ? [String(prerequisite.id)] : [],
    });
...
async function runStage(stageId) {
  if (state.busy) return;
  if (visualMode) {
    setStatus('Это визуальный макет: генерация отключена.', 'success');
    return;
  }
  setBusy(true);
...
function samplePreview() {
  const localizedContent = Object.fromEntries(
    COURSE_LOCALES.map((locale) => [
      locale,
      {
        ...
        structure: [
          '3 интро-экрана со встроенными вопросами',
          'Практика начинается с позиции 4',
          '12 коротких практических сессий',
          'Экзамен сектора',
        ],
      },
    ]),
  ...
  return {
    stage: latestByKind('learning_v2_lesson_outline'),
    reviewFingerprint: 'visual-only',
    payload: { result: { localizedContent } },
  };
}
...
function sampleWavePreview(sessionOrdinal = 1) {
  const intro = {
    ...
    summaryByLocale: sampleLocalized(
      'Полноценное объяснение первых приветствий для абсолютного нуля',
    ),
    learningGoalByLocale: sampleLocalized(
      'Узнать приветствие и назвать своё имя',
    ),
    pages: Array.from({ length: 3 }, (_, index) => ({
      pageOrdinal: index + 1,
      pageId: `intro-page-${index + 1}`,
      kind: 'concept',
      titleByLocale: sampleLocalized(`Блок объяснения ${index + 1}`),
      bodyByLocale: sampleLocalized(`Подробное содержание блока ${index + 1}`),
      question: {
        questionId: `intro-question-${index + 1}`,
        ...
        choicesByLocale: Object.fromEntries(
          COURSE_LOCALES.map((locale) => [
            locale,
            [
              `Правильный вариант ${locale}`,
              `Отвлекающий вариант A ${locale}`,
              `Отвлекающий вариант B ${locale}`,
            ],
          ]),
        ),
  ...
  const cards = Array.from({ length: 12 }, (_, index) => ({
    ...
    contentItem: {
      ...
      learnerMeanings: COURSE_LOCALES.map((locale) => ({
        locale,
        value: `Значение задания ${index + 1} · ${locale}`,
      })),
    },
  ...
  return {
    ...
    sessions: Array.from({ length: 12 }, (_, index) => ({
      ...
      titleByLocale: sampleLocalized(`Сессия ${index + 1}`),
    })),
...
async function previewStage(stageId) {
  ...
  try {
    const response = visualMode
      ? { data: samplePreview() }
      : await callable('adminPreviewContentStage')({
    ...
    setStatus(
      `Не удалось открыть результат: ${error?.message || error}`,
      'error',
    );
  }
}
...
async function previewWave(stage, episodeOrdinal, sessionOrdinal) {
  const requiredWave = waveLabel(stage);
  if (!requiredWave) {
    setStatus('Эта контрольная волна ещё не готова к проверке.', 'error');
    return;
  }
  if (visualMode) {
    ...
    openReviewLayer();
    setStatus(
      'Локальный макет контрольной проверки. Серверные данные не загружены.',
      'success',
    );
    return;
  }
  ...
  try {
    ...
    openReviewLayer();
    setStatus(
      `Открыта проверка ${requiredWave}. Ничего не опубликовано.`,
      'success',
    );
  } catch (error) {
    setStatus(
      `Не удалось проверить созданные сессии: ${error?.message || error}`,
      'error',
    );
  } finally {
    setBusy(false);
...
async function changeWavePreview(episodeOrdinal, sessionOrdinal) {
  ...
  if (visualMode) {
    ...
    renderReview();
    setStatus(
      'Локальный макет выбранной сессии. Серверные данные не загружены.',
      'success',
    );
    return;
  }
...
function renderStageReview() {
  ...
  const allLocalesReady = COURSE_LOCALES.every(
    (locale) => result.localizedContent?.[locale] != null,
  );
  byId('review-title').textContent =
    COURSE_STAGES.find((item) => item.kind === state.previewStage?.kind)
      ?.title || 'Результат этапа';
  byId('review-subtitle').textContent =
    `Версия ${Number(state.previewStage?.revision || 1)} · ${stageStatus(state.previewStage).label}`;
  const summary = byId('review-summary');
  summary.className = `review-summary${allLocalesReady ? '' : ' is-incomplete'}`;
  summary.textContent = allLocalesReady
    ? 'Все 8 обязательных языков присутствуют в пакете.'
    : 'Пакет неполный: один или несколько языков отсутствуют. Одобрение недоступно.';
  byId('review-content').textContent =
    localized == null
      ? 'Для этого языка результат отсутствует.'
...
function localizedText(value, locale) {
  return typeof value?.[locale] === 'string'
    ? value[locale]
    : 'Текст для этого языка отсутствует.';
}
...
function renderWaveReview() {
  ...
  const requiredWave =
    WAVE_LABELS[preview.waveId] || String(preview.waveId || 'волны');
  byId('review-title').textContent = `Проверка ${requiredWave}`;
  byId('review-subtitle').textContent =
    `Эпизод ${Number(preview.episodeOrdinal)} из ${Number(preview.lastEpisodeOrdinal)} · сессия ${Number(preview.selectedSessionOrdinal)} из 12`;
  const summary = byId('review-summary');
  ...
  byId('wave-episode-label').textContent =
    `Эпизод ${episode} из ${lastEpisode}`;
  ...
  for (const session of Array.isArray(preview.sessions)
    ? preview.sessions
    : []) {
    const option = document.createElement('option');
    option.value = String(session.sessionOrdinal);
    option.selected =
      Number(session.sessionOrdinal) === Number(preview.selectedSessionOrdinal);
  ...
  introTitle.textContent = localizedText(
    shard.intro?.titleByLocale,
    state.locale,
  );
  introSection.append(introTitle);
  appendParagraph(
    introSection,
    localizedText(shard.intro?.summaryByLocale, state.locale),
  );
  appendParagraph(
    introSection,
    `Цель: ${localizedText(shard.intro?.learningGoalByLocale, state.locale)}`,
  );
  for (const page of Array.isArray(shard.intro?.pages)
    ? shard.intro.pages
    : []) {
    const item = document.createElement('div');
    item.className = 'wave-review-block';
    const title = document.createElement('strong');
    title.textContent = `Экран ${page.pageOrdinal}: ${localizedText(page.titleByLocale, state.locale)}`;
    item.append(title);
    appendParagraph(
      item,
      localizedText(page.bodyByLocale, state.locale),
      'wave-review-copy',
    );
    const question = page.question;
    appendParagraph(
      item,
      `Вопрос внизу · каноническая позиция ${question.requiredTaskSlot}`,
      'wave-review-meta',
    );
    appendParagraph(
      item,
      localizedText(question.promptByLocale, state.locale),
      'wave-review-target',
    );
    ...
    if (Array.isArray(choices))
      choices.forEach((choice, index) =>
        appendParagraph(
          item,
          `${index === question.correctChoiceIndex ? 'Правильный ответ' : 'Вариант'}: ${choice}`,
          'wave-review-copy',
        ),
      );
    appendParagraph(
      item,
      localizedText(question.explanationByLocale, state.locale),
      'wave-review-copy',
    );
  ...
  for (const card of Array.isArray(shard.cards) ? shard.cards : []) {
    ...
    appendParagraph(
      item,
      `Задание ${card.taskSlot} · ${String(card.family || '')}`,
      'wave-review-meta',
    );
    appendParagraph(
      item,
      String(card.contentItem?.target?.text || ''),
      'wave-review-target',
    );
    const meaning = Array.isArray(card.contentItem?.learnerMeanings)
      ? card.contentItem.learnerMeanings.find(
          (candidate) => candidate?.locale === state.locale,
        )?.value
      : null;
    if (meaning)
      appendParagraph(item, `Значение: ${meaning}`, 'wave-review-copy');
    appendParagraph(
      item,
      localizedText(card.instructionByLocale, state.locale),
      'wave-review-copy',
    );
    appendParagraph(
      item,
      `Подсказка: ${localizedText(card.hintByLocale, state.locale)}`,
      'wave-review-copy',
    );
    appendParagraph(
      item,
      `После ошибки: ${localizedText(card.errorExplanationByLocale, state.locale)}`,
      'wave-review-copy',
    );
...
async function reviewStage(status) {
  ...
  if (reason.length < 5) {
    setStatus('Добавьте короткий комментарий к решению.', 'error');
    return;
  }
  setBusy(true);
  setStatus(
    status === 'approved'
      ? 'Сохраняем ваше одобрение…'
      : 'Возвращаем этап на исправление…',
  );
  try {
    await callable('adminReviewContentStage')({
      stageId: String(state.previewStage.id),
      status,
      reason,
      expectedReviewFingerprint: String(state.preview.reviewFingerprint),
    });
    closeReview(true);
    setStatus(
      status === 'approved'
        ? 'Этап одобрен. Следующий этап теперь доступен.'
        : 'Этап возвращён на исправление.',
      'success',
    );
  } catch (error) {
...
async function reviewWave(status) {
  ...
  const reason = String(byId('review-reason').value || '').trim();
  if (reason.length < 5) {
    setStatus('Добавьте понятный комментарий к решению.', 'error');
    return;
  }
  ...
  setStatus(
    status === 'approved'
      ? `Сохраняем одобрение ${requiredWave}…`
      : `Останавливаем ${requiredWave} и сохраняем возврат…`,
  );
  try {
    const callableName =
      status === 'approved'
    ...
    await callable(callableName)({
      ...
      expectedCheckpointFingerprint: String(
        state.preview.checkpointFingerprint || '',
      ),
      reason,
    });
    closeReview(true);
    setStatus(
      status === 'approved'
        ...
        : `${requiredWave} возвращено. Текущая версия сохранена для аудита; следующая будет отдельной.`,
      'success',
    );
  } catch (error) {
    setStatus(`Решение не сохранено: ${error?.message || error}`, 'error');
...
async function primaryAction() {
  try {
    ...
    if (
      ...
      return;
    if (['queued', 'failed', 'paused'].includes(stage.state))
      return await runStage(stage.id);
...
function showVisualMode() {
  state.stages = [
    ...
    {
      id: 'visual-localized-course',
      kind: 'learning_v2_localized_course',
      state: 'paused',
      revision: 1,
      learningV2PauseReason: 'owner_wave_approval_required',
      learningV2RequiredWaveApproval: 'e1',
    },
  ];
  ...
  renderWorkflow();
  setStatus(
    'Локальный макет: серверные данные не загружаются и не меняются.',
    'success',
  );
}
...
async function resolveFirebaseConfig() {
  ...
  if (injected && typeof injected === 'object')
    return applyMobileHostingAuthDomain(injected);
  ...
  if (!response.ok)
    throw new Error(
      'Конфигурация Firebase недоступна. Откройте страницу через Firebase Hosting.',
    );
...
function applyMobileHostingAuthDomain(config) {
  if (!config || typeof config !== 'object')
    throw new Error('Конфигурация Firebase повреждена.');
  ...
  const hostname = String(location.hostname || '')
    ...
    .toLowerCase()
    .replace(/\.$/, '');
  if (!isMobile || !projectId || hostname !== `${projectId}.web.app`)
    return config;
  ...
  try {
    ...
    byId('sign-in-button').addEventListener('click', async () => {
      ...
      try {
        ...
        byId('auth-error').textContent =
          `Вход не выполнен: ${error?.message || error}`;
    ...
    onAuthStateChanged(state.auth, async (user) => {
      ...
      if (token.claims?.admin !== true) {
        ...
        byId('auth-error').textContent =
          'У этого Google-аккаунта нет прав администратора.';
        byId('auth-error').hidden = false;
        return;
      }
...
byId('wave-previous-episode').addEventListener('click', () =>
  changeWavePreview(Number(state.preview?.episodeOrdinal || 1) - 1, 1),
);
byId('wave-next-episode').addEventListener('click', () =>
  changeWavePreview(Number(state.preview?.episodeOrdinal || 1) + 1, 1),
);
byId('wave-session-select').addEventListener('change', () =>
  changeWavePreview(
    Number(state.preview?.episodeOrdinal || 1),
    Number(byId('wave-session-select').value || 1),
  ),
);
document
...
document.addEventListener('keydown', (event) => {
  if (event.key === 'Escape' && !byId('review-layer').hidden) closeReview();
});
byId('course-target').addEventListener('change', () => {
  const language = byId('course-target').value;
  const requestInput = byId('course-request');
  if (/^learning-v2-[a-z-]+-v1$/.test(requestInput.value))
    requestInput.value = `learning-v2-${language}-v1`;
  state.stages = [];
  renderWorkflow();
  void refreshStages();
});
