...
Граница ответственности по решению владельца от 2026-08-12: Codex обязан
довести генератор до возможности полностью и воспроизводимо собрать любой
эпизод E1–E32, показать его в preview и выдать точные причины блокировки. Codex
не обязан заранее авторить или утверждать реальный контент всех 32 эпизодов.
Владелец вручную запускает генерацию, просматривает и отдельно утверждает каждый
реальный эпизод. Вручную владелец делает только выбор эпизода, запуск, проверку
preview и решение об утверждении: 12 сессий, 144 задания, восемь локалей,
аудиоварианты и итоговый пакет обязан собрать генератор. Автоматические fixtures
и smoke-пакеты доказывают возможности генератора, но не считаются owner approval
или готовыми E1–E32.
...
После готовности генератора владелец вручную генерирует и проверяет эпизоды
волнами, а не слепым bulk batch: E1–E8 и chapter checkpoint, затем E9–E32.
