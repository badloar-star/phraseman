...
test('premium session route uses the real twelve-card compiler without the rejected lab player', () => {
  ...
  expect(map).toContain('InteractionManager.runAfterInteractions(warmLesson1SessionRuntime)');
  expect(runtime).not.toMatch(/@react-native-firebase|httpsCallable|fetch\(|XMLHttpRequest/);
  ...
  expect(runtime).toContain('STAR_QUALITY_COPY');
  expect(runtime).toContain('playWrongAnswerMotion();');
  expect(runtime).toContain('Math.max(520, windowHeight - insets.top - 210)');
  expect(runtime).toContain('sessionStarTier(completionStars)');
  expect(runtime).toContain('ceremonyProgress.value = withTiming(target');
  expect(runtime).toContain('Собрано ${displayedStars} из 36 звёзд в этой сессии');
  expect(runtime).toContain('Можно улучшить результат ещё на');
  expect(runtime).toContain('Результат сохранён на устройстве и синхронизируется автоматически.');
  expect(runtime).toContain('На карту');
  ...
  expect(runtime).toContain('Проверка интро · вопрос ${cardIndex + 1} из 3');
  expect(runtime).toContain('createLesson1LocalProgressStore(AsyncStorage');
  ...
  expect(map).toContain('createRequiredSessionLocalCommitCoordinator(AsyncStorage, guard, SESSION_IDS).recover(scope)');
  expect(map).toContain('const state = await withAccountTransitionLock(async () =>');
  ...
  expect(map).toContain('releaseLearningV2SessionNetworkIntentAfterResultFrame(resultTarget)');
  expect(map).toContain("setLocalRecoveryOutcome('failure')");
  expect(map).toContain('secondFrame = requestAnimationFrame(() =>');
  expect(map).toContain('releaseLearningV2SessionNetworkIntentAfterExitFrame(exitTarget)');
  expect(map.indexOf('prepareLearningV2SessionNetworkIntent(node.id)'))
    .toBeLessThan(map.indexOf('router.push({ pathname:'));
  ...
  expect(runtime).toContain('This wait is evidence/fencing only. It never gates local lesson controls.');
...
test('background completion transport stays outside the active session route', () => {
  ...
  expect(sync).toContain('rebindRequiredSessionCompletionEnvelope(item.payload');
...
test('the twelve-card session is fully local and never waits on a server per answer', () => {
  ...
  expect(chooseBody).toContain('normalize(answer) === normalize(correctAnswer)');
  expect(chooseBody).toContain("setResult('correct')");
  expect(chooseBody).not.toMatch(/await|fetch\(|httpsCallable|firebase|AsyncStorage/i);
});
...
test('the real session exposes an accessible zero-star skip without transport work', () => {
  ...
  expect(runtime).toContain('accessibilityLabel="Пропустить это задание"');
  expect(runtime).toContain('Задание получит ноль звёзд, сессия продолжится');
  expect(skipBody).toContain("disposition: 'skipped'");
  expect(skipBody).toContain('learnerAttempts: Math.max(0, attempts - 1)');
  expect(skipBody).not.toMatch(/await|fetch\(|httpsCallable|firebase|AsyncStorage/i);
});
...
test('Lesson 1 runtime is a bounded frozen cache shared by map warm-up and session mounts', () => {
  ...
  expect(first.compiled.sessions.every((session) => session.cards.length === 12)).toBe(true);
...
test('real Lesson 1 exposes all seven approved required-mode families with pinned activities', () => {
  ...
  expect(new Set(cards.map((card) => card.family))).toEqual(new Set([
    'phrase_builder',
    'listen_choose',
    'sound_contrast',
  ...
  expect(cards.every((card) => card.activityId.startsWith(`lesson1-${card.family}-`))).toBe(true);
});
