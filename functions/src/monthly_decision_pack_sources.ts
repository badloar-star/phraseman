...
import {
  loadProductAnalyticsAggregateRows,
...
interface ProjectionInput {
  ...
  productMonthRows: ProductAnalyticsQueryRow[];
  productBaselineRows: ProductAnalyticsQueryRow[];
  productExportPending: boolean;
  productUnavailableReason?: string;
  revenueMonth: JsonRecord;
  revenueBaseline: JsonRecord;
  revenueTruncated: boolean;
  revenueRows: number;
}

function payloads(rows: ProductAnalyticsQueryRow[], kind: string): JsonRecord[] {
  return rows.filter((row) => row.row_kind === kind).flatMap((row) => {
    try { return row.payload ? [JSON.parse(row.payload) as JsonRecord] : []; } catch { return []; }
  });
}

function first(rows: ProductAnalyticsQueryRow[], kind: string): JsonRecord {
...
function addExecutiveRows(target: JsonRecord[], scope: string, quality: JsonRecord): void {
  for (const metricId of ['consented_app_instances', 'sessions', 'screen_views']) {
    const value = finite(quality[metricId]);
    if (value != null) target.push({ scope, metric_id: metricId, value, denominator: value, status: 'available' });
  }
}

function addProductRows(sections: NonNullable<DecisionPackInput['sections']>, scope: string, rows: ProductAnalyticsQueryRow[]): void {
  ...
  for (const [stage, key] of [
    ...
    if (count != null) sections.acquisition_activation!.push({ scope, channel: 'all_consent_observed', stage, app_instances: count, rate: firstTouch && firstTouch > 0 ? count / firstTouch : null, denominator: firstTouch, status: 'available' });
  }

  for (const row of payloads(rows, 'true_retention_day')) {
    for (const day of [1, 7, 14, 30]) {
      const eligible = finite(row[`eligible_d${day}`]);
      if (eligible != null && eligible > 0) sections.retention_cohorts!.push({
        scope,
  ...
  for (const [metricId, key] of [
    ...
    if (value != null) sections.data_quality!.push({ scope, source: 'firebase_analytics', metric_id: metricId, value, denominator: finite(quality.consented_app_instances), status: 'available' });
  }
}

function addRevenueRows(target: JsonRecord[], scope: string, analytics: JsonRecord): void {
  ...
  for (const [metricId, key] of [
    ...
    if (value != null) target.push({ scope, metric_id: metricId, unit: 'usd_micros', currency: 'USD', value: null, value_micros: value, chains, denominator: chains, coverage: JSON.stringify(coverage), status });
  }
  const trial = record(analytics.trialToPaid);
  if (finite(trial.eligibleTrialChains) != null) target.push({ scope, metric_id: 'trial_to_paid_rate', unit: 'ratio', currency: '', value: finite(trial.rate), value_micros: null, chains: finite(trial.convertedTrialChains), denominator: finite(trial.eligibleTrialChains), coverage: JSON.stringify(coverage), status });
...
export function buildDecisionPackAggregateInput(input: ProjectionInput): DecisionPackInput {
  const sections: NonNullable<DecisionPackInput['sections']> = {
    metric_dictionary: [
  ...
  return {
    ...
    sources: [
      ...
      { id: 'revenuecat_webhooks', status: input.revenueTruncated ? 'truncated_not_decision_grade' : 'available', dataThroughMs: revenueWatermark ?? undefined, reason: revenueWatermark == null ? 'watermark_unavailable_no_observed_events' : undefined, analysisCutoffMs: input.window.endExclusiveMs - 1, queryAsOfMs: input.generatedAtMs, rowCount: input.revenueRows, rowCap: DECISION_PACK_REVENUECAT_ROW_CAP },
...
async function loadRevenueRows(fromMs: number): Promise<{ rows: ServerRevenueRow[]; truncated: boolean }> {
  const rows: ServerRevenueRow[] = [];
  let cursor: FirebaseFirestore.QueryDocumentSnapshot | null = null;
  while (rows.length < DECISION_PACK_REVENUECAT_ROW_CAP) {
    let query: FirebaseFirestore.Query = admin.firestore().collection('revenuecat_premium_events')
      .where('createdAt', '>=', admin.firestore.Timestamp.fromMillis(fromMs))
      .orderBy('createdAt', 'desc')
      .limit(Math.min(REVENUECAT_PAGE_SIZE, DECISION_PACK_REVENUECAT_ROW_CAP - rows.length));
    ...
    if (snapshot.size < REVENUECAT_PAGE_SIZE) break;
  }
  return { rows, truncated: rows.length >= DECISION_PACK_REVENUECAT_ROW_CAP };
}
...
export async function loadDecisionPackAggregateInput(window: MonthlyReportingWindow, generatedAtMs: number): Promise<DecisionPackInput> {
  ...
  const projected = buildDecisionPackAggregateInput({
    ...
    revenueMonth: aggregateServerRevenueAnalytics(monthRows, { fromMs: window.startMs, truncated: revenue.truncated }) as unknown as JsonRecord,
    revenueBaseline: aggregateServerRevenueAnalytics(baselineRows, { fromMs: window.baselineStartMs, truncated: revenue.truncated }) as unknown as JsonRecord,
