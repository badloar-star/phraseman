import { horizonPalette, type HorizonPalette } from "./model";
function landmark(index: number) {
  const monuments = [
...
function landscape(index: number, tall = false) {
  const id = `terrain-${index}-${tall}`;
...
function resolveColors(xml: string, p: HorizonPalette): string {
  return xml
    .replace(/var\(--([a-z0-9]+)\)/g, (_, key: keyof HorizonPalette) => p[key])
    .replace(/ class="[^"]*"/g, "")
    .replace(/ aria-hidden="true"/g, "");
}
export function horizonLandscapeXml(lesson: number, tall = false, palette = horizonPalette(lesson)): string {
  return resolveColors(
    landscape((lesson - 1) % 7, tall),
    palette,
  );
}
export function horizonPortalXml(lesson: number, p = horizonPalette(lesson)): string {
  const island = horizonLandscapeXml(lesson, false, p)
    .replace(/<rect width="390"[^>]*\/>/, "")
    .replace("<svg ", '<svg x="-60" y="82" width="380" height="240" ');
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 280 330"><defs><linearGradient id="rim" x2="1" y2="1"><stop stop-color="${p.accent}"/><stop offset=".45" stop-color="${p.terrain}"/><stop offset="1" stop-color="${p.accent}"/></linearGradient><linearGradient id="sky" x2="0" y2="1"><stop stop-color="${p.terrain3}"/><stop offset="1" stop-color="${p.bg}"/></linearGradient><clipPath id="arch"><path d="M37 278V126a103 103 0 0 1 206 0v152Z"/></clipPath></defs><ellipse cx="140" cy="309" rx="132" ry="13" fill="${p.surface2}"/><ellipse cx="140" cy="302" rx="123" ry="12" fill="${p.terrain3}"/><ellipse cx="140" cy="293" rx="109" ry="11" fill="${p.terrain}"/><path d="M41 283V126a106 106 0 0 1 212 0v157Z" fill="${p.surface2}" stroke="${p.edge}" stroke-width="3"/><path d="M30 279V126a110 110 0 0 1 220 0v153Z" fill="url(#rim)"/><g clip-path="url(#arch)"><path d="M37 278V126a103 103 0 0 1 206 0v152Z" fill="url(#sky)"/><circle cx="140" cy="92" r="39" fill="${p.accent}" opacity=".85"/><path d="m0 240 85-104 43 62 59-46 93 94v60H0Z" fill="${p.terrain3}" opacity=".45"/>${island}</g><path d="M34 279V126a106 106 0 0 1 212 0v153Z" fill="none" stroke="${p.accent}" stroke-opacity=".35"/><circle cx="140" cy="287" r="20" fill="${p.surface}" stroke="${p.edge}"/><text x="140" y="292" text-anchor="middle" fill="${p.accent}" font-size="12" font-family="sans-serif" letter-spacing="2">${String(lesson).padStart(2, "0")}</text></svg>`;
}
