...
class MemoryRepository {
    ...
    async query(input) {
        return [...this.documents.entries()]
            ...
            .sort((a, b) => Number(b.data[input.orderBy]) - Number(a.data[input.orderBy])
...
describe('Admin Plans ledger', () => {
    test('creates an owner-only durable draft with audit-safe server metadata', async () => {
        ...
        const ledger = new ledger_1.AdminPlansLedger(repository, () => NOW_MS);
    ...
    test('gets and lists only safe owner projections in newest-first order', async () => {
        ...
        const newerLedger = new ledger_1.AdminPlansLedger(repository, () => NOW_MS + 1);
    ...
    test('validates read inputs and returns not-found for an absent plan', async () => {
        const ledger = new ledger_1.AdminPlansLedger(new MemoryRepository(), () => NOW_MS);
