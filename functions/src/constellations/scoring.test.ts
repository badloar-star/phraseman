...
import {
  computeFinalScores,
  rankPlayers,
  roundConstellationBonus,
  starPointsFor,
} from './scoring';

const SCORING = CONSTELLATION_DEFAULTS.scoring;

describe('constellations/scoring — очки за звёзды', () => {
  test('очки по кольцам: внешнее 10, среднее 20, внутреннее 30, Полярная 50', () => {
    expect(starPointsFor(['3,0'], SCORING.starPoints)).toBe(10); // внешнее
    expect(starPointsFor(['2,0'], SCORING.starPoints)).toBe(20); // среднее
    expect(starPointsFor(['1,0'], SCORING.starPoints)).toBe(30); // внутреннее
    expect(starPointsFor(['0,0'], SCORING.starPoints)).toBe(50); // Полярная
    expect(starPointsFor(['3,0', '0,0'], SCORING.starPoints)).toBe(60);
    expect(starPointsFor([], SCORING.starPoints)).toBe(0);
  });
});
...
describe('constellations/scoring — бонус смежности (созвездия)', () => {
  ...
  test('меньше 3 соединённых звёзд — бонуса нет', () => {
    expect(roundConstellationBonus([], opts)).toBe(0);
    expect(roundConstellationBonus(['0,0'], opts)).toBe(0);
...
describe('constellations/scoring — финальный счёт и места', () => {
  ...
  test('полное равенство разруливается детерминированно (по uid), не порядком вставки', () => {
    ...
    const a = rankPlayers([{ uid: 'zz', ...base }, { uid: 'aa', ...base }]);
    const b = rankPlayers([{ uid: 'aa', ...base }, { uid: 'zz', ...base }]);
