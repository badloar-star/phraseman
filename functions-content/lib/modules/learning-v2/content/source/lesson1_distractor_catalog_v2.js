...
exports.lesson1DistractorChoicesV2 = lesson1DistractorChoicesV2;
exports.upgradeLesson1SessionDistractorsV2 = upgradeLesson1SessionDistractorsV2;
const LOCALES = [
...
const FORM_NEIGHBORS = Object.freeze({
...
function groupFor(correct) {
    const token = key(correct);
    if (/^(?:a|an|the)$/u.test(token))
        return GROUPS.article;
    if (/^(?:am|is|are|do|does|isn't|aren't|not|i'm|you're|he's|she's|it's|we're|they're)$/u.test(token))
        return GROUPS.copula;
    if (/^(?:i|you|he|she|it|we|they|me)$/u.test(token))
        return GROUPS.subject;
    if (/^(?:my|your|his|her|mine|yours|hers|whose)$/u.test(token))
        return GROUPS.possessive;
    if (/^(?:how|what|where|who)$/u.test(token))
        return GROUPS.question;
    if (/^(?:at|in|on|to)$/u.test(token))
        return GROUPS.preposition;
    if (/^(?:and|but|too|all|alone|together)$/u.test(token))
        return GROUPS.connector;
    if (GROUPS.plural.includes(token))
        return GROUPS.plural;
    if (GROUPS.family.includes(token))
        return GROUPS.family;
    if (GROUPS.profession.includes(token))
        return GROUPS.profession;
    if (GROUPS.number.includes(token))
        return GROUPS.number;
    if (GROUPS.formula.includes(token))
        return GROUPS.formula;
    if (GROUPS.place.includes(token))
        return GROUPS.place;
    if (GROUPS.object.includes(token))
        return GROUPS.object;
    if (GROUPS.emotion.includes(token))
        return GROUPS.emotion;
    if (GROUPS.physical.includes(token))
        return GROUPS.physical;
    if (GROUPS.readiness.includes(token))
        return GROUPS.readiness;
    if (GROUPS.weather.includes(token))
        return GROUPS.weather;
    if (GROUPS.quality.includes(token))
        return GROUPS.quality;
    if (GROUPS.timing.includes(token))
        return GROUPS.timing;
    if (/^(?:this|that)$/u.test(token))
        return GROUPS.article;
    throw new Error(`lesson1_distractor_catalog_missing:${correct}`);
}
function distractorValues(correct, sessionOrdinal) {
    ...
    if (editorialNeighbors)
        return editorialNeighbors.map(display);
    const group = [...new Set(groupFor(correct).map(display))];
    const correctIndex = group.findIndex((value) => key(value) === normalizedCorrect);
    if (correctIndex < 0)
        throw new Error(`lesson1_distractor_catalog_target_not_in_group:${correct}`);
    const candidates = group
        .slice(correctIndex + 1)
        .concat(group.slice(0, correctIndex));
    const preferred = candidates.filter((value) => PHONETIC_PAIRS.has(`${normalizedCorrect}|${key(value)}`) ||
    ...
    if (result.length !== 5)
        throw new Error(`lesson1_distractor_catalog_too_small:${correct}`);
    return result;
}
function trapType(correct, alternative) {
    const pair = `${key(correct)}|${key(alternative)}`;
    if (PHONETIC_PAIRS.has(pair))
        return 'phonetic';
    if (ORTHOGRAPHIC_PAIRS.has(pair))
        return 'orthographic';
    if (NUMBER_AGREEMENT_PAIRS.has(pair))
        return 'grammar';
    if (/^(?:sorry|pardon|excuse|please|thank|thanks|welcome|nice|meet|see|take|care|good|morning|afternoon|evening|night|later)$/iu.test(correct))
        return 'collocation_pragmatics';
    if (/^(?:and|but|too|not)$/iu.test(correct))
        return 'phrase_assembly';
    if (/^(?:i|me|a|an|the|at|in|on|to)$/iu.test(correct))
        return 'l1_transfer';
    if (/^(?:am|is|are|do|does|isn't|aren't|i'm|you're|he's|she's|it's|we're|they're|you|he|she|it|we|they|my|your|his|her|mine|yours|hers|whose|how|what|where|who|this|that)$/iu.test(key(correct)))
        return 'grammar';
    return 'semantic_neighbor';
}
function semanticFamily(correct) {
    const token = key(correct);
    if (GROUPS.emotion.includes(token))
        return 'emotion';
    if (GROUPS.family.includes(token))
...
const SEMANTIC_FAMILY_FOCUS = Object.freeze({
    ru: {
...
function semanticNeighborReason(locale, wrong, correct, phrase) {
...
const PAIR_REASON = Object.freeze({
    ru: {
...
function pairSpecificReason(locale, wrong, correct, phrase) {
    const pair = `${key(correct)}|${key(wrong)}`;
    return PAIR_REASON[locale][pair]?.(phrase);
}
const LOCALIZED_DISTRACTOR_CACHE = new Map();
function createWordDistractors(locale, correct, phrase, sessionOrdinal) {
    return Object.freeze(distractorValues(correct, sessionOrdinal).map((value) => {
        const kind = trapType(correct, value);
        const reason = pairSpecificReason(locale, value, correct, phrase) ??
...
function buildWordDistractors(locale, correct, phrase, sessionOrdinal) {
 ...
 */
function lesson1DistractorChoicesV2(locale, correct, phrase, sessionOrdinal) {
    if (locale === 'en')
        return [];
    return buildWordDistractors(locale, correct, phrase, sessionOrdinal);
}
function upgradePhraseInPlace(phrase, sessionOrdinal) {
    ...
    const localizedDetails = phrase.localizedDetails
        ...
        ? Object.fromEntries(LOCALES.flatMap((locale) => {
            ...
            return [[locale, upgraded]];
        }))
        : phrase.localizedDetails;
    const mutable = phrase;
    mutable.words = words;
 ...
 */
function upgradeLesson1SessionDistractorsV2(source) {
    if (source.distractorAuthorship === 'manual')
        return source;
    source.phrases.forEach((phrase) => upgradePhraseInPlace(phrase, source.requiredSessionOrdinal));
    return source;
}
//# sourceMappingURL=lesson1_distractor_catalog_v2.js.map