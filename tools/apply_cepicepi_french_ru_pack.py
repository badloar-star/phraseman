...
ROLE_CONFIG = {
    "fr1": {
        "voice": "coral",
        "instructions": "Speak clear, pleasant standard French for A1 learners. Warm natural teacher voice. Say only the phrase.",
    },
    "ru": {
        "voice": "shimmer",
    ...
    "fr2": {
        "voice": "nova",
        "instructions": "Speak clear standard French with a second pleasant voice, calm and natural for A1 learners. Say only the phrase.",
    },
}
...
def french_hint(text: str) -> str:
    value = normalize_spaces(text).rstrip(".!?").lower()
    replacements = [
        ("j'ai", "zhe"),
        ("ch", "sh"),
        ("eau", "o"),
...
def openai_tts(api_key: str, text: str, role: str, out_path: Path) -> None:
    ...
    payload = json.dumps(
        {"model": OPENAI_MODEL, "voice": cfg["voice"], "input": text, "instructions": cfg["instructions"], "response_format": "wav"},
        ensure_ascii=False,
    ).encode("utf-8")
...
def build_rows() -> list[dict[str, Any]]:
    ...
    for row in rows:
        row["pronunciation"] = french_hint(row["french"])
    (PACK / "french_chains_100_with_pronunciation.json").write_text(json.dumps(rows, ensure_ascii=False, indent=2), encoding="utf-8")
    return rows
...
def generate_audio(content: dict[str, Any], rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for idx, row in enumerate(rows):
        jobs.extend(
            [
                {"part": 1, "role": "ru", "text": row["russian"], "track": 18, "seg": idx},
                {"part": 1, "role": "fr1", "text": row["french"], "track": 19, "seg": idx},
                {"part": 1, "role": "fr2", "text": row["french"], "track": 20, "seg": idx},
                {"part": 2, "role": "ru", "text": row["russian"], "track": 18, "seg": 100 + idx},
                {"part": 2, "role": "fr1", "text": row["french"], "track": 19, "seg": 100 + idx},
                {"part": 2, "role": "fr2", "text": row["french"], "track": 20, "seg": 100 + idx},
                {"part": 3, "role": "fr1", "text": row["french"], "track": 19, "seg": 200 + idx},
                {"part": 3, "role": "fr2", "text": row["french"], "track": 20, "seg": 200 + idx},
            ]
        )
    generated_raw = 0
    fitted = []
...
def apply() -> dict[str, Any]:
    ...
    for idx, row in enumerate(rows):
        ...
        set_text(texts[content["tracks"][4]["segments"][idx]["material_id"]], wrap_text(row["pronunciation"], 52))
        set_text(texts[content["tracks"][5]["segments"][idx]["material_id"]], wrap_text(row["french"], 40))
        set_text(texts[content["tracks"][3]["segments"][100 + idx]["material_id"]], wrap_text(row["french"], 40))
        set_text(texts[content["tracks"][4]["segments"][100 + idx]["material_id"]], wrap_text(row["pronunciation"], 52))
        set_text(texts[content["tracks"][5]["segments"][100 + idx]["material_id"]], wrap_text(row["russian"], 34))
        set_text(texts[content["tracks"][4]["segments"][200 + idx]["material_id"]], wrap_text(row["pronunciation"], 52))
        set_text(texts[content["tracks"][5]["segments"][200 + idx]["material_id"]], wrap_text(row["french"], 40))
        text_changes += 8
    ...
    for idx, _row in enumerate(rows):
        for role, track, seg_idx in [
            ("ru", 18, idx), ("fr1", 19, idx), ("fr2", 20, idx),
            ("ru", 18, 100 + idx), ("fr1", 19, 100 + idx), ("fr2", 20, 100 + idx),
            ("fr1", 19, 200 + idx), ("fr2", 20, 200 + idx),
        ]:
            item = audio_items[(track, seg_idx, role)]
            local = localize(project, Path(item["path"]), "cepicepi_french_openai_audio")
    ...
    for part in range(3):
        for idx, row in enumerate(rows):
            ...
            local = localize(project, source, "cepicepi_french_phrase_backgrounds")
            mat["path"] = str(local)
            mat["media_path"] = str(local)
            mat["duration"] = int((seg.get("target_timerange") or {}).get("duration", 0))
            ...
            if isinstance(seg.get("clip"), dict):
                seg["clip"]["alpha"] = 1
            seg["common_keyframes"] = [kf for kf in seg.get("common_keyframes", []) if kf.get("property_type") != "KFTypeAlpha"]
            bg_changes += 1
...
def qa() -> dict[str, Any]:
    ...
    bg_next = [m for m in videos.values() if "cepicepi_french_phrase_backgrounds" in str(m.get("path") or m.get("media_path") or "")]
    audio_next = [m for m in audios.values() if "cepicepi_french_openai_audio" in str(m.get("path") or m.get("media_path") or "")]
    if len(bg_next) < 300:
        errors.append(f"French background material count {len(bg_next)} < 300")
    if len(audio_next) < 800:
        errors.append(f"French audio material count {len(audio_next)} < 800")
    report = {
        ...
        "media": {"missing_count": len(missing), "french_background_materials": len(bg_next), "french_audio_materials": len(audio_next)},
        "text": {"line_violations": len(line_bad), "line_violations_sample": line_bad[:10], "bad_encoded_text": len(bad_text)},
    }
