...
describe('personal plan fill progress day 4 recall load', () => {
  it('tracks Day 4 chat drafts for every plan as a recall-heavy progression day', () => {
    ...
    expect(day4Rows).toEqual(expect.arrayContaining([
      expect.objectContaining({ label: 'Mitap Day 4', status: 'chat draft' }),
      expect.objectContaining({ label: 'Voyazh Day 4', status: 'chat draft' }),
      expect.objectContaining({ label: 'Gavan Day 4', status: 'chat draft' }),
      expect.objectContaining({ label: 'Impuls Day 4', status: 'chat draft' }),
      expect.objectContaining({ label: 'Echo Day 4', status: 'chat draft' }),
    ]));
    expect(day4Rows.every((row: { whatExists: string }) => row.whatExists.toLowerCase().includes('recall'))).toBe(true);
    expect(day4Rows.every((row: { notes: string }) => row.notes.toLowerCase().includes('review'))).toBe(true);
  });
});
