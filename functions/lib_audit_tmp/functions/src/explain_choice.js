...
exports.explainChoice = (0, https_1.onCall)({
    ...
    if (!isPremium) {
        try {
            await (0, explain_budget_1.enforceFreeJobGenLimit)('choice', authUid, stableUid, FREE_DAILY_CAP);
        }
        catch (err) {
    ...
    const choiceHash = (0, choice_explain_cache_1.choiceHashFor)(correctEn, distractors, langKey, studyTarget);
    ...
    try {
        gen = await (0, explain_provider_1.openAiChat)({
            apiKey,
            model: jobCfg.model,
            messages: [{ role: 'user', content: (0, choice_explain_prompts_1.buildChoicePrompt)(correctEn, phraseMeaning, distractors, lang, studyTarget) }],
