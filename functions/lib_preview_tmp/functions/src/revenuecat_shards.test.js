...
describe('RevenueCat webhook TRANSFER (anonymous → stable id)', () => {
    ...
    function makeRcDbStub(initialUsers = {}) {
        ...
        const snap = (coll, id, data) => ({ id, ref: docApi(coll, id), exists: !!data, data: () => data });
        ...
        const queryApi = (coll, field, op, value, max = Infinity) => ({
            limit: (nextMax) => queryApi(coll, field, op, value, nextMax),
            get: async () => ({
        ...
        const db = {
            collection: (coll) => ({
                doc: (id) => docApi(coll, id),
                where: (field, op, value) => queryApi(coll, field, op, value),
            }),
            runTransaction: async (fn) => {
                const tx = {
                    ...
                    delete: async (ref) => { delete store[ref.collection][ref.id]; },
                };
                return fn(tx);
            },
        };
    ...
    it('is idempotent — a re-delivered TRANSFER does not move twice', async () => {
        ...
        admin.firestore.FieldValue = realFieldValue ?? { serverTimestamp: () => 'ts' };
        try {
            const event = { id: 'evt_dup', type: 'TRANSFER', event_timestamp_ms: 2000001, transferred_from: ['$RCAnonymousID:bbb'], transferred_to: ['stable-2'] };
            const res = makeRes();
            await revenuecat_shards_1.__revenueCatWebhookTestHooks.handleTransferEvent(event, 'TRANSFER', res);
