...
function parseAndValidate(raw, packet) {
    ...
    if (packet.kind === 'challenge_topic' && packet.promptVersion === 'v2')
        errors.push(...(0, question_artifacts_1.validateTopicArtifact)(artifact, { kind: packet.kind, cefr: packet.context.cefr }));
    if (packet.kind === 'challenge_questions' && packet.promptVersion === 'v2')
        errors.push(...(0, question_artifacts_1.validateQuestionBatchArtifact)(artifact, { kind: packet.kind, count: packet.context.count, grounding: packet.grounding }));
    if (packet.kind === 'challenge_question_replacement' && packet.promptVersion === 'v2')
        errors.push(...(0, question_artifacts_1.validateQuestionReplacementArtifact)(artifact, { kind: packet.kind, grounding: packet.grounding }));
