...
const words = ["visible", "hidden", "clear"] as const;
const examples = ["It is not visible.", "It is not hidden.", "It is not clear."] as const;
const families = [
...
const packet = LEARNING_V2_ENGLISH_EXACT_SESSION_PACKETS_V2.find(
  (entry) => entry.sessionId === "lesson-02:session:04",
);
assert.ok(packet);
assert.equal(packet.absoluteSessionOrdinal, 60);
assert.equal(packet.role, "guided_application");
...
assert.equal(packet.learningDeltaRu, "Самостоятельно выбрать и построить форму в новом бытовом контексте.");
...
const assignment = LEARNING_V2_ENGLISH_SESSION_LEXICAL_ASSIGNMENTS_V2.find(
  (entry) => entry.absoluteSessionOrdinal === 60,
);
...
const sourcePath = resolve(process.cwd(), "modules/learning-v2/content/source/episode_02_session_04_v1.ts");
assert.equal(existsSync(sourcePath), true, "lesson2_session04_source_missing");
const source = (createRequire(import.meta.url)(sourcePath) as Readonly<{
  EPISODE_02_SESSION_04_SOURCE: SessionSource;
}>).EPISODE_02_SESSION_04_SOURCE;

assert.equal(source.episodeOrdinal, 2);
assert.equal(source.requiredSessionOrdinal, 4);
assert.equal(source.distractorAuthorship, "manual");
assert.equal(source.modeNativePlanId, "lesson2-session04-full-b1-exact-mode-native-v1");
...
assert.equal(finalPayload?.family === "scripted_repeat_compare" ? finalPayload.targetPhrase : null, "It's not hidden.");
...
for (const locale of locales) {
  const children = buildSessionChildBodiesFromShard(shard, locale, "lesson-02:session:04");
...
process.stdout.write("LEARNING V2 LESSON 2 SESSION 4 EXACT PACKET GATE: PASS\n");
