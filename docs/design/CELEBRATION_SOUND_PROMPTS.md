...
## Празднование ПЕРВОЙ ПОКУПКИ — «Печать» (2026-09-21)

Отдельный случай: **новый звук не генерируется**. Празднование первой покупки
(диалог за руны, урок за жемчужины) переиспользует уже существующий
`pm.celebration.promo_stamp` — ассет `cel_promo_stamp_v1.m4a`.

Причина простая: у варианта «Печать» одна кульминация — оттиск, а звук штампа
и по смыслу, и по форме (`shape: 'double'`, два удара) — ровно то, что нужно.
Заводить новый файл ради того же события значило бы плодить ассеты.

### Карта ударов

Снята не на глаз, а из констант компонента
`components/FirstPurchaseSealCelebration.tsx`:

| мс | событие на экране | звук |
|---|---|---|
| 120 | монета начинает падать сверху | тишина |
| 250 | — | **старт** `promo_stamp` (атака 410 мс) |
| 660 | **оттиск**: монета села, пошла волна | **пик звука** + `hapticSuccess` |
| 780 | заголовок и название поднимаются | хвост звука |
| 980 | кнопка приезжает последней | тишина |

### Почему звук стартует на 250, а не на 660

У `promo_stamp` в `modules/audio/sound_motion.ts` стоит `attackMs: 410` — пик
приходит через 410 мс после запуска. Запусти файл В момент оттиска, и грохот
опоздал бы на те же 410 мс: удар был бы виден раньше, чем слышен.

Поэтому в компоненте звук стартует за длину атаки до оттиска:
`IMPACT_MS − STAMP_SOUND_ATTACK_MS` = 660 − 410 = **250 мс**.

**Оба числа связаны по построению.** Меняешь `IMPACT_MS` — пересчитай задержку
звука, иначе пик уедет и празднование развалится на «картинку отдельно, звук
отдельно».

### Воспроизведение

Через `playCelebrationSceneSound` — тот же путь, что у празднования Plus:
общий канал `soundDirector` обрывает предыдущий звук, а сценовый плеер даёт
свой независимый и доигрывает до конца.

Проверить без покупки: **Дев-хаб → «Первая покупка»** (самая верхняя секция),
две кнопки — диалог за руны и урок за жемчужины.
