...
function renderLanguageTemplate(template, interfaceLang, studyTarget = 'en') {
...
function renderGlobalRules(cefr, interfaceLang, studyTarget = 'en') {
...
function buildScenarioSystemPrompt(cefr, data) {
    ...
    const block = SCENARIO_BLOCK
        ...
        .replace(/\{CEFR\}/g, cefr)
        .replace(/\{TARGET_LANG\}/g, (0, ai_language_contract_1.studyTargetName)(studyTarget));
 ...
 */
function gameBlock(data, cefr, interfaceLang, studyTarget = 'en') {
 ...
 */
function assertDialogReplyMatchesTarget(reply, studyTarget = 'en') {
 ...
 */
function cefrReinjection(cefr, studyTarget = 'en') {
    return `\n\nREMINDER (keep enforcing every turn): reply ONLY in ${(0, ai_language_contract_1.studyTargetName)(studyTarget)} (never switch to the learner's language). Stay at CEFR ${cefr}: short, warm, simple everyday words, at most one new word per turn, one question at the end. Do NOT drift to native-level complexity.`;
}
...
function buildCompanionSystemPrompt(cefr, memory, rawInterfaceLang, rawStudyTarget = 'en') {
    const interfaceLang = asInterfaceLang(rawInterfaceLang);
    const studyTarget = (0, ai_language_contract_1.resolveStudyTarget)(rawStudyTarget);
...
exports.premiumDialogSend = (0, https_1.onCall)({
    ...
    if (!isPremium && aiDialogGatedByPremium) {
        ...
        throw new https_1.HttpsError('permission-denied', 'dialog_plus_required');
    }
    ...
    const remaining = await enforceDailyQuota(authUid, stableUid, isPremium, isPremium ? dialogQuota.premiumDailyReplies : dialogQuota.freeDailyReplies);
...
exports.premiumDialogTranslate = (0, https_1.onCall)({
    ...
    const sourceStudyTarget = (0, ai_language_contract_1.resolveStudyTarget)(data.studyTarget);
    const db = admin.firestore();
    const authUid = request.auth.uid;
