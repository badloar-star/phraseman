...
exports.RUNTIME_CONFIG_DOC = exports.RUNTIME_CONFIG_COLLECTION = exports.CONSTELLATION_DEFAULTS = void 0;
exports.constellationConfigFromData = constellationConfigFromData;
exports.resolveConstellationConfig = resolveConstellationConfig;
exports.CONSTELLATION_DEFAULTS = {
    // ── Раунд и фазы (A3) ──────────────────────────────────────────────────
    roundsTotal: 10,
    choosePhaseSec: 12,
    answerPhaseSec: 38,
...
function mergeNumberArray(def, raw) {
    if (!Array.isArray(raw) || raw.length !== def.length)
        return [...def];
...
function mergeSection(defaults, raw) {
    ...
    for (const key of Object.keys(defaults)) {
        ...
        if (typeof def === 'number') {
            ...
            out[key] = mergeNumberArray(def, value);
        }
        else if (isPlainObject(def)) {
            out[key] = mergeSection(def, value);
        }
        else {
            out[key] = def;
        }
 ...
 */
function constellationConfigFromData(raw) {
    return mergeSection(exports.CONSTELLATION_DEFAULTS, raw);
}
exports.RUNTIME_CONFIG_COLLECTION = 'admin_runtime_config';
exports.RUNTIME_CONFIG_DOC = 'constellations';
/**
 ...
 */
async function resolveConstellationConfig(db) {
    try {
        const snap = await db.collection(exports.RUNTIME_CONFIG_COLLECTION).doc(exports.RUNTIME_CONFIG_DOC).get();
