...
import {
  ...
  GAVAN_WEEK1_DAY5_MATERIAL_CANDIDATE_PATH,
  validateGavanWeek1Day5MaterialCandidate,
  writeGavanWeek1Day5MaterialCandidate,
} from '../tools/personal_plan_gavan_week1_day5_material_candidate';

const GENERATED_AT = '2026-06-03T21:00:00.000Z';
...
describe('Gavan week 1 day 5 concrete material candidate', () => {
  beforeEach(() => {
    if (existsSync(GAVAN_WEEK1_DAY5_MATERIAL_CANDIDATE_PATH)) {
      rmSync(GAVAN_WEEK1_DAY5_MATERIAL_CANDIDATE_PATH, { force: true });
    }
  });

  afterAll(() => {
    writeGavanWeek1Day5MaterialCandidate(seed(), day4Export(), {
      generatedAt: GENERATED_AT,
      targetPath: GAVAN_WEEK1_DAY5_MATERIAL_CANDIDATE_PATH,
    });
  });

  it('builds a non-live day 5 material candidate from seed and day 4 export', () => {
  ...
  it('uses only the reset day 5 seed phrases', () => {
    const material = candidate();
  ...
  it('uses a different concrete rhythm from day 4 with honest listening placeholders', () => {
    const material = candidate();
    ...
    expect(material.listeningPlaceholders).toHaveLength(4);
    expect(material.listeningPlaceholders.every((item) => item.assetStatus === 'not_generated')).toBe(true);
    expect(material.listeningPlaceholders.every((item) => item.finalAudioReady === false)).toBe(true);
  });

  it('builds phrase tiles with exact target word counts and safe distractors', () => {
    const material = candidate();

    expect(material.phraseBuildItems).toHaveLength(4);
    material.phraseBuildItems.forEach((item) => {
  ...
  it('builds natural choice items with exact unique option counts', () => {
    const material = candidate();
  ...
  it('passes its own quality gate and catches corrupted material candidates', () => {
    ...
    expect(validateGavanWeek1Day5MaterialCandidate(material, seed(), day4Export())).toEqual({
      valid: true,
      issues: [],
    });
    ...
    const corrupted = {
      ...
      listeningPlaceholders: [
        {
          ...material.listeningPlaceholders[0],
      ...
      materialPhrases: [
        {
          ...
          english: 'My email is test@example.com.',
    ...
    const result = validateGavanWeek1Day5MaterialCandidate(corrupted as any, seed(), day4Export());
  ...
  it('writes deterministic material JSON only under temp or report roots', () => {
    const result = writeGavanWeek1Day5MaterialCandidate(seed(), day4Export(), {
      generatedAt: GENERATED_AT,
      targetPath: GAVAN_WEEK1_DAY5_MATERIAL_CANDIDATE_PATH,
    });
    ...
    expect(existsSync(GAVAN_WEEK1_DAY5_MATERIAL_CANDIDATE_PATH)).toBe(true);

    const parsed = JSON.parse(readFileSync(GAVAN_WEEK1_DAY5_MATERIAL_CANDIDATE_PATH, 'utf8'));
    expect(parsed.kind).toBe('gavan_week1_day5_material_candidate');
    expect(parsed.dayId).toBe('gavan-week1-day5');
  });

  it('rejects app tools tests and root config write targets', () => {
    const appResult = writeGavanWeek1Day5MaterialCandidate(seed(), day4Export(), {
      generatedAt: GENERATED_AT,
      targetPath: path.join(process.cwd(), 'app', 'gavan-week1-day5-material.json'),
    });
    const toolsResult = writeGavanWeek1Day5MaterialCandidate(seed(), day4Export(), {
      generatedAt: GENERATED_AT,
      targetPath: path.join(process.cwd(), 'tools', 'gavan-week1-day5-material.json'),
    });
    const testsResult = writeGavanWeek1Day5MaterialCandidate(seed(), day4Export(), {
      generatedAt: GENERATED_AT,
      targetPath: path.join(process.cwd(), 'tests', 'gavan-week1-day5-material.json'),
    });
  ...
  it('does not import or mutate live app routes catalog quiz UI storage audio scoring or navigation', () => {
    const source = readFileSync(
      path.join(process.cwd(), 'tools', 'personal_plan_gavan_week1_day5_material_candidate.ts'),
      'utf8',
    );
