...
describe("retired competitive mode full removal", () => {
  ...
  // зачем 2026-08-23 (владелец): турниры выключены ФУЛЛ и заархивированы в
  // _archive/tournaments. Раньше этот блок сторожил обратное — что удаление
  // отменённого режима «Арена-клуб» не заденет турниры как соседний продукт.
  // Правило отменено прямым распоряжением владельца, поэтому сторож переписан:
  // теперь он охраняет ГРАНИЦУ в другую сторону — живой код не должен тянуть
  // заархивированные турнирные экраны, иначе сборка молча потянет мёртвый код.
  test("keeps archived tournaments out of the live client", () => {
    for (const file of [
    ...
    // Серверный исходник турниров сохранён намеренно: с него живёт
    // tournamentClaimReward — забор УЖЕ начисленных наград.
    expect(exists("functions/src/tournaments.ts")).toBe(true);
    expect(read("functions/src/tournaments.ts")).toMatch(
      /export const tournamentClaimReward/,
    );
  });
