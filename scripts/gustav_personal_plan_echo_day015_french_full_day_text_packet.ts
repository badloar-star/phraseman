...
const DAY_INDEX = 15 as const;
const GENERATED_BY = 'gustav_personal_plan_echo_day015_french_full_day_text_packet' as const;

const EXPLANATIONS = [
  {
    phraseId: 'echo_d15_p1',
    title: 'Comment dire « j’ai lu » au passé',
    rule: 'Pour les verbes réguliers au passé, on ajoute -ed. Read est spécial : il s’écrit pareil au passé, mais se prononce autrement. I read = j’ai lu.',
    why: 'Le passé montre que l’action est déjà terminée : hier, la semaine dernière.',
    commonMistake: 'Ne dis pas « I readed » : read a une forme spéciale au passé, il ne change pas à l’écrit.',
  },
  {
    phraseId: 'echo_d15_p2',
    title: 'Comment poser une question au passé',
    rule: 'Question au passé : What did you + verbe ? Did est l’auxiliaire des questions au passé.',
    why: 'Sans did, la question sonne incorrecte. Did porte la marque du passé.',
    commonMistake: 'Ne dis pas « What you read? » : il faut did, donc « What did you read? ».',
  },
  {
    phraseId: 'echo_d15_p3',
    title: 'Expliquer le thème de l’article',
    rule: 'It was about + thème = cela parlait de... Was est le passé de is.',
    why: 'Was indique que l’article a déjà été lu. C’est une formule pratique pour n’importe quel sujet.',
    commonMistake: 'Ne dis pas « It is about » quand tu parles de quelque chose que tu as déjà lu : utilise was.',
  },
  {
    phraseId: 'echo_d15_p4',
    title: '« Mon », « ton », « son » : les mots de possession',
    rule: 'My = mon/ma/mes. Il va avant le nom : my favourite part = ma partie préférée.',
    why: 'My ne change pas de forme : toujours my, peu importe le genre ou le nombre. C’est pratique.',
    commonMistake: 'Ne dis pas « I my found » : my va seulement avant le nom, comme my part.',
  },
  {
    phraseId: 'echo_d15_p5',
    title: 'Question « où » au passé',
    rule: 'Where did you + verbe ? Where signifie où. Did est l’auxiliaire du passé.',
    why: 'Where demande le lieu. Avec did, la question passe au passé.',
    commonMistake: 'Ne dis pas « Where you found? » : il faut did, donc « Where did you find? ».',
  },
  {
    phraseId: 'echo_d15_p6',
    title: 'Comment dire « j’ai partagé »',
    rule: 'Shared = j’ai partagé. C’est un verbe régulier : share devient shared, on ajoute -d. My friend = mon ami.',
    why: 'Share + with = partager avec quelqu’un. C’est une combinaison très utile pour parler de choses intéressantes.',
    commonMistake: 'Ne dis pas « I shared article » : il faut « the article », l’article précis.',
  },
] as const;
...
const FIELD_SPECS: FieldSpec[] = [
  {
    fieldPath: 'topic',
    fieldKind: 'day_topic',
    frenchText: 'Raconter ce que tu as lu',
    source: (day) => day.topic,
  },
  {
    fieldPath: 'outcome',
    fieldKind: 'day_outcome',
    frenchText: 'Tu peux raconter à un ami ce que tu as lu hier : dire le sujet et partager ton impression.',
    source: (day) => day.outcome,
  },
  {
    fieldPath: 'intro[0].title',
    fieldKind: 'intro_title',
    frenchText: 'Comment raconter ce que tu as lu',
    source: (day) => day.intro?.[0]?.title,
  },
  {
    fieldPath: 'intro[0].body',
    fieldKind: 'intro_body',
    frenchText: 'Quand tu veux partager un article ou un livre, commence par dire ce que tu as lu et quand. Ensuite, dis de quoi cela parlait. Pour cela, tu utilises le passé, le même que tu as déjà appris.',
    source: (day) => day.intro?.[0]?.body,
  },
  {
    fieldPath: 'intro[0].examples[0].gloss',
    fieldKind: 'intro_example_gloss',
    frenchText: 'Hier, j’ai lu un article intéressant.',
  ...
  },
  {
    ...
    fieldKind: 'intro_title',
    frenchText: 'Dis de quoi parlait l’article',
    source: (day) => day.intro?.[1]?.title,
  },
  {
    fieldPath: 'intro[1].body',
    fieldKind: 'intro_body',
    frenchText: 'Pour expliquer le sujet, dis : « It was about... » — « Cela parlait de... ». Par exemple : It was about space. C’est une façon courte et pratique de résumer le thème.',
    source: (day) => day.intro?.[1]?.body,
  },
  {
    fieldPath: 'intro[1].examples[0].gloss',
    fieldKind: 'intro_example_gloss',
    frenchText: 'Cela parlait du changement climatique.',
  ...
  },
  {
    ...
    fieldKind: 'intro_example_gloss',
    frenchText: 'Cela parlait de la vie dans d’autres pays.',
  ...
  },
  {
    ...
    fieldKind: 'intro_title',
    frenchText: 'Demande à ton ami ce qu’il en pense',
    source: (day) => day.intro?.[2]?.title,
  },
  {
    fieldPath: 'intro[2].body',
    fieldKind: 'intro_body',
    frenchText: 'Après avoir parlé de l’article, tu peux demander : « What do you think? » — « Qu’en penses-tu ? ». Ou : « Did you read about this? » — « As-tu lu quelque chose à ce sujet ? ».',
    source: (day) => day.intro?.[2]?.body,
  },
  {
    fieldPath: 'intro[2].examples[0].gloss',
    fieldKind: 'intro_example_gloss',
    frenchText: 'Qu’en as-tu pensé ?',
  ...
  },
  {
    ...
    fieldKind: 'intro_example_gloss',
    frenchText: 'As-tu lu quelque chose sur ce sujet ?',
...
function rowIdFromPath(fieldPath: string): string {
  return `fr_echo_d015_${fieldPath.replace(/[^a-zA-Z0-9]+/g, '_').replace(/^_+|_+$/g, '').toLowerCase()}`;
}

function renderMarkdown(report: any): string {
  return [
    '# GUSTAV Personal Plan Echo Day 015 French Full-Day Text Packet',
    '',
    `Run: \`${report.runId}\``,
    '',
...
async function main(): Promise<void> {
  ...
  if (!runArg) {
    throw new Error('Usage: npx tsx scripts/gustav_personal_plan_echo_day015_french_full_day_text_packet.ts --run <run-dir>');
  }
  ...
  const rowsPath = path.join(outDir, 'echo_day_015_full_day_fr_rows.jsonl');
  const reviewerQueuePath = path.join(outDir, 'echo_day_015_full_day_fr_reviewer_queue.tsv');
  const packetJsonPath = path.join(auditsDir, 'personal_plan_echo_day015_french_full_day_text_packet.json');
  const packetMdPath = path.join(auditsDir, 'personal_plan_echo_day015_french_full_day_text_packet.md');

  const findings: Finding[] = [];
  const day = await readEchoDay(repoRoot);
  if (!day) addFinding(findings, 'blocker', 'echo_day_015_missing', 'Echo day 15 is missing from source content.', rel(repoRoot, sourcePath));

  let existingPhraseMeaningRowsForDay = 0;
  if (!fs.existsSync(phraseMeaningRowsPath)) {
    addFinding(findings, 'blocker', 'phrase_meaning_rows_missing', 'Echo day 15 phrase meaning rows are missing.', rel(repoRoot, phraseMeaningRowsPath));
  } else {
    ...
    if (existingPhraseMeaningRowsForDay !== 6) addFinding(findings, 'blocker', 'phrase_meaning_rows_day_015_incomplete', `Expected 6 reviewer-needed phrase meaning rows for Echo day 15 but found ${existingPhraseMeaningRowsForDay}.`, rel(repoRoot, phraseMeaningRowsPath));
  }
  ...
  const report = {
    schemaVersion: 'gustav-personal-plan-echo-day015-french-full-day-text-packet-v0',
    ...
    fieldKindCounts,
    generationPolicy: [
      'This packet translates Echo day 15 full-day text fields except phrase meanings, which are already covered by the phrase-meaning layer.',
  ...
  console.log(`GUSTAV Echo day 015 French full-day text packet: ${report.status}`);
