...
function parseRecipes(value, episodeIds) {
    ...
    const result = value.map((item) => {
        ...
        if (item.dialogue !== undefined && typeof item.dialogue !== 'boolean')
            throw new Error('v2_generation_recipes_invalid');
        if (item.speakingClub !== undefined && typeof item.speakingClub !== 'boolean')
            throw new Error('v2_generation_recipes_invalid');
        seen.add(episodeId);
...
function parseV2AdminGenerationRequest(data) {
    ...
    return Object.freeze({ schemaVersion: 'v2-admin-generation-request.v1', seasonId, scope: scope, episodeIds: Object.freeze(episodeIds), ...(recipes ? { recipes } : {}), studyTarget, sourceLocale, targetLocales: Object.freeze(locales), templateBindings, idempotencyKey });
}
function buildV2AdminGenerationPlan(request) {
    const stages = (0, generation_plan_1.buildV2SeasonPlan)(request);
