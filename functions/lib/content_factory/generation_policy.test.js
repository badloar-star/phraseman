...
describe('stage generation policy', () => {
    test('uses versioned stage-specific limits instead of one global 8000-token setting', () => {
        ...
        expect((0, generation_policy_1.resolveStageGenerationPolicy)('quiz_topic', 'gpt-4.1-mini')).toMatchObject({ maxTokens: 3000 });
    });
    test('falls back to json_object for unknown provider model capability', () => {
        expect((0, generation_policy_1.resolveStageGenerationPolicy)('quiz_questions', 'custom-provider-model')).toMatchObject({ responseCapability: 'json_object' });
    });
});
