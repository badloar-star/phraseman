...
exports.EPISODE_01_WORLD_LOCALES = void 0;
exports.episode01WorldMeaning = episode01WorldMeaning;
exports.episode01WorldDistractorReason = episode01WorldDistractorReason;
exports.buildEpisode01WorldLocalizedDetails = buildEpisode01WorldLocalizedDetails;
exports.EPISODE_01_WORLD_LOCALES = [
...
const NUMBERS = { one: '1', two: '2', three: '3', four: '4', five: '5', six: '6', seven: '7', eight: '8', nine: '9', ten: '10', eleven: '11', twelve: '12', thirteen: '13', fourteen: '14', fifteen: '15', sixteen: '16', seventeen: '17', eighteen: '18', nineteen: '19', twenty: '20' };
function subjectLabel(labels, value) {
    return { i: labels.speaker, you: labels.listener, he: labels.male, she: labels.female, we: labels.groupUs, they: labels.groupThem, it: labels.thing }[value] ?? value;
}
function semanticParts(locale, english) {
    ...
    if (/\bthis\b/iu.test(lower))
        parts.push(labels.near);
    if (/\bthat\b/iu.test(lower))
        parts.push(labels.far);
    if (/^whose\b/iu.test(lower))
        parts.push(`${labels.owner}: ?`);
    const subject = lower.match(/^(?:is |are |am )?(i|you|he|she|it|we|they)\b/u)?.[1];
    if (subject)
        parts.push(subjectLabel(labels, subject));
    const owner = lower.match(/\b(my|your|his|her|mine|yours|hers)\b/u)?.[1];
    if (owner)
        parts.push(`${labels.owner}: ${labels.words[owner]}`);
    const family = lower.match(/\b(mother|father|sister|brother)'s\b/u)?.[1];
    if (family)
        parts.push(`${labels.owner}: ${labels.words[family]}`);
    ...
    if (age)
        parts.push(`${labels.age}: ${NUMBERS[age]} ${labels.years}`);
    else if (number)
        parts.push(`× ${NUMBERS[number]}`);
    const noun = lower.match(/\b(book|books|bag|bags|cup|cups|key|keys)\b/u)?.[1]?.replace(/s$/u, '');
    if (noun)
        parts.push(`${labels.object}: ${labels.words[noun]}`);
    for (const word of ['red', 'blue', 'green', 'big', 'small', 'ready', 'happy', 'kind', 'funny', 'quiet', 'calm', 'busy', 'tired', 'fine', 'okay', 'cold', 'warm', 'sunny', 'rainy']) {
        if (new RegExp(`\\b${word}\\b`, 'iu').test(lower))
            parts.push(`${labels.quality}: ${labels.words[word]}`);
    }
    if (/\bhere\b/iu.test(lower))
        parts.push(`${labels.place}: ${labels.words.here}`);
    if (/\bhome\b/iu.test(lower))
        parts.push(`${labels.place}: ${labels.words.home}`);
    if (/\btoo\b/iu.test(lower))
        parts.push(labels.too);
    if (/\band\b/iu.test(lower))
        parts.push(labels.and);
    if (/\bbut\b/iu.test(lower))
        parts.push(labels.but);
    return parts;
}
function episode01WorldMeaning(locale, english) {
    return `${semanticParts(locale, english).join('; ')}.`;
}
function explanation(locale, english) {
...
function wordRole(word) {
    if (/^(?:am|is|are)$/iu.test(word))
        return 'link';
    if (/^(?:my|your|his|her|mine|yours|hers|whose)$/iu.test(word) || /'s$/iu.test(word))
        return 'owner';
    if (/^(?:and|but|too)$/iu.test(word))
        return 'connector';
    return 'content';
}
...
function episode01WorldDistractorReason(locale, correct, alternative) {
...
function buildEpisode01WorldLocalizedDetails(english, phraseTokens, alternatives) {
    return Object.fromEntries(exports.EPISODE_01_WORLD_LOCALES.map((locale) => [locale, {
...
//# sourceMappingURL=episode_01_phrase_localization_41_56_v1.js.map