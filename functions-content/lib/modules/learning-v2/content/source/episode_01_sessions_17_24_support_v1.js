...
exports.buildEpisode01Session17To24 = buildEpisode01Session17To24;
exports.assertEpisode01Session17To24Contract = assertEpisode01Session17To24Contract;
...
const L = (select) => Object.fromEntries(LOCALES.map((locale) => [locale, select(COPY[locale])]));
const tokenise = (english) => english.replace(/[?.!]/g, '').split(/\s+/).filter(Boolean);
const wordCategory = (word) => /^(He|She|It|he|she|it)$/u.test(word) ? 'pronoun' : /^(is|Is|[Hh]e’s|[Ss]he’s|[Ii]t’s)$/u.test(word) ? 'to-be' : word === 'not' ? 'negation' : 'lexical';
const turkishQuestionParticle = (word) => {
    if (word.toLocaleLowerCase('tr') === 'meşgul')
        return 'mü';
...
function localizedDetails(english) {
...
function wordPrompt(locale, english, word) {
    const meaning = translateMeaning(locale, english);
    const prompts = {
...
function localizedContrast(locale, correct, alternative) {
...
function localizedReason(locale, correct, alternative) {
    if (alternative === 'this') {
        const thisReason = {
...
const wordAlternatives = (word) => {
    if (/^(He|She|It|he|she|it)$/u.test(word))
        return ['I', 'you', 'we', 'they', 'this'];
    if (/^(is|Is)$/u.test(word))
        return ['am', 'are', 'was', 'be', 'been'];
    if (/^([Hh]e|[Ss]he|[Ii]t)’s$/u.test(word))
        return ['I’m', 'you’re', 'we’re', 'they’re', 'is'];
    if (word === 'not')
        return ['very', 'also', 'too', 'really', 'quite'];
    if (word === 'a')
        return ['an', 'the', 'this', 'that', 'my'];
    if (/^(My|my)$/u.test(word))
        return ['your', 'his', 'her', 'our', 'their'];
    if (/^(mother|father|sister|brother)$/u.test(word))
        return ['friend', 'teacher', 'child', 'neighbor', 'parent'];
    if (/^(ready|tired|calm|happy|busy|cold|warm|okay|sunny|rainy|windy|cloudy)$/u.test(word))
        return ['ready', 'tired', 'calm', 'happy', 'busy', 'cold', 'warm', 'okay', 'sunny', 'rainy'].filter((value) => value !== word).slice(0, 5);
    if (/^(here|book|bag|cup|pen|phone|key)$/u.test(word))
        return ['here', 'home', 'book', 'bag', 'phone', 'key'].filter((value) => value !== word).slice(0, 5);
    return ['I', 'you', 'he', 'she', 'it'];
};
function localizedExplanation(locale, english, meaning) {
...
function translateMeaning(locale, english) {
    ...
    if (isPersonTemperature) {
        ...
        const temperatures = { ru: adjective === 'cold' ? 'холодно' : 'тепло', uk: adjective === 'cold' ? 'холодно' : 'тепло', es: adjective === 'cold' ? 'frío' : 'calor', 'pt-BR': adjective === 'cold' ? 'frio' : 'calor', vi: adjective === 'cold' ? 'lạnh' : 'ấm', id: adjective === 'cold' ? 'kedinginan' : 'hangat', tr: adjective === 'cold' ? 'üşüyor' : 'sıcak', pl: adjective === 'cold' ? 'zimno' : 'ciepło' };
        const temperature = temperatures[locale];
        if (question) {
            const questions = { ru: `${person} ${temperature}?`, uk: `${person} ${temperature}?`, es: `¿${person} ${temperature}?`, 'pt-BR': `${person} ${temperature}?`, vi: `${person} ${temperature} phải không?`, id: `Apakah ${person.toLowerCase()} ${temperature}?`, tr: `${person} ${temperature} ${turkishQuestionParticle(temperature)}?`, pl: `Czy ${person.toLowerCase()} ${temperature}?` };
            return questions[locale];
        }
        if (!negative)
            return `${person} ${temperature}${punctuation}`;
        const negativeTemperature = {
    ...
    if (isIt) {
        ...
        if (forms)
            return `${forms[question ? 2 : negative ? 1 : 0]}${forms[question ? 2 : negative ? 1 : 0].endsWith('?') ? '' : '.'}`;
    }
    if (locale === 'ru') {
        if (subject === 'It') {
        ...
        const feminine = { готов: 'готова', устал: 'устала', спокоен: 'спокойна', счастлив: 'счастлива', занят: 'занята', холоден: 'холодна' };
    ...
    if (locale === 'uk') {
        ...
        const feminine = { готовий: 'готова', втомлений: 'втомлена', спокійний: 'спокійна', щасливий: 'щаслива', зайнятий: 'зайнята' };
        const spoken = female ? (feminine[value] ?? value) : value;
        return `${person} ${negative ? 'не ' : ''}${spoken}${question ? '?' : '.'}`;
    }
    if (locale === 'es') {
        ...
        if (subject === 'It' && (adjective === 'cold' || adjective === 'warm'))
            return `${negative ? 'No ' : ''}Hace ${adjective === 'cold' ? 'frío' : 'calor'}${question ? '?' : '.'}`;
        if ((subject === 'He' || subject === 'She') && (adjective === 'cold' || adjective === 'warm'))
            return `${p} ${negative ? 'no ' : ''}tiene ${adjective === 'cold' ? 'frío' : 'calor'}${question ? '?' : '.'}`;
        const feminine = { listo: 'lista', cansado: 'cansada', tranquilo: 'tranquila', ocupado: 'ocupada', frío: 'fría' };
        const spoken = female ? (feminine[value] ?? value) : value;
    ...
    if (locale === 'pt-BR') {
        ...
        if (subject === 'It' && (adjective === 'cold' || adjective === 'warm'))
            return `${negative ? 'Não ' : ''}Está ${adjective === 'cold' ? 'frio' : 'quente'}${question ? '?' : '.'}`;
        if ((subject === 'He' || subject === 'She') && (adjective === 'cold' || adjective === 'warm'))
            return `${p} ${negative ? 'não ' : ''}está com ${adjective === 'cold' ? 'frio' : 'calor'}${question ? '?' : '.'}`;
        const feminine = { pronto: 'pronta', cansado: 'cansada', calmo: 'calma', ocupado: 'ocupada', frio: 'fria' };
        const spoken = female ? (feminine[value] ?? value) : value;
        return `${p} ${negative ? 'não ' : ''}está ${spoken}${question ? '?' : '.'}`;
    }
    if (locale === 'vi') {
        const p = subject === 'She' ? 'Cô ấy' : subject === 'He' ? 'Anh ấy' : subject === 'It' ? 'Trời' : ({ 'My mother': 'Mẹ tôi', 'My father': 'Bố tôi', 'My sister': 'Chị tôi', 'My brother': 'Anh tôi' }[subject] ?? 'Mẹ tôi');
        return subject === 'It' && adjective === 'cold' ? 'Trời lạnh.' : question ? `${p} ${value} phải không?` : negative ? `${p} không ${value}.` : `${p} ${value}.`;
    }
    if (locale === 'id') {
        const p = subject === 'It' ? 'Cuacanya' : ({ 'My mother': 'Ibu saya', 'My father': 'Ayah saya', 'My sister': 'Saudari saya', 'My brother': 'Saudara laki-laki saya' }[subject] ?? 'Dia');
        return subject === 'It' && adjective === 'cold' ? 'Cuacanya dingin.' : question ? `Apakah ${p.toLowerCase()} ${value}?` : negative ? `${p} tidak ${value}.` : `${p} ${value}.`;
    }
    if (locale === 'tr') {
        const p = subject === 'It' ? 'Hava' : ({ 'My mother': 'Annem', 'My father': 'Babam', 'My sister': 'Kız kardeşim', 'My brother': 'Erkek kardeşim' }[subject] ?? 'O');
        return subject === 'It' && adjective === 'cold' ? 'Hava soğuk.' : question ? `${p} ${value} ${turkishQuestionParticle(value)}?` : negative ? `${p} ${value} değil.` : `${p} ${value}.`;
    }
    ...
    const feminine = { gotowy: 'gotowa', zmęczony: 'zmęczona', spokojny: 'spokojna', szczęśliwy: 'szczęśliwa', zajęty: 'zajęta' };
    const spoken = female ? (feminine[value] ?? value) : value;
    return question ? `Czy ${p.toLowerCase()} jest ${spoken}?` : `${p} ${negative ? 'nie ' : ''}jest ${spoken}.`;
}
function phrase(ordinal, position, english, features) {
...
function runs(body, target) {
    return Object.fromEntries(LOCALES.map((locale) => {
        ...
        return [locale, at < 0 ? [{ text, semantic: 'explanation' }] : [{ text: text.slice(0, at), semantic: 'explanation' }, { text: target, semantic: 'targetCorrect' }, { text: text.slice(at + target.length), semantic: 'explanation' }]];
    }));
}
const PHRASES = {
...
function pageRule(locale, ordinal, index) {
...
function introTail(locale, english) {
    const meaning = translateMeaning(locale, english);
    const copy = {
...
function answerExplanation(locale, english, formula) {
    const meaning = translateMeaning(locale, english);
    const copy = {
...
function buildEpisode01Session17To24(ordinal) {
    const topicIndex = ordinal - 17;
    const bodies = [0, 1, 2].map((index) => Object.fromEntries(LOCALES.map((locale) => {
    ...
    const title = Object.fromEntries(LOCALES.map((locale) => [locale, TITLES[locale][topicIndex]]));
    const introPages = bodies.map((body, index) => {
        const pageTitle = Object.fromEntries(LOCALES.map((locale) => [locale, PAGE_TITLES[locale][topicIndex][index]]));
        const pageTarget = PHRASES[ordinal][index];
        ...
        const explanation = Object.fromEntries(LOCALES.map((locale) => [locale, answerExplanation(locale, pageTarget, SESSION_RULES[ordinal].formula)]));
        return { kind: ['concept', 'formula', 'trap'][index], title: pageTitle, body, bodyRuns: runs(body, pageTarget), question: { prompt: Object.fromEntries(LOCALES.map((locale) => [locale, QUIZ_PROMPT[locale](translateMeaning(locale, pageTarget))])), choices: [L(() => pageTarget), L(() => pageWrongOne), L(() => pageWrongTwo)], correctChoiceIndex: 0, explanation } };
    });
    return { packageId: 'learning-v2-en-v1', targetLanguage: 'en', episodeOrdinal: 1, requiredSessionOrdinal: ordinal, canDoOutcomeId: 'obj-e01-third-person-is', generationInputFingerprint: `authored-e01-s${ordinal}-v2`, title, summary: Object.fromEntries(LOCALES.map((locale) => [locale, TOPICS[locale][topicIndex]])), learningGoal: Object.fromEntries(LOCALES.map((locale) => [locale, `${PAGE_TITLES[locale][topicIndex][0]}. ${PAGE_TITLES[locale][topicIndex][1]}.`])), introPages, phrases: PHRASES[ordinal].map((english, index) => phrase(ordinal, index, english, FEATURES[ordinal])) };
}
function assertEpisode01Session17To24Contract(source, ordinal, kind, teaches) {
    expect(source.requiredSessionOrdinal).toBe(ordinal);
    expect(source.phrases).toHaveLength(15);
    expect(source.introPages.map((page) => page.kind)).toEqual(['concept', 'formula', 'trap']);
...
//# sourceMappingURL=episode_01_sessions_17_24_support_v1.js.map