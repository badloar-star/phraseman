...
describe("Learning V2 lesson map model", () => {
  it("always renders the owner-approved 12-session, three-zone route", () => {
    ...
    expect(
      model.zones.flatMap((zone) => zone.nodes).map((node) => node.id),
    ).toEqual([
      "lesson-1-understand-1",
      "lesson-1-understand-2",
      "lesson-1-understand-3",
      "lesson-1-understand-4",
      "lesson-1-use-1",
      "lesson-1-use-2",
      "lesson-1-use-3",
      "lesson-1-use-4",
  ...
  it("maps completed/current/next/locked states without a connector line", () => {
    ...
    expect(nodes.map((node) => node.xOffset)).toEqual([
  ...
  it("ignores tournament context instead of adding it to the lesson map", () => {
    ...
    expect(
      model.zones
        ...
        .some((node) => node.id === "tournament-1"),
    ).toBe(false);
  });

  it("keeps every node locked when lesson runtime content is not released", () => {
    const model = buildLessonMapModel({
      lessonId: 2,
      completedSessionIds: [],
      available: false,
