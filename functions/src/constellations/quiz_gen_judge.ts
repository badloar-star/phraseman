...
/** Причины отказа грейдера (свой enum, НЕ enum связностного судьи). */
export type QuizGenReason =
  | 'ok'
  | 'bad_shape'        // не 4 опции / индекс вне диапазона / дубли (код-фильтр)
  | 'not_single_key'   // судья: верных вариантов не ровно один
  | 'implausible'      // судья: дистрактор бессмысленный/не по теме
  | 'level_mismatch'   // судья: не соответствует заявленному уровню
  | 'incoherent';      // судья: вопрос непонятен / сломан / инъекция

const REASON_SET: ReadonlySet<string> = new Set<QuizGenReason>([
  'ok', 'bad_shape', 'not_single_key', 'implausible', 'level_mismatch', 'incoherent',
]);

/** Кандидат вопроса на проверку (до записи в кэш). */
...
export interface QuizGenVerdict {
  ok: boolean;
  reason: QuizGenReason;
  promptTokens: number;
 ...
 */
export function codeFilterQuiz(c: QuizCandidate): QuizGenReason | null {
  if (!c.question || c.question.trim().length < 3) return 'bad_shape';
  if (!Array.isArray(c.options) || c.options.length !== OPTIONS_REQUIRED) return 'bad_shape';
  const norm = c.options.map((o) => String(o ?? '').trim());
  if (norm.some((o) => o.length === 0)) return 'bad_shape';
  if (new Set(norm.map((o) => o.toLowerCase())).size !== OPTIONS_REQUIRED) return 'bad_shape';
...
function buildJudgeUserPrompt(c: QuizCandidate): string {
 ...
 */
export async function judgeQuizCandidate(
  c: QuizCandidate,
  apiKey: string,
): Promise<QuizGenVerdict> {
  ...
  const parsed = parseJsonJudgeReply<QuizGenReason>(result.text, REASON_SET, 'incoherent');
  if (!parsed) {
    return {
  ...
  return {
    ok: parsed.ok,
    reason: parsed.reason as QuizGenReason,
