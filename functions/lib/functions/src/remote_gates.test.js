...
describe('remote_gates — pickRemoteBool (серверное чтение feature-флага)', () => {
    ...
    it('ai_global_disable — дефолт FALSE (ИИ работает), пока админ не поставит TRUE', () => {
        ...
        expect((0, remote_gates_1.pickRemoteBool)(undefined, 'ai_global_disable', false)).toBe(false);
        expect((0, remote_gates_1.pickRemoteBool)({}, 'ai_global_disable', false)).toBe(false);
        // Админ включил рубильник → true.
        expect((0, remote_gates_1.pickRemoteBool)({ ai_global_disable: true }, 'ai_global_disable', false)).toBe(true);
        expect((0, remote_gates_1.pickRemoteBool)({ ai_global_disable: 'true' }, 'ai_global_disable', false)).toBe(true);
    });
