...
function makeProbeDependenciesReady(input: EvaluationInput): void {
  ...
  input.p60Ready = true;
  input.p60State = 'p46_apply_transaction_command_preflight_ready_waiting_for_p45_sequence';
  input.p60FreshAfterP47 = true;
  input.p60CommandAllowedNow = false;
  input.p60CommandAllowedAfterP45Sequence = true;
  input.p60CommandExecutedByThisScript = false;
  input.p60ReadyForApply = false;
  input.p60MayModifyProductionAppFiles = false;
  input.p60ActivationApproved = false;
  input.p46Status = 'HOLD';
  input.p46TransactionState = 'waiting_for_activation_sequence_preflight';
