...
export function statsAccent(
  themeMode: ThemeMode,
  _tone: StatsChromeTone,
): string {
...
export function statsThemeAccent(themeMode: ThemeMode): string {
  return (
    STATS_CHROME_ACCENT_BY_THEME[themeMode] ?? STATS_CHROME_ACCENT_BY_THEME.dark
  );
}
...
export function statsThemeSoftBg(
  themeMode: ThemeMode,
  strength: "quiet" | "normal" | "strong" = "normal",
): string {
  const alpha =
    strength === "strong" ? "2E" : strength === "quiet" ? "14" : "22";
  return alphaColor(statsThemeAccent(themeMode), alpha);
}

export function statsBorder(
  themeMode: ThemeMode,
  _tone: StatsChromeTone,
  strength: "soft" | "medium" | "strong" = "medium",
): string {
  const alpha =
    strength === "strong" ? "B8" : strength === "soft" ? "59" : "7A";
  return alphaColor(statsThemeAccent(themeMode), alpha);
}

export function statsHairline(
  themeMode: ThemeMode,
  _tone: StatsChromeTone,
): string {
...
export function statsSoftBg(
  themeMode: ThemeMode,
  tone: StatsChromeTone,
  strength: "quiet" | "normal" | "strong" = "normal",
): string {
  const alpha =
    strength === "strong" ? "2E" : strength === "quiet" ? "14" : "22";
...
export function statsGlowStyle(
  themeMode: ThemeMode,
  _tone: StatsChromeTone,
