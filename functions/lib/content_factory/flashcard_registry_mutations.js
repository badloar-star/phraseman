...
exports.flashcardRegistryEntriesForPack = flashcardRegistryEntriesForPack;
exports.applyFlashcardRegistryDocumentMutation = applyFlashcardRegistryDocumentMutation;
exports.planFlashcardRegistryPackMutation = planFlashcardRegistryPackMutation;
const flashcard_semantic_registry_1 = require("./flashcard_semantic_registry");
const SOURCE_LOCALES = ['ru', 'uk', 'es', 'pt-BR', 'vi', 'id', 'tr', 'pl'];
function record(value) { return typeof value === 'object' && value !== null && !Array.isArray(value) ? value : undefined; }
function flashcardRegistryEntriesForPack(pack) {
    const entries = [];
    for (const raw of Array.isArray(pack.cards) ? pack.cards : []) {
        ...
        for (const sourceLocale of SOURCE_LOCALES) {
            ...
            if (!String(back ?? '').trim())
                continue;
            entries.push((0, flashcard_semantic_registry_1.buildFlashcardRegistryEntry)({ partition: { surface: 'community_flashcards', studyTarget: String(pack.studyTarget ?? 'en'), sourceLocale }, packId: pack.id, card: { id: card.id, front: card.front ?? card.en, back } }));
        }
    }
    const grouped = new Map();
    for (const entry of entries)
        grouped.set(entry.docId, grouped.has(entry.docId) ? (0, flashcard_semantic_registry_1.mergeFlashcardRegistrySource)(grouped.get(entry.docId), entry) : entry);
    return Object.freeze([...grouped.values()].sort((a, b) => a.docId.localeCompare(b.docId)));
}
function applyFlashcardRegistryDocumentMutation(current, mutation) {
    if (current && (current.partitionKey !== mutation.partitionKey || current.canonicalKey !== mutation.canonicalKey || current.docId !== mutation.docId))
    ...
    for (const source of mutation.addSources)
        sources.set(`${source.packId}\u0000${source.cardId}`, source);
...
function planFlashcardRegistryPackMutation(previous, next) {
    ...
    return Object.freeze(ids.map((docId) => {
        const oldEntry = before.find((entry) => entry.docId === docId);
        const newEntry = after.find((entry) => entry.docId === docId);
        const identity = newEntry ?? oldEntry;
        const key = (source) => `${source.packId}\u0000${source.cardId}`;
        const oldKeys = new Set((oldEntry?.sources ?? []).map(key));
        const newKeys = new Set((newEntry?.sources ?? []).map(key));
...
//# sourceMappingURL=flashcard_registry_mutations.js.map