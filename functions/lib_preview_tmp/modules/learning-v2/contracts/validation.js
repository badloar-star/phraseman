...
exports.validateV2EpisodeContract = exports.validateV2CheckpointContract = exports.validateV2CurriculumProjection = exports.validateV2LearningPackage = exports.validateContentGateReceiptBody = exports.validateContentReceiptSubject = exports.validateModeTemplateLifecycleHead = exports.validateModeTemplateArtifactBody = exports.validateVoiceReleaseRequirementsShape = void 0;
...
const TEMPLATE_KEYS = ["templateRef", "body"];
...
const TEMPLATE_BODY_OPTIONAL_KEYS = ["voiceReleaseRequirements"];
...
const POLICY_DESCRIPTOR_KEYS = ["ref", "body"];
...
const EPISODE_V2_KEYS = [...EPISODE_KEYS, "sessionSetRef"];
...
const GRAPH_KEYS = ["startNodeId", "capstoneNodeId", "nodes", "edges"];
const NODE_KEYS = [
    "nodeId",
    "activityId",
    "position",
...
const EDGE_KEYS = ["edgeId", "fromNodeId", "toNodeId", "condition"];
...
const isRecordArray = (value) => Array.isArray(value) && value.every(isPlainObject);
 ...
 */
const snapshotCanonicalJsonInput = (input, rootPath, options) => {
    ...
    try {
        while (stack.length > 0) {
            ...
            if (Array.isArray(value)) {
                ...
                for (const entry of entries.sort((left, right) => right.index - left.index)) {
                    stack.push({
                        kind: "visit",
                        value: entry.value,
...
const sameValue = (left, right) => {
    try {
        return (0, decision_registry_1.canonicalJsonV1)(left) === (0, decision_registry_1.canonicalJsonV1)(right);
    }
    catch {
        return false;
    }
...
const exactObjectIssues = (value, requiredKeys, optionalKeys, path, unknownCode = "field_unknown") => {
...
const episodeContainerShapeIssue = (episode) => {
...
const uniqueSecondIndex = (values) => {
    const seen = new Set();
...
const resolveRegistry = (context) => {
    if (!isPlainObject(context) || !hasOwn(context, "decisionRegistry")) {
        return fail([
            issue("decision_registry_context_invalid", "$.decisionRegistry"),
        ]);
    }
    const result = (0, decision_registry_1.validateDecisionRegistry)(context.decisionRegistry);
    if (!result.ok)
        return fail([
            issue("decision_registry_context_invalid", "$.decisionRegistry"),
        ]);
...
const decisionSettings = (registry, decisionId) => {
...
const rangeContains = (range, value) => isPlainObject(range) &&
    typeof range.min === "number" &&
    typeof range.max === "number" &&
    value >= range.min &&
    value <= range.max;
const validatePackageShape = (input) => {
    ...
    const dependencyIssues = exactObjectIssues(root.dependencies, DEPENDENCY_KEYS, [], "$.dependencies");
    ...
    for (let index = 0; index < dependencies.templates.length; index += 1) {
        const templateIssues = exactObjectIssues(dependencies.templates[index], TEMPLATE_KEYS, [], `$.dependencies.templates[${index}]`);
        if (templateIssues.length > 0)
            return templateIssues;
    }
    const episode = root.episode;
    const episodeKeys = episode?.schemaVersion === "v2-episode-contract.v2"
        ? EPISODE_V2_KEYS
        : EPISODE_KEYS;
    ...
    const episodeIssues = exactObjectIssues(root.episode, episodeKeys.filter((key) => key !== "checkpointContract"), ["checkpointContract"], "$.episode");
    if (episodeIssues.length > 0)
        return episodeIssues;
    ...
    if (episode.schemaVersion === "v2-episode-contract.v2") {
        const episodeResult = (0, exports.validateV2EpisodeContract)(episode);
        if (!episodeResult.ok)
            return episodeResult.issues;
    }
    if (!isRecordArray(episode.activities))
        return [issue("field_type_invalid", "$.episode.activities")];
    const activityUnknownIssues = [];
    for (let index = 0; index < episode.activities.length; index += 1) {
        activityUnknownIssues.push(...exactObjectIssues(episode.activities[index], ACTIVITY_KEYS, [], `$.episode.activities[${index}]`, "activity_forbidden_field"));
        const activity = episode.activities[index];
        const tagIssues = exactObjectIssues(activity.tags, ["skillIds", "grammar", "vocabulary", "scenario", "modalities"], [], `$.episode.activities[${index}].tags`);
        activityUnknownIssues.push(...tagIssues);
        const capabilityIssues = exactObjectIssues(activity.capabilities, ["microphone", "speechRecognition", "audioPlayback", "network"], [], `$.episode.activities[${index}].capabilities`);
        activityUnknownIssues.push(...capabilityIssues);
    }
    if (activityUnknownIssues.length > 0)
        return activityUnknownIssues;
    const scenarioIssues = exactObjectIssues(episode.scenario, [
        "scenarioId",
        "title",
        "setting",
        "learnerRole",
    ...
    for (let index = 0; index < episode.phraseFrames.length; index += 1) {
        const phraseIssues = exactObjectIssues(episode.phraseFrames[index], [
            "phraseFrameId",
            "targetPattern",
            "learnerMeaning",
            "semanticSlotIds",
    ...
    for (let index = 0; index < episode.semanticSlots.length; index += 1) {
        const slotIssues = exactObjectIssues(episode.semanticSlots[index], [
            "semanticSlotId",
            "role",
    ...
    const graphIssues = exactObjectIssues(episode.graph, GRAPH_KEYS, [], "$.episode.graph");
    if (graphIssues.length > 0)
        return graphIssues;
    const graph = episode.graph;
    ...
    for (let index = 0; index < graph.nodes.length; index += 1) {
        ...
        const nodeIssues = exactObjectIssues(graph.nodes[index], NODE_KEYS.filter((key) => ![
            "fallback",
            "transferFromNodeId",
            "variedSemanticSlotIds",
            "starSlotId",
        ...
        for (let declarationIndex = 0; declarationIndex < declarations.length; declarationIndex += 1) {
            const declarationIssues = exactObjectIssues(declarations[declarationIndex], DECLARATION_KEYS, [], `${nodePath}.evidenceDeclarations[${declarationIndex}]`);
            if (declarationIssues.length > 0)
                return declarationIssues;
        }
    }
    for (let index = 0; index < graph.edges.length; index += 1) {
        const edgeIssues = exactObjectIssues(graph.edges[index], EDGE_KEYS, [], `$.episode.graph.edges[${index}]`);
        if (edgeIssues.length > 0)
            return edgeIssues;
    }
    const loopShapeIssues = exactObjectIssues(episode.requiredLoops, ["encounterBuildNodeIds", "nearTransferNodeIds"], [], "$.episode.requiredLoops");
    if (loopShapeIssues.length > 0)
        return loopShapeIssues;
    const assessmentShapeIssues = exactObjectIssues(episode.assessmentNodes, ["independentProbeNodeIds", "optionalReviewNodeIds"], [], "$.episode.assessmentNodes");
    if (assessmentShapeIssues.length > 0)
        return assessmentShapeIssues;
    ...
    for (let index = 0; index < episode.reviewLinks.length; index += 1) {
        ...
        if (materializedKey) {
            return [
                issue("review_link_materialization_forbidden", `$.episode.reviewLinks[${index}].${materializedKey}`),
            ];
        }
    }
    return [];
};
const oneExactObjectIssue = (value, requiredKeys, optionalKeys, path) => exactObjectIssues(value, requiredKeys, optionalKeys, path)[0];
const identityValueIssue = (value, path) => typeof value === "string" && !identities_1.V2_IDENTITY_REGEX.test(value)
    ? issue("identity_invalid", path)
    : undefined;
const identityArrayIssue = (value, path) => {
    if (!isStringArray(value))
        return undefined;
    const invalidIndex = value.findIndex((candidate) => !identities_1.V2_IDENTITY_REGEX.test(candidate));
    return invalidIndex >= 0
        ? issue("identity_invalid", `${path}[${invalidIndex}]`)
        : undefined;
};
const identityFieldsIssue = (value, path, scalarFields, arrayFields) => {
    if (!isPlainObject(value))
        return undefined;
...
const declarationIdentityIssue = (value, path) => {
    const declarationIssue = identityFieldsIssue(value, path, ["objectiveId", "skillId"], []);
    if (declarationIssue || !isPlainObject(value))
        return declarationIssue;
    return identityFieldsIssue(value.target, `${path}.target`, ["targetId"], []);
};
const pedagogicalContextIdentityIssue = (value, path) => {
    if (!isPlainObject(value))
        return undefined;
    return (identityFieldsIssue(value.context, `${path}.context`, ["contextId", "surfaceFormId"], []) ?? identityFieldsIssue(value.prompt, `${path}.prompt`, ["promptId"], []));
};
const templateIdentityIssue = (value, path) => {
    if (!isPlainObject(value))
        return undefined;
    const refIssue = identityFieldsIssue(value.templateRef, `${path}.templateRef`, ["templateId"], []);
    if (refIssue || !isPlainObject(value.body))
        return refIssue;
    ...
    if (bodyIssue)
        return bodyIssue;
    const purposeIssue = identityFieldsIssue(body.pedagogicalPurpose, `${bodyPath}.pedagogicalPurpose`, [], ["primarySkillIds"]);
    if (purposeIssue)
        return purposeIssue;
    const kernelIssue = identityFieldsIssue(body.kernel, `${bodyPath}.kernel`, ["activityTypeKey", "rendererKey", "payloadSchemaKey"], []);
    if (kernelIssue)
        return kernelIssue;
    if (isPlainObject(body.policies)) {
        for (const kind of POLICY_KINDS) {
            const policyRefIssue = identityFieldsIssue(body.policies[kind], `${bodyPath}.policies.${kind}`, ["key"], []);
            if (policyRefIssue)
                return policyRefIssue;
        }
    }
    if (isPlainObject(body.capabilityContract)) {
        const fallbackIssue = identityFieldsIssue(body.capabilityContract.fallbackTemplateRef, `${bodyPath}.capabilityContract.fallbackTemplateRef`, ["templateId"], []);
        if (fallbackIssue)
            return fallbackIssue;
    }
    if (isRecordArray(body.fixtures)) {
        for (let index = 0; index < body.fixtures.length; index += 1) {
            const fixtureIssue = identityFieldsIssue(body.fixtures[index], `${bodyPath}.fixtures[${index}]`, ["fixtureId"], []);
            if (fixtureIssue)
                return fixtureIssue;
        }
    }
    const compatibilityIssue = identityFieldsIssue(body.compatibility, `${bodyPath}.compatibility`, ["progressCompatibilityNamespace"], []);
    if (compatibilityIssue)
        return compatibilityIssue;
    if (isPlainObject(body.learningContractRefs)) {
        for (const key of TEMPLATE_LEARNING_REF_KEYS) {
            const learningRefIssue = identityFieldsIssue(body.learningContractRefs[key], `${bodyPath}.learningContractRefs.${key}`, ["id"], []);
            if (learningRefIssue)
                return learningRefIssue;
        }
    }
    if (isPlainObject(body.voiceReleaseRequirements)) {
        const voicePath = `${bodyPath}.voiceReleaseRequirements`;
        const calibrationIssue = identityFieldsIssue(body.voiceReleaseRequirements.calibrationReceiptRef, `${voicePath}.calibrationReceiptRef`, ["receiptId"], []);
        if (calibrationIssue)
            return calibrationIssue;
        const policyIssue = identityFieldsIssue(body.voiceReleaseRequirements.voiceDataPolicyRef, `${voicePath}.voiceDataPolicyRef`, ["policyId"], []);
        if (policyIssue)
            return policyIssue;
        const activeRegistryIssue = identityFieldsIssue(body.voiceReleaseRequirements, voicePath, ["activePolicyRegistryKey"], []);
        if (activeRegistryIssue)
            return activeRegistryIssue;
        return identityFieldsIssue(body.voiceReleaseRequirements.networkEgressRef, `${voicePath}.networkEgressRef`, ["gatewayId"], []);
    }
    return undefined;
};
const activityIdentityIssue = (value, path) => {
    if (!isPlainObject(value))
        return undefined;
    const directIssue = identityFieldsIssue(value, path, ["activityId", "progressCompatibilityKey", "activityTypeKey"], ["contentUnitIds", "assetIds"]);
    if (directIssue)
        return directIssue;
    const templateIssue = identityFieldsIssue(value.templateRef, `${path}.templateRef`, ["templateId"], []);
    if (templateIssue)
        return templateIssue;
    const requirementsIssue = identityFieldsIssue(value.requirements, `${path}.requirements`, [], ["prompt", "media", "input"]);
    if (requirementsIssue)
        return requirementsIssue;
    const targetsIssue = identityFieldsIssue(value.targets, `${path}.targets`, [], ["skillIds", "phraseFrameIds", "semanticSlotIds"]);
    if (targetsIssue)
        return targetsIssue;
    return identityFieldsIssue(value.tags, `${path}.tags`, [], ["skillIds", "grammar", "vocabulary", "scenario"]);
};
const graphNodeIdentityIssue = (value, path) => {
    if (!isPlainObject(value))
        return undefined;
    const nodeIssue = identityFieldsIssue(value, path, ["nodeId", "activityId", "transferFromNodeId", "starSlotId"], ["variedSemanticSlotIds"]);
    if (nodeIssue)
        return nodeIssue;
    const fallbackIssue = identityFieldsIssue(value.fallback, `${path}.fallback`, ["alternateNodeId"], []);
    if (fallbackIssue)
        return fallbackIssue;
    if (isRecordArray(value.evidenceDeclarations)) {
        for (let index = 0; index < value.evidenceDeclarations.length; index += 1) {
            const declarationIssue = declarationIdentityIssue(value.evidenceDeclarations[index], `${path}.evidenceDeclarations[${index}]`);
            if (declarationIssue)
                return declarationIssue;
        }
    }
    return pedagogicalContextIdentityIssue(value.pedagogicalContextContract, `${path}.pedagogicalContextContract`);
};
const episodeIdentityIssue = (value) => {
    if (!isPlainObject(value))
        return undefined;
    const path = "$.episode";
    const episodeIssue = identityFieldsIssue(value, path, ["episodeId", "seasonId", "chapterId"], [
        "objectiveIds",
        "skillIds",
        "grammarDistinctionIds",
        "soundFocusIds",
    ...
    if (episodeIssue)
        return episodeIssue;
    const scenarioIssue = identityFieldsIssue(value.scenario, `${path}.scenario`, ["scenarioId"], ["criticalConstraintIds"]);
    if (scenarioIssue)
        return scenarioIssue;
    if (isRecordArray(value.phraseFrames)) {
        for (let index = 0; index < value.phraseFrames.length; index += 1) {
            const phraseIssue = identityFieldsIssue(value.phraseFrames[index], `${path}.phraseFrames[${index}]`, ["phraseFrameId"], ["semanticSlotIds", "skillIds"]);
            if (phraseIssue)
                return phraseIssue;
        }
    }
    if (isRecordArray(value.semanticSlots)) {
        for (let index = 0; index < value.semanticSlots.length; index += 1) {
            const slotIssue = identityFieldsIssue(value.semanticSlots[index], `${path}.semanticSlots[${index}]`, ["semanticSlotId"], ["allowedContentUnitIds"]);
            if (slotIssue)
                return slotIssue;
        }
    }
    if (isRecordArray(value.criticalConstraints)) {
        for (let index = 0; index < value.criticalConstraints.length; index += 1) {
            const constraintIssue = identityFieldsIssue(value.criticalConstraints[index], `${path}.criticalConstraints[${index}]`, ["criticalConstraintId"], []);
            if (constraintIssue)
    ...
    if (isRecordArray(value.activities)) {
        for (let index = 0; index < value.activities.length; index += 1) {
            const activityIssue = activityIdentityIssue(value.activities[index], `${path}.activities[${index}]`);
            if (activityIssue)
                return activityIssue;
        }
    }
    if (isPlainObject(value.graph)) {
        const graphIssue = identityFieldsIssue(value.graph, `${path}.graph`, ["startNodeId", "capstoneNodeId"], []);
        if (graphIssue)
            return graphIssue;
        if (isRecordArray(value.graph.nodes)) {
            for (let index = 0; index < value.graph.nodes.length; index += 1) {
                const nodeIssue = graphNodeIdentityIssue(value.graph.nodes[index], `${path}.graph.nodes[${index}]`);
                if (nodeIssue)
                    return nodeIssue;
            }
        }
        if (isRecordArray(value.graph.edges)) {
            for (let index = 0; index < value.graph.edges.length; index += 1) {
                const edgeIssue = identityFieldsIssue(value.graph.edges[index], `${path}.graph.edges[${index}]`, ["edgeId", "fromNodeId", "toNodeId"], []);
                if (edgeIssue)
                    return edgeIssue;
            }
        }
    ...
    if (isRecordArray(value.starSlots)) {
        for (let index = 0; index < value.starSlots.length; index += 1) {
            const starIssue = identityFieldsIssue(value.starSlots[index], `${path}.starSlots[${index}]`, ["starSlotId"], ["acceptedNodeIds"]);
            if (starIssue)
                return starIssue;
        }
    }
    const loopIssue = identityFieldsIssue(value.requiredLoops, `${path}.requiredLoops`, [], ["encounterBuildNodeIds", "nearTransferNodeIds"]);
    if (loopIssue)
        return loopIssue;
    const assessmentIssue = identityFieldsIssue(value.assessmentNodes, `${path}.assessmentNodes`, [], ["independentProbeNodeIds", "optionalReviewNodeIds"]);
    if (assessmentIssue)
        return assessmentIssue;
    const capstoneIssue = identityFieldsIssue(value.capstoneContract, `${path}.capstoneContract`, [], [
        "objectiveIds",
    ...
    if (capstoneIssue)
        return capstoneIssue;
    if (isPlainObject(value.learningDesign)) {
        const learningPath = `${path}.learningDesign`;
        const learningIssue = identityFieldsIssue(value.learningDesign, learningPath, ["primaryOutcomeId", "independentProbeRef", "delayedWindowPolicyId"], ["objectiveIds"]);
        if (learningIssue)
            return learningIssue;
        if (isRecordArray(value.learningDesign.prerequisiteEdges)) {
            for (let index = 0; index < value.learningDesign.prerequisiteEdges.length; index += 1) {
                const edge = value.learningDesign.prerequisiteEdges[index];
                const edgePath = `${learningPath}.prerequisiteEdges[${index}]`;
                const fromIssue = identityFieldsIssue(edge.from, `${edgePath}.from`, ["id", "sourceEpisodeId"], []);
                if (fromIssue)
                    return fromIssue;
                const toIssue = identityFieldsIssue(edge, edgePath, ["toObjectiveId"], []);
                if (toIssue)
                    return toIssue;
            }
        }
        if (isRecordArray(value.learningDesign.supportPlan)) {
            for (let index = 0; index < value.learningDesign.supportPlan.length; index += 1) {
                const supportIssue = identityFieldsIssue(value.learningDesign.supportPlan[index], `${learningPath}.supportPlan[${index}]`, ["objectiveId", "fadeRuleId", "escalationRuleId"], []);
                if (supportIssue)
                    return supportIssue;
            }
        }
        const delayedRefIssue = identityFieldsIssue(value.learningDesign.delayedProbeRef, `${learningPath}.delayedProbeRef`, ["probeId"], []);
        if (delayedRefIssue)
            return delayedRefIssue;
    }
    if (isPlainObject(value.masteryContract)) {
        const masteryPath = `${path}.masteryContract`;
        const policyIssue = identityFieldsIssue(value.masteryContract.evidencePolicyRef, `${masteryPath}.evidencePolicyRef`, ["key"], []);
        if (policyIssue)
            return policyIssue;
        if (isRecordArray(value.masteryContract.requirements)) {
            for (let index = 0; index < value.masteryContract.requirements.length; index += 1) {
                const requirementIssue = identityFieldsIssue(value.masteryContract.requirements[index], `${masteryPath}.requirements[${index}]`, ["objectiveId"], []);
                if (requirementIssue)
    ...
    if (isRecordArray(value.delayedProbeDefinitions)) {
        for (let index = 0; index < value.delayedProbeDefinitions.length; index += 1) {
            const definition = value.delayedProbeDefinitions[index];
            const definitionPath = `${path}.delayedProbeDefinitions[${index}]`;
            const refIssue = identityFieldsIssue(definition.ref, `${definitionPath}.ref`, ["probeId"], []);
            if (refIssue || !isPlainObject(definition.body))
                return refIssue;
            const body = definition.body;
            const bodyPath = `${definitionPath}.body`;
            const bodyIssue = identityFieldsIssue(body, bodyPath, [
                "probeId",
                "targetEpisodeId",
                "probeNodeId",
            ...
            if (bodyIssue)
                return bodyIssue;
            const bindingIssue = identityFieldsIssue(body.activityBinding, `${bodyPath}.activityBinding`, ["activityId", "progressCompatibilityKey"], []);
            if (bindingIssue)
                return bindingIssue;
            if (isPlainObject(body.activityBinding)) {
                const templateIssue = identityFieldsIssue(body.activityBinding.templateRef, `${bodyPath}.activityBinding.templateRef`, ["templateId"], []);
                if (templateIssue)
                    return templateIssue;
            }
            if (isRecordArray(body.evidenceDeclarations)) {
                for (let declarationIndex = 0; declarationIndex < body.evidenceDeclarations.length; declarationIndex += 1) {
                    const declarationIssue = declarationIdentityIssue(body.evidenceDeclarations[declarationIndex], `${bodyPath}.evidenceDeclarations[${declarationIndex}]`);
                    if (declarationIssue)
                        return declarationIssue;
                }
            }
            const contextIssue = pedagogicalContextIdentityIssue(body.pedagogicalContextContract, `${bodyPath}.pedagogicalContextContract`);
            if (contextIssue)
                return contextIssue;
        }
    }
    if (isRecordArray(value.reviewLinks)) {
        for (let index = 0; index < value.reviewLinks.length; index += 1) {
            ...
            const linkIssue = identityFieldsIssue(link, linkPath, ["targetEpisodeId", "windowPolicyId"], ["skillIds"]);
            if (linkIssue)
                return linkIssue;
            const probeIssue = identityFieldsIssue(link.probeRef, `${linkPath}.probeRef`, ["probeId"], []);
            if (probeIssue)
                return probeIssue;
        }
    }
    if (isRecordArray(value.accessibilityRoutes)) {
        for (let index = 0; index < value.accessibilityRoutes.length; index += 1) {
            const routeIssue = identityFieldsIssue(value.accessibilityRoutes[index], `${path}.accessibilityRoutes[${index}]`, ["routeId"], ["nodeIds", "reachableStarSlotIds"]);
            if (routeIssue)
                return routeIssue;
        }
    }
...
const curriculumMaterialIdentityIssue = (value, path) => identityFieldsIssue(value, path, [], [
    "skillIds",
    "phraseFrameIds",
    "grammarDistinctionIds",
    "semanticSlotIds",
...
const curriculumIdentityIssue = (value) => {
    if (!isPlainObject(value))
        return undefined;
    const path = "$.curriculum";
    const curriculumIssue = identityFieldsIssue(value, path, ["curriculumId", "seasonId"], ["requiredAssetIds", "requiredCapabilityKeys"]);
    if (curriculumIssue)
        return curriculumIssue;
    const registryIssue = identityFieldsIssue(value.decisionRegistryRef, `${path}.decisionRegistryRef`, ["id"], []);
    if (registryIssue)
        return registryIssue;
    if (isRecordArray(value.chapters)) {
        for (let index = 0; index < value.chapters.length; index += 1) {
            const chapterIssue = identityFieldsIssue(value.chapters[index], `${path}.chapters[${index}]`, ["chapterId", "checkpointEpisodeId"], ["episodeIds"]);
            if (chapterIssue)
                return chapterIssue;
        }
    }
    if (isRecordArray(value.episodeRefs)) {
        for (let index = 0; index < value.episodeRefs.length; index += 1) {
            ...
            const episodeIssue = identityFieldsIssue(episodeRef, episodePath, ["episodeId", "chapterId"], ["prerequisiteEpisodeIds"]);
            if (episodeIssue)
                return episodeIssue;
            const introducedIssue = curriculumMaterialIdentityIssue(episodeRef.introducedMaterial, `${episodePath}.introducedMaterial`);
            if (introducedIssue)
                return introducedIssue;
            const requiredIssue = curriculumMaterialIdentityIssue(episodeRef.requiredCheckpointMaterial, `${episodePath}.requiredCheckpointMaterial`);
            if (requiredIssue)
...
const learningPackageIdentityIssue = (root) => {
    if (isPlainObject(root.dependencies)) {
        const dependencies = root.dependencies;
        if (isRecordArray(dependencies.templates)) {
            for (let index = 0; index < dependencies.templates.length; index += 1) {
                const currentIssue = templateIdentityIssue(dependencies.templates[index], `$.dependencies.templates[${index}]`);
                if (currentIssue)
                    return currentIssue;
            }
        }
        if (isRecordArray(dependencies.policies)) {
            for (let index = 0; index < dependencies.policies.length; index += 1) {
                ...
                const refIssue = identityFieldsIssue(policy.ref, `${policyPath}.ref`, ["key"], []);
                if (refIssue)
                    return refIssue;
                const bodyIssue = identityFieldsIssue(policy.body, `${policyPath}.body`, ["key"], ["compatibleKernelKeys"]);
                if (bodyIssue)
                    return bodyIssue;
            }
        }
        const dependencyIssue = identityFieldsIssue(dependencies, "$.dependencies", [], ["fadeRuleIds", "escalationRuleIds", "assetIds", "contentUnitIds"]);
        if (dependencyIssue)
            return dependencyIssue;
    }
    return (episodeIdentityIssue(root.episode) ??
        curriculumIdentityIssue(root.curriculum));
};
const validateTemplateRefShape = (value, path) => {
    const shapeIssue = oneExactObjectIssue(value, ["templateId", "version", "contentHash"], [], path);
    if (shapeIssue)
        return shapeIssue;
    const ref = value;
    if (!isNonEmptyString(ref.templateId) ||
...
const validatePolicyRefShape = (value, path, expectedKind) => {
    const shapeIssue = oneExactObjectIssue(value, ["kind", "key", "version", "contentHash"], [], path);
    if (shapeIssue)
        return shapeIssue;
    const ref = value;
    if (!POLICY_KINDS.includes(ref.kind) ||
...
const validateCapabilitiesShape = (value, path) => {
    const shapeIssue = oneExactObjectIssue(value, ["microphone", "speechRecognition", "audioPlayback", "network"], [], path);
    if (shapeIssue)
        return shapeIssue;
    const capabilities = value;
    if (!["none", "optional", "required"].includes(String(capabilities.microphone)) ||
        !["none", "optional", "required"].includes(String(capabilities.speechRecognition)) ||
...
const validatePolicyDescriptorShape = (value, path) => {
    const descriptorShape = oneExactObjectIssue(value, POLICY_DESCRIPTOR_KEYS, [], path);
    if (descriptorShape)
    ...
    if (refIssue)
        return refIssue;
    const bodyShape = oneExactObjectIssue(descriptor.body, POLICY_BODY_KEYS, [], `${path}.body`);
    if (bodyShape)
        return bodyShape;
    ...
    if (body.schemaVersion !== "content-studio-policy-body.v1" ||
        !POLICY_KINDS.includes(body.kind) ||
        ...
        body.compatibleFamilies.some((family) => !activity_1.V2_ACTIVITY_FAMILIES.includes(family)) ||
        ...
        body.evidenceKinds.some((kind) => !["semantic", "listening", "recall", "spoken", "interaction"].includes(kind)) ||
        !isStringArray(body.claims) ||
        body.claims.some((claim) => !claims.includes(claim))) {
        return issue("field_value_invalid", `${path}.body`);
    }
    if (ref.kind !== body.kind ||
        ref.key !== body.key ||
        ref.version !== body.version) {
        return issue("policy_identity_mismatch", `${path}.ref`);
    }
    if (ref.contentHash !== (0, decision_registry_1.hashCanonicalBody)(body)) {
        return issue("policy_content_hash_mismatch", `${path}.ref.contentHash`);
    }
    return undefined;
};
const validateLocalizedValuesShape = (value, path) => {
    if (!isRecordArray(value) || value.length === 0)
        return issue("field_type_invalid", path);
    for (let index = 0; index < value.length; index += 1) {
        const itemPath = `${path}[${index}]`;
        const shapeIssue = oneExactObjectIssue(value[index], ["locale", "value", "sourceHash"], [], itemPath);
        if (shapeIssue)
            return shapeIssue;
        const item = value[index];
        if (!isNonEmptyString(item.locale) ||
...
const validateVersionRefShape = (value, path) => {
    const shapeIssue = oneExactObjectIssue(value, ["id", "version", "contentHash"], [], path);
    if (shapeIssue)
        return shapeIssue;
    const ref = value;
...
const validateVoiceReleaseRequirementsShape = (value, path) => {
    if (!isPlainObject(value))
        return issue("field_type_invalid", path);
    const onDevice = value.processingMode === "on_device_only";
    const network = value.processingMode === "network_allowed" ||
    ...
    const shapeIssue = oneExactObjectIssue(value, requiredKeys, ["calibrationReceiptRef"], path);
    if (shapeIssue)
        return shapeIssue;
    if (!isStringArray(value.allowedTaskTypes) ||
        value.allowedTaskTypes.length === 0 ||
        value.allowedTaskTypes.some((taskType) => !["scripted", "spontaneous"].includes(taskType)) ||
    ...
    if (hasOwn(value, "calibrationReceiptRef")) {
        ...
        const calibrationShape = oneExactObjectIssue(value.calibrationReceiptRef, ["receiptId", "version", "contentHash"], [], calibrationPath);
        if (calibrationShape)
            return calibrationShape;
        const calibration = value.calibrationReceiptRef;
        if (!isNonEmptyString(calibration.receiptId) ||
    ...
    const policyShape = oneExactObjectIssue(value.voiceDataPolicyRef, ["policyId", "version", "contentHash"], [], policyPath);
    if (policyShape)
        return policyShape;
    const policy = value.voiceDataPolicyRef;
    ...
    if (!isNonEmptyString(value.activePolicyRegistryKey) ||
        !isStringArray(value.requiredNetworkPurposes) ||
        value.requiredNetworkPurposes.length === 0 ||
        value.requiredNetworkPurposes.some((purpose) => ![
            "recognition",
            "speech_scoring",
            "club_reply",
            "safety_review",
    ...
    const egressShape = oneExactObjectIssue(value.networkEgressRef, ["gatewayId", "version", "contentHash"], [], egressPath);
    if (egressShape)
        return egressShape;
    const egress = value.networkEgressRef;
    if (egress.gatewayId !== "voice-network-egress" ||
...
exports.validateVoiceReleaseRequirementsShape = validateVoiceReleaseRequirementsShape;
const validateTemplateArtifactBodyShape = (value, path) => {
    if (!isPlainObject(value))
        return issue("field_type_invalid", path);
    if (value.schemaVersion !== "v2-mode-template-body.v1") {
        return issue("field_value_invalid", `${path}.schemaVersion`);
    }
    const bodyShape = oneExactObjectIssue(value, TEMPLATE_BODY_KEYS, TEMPLATE_BODY_OPTIONAL_KEYS, path);
    if (bodyShape)
        return bodyShape;
    if (!isNonEmptyString(value.templateId) ||
        !isPositiveInteger(value.version) ||
        !activity_1.V2_ACTIVITY_FAMILIES.includes(value.family) ||
        !isNonEmptyString(value.humanName) ||
        !isNonEmptyString(value.description)) {
        return issue("field_value_invalid", path);
    }
    const purposePath = `${path}.pedagogicalPurpose`;
    const purposeShape = oneExactObjectIssue(value.pedagogicalPurpose, TEMPLATE_PURPOSE_KEYS, [], purposePath);
    if (purposeShape)
        return purposeShape;
    const purpose = value.pedagogicalPurpose;
    const purposePhases = [
        "discover",
        "comprehend",
    ...
    if (!purposePhases.includes(String(purpose.phase)) ||
    ...
    const invalidPrimarySkillIndex = purpose.primarySkillIds.findIndex((skillId) => !identities_1.V2_IDENTITY_REGEX.test(skillId));
    if (invalidPrimarySkillIndex >= 0) {
        return issue("identity_invalid", `${purposePath}.primarySkillIds[${invalidPrimarySkillIndex}]`);
    }
    const kernelPath = `${path}.kernel`;
    const kernelShape = oneExactObjectIssue(value.kernel, TEMPLATE_KERNEL_KEYS, [], kernelPath);
    if (kernelShape)
        return kernelShape;
    const kernel = value.kernel;
    if (!isNonEmptyString(kernel.activityTypeKey) ||
    ...
    const policyExtra = Object.keys(value.policies)
        .filter((key) => !POLICY_KINDS.includes(key))
        .sort()[0];
    if (policyExtra)
        return issue("activity_policy_set_invalid", `${path}.policies.${policyExtra}`);
    for (const kind of POLICY_KINDS) {
        if (!hasOwn(value.policies, kind)) {
            return issue("activity_policy_set_invalid", `${path}.policies.${kind}`);
        }
        const policyRefIssue = validatePolicyRefShape(value.policies[kind], `${path}.policies.${kind}`);
        if (policyRefIssue)
            return policyRefIssue;
        if (value.policies[kind].kind !== kind) {
            return issue("activity_policy_ref_mismatch", `${path}.policies.${kind}.kind`);
        }
    }
    const authoringPath = `${path}.authoring`;
    const authoringShape = oneExactObjectIssue(value.authoring, TEMPLATE_AUTHORING_KEYS, [], authoringPath);
    if (authoringShape)
        return authoringShape;
    const authoring = value.authoring;
    if (!isStringArray(authoring.editableFieldPaths) ||
    ...
    const capabilityShape = oneExactObjectIssue(value.capabilityContract, TEMPLATE_CAPABILITY_KEYS, ["fallbackTemplateRef"], capabilityPath);
    if (capabilityShape)
        return capabilityShape;
    const capability = value.capabilityContract;
    if (!["none", "optional", "required"].includes(String(capability.microphone)) ||
        !["none", "optional", "required"].includes(String(capability.speechRecognition)) ||
    ...
    if (hasOwn(capability, "fallbackTemplateRef")) {
        const fallbackIssue = validateTemplateRefShape(capability.fallbackTemplateRef, `${capabilityPath}.fallbackTemplateRef`);
        if (fallbackIssue)
            return fallbackIssue;
    }
    const claims = [
    ...
    if (!isStringArray(value.evidenceClaims) ||
    ...
    const copyShape = oneExactObjectIssue(value.learnerCopy, TEMPLATE_COPY_KEYS, [], copyPath);
    if (copyShape)
        return copyShape;
    const copy = value.learnerCopy;
    for (const key of TEMPLATE_COPY_KEYS) {
        const localizedIssue = validateLocalizedValuesShape(copy[key], `${copyPath}.${key}`);
        if (localizedIssue)
            return localizedIssue;
        const locales = copy[key].map((entry) => entry.locale);
    ...
    for (let index = 0; index < value.fixtures.length; index += 1) {
        ...
        const fixtureShape = oneExactObjectIssue(value.fixtures[index], TEMPLATE_FIXTURE_KEYS, ["expectedResultKind"], fixturePath);
        if (fixtureShape)
            return fixtureShape;
    ...
    const compatibilityShape = oneExactObjectIssue(value.compatibility, TEMPLATE_COMPATIBILITY_KEYS, [], compatibilityPath);
    if (compatibilityShape)
        return compatibilityShape;
    const compatibility = value.compatibility;
    if (!isNonEmptyString(compatibility.minAppVersion) ||
        !isStringArray(compatibility.requiredSupportManifestHashes) ||
        compatibility.requiredSupportManifestHashes.length === 0 ||
        compatibility.requiredSupportManifestHashes.some((hash) => !HASH_PATTERN.test(hash)) ||
    ...
    const learningShape = oneExactObjectIssue(value.learningContractRefs, TEMPLATE_LEARNING_REF_KEYS, [], learningPath);
    if (learningShape)
        return learningShape;
    const learningRefs = value.learningContractRefs;
    for (const key of TEMPLATE_LEARNING_REF_KEYS) {
        const refIssue = validateVersionRefShape(learningRefs[key], `${learningPath}.${key}`);
        if (refIssue)
            return refIssue;
    }
    if (hasOwn(value, "voiceReleaseRequirements")) {
        return (0, exports.validateVoiceReleaseRequirementsShape)(value.voiceReleaseRequirements, `${path}.voiceReleaseRequirements`);
    }
    return undefined;
};
/** Public adapter for immutable Content Studio ModeTemplate resolvers. */
const validateModeTemplateArtifactBody = (input) => {
    const issueResult = validateTemplateArtifactBodyShape(input, "$.template");
    return issueResult
        ? fail([issueResult])
        : pass(input);
};
exports.validateModeTemplateArtifactBody = validateModeTemplateArtifactBody;
/** Shared strict validator for the mutable ModeTemplate lifecycle projection. */
const validateModeTemplateLifecycleHead = (input) => {
    ...
    if (input.schemaVersion !== "v2-mode-template-lifecycle.v1" ||
    ...
    if (hasNoReplacement && input.noReplacement !== true)
        return fail([
            issue("mode_template_lifecycle_no_replacement_invalid", "$.noReplacement"),
        ]);
    if (hasReplacement) {
        if (!isPlainObject(input.replacementRef))
            return fail([
                issue("mode_template_lifecycle_replacement_invalid", "$.replacementRef"),
            ]);
        const replacement = input.replacementRef;
        if (Object.keys(replacement).some((key) => !["templateId", "version", "contentHash"].includes(key)) ||
        ...
        if (replacement.templateId === input.templateId &&
            replacement.version === input.version &&
            replacement.contentHash === input.contentHash)
            return fail([
                issue("mode_template_lifecycle_replacement_self_reference", "$.replacementRef"),
            ]);
    }
...
exports.validateModeTemplateLifecycleHead = validateModeTemplateLifecycleHead;
...
const validateContentReceiptSubject = (input) => {
    if (!isPlainObject(input) ||
        Object.keys(input).length !== 4 ||
        !["entityType", "entityId", "entityRevision", "entityFingerprint"].every((key) => hasOwn(input, key)) ||
...
exports.validateContentReceiptSubject = validateContentReceiptSubject;
const validateContentGateReceiptBody = (input) => {
    if (!isPlainObject(input))
    ...
    const subject = (0, exports.validateContentReceiptSubject)(input.subject);
    if (!subject.ok)
        return fail(subject.issues);
    if (input.schemaVersion !== "content-gate-receipt-body.v1" ||
        ...
        ![
            ...
            "reviewReceiptHash",
            "waiverSetHash",
        ].every((key) => typeof input[key] === "string" && HASH_PATTERN.test(String(input[key]))) ||
    ...
    if (hasOwn(input, "devicePreviewReceiptHashes")) {
        ...
        if (!isPlainObject(previews) ||
            ...
            return fail([
                issue("content_gate_receipt_preview_hashes_invalid", "$.devicePreviewReceiptHashes"),
            ]);
    }
    return pass(input);
};
exports.validateContentGateReceiptBody = validateContentGateReceiptBody;
const validateEvidenceDeclarationShape = (value, path) => {
    const declarationShape = oneExactObjectIssue(value, DECLARATION_KEYS, [], path);
    if (declarationShape)
        return declarationShape;
    const declaration = value;
    if (!isNonEmptyString(declaration.objectiveId) ||
        !isNonEmptyString(declaration.skillId) ||
        !["semantic", "listening", "recall", "spoken", "interaction"].includes(String(declaration.construct)) ||
        ![
            "encounter_build",
            "near_transfer",
            "independent_probe",
        ...
        return issue("field_value_invalid", !["semantic", "listening", "recall", "spoken", "interaction"].includes(String(declaration.construct))
            ? `${path}.construct`
            : path);
    }
    const targetPath = `${path}.target`;
    const targetShape = oneExactObjectIssue(declaration.target, ["targetKind", "targetId"], [], targetPath);
    if (targetShape)
        return targetShape;
    const target = declaration.target;
    if (!["objective", "semantic_slot", "critical_constraint"].includes(String(target.targetKind))) {
...
const validatePedagogicalContextShape = (value, path, expectedPhase) => {
    const shapeIssue = oneExactObjectIssue(value, [
        "phase",
    ...
    const contextShape = oneExactObjectIssue(contract.context, ["contextId", "surfaceFormId", "novelty"], ["newSurfaceForm"], `${path}.context`);
    if (contextShape)
        return contextShape;
    const promptShape = oneExactObjectIssue(contract.prompt, ["promptId", "reusedFromTraining"], ["separatePrompt"], `${path}.prompt`);
    if (promptShape)
        return promptShape;
    ...
    if (expectedPhase === "independent_probe" ||
        expectedPhase === "delayed_probe") {
        const assessmentSupportLevels = ["partial_cue", "visual_only", "none"];
        if (contract.allowedSupportLevels.length === 0 ||
            uniqueSecondIndex(contract.allowedSupportLevels) >= 0 ||
            contract.allowedSupportLevels.some((level) => !assessmentSupportLevels.includes(level))) {
            return issue(expectedPhase === "delayed_probe"
                ? "delayed_probe_context_invalid"
                : "independent_context_invalid", path);
        }
    }
    if (contract.allowedSupportLevels.some((level) => !supportLevels.includes(level)) ||
...
const validateFallbackShape = (value, path) => {
    const shapeIssue = oneExactObjectIssue(value, [
        "policy",
        "reasonCodes",
        "coreCompletionEquivalent",
        "voiceEvidenceEquivalent",
    ], ["alternateNodeId"], path);
    if (shapeIssue)
        return shapeIssue;
    ...
    if (!["same_node", "alternate_node"].includes(String(fallback.policy)) ||
...
const validateActivityDeepShape = (activity, path) => {
    const templateRefIssue = validateTemplateRefShape(activity.templateRef, `${path}.templateRef`);
    if (templateRefIssue)
        return templateRefIssue;
    const capabilityIssue = validateCapabilitiesShape(activity.capabilities, `${path}.capabilities`);
    if (capabilityIssue)
        return capabilityIssue;
    if (!activity_1.V2_ACTIVITY_FAMILIES.includes(activity.family)) {
    ...
    if (!isNonEmptyString(activity.activityId) ||
        ...
        return issue("field_value_invalid", !isPositiveInteger(activity.estimatedSeconds)
            ? `${path}.estimatedSeconds`
            : path);
    }
    if (!isStringArray(activity.contentUnitIds) ||
        !isStringArray(activity.assetIds)) {
        return issue("field_type_invalid", path);
    }
    const requirementsShape = oneExactObjectIssue(activity.requirements, ["prompt", "media", "input", "fallback"], [], `${path}.requirements`);
    if (requirementsShape)
        return requirementsShape;
    const requirements = activity.requirements;
    const fallbackShape = oneExactObjectIssue(requirements.fallback, ["deterministicScripted", "offline", "nonVoiceCoreEquivalent"], [], `${path}.requirements.fallback`);
    if (fallbackShape)
        return fallbackShape;
    const fallback = requirements.fallback;
    if (!isStringArray(requirements.prompt) ||
        ...
        !["full", "cached_assets_only", "not_supported"].includes(String(fallback.offline)) ||
        typeof fallback.nonVoiceCoreEquivalent !== "boolean") {
        return issue("field_type_invalid", `${path}.requirements`);
    }
    const targetsShape = oneExactObjectIssue(activity.targets, ["skillIds", "phraseFrameIds", "semanticSlotIds"], [], `${path}.targets`);
    if (targetsShape)
        return targetsShape;
    const targets = activity.targets;
    if (!isStringArray(targets.skillIds) ||
    ...
    if (activity.tags.modalities.some((value) => !["reading", "listening", "writing", "speaking"].includes(value))) {
        return issue("field_value_invalid", `${path}.tags.modalities`);
    }
    return undefined;
};
const validateEpisodeDeepShape = (episode) => {
    if (!isNonEmptyString(episode.episodeId) ||
    ...
    for (const key of [
        ...
        const localizedIssue = validateLocalizedValuesShape(episode.scenario[key], `$.episode.scenario.${key}`);
        if (localizedIssue)
            return [localizedIssue];
    }
    if (!isNonEmptyString(episode.scenario.scenarioId) ||
    ...
    for (let index = 0; index < episode.phraseFrames.length; index += 1) {
        const phrase = episode.phraseFrames[index];
        const phrasePath = `$.episode.phraseFrames[${index}]`;
        if (!isNonEmptyString(phrase.phraseFrameId) ||
        ...
        const localizedIssue = validateLocalizedValuesShape(phrase.learnerMeaning, `${phrasePath}.learnerMeaning`);
        if (localizedIssue)
            return [localizedIssue];
    }
    for (let index = 0; index < episode.semanticSlots.length; index += 1) {
        const slot = episode.semanticSlots[index];
        const slotPath = `$.episode.semanticSlots[${index}]`;
        if (!isNonEmptyString(slot.semanticSlotId) ||
        ...
        const localizedIssue = validateLocalizedValuesShape(slot.role, `${slotPath}.role`);
        if (localizedIssue)
            return [localizedIssue];
    }
    ...
    for (let index = 0; index < episode.criticalConstraints.length; index += 1) {
        ...
        const shapeIssue = oneExactObjectIssue(episode.criticalConstraints[index], ["criticalConstraintId", "description"], [], path);
        if (shapeIssue)
            return [shapeIssue];
    }
    for (let index = 0; index < episode.activities.length; index += 1) {
        const activityIssue = validateActivityDeepShape(episode.activities[index], `$.episode.activities[${index}]`);
        if (activityIssue)
            return [activityIssue];
    }
    const graph = episode.graph;
    ...
    if (!isNonEmptyString(graph.startNodeId) ||
    ...
    for (let index = 0; index < nodes.length; index += 1) {
        ...
        if (hasOwn(node, "fallback")) {
            const fallbackIssue = validateFallbackShape(node.fallback, `${nodePath}.fallback`);
            if (fallbackIssue)
                return [fallbackIssue];
        }
        const declarations = node.evidenceDeclarations;
        for (let declarationIndex = 0; declarationIndex < declarations.length; declarationIndex += 1) {
            const declarationIssue = validateEvidenceDeclarationShape(declarations[declarationIndex], `${nodePath}.evidenceDeclarations[${declarationIndex}]`);
            if (declarationIssue)
                return [declarationIssue];
        }
        if (node.phase !== "optional_review") {
            const contextIssue = validatePedagogicalContextShape(node.pedagogicalContextContract, `${nodePath}.pedagogicalContextContract`, node.phase);
            if (contextIssue)
                return [contextIssue];
        }
    }
    ...
    for (let index = 0; index < episode.starSlots.length; index += 1) {
        ...
        const shapeIssue = oneExactObjectIssue(episode.starSlots[index], ["starSlotId", "maxStars", "acceptedNodeIds"], [], path);
        if (shapeIssue)
            return [shapeIssue];
        if (!isNonEmptyString(episode.starSlots[index].starSlotId) ||
    ...
    for (const key of [
        ...
        if (!isStringArray(episode.capstoneContract[key])) {
            return [issue("field_type_invalid", `$.episode.capstoneContract.${key}`)];
        }
    }
    const learningShape = oneExactObjectIssue(episode.learningDesign, [
        "primaryOutcomeId",
        "objectiveIds",
        "prerequisiteEdges",
        "supportPlan",
    ...
    if (learningShape)
        return [learningShape];
    const learning = episode.learningDesign;
    if (!isNonEmptyString(learning.primaryOutcomeId) ||
    ...
    if (!isPlainObject(learning.delayedProbeRef)) {
        return [
            issue("delayed_probe_ref_invalid", "$.episode.learningDesign.delayedProbeRef"),
        ];
    }
    const delayedRefShape = oneExactObjectIssue(learning.delayedProbeRef, ["probeId", "contentHash"], [], "$.episode.learningDesign.delayedProbeRef");
    if (delayedRefShape)
        return [delayedRefShape];
    for (let index = 0; index < learning.prerequisiteEdges.length; index += 1) {
        const path = `$.episode.learningDesign.prerequisiteEdges[${index}]`;
        const edgeShape = oneExactObjectIssue(learning.prerequisiteEdges[index], ["from", "toObjectiveId", "requiredState"], [], path);
        if (edgeShape)
            return [edgeShape];
        const fromShape = oneExactObjectIssue(learning.prerequisiteEdges[index].from, ["kind", "id", "sourceEpisodeId"], [], `${path}.from`);
        if (fromShape)
            return [fromShape];
    }
    for (let index = 0; index < learning.supportPlan.length; index += 1) {
        const path = `$.episode.learningDesign.supportPlan[${index}]`;
        const supportShape = oneExactObjectIssue(learning.supportPlan[index], ["objectiveId", "initialSupport", "fadeRuleId", "escalationRuleId"], [], path);
        if (supportShape)
            return [supportShape];
    }
    const masteryShape = oneExactObjectIssue(episode.masteryContract, [
        "evidencePolicyRef",
        "requirements",
    ...
    if (masteryShape)
        return [masteryShape];
    const mastery = episode.masteryContract;
    const masteryPolicyIssue = validatePolicyRefShape(mastery.evidencePolicyRef, "$.episode.masteryContract.evidencePolicyRef", "evidence");
    if (masteryPolicyIssue)
        return [masteryPolicyIssue];
    const masteryContractLiterals = [
    ...
    if (mastery.requirements.length === 0) {
        return [
            issue("mastery_requirement_invalid", "$.episode.masteryContract.requirements"),
        ];
    }
    const requirementIdentities = [];
    for (let index = 0; index < mastery.requirements.length; index += 1) {
        const path = `$.episode.masteryContract.requirements[${index}]`;
        const requirementShape = oneExactObjectIssue(mastery.requirements[index], [
            "objectiveId",
            "construct",
            "phase",
            "requiredValidity",
    ...
    if (!mastery.requirements.some((requirement) => requirement.phase === "independent_probe") ||
        !mastery.requirements.some((requirement) => requirement.phase === "delayed_probe")) {
        return [
            issue("mastery_requirement_invalid", "$.episode.masteryContract.requirements"),
        ];
    }
    ...
    for (let index = 0; index < episode.delayedProbeDefinitions.length; index += 1) {
        const definition = episode.delayedProbeDefinitions[index];
        const path = `$.episode.delayedProbeDefinitions[${index}]`;
        const pairShape = oneExactObjectIssue(definition, ["ref", "body"], [], path);
        if (pairShape)
            return [pairShape];
        const refShape = oneExactObjectIssue(definition.ref, ["probeId", "contentHash"], [], `${path}.ref`);
        if (refShape)
            return [refShape];
        const bodyShape = oneExactObjectIssue(definition.body, [
            "schemaVersion",
            "probeId",
            "targetEpisodeId",
            "probeNodeId",
            ...
            "evidenceDeclarations",
            "pedagogicalContextContract",
        ], ["accessibilityAlternateActivityId"], `${path}.body`);
        if (bodyShape)
            return [bodyShape];
        const body = definition.body;
        const bindingShape = oneExactObjectIssue(body.activityBinding, ["activityId", "progressCompatibilityKey", "templateRef"], [], `${path}.body.activityBinding`);
        if (bindingShape)
            return [bindingShape];
        const bindingRefIssue = validateTemplateRefShape(body.activityBinding.templateRef, `${path}.body.activityBinding.templateRef`);
        if (bindingRefIssue)
            return [bindingRefIssue];
        if (!isRecordArray(body.evidenceDeclarations))
            return [issue("field_type_invalid", `${path}.body.evidenceDeclarations`)];
        for (let declarationIndex = 0; declarationIndex < body.evidenceDeclarations.length; declarationIndex += 1) {
            const declarationIssue = validateEvidenceDeclarationShape(body.evidenceDeclarations[declarationIndex], `${path}.body.evidenceDeclarations[${declarationIndex}]`);
            if (declarationIssue)
                return [declarationIssue];
        }
        const contextIssue = validatePedagogicalContextShape(body.pedagogicalContextContract, `${path}.body.pedagogicalContextContract`, "delayed_probe");
        if (contextIssue)
            return [contextIssue];
    }
    for (let index = 0; index < episode.reviewLinks.length; index += 1) {
        ...
        if (materializedKey)
            return [
                issue("review_link_materialization_forbidden", `${path}.${materializedKey}`),
            ];
        const requiredKeys = link.scheduleKind === "optional_review"
    ...
    for (let index = 0; index < episode.accessibilityRoutes.length; index += 1) {
        ...
        const routeShape = oneExactObjectIssue(route, [
            "routeId",
            "routeKind",
            "nodeIds",
        ...
        if (!isNonEmptyString(route.routeId) ||
            !["primary", "accessibility", "capability_fallback"].includes(String(route.routeKind)) ||
    ...
    if (!episode.accessibilityRoutes.some((route) => route.routeKind === "primary")) {
        return [
            issue("accessibility_primary_route_missing", "$.episode.accessibilityRoutes"),
        ];
    }
    return [];
};
const validateCurriculumDeepShape = (input) => {
    if (!isPlainObject(input))
        return [issue("field_type_invalid", "$.curriculum")];
    const curriculum = input;
    const scopeShape = oneExactObjectIssue(curriculum.scope, ["kind", "includedChapterOrdinals", "includedEpisodeOrdinals"], [], "$.curriculum.scope");
    if (scopeShape)
        return [scopeShape];
    if (!isStringArray(curriculum.requiredLocales) ||
    ...
    if (!isNonEmptyString(curriculum.studyTarget)) {
        return [
            issue(typeof curriculum.studyTarget === "string"
                ? "field_value_invalid"
                : "field_type_invalid", "$.curriculum.studyTarget"),
        ];
    }
    if (!isNonEmptyString(curriculum.learnerSourceLocale)) {
        return [
            issue(typeof curriculum.learnerSourceLocale === "string"
                ? "field_value_invalid"
                : "field_type_invalid", "$.curriculum.learnerSourceLocale"),
        ];
    }
    if (curriculum.requiredLocales.length === 0 ||
        ...
        return [
            issue("curriculum_locale_closure_invalid", "$.curriculum.requiredLocales"),
        ];
    }
    if (curriculum.requiredCapabilityKeys.some((key) => !isNonEmptyString(key)) ||
        uniqueSecondIndex(curriculum.requiredCapabilityKeys) >= 0) {
        return [
            issue("curriculum_capability_closure_invalid", "$.curriculum.requiredCapabilityKeys"),
        ];
    }
    ...
    for (let index = 0; index < curriculum.chapters.length; index += 1) {
        ...
        const chapterShape = oneExactObjectIssue(chapter, ["chapterId", "ordinal", "episodeIds"], ["checkpointEpisodeId"], path);
        if (chapterShape)
            return [chapterShape];
    ...
    for (let index = 0; index < curriculum.episodeRefs.length; index += 1) {
        ...
        const refShape = oneExactObjectIssue(ref, [
            "episodeId",
            "ordinal",
            "chapterId",
            "episodeKind",
        ...
        if (refShape)
            return [refShape];
        if (!isNonEmptyString(ref.episodeId) ||
        ...
        for (const materialKey of [
            ...
            const materialShape = oneExactObjectIssue(ref[materialKey], materialKeys, [], materialPath);
            if (materialShape)
                return [materialShape];
            for (const key of materialKeys) {
                if (!isStringArray(ref[materialKey][key])) {
...
const validatePackageDeepShape = (root) => {
    const dependencies = root.dependencies;
    const templates = dependencies.templates;
    for (let index = 0; index < templates.length; index += 1) {
        const template = templates[index];
        const path = `$.dependencies.templates[${index}]`;
        const templateRefIssue = validateTemplateRefShape(template.templateRef, `${path}.templateRef`);
        if (templateRefIssue)
            return [templateRefIssue];
        const bodyShapeIssue = validateTemplateArtifactBodyShape(template.body, `${path}.body`);
        if (bodyShapeIssue)
            return [bodyShapeIssue];
        const body = template.body;
        const templateRef = template.templateRef;
        if (templateRef.templateId !== body.templateId ||
            templateRef.version !== body.version) {
            return [issue("template_identity_mismatch", `${path}.templateRef`)];
        }
    }
    for (let index = 0; index < templates.length; index += 1) {
        const capabilityContract = templates[index].body
            .capabilityContract;
        const fallbackTemplateRef = capabilityContract.fallbackTemplateRef;
        if (fallbackTemplateRef !== undefined &&
            !templates.some((candidate) => sameValue(candidate.templateRef, fallbackTemplateRef))) {
            return [
                issue("template_fallback_reference_missing", `$.dependencies.templates[${index}].body.capabilityContract.fallbackTemplateRef`),
            ];
        }
    }
    const policies = dependencies.policies;
    for (let index = 0; index < policies.length; index += 1) {
        const policyIssue = validatePolicyDescriptorShape(policies[index], `$.dependencies.policies[${index}]`);
        if (policyIssue)
            return [policyIssue];
    }
    const episodeIssues = validateEpisodeDeepShape(root.episode);
    if (episodeIssues.length > 0)
        return episodeIssues;
    return validateCurriculumDeepShape(root.curriculum);
};
const validatePoliciesAndActivities = (root) => {
    const dependencies = root.dependencies;
    const templates = dependencies.templates;
    const policies = dependencies.policies;
    const episode = root.episode;
    const activities = episode.activities;
    const duplicatePolicyIndex = uniqueSecondIndex(policies.map((candidate) => {
        const ref = candidate.ref;
        return `${String(ref.kind)}\u0000${String(ref.key)}\u0000${String(ref.version)}`;
    }));
    if (duplicatePolicyIndex >= 0) {
        return [
            issue("dependency_duplicate_id", `$.dependencies.policies[${duplicatePolicyIndex}]`),
        ];
    }
    const duplicateTemplateIndex = uniqueSecondIndex(templates.map((candidate) => {
    ...
    if (duplicateTemplateIndex >= 0) {
        return [
            issue("dependency_duplicate_id", `$.dependencies.templates[${duplicateTemplateIndex}]`),
        ];
    }
    for (let templateIndex = 0; templateIndex < templates.length; templateIndex += 1) {
        ...
        for (const kind of POLICY_KINDS) {
            ...
            const sameKind = policies.filter((candidate) => candidate.ref.kind === kind);
            if (sameKind.length === 0)
                return [issue("activity_policy_ref_mismatch", refPath)];
            const sameKey = sameKind.filter((candidate) => candidate.ref.key === ref.key);
            if (sameKey.length === 0)
                return [issue("activity_policy_ref_mismatch", `${refPath}.key`)];
            const sameVersion = sameKey.filter((candidate) => candidate.ref.version === ref.version);
            if (sameVersion.length === 0)
                return [issue("activity_policy_ref_mismatch", `${refPath}.version`)];
            const resolvedPolicy = sameVersion.find((candidate) => candidate.ref.contentHash === ref.contentHash);
            if (!resolvedPolicy) {
                return [
                    issue("activity_policy_ref_mismatch", `${refPath}.contentHash`),
                ];
            }
            const resolvedBody = resolvedPolicy.body;
            const kernel = templateBody.kernel;
            if (!resolvedBody.compatibleFamilies.includes(String(templateBody.family)) ||
                !resolvedBody.compatibleKernelKeys.includes(String(kernel.activityTypeKey))) {
                return [issue("policy_compatibility_mismatch", refPath)];
            }
        }
        if (template.templateRef.contentHash !==
            (0, decision_registry_1.hashCanonicalBody)(templateBody)) {
            return [
                issue("template_content_hash_mismatch", `$.dependencies.templates[${templateIndex}].templateRef.contentHash`),
            ];
        }
    }
    const duplicateActivityIndex = uniqueSecondIndex(activities.map((activity) => activity.activityId));
    if (duplicateActivityIndex >= 0) {
        return [
            issue("graph_duplicate_id", `$.episode.activities[${duplicateActivityIndex}].activityId`),
        ];
    }
    ...
    for (let activityIndex = 0; activityIndex < activities.length; activityIndex += 1) {
        ...
        if (typeof activity.activityId !== "string" ||
            !identities_1.V2_IDENTITY_REGEX.test(activity.activityId)) {
            return [issue("identity_invalid", `${activityPath}.activityId`)];
        }
        if (!isStringArray(activity.assetIds) ||
            !isStringArray(activity.contentUnitIds)) {
            return [issue("field_type_invalid", activityPath)];
        }
        for (let assetIndex = 0; assetIndex < activity.assetIds.length; assetIndex += 1) {
            if (!dependencyAssetIds.includes(activity.assetIds[assetIndex])) {
                return [
                    issue("asset_reference_missing", `${activityPath}.assetIds[${assetIndex}]`),
                ];
            }
        }
        for (let contentIndex = 0; contentIndex < activity.contentUnitIds.length; contentIndex += 1) {
            if (!dependencyContentUnitIds.includes(activity.contentUnitIds[contentIndex])) {
                return [
                    issue("content_reference_missing", `${activityPath}.contentUnitIds[${contentIndex}]`),
                ];
            }
        }
        if (!activity_1.V2_ACTIVITY_FAMILIES.includes(activity.family)) {
        ...
        const templateBody = template.body;
        const kernel = templateBody.kernel;
        const capability = templateBody.capabilityContract;
...
const validateNodePhases = (episode) => {
    const graph = episode.graph;
    const nodes = graph.nodes;
    ...
    for (let index = 0; index < nodes.length; index += 1) {
        ...
        if (!["encounter_build", "near_transfer", "independent_probe"].includes(String(node.phase))) {
            return [issue("phase_contract_mismatch", `${nodePath}.phase`)];
        }
        if (!isPlainObject(node.pedagogicalContextContract) ||
            node.pedagogicalContextContract.phase !== node.phase) {
            return [
                issue("phase_contract_mismatch", `${nodePath}.pedagogicalContextContract.phase`),
            ];
        }
        for (let declarationIndex = 0; declarationIndex < declarations.length; declarationIndex += 1) {
            if (declarations[declarationIndex].phase !== node.phase) {
                return [
                    issue("phase_contract_mismatch", `${nodePath}.evidenceDeclarations[${declarationIndex}].phase`),
                ];
            }
        }
        const declarationKeys = declarations.map((declaration) => (0, decision_registry_1.canonicalJsonV1)(declaration));
        const duplicateDeclarationIndex = uniqueSecondIndex(declarationKeys);
        if (duplicateDeclarationIndex >= 0) {
            return [
                issue("evidence_tuple_duplicate", `${nodePath}.evidenceDeclarations[${duplicateDeclarationIndex}]`),
            ];
        }
        if (node.phase === "independent_probe" &&
...
const validateAssessmentProjection = (episode) => {
    const graph = episode.graph;
    const nodes = graph.nodes;
    const assessment = episode.assessmentNodes;
    const checks = [
        ["independentProbeNodeIds", "independent_probe"],
        ["optionalReviewNodeIds", "optional_review"],
    ];
    for (const [key, phase] of checks) {
        const declaredIds = assessment[key];
        ...
        if (duplicateIndex >= 0) {
            return [
                issue("assessment_node_invalid", `$.episode.assessmentNodes.${key}[${duplicateIndex}]`),
            ];
        }
        for (let index = 0; index < declaredIds.length; index += 1) {
            const node = nodes.find((candidate) => candidate.nodeId === declaredIds[index]);
            if (!node || node.phase !== phase) {
                return [
                    issue("assessment_node_invalid", `$.episode.assessmentNodes.${key}[${index}]`),
                ];
            }
        }
    ...
    const independentRef = episode.learningDesign
        .independentProbeRef;
    if (!assessment.independentProbeNodeIds.includes(String(independentRef))) {
        return [
            issue("independent_probe_ref_invalid", "$.episode.learningDesign.independentProbeRef"),
        ];
    }
    return [];
};
const validateEpisodePackageBoundary = (root) => {
    const episode = root.episode;
    ...
    if (episode.episodeKind === "checkpoint") {
        if (!isPlainObject(episode.graph) ||
            !isRecordArray(episode.graph.nodes) ||
            episode.graph.nodes.length !== 9 ||
            episode.graph.nodes.filter((node) => node.visible === true).length !== 9) {
            return [
    ...
    if (isRecordArray(curriculum.episodeRefs)) {
        const ref = curriculum.episodeRefs.find((candidate) => candidate.episodeId === episode.episodeId);
        if (!ref || ref.ordinal !== episode.ordinal) {
            return [issue("episode_identity_mismatch", "$.episode.ordinal")];
        }
        if (ref.chapterId !== episode.chapterId ||
...
const collectLocalizedValueSets = (value, path, output) => {
    if (Array.isArray(value)) {
        if (value.length > 0 &&
            ...
            output.push({
                path,
                locales: value.map((entry) => String(entry.locale)),
            });
        }
        value.forEach((entry, index) => collectLocalizedValueSets(entry, `${path}[${index}]`, output));
        return;
    }
    if (isPlainObject(value)) {
...
const detectDirectedCycle = (adjacency) => {
...
const validateLearningReferences = (episode, dependencies) => {
    const objectiveIds = episode.objectiveIds;
    const skillIds = episode.skillIds;
    const phraseFrames = episode.phraseFrames;
    const semanticSlots = episode.semanticSlots;
    const constraintIds = episode.criticalConstraints.map((constraint) => String(constraint.criticalConstraintId));
    ...
    const contentUnitIds = dependencies.contentUnitIds;
    const graph = episode.graph;
    const nodes = graph.nodes;
    const nodeIds = nodes.map((node) => String(node.nodeId));
    for (let phraseIndex = 0; phraseIndex < phraseFrames.length; phraseIndex += 1) {
        const phrase = phraseFrames[phraseIndex];
        for (let index = 0; index < phrase.semanticSlotIds.length; index += 1) {
            if (!slotIds.includes(phrase.semanticSlotIds[index])) {
                return [
                    issue("content_reference_missing", `$.episode.phraseFrames[${phraseIndex}].semanticSlotIds[${index}]`),
                ];
            }
        }
        for (let index = 0; index < phrase.skillIds.length; index += 1) {
            if (!skillIds.includes(phrase.skillIds[index])) {
                return [
                    issue("content_reference_missing", `$.episode.phraseFrames[${phraseIndex}].skillIds[${index}]`),
                ];
            }
        }
    }
    for (let slotIndex = 0; slotIndex < semanticSlots.length; slotIndex += 1) {
        const allowed = semanticSlots[slotIndex].allowedContentUnitIds;
        for (let index = 0; index < allowed.length; index += 1) {
            if (!contentUnitIds.includes(allowed[index])) {
                return [
                    issue("content_reference_missing", `$.episode.semanticSlots[${slotIndex}].allowedContentUnitIds[${index}]`),
                ];
            }
        }
    }
    const validateDeclaration = (declaration, path) => {
        ...
        const validTarget = target.targetKind === "objective"
    ...
    for (let nodeIndex = 0; nodeIndex < nodes.length; nodeIndex += 1) {
        const declarations = nodes[nodeIndex].evidenceDeclarations;
        for (let declarationIndex = 0; declarationIndex < declarations.length; declarationIndex += 1) {
            const declarationIssue = validateDeclaration(declarations[declarationIndex], `$.episode.graph.nodes[${nodeIndex}].evidenceDeclarations[${declarationIndex}]`);
            if (declarationIssue)
                return [declarationIssue];
        }
    }
    ...
    for (let activityIndex = 0; activityIndex < activities.length; activityIndex += 1) {
        ...
        for (const [key, published] of [
            ...
            const missingIndex = values.findIndex((value) => !published.includes(value));
            if (missingIndex >= 0)
                return [
                    issue("content_reference_missing", `${path}.targets.${key}[${missingIndex}]`),
                ];
        }
        const requirements = activity.requirements;
        const payload = isPlainObject(activity.payload) ? activity.payload : {};
        if (isNonEmptyString(payload.promptId) &&
            !requirements.prompt.includes(payload.promptId)) {
            return [
                issue("activity_requirement_mismatch", `${path}.requirements.prompt`),
            ];
        }
        if (isNonEmptyString(payload.inputMode) &&
            !requirements.input.includes(payload.inputMode)) {
            return [
                issue("activity_requirement_mismatch", `${path}.requirements.input`),
            ];
        }
        if (requirements.media.some((assetId) => !activity.assetIds.includes(assetId))) {
            return [
                issue("activity_requirement_mismatch", `${path}.requirements.media`),
            ];
        }
        if (isStringArray(payload.phraseFrameIds) &&
            !sameStringSet(payload.phraseFrameIds, targets.phraseFrameIds)) {
            return [
                issue("activity_target_mismatch", `${path}.targets.phraseFrameIds`),
            ];
        }
    }
    const delayedDefinitions = episode.delayedProbeDefinitions;
    for (let definitionIndex = 0; definitionIndex < delayedDefinitions.length; definitionIndex += 1) {
        const body = delayedDefinitions[definitionIndex].body;
        const declarations = body.evidenceDeclarations;
        for (let declarationIndex = 0; declarationIndex < declarations.length; declarationIndex += 1) {
            const declarationIssue = validateDeclaration(declarations[declarationIndex], `$.episode.delayedProbeDefinitions[${definitionIndex}].body.evidenceDeclarations[${declarationIndex}]`);
            if (declarationIssue)
                return [declarationIssue];
        }
    }
    const capstone = episode.capstoneContract;
    for (const [key, published] of [
        ...
        const values = capstone[key];
        const missingIndex = values.findIndex((value) => !published.includes(value));
        if (missingIndex >= 0)
            return [
                issue("capstone_reference_missing", `$.episode.capstoneContract.${key}[${missingIndex}]`),
            ];
    }
    const expectedCapstoneSlots = episode.semanticSlots
        .filter((slot) => slot.requiredInCapstone === true)
        .map((slot) => String(slot.semanticSlotId));
    if (!sameStringSet(capstone.requiredSemanticSlotIds, expectedCapstoneSlots)) {
        return [
            issue("capstone_reference_invalid", "$.episode.capstoneContract.requiredSemanticSlotIds"),
        ];
    }
    const expectedCriticalConstraints = episode.criticalConstraints.map((constraint) => String(constraint.criticalConstraintId));
    if (!sameStringSet(capstone.criticalConstraintIds, expectedCriticalConstraints)) {
        return [
            issue("capstone_reference_invalid", "$.episode.capstoneContract.criticalConstraintIds"),
        ];
    }
    const graphForCapstone = episode.graph;
    if (!sameStringSet(capstone.primaryNodeIds, [
        String(graphForCapstone.capstoneNodeId),
    ])) {
        return [
            issue("capstone_reference_invalid", "$.episode.capstoneContract.primaryNodeIds"),
        ];
    }
    const capstonePrimaryNodeIds = capstone.primaryNodeIds;
    const capstoneAlternateNodeIds = capstone.deterministicAlternateNodeIds;
    const duplicateCapstoneAlternateIndex = uniqueSecondIndex(capstoneAlternateNodeIds);
    if (duplicateCapstoneAlternateIndex >= 0) {
        return [
            issue("capstone_fallback_invalid", `$.episode.capstoneContract.deterministicAlternateNodeIds[${duplicateCapstoneAlternateIndex}]`),
        ];
    }
    if (capstoneAlternateNodeIds.length !== 0 &&
        capstoneAlternateNodeIds.length !== capstonePrimaryNodeIds.length) {
        return [
            issue("capstone_fallback_invalid", `$.episode.capstoneContract.deterministicAlternateNodeIds[${capstonePrimaryNodeIds.length}]`),
        ];
    }
    const nodesById = new Map(nodes.map((node) => [String(node.nodeId), node]));
    const capstoneEdges = graphForCapstone.edges;
    const activitiesById = new Map(episode.activities.map((activity) => [
        String(activity.activityId),
        activity,
    ]));
    const graphOutgoing = new Map();
    for (const node of nodes)
        graphOutgoing.set(String(node.nodeId), []);
    for (const edge of capstoneEdges) {
        graphOutgoing.get(String(edge.fromNodeId))?.push(String(edge.toNodeId));
    }
    const reaches = (fromNodeId, targetNodeId) => {
    ...
    const requiredCapstoneSlots = capstone.requiredSemanticSlotIds;
    for (let index = 0; index < capstoneAlternateNodeIds.length; index += 1) {
        ...
        const alternateActivity = alternateNode
            ...
            : undefined;
        const alternateTargets = alternateActivity?.targets;
        const alternateSlotIds = alternateTargets?.semanticSlotIds;
        ...
        const alternateObjectiveIds = isRecordArray(alternateDeclarations)
            ? alternateDeclarations.map((declaration) => String(declaration.objectiveId))
            : [];
        const alternateCriticalConstraintIds = isRecordArray(alternateDeclarations)
            ? alternateDeclarations
                .filter((declaration) => isPlainObject(declaration.target) &&
                declaration.target.targetKind === "critical_constraint")
                .map((declaration) => String(declaration.target.targetId))
            : [];
        ...
        const alternateCapabilities = isPlainObject(alternateActivity?.capabilities)
            ...
            : undefined;
        const hasReachableFallbackBranch = capstoneEdges.some((edge) => edge.condition === "fallback_selected" &&
            String(edge.toNodeId) === alternateNodeId &&
            reaches(String(graph.startNodeId), String(edge.fromNodeId)));
        if (alternateNodeId === primaryNodeId ||
            ...
            requiredCapstoneSlots.some((slotId) => !alternateSlotIds.includes(slotId)) ||
            capstone.objectiveIds.some((objectiveId) => !alternateObjectiveIds.includes(objectiveId)) ||
            capstone.criticalConstraintIds.some((constraintId) => !alternateCriticalConstraintIds.includes(constraintId))) {
            return [issue("capstone_fallback_invalid", fallbackPath)];
        }
    }
    const learningDesign = episode.learningDesign;
    const designObjectives = learningDesign.objectiveIds;
    if (!sameStringSet(designObjectives, objectiveIds)) {
        return [
            issue("support_plan_invalid", "$.episode.learningDesign.objectiveIds"),
        ];
    }
    const supportPlan = learningDesign.supportPlan;
    const supportObjectives = supportPlan.map((support) => String(support.objectiveId));
    if (uniqueSecondIndex(supportObjectives) >= 0 ||
    ...
    for (let index = 0; index < supportPlan.length; index += 1) {
        ...
        const encounterNodes = nodes.filter((node) => node.phase === "encounter_build" &&
            node.evidenceDeclarations.some((declaration) => declaration.objectiveId === objectiveId));
        ...
        if (supportedInitialValues.includes(initialSupport) &&
            encounterNodes.length > 0 &&
            encounterNodes.some((node) => !isPlainObject(node.pedagogicalContextContract) ||
                !node.pedagogicalContextContract
                    .allowedSupportLevels.includes(initialSupport))) {
            return [
                issue("support_plan_invalid", `$.episode.learningDesign.supportPlan[${index}].initialSupport`),
            ];
        }
    }
    ...
    for (const edge of prerequisiteEdges) {
        ...
        if (from.sourceEpisodeId === episode.episodeId &&
            from.kind === "objective") {
            const fromId = String(from.id);
            const toId = String(edge.toObjectiveId);
            if (!objectiveIds.includes(fromId) || !objectiveIds.includes(toId)) {
                return [
                    issue("prerequisite_reference_missing", "$.episode.learningDesign.prerequisiteEdges"),
                ];
            }
    ...
    const graphContexts = nodes
        ...
        .map((node) => ({
        node,
        contract: node.pedagogicalContextContract,
    }));
    const independent = graphContexts.find(({ node }) => node.nodeId === learningDesign.independentProbeRef);
    if (independent) {
        const independentContext = independent.contract.context;
        const independentPrompt = independent.contract.prompt;
        const reusesTraining = graphContexts.some(({ node, contract }) => node.nodeId !== learningDesign.independentProbeRef &&
            (contract.context.contextId ===
                independentContext.contextId ||
                contract.context.surfaceFormId ===
                    independentContext.surfaceFormId ||
                contract.prompt.promptId ===
                    independentPrompt.promptId));
        if (reusesTraining) {
            const index = nodes.findIndex((node) => node.nodeId === learningDesign.independentProbeRef);
            return [
                issue("probe_surface_reused", `$.episode.graph.nodes[${index}].pedagogicalContextContract`),
            ];
        }
    }
    const allGraphSurfaces = graphContexts.map(({ contract }) => ({
        contextId: contract.context.contextId,
        surfaceFormId: contract.context.surfaceFormId,
        promptId: contract.prompt.promptId,
    }));
    for (let index = 0; index < delayedDefinitions.length; index += 1) {
        const delayedContext = delayedDefinitions[index].body
            .pedagogicalContextContract;
        const context = delayedContext.context;
        const prompt = delayedContext.prompt;
        if (allGraphSurfaces.some((surface) => surface.contextId === context.contextId ||
            surface.surfaceFormId === context.surfaceFormId ||
            surface.promptId === prompt.promptId)) {
            return [
                issue("probe_surface_reused", `$.episode.delayedProbeDefinitions[${index}].body.pedagogicalContextContract`),
            ];
        }
    }
    const mastery = episode.masteryContract;
    const evidencePolicy = mastery.evidencePolicyRef;
    const policies = dependencies.policies;
    const resolvedEvidencePolicy = policies.find((policy) => sameValue(policy.ref, evidencePolicy));
    if (!resolvedEvidencePolicy) {
        return [
            issue("mastery_policy_mismatch", "$.episode.masteryContract.evidencePolicyRef"),
        ];
    }
    const evidenceKinds = resolvedEvidencePolicy.body
        .evidenceKinds;
    const graphDeclarations = nodes.flatMap((node) => node.evidenceDeclarations);
    const delayedDeclarations = delayedDefinitions.flatMap((definition) => definition.body.evidenceDeclarations);
    const independentCoverage = new Set(graphDeclarations
        .filter((declaration) => declaration.phase === "independent_probe")
        .map((declaration) => `${String(declaration.objectiveId)}\u0000${String(declaration.construct)}`));
    const delayedCoverage = new Set(delayedDeclarations.map((declaration) => `${String(declaration.objectiveId)}\u0000${String(declaration.construct)}`));
    if (independentCoverage.size !== delayedCoverage.size ||
        [...independentCoverage].some((key) => !delayedCoverage.has(key))) {
        return [
            issue("probe_coverage_mismatch", "$.episode.delayedProbeDefinitions[0].body.evidenceDeclarations"),
        ];
    }
    const measurableDeclarations = [...graphDeclarations, ...delayedDeclarations];
    const masteryRequirements = mastery.requirements;
    for (let index = 0; index < masteryRequirements.length; index += 1) {
        ...
        if (!evidenceKinds.includes(String(requirement.construct))) {
            return [
                issue("mastery_policy_mismatch", `$.episode.masteryContract.requirements[${index}].construct`),
            ];
        }
        if (!objectiveIds.includes(String(requirement.objectiveId)) ||
            !measurableDeclarations.some((declaration) => declaration.objectiveId === requirement.objectiveId &&
                declaration.construct === requirement.construct &&
                declaration.phase === requirement.phase)) {
            return [
                issue("mastery_requirement_invalid", `$.episode.masteryContract.requirements[${index}]`),
            ];
        }
    }
    return [];
};
const validateGraph = (episode) => {
    const graph = episode.graph;
    const nodes = graph.nodes;
    const edges = graph.edges;
    const duplicateNodeIndex = uniqueSecondIndex(nodes.map((node) => node.nodeId));
    if (duplicateNodeIndex >= 0) {
        return [
            issue("graph_duplicate_id", `$.episode.graph.nodes[${duplicateNodeIndex}].nodeId`),
        ];
    }
    const nodeIds = new Set(nodes.map((node) => node.nodeId));
    const activityIds = new Set(episode.activities.map((activity) => activity.activityId));
    for (let nodeIndex = 0; nodeIndex < nodes.length; nodeIndex += 1) {
        if (!activityIds.has(nodes[nodeIndex].activityId)) {
            return [
                issue("graph_reference_missing", `$.episode.graph.nodes[${nodeIndex}].activityId`),
            ];
        }
    }
    for (let edgeIndex = 0; edgeIndex < edges.length; edgeIndex += 1) {
        for (const endpoint of ["fromNodeId", "toNodeId"]) {
            if (!nodeIds.has(edges[edgeIndex][endpoint])) {
                return [
                    issue("graph_reference_missing", `$.episode.graph.edges[${edgeIndex}].${endpoint}`),
                ];
            }
        }
    }
    const duplicateEdgeIndex = uniqueSecondIndex(edges.map((edge) => edge.edgeId));
    if (duplicateEdgeIndex >= 0) {
        return [
            issue("graph_duplicate_id", `$.episode.graph.edges[${duplicateEdgeIndex}].edgeId`),
        ];
    }
    ...
    if (!nodeIds.has(graph.capstoneNodeId) ||
    ...
    for (let index = 0; index < nodes.length; index += 1) {
        if (nodes[index].requiredForCore === true &&
    ...
    const entries = nodes.filter((node) => (incomingCount.get(node.nodeId) ?? 0) === 0);
    if (entries.length !== 1 || entries[0]?.nodeId !== graph.startNodeId) {
        return [issue("graph_entry_count_invalid", "$.episode.graph")];
    }
    return [];
};
const validateLoopsAndNearTransfer = (episode) => {
    const graph = episode.graph;
    const nodes = graph.nodes;
    ...
    for (const [key, expectedPhase] of loopChecks) {
        ...
        if (duplicateIndex >= 0) {
            return [
                issue("loop_duplicate_id", `$.episode.requiredLoops.${key}[${duplicateIndex}]`),
            ];
        }
        for (let index = 0; index < ids.length; index += 1) {
            if (nodesById.get(ids[index])?.phase !== expectedPhase) {
                return [
                    issue("loop_phase_mismatch", `$.episode.requiredLoops.${key}[${index}]`),
                ];
            }
        }
    }
    for (let index = 0; index < nodes.length; index += 1) {
        const node = nodes[index];
        if (node.phase !== "near_transfer")
            continue;
        const source = nodesById.get(node.transferFromNodeId);
        const context = isPlainObject(node.pedagogicalContextContract)
            ? node.pedagogicalContextContract
            : undefined;
        const sourceContext = source && isPlainObject(source.pedagogicalContextContract)
            ? source.pedagogicalContextContract
            : undefined;
        const identity = context && isPlainObject(context.context) ? context.context : undefined;
        const sourceIdentity = sourceContext && isPlainObject(sourceContext.context)
            ? sourceContext.context
            : undefined;
        const prompt = context && isPlainObject(context.prompt) ? context.prompt : undefined;
        const sourcePrompt = sourceContext && isPlainObject(sourceContext.prompt)
        ...
        if (!source ||
            ...
            varied.length === 0) {
            return [
                issue("near_transfer_replay_forbidden", `$.episode.graph.nodes[${index}]`),
            ];
        }
    }
    return [];
};
const validateEpisodePolicyLimits = (episode, registry) => {
    ...
    if (episode.episodeKind === "ordinary") {
        ...
        if (episode.schemaVersion === "v2-episode-contract.v1" &&
            (typeof episode.estimatedMinutes !== "number" ||
                !rangeContains(nodeSettings.targetEpisodeMinutes, episode.estimatedMinutes))) {
            return [
        ...
        if (!isStringArray(episode.grammarDistinctionIds) ||
            episode.grammarDistinctionIds.length > 2) {
            return [
                issue("grammar_load_exceeded", "$.episode.grammarDistinctionIds"),
            ];
        }
        if (!isStringArray(episode.soundFocusIds) ||
            episode.soundFocusIds.length !== 1 ||
            !rangeContains(dosageSettings.newSoundContrasts, episode.soundFocusIds.length)) {
            return [
    ...
    if (typeof expectedSlotCount !== "number" ||
    ...
    for (let index = 0; index < slots.length; index += 1) {
        if (slots[index].maxStars !== expectedMaxStars) {
            return [
                issue("star_slot_policy_mismatch", `$.episode.starSlots[${index}].maxStars`),
            ];
        }
    }
    const duplicateSlotIndex = uniqueSecondIndex(slots.map((slot) => slot.starSlotId));
    if (duplicateSlotIndex >= 0) {
        return [
            issue("graph_duplicate_id", `$.episode.starSlots[${duplicateSlotIndex}].starSlotId`),
        ];
    }
    const slotIds = new Set(slots.map((slot) => slot.starSlotId));
    const nodeById = new Map(nodes.map((node) => [node.nodeId, node]));
    for (let slotIndex = 0; slotIndex < slots.length; slotIndex += 1) {
        const acceptedNodeIds = slots[slotIndex].acceptedNodeIds;
        if (acceptedNodeIds.length === 0 ||
            uniqueSecondIndex(acceptedNodeIds) >= 0) {
            return [
                issue("star_slot_binding_invalid", `$.episode.starSlots[${slotIndex}].acceptedNodeIds`),
            ];
        }
        for (let acceptedIndex = 0; acceptedIndex < acceptedNodeIds.length; acceptedIndex += 1) {
            ...
            if (!accepted ||
                ...
                return [
                    issue("star_slot_binding_invalid", `$.episode.starSlots[${slotIndex}].acceptedNodeIds[${acceptedIndex}]`),
                ];
            }
        }
    }
    for (let index = 0; index < nodes.length; index += 1) {
        ...
        if (node.gateEligible === true) {
            const slot = slots.find((candidate) => candidate.starSlotId === node.starSlotId);
            if (!slot ||
                !slot.acceptedNodeIds.includes(String(node.nodeId))) {
                return [issue("star_slot_binding_invalid", nodePath)];
            }
        }
        if (isPlainObject(node.fallback) &&
            ...
            if (!alternate || alternate.starSlotId !== node.starSlotId) {
                return [
                    issue("alternate_star_slot_mismatch", `${nodePath}.fallback.alternateNodeId`),
                ];
            }
        }
    }
    ...
    const requiredLoopIds = [
        ...episode.requiredLoops
            .encounterBuildNodeIds,
        ...episode.requiredLoops.nearTransferNodeIds,
    ];
    for (let index = 0; index < episode.accessibilityRoutes.length; index += 1) {
        const route = episode.accessibilityRoutes[index];
        ...
        const derivedSlots = routeNodes
            .filter((node) => Boolean(node) && node?.gateEligible === true)
        ...
        for (const nodeId of routeNodeIds)
            routeOutgoing.set(nodeId, []);
        for (const edge of graphEdges) {
            if (routeNodeSet.has(String(edge.fromNodeId)) &&
        ...
        while (routeStack.length > 0) {
            const nodeId = routeStack.pop();
            if (reached.has(nodeId))
                continue;
            reached.add(nodeId);
            routeStack.push(...(routeOutgoing.get(nodeId) ?? []));
        }
        const routeInvalid = routeNodeIds.length === 0 ||
        ...
        if (routeInvalid) {
            return [
                issue("accessibility_route_unreachable", `$.episode.accessibilityRoutes[${index}]`),
            ];
        }
        if (route.routeKind === "accessibility" &&
            isStringArray(route.measuredConstructs) &&
            route.measuredConstructs.some((construct) => construct === "spoken" || construct === "listening")) {
            return [
                issue("accessibility_false_claim", `$.episode.accessibilityRoutes[${index}].measuredConstructs`),
            ];
        }
    }
    return [];
};
const validateDelayedAndLearning = (episode, dependencies, registry) => {
    const graph = episode.graph;
    const nodes = graph.nodes;
    const graphNodeIds = new Set(nodes.map((node) => String(node.nodeId)));
    const schedulerNamespaceIds = new Set(graphNodeIds);
    for (const nodeId of episode.assessmentNodes
        .independentProbeNodeIds)
        schedulerNamespaceIds.add(nodeId);
    for (const nodeId of episode.assessmentNodes
        .optionalReviewNodeIds)
        schedulerNamespaceIds.add(nodeId);
    for (const slot of episode.starSlots)
    ...
    if (isPlainObject(checkpoint)) {
        for (const nodeId of checkpoint.assessmentNodeIds ?? [])
            schedulerNamespaceIds.add(nodeId);
        for (const route of checkpoint.deterministicAlternateRoutes ?? []) {
    ...
    if (!isPlainObject(learningDesign))
        return [issue("field_type_invalid", "$.episode.learningDesign")];
    if (!isPlainObject(learningDesign.delayedProbeRef) ||
        exactObjectIssues(learningDesign.delayedProbeRef, ["probeId", "contentHash"], [], "$.episode.learningDesign.delayedProbeRef").length > 0) {
        return [
            issue("delayed_probe_ref_invalid", "$.episode.learningDesign.delayedProbeRef"),
        ];
    }
    const delayedProbeIds = new Set();
    for (let index = 0; index < episode.delayedProbeDefinitions.length; index += 1) {
        const definition = episode.delayedProbeDefinitions[index];
        ...
        if (schedulerNamespaceIds.has(String(body.probeNodeId))) {
            return [
                issue("delayed_probe_namespace_invalid", `${definitionPath}.body.probeNodeId`),
            ];
        }
        schedulerNamespaceIds.add(String(body.probeNodeId));
        if (!isPlainObject(body.activityBinding)) {
            return [
                issue("delayed_probe_binding_mismatch", `${definitionPath}.body.activityBinding`),
            ];
        }
        const activityBinding = body.activityBinding;
        const activity = activities.find((candidate) => candidate.activityId === activityBinding.activityId);
        if (!activity ||
        ...
        if (hasOwn(body, "accessibilityAlternateActivityId")) {
            const alternateActivity = activities.find((candidate) => candidate.activityId === body.accessibilityAlternateActivityId);
            const alternateFallback = isPlainObject(alternateActivity?.requirements)
                ? alternateActivity.requirements.fallback
                : undefined;
            ...
            const alternateCapabilities = isPlainObject(alternateActivity?.capabilities)
                ? alternateActivity.capabilities
                : undefined;
            const targetsEquivalent = isPlainObject(primaryTargets) &&
                ...
                sameStringSet(primaryTargets.phraseFrameIds, alternateTargets.phraseFrameIds) &&
                sameStringSet(primaryTargets.semanticSlotIds, alternateTargets.semanticSlotIds);
            const evidenceEquivalent = targetsEquivalent &&
                isRecordArray(body.evidenceDeclarations) &&
                body.evidenceDeclarations.every((declaration) => isPlainObject(declaration) &&
                    alternateTargets.skillIds.includes(String(declaration.skillId)) &&
                    (!isPlainObject(declaration.target) ||
                        declaration.target.targetKind !== "semantic_slot" ||
                        alternateTargets.semanticSlotIds.includes(String(declaration.target.targetId))));
            const alternateEvidenceDeclarations = nodes
                .filter((node) => node.activityId === alternateActivity?.activityId)
                .flatMap((node) => isRecordArray(node.evidenceDeclarations)
                ? node.evidenceDeclarations
                : []);
            const evidenceSignature = (declaration) => (0, decision_registry_1.canonicalJsonV1)({
                objectiveId: declaration.objectiveId,
                skillId: declaration.skillId,
                construct: declaration.construct,
                target: declaration.target,
            });
            const declarationsEquivalent = isRecordArray(body.evidenceDeclarations) &&
                body.evidenceDeclarations.every(isPlainObject) &&
                alternateEvidenceDeclarations.every(isPlainObject) &&
                sameValue(body.evidenceDeclarations
                    .map((declaration) => evidenceSignature(declaration))
                    .sort(), alternateEvidenceDeclarations
                    .map((declaration) => evidenceSignature(declaration))
                    .sort());
            const delayedObjectiveIds = isRecordArray(body.evidenceDeclarations)
                ? body.evidenceDeclarations.map((declaration) => isPlainObject(declaration) ? String(declaration.objectiveId) : "")
                : [];
            const alternateObjectiveIds = alternateEvidenceDeclarations.map((declaration) => isPlainObject(declaration) ? String(declaration.objectiveId) : "");
            const criticalConstraintIds = (declarations) => declarations.flatMap((declaration) => isPlainObject(declaration) &&
                isPlainObject(declaration.target) &&
                declaration.target.targetKind === "critical_constraint"
                ? [String(declaration.target.targetId)]
                : []);
            const criticalCoverageEquivalent = sameStringSet(criticalConstraintIds(isRecordArray(body.evidenceDeclarations)
            ...
            if (!alternateActivity ||
                ...
                return [
                    issue("delayed_probe_accessibility_alternate_invalid", `${definitionPath}.body.accessibilityAlternateActivityId`),
                ];
            }
        }
        if (body.targetEpisodeId !== episode.episodeId ||
            body.probeId !== ref.probeId ||
            !isRecordArray(body.evidenceDeclarations) ||
            body.evidenceDeclarations.some((declaration) => declaration.phase !== "delayed_probe") ||
        ...
        if (!contextIdentity ||
            ...
            return [
                issue("delayed_probe_context_invalid", `${definitionPath}.body.pedagogicalContextContract`),
            ];
        }
        let bodyHash;
        try {
            bodyHash = (0, decision_registry_1.hashCanonicalBody)(body);
        }
        catch {
            return [
                issue("delayed_probe_hash_mismatch", `${definitionPath}.ref.contentHash`),
            ];
        }
        ...
        if (!sameValue(learningDesign.delayedProbeRef, ref)) {
            return [
                issue("delayed_probe_ref_mismatch", "$.episode.learningDesign.delayedProbeRef"),
            ];
        }
    }
    if (!isRecordArray(episode.reviewLinks))
    ...
    if (delayedScheduleIndexes.length > 1) {
        return [
            issue("delayed_probe_schedule_ambiguous", `$.episode.reviewLinks[${delayedScheduleIndexes[1]}]`),
        ];
    }
    for (let index = 0; index < episode.reviewLinks.length; index += 1) {
        const link = episode.reviewLinks[index];
        ...
        if (link.scheduleKind === "optional_review") {
            if (!["next_episode", "chapter_checkpoint"].includes(String(link.delay)) ||
            ...
            if (!sameValue(link.probeRef, learningDesign.delayedProbeRef) ||
    ...
    if (episode.masteryContract.performanceStarsAreLearningEvidence !== false) {
        return [
            issue("mastery_contract_invalid", "$.episode.masteryContract.performanceStarsAreLearningEvidence"),
        ];
    }
    if (episode.masteryContract.confidentVoiceTurnCountAloneIsSufficient !== false) {
        return [
            issue("mastery_contract_invalid", "$.episode.masteryContract.confidentVoiceTurnCountAloneIsSufficient"),
        ];
    }
    ...
    const prerequisiteEdges = learningDesign.prerequisiteEdges;
    if (prerequisiteEdges.some((edge) => isPlainObject(edge.from) &&
    ...
    for (let index = 0; index < prerequisiteEdges.length; index += 1) {
        ...
        if (!isPlainObject(edge.from))
            continue;
        const source = edge.from;
        if (!["objective", "outcome"].includes(String(source.kind))) {
            return [
                issue("field_value_invalid", `$.episode.learningDesign.prerequisiteEdges[${index}].from.kind`),
            ];
        }
        if (!["exposed", "supported_success", "independent_evidence"].includes(String(edge.requiredState))) {
            return [
                issue("field_value_invalid", `$.episode.learningDesign.prerequisiteEdges[${index}].requiredState`),
            ];
        }
        if (edge.requiredState !== "exposed") {
            const hasExposure = prerequisiteEdges.some((candidate) => isPlainObject(candidate.from) &&
            ...
            if (!hasExposure) {
                return [
                    issue("prerequisite_exposure_missing", `$.episode.learningDesign.prerequisiteEdges[${index}]`),
                ];
            }
        }
    }
    ...
    for (let index = 0; index < learningDesign.supportPlan.length; index += 1) {
        ...
        if (!["model", "full_text", "partial_cue", "visual_only", "none"].includes(String(support.initialSupport))) {
            return [
                issue("field_value_invalid", `$.episode.learningDesign.supportPlan[${index}].initialSupport`),
            ];
        }
        if (!fadeIds.includes(String(support.fadeRuleId))) {
            return [
                issue("support_rule_missing", `$.episode.learningDesign.supportPlan[${index}].fadeRuleId`),
            ];
        }
        if (!CODE_OWNED_FADE_RULE_IDS.has(String(support.fadeRuleId))) {
            return [
                issue("support_rule_untrusted", `$.episode.learningDesign.supportPlan[${index}].fadeRuleId`),
            ];
        }
        if (!escalationIds.includes(String(support.escalationRuleId))) {
            return [
                issue("support_rule_missing", `$.episode.learningDesign.supportPlan[${index}].escalationRuleId`),
            ];
        }
        if (!CODE_OWNED_ESCALATION_RULE_IDS.has(String(support.escalationRuleId))) {
            return [
                issue("support_rule_untrusted", `$.episode.learningDesign.supportPlan[${index}].escalationRuleId`),
            ];
        }
    }
    return [];
};
const validateCurriculumInternal = (input, registry) => {
    const shapeIssues = exactObjectIssues(input, CURRICULUM_KEYS, [], "$.curriculum");
    if (shapeIssues.length > 0)
        return fail(shapeIssues);
    const deepShapeIssues = validateCurriculumDeepShape(input);
    if (deepShapeIssues.length > 0)
        return fail(deepShapeIssues);
    const identityIssue = curriculumIdentityIssue(input);
    if (identityIssue)
        return fail([identityIssue]);
    const curriculum = input;
    ...
    if (!isPlainObject(curriculum.decisionRegistryRef)) {
        return fail([
            issue("decision_registry_ref_invalid", "$.curriculum.decisionRegistryRef"),
        ]);
    }
    if (hasOwn(curriculum.decisionRegistryRef, "latest")) {
        return fail([
            issue("decision_registry_ref_invalid", "$.curriculum.decisionRegistryRef.latest"),
        ]);
    }
    const refIssues = exactObjectIssues(curriculum.decisionRegistryRef, ["id", "version", "contentHash"], [], "$.curriculum.decisionRegistryRef", "decision_registry_ref_invalid");
    if (refIssues.length > 0)
        return fail(refIssues);
    if (!isRecordArray(curriculum.episodeRefs)) {
    ...
    for (let index = 0; index < curriculum.episodeRefs.length; index += 1) {
        ...
        if (seenIds.has(episodeRef.episodeId) ||
            seenOrdinals.has(episodeRef.ordinal) ||
            episodeRef.ordinal !== index + 1) {
            return fail([
                issue("curriculum_episode_sequence_invalid", `$.curriculum.episodeRefs[${index}]`),
            ]);
        }
        seenIds.add(episodeRef.episodeId);
        seenOrdinals.add(episodeRef.ordinal);
    }
    if (!isPlainObject(curriculum.scope) ||
        !Array.isArray(curriculum.scope.includedChapterOrdinals) ||
        !curriculum.scope.includedChapterOrdinals.every((value) => Number.isSafeInteger(value)) ||
        !Array.isArray(curriculum.scope.includedEpisodeOrdinals) ||
        !curriculum.scope.includedEpisodeOrdinals.every((value) => Number.isSafeInteger(value))) {
    ...
    const expected = scope.kind === "vertical_slice"
    ...
    const allowedEnvironments = scope.kind === "vertical_slice"
        ? ["lab", "staging"]
        : scope.kind === "chapter_internal"
            ? ["internal", "staging"]
    ...
    for (const checkpointOrdinal of expected.checkpoints) {
        if (curriculum.episodeRefs[checkpointOrdinal - 1]?.episodeKind !==
            "checkpoint") {
            return fail([
                issue("curriculum_checkpoint_invalid", `$.curriculum.episodeRefs[${checkpointOrdinal - 1}]`),
            ]);
        }
    }
    for (let index = 0; index < curriculum.episodeRefs.length; index += 1) {
        if (!expected.checkpoints.includes(index + 1) &&
            curriculum.episodeRefs[index].episodeKind !== "ordinary") {
            return fail([
                issue("curriculum_checkpoint_invalid", `$.curriculum.episodeRefs[${index}]`),
            ]);
        }
    }
    if (!isRecordArray(curriculum.chapters) ||
    ...
    for (let index = 0; index < curriculum.chapters.length; index += 1) {
        ...
        const expectedChapterRefs = curriculum.episodeRefs.slice(chapterStart, chapterEnd);
        const expectedEpisodeIds = expectedChapterRefs.map((episodeRef) => String(episodeRef.episodeId));
        const expectedCheckpointId = expectedChapterRefs.find((episodeRef) => episodeRef.episodeKind === "checkpoint")?.episodeId;
        if (curriculum.chapters.some((candidate, candidateIndex) => candidateIndex < index && candidate.chapterId === chapter.chapterId)) {
            return fail([
                issue("curriculum_chapter_invalid", `$.curriculum.chapters[${index}].chapterId`),
            ]);
        }
        if (chapter.ordinal !== chapterOrdinal ||
            !sameValue(chapter.episodeIds, expectedEpisodeIds) ||
            expectedChapterRefs.some((episodeRef) => episodeRef.chapterId !== chapter.chapterId) ||
    ...
    const curriculumEpisodeIds = new Set(curriculum.episodeRefs.map((episodeRef) => episodeRef.episodeId));
    for (let index = 0; index < curriculum.episodeRefs.length; index += 1) {
        const prerequisites = curriculum.episodeRefs[index].prerequisiteEpisodeIds;
        if (!isStringArray(prerequisites)) {
            return fail([
                issue("curriculum_prerequisite_invalid", `$.curriculum.episodeRefs[${index}].prerequisiteEpisodeIds`),
            ]);
        }
        for (let prerequisiteIndex = 0; prerequisiteIndex < prerequisites.length; prerequisiteIndex += 1) {
            const prerequisiteId = prerequisites[prerequisiteIndex];
            const prerequisiteRefIndex = curriculum.episodeRefs.findIndex((candidate) => candidate.episodeId === prerequisiteId);
            if (prerequisites.slice(0, prerequisiteIndex).includes(prerequisiteId)) {
                return fail([
                    issue("curriculum_prerequisite_invalid", `$.curriculum.episodeRefs[${index}].prerequisiteEpisodeIds[${prerequisiteIndex}]`),
                ]);
            }
            if (!curriculumEpisodeIds.has(prerequisiteId) ||
    ...
    const accumulatedMaterial = new Map(materialKeys.map((key) => [key, new Set()]));
    for (let index = 0; index < curriculum.episodeRefs.length; index += 1) {
        const episodeRef = curriculum.episodeRefs[index];
        const requiredMaterial = episodeRef.requiredCheckpointMaterial;
        if (episodeRef.episodeKind === "checkpoint") {
            for (const key of materialKeys) {
                const values = requiredMaterial[key];
                for (let valueIndex = 0; valueIndex < values.length; valueIndex += 1) {
                    if (!accumulatedMaterial.get(key)?.has(values[valueIndex])) {
                        return fail([
                            issue("curriculum_checkpoint_material_invalid", `$.curriculum.episodeRefs[${index}].requiredCheckpointMaterial.${key}[${valueIndex}]`),
                        ]);
                    }
                }
            }
        }
        else if (materialKeys.some((key) => requiredMaterial[key].length > 0)) {
            return fail([
                issue("curriculum_checkpoint_material_invalid", `$.curriculum.episodeRefs[${index}].requiredCheckpointMaterial`),
            ]);
        }
        const introducedMaterial = episodeRef.introducedMaterial;
        for (const key of materialKeys) {
            for (const value of introducedMaterial[key]) {
...
const validateLearningPackageInternal = (input, context) => {
    ...
    if (loopIssues.length > 0)
        return fail(loopIssues);
    const policyLimitIssues = validateEpisodePolicyLimits(episode, registry);
    if (policyLimitIssues.length > 0)
        return fail(policyLimitIssues);
    const learningReferenceIssues = validateLearningReferences(episode, root.dependencies);
    if (learningReferenceIssues.length > 0)
        return fail(learningReferenceIssues);
    const delayedIssues = validateDelayedAndLearning(episode, root.dependencies, registry);
    if (delayedIssues.length > 0)
        return fail(delayedIssues);
    const curriculumResult = validateCurriculumInternal(root.curriculum, registry);
    if (!curriculumResult.ok)
        return fail(curriculumResult.issues);
    const requiredLocales = root.curriculum
        .requiredLocales;
    const localizedValueSets = [];
    collectLocalizedValueSets(root.episode, "$.episode", localizedValueSets);
    for (const localized of localizedValueSets) {
        if (uniqueSecondIndex(localized.locales) >= 0 ||
            !sameStringSet([...localized.locales], requiredLocales)) {
            return fail([issue("curriculum_locale_closure_invalid", localized.path)]);
        }
    }
    const learningDesign = episode.learningDesign;
    const objectiveIds = new Set(learningDesign.objectiveIds.map(String));
    const curriculumRefs = root.curriculum
        .episodeRefs;
    const publishedEpisodeIds = new Set(curriculumRefs.map((ref) => String(ref.episodeId)));
    const prerequisiteEdges = learningDesign.prerequisiteEdges;
    for (let index = 0; index < prerequisiteEdges.length; index += 1) {
        const source = prerequisiteEdges[index].from;
    ...
    if (episode.episodeKind === "checkpoint") {
        ...
        const nodes = graph.nodes;
        const curriculum = root.curriculum;
        const curriculumRefs = curriculum.episodeRefs;
        const currentRefIndex = curriculumRefs.findIndex((ref) => ref.episodeId === episode.episodeId);
        const currentRef = curriculumRefs[currentRefIndex];
        const taughtScope = Object.fromEntries(materialKeys.map((key) => [
            key,
            [
                ...new Set(curriculumRefs
                    .slice(0, currentRefIndex)
                    .flatMap((ref) => ref.introducedMaterial[key])),
            ],
        ]));
        const checkpointContext = {
            ...
            nodePhases: Object.fromEntries(nodes.map((node) => [String(node.nodeId), String(node.phase)])),
            declarations: nodes.flatMap((node) => node.evidenceDeclarations.map((declaration) => ({
                nodeId: String(node.nodeId),
                declaration,
            }))),
            taughtScope,
            requiredCheckpointMaterial: currentRef.requiredCheckpointMaterial,
        };
        const checkpointResult = validateCheckpointInternal(episode.checkpointContract, checkpointContext);
        if (!checkpointResult.ok) {
            return fail(checkpointResult.issues.map((checkpointIssue) => ({
                ...checkpointIssue,
                path: checkpointIssue.path.startsWith("$.checkpoint")
                    ? checkpointIssue.path.replace("$.checkpoint", "$.episode.checkpointContract")
    ...
    const dependencies = root.dependencies;
    const curriculum = root.curriculum;
    const currentRefIndex = curriculum.episodeRefs.findIndex((candidate) => candidate.episodeId === episode.episodeId);
    const currentRef = currentRefIndex >= 0
        ? curriculum.episodeRefs[currentRefIndex]
        : undefined;
    if (!currentRef) {
        return fail([
    ...
    const episodeMaterial = {
        ...
        criticalConstraintIds: isRecordArray(episode.criticalConstraints)
            ? episode.criticalConstraints.map((constraint) => String(constraint.criticalConstraintId))
            : [],
    };
    const introducedMaterial = currentRef.introducedMaterial;
    for (const key of materialKeys) {
        if (!sameStringSet(introducedMaterial[key], episodeMaterial[key])) {
            return fail([
                issue("curriculum_material_closure_invalid", `$.curriculum.episodeRefs[${currentRefIndex}].introducedMaterial.${key}`),
            ]);
        }
    }
    const dependencyAssetIds = new Set(dependencies.assetIds);
    if (curriculum.requiredAssetIds.some((assetId) => !dependencyAssetIds.has(assetId))) {
        return fail([
    ...
    const episodeAssetIds = isStringArray(episode.assetIds)
        ? episode.assetIds
        : [];
    if (!sameStringSet(curriculum.requiredAssetIds, episodeAssetIds)) {
        return fail([
            issue("curriculum_asset_closure_invalid", "$.curriculum.requiredAssetIds"),
        ]);
    }
    const resolvedCapabilityKeys = new Set(dependencies.templates.map((template) => String(template.body.kernel.activityTypeKey)));
    const templatesById = new Map(dependencies.templates.map((template) => [
        String(template.templateRef.templateId),
        template,
    ]));
    const usedCapabilityKeys = new Set();
    for (const activity of episode.activities) {
        const template = templatesById.get(String(activity.templateRef.templateId));
        if (template) {
            usedCapabilityKeys.add(String(template.body.kernel.activityTypeKey));
        }
    }
    for (const definition of episode.delayedProbeDefinitions) {
        const binding = definition.body
            .activityBinding;
        const template = templatesById.get(String(binding.templateRef.templateId));
        if (template) {
            usedCapabilityKeys.add(String(template.body.kernel.activityTypeKey));
        }
    }
    if (curriculum.requiredCapabilityKeys.some((key) => !resolvedCapabilityKeys.has(key)) ||
        !sameStringSet(curriculum.requiredCapabilityKeys, [
            ...usedCapabilityKeys,
        ])) {
        return fail([
            issue("curriculum_capability_missing", "$.curriculum.requiredCapabilityKeys"),
        ]);
    }
    const episodeRef = root.curriculum.episodeRefs;
    if (isRecordArray(episodeRef)) {
        const matchingRef = episodeRef.find((candidate) => candidate.episodeId === episode.episodeId);
        if (!matchingRef ||
            matchingRef.contentHash !== (0, decision_registry_1.hashCanonicalBody)(episode)) {
            return fail([
...
const validateV2LearningPackage = (input, context) => {
    let snapshotOptions;
    try {
        if (input !== null && typeof input === "object" && !Array.isArray(input)) {
            const episodeDescriptor = Reflect.getOwnPropertyDescriptor(input, "episode");
            if (episodeDescriptor?.enumerable === true &&
                !episodeDescriptor.get &&
                !episodeDescriptor.set &&
                hasOwn(episodeDescriptor, "value") &&
                episodeDescriptor.value !== null &&
                typeof episodeDescriptor.value === "object" &&
                !Array.isArray(episodeDescriptor.value)) {
                const episode = episodeDescriptor.value;
                const schemaDescriptor = Reflect.getOwnPropertyDescriptor(episode, "schemaVersion");
                if (schemaDescriptor?.enumerable === true &&
                    !schemaDescriptor.get &&
                    !schemaDescriptor.set &&
                    hasOwn(schemaDescriptor, "value") &&
    ...
    if (inputSnapshot.ok === false)
        return fail([inputSnapshot.issue]);
    return safely(() => validateLearningPackageInternal(inputSnapshot.value, context));
};
exports.validateV2LearningPackage = validateV2LearningPackage;
const validateV2CurriculumProjection = (input, context) => {
    ...
    return safely(() => {
        ...
        if (!registryResult.ok)
            return fail(registryResult.issues);
        return validateCurriculumInternal(inputSnapshot.value, registryResult.value);
    });
};
exports.validateV2CurriculumProjection = validateV2CurriculumProjection;
const materialKeys = [
    "skillIds",
    "phraseFrameIds",
    "grammarDistinctionIds",
...
const validateCheckpointDeepShape = (checkpoint) => {
    const stringSetKeys = [
        "coveredEpisodeIds",
    ...
    for (const key of stringSetKeys) {
        if (!isStringArray(checkpoint[key]) ||
            checkpoint[key].some((value) => value.length === 0)) {
    ...
    for (let index = 0; index < checkpoint.evidenceRequirements.length; index += 1) {
        const requirement = checkpoint.evidenceRequirements[index];
        const path = `$.checkpoint.evidenceRequirements[${index}]`;
        const shapeIssue = oneExactObjectIssue(requirement, [
            "assessmentNodeId",
            "objectiveId",
            "skillId",
            "construct",
        ...
        if (shapeIssue)
            return shapeIssue;
        const targetShape = oneExactObjectIssue(requirement.target, ["targetKind", "targetId"], [], `${path}.target`);
        if (targetShape)
            return targetShape;
        const target = requirement.target;
        if (!isNonEmptyString(requirement.assessmentNodeId) ||
            !isNonEmptyString(requirement.objectiveId) ||
            !isNonEmptyString(requirement.skillId) ||
            !["semantic", "listening", "recall", "spoken", "interaction"].includes(String(requirement.construct)) ||
            ![
                "encounter_build",
                "near_transfer",
                "independent_probe",
            ...
            !["objective", "semantic_slot", "critical_constraint"].includes(String(target.targetKind)) ||
            !isNonEmptyString(target.targetId) ||
    ...
    if (!isRecordArray(checkpoint.deterministicAlternateRoutes)) {
        return issue("field_type_invalid", "$.checkpoint.deterministicAlternateRoutes");
    }
    for (let index = 0; index < checkpoint.deterministicAlternateRoutes.length; index += 1) {
        const route = checkpoint.deterministicAlternateRoutes[index];
        const path = `$.checkpoint.deterministicAlternateRoutes[${index}]`;
        const shapeIssue = oneExactObjectIssue(route, [
            "primaryNodeId",
            "alternateNodeId",
            "assessedObjectiveIds",
            "evidenceTupleKeys",
        ...
        if (shapeIssue)
            return shapeIssue;
        if (!isNonEmptyString(route.primaryNodeId) ||
    ...
    for (let index = 0; index < checkpoint.criticalRepairRoutes.length; index += 1) {
        const route = checkpoint.criticalRepairRoutes[index];
        const path = `$.checkpoint.criticalRepairRoutes[${index}]`;
        const shapeIssue = oneExactObjectIssue(route, ["target", "repairNodeId", "reassessmentNodeId"], [], path);
        if (shapeIssue)
            return shapeIssue;
        const targetShape = oneExactObjectIssue(route.target, ["targetKind", "targetId"], [], `${path}.target`);
        if (targetShape)
            return targetShape;
        const target = route.target;
        if (!["semantic_slot", "critical_constraint"].includes(String(target.targetKind)) ||
            !isNonEmptyString(target.targetId) ||
...
const validateCheckpointInternal = (input, context) => {
    ...
    const shapeIssues = exactObjectIssues(input, checkpointKeys, [], "$.checkpoint");
    if (shapeIssues.length > 0)
        return fail(shapeIssues);
    ...
    if (deepShapeIssue)
        return fail([deepShapeIssue]);
    if (!isStringArray(checkpoint.assessmentNodeIds) ||
        !isRecordArray(checkpoint.evidenceRequirements)) {
        return fail([
            issue("checkpoint_set_mismatch", "$.checkpoint.assessmentNodeIds"),
        ]);
    }
    const requirementNodeIds = checkpoint.evidenceRequirements.map((requirement) => String(requirement.assessmentNodeId));
    if (uniqueSecondIndex(checkpoint.assessmentNodeIds) >= 0 ||
        !sameStringSet(checkpoint.assessmentNodeIds, requirementNodeIds)) {
        return fail([
            issue("checkpoint_set_mismatch", "$.checkpoint.assessmentNodeIds"),
        ]);
    }
    if (!isRecordArray(context.declarations) ||
        !isStringArray(context.independentProbeNodeIds)) {
        return fail([issue("checkpoint_context_invalid", "$.checkpointContext")]);
    }
    for (let index = 0; index < checkpoint.evidenceRequirements.length; index += 1) {
        const requirement = checkpoint.evidenceRequirements[index];
        const contextual = context.declarations.find((candidate) => candidate.nodeId === requirement.assessmentNodeId);
        ...
        if (!contextual ||
            ...
            requirement.requiredOutcome !== "success" ||
            !context.independentProbeNodeIds.includes(String(requirement.assessmentNodeId))) {
            return fail([
                issue("checkpoint_requirement_mismatch", `$.checkpoint.evidenceRequirements[${index}]`),
            ]);
        }
    }
    ...
    const requirementObjectiveIds = [
        ...new Set(checkpoint.evidenceRequirements.map((requirement) => String(requirement.objectiveId))),
    ];
    if (!sameStringSet(checkpoint.assessedObjectiveIds, requirementObjectiveIds)) {
        return fail([
            issue("checkpoint_set_mismatch", "$.checkpoint.assessedObjectiveIds"),
        ]);
    }
    if (!isPlainObject(context.nodePhases) ||
    ...
    for (let index = 0; index < checkpoint.deterministicAlternateRoutes.length; index += 1) {
        ...
        if (!checkpoint.assessmentNodeIds.includes(String(route.primaryNodeId))) {
            return fail([
                issue("checkpoint_alternate_invalid", `${routePath}.primaryNodeId`),
            ]);
        }
        const routeRequirements = checkpoint.evidenceRequirements.filter((requirement) => requirement.assessmentNodeId === route.primaryNodeId);
        const expectedObjectiveIds = [
            ...new Set(routeRequirements.map((requirement) => String(requirement.objectiveId))),
        ];
        const expectedEvidenceTupleKeys = routeRequirements.map((requirement) => (0, evidence_1.buildLearningEvidenceTupleKey)({
            nodeId: String(requirement.assessmentNodeId),
            objectiveId: String(requirement.objectiveId),
            skillId: String(requirement.skillId),
            construct: String(requirement.construct),
            phase: String(requirement.phase),
            targetKind: String(requirement.target.targetKind),
            targetId: String(requirement.target.targetId),
        }));
        if (!isStringArray(route.assessedObjectiveIds) ||
            ...
            return fail([
                issue("checkpoint_alternate_invalid", `${routePath}.assessedObjectiveIds`),
            ]);
        }
        if (!isStringArray(route.evidenceTupleKeys) ||
        ...
        if (context.nodePhases[String(route.primaryNodeId)] !== "independent_probe" ||
        ...
        const primaryDeclaration = context.declarations.find((entry) => entry.nodeId === String(route.primaryNodeId))?.declaration;
        const alternateDeclaration = context.declarations.find((entry) => entry.nodeId === String(route.alternateNodeId))?.declaration;
        const declarationTuple = (declaration) => declaration &&
            JSON.stringify({
                objectiveId: declaration.objectiveId,
                skillId: declaration.skillId,
                construct: declaration.construct,
    ...
    const alternatePrimaryIds = checkpoint.deterministicAlternateRoutes.map((route) => String(route.primaryNodeId));
    const alternateNodeIds = checkpoint.deterministicAlternateRoutes.map((route) => String(route.alternateNodeId));
    if (uniqueSecondIndex(alternatePrimaryIds) >= 0 ||
        uniqueSecondIndex(alternateNodeIds) >= 0 ||
        !sameStringSet(alternatePrimaryIds, checkpoint.assessmentNodeIds)) {
        return fail([
            issue("checkpoint_alternate_invalid", "$.checkpoint.deterministicAlternateRoutes"),
        ]);
    }
    if (!isStringArray(checkpoint.criticalSemanticSlotIds) ||
    ...
    const duplicateSlotIndex = uniqueSecondIndex(checkpoint.criticalSemanticSlotIds);
    if (duplicateSlotIndex >= 0) {
        return fail([
            issue("checkpoint_critical_coverage_invalid", `$.checkpoint.criticalSemanticSlotIds[${duplicateSlotIndex}]`),
        ]);
    }
    const duplicateConstraintIndex = uniqueSecondIndex(checkpoint.criticalConstraintIds);
    if (duplicateConstraintIndex >= 0) {
        return fail([
            issue("checkpoint_critical_coverage_invalid", `$.checkpoint.criticalConstraintIds[${duplicateConstraintIndex}]`),
        ]);
    }
    const criticalTargets = [
        ...checkpoint.criticalSemanticSlotIds.map((id) => `semantic_slot:${id}`),
        ...checkpoint.criticalConstraintIds.map((id) => `critical_constraint:${id}`),
    ];
    const requirementTargets = checkpoint.evidenceRequirements.map((requirement) => targetKey(requirement.target));
    const repairTargets = checkpoint.criticalRepairRoutes.map((route) => targetKey(route.target));
    for (let index = 0; index < checkpoint.criticalRepairRoutes.length; index += 1) {
        const route = checkpoint.criticalRepairRoutes[index];
        if (context.nodePhases[String(route.repairNodeId)] !== "near_transfer" ||
            context.nodePhases[String(route.reassessmentNodeId)] !==
                "independent_probe" ||
            !context.independentProbeNodeIds.includes(String(route.reassessmentNodeId))) {
            return fail([
                issue("checkpoint_critical_coverage_invalid", `$.checkpoint.criticalRepairRoutes[${index}].reassessmentNodeId`),
            ]);
        }
    }
    if (criticalTargets.some((key) => requirementTargets.filter((candidate) => candidate === key).length !==
        1 ||
        repairTargets.filter((candidate) => candidate === key).length !== 1)) {
        return fail([
            issue("checkpoint_critical_coverage_invalid", "$.checkpoint.criticalRepairRoutes"),
        ]);
    }
    if (!isPlainObject(context.taughtScope) ||
    ...
    for (const key of materialKeys) {
        ...
        if (!isStringArray(taught) ||
            !isStringArray(required) ||
            required.some((id) => !taught.includes(id))) {
            return fail([
                issue("checkpoint_new_material_forbidden", `$.checkpointContext.requiredCheckpointMaterial.${key}`),
            ]);
        }
    }
    return pass(input);
};
const validateV2CheckpointContract = (input, context) => {
    const inputSnapshot = snapshotCanonicalJsonInput(input, "$.checkpoint");
    if (inputSnapshot.ok === false)
        return fail([inputSnapshot.issue]);
    const contextSnapshot = snapshotCanonicalJsonInput(context, "$.checkpointContext");
    if (contextSnapshot.ok === false)
        return fail([contextSnapshot.issue]);
    return safely(() => validateCheckpointInternal(inputSnapshot.value, contextSnapshot.value));
};
exports.validateV2CheckpointContract = validateV2CheckpointContract;
/** Validate an immutable Episode body without requiring a full package wrapper. */
const validateV2EpisodeContract = (input) => {
    let snapshotOptions;
    try {
        if (input !== null && typeof input === "object" && !Array.isArray(input)) {
            const schemaDescriptor = Reflect.getOwnPropertyDescriptor(input, "schemaVersion");
            if (schemaDescriptor?.enumerable === true &&
                !schemaDescriptor.get &&
                !schemaDescriptor.set &&
                hasOwn(schemaDescriptor, "value") &&
    ...
    return safely(() => {
        ...
        if (snapshot.value.schemaVersion === "v2-episode-contract.v2") {
            if (!hasOwn(snapshot.value, "sessionSetRef") ||
                snapshot.value.sessionSetRef === undefined) {
                return fail([
                    issue("episode_session_set_ref_required", "$.episode.sessionSetRef"),
                ]);
            }
            const exactV2Issues = exactObjectIssues(snapshot.value, EPISODE_V2_KEYS.filter((key) => key !== "checkpointContract"), ["checkpointContract"], "$.episode");
            if (exactV2Issues.length > 0)
                return fail(exactV2Issues);
            if (!Number.isSafeInteger(snapshot.value.estimatedMinutes) ||
                Number(snapshot.value.estimatedMinutes) < 30 ||
                Number(snapshot.value.estimatedMinutes) > 48) {
                return fail([
                    issue("episode_estimated_minutes_invalid", "$.episode.estimatedMinutes"),
                ]);
            }
            const sessionSetRef = snapshot.value.sessionSetRef;
            if (!isPlainObject(sessionSetRef) ||
                exactObjectIssues(sessionSetRef, ["episodeId", "version", "contentHash"], [], "$.episode.sessionSetRef").length > 0 ||
                sessionSetRef.episodeId !== snapshot.value.episodeId ||
                !isNonEmptyString(sessionSetRef.episodeId) ||
                !identities_1.V2_IDENTITY_REGEX.test(sessionSetRef.episodeId) ||
            ...
            if (containerShapeIssue)
                return fail([containerShapeIssue]);
            const v2Issues = validateEpisodeDeepShape(snapshot.value);
            return v2Issues.length > 0
                ? fail(v2Issues)
                : pass(snapshot.value);
        }
        const exactIssues = exactObjectIssues(snapshot.value, EPISODE_KEYS.filter((key) => key !== "checkpointContract"), ["checkpointContract"], "$.episode");
        if (exactIssues.length > 0)
            return fail(exactIssues);
...
exports.validateV2EpisodeContract = validateV2EpisodeContract;
