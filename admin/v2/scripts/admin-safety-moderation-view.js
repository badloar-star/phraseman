...
function banRows(model, escapeHtml, can) {
  const rows = model.items || [];
  const sourceContext = model.manualBanContext || {};
  const sourceNote = sourceContext.source === 'help_board'
    ? `<div class="notice" role="status"><strong>Передано из Help Board.</strong> Источник: ${escapeHtml(sourceContext.sourceTargetType || '—')} / <span class="mono">${escapeHtml(sourceContext.sourceTargetId || '—')}</span>. Контекст будет сохранён в preview, истории и аудите.</div>`
    : '';
  const manual = can('users.moderation.ban.write') ? `<section class="card section"><div class="card-header"><div><h2>Заблокировать по UID</h2><p>Операция требует preview и подтверждения второго администратора. Независимая блокировка чата не меняется.</p></div></div><div class="card-body">${sourceNote}<div class="report-filters"><div class="field"><label for="safety-manual-ban-uid">Канонический UID</label><input id="safety-manual-ban-uid" value="${escapeHtml(model.manualBanUid || '')}" autocomplete="off"></div><div class="field"><label for="safety-manual-ban-name">Имя для аудита</label><input id="safety-manual-ban-name" maxlength="160"></div><div class="field"><label for="safety-manual-ban-reason">Причина</label><input id="safety-manual-ban-reason" maxlength="500"></div><button class="button danger-ghost" data-action="safety-preview-manual-ban" type="button">Подготовить блокировку</button></div></div></section>` : '';
