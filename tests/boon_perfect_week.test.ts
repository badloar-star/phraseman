...
describe('PERFECT_WEEK_REWARD', () => {
  ...
  // пустое окно с анимацией. Владелец (2026-07-27) закрыл этот вопрос и вернул
  // маленькую выплату: 5 жемчужин — верх скромной шкалы сундука недели
  // (MYSTERY_TIERS 1/2/3/5). Тест ждал ноль и падал на исправленном баге.
  it('приз за полную неделю не пустой', () => {
    expect(PERFECT_WEEK_REWARD.shards).toBeGreaterThan(0);
  });
  // Полная неделя даётся тяжелее случайного сундука, поэтому не должна давать
  // меньше его верхнего тира — и при этом не размывать продажу жемчужин.
  it('не меньше верхнего тира сундука и не больше него', () => {
    expect(PERFECT_WEEK_REWARD.shards).toBe(5);
  });
});
