...
import { assertCoverageContainsRootFootprints, makeHeadProxyFromBody, makeTaperedClumpHair } from './lib/tapered_clump_mesh.mjs';
...
export const HAIR_CONFIG_SHA256 = '80339fe3fd7e20213647d831f9ef7480da5fa579489ab8bb1fd41685e654f8fa';
...
async function verifyProvenance(vendor) {
  ...
  const hair = await readFile(path.join(root, 'config', 'avatar-dna', 'human_v2_hair.v1.json'));
  if (createHash('sha256').update(hair).digest('hex') !== HAIR_CONFIG_SHA256) throw new Error('pinned hair provenance mismatch');
  return { materials: JSON.parse(materials.toString('utf8')), hair: JSON.parse(hair.toString('utf8')) };
}
...
export async function buildHumanV2Cc0Glb(options = {}) {
  ...
  const provenance = await verifyProvenance(vendor), materialConfig = provenance.materials, hairConfig = provenance.hair;
  ...
  const addGeneratedSurface = (name, surface, material, extras) => {
    const position = accessor(surface.positions, 5126, 'VEC3', typedBounds(surface.positions, 3));
    const normalAccessor = accessor(surface.normals, 5126, 'VEC3', typedBounds(surface.normals, 3));
    const index = accessor(surface.indices, 5125, 'SCALAR', typedBounds(surface.indices, 1));
    meshes.push({ name, primitives: [{ attributes: { POSITION: position, NORMAL: normalAccessor }, indices: index, material }], extras });
    nodes.push({ name, mesh: meshes.length - 1 });
  };
  const headProxy = makeHeadProxyFromBody(body);
  for (const style of ['wave', 'crop']) {
    const coverageMargin = assertCoverageContainsRootFootprints(headProxy, hairConfig[style], hairConfig.minimumScalpClearance), hair = makeTaperedClumpHair({ proxy: headProxy, families: hairConfig[style].families, coverage: hairConfig[style].coverage, minimumScalpClearance: hairConfig.minimumScalpClearance, lengthSegments: hairConfig.lengthSegments, radialSegments: hairConfig.radialSegments });
    addGeneratedSurface(`avatar_hair_${style}`, hair, materialIndex.material_hair, { generator: hairConfig.generator, style, familyCounts: hairConfig[style].families.map(family => family.count), lengthSegments: hair.lengthSegments, radialSegments: hair.radialSegments, minScalpClearance: hairConfig.minimumScalpClearance, rootRingClearance: hair.rootRingClearance, clumpCount: hair.clumpCount, tubeVertexCount: hair.clumpCount * (hair.lengthSegments * hair.radialSegments + 1), scapCapVertexCount: hair.cap.vertexCount, scapCapTriangleCount: hair.cap.triangleCount, coverage: hairConfig[style].coverage, coverageMargin, headProxy, configSha256: HAIR_CONFIG_SHA256 });
  }
  const terra = makeFittedTerraGarment({ positions: body.positions, triangles: body.triangles, sourceVertexIndex: body.sourceVertexIndex, sourcePositions: body.sourcePositions, groups: body.groups });
  addGeneratedSurface('avatar_outfit_terra', terra, materialIndex.material_cloth, { generator: 'fitted-garment-v1', sourceTriangleCount: terra.sourceTriangleCount, sourceTriangleHash: terra.sourceTriangleHash, sourceTriangleIndices: terra.sourceTriangleIndices, sourceVertexIndices: terra.sourceVertexIndices, surfaceOffset: terra.surfaceOffset, torsoBounds: terra.torsoBounds, lateralLimit: terra.lateralLimit, inwardTriangleCount: terra.inwardTriangleCount, configSha256: HAIR_CONFIG_SHA256 });
  const neck = helperBounds(body.groups['joint-neck']).center, hood = makeAssassinHood({ proxy: headProxy, neckAnchor: neck });
  addGeneratedSurface('avatar_hood_assassin', hood, materialIndex.material_cloth, { generator: 'hood-swept-rings-v1', ringSegments: hood.ringSegments, radialSegments: hood.radialSegments, anchor: hood.anchor, headProxy, configSha256: HAIR_CONFIG_SHA256 });
