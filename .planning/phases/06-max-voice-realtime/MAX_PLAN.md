...
Коммиты, в которых зафиксирована фича (последовательно, HEAD → назад):
1. **`a1888340fd52b197a07d69c2604211367918d089`** — "MAX Voice: wire post-call review to premiumDialogReview(mode:'voice')" (текущий HEAD)
2. `11d6765e783d80f5914f9f1d564c56c16c94ed43` — "Add MAX_PLAN.md: continuity spec/status doc"
3. `fb2ddab3a5de41d0360cbf766e782bcf879d8d9a` — "Add MAX Voice: realtime voice calls with AI companion (WebRTC)" (43 файла ядра фичи), родитель `1848756602db3ee07dba924669cd46545caa3777`

Все три коммита подтверждены на Mac (`git log -1` показывает `a1888340` как HEAD, скоуп-нутый `git diff --name-only` между деревьями показал ровно ожидаемые списки файлов и ничего лишнего).
...
## 9. Как запустить и проверить MAX Voice живьём на этом Mac (делать в Terminal, НЕ через device_bash)

Почему не запущено автоматически этой сессией: у `device_bash` нет способа показать мне картинку симулятора (нет screenshot-инструмента для нативных приложений, только для Chrome/артефактов), и сама сборка (`pod install`, компиляция Xcode-проекта) занимает сильно больше 45 секунд на вызов. Поэтому дальше — точные команды ДЛЯ ТЕБЯ, каждая безопасна и обычна, никакого отношения к ограничениям device_bash не имеет.

**Шаг 1 — синхронизировать зависимости:**
```bash
cd ~/Documents/phraseman
git pull    # если репозиторий подключён к remote; если работаешь прямо в рабочей копии — просто убедись, что HEAD = 3cd8d34c (или новее)
npm install
```
Это подтянет `react-native-webrtc`/`react-native-incall-manager` в `node_modules` (в package.json/package-lock.json они уже есть — см. раздел 6).

**Шаг 2 — временно включить FORCE_PREMIUM для локального теста (опционально, но иначе сервер откажет в минте без активной Premium-подписки):**
В `app/config.ts` есть готовый dev-переключатель:
```ts
const FORCE_PREMIUM_DEV_INTENT = false;  // временно поставь true для локального теста
```
Он безопасен: активен ТОЛЬКО в dev-рантайме и гасится в стор-сборках (`IS_DEV_RUNTIME && !IS_STORE_RELEASE`), см. `app/premium_guard.ts:179`. Не забудь вернуть `false` перед коммитом/публичной сборкой.

**Шаг 3 — включить голосовой kill switch локально (без похода в Firestore):**
```bash
EXPO_PUBLIC_MAX_VOICE_ENABLED=true npm run ios
```
`npm run ios` = `expo run:ios` (см. `package.json`) — сам синхронизирует `ios/` (там уже есть нативный проект) и прогонит `pod install` при необходимости, соберёт и запустит iOS-симулятор. Первая сборка после добавления webrtc — ожидай CocoaPods дольше обычного (нативная библиотека).

**Шаг 4 — что проверить руками в симуляторе (это и есть "смоук-тест", который я не могу провести за тебя без визуального доступа):**
1. `Диалоги` → должна появиться карточка "Позвонить собеседнику" с бейджем MAX (видна только если шаг 3 сработал — `isMaxVoiceEntryVisible()` = true).
2. Тап на карточку → экран `max_call_prestart` (пре-старт, остаток минут дня).
3. "Позвонить" → экран `max_call_session` — халo-сфера должна дышать/пульсировать; проверь, что запрашивается разрешение на микрофон (симулятор Mac умеет передавать звук с host-микрофона, если он подключён в настройках симулятора I/O).
4. Скажи фразу → должен появиться транскрипт снизу, ассистент должен ответить голосом; попробуй перебить ответ (barge-in) — халo должна мгновенно среагировать сменой состояния.
5. Заверши звонок → экран `max_voice_review` — секция "Разбор твоих фраз" должна показать спиннер, затем реальные корректировки (это то, что я подключил в этой сессии, коммит `a1888340`) — если сеть/OpenAI-ключ работают, там не должно остаться старого текста "скоро появится".
6. Проверь реконнект: во время звонка включи Airplane Mode на несколько секунд в симуляторе (Device → набор функций ограничен, реальнее тестировать это на реальном устройстве через Wi-Fi toggle) — звонок должен попытаться переподключиться, а не просто оборваться молча.

Если на шаге 3 карточка MAX не появилась — почти наверняка причина в том, что `expo run:ios` не подхватил `EXPO_PUBLIC_MAX_VOICE_ENABLED` (проверь, что переменная реально экспортирована ПЕРЕД командой в той же строке/сессии шелла) либо симулятор запустил закэшированный JS-бандл — сделай `npx expo start -c` (сброс кэша Metro) и пересобери.

---

## 10. Статус этой сессии на момент записи документа (обновлено)

Коммит `3cd8d34ca8999f16ab49438837e4338242853b8e` подтверждённо на HEAD Mac-репозитория (7 коммитов поверх исходного `1848756602db3ee07dba924669cd46545caa3777`, см. раздел заголовка). Ядро фичи, серверная voice-ветка пост-звонкового ревью, нативные зависимости в package.json/app.json (validated: npm install + tsc + 280 тестов) реализованы и закоммичены. Документ МАКС ПЛАН держится в актуальном состоянии по ходу работы; копия лежит в репозитории по пути `.planning/phases/06-max-voice-realtime/MAX_PLAN.md` и отправляется пользователю напрямую при каждом существенном обновлении.

**Честная граница того, что я могу подтвердить сам, а что — только ты:** я подтвердил кодом и автотестами (280 юнит/контрактных тестов, tsc, чистый npm install) всё, что можно проверить без визуального доступа к симулятору/устройству. Реальный end-to-end звонок (голос идёт туда-обратно, халo реагирует на звук на экране, барж-ин физически прерывает речь) я не наблюдал — у меня нет инструмента показать мне картинку нативного iOS/Android-приложения (device_bash — не GUI-мост, скриншот-тулы этой сессии работают только для Chrome-вкладок и HTML-артефактов). Раздел 9 выше — это ровно тот "смоук-тест", который остаётся сделать тебе лично в Terminal + симулятор, я довёл всё до точки "нажми и проверь".

**Оставшиеся объективные блокеры к полным 100% (не техническая недоработка, а внешние шаги):**
1. `expo prebuild`/`pod install`/фактическая сборка — Terminal на Mac, раздел 9.
2. Реальный OpenAI-ключ в Firebase Functions secrets — скорее всего уже есть (используется другими AI-фичами), но я не проверял именно доступность в проде — стоит свериться перед первым живым звонком.

**НЕ блокер (важная находка):** MAX-пейволл + отдельный RevenueCat offering — НЕ нужны для запуска v1/беты. `hasMax = isPremium && voiceForPremiumBeta` (default `true`) — обычный Premium уже открывает MAX-звонки. Раздел 6, п.3 — это готовый план на будущее (v2-монетизация MAX отдельно от Premium), не текущий блокер.

**Осталось в порядке приоритета (раздел 6):** (1) MAX paywall-экран + RevenueCat offering — можно и стоит делать в этой же сессии тем же паттерном (реализация в облачном репо → тесты → перенос на Mac через device_bash), это безопасно, никакого native-билда не требует. (2) Remote config документ в проде — требует ручного создания в Firestore-консоли, не автоматизируется отсюда. (3) Нативные зависимости/prebuild — см. предупреждение в разделе 6, п.1, делать НЕ через device_bash.

Следующий практический шаг (если сессия продолжится): реализовать `app/paywall_h.tsx` + подключение к `ai_dialog_home.tsx` (сейчас карточка входа в MAX просто ведёт на `max_call_prestart`, минуя paywall — гейт по факту только через `gate_ai_voice_call` remote flag, без монетизационного экрана).
