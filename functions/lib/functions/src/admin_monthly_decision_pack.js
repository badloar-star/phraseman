...
const DEFAULT_DEPENDENCIES = {
    nowMs: () => Date.now(),
    hasPermission: (auth) => (0, permissions_1.hasVerifiedCallablePermission)(auth, 'money.read'),
...
async function generateMonthlyDecisionPackResponse(data, auth, dependencies = DEFAULT_DEPENDENCIES) {
    if (!dependencies.hasPermission(auth))
    ...
    const callableAuth = auth && typeof auth === 'object' ? auth : null;
    console.info('admin_monthly_decision_pack authorized', {
        ...
        maxInlineZipBytes: monthly_decision_pack_zip_1.DECISION_PACK_ZIP_LIMIT_BYTES,
    });
