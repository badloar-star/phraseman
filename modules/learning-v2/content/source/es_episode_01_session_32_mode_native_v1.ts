...
// + Rules §4.1, Глава 4 "Мы и они" финал): испанская checkpoint-сессия 32
// переписывается в mode-native формат, зеркаля КОД (не данные) сессии 24 —
// та же checkpoint-механика. Как и voice-сессия 31, checkpoint НЕ вводит
// новых слов и не имеет newVocabulary — переиспользует РОВНО тот же набор
// 15 фраз, что и сессия 31 (ES_EPISODE_01_SESSION_32_CHECKPOINT_PHRASES =
// алиас на ES_EPISODE_01_SESSION_31_VOICE_PHRASES). Разница между voice и
// checkpoint — не в материале, а в families и в support/promptNovelty: voice
// отрабатывает произношение вслух (scripted_repeat_compare/listen_choose/
// listen_build_dictation), checkpoint проверяет применение вразнобой без
// подсказки (speed_match/listen_build_dictation/context_gap_grammar/
// phrase_builder), support:'none', promptNovelty:'novel' (см. supportFor()
// в lesson1_session_choreography_v1.ts).
//
// зачем легаси checkpointSteps() не нужно чинить так же, как voiceSteps()
// (владелец, 2026-08-28, тот же вывод, что и в сессии 24): checkpointSteps()
...
// там нет. Явный override здесь нужен по тому же аудиту сессий 14/17/25,
// который требует sessionKindOverride: choreography без явного плана молча
// берёт kind из английской карты, а не из испанской.
const phrases = ES_EPISODE_01_SESSION_32_CHECKPOINT_PHRASES;
if (phrases.length !== 15) {
  throw new Error('es_session_32_mode_native_phrase_count_invalid');
}

const L = (source: LocalizedSource): LearningV2Localized<string> => expandLocalized(source);

// зачем эта функция (владелец, 2026-08-28, тот же класс бага сессий 5/6/8-31):
...
// (fácil/único/rápido) из сессий 25/26/27.
...
// зачем 'en' вместо 'es' здесь (как в испанских сессиях 1-31): в испанском
...
// 25/26/27 через сессию 31, включая уже готовый localizedDetails.
function localizedPhraseField(
  phraseIndex: number,
  field: 'meaning' | 'explanation',
): LearningV2Localized<string> {
  const phrase = phrases[phraseIndex];
  if (!phrase?.localizedDetails) throw new Error(`es_session_32_phrase_details_missing:${phraseIndex}`);
  return Object.fromEntries(LEARNING_V2_INTERFACE_LOCALES.map((locale) => {
    ...
    if (!detail) throw new Error(`es_session_32_phrase_locale_missing:${phraseIndex}:${locale}`);
...
function localizedPhraseDistractorFeedback(
  ...
  return Object.fromEntries(LEARNING_V2_INTERFACE_LOCALES.map((locale) => {
    ...
    if (!reason) throw new Error(`es_session_32_phrase_distractor_missing:${phraseIndex}:${locale}:${value}`);
...
// зачем phrase_builder/listen_build_dictation НЕ используются на индексе 14
// (владелец, 2026-08-28, тот же 8-дистракторный бюджет, что и в сессиях
// 25-31): es-e01-s27-somos-de-acuerdo-son-de-acuerdo-tambien несёт 11
// уникальных дистракторов после дедупликации — сверх лимита в 8, который
// ломает responseFeedbackById в course_session_client_children_v1.ts.
// Индекс 14 получает speed_match вместо builder/dictation семей.
...
function listenBuildPhrase(phraseIndex: number): LearningV2ModeNativePayloadV1 {
  ...
  return Object.freeze({
    ...
    referenceAudio: { audioTargetId: `es-e01-s32-phrase-${phraseIndex + 1}`, transcript: phrase.english },
    slowReferenceAudio: { audioTargetId: `es-e01-s32-phrase-${phraseIndex + 1}-slow`, transcript: phrase.english },
...
// (владелец, 2026-08-28, checkpoint-специфика, как и сессии 8/16/24):
// checkpoint вразнобой проверяет ЛЮБОЙ признак — поэтому гейт универсально
// ставится на последнее содержательное слово каждой фразы (последний
// элемент words[]), с его собственными authored-дистракторами из
// phrases-файла.
...
// 2026-08-28, как и сессии 8/16/24): checkpoint — единственная сессия
// главы, где цель — проверить владение ЦЕЛОЙ главой сразу, без разбивки.
// pairGrid сопоставляет target испанский текст фразы с её точным переводом;
// ни одно новое слово не вводится — все 15 фраз уже честно изучены в
// сессиях 25/26/27 и повторены в сессии 31.
const SPEED_IDS = phrases.map((_phrase, index) => `es-e01-s32-pair-${index + 1}`);
...
// es_episode_01_session_24_mode_native_v1.ts по составу families и
// индексов, learningStage: 'apply_in_phrase'): три первые фразы (0-2) уже
// отработаны в интро-carousel. Оставшиеся 12 (3-14) чередуют families в
// том же порядке, что и легаси checkpointSteps(): speed_match/
// listen_build_dictation/context_gap_grammar/phrase_builder, с нарастающей
// сложностью purpose (retrieval → near_transfer → independent_check).
// Индекс 14 (11 дистракторов, сверх бюджета) получает speed_match вместо
// builder/dictation — единственное отклонение от механической мозаики
// сессии 24.
export const ES_EPISODE_01_SESSION_32_MODE_NATIVE_PRACTICE_V1 = Object.freeze<readonly SessionModeNativePracticeSourceV1[]>([
  ...
  { family: 'phrase_builder', instruction: INSTRUCTION.buildPhrase, purpose: 'independent_check', learningStage: 'apply_in_phrase', target: { kind: 'phrase', sourceIndex: 11 }, modePayload: fullPhraseBuilder(11) },
  ...
  { family: 'listen_build_dictation', instruction: INSTRUCTION.listenBuildPhrase, purpose: 'independent_check', learningStage: 'apply_in_phrase', target: { kind: 'phrase', sourceIndex: 13 }, modePayload: listenBuildPhrase(13) },
...
if (ES_EPISODE_01_SESSION_32_MODE_NATIVE_PRACTICE_V1.length !== 12) {
  throw new Error('es_session_32_mode_native_practice_count_invalid');
}
