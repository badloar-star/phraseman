...
const DIRECT_OPERATIONS = ['get', 'list', 'create', 'update', 'delete'];
/**
 ...
 */
const CRITICAL_LEGACY_ACCESS_CASES = [
    {
 ...
 */
const LEGACY_DIRECT_NAMESPACE_SPECS = [
    {
...
const NESTED_DYNAMIC_LEGACY_ACCESS_CASES = [
    {
...
function legacyNamespaceCases(specs) {
    return specs.flatMap((spec) => {
        const collectionPath = spec.collectionPath ?? spec.documentPath.slice(0, spec.documentPath.lastIndexOf('/'));
        return spec.operations.map((operation) => {
            const lastSlash = spec.documentPath.lastIndexOf('/');
            const operationDocumentPath = `${spec.documentPath.slice(0, lastSlash)}/` +
                `${spec.documentPath.slice(lastSlash + 1)}-${operation}`;
            return {
                label: `${spec.namespace} ${operation}`,
                operation: operation === 'list' && spec.listMode === 'collection-group'
...
const LEGACY_DIRECT_ACCESS_CASES = [
...
describe('Learning V2 direct Firestore authoring security', () => {
    ...
    beforeAll(async () => {
        if (!process.env.FIRESTORE_EMULATOR_HOST) {
            throw new Error('FIRESTORE_EMULATOR_HOST is required; run this test through firebase emulators:exec.');
        }
        testEnv = await (0, rules_unit_testing_1.initializeTestEnvironment)({
            projectId: PROJECT_ID,
            firestore: {
                rules: (0, node_fs_1.readFileSync)(RULES_PATH, 'utf8'),
            },
        });
    ...
    async function seed(pathValue, data = {}) {
        await testEnv.withSecurityRulesDisabled(async (context) => {
            ...
            await (0, firestore_1.setDoc)((0, firestore_1.doc)(firestore, pathValue), {
                marker: 'seed',
                ...data,
            });
        });
    }
    async function runDirectOperation(collectionName, operation) {
        ...
        switch (operation) {
            case 'get':
                return (0, firestore_1.getDoc)((0, firestore_1.doc)(adminDb, targetPath));
            case 'list':
                return (0, firestore_1.getDocs)((0, firestore_1.query)((0, firestore_1.collection)(adminDb, collectionName), (0, firestore_1.limit)(10)));
            case 'create':
                return (0, firestore_1.setDoc)((0, firestore_1.doc)(adminDb, targetPath), { marker: 'created' });
            case 'update':
                return (0, firestore_1.updateDoc)((0, firestore_1.doc)(adminDb, targetPath), { marker: 'updated' });
            case 'delete':
                return (0, firestore_1.deleteDoc)((0, firestore_1.doc)(adminDb, targetPath));
        }
    }
    async function runLegacyOperation(testCase) {
        ...
        switch (testCase.operation) {
            case 'get':
                return (0, firestore_1.getDoc)((0, firestore_1.doc)(adminDb, testCase.path));
            case 'list':
                return (0, firestore_1.getDocs)((0, firestore_1.query)((0, firestore_1.collection)(adminDb, testCase.path), (0, firestore_1.limit)(10)));
            case 'create':
                return (0, firestore_1.setDoc)((0, firestore_1.doc)(adminDb, testCase.path), writeData);
            case 'update':
                return (0, firestore_1.updateDoc)((0, firestore_1.doc)(adminDb, testCase.path), writeData);
            case 'delete':
                return (0, firestore_1.deleteDoc)((0, firestore_1.doc)(adminDb, testCase.path));
            case 'set-merge-create':
            case 'set-merge-update':
                return (0, firestore_1.setDoc)((0, firestore_1.doc)(adminDb, testCase.path), writeData, { merge: true });
            case 'batch-set-create':
            case 'batch-set-update': {
                const batch = (0, firestore_1.writeBatch)(adminDb);
                batch.set((0, firestore_1.doc)(adminDb, testCase.path), writeData);
                return batch.commit();
            }
            case 'collection-group-list':
                return (0, firestore_1.getDocs)((0, firestore_1.query)((0, firestore_1.collectionGroup)(adminDb, testCase.path), (0, firestore_1.limit)(10)));
            case 'transaction-update':
                return (0, firestore_1.runTransaction)(adminDb, async (transaction) => {
                    const target = (0, firestore_1.doc)(adminDb, testCase.path);
                    await transaction.get(target);
                    transaction.update(target, writeData);
                });
        }
    }
    test.each(SERVER_ONLY_COLLECTIONS.flatMap((collectionName) => DIRECT_OPERATIONS.map((operation) => ({ collectionName, operation }))))('$collectionName denies direct admin $operation', async ({ collectionName, operation }) => {
        await (0, rules_unit_testing_1.assertFails)(runDirectOperation(collectionName, operation));
    });
    test.each(['get', 'create', 'update', 'delete'])('owner client cannot access nested V2 progress $0', async (operation) => {
        const path = 'users/progress-owner/v2_progress/season-r1/episodes/episode-1/evidence/letk1-tuple';
        const run = operation === 'get' ? (0, firestore_1.getDoc)((0, firestore_1.doc)(adminDb, path)) : operation === 'create' ? (0, firestore_1.setDoc)((0, firestore_1.doc)(adminDb, path), { marker: 'client' }) : operation === 'update' ? (0, firestore_1.updateDoc)((0, firestore_1.doc)(adminDb, path), { marker: 'client' }) : (0, firestore_1.deleteDoc)((0, firestore_1.doc)(adminDb, path));
        await (0, rules_unit_testing_1.assertFails)(run);
    });
    ...
    test('matchmaking_queue denies admin get of another player', async () => {
        ...
        await (0, rules_unit_testing_1.assertFails)((0, firestore_1.getDoc)((0, firestore_1.doc)(adminDb, targetPath)));
    });
    test.each(LEGACY_DIRECT_ACCESS_CASES)('preserves legacy admin operation: $label', async (testCase) => {
        await (0, rules_unit_testing_1.assertSucceeds)(runLegacyOperation(testCase));
    });
});
//# sourceMappingURL=v2_authoring_rules.emulator.test.js.map