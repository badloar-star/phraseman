...
const quoteKey = (stableId, quoteId) => `learning-v2:access-quote:${stableId}:${quoteId}`;
...
async function finalizeV2AccessPurchase(repository, policy, input) {
    ...
    return repository.runTransaction(async (transaction) => {
        ...
        const liveGeneration = Number(account?.accountGeneration ??
            account
                ?.generation);
        ...
        const [quoteDocument, gateDocument] = await Promise.all([
            transaction.get(quoteKey(input.stableId, input.request.quoteId)),
