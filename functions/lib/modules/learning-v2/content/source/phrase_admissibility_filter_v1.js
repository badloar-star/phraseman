...
exports.checkPhraseAdmissibility = checkPhraseAdmissibility;
exports.isPhraseAdmissible = isPhraseAdmissible;
/**
 ...
 */
const ALLOWED_PROPER_NAMES = Object.freeze([]);
/**
 ...
 */
const ALLOWED_PLACE_NAMES = Object.freeze([
    'Madrid',
    'London',
    'Berlin',
    'Paris',
...
const SHORT_ANSWER_RE = /^(yes|no)\s*,?\s+(i|you|he|she|it|we|they)\s*(?:\s(?:am|is|are)|['’](?:m|s|re))(\s+not|n['’]?t)?\s*[.!?]?$/i;
/**
 ...
 */
const NO_STANDALONE_MEANING = Object.freeze([
    'and you',
    'me too',
    'you too',
    'it is',
 ...
 */
const TEXTBOOK_DEAD = Object.freeze([
    {
 ...
 */
function findProperName(english) {
    ...
    for (let i = 0; i < words.length; i += 1) {
        ...
        if (ALLOWED_PROPER_NAMES.some((n) => n.toLowerCase() === lower))
            continue;
        if (ALLOWED_PLACE_NAMES.some((n) => n.toLowerCase() === lower))
            continue;
        return stem;
    }
    return null;
 ...
 */
function checkPhraseAdmissibility(input) {
    ...
    if (SHORT_ANSWER_RE.test(english)) {
        rejections.push({
            ...
            why: `«${english}» — краткий ответ. Русский краткий ответ не повторяет глагол, ` +
    ...
    if (NO_STANDALONE_MEANING.includes(flat)) {
        rejections.push({
            ...
            why: `«${english}» вне диалога ничего не значит — в карточке выглядит обрубком. ` +
    ...
    if (name !== null) {
        rejections.push({
            code: 'proper_name',
            why: `«${english}» содержит чужое имя (${name}). Имя постороннего человека мешает ` +
...
function isPhraseAdmissible(input) {
    return checkPhraseAdmissibility(input).length === 0;
}
//# sourceMappingURL=phrase_admissibility_filter_v1.js.map