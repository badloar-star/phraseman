...
(function () {
  ...
  // Темы — строго палитры приложения: constants/cinemaThemes.ts (Полночь по умолчанию, Уголь),
  // components/ui/v2_theme.ts (Лес) и два одобренных варианта хрома Атласа.
  const THEMES = [
    { id: "midnight", label: "Полночь", sw: "#8FA0FF" },
    { id: "ember", label: "Уголь", sw: "#FFCC55" },
    { id: "forest", label: "Лес", sw: "#47C870" },
    { id: "gold", label: "Золото", sw: "#D6B35A" },
    { id: "coral", label: "Коралл", sw: "#FF7F50" },
  ];
  const state = { current: null, slow: false, reduced: false, theme: "midnight", variant: "a", cfg: null };
  ...
  function init(cfg) {
    ...
    const h1 = el("h1", null, [el("small", null, [cfg.kicker || "Диалоги"]), cfg.title]);
    ...
    // Варианты интерфейса (три типа на экран, владелец выбирает)
    if (cfg.variants && cfg.variants.length) {
      const vb = el("div");
      vb.appendChild(el("h2", null, ["Вариант интерфейса"]));
      const vg = el("div", { class: "group" });
      cfg.variants.forEach((v) => {
        vg.appendChild(el("button", {
          class: "pbtn", "data-variant-id": v.id, "aria-pressed": String(v.id === state.variant),
          onclick: () => {
            state.variant = v.id;
            const ph = document.querySelector(".phone");
            if (ph) ph.setAttribute("data-variant", v.id);
            document.querySelectorAll(".pbtn[data-variant-id]").forEach((x) => x.setAttribute("aria-pressed", String(x.getAttribute("data-variant-id") === v.id)));
            const vn = document.getElementById("variant-note");
            if (vn) vn.innerHTML = v.note || "";
      ...
      panel.appendChild(el("div", { class: "note", id: "variant-note", html: (cfg.variants.find((v) => v.id === state.variant) || cfg.variants[0]).note || "" }));
    }
    ...
    if (cfg.variants && fromQuery && cfg.variants.some((v) => v.id === fromQuery)) state.variant = fromQuery;
    const ph0 = document.querySelector(".phone");
    if (ph0 && cfg.variants) ph0.setAttribute("data-variant", state.variant);
    document.querySelectorAll(".pbtn[data-variant-id]").forEach((x) => x.setAttribute("aria-pressed", String(x.getAttribute("data-variant-id") === state.variant)));
