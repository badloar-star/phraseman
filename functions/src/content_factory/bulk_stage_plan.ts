...
import { activePromptProfile } from './prompt_promotion_registry';
...
export interface BulkStagePlanRequest {
  ...
  readonly kinds: readonly GenerationStageKind[];
  readonly scopes: readonly string[];
  readonly dependencyPolicy: 'approved_only';
}

export interface ApprovedBulkPrerequisite {
...
export function parseBulkStagePlanRequest(value: unknown): BulkStagePlanRequest {
  if (!record(value) || Object.keys(value).some((key) => !FIELDS.has(key))) throw new Error('bulk_stage_plan_invalid');
  ...
  const kinds = Array.isArray(value.kinds) ? [...new Set(value.kinds.map(String))].sort() as GenerationStageKind[] : [];
  if (!TOKEN_RE.test(requestId) || !TOKEN_RE.test(idempotencyKey) || !LOCALE_RE.test(studyTarget) || !LOCALE_RE.test(sourceLocale) || !objective || objective.length > 1000 || dependencyPolicy !== 'approved_only' || kinds.length === 0 || kinds.some((kind) => !KINDS.has(kind))) throw new Error('bulk_stage_plan_invalid');

  const hasRange = value.lessonRange !== undefined;
  const hasScopes = value.scopes !== undefined;
  if (hasRange === hasScopes) throw new Error('bulk_stage_scope_mode_invalid');
  let scopes: string[];
  if (hasRange) {
    if (!record(value.lessonRange)) throw new Error('bulk_stage_lesson_range_invalid');
    const start = Number(value.lessonRange.start);
    const end = Number(value.lessonRange.end);
    if (!Number.isSafeInteger(start) || !Number.isSafeInteger(end) || start < 1 || end < start || end > 32 || kinds.some((kind) => stageCapability(kind).scopeType !== 'lesson')) throw new Error('bulk_stage_lesson_range_invalid');
    ...
    if (scopes.length === 0 || scopes.some((scope) => !TOKEN_RE.test(scope))) throw new Error('bulk_stage_plan_invalid');
    for (const kind of kinds) {
      const prefix = `${stageCapability(kind).scopeType}-`;
      if (scopes.some((scope) => !scope.startsWith(prefix))) throw new Error('bulk_stage_scope_kind_invalid');
    }
  }
  if (scopes.length * kinds.length > 100) throw new Error('bulk_stage_plan_too_large');
...
export function buildBulkStagePlan(input: BulkStagePlanRequest, approved: readonly ApprovedBulkPrerequisite[]) {
  const units: GenerationStageUnit[] = [];
  const conflicts: Array<{ kind: GenerationStageKind; scopeId: string; code: 'approved_prerequisite_missing' | 'approved_prerequisite_ambiguous'; prerequisiteKind: GenerationStageKind }> = [];
  for (const scopeId of input.scopes) {
    for (const kind of input.kinds) {
      ...
      assertStageCapabilityRequest({ kind, count, cefr: input.cefr, studyTarget: input.studyTarget, sourceLocale: input.sourceLocale, prerequisiteKinds: capability.prerequisiteKinds });
      const activePrompt = activePromptProfile(kind);
      units.push(createGenerationStageUnit({
  ...
  const planFingerprint = createHash('sha256').update(`content-stage-bulk-v1\n${canonical(normalized)}`).digest('hex');
