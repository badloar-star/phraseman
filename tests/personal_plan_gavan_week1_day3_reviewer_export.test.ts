...
import {
  ...
  GAVAN_WEEK1_DAY3_REVIEWER_EXPORT_PATH,
  validateGavanWeek1Day3ReviewerExport,
  writeGavanWeek1Day3ReviewerExport,
} from '../tools/personal_plan_gavan_week1_day3_reviewer_export';

const GENERATED_AT = '2026-06-02T18:00:00.000Z';
const BROKEN_ENCODING_RE = /[\u00d0\u00d1\u00c2\u00e2\ufffd]/;

function candidate() {
  return buildGavanWeek1Day3BlueprintCandidate(
    buildGavanWeek1ExpansionStandards({ generatedAt: GENERATED_AT }),
    { generatedAt: GENERATED_AT },
  );
}

describe('Gavan week 1 day 3 reviewer export', () => {
  beforeEach(() => {
    if (existsSync(GAVAN_WEEK1_DAY3_REVIEWER_EXPORT_PATH)) {
      rmSync(GAVAN_WEEK1_DAY3_REVIEWER_EXPORT_PATH, { force: true });
    }
  });

  afterAll(() => {
    writeGavanWeek1Day3ReviewerExport(candidate(), {
      generatedAt: GENERATED_AT,
      targetPath: GAVAN_WEEK1_DAY3_REVIEWER_EXPORT_PATH,
    });
  });

  it('exports every content unit and explanation card for manual review', () => {
    const blueprint = candidate();
    const reviewExport = buildGavanWeek1Day3ReviewerExport(blueprint, {
      generatedAt: GENERATED_AT,
    });

    expect(reviewExport.kind).toBe('gavan_week1_day3_reviewer_export');
    expect(reviewExport.dayId).toBe('gavan-week1-day3');
    ...
    expect(validateGavanWeek1Day3ReviewerExport(reviewExport, blueprint)).toEqual({
      valid: true,
      issues: [],
    });
  });

  it('keeps media and pronunciation claims honest for a non-final blueprint', () => {
    const reviewExport = buildGavanWeek1Day3ReviewerExport(candidate(), {
      generatedAt: GENERATED_AT,
    });
  ...
  it('exports exercise blueprint coverage without pretending exercises are built', () => {
    const blueprint = candidate();
    const reviewExport = buildGavanWeek1Day3ReviewerExport(blueprint, {
      generatedAt: GENERATED_AT,
    });
    ...
    expect(reviewExport.exerciseCoverage).toEqual({
      requiredTypes: ['missing_word', 'micro_dialogue', 'listening_choice', 'active_recall'],
      presentTypes: ['missing_word', 'micro_dialogue', 'listening_choice', 'active_recall'],
      valid: true,
    });
  ...
  it('catches missing rows fake media claims forbidden anchors and missing exercise coverage', () => {
    ...
    const corrupted = {
      ...
      exerciseRows: reviewExport.exerciseRows.filter((row) => row.exerciseType !== 'micro_dialogue'),
      exerciseCoverage: {
        ...reviewExport.exerciseCoverage,
        presentTypes: reviewExport.exerciseCoverage.presentTypes.filter((type) => type !== 'micro_dialogue'),
        valid: false,
      },
    ...
    const result = validateGavanWeek1Day3ReviewerExport(corrupted, blueprint);
  ...
  it('writes deterministic reviewer JSON only under temp or report roots', () => {
    const result = writeGavanWeek1Day3ReviewerExport(candidate(), {
      generatedAt: GENERATED_AT,
      targetPath: GAVAN_WEEK1_DAY3_REVIEWER_EXPORT_PATH,
    });
    ...
    expect(existsSync(GAVAN_WEEK1_DAY3_REVIEWER_EXPORT_PATH)).toBe(true);

    const parsed = JSON.parse(readFileSync(GAVAN_WEEK1_DAY3_REVIEWER_EXPORT_PATH, 'utf8'));
    expect(parsed.kind).toBe('gavan_week1_day3_reviewer_export');
  ...
  it('rejects app tools tests and root config write targets', () => {
    const appResult = writeGavanWeek1Day3ReviewerExport(candidate(), {
      generatedAt: GENERATED_AT,
      targetPath: path.join(process.cwd(), 'app', 'gavan-week1-day3-reviewer-export.json'),
    });
    const toolsResult = writeGavanWeek1Day3ReviewerExport(candidate(), {
      generatedAt: GENERATED_AT,
      targetPath: path.join(process.cwd(), 'tools', 'gavan-week1-day3-reviewer-export.json'),
    });
    const testsResult = writeGavanWeek1Day3ReviewerExport(candidate(), {
      generatedAt: GENERATED_AT,
      targetPath: path.join(process.cwd(), 'tests', 'gavan-week1-day3-reviewer-export.json'),
    });
  ...
  it('does not import live catalog quiz UI storage audio scoring or navigation', () => {
    const source = readFileSync(
      path.join(process.cwd(), 'tools', 'personal_plan_gavan_week1_day3_reviewer_export.ts'),
      'utf8',
    );
