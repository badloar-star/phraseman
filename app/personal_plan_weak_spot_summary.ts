...
export type PlanWeakSpotSummary = {
  ...
  due: {
    recall: PlanWeakSpotDueSummary;
    trainer: PlanWeakSpotDueSummary;
    mistakeAnalytics: PlanWeakSpotDueSummary;
  };
...
function addRecoveryToSignal(signal: MutablePlanWeakSpotSignal, candidate: PlanRecoveryCandidate): void {
  if (candidate.target === 'recall') {
    signal.recallDueCount += 1;
  } else if (candidate.target === 'trainer') {
    signal.trainerDueCount += 1;
  } else {
    signal.mistakeAnalyticsDueCount += 1;
  }
...
function finalizeSignal(signal: MutablePlanWeakSpotSignal): PlanWeakSpotSignal {
  return {
    ...
    recallDueCount: signal.recallDueCount,
    trainerDueCount: signal.trainerDueCount,
    mistakeAnalyticsDueCount: signal.mistakeAnalyticsDueCount,
...
export function buildPlanWeakSpotSummary(
  ...
  const weakSpots = [...signals.values()]
    ...
    .filter((signal) =>
      signal.wrongCount > 0 ||
      signal.recallDueCount > 0 ||
      signal.trainerDueCount > 0 ||
      signal.mistakeAnalyticsDueCount > 0,
    )
    .sort((a, b) =>
      b.wrongCount - a.wrongCount ||
      b.recallDueCount + b.trainerDueCount + b.mistakeAnalyticsDueCount -
        (a.recallDueCount + a.trainerDueCount + a.mistakeAnalyticsDueCount) ||
  ...
  return {
    ...
    due: {
      recall: dueFor(recoveryCandidates, 'recall'),
      trainer: dueFor(recoveryCandidates, 'trainer'),
      mistakeAnalytics: dueFor(recoveryCandidates, 'mistake_analytics'),
    },
    weakSpots,
  };
}
