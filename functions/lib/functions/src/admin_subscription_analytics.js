...
exports.handleAdminSubscriptionAnalytics = handleAdminSubscriptionAnalytics;
...
function opaqueIdentifier(value, requestKey) {
    const normalized = String(value ?? '').trim();
    return normalized ? (0, node_crypto_1.createHmac)('sha256', requestKey).update(normalized).digest('hex') : undefined;
}
async function handleAdminSubscriptionAnalytics(request) {
    if (!(0, permissions_1.hasVerifiedCallablePermission)(request.auth, 'money.read')) {
    ...
    const requestIdentifierKey = (0, node_crypto_1.randomBytes)(32);
    const rows = [];
    let cursor = null;
