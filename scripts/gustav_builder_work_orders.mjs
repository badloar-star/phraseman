...
const WORK_ORDER_OVERRIDES = {
  ...
  flashcards: {
    builderId: 'french_flashcard_pack_builder',
    requiredSources: ['le_robert_dictionary', 'tv5monde_apprendre'],
    outputArtifacts: ['fr_flashcard_starter_packs.json', 'fr_flashcard_french_realities_packs.json'],
    gates: ['not_lesson_fanout_gate', 'card_content_language_gate', 'srs_target_gate'],
    reasoningLevel: 'high',
  },
...
function defaultOrder(feature) {
  return {
    ...
    reasoningLevel: feature.priority <= 10 ? 'deep' : 'high',
  };
}
