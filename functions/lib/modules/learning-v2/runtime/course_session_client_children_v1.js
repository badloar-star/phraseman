...
function materializeLearningV2CourseSessionSavablePhraseV1(input) {
    ...
    const targetText = text(input.targetText, 4000);
    const meaningByLocale = localized(input.meaningByLocale, 4000);
...
function preflight(value) {
    ...
    while (stack.length) {
        ...
        if (++nodes > 12000 || current.depth > 20)
            fail();
...
function parseLearningV2CourseSessionIntroChildV1(raw) {
    ...
    const body = {
        ...
        learningOutcomeByLocale: localized(value.learningOutcomeByLocale, 1000),
    ...
    const interactions = Object.freeze(value.interactions.map((entry, index) => {
        ...
        if (entry.scriptedAlternate !== null) {
            ...
            scriptedAlternate = Object.freeze({
                ...
                instruction: text(entry.scriptedAlternate.instruction, 1000),
        ...
        return Object.freeze({
            ...
            accessibilityLabel: text(entry.accessibilityLabel, 1000),
            scriptedAlternate,
        });
    }));
    const body = {
