...
describe("Arena chrome layout source contract", () => {
  ...
  test("фон экрана идёт под сейф-зону: отступ живёт на содержимом, не на корне", () => {
    ...
    expect(screen).not.toContain(
      "styles.root, { backgroundColor: P.bg, paddingTop",
    );
  ...
  test("место под плавающий таббар добавляется содержимому через контекст", () => {
    ...
    const withInset =
      screen.match(/paddingBottom: Math\.max\([^)]*\) \+ chromeInset/g) ?? [];
...
describe("Arena screen back lock source contract", () => {
  ...
  test("allows callers to disable the actual header back control without changing its default", () => {
    const backPressable = screen.match(
      /<Pressable\b(?:(?!<\/Pressable>)[\s\S])*accessibilityLabel=\{arenaText\(lang, 'back'\)\}(?:(?!<\/Pressable>)[\s\S])*<\/Pressable>/,
    )?.[0];

    expect(screen).toContain("backDisabled = false");
    expect(screen).toContain("backDisabled?: boolean;");
    expect(backPressable).toContain("disabled={backDisabled}");
    expect(backPressable).toContain(
      "accessibilityState={{ disabled: backDisabled }}",
    );
    expect(backPressable).toContain(
      "backDisabled ? styles.backDisabled : null",
    );
    expect(backPressable).toContain(
      "onPress={onBack ?? (() => safeRouterBack(router, navigationFallbackForPath(pathname) as never))}",
    );
    expect(screen).toContain("backDisabled: { opacity: 0.45 }");
  });
});
