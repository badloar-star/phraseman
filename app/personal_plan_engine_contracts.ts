...
export type PlanRecoveryCandidateTarget = 'mistake_practice';
...
function recoveryTargetsForPolicy(policy: PlanExerciseRecoveryPolicy): PlanRecoveryCandidateTarget[] {
  if (policy === 'capture_mistake') return ['mistake_practice'];
  return [];
}
...
function exerciseTypeForTask(task: PlanDailyTask): PlanExerciseType {
  ...
  if (task.kind === 'plan_phrase_recall') return 'plan_phrase_recall';
  ...
  if (task.kind === 'mistake_practice') return 'mistake_practice';
...
function recoveryPolicyForTask(task: PlanDailyTask): PlanExerciseRecoveryPolicy {
  if (task.kind === 'plan_quiz') return 'capture_mistake';
  if (task.kind === 'plan_phrase_lesson') return 'capture_mistake';
