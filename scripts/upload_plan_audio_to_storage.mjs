...
const ONLY_FILE = onlyFileArg
  ...
  : null;
const PARTIAL_UPLOAD = Boolean(ONLY || ONLY_FILE);
...
function loadExistingUrlMap() {
  const entries = new Map();
  if (!fs.existsSync(OUT_MAP_TS)) return entries;
  const src = fs.readFileSync(OUT_MAP_TS, 'utf8');
  const re = /^\s*"((?:[^"\\]|\\.)*)":\s*"((?:[^"\\]|\\.)*)",?\s*$/gm;
...
// instead of bundling ${sortedEntries.length} mp3 (~126 MB) into the native binary.
...
`;
  ...
  if (PARTIAL_UPLOAD) {
    console.log(`Merged ${mapEntries.length} uploaded/skipped entries into existing URL map.`);
  }
