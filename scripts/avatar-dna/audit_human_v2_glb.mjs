...
async function auditProjectHairAndGarments(gltf, bin, errors) {
  ...
  if (outfit?.primitives?.[0]) {
    const primitive = outfit.primitives[0], position = reader(gltf, bin, primitive.attributes?.POSITION, errors), normalAccessor = reader(gltf, bin, primitive.attributes?.NORMAL, errors), index = reader(gltf, bin, primitive.indices, errors), sourceIds = outfit.extras?.sourceVertexIndices;
    if (!Array.isArray(sourceIds) || !position || sourceIds.length !== position.accessor.count || new Set(sourceIds).size !== sourceIds.length || sourceIds.some(id => !isUint(id) || id >= body.sourcePositions.length)) err(errors, 'garment_split_seam');
    else if (normalAccessor) for (let row=0; row<position.accessor.count; row+=1) { const source=body.sourcePositions[sourceIds[row]], delta=[position.read(row,0)-source[0],position.read(row,1)-source[1],position.read(row,2)-source[2]], length=Math.hypot(...delta), normalLength=Math.hypot(normalAccessor.read(row,0),normalAccessor.read(row,1),normalAccessor.read(row,2)), dot=(delta[0]*normalAccessor.read(row,0)+delta[1]*normalAccessor.read(row,1)+delta[2]*normalAccessor.read(row,2))/(length*normalLength); if(Math.abs(length-.032)>1e-5||Math.abs(normalLength-1)>1e-5||dot<.99999)err(errors,'invalid_garment_offset'); }
    if (position && index && normalAccessor) for(let row=0;row<index.accessor.count;row+=3){const a=index.read(row,0),b=index.read(row+1,0),c=index.read(row+2,0),ux=position.read(b,0)-position.read(a,0),uy=position.read(b,1)-position.read(a,1),uz=position.read(b,2)-position.read(a,2),vx=position.read(c,0)-position.read(a,0),vy=position.read(c,1)-position.read(a,1),vz=position.read(c,2)-position.read(a,2), face=[uy*vz-uz*vy,uz*vx-ux*vz,ux*vy-uy*vx], expected=[normalAccessor.read(a,0)+normalAccessor.read(b,0)+normalAccessor.read(c,0),normalAccessor.read(a,1)+normalAccessor.read(b,1)+normalAccessor.read(c,1),normalAccessor.read(a,2)+normalAccessor.read(b,2)+normalAccessor.read(c,2)];if(face[0]*expected[0]+face[1]*expected[1]+face[2]*expected[2]<=0)err(errors,'invalid_garment_winding');}
  }
