...
describe('OpenAI transport request accounting hook', () => {
  ...
  test('runs the hook before every real HTTP retry', async () => {
    ...
    global.fetch = jest.fn(async () => {
      request += 1;
      if (request === 1) return new Response('retry', { status: 500 });
    ...
    await openAiChat({ apiKey: 'test', model: 'fake', messages: [{ role: 'user', content: 'json' }], maxTokens: 10, temperature: 0, responseFormat: { type: 'json_object' }, beforeRequest: async (attempt) => { attempts.push(attempt); } });
  ...
  test('stops before fetch when the budget hook rejects', async () => {
    ...
    await expect(openAiChat({ apiKey: 'test', model: 'fake', messages: [{ role: 'user', content: 'json' }], maxTokens: 10, temperature: 0, beforeRequest: async () => { throw new Error('cap'); } })).rejects.toThrow('cap');
