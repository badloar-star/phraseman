          ...
          {hasBundledCompatibilityPlanContentDay(planId, dayIndex) ? (
            <ReportErrorButton
              ...
              dataText={triLang(lang, {
                  ...
                  en: `Day ${dayIndex} completed${taskTitle ? ` · ${taskTitle}` : ''}`,
        ...
        >
          ...
          <View style={[styles.labelPill, { backgroundColor: accent + '1F' }]}>
            <Text style={[styles.labelText, { color: accent }]}>
              {allDone
                  ? triLang(lang, { ru: 'День завершён!', uk: 'День завершено!', en: 'Day complete!', es: '¡Día completado!', 'pt-BR': 'Dia concluído!', vi: 'Đã hoàn thành ngày!', id: 'Hari selesai!', tr: 'Gün tamamlandı!', pl: 'Dzień ukończony!' })
                  : triLang(lang, { ru: 'Задание выполнено!', uk: 'Завдання виконано!', en: 'Task complete!', es: '¡Tarea completada!', 'pt-BR': 'Tarefa concluída!', vi: 'Đã hoàn thành nhiệm vụ!', id: 'Tugas selesai!', tr: 'Görev tamamlandı!', pl: 'Zadanie ukończone!' })}
            </Text>
          </View>

          <Text style={[styles.title, { color: t.textPrimary }]}>
            {allDone
                ? triLang(lang, { ru: `День ${dayIndex} закрыт`, uk: `День ${dayIndex} закрито`, en: `Day ${dayIndex} completed`, es: `Día ${dayIndex} completado`, 'pt-BR': `Dia ${dayIndex} concluído`, vi: `Ngày ${dayIndex} đã hoàn thành`, id: `Hari ${dayIndex} selesai`, tr: `${dayIndex}. gün tamamlandı`, pl: `Dzień ${dayIndex} zamknięty` })
                : taskTitle || triLang(lang, { ru: 'Задание выполнено', uk: 'Завдання виконано', en: 'Task completed', es: 'Tarea completada', 'pt-BR': 'Tarefa concluída', vi: 'Nhiệm vụ đã hoàn thành', id: 'Tugas selesai', tr: 'Görev tamamlandı', pl: 'Zadanie ukończone' })}
          </Text>

          <Text style={[styles.subtitle, { color: t.textMuted }]}>
            {allDone ? celebLine : triLang(lang, { ru: 'Так держать! Продолжай план.', uk: 'Так тримати! Продовжуй план.', en: 'Keep it up! Continue the plan.', es: '¡Sigue así! Continúa con el plan.', 'pt-BR': 'Continue assim! Siga com o plano.', vi: 'Cứ tiếp tục như vậy! Hãy tiếp tục kế hoạch.', id: 'Terus semangat! Lanjutkan rencananya.', tr: 'Böyle devam et! Plana devam et.', pl: 'Tak trzymaj! Kontynuuj plan.' })}
          </Text>
                ...
                {comparison.totalUsers > 0 ? (
                  <Text style={[styles.compareSub, { color: t.textMuted }]}>
                    {triLang(lang, {
                        ...
                        en: `Among ${comparison.totalUsers.toLocaleString(NUMBER_LOCALE_BY_LANG[lang])} students`,
          ...
          {dayProgress > 0 ? (
            <View style={styles.statsRow}>
              <View style={[styles.statBox, { backgroundColor: t.bgCard }]}>
                ...
                <Text style={[styles.statLabel, { color: t.textMuted }]}>{triLang(lang, { ru: 'день', uk: 'день', en: 'day', es: 'día', 'pt-BR': 'dia', vi: 'ngày', id: 'hari', tr: 'gün', pl: 'dzień' })}</Text>
              </View>
              {allDone ? (
                <View style={[styles.statBox, { backgroundColor: t.bgCard }]}>
                  <Text style={[styles.statValue, { color: '#4ECDC4' }]}>✓</Text>
                  <Text style={[styles.statLabel, { color: t.textMuted }]}>{triLang(lang, { ru: 'все задачи', uk: 'усі завдання', en: 'all tasks', es: 'todas las tareas', 'pt-BR': 'todas as tarefas', vi: 'tất cả nhiệm vụ', id: 'semua tugas', tr: 'tüm görevler', pl: 'wszystkie zadania' })}</Text>
                </View>
              ) : null}
              <View style={[styles.statBox, { backgroundColor: t.bgCard }]}>
                <Text style={[styles.statValue, { color: t.textPrimary }]}>{dayIndex}</Text>
                <Text style={[styles.statLabel, { color: t.textMuted }]}>{triLang(lang, { ru: 'номер дня', uk: 'номер дня', en: 'day number', es: 'número de día', 'pt-BR': 'número do dia', vi: 'số ngày', id: 'nomor hari', tr: 'gün numarası', pl: 'numer dnia' })}</Text>
              </View>
            </View>
          ) : null}
        </Animated.View>
          ...
          >
            ...
            >
              <Text style={[styles.primaryText, { color: t.correctText }]}>
                {allDone
                    ? triLang(lang, { ru: 'Отлично, завтра продолжим', uk: 'Чудово, завтра продовжимо', en: 'Great, we’ll continue tomorrow', es: 'Genial, seguimos mañana', 'pt-BR': 'Ótimo, continuamos amanhã', vi: 'Tuyệt vời, mai tiếp tục', id: 'Bagus, kita lanjutkan besok', tr: 'Harika, yarın devam ediyoruz', pl: 'Świetnie, jutro kontynuujemy' })
                    : triLang(lang, { ru: 'К плану дня', uk: 'До плану дня', en: 'To the day’s plan', es: 'Al plan del día', 'pt-BR': 'Para o plano do dia', vi: 'Đến kế hoạch trong ngày', id: 'Ke rencana harian', tr: 'Günlük plana git', pl: 'Do planu dnia' })}
              </Text>
