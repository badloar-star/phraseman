...
function arenaQuestionsFromCourseSurfaceEntries(rawIdentity, entries) {
    ...
    return entries.flatMap((entry) => {
        ...
        return entry.payload.items.map((item) => {
            if (!isRecord(item) || typeof item.id !== 'string' || !item.id.trim() || !Array.isArray(item.options) || item.options.length !== 4 || item.options.some((option) => typeof option !== 'string' || !option.trim()))
                throw new Error('arena_release_payload_invalid');
            ...
            if (entry.engineResolved === 'stage' && !structured)
                throw new Error('arena_release_engine_payload_mismatch');
            ...
            if (!question || !answer || seenIds.has(sourceId) || new Set(options).size !== 4 || options.filter((option) => option === answer).length !== 1 || !Number.isInteger(correctIndex) || correctIndex < 0 || correctIndex > 3 || options[correctIndex] !== answer)
                throw new Error('arena_release_payload_invalid');
            if (structured && (item.level !== levelForLesson(entry.lessonId) || typeof item.task !== 'string' || !item.task.trim() || typeof item.rule !== 'string' || !item.rule.trim() || !Number.isSafeInteger(item.expectedAnswerTimeMs) || Number(item.expectedAnswerTimeMs) < 2000 || Number(item.expectedAnswerTimeMs) > 12000 || !Array.isArray(item.sourceReferences)))
                throw new Error('arena_release_payload_invalid');
            ...
            const rand = Number.parseInt((0, node_crypto_1.createHash)('sha256').update(`${identity.courseReleaseId}:${sourceId}:rand`).digest('hex').slice(0, 12), 16) / 0x1000000000000;
            return Object.freeze({
                id,
                level: levelForLesson(entry.lessonId),
                type: structured ? String(item.type) : 'translate',
