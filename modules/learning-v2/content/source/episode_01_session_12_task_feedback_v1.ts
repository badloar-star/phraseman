...
export const EPISODE_01_SESSION_12_LISTEN_FEEDBACK_V1: Readonly<Record<string, LocalizedSource>> = Object.freeze({
...
export function episode01Session12ListenFeedbackV1(
  locale: Locale,
  correct: string,
  wrong: string,
): string | undefined {
  return EPISODE_01_SESSION_12_LISTEN_FEEDBACK_V1[`${correct}\u0000${wrong}`]?.[locale];
}

const FORM_FEEDBACK: Readonly<Record<'Is' | 'Are', LocalizedSource>> = Object.freeze({
  Is: L({
...
export function episode01Session12FormFeedbackV1(
  locale: Locale,
  target: string,
  wrong: string,
): string | undefined {
  if (!/^Am I\b/u.test(target) || (wrong !== 'Is' && wrong !== 'Are')) return undefined;
