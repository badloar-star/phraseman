...
describe('codeFilterQuiz — детерминированный код-фильтр (0 токенов)', () => {
    test('валидный вопрос проходит (null)', () => {
        expect((0, quiz_gen_judge_1.codeFilterQuiz)(good)).toBeNull();
    });
    test('не 4 опции → bad_shape', () => {
        expect((0, quiz_gen_judge_1.codeFilterQuiz)({ ...good, options: ['a', 'b', 'c'] })).toBe('bad_shape');
        expect((0, quiz_gen_judge_1.codeFilterQuiz)({ ...good, options: ['a', 'b', 'c', 'd', 'e'] })).toBe('bad_shape');
    });
    test('дубли опций → bad_shape', () => {
        expect((0, quiz_gen_judge_1.codeFilterQuiz)({ ...good, options: ['go', 'go', 'going', 'gone'] })).toBe('bad_shape');
    });
    test('пустая опция → bad_shape', () => {
        expect((0, quiz_gen_judge_1.codeFilterQuiz)({ ...good, options: ['goes', '', 'going', 'gone'] })).toBe('bad_shape');
    });
    test('correctIndex вне диапазона → bad_shape', () => {
        expect((0, quiz_gen_judge_1.codeFilterQuiz)({ ...good, correctIndex: 4 })).toBe('bad_shape');
        expect((0, quiz_gen_judge_1.codeFilterQuiz)({ ...good, correctIndex: -1 })).toBe('bad_shape');
    });
    test('пустой вопрос → bad_shape', () => {
        expect((0, quiz_gen_judge_1.codeFilterQuiz)({ ...good, question: '  ' })).toBe('bad_shape');
    });
});
describe('judgeQuizCandidate — LLM-судья (fail-closed)', () => {
    ...
    test('брак формы отклоняется БЕЗ вызова судьи (0 токенов)', async () => {
        const v = await (0, quiz_gen_judge_1.judgeQuizCandidate)({ ...good, correctIndex: 9 }, 'key');
    ...
    test('судья подтвердил → ok:true', async () => {
        ...
        const v = await (0, quiz_gen_judge_1.judgeQuizCandidate)(good, 'key');
...
//# sourceMappingURL=quiz_gen_judge.test.js.map