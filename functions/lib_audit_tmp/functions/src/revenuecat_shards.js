...
exports.isManagedPremiumProductId = isManagedPremiumProductId;
exports.shouldDeactivateOnInactiveEvent = shouldDeactivateOnInactiveEvent;
exports.isSandboxEvent = isSandboxEvent;
exports.applyVerifiedPremiumSubscriptionEvent = applyVerifiedPremiumSubscriptionEvent;
...
function isManagedPremiumProductId(rawProductId) {
...
async function resolvePremiumOwnerRef(tx, db, candidates) {
    ...
    for (const candidate of candidates) {
        ...
        if (ownerMapSnap.exists)
            addCanonicalCandidate(ownerMapSnap.data()?.canonicalStableId);
    }
...
async function resolveCanonicalDonorUid(tx, db, sourceUid) {
    ...
    for (let depth = 0; depth <= revenuecat_premium_lineage_1.MAX_OWNER_LINEAGES; depth += 1) {
        ...
        if (seen.has(uid))
            return { status: 'invalid', reason: 'donor_owner_map_cycle' };
        seen.add(uid);
        const ownerMapSnap = await tx.get(db.collection('account_identity_owner_map').doc(uid));
        if (!ownerMapSnap.exists)
            return { status: 'resolved', uid };
        const mappedUid = cleanId(ownerMapSnap.data()?.canonicalStableId);
        ...
        if (mappedUid === uid)
            return { status: 'resolved', uid };
        if (depth === revenuecat_premium_lineage_1.MAX_OWNER_LINEAGES) {
...
async function readPermanentDeletionDenials(tx, db, identities) {
    const unique = [...new Set(identities.map(cleanId).filter(Boolean))];
    const snapshots = [];
    for (const identity of unique) {
        snapshots.push(await tx.get(db.collection(account_delete_job_1.ACCOUNT_DELETE_PERMANENT_DENIALS).doc((0, account_delete_job_1.accountDeletePermanentDenialId)(identity))));
    }
    return snapshots;
}
...
async function applyVerifiedPremiumSubscriptionEvent(rawEvent, eventType, productId) {
...
async function handlePremiumSubscriptionEvent(event, eventType, productId, res) {
    try {
        const outcome = await applyVerifiedPremiumSubscriptionEvent(event, eventType, productId);
...
async function handleShardPurchaseEvent(event, eventType, productId, pack, res) {
    ...
    const boundedCandidates = (0, revenuecat_premium_lineage_1.boundPremiumOwnerCandidates)(candidateUserIds(event));
    if (boundedCandidates.status === 'quarantine') {
    ...
    try {
        const out = await db.runTransaction(async (tx) => {
            ...
            if ([...deletionSnapshots, ...permanentDenialSnapshots].some((snapshot) => snapshot.exists)) {
                return { granted: false, reason: 'account_deletion_pending_or_permanent' };
            }
    ...
    try {
        const out = await db.runTransaction(async (tx) => {
            ...
            const deletionSnapshots = [
                await tx.get(db.collection(account_delete_job_1.ACCOUNT_DELETE_TOMBSTONES).doc(uid)),
                await tx.get(db.collection(account_delete_job_1.ACCOUNT_DELETE_AUTH_MARKERS).doc(uid)),
                await tx.get(db.collection(account_delete_job_1.ACCOUNT_DELETE_TOMBSTONES).doc(canonicalUid)),
                await tx.get(db.collection(account_delete_job_1.ACCOUNT_DELETE_AUTH_MARKERS).doc(canonicalUid)),
            ];
            if ([...deletionSnapshots, ...permanentDenialSnapshots].some((snapshot) => snapshot.exists)) {
                tx.set(refundRef, {
                    eventId,
                    originalTransactionId: originalTxId,
                    productId,
                    uidHash: (0, crypto_1.createHash)('sha256').update(canonicalUid).digest('hex'),
                    reason: 'account_deletion_pending_or_permanent',
                    createdAt: admin.firestore.FieldValue.serverTimestamp(),
                });
                return { refunded: false, reason: 'account_deletion_pending_or_permanent' };
            }
            if (!userSnap.exists) {
                return { refunded: false, reason: 'missing_user_retryable', retryable: true };
            }
            const before = parseShards(userSnap.data()?.shards);
...
function premiumProjectionStrength(progress) {
    const plan = cleanId(progress.premium_plan).toLowerCase();
    if (plan === 'lifetime')
        return Number.POSITIVE_INFINITY;
    if (plan !== 'monthly' && plan !== 'yearly' && plan !== 'annual')
        return 0;
    const expiryMs = eventMs(progress.premium_expiry);
    const rcExpiryMs = eventMs(progress.premium_rc_expiry_ms);
...
function preferredLegacyPremiumBlock(left, right) {
    if (!left)
        return right;
    if (!right)
        return left;
...
async function handleTransferEvent(event, eventType, res) {
    const boundedTargets = (0, revenuecat_premium_lineage_1.boundPremiumOwnerCandidates)(transferTargetIds(event));
    const boundedSources = (0, revenuecat_premium_lineage_1.boundPremiumOwnerCandidates)(transferSourceIds(event));
    ...
    const denialRef = db.collection('revenuecat_premium_denials').doc(`transfer_${(0, crypto_1.createHash)('sha256').update(eventId).digest('hex')}`);
    try {
        const out = await db.runTransaction(async (tx) => {
            const processedSnap = await tx.get(processedRef);
            const denialSnap = await tx.get(denialRef);
            ...
            if (denialSnap.exists)
                return { moved: false, reason: 'denied_receipt_exists' };
            const canonicalSourceSet = new Set();
            ...
            const operationalDeletionSnapshots = [];
            for (const identity of transferIdentities) {
                operationalDeletionSnapshots.push(await tx.get(db.collection(account_delete_job_1.ACCOUNT_DELETE_TOMBSTONES).doc(identity)));
                operationalDeletionSnapshots.push(await tx.get(db.collection(account_delete_job_1.ACCOUNT_DELETE_AUTH_MARKERS).doc(identity)));
            }
            if ([...permanentDenialSnapshots, ...operationalDeletionSnapshots].some((snapshot) => snapshot.exists)) {
                tx.set(denialRef, {
                    ...
                    eventId,
                    eventType,
                    identityHashes: transferIdentities.map((identity) => (0, crypto_1.createHash)('sha256').update(identity).digest('hex')),
                    createdAt: admin.firestore.FieldValue.serverTimestamp(),
                });
                return { moved: false, reason: 'account_deletion_pending_or_permanent', retryable: false };
            }
            ...
            const canonicalDeletionSnapshots = [
                await tx.get(db.collection(account_delete_job_1.ACCOUNT_DELETE_TOMBSTONES).doc(recipientId)),
                await tx.get(db.collection(account_delete_job_1.ACCOUNT_DELETE_AUTH_MARKERS).doc(recipientId)),
            ...
            if (canonicalDeletionSnapshots.some((snapshot) => snapshot.exists)) {
                tx.set(denialRef, {
                    ...
                    eventId,
                    eventType,
                    ownerUidHash: (0, crypto_1.createHash)('sha256').update(recipientId).digest('hex'),
                    createdAt: admin.firestore.FieldValue.serverTimestamp(),
                });
                return { moved: false, reason: 'account_deletion_pending_or_permanent', retryable: false };
            }
            ...
            if (donorLineageDocs.length > revenuecat_premium_lineage_1.MAX_OWNER_LINEAGES
                || recipientLineageSnapshot.docs.length > revenuecat_premium_lineage_1.MAX_OWNER_LINEAGES) {
                tx.set(denialRef, {
                ...
                return { moved: false, reason: 'transfer_lineage_limit', retryable: false };
            }
            const donorUsers = [];
            for (const sourceId of canonicalDonorIds) {
                ...
                if (snap.exists)
                    donorUsers.push({ id: sourceId, ref, snap });
            }
            const recipientProgress = (recipientSnap.data()?.progress ?? {});
            const recipientLineageByHash = new Map();
            for (const lineage of recipientLineageSnapshot.docs) {
                const state = lineage.data();
                ...
                recipientLineageByHash.set(state.lineageHash, {
                    targetRef,
                    state: existing ? (0, revenuecat_premium_lineage_1.chooseAuthoritativePremiumLineageState)(existing.state, state) : state,
                    obsoleteRefs: [
            ...
            for (const lineage of donorLineageDocs) {
                const state = lineage.data();
                const existing = recipientLineageByHash.get(state.lineageHash);
                const selected = existing ? (0, revenuecat_premium_lineage_1.chooseAuthoritativePremiumLineageState)(existing.state, state) : state;
            ...
            if (recipientLineages.length > revenuecat_premium_lineage_1.MAX_OWNER_LINEAGES) {
                tx.set(denialRef, {
                    reason: 'owner_lineage_limit',
                    eventId,
                    eventType,
                ...
                return { moved: false, reason: 'owner_lineage_limit', retryable: false };
            }
            const aggregate = (0, revenuecat_premium_lineage_1.aggregatePremiumLineages)(recipientLineages, recipientProgress, now);
            let legacyPremiumBlock = null;
            for (const donor of donorUsers) {
                const donorProgress = (donor.snap.data()?.progress ?? {});
                legacyPremiumBlock = preferredLegacyPremiumBlock(legacyPremiumBlock, readDonorPremiumBlock(donorProgress, now));
            }
            ...
            for (const donor of donorUsers) {
                if (donor.id === recipientId)
                    continue;
                const donorProgress = (donor.snap.data()?.progress ?? {});
                if (!readDonorPremiumBlock(donorProgress, now))
                    continue;
                tx.update(donor.ref, {
                    progress: {
                        premium_plan: '',
            ...
            return {
                moved: movedPremium,
                ...(movedPremium ? {} : { reason: 'no_active_donor_premium' }),
                recipientId,
                donorIds: donorUsers.map((donor) => donor.id),
            };
        });
