...
export type GuessPhraseCandidate = Readonly<{
  semanticCandidate: TournamentSemanticCandidate;
  grammarProof: StrictGrammarTwinSet;
}>;

export type GuessPhraseCandidateValidation =
  | Readonly<{ ok: true }>
  | Readonly<{
    ok: false;
...
export const TOURNAMENT_GUESS_PROMPT_FAMILIES = Object.freeze([
  'situation', 'intention', 'dialogue',
] as const);
export type TournamentGuessPromptFamily = typeof TOURNAMENT_GUESS_PROMPT_FAMILIES[number];
...
function candidateIdFor(proof: StrictGrammarTwinSet): string {
  const suffix = createHash('sha256')
    ...
    .digest('hex')
    .slice(0, 32);
  return `v11_guess_phrase_${suffix}`;
}

function promptFor(
  proof: StrictGrammarTwinSet,
  translation: string,
): Readonly<{ family: TournamentGuessPromptFamily; text: string }> {
  const rank = createHash('sha256')
    .update(`${proof.provenanceKey}:${proof.ruleId}:${proof.slotIndex}`, 'utf8')
    .digest()[0] % TOURNAMENT_GUESS_PROMPT_FAMILIES.length;
  const family = TOURNAMENT_GUESS_PROMPT_FAMILIES[rank];
  const text = family === 'situation'
    ? `Выберите грамматически правильную фразу для ситуации: «${translation}»`
    : family === 'intention'
      ? `Как грамматически правильно сказать: «${translation}»?`
      : `Выберите реплику без грамматической ошибки: «${translation}»`;
  return Object.freeze({ family, text });
}
...
export function validateEmbeddedGuessPhraseCandidate(
  candidate: TournamentSemanticCandidate,
): GuessPhraseCandidateValidation {
  ...
  return validateGuessPhraseCandidate(
...
export function buildGuessPhraseCandidates(
  day: V11SourceDay,
  phrase: V11SourcePhrase,
): readonly GuessPhraseCandidate[] {
  return Object.freeze(buildStrictGrammarTwinSets(day, phrase)
    .map((proof) => projectGuessPhraseCandidate(day, phrase, proof))
    .filter((candidate): candidate is GuessPhraseCandidate => candidate !== null));
}

export function validateGuessPhraseCandidate(
  day: V11SourceDay,
  phrase: V11SourcePhrase,
  candidate: unknown,
): GuessPhraseCandidateValidation {
  ...
  const envelope = candidate as Partial<GuessPhraseCandidate>;
  ...
  const expected = projectGuessPhraseCandidate(day, phrase, envelope.grammarProof as StrictGrammarTwinSet);
  if (!expected
