...
const writeLog = [];
const FIELD_DELETE = { __op: 'delete' };
let autoId = 0;
let transactionReadHook = null;
let nextTransactionFailure = null;
let transactionInvocation = 0;
let failTransactionAtInvocation = null;
const mockAdminGetUser = jest.fn(async (uid) => ({
    uid,
    disabled: false,
    emailVerified: true,
    providerData: [{ providerId: 'google.com' }],
}));
const mockCreateCustomToken = jest.fn(async (uid, claims) => (`custom-token-${uid}-${String(claims.phrasemanRecoveryEventId ?? '')}`));
const mockAdminGetUserByEmail = jest.fn(async (_email) => ({
...
function deepMerge(target, source) {
    ...
    for (const [key, value] of Object.entries(source)) {
        if (value?.__op === FIELD_DELETE.__op) {
            delete result[key];
            continue;
        }
            ...
            for (let attempt = 0; attempt < 100; attempt += 1) {
                ...
                try {
                    result = await fn({
                        get: async (ref) => {
                            if (writes.length > 0)
                                throw new Error('firestore reads must precede transaction writes');
...
const registeredDocumentCreates = [];
...
jest.mock('firebase-functions/params', () => ({
    defineSecret: (name) => ({ name, value: () => `${name}-test-secret-material-at-least-32-bytes` }),
...
jest.mock('firebase-admin', () => {
    ...
    return {
        firestore,
        auth: jest.fn(() => ({
            getUser: (uid) => mockAdminGetUser(uid),
            getUserByEmail: (email) => mockAdminGetUserByEmail(email),
            createCustomToken: (uid, claims) => mockCreateCustomToken(uid, claims),
        })),
    };
});
...
async function callRequest(data, authUid = 'new-google-uid', session = {}) {
...
async function callCleanRequest(data, authUid = 'new-google-uid', session = {}) {
...
async function callCleanConfirm(data, authUid = 'new-google-uid', session = {}) {
...
async function callCleanWorker(intentId) {
...
async function callIssueHandoff(recoveryEventId, authUid = 'handoff-google-uid', requestId = 'request-1') {
...
async function callCompleteHandoff(recoveryEventId, authUid = 'handoff-google-uid', claimOverrides = {}) {
...
beforeEach(() => {
    ...
    mockAdminGetUser.mockImplementation(async (uid) => ({
        uid,
        disabled: false,
        email: RECOVERY_EMAIL,
        emailVerified: true,
    ...
    mockCreateCustomToken.mockImplementation(async (uid, claims) => (`custom-token-${uid}-${String(claims.phrasemanRecoveryEventId ?? '')}`));
    mockAdminGetUserByEmail.mockImplementation(async (_email) => ({
        uid: 'old-provider-uid',
        disabled: false,
        email: RECOVERY_EMAIL,
        emailVerified: true,
...
describe('authRequestRecoveryCode', () => {
    ...
    it('uses verified Apple token claims for both handlers and ignores data.provider', async () => {
        ...
        const requestResult = await callRequest({ stableId: STABLE, provider: 'google' }, 'replacement-apple-uid', appleSession);
...
describe('authConfirmRecoveryCode', () => {
    ...
    it.each(REJECTED_PROVIDER_SESSIONS)('rejects a %s token-claim session before consuming a valid code', async (_label, targetProvider, session) => {
        seedLinkedUser(targetProvider);
        await callRequest({ stableId: STABLE }, 'replacement-uid', validSessionFor(targetProvider));
    ...
    it.each([
        ...
        seedLinkedUser(targetProvider);
        await callRequest({ stableId: STABLE }, 'replacement-provider-uid', { signInProvider: requestProvider });
        const code = emailedCode();
    ...
    it('rejects a different requester before hash attempts and without writes', async () => {
        ...
        const codeBefore = cloneDoc(docs.get(`auth_recovery_codes/${STABLE}`) ?? {});
        writeLog.length = 0;
        await expect(callConfirm({ stableId: STABLE, code: wrongCode() }, 'different-requester-uid')).rejects.toMatchObject({
            code: 'permission-denied',
    ...
    it('same-target baseline succeeds without creating a second forward owner', async () => {
        ...
        const result = await callConfirm({ stableId: STABLE, code: emailedCode() }, 'same-google-uid');
        expect(result).toEqual({
            ok: true,
            stableId: STABLE,
            recoveryEventId: 'auto-1',
    ...
    it('logs only an allowlisted event when the post-recovery alert fails', async () => {
        ...
        await expect(callConfirm({ stableId: STABLE, code: emailedCode() }, 'new-google-uid')).resolves.toMatchObject({ ok: true, stableId: STABLE });
    ...
    it('перепривязка вытесняет свежий анонимный stable_id нового uid (фиксируется в событии)', async () => {
        ...
        expect(collectionDocs('users').filter(({ data }) => (data.firebaseAuthUid === 'new-google-uid'))).toHaveLength(1);
    });
...
describe('recovery custom-token handoff', () => {
    it('registers issue and complete with App Check and no unrelated secrets', () => {
        ...
        const registrations = registeredCallables.filter(({ handler }) => (handler === authIssueRecoveryHandoffToken || handler === authCompleteRecoveryHandoff));
    ...
    it.each([undefined, '', 'x'.repeat(161), 'bad/request'])('rejects missing or malformed handoff requestId %p before token mint', async (requestId) => {
        const recovery = await seedRecoveryEventForHandoff();
        const { authIssueRecoveryHandoffToken } = require('./auth_recovery');
    ...
    it.each([
        ['wrong event owner', async (eventId) => callIssueHandoff(eventId, 'other-google-uid'), 'recovery_handoff_identity_mismatch'],
        ['deletion marker', async (eventId) => {
                docs.set('account_deletion_auth_markers/handoff-google-uid', { at: NOW });
                return callIssueHandoff(eventId);
            }, 'account_delete_pending'],
        ['disabled Admin Auth user', async (eventId) => {
    ...
    it('honors the 30-second lease and five-attempt/three-success caps', async () => {
        ...
        docs.set(eventPath, { ...base, handoffLeaseId: 'busy', handoffLeaseUntil: NOW + 30000 });
    ...
    it.each(['mint', 'finalize'])('preserves a prior issued token after a second %s failure so it can still ack', async (phase) => {
        const recovery = await seedRecoveryEventForHandoff();
        const eventId = String(recovery.recoveryEventId);
    ...
    it('allows only one concurrent mint lease', async () => {
        ...
        const eventId = String(recovery.recoveryEventId);
        let releaseMint;
        mockCreateCustomToken.mockImplementationOnce(() => new Promise((resolve) => {
...
async function settleCleanInstall(promise) {
...
function cleanRecoveryStoreSnapshot() {
...
async function deliverCleanChallenge(response) {
    await callCleanWorker(String(response.challengeId));
}
function cleanPublicShape(value) {
    const data = value;
    return {
        ok: data.ok,
...
describe('clean-install recovery', () => {
    ...
    it('returns the same non-enumerating response shape for eligible, missing, mismatch, and deleted targets', async () => {
        ...
        expect(new Set(responses.map((item) => String(item.challengeId))).size).toBe(responses.length);
    ...
    it('request creates an intent without sending; worker rechecks Admin Auth and sends only to authoritative email', async () => {
        ...
        const expectedEmailHmac = recovery.hmacCleanRecoveryValue(currentSecret, 'v1', 'email', recovery.normalizeCleanRecoveryEmail(RECOVERY_EMAIL));
        const expectedCodeHmac = recovery.hmacCleanRecoveryValue(currentSecret, 'v1', 'code', `${challengeId}|${String(delivered?.generation)}|${code}`);
        ...
        expect(String(response.challengeId)).not.toContain('stable');
    });
    it('uses versioned domain-separated HMACs and keeps previous-key overlap', () => {
        ...
        const previousChallenge = {
            ...
            keyVersion: 'v0', codeHmac: recovery.hmacCleanRecoveryValue('previous-secret', 'v0', 'code', 'challenge_previous_key_123456|generation-1|123456'),
        };
    ...
    it('confirms a delivered challenge during the previous-key overlap and rejects a retired version', async () => {
        ...
        docs.set(challengePath, {
            ...challenge,
            keyVersion: 'v0',
            emailHmac: recovery.hmacCleanRecoveryValue('AUTH_RECOVERY_CHALLENGE_HMAC_KEY_PREVIOUS-test-secret-material-at-least-32-bytes', 'v0', 'email', recovery.normalizeCleanRecoveryEmail(RECOVERY_EMAIL)),
            codeHmac: recovery.hmacCleanRecoveryValue('AUTH_RECOVERY_CHALLENGE_HMAC_KEY_PREVIOUS-test-secret-material-at-least-32-bytes', 'v0', 'code', `${challengeId}|${String(challenge.generation)}|${code}`),
        });
        ...
        const retiredId = String(retired.challengeId);
    ...
    it.each([
        ...
        mockAdminGetUser.mockImplementation(async (uid) => Object.assign({
        ...
        const { sendTransactionalEmail } = require('./admin_email');
        expect(sendTransactionalEmail).not.toHaveBeenCalled();
        expect(docs.get(`auth_recovery_challenges/${String(response.challengeId)}`))
    ...
    it.each([
        ...
        const session = validSessionFor(provider);
        const requested = await settleCleanInstall(callCleanRequest({ email: RECOVERY_EMAIL, clientRequestId: `realistic-${provider}` }, 'new-google-uid', session));
        await deliverCleanChallenge(requested);
        await expect(settleCleanInstall(callCleanConfirm({
    ...
    it('reclaims an expired delivery lease and ignores the stale worker terminal result', async () => {
        ...
        const { sendTransactionalEmail } = require('./admin_email');
        let finishStale;
        const staleSend = new Promise((resolve) => { finishStale = resolve; });
        sendTransactionalEmail
    ...
    it('does not send decoy mail and keeps clientRequestId idempotent', async () => {
        ...
        const challengeId = String(first.challengeId);
        expectExactKeys(docs.get(`auth_recovery_challenges/${challengeId}`), CLEAN_RECOVERY_CHALLENGE_REQUEST_KEYS);
        expectExactKeys(docs.get(`auth_recovery_delivery_intents/${challengeId}`), CLEAN_RECOVERY_INTENT_REQUEST_KEYS);
    ...
    it('normalizes email consistently for fan-in/fan-out buckets and does not increment on replay', async () => {
        ...
        const unicodeVariant = recovery.cleanRecoveryRateDocIds('uid-a', 'ＲＥＣＯＶＥＲＹ．ＯＷＮＥＲ＠ＥＸＡＭＰＬＥ．ＩＮＶＡＬＩＤ', secret, 'v1');
    ...
    it.each(['requester', 'email', 'pair'])('enforces the %s transactional rate bucket', async (bucket) => {
        seedLinkedUser('google');
        const recovery = require('./auth_recovery');
        const ids = recovery.cleanRecoveryRateDocIds('new-google-uid', RECOVERY_EMAIL, 'AUTH_RECOVERY_CHALLENGE_HMAC_KEY_CURRENT-test-secret-material-at-least-32-bytes', 'v1');
    ...
    it('isolates request idempotency by requester and renews an expired request', async () => {
        ...
        const second = await settleCleanInstall(callCleanRequest({ email: RECOVERY_EMAIL, clientRequestId: 'shared-request-id' }, 'different-google-uid'));
        expect(second.challengeId).not.toBe(first.challengeId);
        jest.setSystemTime(NOW + 11 * 60 * 1000);
        const renewed = await cleanRequest('shared-request-id');
        expect(renewed.challengeId).not.toBe(first.challengeId);
    });
    it('reports only the remaining TTL on a request-idempotency replay', async () => {
        ...
        expect(replay.expiresInSec).toBe(10);
    });
    it('scopes confirm idempotency by challenge and binds the submitted proof', async () => {
        ...
        await expect(settleCleanInstall(callCleanConfirm({
            challengeId: first.challengeId, code: firstCode, clientRequestId: 'shared-confirm-id',
        }))).resolves.toMatchObject({ ok: true });
        const eventsBefore = collectionDocs('auth_recovery_events').length;
        await expect(settleCleanInstall(callCleanConfirm({
            challengeId: second.challengeId, code: secondCode, clientRequestId: 'shared-confirm-id',
        ...
        await expect(settleCleanInstall(callCleanConfirm({
            challengeId: first.challengeId, code: wrongCode(), clientRequestId: 'shared-confirm-id',
    ...
    it('revalidates provider binding before returning a successful confirm replay', async () => {
        ...
        await expect(settleCleanInstall(callCleanConfirm(data, 'new-google-uid', validSessionFor('apple')))).rejects.toMatchObject({ code: 'permission-denied', message: 'clean_recovery_unavailable' });
    });
    ...
    it.each(['resend_transport_failed', 'resend_http_retryable'])('retries %s after the lease with the identical provider idempotency key', async (error) => {
        seedLinkedUser('google');
        ...
        await deliverCleanChallenge(response);
        expect(sendTransactionalEmail.mock.calls[1][0].idempotencyKey).toBe(firstKey);
        expect(docs.get(`auth_recovery_delivery_intents/${String(response.challengeId)}`))
            .toMatchObject({ status: 'sent' });
    });
    it('binds confirm to AppCheck, requester UID, and provider without target-specific errors', async () => {
        ...
        await deliverCleanChallenge(requested);
        const data = { challengeId: requested.challengeId, code: emailedCode(), clientRequestId: 'clean-confirm-1' };
    ...
    it.each([
        ...
        await deliverCleanChallenge(requested);
        const code = emailedCode();
        mockAdminGetUser.mockImplementationOnce(async (uid) => Object.assign({
            uid,
            email: RECOVERY_EMAIL,
            disabled: false,
            emailVerified: true,
    ...
    it('collapses wrong, unknown, expired, and locked proofs to generic public classes', async () => {
        ...
        jest.setSystemTime(NOW + 11 * 60 * 1000);
        await expect(settleCleanInstall(callCleanConfirm({
            challengeId: second.challengeId, code: emailedCode(), clientRequestId: 'clean-confirm-expired',
        }))).rejects.toMatchObject({ code: 'permission-denied', message: 'clean_recovery_code_invalid' });
    });
    it('revalidates deletion fences before any relink mutation', async () => {
        ...
        docs.set(`account_deletion_auth_markers/new-google-uid`, { at: NOW });
        await expect(settleCleanInstall(callCleanConfirm({
            challengeId: requested.challengeId, code: emailedCode(), clientRequestId: 'clean-confirm-fenced',
        }))).rejects.toMatchObject({ code: 'permission-denied', message: 'clean_recovery_unavailable' });
    ...
    it('atomically relinks once and replays the same event for concurrent correct confirms', async () => {
        ...
        const requested = await cleanRequest();
        await deliverCleanChallenge(requested);
        const data = { challengeId: requested.challengeId, code: emailedCode(), clientRequestId: 'clean-confirm-success' };
        const challengePath = `auth_recovery_challenges/${String(requested.challengeId)}`;
        let releaseFirst;
        let markEntered;
        const entered = new Promise((resolve) => { markEntered = resolve; });
        const release = new Promise((resolve) => { releaseFirst = resolve; });
        ...
        expect(Number(first.handoffEligibleUntil)).toBeGreaterThanOrEqual(NOW + 10 * 60 * 1000);
        ...
        expect(collectionDocs('auth_recovery_events')).toHaveLength(1);
        const consumeWrites = writeLog.filter(({ path, data: writeData }) => (path === challengePath && writeData.status === 'consumed'));
        expect(consumeWrites).toHaveLength(1);
        expect(consumeWrites[0]?.data.recoveryEventId).toBe(first.recoveryEventId);
    });
    it('enforces a minimum equalized response envelope through injected clock and sleep', async () => {
        ...
        await recovery.waitForCleanRecoveryEnvelope(1000, {
    ...
    it('applies the response envelope in the actual callable finally path for eligible, decoy, auth, and db outcomes', async () => {
        ...
        docs.clear();
        jest.setSystemTime(NOW);
        await observe(callCleanRequest({ email: RECOVERY_EMAIL, clientRequestId: 'timing-auth' }, 'new-google-uid', { signInProvider: 'google.com', emailVerified: false }));
        docs.clear();
        jest.setSystemTime(NOW);
        seedLinkedUser('google');
