...
exports.NEW_TOURNAMENT_POOL_MODES = exports.NEW_TOURNAMENT_POOL_EXPOSURE_BUCKET_SIZE = exports.NEW_TOURNAMENT_POOL_TASK_COUNT = exports.NEW_TOURNAMENT_POOL_CELL_QUOTAS = exports.NEW_TOURNAMENT_POOL_VERSION = void 0;
exports.buildUnambiguousFillGapTask = buildUnambiguousFillGapTask;
exports.buildNewTournamentPool = buildNewTournamentPool;
...
exports.NEW_TOURNAMENT_POOL_VERSION = 'tpool_20260801_v8';
exports.NEW_TOURNAMENT_POOL_CELL_QUOTAS = Object.freeze({
    'guess_phrase:1': 400,
    'guess_phrase:2': 400,
    'guess_phrase:3': 400,
    'fill_gap:1': 160,
...
exports.NEW_TOURNAMENT_POOL_TASK_COUNT = 4000;
exports.NEW_TOURNAMENT_POOL_EXPOSURE_BUCKET_SIZE = 40;
exports.NEW_TOURNAMENT_POOL_MODES = [
    'guess_phrase',
    'fill_gap',
    'find_oddity',
    'translate_build',
...
function within(value, maxBytes) {
...
// distractor set.
const AUDITED_CONTENT_REJECTIONS = [
...
const AUDITED_CONTENT_REJECTION = new RegExp(AUDITED_CONTENT_REJECTIONS.map((pattern) => `(?:${pattern.source})`).join('|'), 'iu');
function passesAuditedContentGate(english, russian) {
...
function usablePairs(day) {
    return (day.phrases ?? [])
        .map(authoredPair)
        .filter((pair) => !!pair);
}
function authoredSpeedPairs(day) {
    const pairs = [];
    const seenEnglish = new Set();
    const seenRussian = new Set();
    for (const pair of usablePairs(day)) {
        if ((0, tournament_task_factory_1.phraseTokens)(pair.en).length < 2 || (0, tournament_task_factory_1.phraseTokens)(pair.ru).length < 2
            || !within(pair.en, tournament_core_1.TOURNAMENT_TASK_LIMITS.promptBytes)
            || !within(pair.ru, tournament_core_1.TOURNAMENT_TASK_LIMITS.optionBytes))
            continue;
        const englishKey = normalize(pair.en);
        const russianKey = normalize(pair.ru);
        if (seenEnglish.has(englishKey) || seenRussian.has(russianKey))
            continue;
...
function taskId(mode, difficulty, identity) {
    const shortMode = {
        guess_phrase: 'guess',
        fill_gap: 'gap',
        find_oddity: 'odd',
    ...
    return `tp2_20260801_v8_${shortMode[mode]}_d${difficulty}_${sha256(`${exports.NEW_TOURNAMENT_POOL_VERSION}:${identity}`).slice(0, 20)}`;
}
function tags(day) {
    const result = [
        'source:ai',
        'generator:author-content-v1',
        `pool:${exports.NEW_TOURNAMENT_POOL_VERSION}`,
    ...
    if (topic) {
        const topicBytes = tournament_core_1.TOURNAMENT_TASK_LIMITS.tagBytes - byteLength('topic:');
...
function baseTask(day, mode, identity, phraseIds, payload, explanation) {
    ...
    return {
        ...
        source: 'ai',
        lifecycle: 'published',
        poolVersion: exports.NEW_TOURNAMENT_POOL_VERSION,
...
function replaceAuthoredWord(phrase, word, replacement) {
    const pattern = new RegExp(`(^|[^\\p{L}\\p{N}'’\\-])(${escapeRegExp(word)})(?=$|[^\\p{L}\\p{N}'’\\-])`, 'iu');
    if (!pattern.test(phrase))
        return null;
    return phrase.replace(pattern, (_match, prefix) => `${prefix}${replacement}`);
}
function completeGap(gapPhrase, option) {
    return gapPhrase.replace('___', option);
}
function buildGuessTask(day, phrase) {
    const pair = authoredPair(phrase);
    if (!pair || !within(pair.en, tournament_core_1.TOURNAMENT_TASK_LIMITS.optionBytes))
        return null;
    const distractors = stableShuffle(usablePairs(day), `guess:${pair.id}`)
        ...
        && within(candidate.en, tournament_core_1.TOURNAMENT_TASK_LIMITS.optionBytes))
    ...
    const prompt = truncateToBytes(`Вы хотите сказать: «${withoutTerminalPunctuation(pair.ru)}». Какую английскую реплику выберете?`, tournament_core_1.TOURNAMENT_TASK_LIMITS.phraseBytes);
    const explanation = {
        ruleNote: truncateToBytes(`«${withoutTerminalPunctuation(pair.en)}» точно передаёт мысль «${withoutTerminalPunctuation(pair.ru)}». Здесь решает смысл, а не удача на четырёх кнопках.`, tournament_core_1.TOURNAMENT_TASK_LIMITS.explanationBytes),
        example: truncateToBytes(`${pair.en} — ${pair.ru}`, tournament_core_1.TOURNAMENT_TASK_LIMITS.explanationBytes),
        wrongOptionReasons: optionPairs.map((option) => option.id === pair.id
            ? ''
            : truncateToBytes(`«${option.en}» означает «${withoutTerminalPunctuation(option.ru)}», поэтому мысль «${withoutTerminalPunctuation(pair.ru)}» не передаёт.`, tournament_core_1.TOURNAMENT_TASK_LIMITS.explanationBytes)),
    };
    return baseTask(day, 'guess_phrase', `guess:${day.planId}:${day.dayIndex}:${pair.id}`, [pair.id, ...optionPairs.filter((option) => option.id !== pair.id).map((option) => option.id)], {
        phrase: prompt,
        options,
        correctIndex,
...
const OBJECT_PRONOUN_RU_MARKERS = Object.freeze({
...
const PRESENT_BE_FOR_SUBJECT = Object.freeze({
    i: 'am',
    you: 'are',
    he: 'is',
    she: 'is',
...
function agreesWithPresentBe(subject, beForm) {
    return PRESENT_BE_FOR_SUBJECT[normalize(subject)] === normalize(beForm);
}
function adjacentPresentBe(tokens, tokenIndex) {
...
function adjacentPersonalSubject(tokens, tokenIndex) {
    const adjacent = [tokens[tokenIndex - 1], tokens[tokenIndex + 1]]
        .find((token) => token && SUBJECT_PRONOUNS.has(normalize(token)));
    return adjacent ?? null;
}
function russianMeaningNamesObjectPronoun(russianMeaning, pronoun) {
    const markers = OBJECT_PRONOUN_RU_MARKERS[normalize(pronoun)] ?? [];
    const russianTokens = new Set((0, tournament_task_factory_1.phraseTokens)(russianMeaning).map(normalize));
    return markers.some((marker) => russianTokens.has(marker));
}
function safeFillGapSlot(partOfSpeech, original, distractors, tokens, tokenIndex, russianMeaning) {
    ...
    if (partOfSpeech === 'pronoun' && SUBJECT_PRONOUNS.has(normalizedOriginal)) {
        ...
        if (!beForm || !agreesWithPresentBe(original, beForm))
            return null;
        const agreementTraps = distractors.filter((value) => (SUBJECT_PRONOUNS.has(normalize(value)) && !agreesWithPresentBe(value, beForm)));
    ...
    if (partOfSpeech === 'to-be' && PRESENT_BE_FORMS.has(normalizedOriginal)) {
        ...
        if (!subject || !agreesWithPresentBe(subject, original))
            return null;
...
function fillGapMutations(phrase) {
    const english = String(phrase.english ?? '').trim();
    const tokens = (0, tournament_task_factory_1.phraseTokens)(english);
    ...
    for (const [sourceIndex, word] of (phrase.words ?? []).entries()) {
        ...
        if (!isSingleLexicalWord(original)
            || !within(original, tournament_core_1.TOURNAMENT_TASK_LIMITS.optionBytes))
            continue;
        ...
        if (matchingTokenIndexes.length !== 1)
            continue;
        const authoredDistractors = Array.from(new Map((word.distractors ?? [])
            .map((value) => String(value ?? '').trim())
            .filter((value) => isSingleLexicalWord(value)
            && within(value, tournament_core_1.TOURNAMENT_TASK_LIMITS.optionBytes)
            && normalize(value) !== normalize(original))
            .map((value) => [normalize(value), value])).values());
        const safeSlot = safeFillGapSlot(partOfSpeech, original, authoredDistractors, tokens, matchingTokenIndexes[0], String(phrase.meaning?.ru ?? ''));
        if (!safeSlot)
            continue;
        const selected = stableShuffle(safeSlot.distractors, `fill-gap-traps:${phrase.id}:${sourceIndex}:${original}`).slice(0, 3);
        ...
        if (!gapPhrase || !mutatedPhrase
            || !within(gapPhrase, tournament_core_1.TOURNAMENT_TASK_LIMITS.phraseBytes)
            || !within(mutatedPhrase, tournament_core_1.TOURNAMENT_TASK_LIMITS.optionBytes)
            || normalize(completeGap(gapPhrase, original)) !== normalize(english))
            continue;
    ...
    return mutations.sort((left, right) => sha256(`fill-gap-slot:${phrase.id}:${left.tokenIndex}:${left.original}`).localeCompare(sha256(`fill-gap-slot:${phrase.id}:${right.tokenIndex}:${right.original}`)));
}
function strictGrammarMutations(phrase) {
    const english = String(phrase.english ?? '').trim();
    const tokens = (0, tournament_task_factory_1.phraseTokens)(english);
    const additional = [];
    ...
    for (const word of phrase.words ?? []) {
        ...
        if (partOfSpeech === 'article' && ['a', 'an'].includes(normalizedOriginal)) {
            ...
            if (singularSubjects.has(subject) && /s$/u.test(normalizedOriginal)) {
                ...
                replacement = distractors.find((value) => (normalize(value) === `${normalizedOriginal}s` || normalize(value) === `${normalizedOriginal}es`));
            }
            if (replacement)
                grammarRole = 'verb_agreement';
        }
        else if (partOfSpeech === 'noun' && tokenIndex > 0
            && ['a', 'an', 'one'].includes(normalize(tokens[tokenIndex - 1]))) {
            replacement = distractors.find((value) => (normalize(value) === `${normalizedOriginal}s` || normalize(value) === `${normalizedOriginal}es`));
            if (replacement)
                grammarRole = 'noun_number';
        }
        if (!replacement || !grammarRole)
            continue;
        ...
        const fillGapDistractors = grammarRole === 'article_form'
            ...
            : stableShuffle([replacement, ...distractors.filter((value) => normalize(value) !== normalize(replacement))], `strict-gap-traps:${phrase.id}:${tokenIndex}:${original}`).slice(0, 3);
        additional.push({
            original,
            replacement,
            partOfSpeech,
...
function fillGapRoleNote(mutation) {
...
function strictGrammarRoleNote(grammarRole) {
    if (grammarRole === 'article_form')
        return 'a и an выбираются по следующему звуку';
    if (grammarRole === 'verb_agreement')
        return 'форма смыслового глагола согласуется с подлежащим';
    if (grammarRole === 'noun_number')
        return 'после a, an или one нужна форма единственного числа';
    if (grammarRole === 'lexical_meaning')
        return 'русский перевод фиксирует нужное значение слова в этой фразе';
    return fillGapRoleNote({ grammarRole });
}
function fillGapTrapViolation(grammarRole) {
...
function buildFillGapTask(day, phrase, mutation) {
    const pair = authoredPair(phrase);
    if (!pair)
        return null;
    ...
    if (!within(prompt, tournament_core_1.TOURNAMENT_TASK_LIMITS.phraseBytes))
        return null;
    const roleNote = strictGrammarRoleNote(mutation.grammarRole);
    const trapViolation = fillGapTrapViolation(mutation.grammarRole);
    const explanation = {
        ruleNote: truncateToBytes(`В пропуске нужно «${mutation.original}»: ${roleNote}, поэтому среди четырёх вариантов подходит только эта форма.`, tournament_core_1.TOURNAMENT_TASK_LIMITS.explanationBytes),
        example: truncateToBytes(`${pair.en} — ${pair.ru}`, tournament_core_1.TOURNAMENT_TASK_LIMITS.explanationBytes),
        wrongOptionReasons: options.map((option) => option === mutation.original
            ? ''
            : truncateToBytes(`«${option}» даёт «${withoutTerminalPunctuation(completeGap(mutation.gapPhrase, option))}» — это ловушка: ${trapViolation}. Правильно «${mutation.original}».`, tournament_core_1.TOURNAMENT_TASK_LIMITS.explanationBytes)),
    };
    return baseTask(day, 'fill_gap', `gap:${day.planId}:${day.dayIndex}:${pair.id}:${mutation.tokenIndex}:${mutation.original}`, [pair.id], {
        phrase: prompt,
        options,
        correctIndex,
...
function buildUnambiguousFillGapTask(day, phrase) {
    const mutation = fillGapMutations(phrase)[0];
    return mutation ? buildFillGapTask(day, phrase, mutation) : null;
}
function buildUnambiguousFillGapCandidates(day, phrase) {
    return strictGrammarMutations(phrase).filter((mutation) => mutation.options.length === 4)
        .flatMap((mutation) => {
        const pair = authoredPair(phrase);
        if (!pair || mutation.options.some((option) => (!passesAuditedContentGate(completeGap(mutation.gapPhrase, option), pair.ru))))
            return [];
...
function buildOddityTask(day, phrase, mutation) {
    const pair = authoredPair(phrase);
    if (!pair || !passesAuditedContentGate(mutation.mutatedPhrase, pair.ru))
        return null;
    ...
    const explanation = {
        ruleNote: truncateToBytes(`«${withoutTerminalPunctuation(mutation.mutatedPhrase)}» — ловушка: ${strictGrammarRoleNote(mutation.grammarRole)}. Если сохраняем смысл авторской фразы, исправляем её так: «${withoutTerminalPunctuation(pair.en)}».`, tournament_core_1.TOURNAMENT_TASK_LIMITS.explanationBytes),
        example: truncateToBytes(`${pair.en} — ${pair.ru}`, tournament_core_1.TOURNAMENT_TASK_LIMITS.explanationBytes),
        wrongOptionReasons: optionRecords.map((option) => option.odd
            ? ''
            : truncateToBytes(`«${option.text}» — нормальная фраза со смыслом «${withoutTerminalPunctuation(option.ru)}»; исправлять нужно «${withoutTerminalPunctuation(mutation.mutatedPhrase)}».`, tournament_core_1.TOURNAMENT_TASK_LIMITS.explanationBytes)),
    };
    return baseTask(day, 'find_oddity', `odd:${day.planId}:${day.dayIndex}:${pair.id}:${mutation.replacement}`, [pair.id, ...optionRecords.filter((option) => option.id !== pair.id).map((option) => option.id)], {
...
function authoredLexicalTraps(phrase) {
    const correctTokens = new Set((0, tournament_task_factory_1.phraseTokens)(String(phrase.english ?? '')).map(normalize));
    const traps = [];
    for (const word of phrase.words ?? []) {
        const original = String(word.text ?? '').trim();
        const partOfSpeech = String(word.partOfSpeech ?? '').trim();
        if (!original || !partOfSpeech)
            continue;
        const values = Array.from(new Map((word.distractors ?? [])
            .map((value) => String(value ?? '').trim())
            .filter((value) => isSingleLexicalWord(value)
            && within(value, tournament_core_1.TOURNAMENT_TASK_LIMITS.tokenBytes)
            && normalize(value) !== normalize(original)
            && !correctTokens.has(normalize(value)))
            .map((value) => [normalize(value), value])).values());
        for (const value of stableShuffle(values, `translate-lexical:${phrase.id}:${original}`).slice(0, 2)) {
            traps.push({ value, partOfSpeech, original, grammarRole: 'lexical_meaning' });
        }
    }
    return traps;
}
function buildTranslateTask(day, phrase, trap) {
    const pair = authoredPair(phrase);
    if (!pair || !within(pair.ru, tournament_core_1.TOURNAMENT_TASK_LIMITS.phraseBytes)
        || !within(pair.en, tournament_core_1.TOURNAMENT_TASK_LIMITS.answerBytes))
        return null;
    const correctTokens = (0, tournament_task_factory_1.phraseTokens)(pair.en);
    if (correctTokens.length < 2 || correctTokens.length > 12
        || correctTokens.some((token) => !within(token, tournament_core_1.TOURNAMENT_TASK_LIMITS.tokenBytes)))
        return null;
    ...
    const maxTraps = Math.min(1, tournament_core_1.TOURNAMENT_TASK_LIMITS.maxWordBankItems - correctTokens.length);
    if (maxTraps < 1 || correctTokens.some((token) => normalize(token) === normalize(trap.value)))
        return null;
    const wordBank = stableShuffle([...correctTokens, trap.value], `build-bank:${pair.id}:${trap.value}`);
    const explanation = {
        ruleNote: truncateToBytes(`Правильный порядок — «${withoutTerminalPunctuation(pair.en)}». Ловушка «${trap.value}» конкурирует с конкретной формой «${trap.original}»: ${strictGrammarRoleNote(trap.grammarRole)}.`, tournament_core_1.TOURNAMENT_TASK_LIMITS.explanationBytes),
        example: truncateToBytes(`${pair.ru} — ${pair.en}`, tournament_core_1.TOURNAMENT_TASK_LIMITS.explanationBytes),
        wrongOptionReasons: [],
    };
    return baseTask(day, 'translate_build', `build:${day.planId}:${day.dayIndex}:${pair.id}:${trap.value}`, [pair.id], {
        phrase: pair.ru,
        wordBank,
...
function buildSpeedMatchTask(day, pairs) {
    if (pairs.length !== 6
        || pairs.some((pair) => !within(pair.en, tournament_core_1.TOURNAMENT_TASK_LIMITS.promptBytes)
            || !within(pair.ru, tournament_core_1.TOURNAMENT_TASK_LIMITS.optionBytes)
            || (0, tournament_task_factory_1.phraseTokens)(pair.en).length < 2
            || (0, tournament_task_factory_1.phraseTokens)(pair.ru).length < 2))
        return null;
    if (new Set(pairs.map((pair) => normalize(pair.ru))).size !== 6
        || new Set(pairs.map((pair) => normalize(pair.en))).size !== 6)
        return null;
    ...
    const items = itemPairs.map((pair) => {
        ...
        return {
            ...
            correctIndex,
            explanation: {
                ruleNote: truncateToBytes(`«${withoutTerminalPunctuation(pair.en)}» означает «${withoutTerminalPunctuation(pair.ru)}» в контексте авторского примера.`, tournament_core_1.TOURNAMENT_TASK_LIMITS.explanationBytes),
                example: truncateToBytes(`${pair.en} — ${pair.ru}`, tournament_core_1.TOURNAMENT_TASK_LIMITS.explanationBytes),
                wrongOptionReasons: rightPairs.map((right) => right.id === pair.id
                    ? ''
                    : truncateToBytes(`«${right.ru}» — перевод фразы «${withoutTerminalPunctuation(right.en)}», а не «${withoutTerminalPunctuation(pair.en)}».`, tournament_core_1.TOURNAMENT_TASK_LIMITS.explanationBytes)),
            },
        };
    });
    const first = itemPairs[0];
    const identity = pairs.map((pair) => `${normalize(pair.en)}=${normalize(pair.ru)}`).sort().join('|');
    const task = baseTask(day, 'speed_match', `pairs:${day.planId}:${day.dayIndex}:${identity}`, pairs.map((pair) => pair.phraseId), {
...
function buildSpeedMatchCandidates(day) {
...
function buildGlobalDisjointSpeedCandidates(days) {
    const dayByKey = new Map(days.map((day) => [`${day.planId}:${day.dayIndex}`, day]));
    const reservedPairs = new Set();
    const reservedEnglish = new Set();
    const candidates = [];
    for (const difficulty of [1, 3, 2]) {
        const target = exports.NEW_TOURNAMENT_POOL_CELL_QUOTAS[`speed_match:${difficulty}`] ?? 0;
        ...
        for (let groupIndex = 0; groupIndex < target; groupIndex += 1) {
            ...
            if (!anchor)
                throw new Error(`new_tournament_pool_speed_anchor_missing:d${difficulty}:${groupIndex}`);
            const task = buildSpeedMatchTask(anchor, group);
            if (!task)
                throw new Error(`new_tournament_pool_global_speed_invalid:d${difficulty}:${groupIndex}`);
            candidates.push({
                task,
...
function taskSemanticSignature(task) {
    const normalized = (value) => semanticNormalize(String(value ?? ''));
...
function selectDiverseCandidates(cell, count, state) {
    const remaining = [...cell];
    const selected = [];
    const countOf = (counts, key) => counts.get(key) ?? 0;
    const speedPairFrequency = new Map();
    const speedEnglishFrequency = new Map();
    ...
    const recordCandidate = (winner) => {
        ...
        winner.diversityAxes?.forEach((axis) => (state.axisCounts.set(axis, countOf(state.axisCounts, axis) + 1)));
        if (winner.fillGap) {
            const axes = [
    ...
    if (cell.length > 0 && cell.every((candidate) => candidate.speedPairKeys)) {
        const available = cell.filter((candidate) => (!candidate.speedPairKeys?.some((key) => state.usedSpeedPairs.has(key))
        ...
        for (let attempt = 0; attempt < attemptLimit && best.length < count; attempt += 1) {
            const ordered = [...available].sort((left, right) => {
                if (attempt % 2 === 0) {
                    const conflictScore = (candidate) => ((candidate.speedPairKeys ?? []).reduce((sum, key) => sum + countOf(speedPairFrequency, key), 0)
            ...
            for (const candidate of ordered) {
                if (candidate.speedPairKeys?.some((key) => usedPairs.has(key))
                    || candidate.speedEnglishKeys?.some((key) => usedEnglish.has(key)))
                    continue;
    ...
    while (selected.length < count && remaining.length > 0) {
        const isEligible = (candidate) => {
            if (candidate.fillGap
                && countOf(state.axisCounts, `fill-token:${candidate.fillGap.correctToken}`) >= 95)
                return false;
            if (candidate.primaryPhraseKey
                && candidate.task.mode === 'translate_build'
                && state.usedPrimaryModePhrases.has(`${candidate.task.mode}:${candidate.primaryPhraseKey}`))
                return false;
            if (state.usedSemanticSignatures.has(taskSemanticSignature(candidate.task)))
                return false;
            if (candidate.speedPairKeys?.some((key) => state.usedSpeedPairs.has(key)))
                return false;
            if (candidate.speedEnglishKeys?.some((key) => state.usedSpeedEnglish.has(key)))
                return false;
            return true;
        };
        const compareCandidates = (left, right) => {
            ...
            const axisLoad = (candidate) => (candidate.diversityAxes ?? [])
...
function allCandidates(days) {
    const candidates = [];
    const sortedDays = [...days].sort((left, right) => (String(left.planId).localeCompare(String(right.planId))
    ...
    for (const day of sortedDays) {
        ...
        for (const phrase of phrases) {
            ...
            if (guess)
                candidates.push({ task: guess, ...common });
            candidates.push(...gaps);
            const strictGrammarCandidates = strictGrammarMutations(phrase);
            for (const mutation of strictGrammarCandidates.filter((candidate) => (candidate.grammarRole !== 'object_pronoun_reference'))) {
            ...
            const translationTraps = [...safeTraps, ...authoredLexicalTraps(phrase)]
                .filter((trap, index, all) => all.findIndex((candidate) => (normalize(candidate.value) === normalize(trap.value)
                && normalize(candidate.original) === normalize(trap.original))) === index);
...
function assignExposureBuckets(tasks) {
    const assigned = [];
    const modeBucketCounts = {};
    const bucketSizes = {};
    for (const mode of exports.NEW_TOURNAMENT_POOL_MODES) {
        const modeTasks = tasks.filter((task) => task.mode === mode);
        const bucketCount = Math.ceil(modeTasks.length / exports.NEW_TOURNAMENT_POOL_EXPOSURE_BUCKET_SIZE);
        if (bucketCount <= 0)
            throw new Error(`new_tournament_pool_bucket_count_invalid:${mode}`);
        ...
        buckets.forEach((bucket, bucketIndex) => {
            if (bucket.length === 0 || bucket.length > exports.NEW_TOURNAMENT_POOL_EXPOSURE_BUCKET_SIZE) {
                throw new Error(`new_tournament_pool_bucket_size_invalid:${mode}:${bucketIndex}:${bucket.length}`);
            }
            const exposureBucket = `${exports.NEW_TOURNAMENT_POOL_VERSION}:${mode}:${String(bucketIndex).padStart(3, '0')}`;
...
function buildNewTournamentPool(days) {
    ...
    for (const mode of exports.NEW_TOURNAMENT_POOL_MODES) {
        const modeCells = (0, tournament_pool_plan_1.requiredCells)([mode]).sort((left, right) => {
            if (mode === 'fill_gap')
                return right.difficulty - left.difficulty;
            if (mode !== 'speed_match')
                return left.difficulty - right.difficulty;
            const uniquePairs = (difficulty) => new Set(candidates
        ...
        for (const { difficulty } of modeCells) {
            ...
            const target = exports.NEW_TOURNAMENT_POOL_CELL_QUOTAS[key];
            if (!Number.isSafeInteger(target) || target <= 0) {
                throw new Error(`new_tournament_pool_cell_quota_missing:${key}`);
            }
            const cell = candidates.filter((candidate) => (candidate.task.mode === mode && candidate.task.difficulty === difficulty));
            ...
            const selectionCell = mode === 'translate_build'
                ? [...new Map([...cell].sort((left, right) => (sha256(`translate-primary:${left.task.taskId}`)
                        .localeCompare(sha256(`translate-primary:${right.task.taskId}`)))).map((candidate) => [candidate.primaryPhraseKey, candidate])).values()]
                : cell;
            ...
            const selectedAdvanced = selectDiverseCandidates(advancedFillGap, advancedTarget, selectionState);
            const selected = [
                ...selectedAdvanced,
            ...
            for (const candidate of selected) {
                ...
                const validation = (0, tournament_core_1.validateTournamentTaskForNewRoom)(task);
                if (!validation.ok)
                    throw new Error(`new_tournament_pool_task_invalid:${task.taskId}:${validation.reason}`);
                tasks.push(task);
            }
    ...
    if (tasks.length !== exports.NEW_TOURNAMENT_POOL_TASK_COUNT) {
    ...
    return {
        ...
        manifest: {
            ...
            candidateCounts,
            exposure: {
                bucketMaxTasks: exports.NEW_TOURNAMENT_POOL_EXPOSURE_BUCKET_SIZE,
...
//# sourceMappingURL=tournament_pool_v2_factory.js.map