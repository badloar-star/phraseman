...
describe('lesson card contrast contract', () => {
  it('keeps lesson meta labels dark on bright green lesson cards', () => {
    ...
    expect(source).toContain('const useDarkMetaText = useFilledMetaText || (!isGoldTheme && !isCoralTheme && isUnlocked);');
    ...
    const metaStart = source.indexOf('const lessonMetaColor = isGoldTheme');
