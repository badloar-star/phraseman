...
describe('agent-manager execution job contract', () => {
    ...
    test('accepts only the bounded execution scopes, including local-only code preparation', () => {
        ...
        expect((0, execution_contracts_1.parseExecutionJob)(job({ scope: 'code_prepare', handlerVersion: 'code-prepare-v1' })).scope).toBe('code_prepare');
        expect(() => (0, execution_contracts_1.parseExecutionJob)(job({ scope: 'content_prepare', handlerVersion: 'content-prepare-v1' }))).toThrow(https_1.HttpsError);
    });
