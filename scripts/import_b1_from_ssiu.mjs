/**
 * Строгий импорт B1 из SSIU/*.txt → scripts/b1_seed.json
 * - вырезает JSON-массивы из текста (в т.ч. с обрывами и ``` fences)
 * - убирает quiz_logic
 * - снимает префиксы A)–D) у вариантов и correct
 * - нормализует " / " → " · " (как в A1/A2)
 * - для translate_meaning: если correct не совпал с вариантом, но совпала англ. часть до · — подставляет канонический вариант из списка
 * - отбрасывает дубли вариантов, пустые поля, пропуски без ___, correct ∉ options
 */
import fs from 'fs';
...
/** Первый сегмент до · (для сопоставления EN части в translate_meaning) */
...
/**
 * Вытащить все топ-level JSON-массивы [...] из текста (учёт строк в кавычках).
 */
...
function loadAllRaw() {
  const paths = [
    path.join(root, 'SSIU', 'ARENA B1 GPT.txt'),
    path.join(root, 'SSIU', 'ARENA B1 GPT - Copy.txt'),
  ];
  const out = [];
  for (const p of paths) {
    if (!fs.existsSync(p)) continue;
    const text = fs.readFileSync(p, 'utf8');
    out.push(...extractJsonArrays(text));
  }
  return out;
}

/**
 * Подобрать correct из options (точное совпадение или EN-сегмент для translate_meaning).
 */
...
function toSeedItem(raw, stats) {
  ...
  if (String(raw.level || '').toUpperCase().replace(/\s/g, '') !== 'B1') {
    stats.dropLevel++;
    return null;
  }
  const type = raw.type;
...
const seedPath = path.join(__dirname, 'b1_seed.json');
...
const r = spawnSync(process.execPath, [path.join(__dirname, 'build_arena_b1.mjs')], {
  cwd: root,
  stdio: 'inherit',
});
