    ...
    return {
      ...
      payload: {
        ...
        items: rightOptions.map((_, index) => ({
          ...
          correctIndex: index,
          explanation: {
            ruleNote: `word-${index + 1} has one exact match.`,
            example: `word-${index + 1} — ${rightOptions[index]}.`,
...
describe('owner tournament round and tie contracts', () => {
  ...
  it('keeps the inter-round leaderboard on screen for exactly six seconds', () => {
    expect(TOURNAMENT_TABLE_DISPLAY_MS).toBe(6_000);
  });
