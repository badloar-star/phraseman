...
const [{ renderAdminAnalytics }, { completeAnalyticsLoad }] = await Promise.all([
...
test('renders an unavailable funnel ratio as an em dash', () => {
  ...
  assert.match(html, /<strong>—<\/strong><small>Завершённые покупки относительно показов экрана оплаты<\/small>/);
  assert.doesNotMatch(html, /<strong>0\.0%<\/strong><small>Завершённые покупки относительно показов экрана оплаты<\/small>/);
});
