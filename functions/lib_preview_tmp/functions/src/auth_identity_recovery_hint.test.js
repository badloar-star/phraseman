...
let currentDb = null;
const registeredCallableOptions = new WeakMap();
...
const { authRecoveryHint, buildRecoveryHintFromUserData, maskEmailForRecoveryHint, } = require('./auth_identity');
function makeRecoveryDb(initial = {}, failPaths = []) {
    const reads = [];
    currentDb = {
...
function runRecoveryHint(input) {
    const handler = authRecoveryHint;
    return handler({
...
describe('authRecoveryHint callable disclosure boundary', () => {
    ...
    it('registers with unconditional App Check enforcement', () => {
        expect(registeredCallableOptions.get(authRecoveryHint))
    ...
    ])('returns the same neutral response for a %s anchor without reading the target', async (_case, authLinks) => {
        const { reads } = makeRecoveryDb({
    ...
    ])('maps a %s read fault to generic identity-check unavailable', async (_case, failPath) => {
        makeRecoveryDb({
            auth_links: { 'auth-a': { stable_id: 'stable-a' } },
