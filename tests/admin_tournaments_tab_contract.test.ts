/**
 * Контракт вкладки «Турниры» в живой админке (admin/legacy.html).
 *
...
const ADMIN = path.resolve(__dirname, "..", "admin", "legacy.html");
...
function runTab() {
  ...
  const sandbox: Record<string, unknown> = {
    httpsCallable:
      ...
      async (payload: Record<string, unknown>) => {
              ...
              items: [
                {
                  ...
                  payload: {
                    phrase: "I am here",
                    options: ["Я здесь", "Как дела", "Спасибо", "Пока"],
                    correctIndex: 0,
                  },
                },
              ],
        ...
        if (name === "adminTournamentPoolStats") {
          return {
            data: {
              ...
              poolReady: true,
              rounds: [
                { round: 1, available: 120, ready: true },
                { round: 2, available: 120, ready: true },
                { round: 3, available: 80, ready: true },
                { round: 4, available: 90, ready: true },
              ],
            },
          };
        }
        ...
        if (name === "adminGenerateTournamentTasks") {
          return {
            data: {
              stats: { produced: 10, phrasesSeen: 5 },
              written: 10,
              keptPublished: 3,
              samples: [],
            },
    ...
    confirm: () => true,
    open: (url: string) => {
      calls.push({ name: "window.open", payload: { url } });
    },
  };
    ...
    getElementById: (id: string) =>
      (elements[id] ||= {
        ...
        value:
          id === "tn-status"
            ? "draft"
            : id === "tn-source" || id === "tn-ai-topic" || id === "tn-cur-date"
              ? ""
              : "0",
        querySelector: () => null,
      }),
    querySelectorAll: (selector: string) =>
      selector.includes("tn-pick") || selector.includes("tn-slot")
        ? checkboxes
        : [],
    querySelector: () => null,
  };
...
describe("вкладка «Турниры» в админке", () => {
  ...
  it("без отмеченных заданий публикация не уходит на сервер", async () => {
    ...
    expect(
      calls.some((call) => call.name === "adminMutateTournamentTasks"),
    ).toBe(false);
    expect(
      calls.some((call) =>
        String(call.toast ?? "").includes("Нечего публиковать"),
      ),
    ).toBe(true);
  });
  ...
  it("публикация уходит только по явному действию и только с отмеченными", async () => {
    ...
    const mutate = calls.find(
      (call) => call.name === "adminMutateTournamentTasks",
    );
  ...
  it("новые типы вопросов рисуются, а не выходят пустой карточкой", () => {
    ...
    const gap = render({
      ...
      valid: true,
      payload: {
        phrase: "I am looking ___ my keys",
    ...
    const oddity = render({
      ...
      valid: true,
      payload: {
        phrase: "Какая фраза звучит неправильно?",
        options: [
          "I feel good",
          "I feel myself good",
          "I am fine",
  ...
  it("кураторский набор: отбор галочками → раунд → сохранение в формате сервера", async () => {
    ...
    const save = calls.find(
      (call) => call.name === "adminSetTournamentCurated",
    );
  ...
  it("публикация и кураторский отбор берут галочки ТОЛЬКО из списка ревью", () => {
    ...
    expect(html.slice(publishStart, publishStart + 800)).toContain(
      "'#tn-list .tn-pick:checked'",
    );
    const curatedStart = html.indexOf("window.tnCuratedAdd");
    expect(html.slice(curatedStart, curatedStart + 800)).toContain(
      "'#tn-list .tn-pick:checked'",
    );
  });
  ...
  it("кураторский набор без даты не уходит на сервер", async () => {
    ...
    await (sandbox.tnCuratedSave as () => Promise<void>)();
    expect(
      calls.some((call) => call.name === "adminSetTournamentCurated"),
    ).toBe(false);
  });
  ...
  it("разметка вкладки следует правилам владельца", () => {
    ...
    expect((section.match(/<div/g) ?? []).length).toBe(
      (section.match(/<\/div>/g) ?? []).length,
    );
    // Контейнеры разделяются тоном, а не обводками (правило владельца).
    expect(section).not.toMatch(
      /border:1px solid[^"]*"\s*>\s*<div style="font-size:15px;font-weight:800/,
    );
  });
});
