...
exports.vipReconcileOrphanGrant = void 0;
exports.decideVipReconcile = decideVipReconcile;
/**
 ...
 */
function vipActiveNow(progress, now) {
    if (isFalsyFlag(progress.vip_active) || isFalsyFlag(progress.vip_admin_override))
        return false;
 ...
 */
function decideVipReconcile(orphanProgress, canonicalProgress, now, orphanStableId) {
    if (!vipActiveNow(orphanProgress, now))
        return { kind: 'skip' };
    ...
    for (const k of VIP_KEYS) {
        if (orphanProgress[k] !== undefined)
            vipPatch[k] = orphanProgress[k];
    }
...
function extinguishPatch(canonicalStableId, now) {
    return {
        vip_active: 'false',
...
exports.vipReconcileOrphanGrant = functions.firestore.onDocumentWritten({ document: `${USERS}/{userId}`, region: REGION }, async (event) => {
    const after = event.data?.after.exists ? event.data?.after.data() : undefined;
    if (!after)
        return;
    // Срабатываем ТОЛЬКО на осиротевшем документе с указанием живого canonical.
    if (after.identityHidden !== true)
        return;
    const userId = event.params.userId;
    const canonicalStableId = cleanStr(after.canonicalStableId);
    if (!canonicalStableId || canonicalStableId === userId)
        return;
    const now = Date.now();
    const orphanProgress = after.progress || {};
    if (!vipActiveNow(orphanProgress, now))
        return; // нет активного VIP для переноса
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (!canonicalSnap.exists)
            return; // живого дока нет — не создаём, ждём первого синка
        ...
        if (canonicalData.identityHidden === true)
            return;
        const canonicalProgress = canonicalData.progress || {};
...
//# sourceMappingURL=vip_orphan_reconcile.js.map