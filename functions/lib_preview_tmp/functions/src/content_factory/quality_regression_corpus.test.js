...
describe('prompt regression corpus', () => {
    ...
    it('accepts every golden case and rejects every red-team case with expected hard failures', () => {
        ...
        expect(report.summary).toEqual({ total: quality_regression_corpus_1.PROMPT_REGRESSION_CASES.length, goldenAccepted: 3, goldenRejected: 0, redTeamRejected: quality_regression_corpus_1.PROMPT_REGRESSION_CASES.length - 3, redTeamAccepted: 0, regressions: 0, candidateFailures: 0 });
    ...
    it('contains all named R3-R6 regression families', () => {
        ...
        for (const category of ['incomplete_phrase', 'source_calque', 'grammar_invalid', 'word_salad', 'hard_too_weak', 'ambiguous_answer', 'correct_index_mismatch', 'locale_drift', 'cefr_drift', 'semantic_duplicate', 'invented_grounding'])
