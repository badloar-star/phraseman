...
exports.LEARNING_V2_ACTIVITY_LEARNER_CORE_CAPSULE_MAX_BYTES_V1 = exports.LEARNING_V2_ACTIVITY_LEARNER_CORE_RENDER_MAX_BYTES_V1 = exports.LEARNING_V2_ACTIVITY_LEARNER_CORE_RELEASE_INDEX_MAX_BYTES_V1 = exports.LEARNING_V2_ACTIVITY_LEARNER_CORE_RELEASE_INDEX_SCHEMA_V1 = void 0;
exports.materializeLearningV2ActivityLearnerCoreReleaseIndexV1 = materializeLearningV2ActivityLearnerCoreReleaseIndexV1;
exports.parseLearningV2ActivityLearnerCoreReleaseIndexV1 = parseLearningV2ActivityLearnerCoreReleaseIndexV1;
...
exports.LEARNING_V2_ACTIVITY_LEARNER_CORE_RENDER_MAX_BYTES_V1 = 256 * 1024;
exports.LEARNING_V2_ACTIVITY_LEARNER_CORE_CAPSULE_MAX_BYTES_V1 = 64 * 1024;
...
function expectedChildPath(stageId, sessionOrdinal, kind, contentHash) {
    return `learning-v2/canonical/activity-instances/${(0, decision_registry_1.sha256Utf8)(stageId)}/sessions/${String(sessionOrdinal).padStart(2, "0")}/${kind}/${contentHash}.json`;
}
function parsePin(value, stageId, sessionOrdinal, kind, raw) {
    if (!record(value))
        fail();
    exactKeys(value, PIN_KEYS);
    const contentHash = hash(value.contentHash);
    const maximumBytes = kind === "render"
        ? exports.LEARNING_V2_ACTIVITY_LEARNER_CORE_RENDER_MAX_BYTES_V1
        : exports.LEARNING_V2_ACTIVITY_LEARNER_CORE_CAPSULE_MAX_BYTES_V1;
    if (typeof value.objectGeneration !== "string" ||
        ...
        (raw !== undefined &&
            ((0, decision_registry_1.sha256Utf8)(raw) !== contentHash ||
                (0, decision_registry_1.utf8ByteLengthV1)(raw) !== Number(value.byteSize))))
        fail();
...
function sessionFromMaterial(value, stageId, episodeId, studyTarget, expectedOrdinal) {
    if (!record(value))
        fail();
    exactKeys(value, [
        "sessionId",
    ...
    const render = canonical(value.renderRaw, exports.LEARNING_V2_ACTIVITY_LEARNER_CORE_RENDER_MAX_BYTES_V1);
    exactKeys(render, RENDER_KEYS);
    if (!record(render.session))
        fail();
    ...
    const capsule = canonical(value.capsuleEnvelopeRaw, exports.LEARNING_V2_ACTIVITY_LEARNER_CORE_CAPSULE_MAX_BYTES_V1);
    exactKeys(capsule, CAPSULE_KEYS);
    if (capsule.schemaVersion !== "v2-activity-session-capsule-envelope.v1" ||
    ...
    return Object.freeze({
        ...
        sourceFingerprint,
        renderFingerprint: (0, decision_registry_1.hashCanonicalBody)(render),
        capsuleEnvelopeFingerprint: (0, decision_registry_1.hashCanonicalBody)(capsule),
        render: parsePin(value.renderPin, stageId, expectedOrdinal, "render", value.renderRaw),
        capsule: parsePin(value.capsulePin, stageId, expectedOrdinal, "capsule", value.capsuleEnvelopeRaw),
    });
}
function parseSession(value, stageId, expectedOrdinal) {
    if (!record(value))
        fail();
...
function parseValue(value) {
    ...
    if (value.schemaVersion !==
        exports.LEARNING_V2_ACTIVITY_LEARNER_CORE_RELEASE_INDEX_SCHEMA_V1 ||
    ...
    if (value.releaseIdentityEvidence !== body.releaseIdentityEvidence ||
        ...
        (0, decision_registry_1.hashCanonicalBody)(body) !== indexFingerprint)
        fail();
    const result = Object.freeze({ ...body, indexFingerprint });
    handles.add(result);
    return result;
}
function materializeLearningV2ActivityLearnerCoreReleaseIndexV1(input) {
    ...
    return parseLearningV2ActivityLearnerCoreReleaseIndexV1((0, decision_registry_1.canonicalJsonV1)({ ...body, indexFingerprint: (0, decision_registry_1.hashCanonicalBody)(body) }));
}
function parseLearningV2ActivityLearnerCoreReleaseIndexV1(raw) {
    const value = canonical(raw, exports.LEARNING_V2_ACTIVITY_LEARNER_CORE_RELEASE_INDEX_MAX_BYTES_V1);
    return parseValue(value);
}
function encodeLearningV2ActivityLearnerCoreReleaseIndexV1(index) {
    if (!isLearningV2ActivityLearnerCoreReleaseIndexV1(index))
        fail();
    return (0, decision_registry_1.canonicalJsonV1)(index);
}
function isLearningV2ActivityLearnerCoreReleaseIndexV1(value) {
    return typeof value === "object" && value !== null && handles.has(value);
}
//# sourceMappingURL=activity_learner_core_release_index_v1.js.map