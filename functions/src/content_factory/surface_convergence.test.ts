import { compareArenaSurfaceArtifacts } from './surface_convergence';
...
describe('Arena surface convergence comparator', () => {
  it('accepts semantic parity while ignoring option order and explicit stage-only metadata', () => {
    ...
    const receipt = compareArenaSurfaceArtifacts({ comparatorVersion: 'arena-parity-v1', legacyIdentity: identity, stageIdentity: identity, legacyArtifact: legacy, stageArtifact: reordered, legacyArtifactHash: 'a'.repeat(64), stageArtifactHash: 'b'.repeat(64), legacyQaOutcome: 'pass', stageQaOutcome: 'pass' });
  ...
  it.each([
    ...
    const receipt = compareArenaSurfaceArtifacts({ comparatorVersion: 'arena-parity-v1', legacyIdentity: identity, stageIdentity: identity, legacyArtifact: legacy, stageArtifact: staged, legacyArtifactHash: 'a'.repeat(64), stageArtifactHash: 'b'.repeat(64), legacyQaOutcome: 'pass', stageQaOutcome: 'pass', ...override });
