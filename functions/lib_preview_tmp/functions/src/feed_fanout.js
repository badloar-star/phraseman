...
exports.feedPruneCron = exports.feedFanoutOnMyEvent = void 0;
exports.pruneExpiredFeedBatch = pruneExpiredFeedBatch;
/**
 ...
 */
exports.feedFanoutOnMyEvent = (0, firestore_1.onDocumentCreated)({ region: REGION, document: 'users/{uid}/my_events/{eventId}' }, async (event) => {
 ...
 */
async function pruneExpiredFeedBatch() {
    const db = admin.firestore();
    let deleted = 0;
    ...
    while (deleted < maxPerRun) {
        ...
        await batch.commit();
        deleted += snap.size;
        if (snap.size < PRUNE_PAGE_SIZE)
            break;
    }
    console.log(JSON.stringify({ event: 'feed_prune_done', deleted }));
    return { deleted };
}
exports.feedPruneCron = functions.scheduler.onSchedule({ schedule: 'every 24 hours', timeZone: 'UTC', region: REGION, memory: '512MiB', timeoutSeconds: 540 }, async () => {
    await pruneExpiredFeedBatch();
});
//# sourceMappingURL=feed_fanout.js.map