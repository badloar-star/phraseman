...
exports.canonicalizeCommunityPurchaseRows = canonicalizeCommunityPurchaseRows;
...
function canonicalizeCommunityPurchaseRows(rows) {
    return rows.map((row) => {
...
exports.adminGetUserProfile = (0, https_1.onCall)({ region: REGION, enforceAppCheck: callable_options_1.ENFORCE_APP_CHECK, timeoutSeconds: 20, memory: '512MiB' }, async (request) => {
    ...
    const sources = {
        ...
        ugcBuys: adapterSource('community_pack_purchases_buyer', { ...ugcBuys, rows: canonicalizeCommunityPurchaseRows(ugcBuys.rows) }, ['id', 'packId', 'packTitle', 'status', 'acquisitionSource', 'priceShards', 'price', 'createdAt'], ['packId', 'packTitle', 'status', 'acquisitionSource']),
        ugcSells: adapterSource('community_pack_purchases_seller', { ...ugcSells, rows: canonicalizeCommunityPurchaseRows(ugcSells.rows) }, ['id', 'packId', 'packTitle', 'status', 'acquisitionSource', 'priceShards', 'price', 'createdAt'], ['packId', 'packTitle', 'status', 'acquisitionSource']),
