export type GeneratedSurfaceItem = { readonly id: string; readonly front: string; readonly back: string };

export interface GeneratedSurfaceArtifact {
  readonly lessonId: number;
  readonly surface: 'flashcard';
...
export function parseGeneratedSurfaceArtifact(raw: string): GeneratedSurfaceArtifact {
  ...
  if (surface !== 'flashcard' || !Number.isInteger(artifact.lessonId) || Number(artifact.lessonId) < 1 || !Array.isArray(artifact.items) || artifact.items.length === 0) throw new Error('generated_surface_invalid');
