...
// + СТАРТ ES §0/§10, Глава 2 "Ты: вопрос"): испанская сессия 14 переписывается
// в mode-native формат, зеркаля код (не данные) сессии 10/11/13 — тот же
// класс сессии (kind: 'phrases', НЕТ newVocabulary). de acuerdo вводится не
// через word-first vocabulary, а прямо во фразах как обычные позиционные
// токены De/de + acuerdo — легаси-комментарий подтверждает research-агентом,
// что word-first vocabulary в этом курсе поддерживает только однословные
// target (task_specific_distractors_v1.ts, session_shard_from_source_v1.ts,
// inferLesson1WordFirstVocabularyCountV1 требуют "!/\s/u.test(target)").
//
...
// в сессиях 10/11/13): phraseSteps() уже использует только утверждённые
// families, каждый шаг получает реальный mode-native payload.
const phrases = ES_EPISODE_01_SESSION_14_PHRASES;
if (phrases.length !== 15) {
  throw new Error('es_session_14_mode_native_phrase_count_invalid');
}

const L = (source: LocalizedSource): LearningV2Localized<string> => expandLocalized(source);

// зачем эта функция (владелец, 2026-08-27, тот же класс бага сессий 5/6/8/9/10/11/12/13):
...
// зачем 'en' вместо 'es' здесь (как в испанских сессиях 1-13): в испанском
...
function localizedPhraseField(
  ...
  if (!phrase?.localizedDetails) throw new Error(`es_session_14_phrase_details_missing:${phraseIndex}`);
  return Object.fromEntries(LEARNING_V2_INTERFACE_LOCALES.map((locale) => {
    ...
    if (!detail) throw new Error(`es_session_14_phrase_locale_missing:${phraseIndex}:${locale}`);
...
function localizedPhraseDistractorFeedback(
  ...
  return Object.fromEntries(LEARNING_V2_INTERFACE_LOCALES.map((locale) => {
    ...
    if (!reason) throw new Error(`es_session_14_phrase_distractor_missing:${phraseIndex}:${locale}:${value}`);
...
function phraseAudio(phraseIndex: number): LearningV2ModeAudioReferenceV1 {
  const phrase = phrases[phraseIndex]!;
  return authoredAudio(`es-e01-s14-phrase-${phraseIndex + 1}`, phrase.english);
}
function phraseSlowAudio(phraseIndex: number): LearningV2ModeAudioReferenceV1 {
  const phrase = phrases[phraseIndex]!;
  return authoredAudio(`es-e01-s14-phrase-${phraseIndex + 1}-slow`, phrase.english);
}
...
// зачем гейт стоит на ПОСЛЕДНЕМ содержательном слове фразы (владелец,
// 2026-08-27, как в checkpoint-сессии 8): у фраз этой сессии нет единого
// грамматического явления в одной позиции — de acuerdo то целиком реплика,
// то часть более длинной фразы (Eres segura, de acuerdo). Гейт на последнем
// слове универсально проверяет то, что действительно варьируется во фразе,
// с authored-дистракторами из phrases-файла, без придумывания нового текста.
// зачем responseId включает entry.value, а не только asciiId(reasonCode)
// (владелец, 2026-08-27, класс бага, найденный в сессии 12): если легаси-
// данные когда-нибудь дадут двум дистракторам одной позиции одинаковый
// reasonCode, entry.value в ключе responseId предотвращает коллизию.
function contextGapLastWord(phraseIndex: number): LearningV2ModeNativePayloadV1 {
  ...
  return Object.freeze({
    ...
    testedDimension: `agreement_reaction:${asciiId(phrase.id)}`,
    choiceFeedback: Object.freeze([
      ...
      ...lastWord.distractors.map((entry) => feedback(
        ...
        false,
        `agreement_reaction:${asciiId(entry.reasonCode)}`,
...
// уже изученного материала, а не смежный блок.
function speedMatchPhrases(pairIndices: readonly number[]): LearningV2ModeNativePayloadV1 {
  ...
  const ids = pairIndices.map((index) => `es-e01-s14-pair-${index + 1}`);
  return Object.freeze({
    family: 'speed_match',
...
// легаси generic phraseSteps(), как и в сессиях 10/11/13): те же 12
// families/purposes/индексов (3-14), реальный mode-native payload вместо
// generic-карточки для каждого шага.
export const ES_EPISODE_01_SESSION_14_MODE_NATIVE_PRACTICE_V1 = Object.freeze<readonly SessionModeNativePracticeSourceV1[]>([
...
if (ES_EPISODE_01_SESSION_14_MODE_NATIVE_PRACTICE_V1.length !== 12) {
  throw new Error('es_session_14_mode_native_practice_count_invalid');
}
