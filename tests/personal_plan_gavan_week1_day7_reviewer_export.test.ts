...
import {
  ...
  GAVAN_WEEK1_DAY7_REVIEWER_EXPORT_PATH,
  validateGavanWeek1Day7ReviewerExport,
  writeGavanWeek1Day7ReviewerExport,
} from '../tools/personal_plan_gavan_week1_day7_reviewer_export';

const GENERATED_AT = '2026-06-03T01:25:00.000Z';
const BROKEN_ENCODING_RE = /[\u00d0\u00d1\u00c2\u00e2\ufffd]/;

function candidate() {
  return buildGavanWeek1Day7BlueprintCandidate(
    buildGavanWeek1ExpansionStandards({ generatedAt: GENERATED_AT }),
    { generatedAt: GENERATED_AT },
  );
}

describe('Gavan week 1 day 7 reviewer export', () => {
  beforeEach(() => {
    if (existsSync(GAVAN_WEEK1_DAY7_REVIEWER_EXPORT_PATH)) {
      rmSync(GAVAN_WEEK1_DAY7_REVIEWER_EXPORT_PATH, { force: true });
    }
  });

  afterAll(() => {
    writeGavanWeek1Day7ReviewerExport(candidate(), {
      generatedAt: GENERATED_AT,
      targetPath: GAVAN_WEEK1_DAY7_REVIEWER_EXPORT_PATH,
    });
  });

  it('exports every day 7 content unit and explanation card for manual review', () => {
    const blueprint = candidate();
    const reviewExport = buildGavanWeek1Day7ReviewerExport(blueprint, {
      generatedAt: GENERATED_AT,
    });

    expect(reviewExport.kind).toBe('gavan_week1_day7_reviewer_export');
    expect(reviewExport.dayId).toBe('gavan-week1-day7');
    expect(reviewExport.dayIndex).toBe(7);
    ...
    expect(validateGavanWeek1Day7ReviewerExport(reviewExport, blueprint)).toEqual({
      valid: true,
      issues: [],
    });
  });

  it('keeps day 7 media and pronunciation claims honest for a non-final blueprint', () => {
    const reviewExport = buildGavanWeek1Day7ReviewerExport(candidate(), {
      generatedAt: GENERATED_AT,
    });
  ...
  it('proves forbidden anchors visible numbers and corrupted copy are absent from exported visible copy', () => {
    const reviewExport = buildGavanWeek1Day7ReviewerExport(candidate(), {
      generatedAt: GENERATED_AT,
    });
    const visibleCopy = JSON.stringify({
      contentUnitRows: reviewExport.contentUnitRows.map((row) => ({
        english: row.english,
        meaningRu: row.meaningRu,
        newWords: row.newWords,
        firstSeenConstructions: row.firstSeenConstructions,
      })),
      explanationRows: reviewExport.explanationRows.map((row) => ({
        covers: row.covers,
        body: row.body,
      })),
      exerciseRows: reviewExport.exerciseRows.map((row) => ({
    ...
    expect(visibleCopy).not.toMatch(
      /phone|email|apartment|rent|document|doctor|bank|087|@|\balex\b|beta8958|\d/,
    );
    expect(visibleCopy).not.toMatch(BROKEN_ENCODING_RE);
  });

  it('exports day 7 exercise blueprint coverage without pretending exercises are built', () => {
    const blueprint = candidate();
    const reviewExport = buildGavanWeek1Day7ReviewerExport(blueprint, {
      generatedAt: GENERATED_AT,
    });
    ...
    expect(reviewExport.exerciseCoverage).toEqual({
      requiredTypes: ['active_recall', 'listening_choice', 'natural_choice', 'micro_dialogue'],
      presentTypes: ['active_recall', 'listening_choice', 'natural_choice', 'micro_dialogue'],
      valid: true,
    });
  ...
  it('catches missing rows fake media claims forbidden anchors visible numbers and missing exercise coverage', () => {
    const blueprint = candidate();
    const reviewExport = buildGavanWeek1Day7ReviewerExport(blueprint, {
      generatedAt: GENERATED_AT,
    });
    ...
    const corrupted = {
      ...
      exerciseRows: reviewExport.exerciseRows.filter((row) => row.exerciseType !== 'active_recall'),
      exerciseCoverage: {
        ...reviewExport.exerciseCoverage,
        presentTypes: reviewExport.exerciseCoverage.presentTypes.filter((type) => type !== 'active_recall'),
        valid: false,
      },
    ...
    const result = validateGavanWeek1Day7ReviewerExport(corrupted, blueprint);
  ...
  it('writes deterministic day 7 reviewer JSON only under temp or report roots', () => {
    const result = writeGavanWeek1Day7ReviewerExport(candidate(), {
      generatedAt: GENERATED_AT,
      targetPath: GAVAN_WEEK1_DAY7_REVIEWER_EXPORT_PATH,
    });
    ...
    expect(existsSync(GAVAN_WEEK1_DAY7_REVIEWER_EXPORT_PATH)).toBe(true);

    const serialized = readFileSync(GAVAN_WEEK1_DAY7_REVIEWER_EXPORT_PATH, 'utf8');
    expect(serialized).not.toMatch(BROKEN_ENCODING_RE);

    const parsed = JSON.parse(serialized);
    expect(parsed.kind).toBe('gavan_week1_day7_reviewer_export');
  ...
  it('rejects app tools tests and root config write targets', () => {
    const appResult = writeGavanWeek1Day7ReviewerExport(candidate(), {
      generatedAt: GENERATED_AT,
      targetPath: path.join(process.cwd(), 'app', 'gavan-week1-day7-reviewer-export.json'),
    });
    const toolsResult = writeGavanWeek1Day7ReviewerExport(candidate(), {
      generatedAt: GENERATED_AT,
      targetPath: path.join(process.cwd(), 'tools', 'gavan-week1-day7-reviewer-export.json'),
    });
    const testsResult = writeGavanWeek1Day7ReviewerExport(candidate(), {
      generatedAt: GENERATED_AT,
      targetPath: path.join(process.cwd(), 'tests', 'gavan-week1-day7-reviewer-export.json'),
    });
  ...
  it('does not import live catalog quiz UI storage audio scoring or navigation', () => {
    const source = readFileSync(
      path.join(process.cwd(), 'tools', 'personal_plan_gavan_week1_day7_reviewer_export.ts'),
      'utf8',
    );
