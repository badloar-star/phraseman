...
const labelForContext: Record<ReviewContext, string> = {
  perfect_lesson: 'КСТАТИ, ПРО УРОК',
  level_exam_pass: 'МАЛЕНЬКИЙ ВОПРОС',
        ...
        >
          ...
          {context === 'streak_milestone' && streakDays ? (
            <View style={{ marginTop: 14, minHeight: 42, paddingHorizontal: 14, borderRadius: 14, backgroundColor: `${accent}33`, alignItems: 'center', justifyContent: 'center' }}>
              <Text style={{ color: accent, fontSize: f.bodyLg, fontWeight: '800' }}>{streakDays} дней подряд</Text>
            </View>
          ) : null}
          <Text style={{ color: t.textMuted, fontSize: f.body, lineHeight: f.body * 1.45, textAlign: 'center', marginTop: 9, maxWidth: 330 }}>
            {variant.subtitle}
          </Text>
          <View style={{ flexDirection: 'row', gap: 8, marginTop: 19 }} accessibilityLabel="Оценка в App Store">
