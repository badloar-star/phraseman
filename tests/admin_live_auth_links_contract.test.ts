...
describe("live admin Auth Links diagnostics", () => {
  ...
  test("lives only on the Firebase-hosted legacy surface", () => {
    const firebase = JSON.parse(read("firebase.json")) as {
      hosting?: Array<{ target?: string; public?: string }>;
    };
    expect(
  ...
  test("uses only protected adminGetUserProfile data for Auth Links", () => {
    ...
    expect(diagnostic).not.toMatch(
      /collection\(|getDoc\(|setDoc\(|updateDoc\(|deleteDoc\(|addDoc\(/,
    );
  ...
  test("renders canonical identities, explicit link state, provenance and safe refresh", () => {
    ...
    expect(live).toContain(
      "const firebaseAuthUid = String(identity.providerUid || '').trim()",
    );
    expect(live).toContain(
      "const providerUid = String(auth.providerUid || '').trim()",
    );
