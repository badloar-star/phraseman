  ...
  const projections = Object.freeze(
    material.intros.map((intro, index) => {
      ...
      return materializeLearningV2ActivitySessionIntroProjectionV1({
        ...
        title: raw.title,
        pages: raw.questions.map((question, questionIndex) => {
          const task = tasks[questionIndex];
          const concept = raw.concepts.find(
            (candidate) =>
              question.coveredConceptIds.length === 1 &&
              candidate.conceptId === question.coveredConceptIds[0],
          );
          if (
            !task ||
            !concept ||
          ...
          return Object.freeze({
            pageOrdinal: (questionIndex + 1) as 1 | 2 | 3,
            conceptId: concept.conceptId,
            heading: concept.heading,
            explanation: concept.explanation,
            question: Object.freeze({
              ...question,
              taskSlot: (questionIndex + 1) as 1 | 2 | 3,
