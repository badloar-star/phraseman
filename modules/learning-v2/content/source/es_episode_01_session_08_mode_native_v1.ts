...
// + СТАРТ ES §0/§10, Rules §4.1): испанская checkpoint-сессия 8 переписывается
// в mode-native формат. Как и voice-сессия 7, checkpoint НЕ вводит новых слов
// и не имеет newVocabulary — переиспользует РОВНО тот же набор 15 фраз, что
// и сессия 7 (ES_EPISODE_01_SESSION_08_CHECKPOINT_PHRASES = алиас на
// ES_EPISODE_01_SESSION_07_VOICE_PHRASES). Разница между voice и checkpoint —
...
// (владелец, 2026-08-27, найдено при чтении choreography перед написанием
// этого файла): checkpointSteps() уже использует только утверждённые families
// (speed_match/listen_build_dictation/context_gap_grammar/phrase_builder) —
// в отличие от voiceSteps() там нет 'sound_contrast'. Единственная правка —
// learningStage: 'independent_assessment' НЕ входит в допустимый набор
// SessionModeNativePracticeSourceV1.learningStage (только recognize/
// retrieve_meaning/build_form/apply_in_phrase/speak_with_model) — заменено на
// 'apply_in_phrase', что точно описывает суть checkpoint-контакта: применение
// уже изученной формы во фразе без подсказки.
const phrases = ES_EPISODE_01_SESSION_08_CHECKPOINT_PHRASES;
if (phrases.length !== 15) {
  throw new Error('es_session_08_mode_native_phrase_count_invalid');
}

const L = (source: LocalizedSource): LearningV2Localized<string> => expandLocalized(source);

// зачем эта функция (владелец, 2026-08-27, тот же класс бага, что и в
// сессиях 5/6): context_gap_grammar передаёт сырой responseId напрямую в
// runtime без позиционного переприсвоения; ID_RE в
// course_session_client_children_v1.ts не допускает диакритику. Фразы этой
// сессии переиспользуют reasonCode из сессий 4-6 (verdadero/único/rápido —
// все с диакритикой), поэтому санитизация обязательна здесь тоже.
...
// зачем 'en' вместо 'es' здесь (как в испанских сессиях 1-7): в испанском
...
function localizedPhraseField(
  ...
  if (!phrase?.localizedDetails) throw new Error(`es_session_08_phrase_details_missing:${phraseIndex}`);
  return Object.fromEntries(LEARNING_V2_INTERFACE_LOCALES.map((locale) => {
    ...
    if (!detail) throw new Error(`es_session_08_phrase_locale_missing:${phraseIndex}:${locale}`);
...
function localizedPhraseDistractorFeedback(
  ...
  return Object.fromEntries(LEARNING_V2_INTERFACE_LOCALES.map((locale) => {
    ...
    if (!reason) throw new Error(`es_session_08_phrase_distractor_missing:${phraseIndex}:${locale}:${value}`);
...
function listenBuildPhrase(phraseIndex: number): LearningV2ModeNativePayloadV1 {
  ...
  return Object.freeze({
    ...
    referenceAudio: { audioTargetId: `es-e01-s08-phrase-${phraseIndex + 1}`, transcript: phrase.english },
    slowReferenceAudio: { audioTargetId: `es-e01-s08-phrase-${phraseIndex + 1}-slow`, transcript: phrase.english },
...
// (владелец, 2026-08-27, checkpoint-специфика): в отличие от сессий 3-6, где
// гейт целенаправленно стоит на одном конкретном грамматическом явлении
// текущей сессии (gender agreement, negation), checkpoint вразнобой проверяет
// ЛЮБОЙ признак — поэтому гейт универсально ставится на последнее
// содержательное слово каждой фразы (последний элемент words[]), с его
// собственными authored-дистракторами из phrases-файла, без придумывания
// нового текста.
function contextGapLastWord(phraseIndex: number): LearningV2ModeNativePayloadV1 {
  ...
  return Object.freeze({
    ...
    gapOptions: Object.freeze([
      ...
      ...lastWord.distractors.map((entry) => ({ responseId: `${phrase.id}:checkpoint:${asciiId(entry.reasonCode)}`, text: entry.value })),
    ]),
    ...
    choiceFeedback: Object.freeze([
      ...
      ...lastWord.distractors.map((entry) => feedback(
        `${phrase.id}:checkpoint:${asciiId(entry.reasonCode)}`,
        false,
...
// 2026-08-27): checkpoint — единственная сессия главы, где цель — проверить
// владение ЦЕЛОЙ главой сразу, без разбивки. pairGrid сопоставляет target
// испанский текст фразы с её точным переводом; ни одно новое слово не
// вводится — все 15 фраз уже честно изучены в сессиях 1-6 и повторены в
// сессии 7.
const SPEED_IDS = phrases.map((_phrase, index) => `es-e01-s08-pair-${index + 1}`);
...
// зачем именно эти 12 шагов на индексах 3-14 (владелец, 2026-08-27, зеркало
// легаси checkpointSteps() по составу families и индексов, но с исправленным
// learningStage): три первые фразы (0-2) уже отработаны в интро-carousel.
// Оставшиеся 12 (3-14) чередуют families в том же порядке, что и старый
// checkpointSteps(): speed_match/listen_build_dictation/context_gap_grammar/
// phrase_builder ×3, с нарастающей сложностью purpose (retrieval →
// near_transfer → independent_check).
export const ES_EPISODE_01_SESSION_08_MODE_NATIVE_PRACTICE_V1 = Object.freeze<readonly SessionModeNativePracticeSourceV1[]>([
...
if (ES_EPISODE_01_SESSION_08_MODE_NATIVE_PRACTICE_V1.length !== 12) {
  throw new Error('es_session_08_mode_native_practice_count_invalid');
}
