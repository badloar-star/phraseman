...
function handler(
  name: "banUser" | "unbanUser" | "unbanFromList" | "banByUidPrompt",
): string {
...
describe("live admin ban callable contract", () => {
  it("caches one protected adminSetUserBan callable", () => {
    expect(source).toContain(
      "_fnAdminSetUserBan = httpsCallable(functionsUs, 'adminSetUserBan');",
    );
    expect(
      source.match(/httpsCallable\(functionsUs, 'adminSetUserBan'\)/g),
    ).toHaveLength(1);
  ...
  it.each([
    ...
    "%s requires reason and calls the server with crypto-backed idempotency",
    (name, banned) => {
  ...
  it.each([
    ...
    "%s uses only the acknowledged server command with a mandatory reason",
    (name, banned) => {
      const body = handler(name);
      expect(body).toContain("showInputModal({");
      expect(body).toContain(".trim();");
  ...
  it.each(["banUser", "unbanUser"] as const)(
    ...
    (name) => {
      ...
      const cachedMutation = body.indexOf(
        `u.banned = ${name === "banUser" ? "true" : "false"};`,
      );
