...
import {
  ...
  GAVAN_WEEK1_DAY3_BLUEPRINT_CANDIDATE_PATH,
  validateGavanWeek1Day3BlueprintCandidate,
  writeGavanWeek1Day3BlueprintCandidate,
} from '../tools/personal_plan_gavan_week1_day3_blueprint_candidate';

const GENERATED_AT = '2026-06-02T17:00:00.000Z';
...
describe('Gavan week 1 day 3 blueprint candidate', () => {
  beforeEach(() => {
    if (existsSync(GAVAN_WEEK1_DAY3_BLUEPRINT_CANDIDATE_PATH)) {
      rmSync(GAVAN_WEEK1_DAY3_BLUEPRINT_CANDIDATE_PATH, { force: true });
    }
  });

  afterAll(() => {
    writeGavanWeek1Day3BlueprintCandidate(standards(), {
      generatedAt: GENERATED_AT,
      targetPath: GAVAN_WEEK1_DAY3_BLUEPRINT_CANDIDATE_PATH,
    });
  });

  it('builds a non-live day 3 only blueprint candidate from the week standards', () => {
  ...
  it('keeps day 3 universal and free of personal or narrow relocation anchors', () => {
    const candidate = buildGavanWeek1Day3BlueprintCandidate(standards(), {
      generatedAt: GENERATED_AT,
    });
  ...
  it('uses practical day 3 phrase candidates for understanding and pace control', () => {
    const candidate = buildGavanWeek1Day3BlueprintCandidate(standards(), {
      generatedAt: GENERATED_AT,
    });
  ...
  it('includes varied day 3 exercise formats without claiming final exercises', () => {
    const candidate = buildGavanWeek1Day3BlueprintCandidate(standards(), {
      generatedAt: GENERATED_AT,
    });
  ...
  it('passes its own quality gate and catches corrupted candidates', () => {
    ...
    expect(validateGavanWeek1Day3BlueprintCandidate(candidate, standards())).toEqual({
      valid: true,
      issues: [],
    });
      ...
      contentUnits: [
        {
          ...
          english: 'My email is alex@example.com.',
          explanationCards: [],
        },
      ],
    ...
    const result = validateGavanWeek1Day3BlueprintCandidate(corrupted, standards());
  ...
  it('writes deterministic day 3 candidate JSON only under temp or report roots', () => {
    const result = writeGavanWeek1Day3BlueprintCandidate(standards(), {
      generatedAt: GENERATED_AT,
      targetPath: GAVAN_WEEK1_DAY3_BLUEPRINT_CANDIDATE_PATH,
    });
    ...
    expect(existsSync(GAVAN_WEEK1_DAY3_BLUEPRINT_CANDIDATE_PATH)).toBe(true);

    const parsed = JSON.parse(readFileSync(GAVAN_WEEK1_DAY3_BLUEPRINT_CANDIDATE_PATH, 'utf8'));
    expect(parsed.kind).toBe('gavan_week1_day3_blueprint_candidate');
    expect(parsed.dayId).toBe('gavan-week1-day3');
  });

  it('rejects app tools tests and root config write targets', () => {
    const appResult = writeGavanWeek1Day3BlueprintCandidate(standards(), {
      generatedAt: GENERATED_AT,
      targetPath: path.join(process.cwd(), 'app', 'gavan-week1-day3-blueprint.json'),
    });
    const toolsResult = writeGavanWeek1Day3BlueprintCandidate(standards(), {
      generatedAt: GENERATED_AT,
      targetPath: path.join(process.cwd(), 'tools', 'gavan-week1-day3-blueprint.json'),
    });
    const testsResult = writeGavanWeek1Day3BlueprintCandidate(standards(), {
      generatedAt: GENERATED_AT,
      targetPath: path.join(process.cwd(), 'tests', 'gavan-week1-day3-blueprint.json'),
    });
  ...
  it('does not import live catalog quiz UI storage audio scoring or navigation', () => {
    const source = readFileSync(
      path.join(process.cwd(), 'tools', 'personal_plan_gavan_week1_day3_blueprint_candidate.ts'),
      'utf8',
    );
