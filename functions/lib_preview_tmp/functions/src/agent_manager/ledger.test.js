...
class MemoryRepository {
    ...
    async runTransaction(body) {
        ...
        const result = await body({
            get: async (path) => {
                if (this.enforceReadBeforeWrite && wrote)
...
function seedReadyGlobalControl(repo, overrides = {}) {
    repo.documents.set('agent_office_control/global', { id: 'global', data: {
            ...
            lastChangedAtMs: 1999999999000, lastChangedByUid: 'owner-uid',
...
describe('Agent Manager task ledger', () => {
    test('initializes the bounded specialist roster without credentials or execution settings', async () => {
        ...
        const ledger = new ledger_1.AgentManagerLedger(repo, () => 2000000000000);
    ...
    test.each([
        ...
        const repo = new MemoryRepository();
        if (controlOverrides)
            seedReadyGlobalControl(repo, controlOverrides);
        const ledger = new ledger_1.AgentManagerLedger(repo, () => 2000000000000);
        await ledger.initializeRoster(OWNER);
    ...
    test('creates an idempotent PII-free task from an opaque support intake link', async () => {
        const repo = new MemoryRepository();
        const ledger = new ledger_1.AgentManagerLedger(repo, () => 2000000000000);
        const input = { sourceType: 'support', sourceRef: `support:sha256:${'b'.repeat(64)}`, reportSource: null, sourceDocumentId: `m_${'b'.repeat(64)}` };
    ...
    test('does not project runbooks to non-owner roles', async () => {
        const ledger = new ledger_1.AgentManagerLedger(new MemoryRepository());
        const admin = { uid: 'admin-uid', token: { admin: true, adminRole: 'admin' } };
    ...
    test('atomically consumes a verified Telegram approval and queues only the bound task revision', async () => {
        ...
        const tokenIdHash = (0, telegram_contracts_1.managerTelegramTokenHash)(nonce);
        repo.documents.set(`agent_manager_telegram_tokens/${tokenIdHash}`, { id: tokenIdHash, data: {
                ...
                issuedAtMs: 1999999999000, validUntilMs: 2000000000500, consumedAtMs: null, consumedDecisionId: null, consumedUpdateIdHash: null,
            } });
        const result = await ledger.decideTelegramTask(OWNER, { token: repo.documents.get(`agent_manager_telegram_tokens/${tokenIdHash}`).data, decision: 'approve', updateIdHash: 'e'.repeat(64) });
        ...
        await expect(ledger.decideTelegramTask(OWNER, { token: consumedToken, decision: 'approve', updateIdHash: 'e'.repeat(64) }))
            .resolves.toMatchObject({ ok: true, idempotent: true, decision: 'approve', item: { status: 'queued', revision: 3 } });
        await expect(ledger.decideTelegramTask(OWNER, { token: consumedToken, decision: 'approve', updateIdHash: 'f'.repeat(64) }))
            .rejects.toMatchObject({ code: 'failed-precondition' });
    });
    test('Telegram rejection atomically cancels only the token-bound task', async () => {
        ...
        await ledger.createTask(OWNER, createInput());
        await ledger.transitionTask(OWNER, { taskId: 'task-001', expectedRevision: 1, status: 'awaiting_approval' });
        const tokenIdHash = (0, telegram_contracts_1.managerTelegramTokenHash)('r'.repeat(32));
        repo.documents.set(`agent_manager_telegram_tokens/${tokenIdHash}`, { id: tokenIdHash, data: {
                schemaVersion: 1, tokenIdHash, status: 'active', ownerUid: 'owner-uid', telegramChatId: '70000001', telegramUserId: '70000001',
                taskId: 'task-001', expectedRevision: 2, permittedDecision: 'reject', projectionHash: 'f'.repeat(64),
                issuedAtMs: 1999999999000, validUntilMs: 2000000000500, consumedAtMs: null, consumedDecisionId: null, consumedUpdateIdHash: null,
            } });
        const result = await ledger.decideTelegramTask(OWNER, { token: repo.documents.get(`agent_manager_telegram_tokens/${tokenIdHash}`).data, decision: 'reject', updateIdHash: '1'.repeat(64) });
    ...
    test('rechecks global control inside the Telegram approval transaction before queueing', async () => {
        ...
        await ledger.createTask(OWNER, createInput({ allowedScope: 'code_prepare' }));
        await ledger.transitionTask(OWNER, { taskId: 'task-001', expectedRevision: 1, status: 'awaiting_approval' });
        const tokenIdHash = (0, telegram_contracts_1.managerTelegramTokenHash)('g'.repeat(32));
        const token = {
            schemaVersion: 1, tokenIdHash, status: 'active', ownerUid: 'owner-uid', telegramChatId: '70000001', telegramUserId: '70000001',
        ...
        await expect(ledger.decideTelegramTask(OWNER, { token: token, decision: 'approve', updateIdHash: '8'.repeat(64) }))
            .rejects.toMatchObject({ code: 'failed-precondition' });
        expect(repo.documents.get('agent_manager_tasks/task-001')?.data).toMatchObject({ status: 'awaiting_approval', revision: 2 });
    ...
    test('projects only a bounded execution preparation status for the task queue', async () => {
        ...
        repo.documents.set('agent_manager_execution_jobs/task-001__r3', { id: 'task-001__r3', data: {
                ...
                createdAtMs: 2000000000000, updatedAtMs: 2000000000500, leasedAtMs: 2000000000100,
                finishedAtMs: 2000000000500, outputRef: `execution_output:${'c'.repeat(64)}`, outputHash: 'c'.repeat(64),
            } });
        const result = await ledger.listTasks(OWNER, { limit: 10 });
        expect(result.items[0]).toMatchObject({ taskId: 'task-001', execution: {
                state: 'succeeded', attempts: 1, maxAttempts: 2, updatedAtMs: 2000000000500, finishedAtMs: 2000000000500,
            } });
    ...
    test('retains a cancelled execution status after a queued task advances one manual revision', async () => {
        ...
        repo.documents.set('agent_manager_execution_jobs/task-001__r3', { id: 'task-001__r3', data: {
                ...
                createdAtMs: 2000000000000, updatedAtMs: 2000000000100, leasedAtMs: 2000000000010,
                finishedAtMs: 2000000000100, outputRef: null, outputHash: null,
            } });
        const result = await ledger.listTasks(OWNER, { limit: 10 });
        expect(result.items[0].execution).toEqual({ state: 'cancelled', attempts: 1, maxAttempts: 2, updatedAtMs: 2000000000100, finishedAtMs: 2000000000100 });
    });
});
