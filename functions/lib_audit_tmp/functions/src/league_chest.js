...
const AVATAR_AURA_IDS = ['aura-aurora', 'aura-ember', 'aura-mint', 'aura-violet', 'aura-coral'];
const CUSTOM_AVATAR_DROP_IDS = Array.from({ length: 30 }, (_, index) => `custom-gen-${String(index + 1).padStart(2, '0')}`);
const CUSTOM_AVATAR_GRADIENT_IDS = ['aurora', 'ember', 'cosmic', 'forest', 'citrine', 'royal', 'ruby', 'magma', 'noirgold', 'sakura'];
...
function rollChance(seed, chance) {
    return rollUnit(seed) < chance;
}
function rollInt(seed, min, max) {
...
function sumShardDrops(drops) {
    return drops.reduce((sum, drop) => (drop.kind === 'shards' || drop.kind === 'gold_theme_duplicate'
        ? sum + Math.max(0, readInt(drop.amount, 0))
        : sum), 0);
}
function isLeagueChestAuraId(value) {
    return typeof value === 'string' && AVATAR_AURA_IDS.includes(value);
}
...
function buildLeagueRewardDrops(params) {
    ...
    const addIf = (id, chance, drop) => {
        if (rollChance(`${seed}:${id}`, chance))
            drops.push(drop);
    };
    addIf('xp_boost', 0.45, {
        id: 'league_xp_2x_3',
...
function buildRewardProgressPatch(params) {
    ...
    if (energyBoost) {
        progressPatch[ENERGY_OVERRIDE_KEY] = JSON.stringify({
            recoveryMs: Math.max(60000, readInt(energyBoost.recoveryMs, ENERGY_MS)),
            expiresAt,
        });
    }
...
exports.leagueChestClaim = (0, https_1.onCall)({ region: REGION, enforceAppCheck: callable_options_1.ENFORCE_APP_CHECK }, async (request) => {
    ...
    return db.runTransaction(async (tx) => {
        ...
        tx.set(claimRef, {
            ...
            rewards: rewardDrops,
            shards: shardReward,
            energyRecoveryMs: energyBoost ? Math.max(60000, readInt(energyBoost.recoveryMs, ENERGY_MS)) : 0,
        ...
        return {
            ...
            rewards: {
                drops: rewardDrops,
                shards: shardReward,
                energyRecoveryMs: energyBoost ? Math.max(60000, readInt(energyBoost.recoveryMs, ENERGY_MS)) : undefined,
