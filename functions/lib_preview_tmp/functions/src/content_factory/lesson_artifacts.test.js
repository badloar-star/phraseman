...
describe('lesson stage artifact contracts', () => {
    ...
    it('accepts a grounded outline with explicit coverage and exclusions', () => {
        expect((0, lesson_artifacts_1.validateLessonStageArtifact)({
    ...
    it('requires exactly 50 unique, directionally distinct phrases', () => {
        ...
        expect((0, lesson_artifacts_1.validateLessonStageArtifact)({ stage: 'lesson_phrases', items }, { kind: 'lesson_phrases', count: 50, cefr: 'A2' })).toEqual([]);
        items[49] = { ...items[0], id: 'phrase-50' };
        expect((0, lesson_artifacts_1.validateLessonStageArtifact)({ stage: 'lesson_phrases', items }, { kind: 'lesson_phrases', count: 50, cefr: 'A2' })).toEqual(expect.arrayContaining(['lesson_phrase_meaning_duplicate']));
    });
    it('rejects an outline that replaces the server-selected blueprint topic or invents its focus', () => {
        ...
        expect((0, lesson_artifacts_1.validateLessonStageArtifact)(artifact, { kind: 'lesson_outline', count: 1, cefr: 'A2', grounding })).toEqual(expect.arrayContaining(['lesson_outline_blueprint_identity_mismatch', 'lesson_outline_blueprint_focus_unapproved']));
    });
    it('rejects wrong CEFR, empty coverage and source copied into target', () => {
        ...
        expect((0, lesson_artifacts_1.validateLessonStageArtifact)({ stage: 'lesson_phrases', items }, { kind: 'lesson_phrases', count: 50, cefr: 'A2' })).toEqual(expect.arrayContaining([
    ...
    it('requires v3 phrase coverage and exclusion receipts to match the approved outline', () => {
        ...
        expect((0, lesson_artifacts_1.validateLessonStageArtifact)({ stage: 'lesson_phrases', items, coverageReceipt: { coveredTags: ['request', 'response'], respectedExclusions: ['advanced geography'] } }, { kind: 'lesson_phrases', count: 50, cefr: 'A2', grounding, strictV3: true })).toEqual([]);
        items[0] = { ...items[0], coverageTag: 'unapproved-topic', targetText: 'Advanced geography is required.' };
        expect((0, lesson_artifacts_1.validateLessonStageArtifact)({ stage: 'lesson_phrases', items, coverageReceipt: { coveredTags: ['request'], respectedExclusions: [] } }, { kind: 'lesson_phrases', count: 50, cefr: 'A2', grounding, strictV3: true })).toEqual(expect.arrayContaining(['lesson_phrase_outline_coverage_mismatch', 'lesson_phrase_outline_receipt_mismatch', 'lesson_phrase_outline_exclusion_violated']));
    });
});
//# sourceMappingURL=lesson_artifacts.test.js.map