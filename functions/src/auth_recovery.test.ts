...
async function callRequest(data: DocData, authUid = 'fresh-provider-uid', session: SessionOptions = {}) {
...
describe('authConfirmRecoveryCode', () => {
  ...
  it('[unit-only synthetic concurrency] allows at most one success when a correct code is replayed', async () => {
    ...
    expect(rejected.every((result) => (
      String((result as PromiseRejectedResult).reason?.message ?? '') === 'recovery_code_missing'
    ))).toBe(true);
    expect(docs.get(`auth_recovery_codes/${STABLE}`)).toMatchObject({
      status: 'consumed',
