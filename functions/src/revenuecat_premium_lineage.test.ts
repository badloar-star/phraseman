...
const base = {
  ...
  expiration_at_ms: 4_000_000,
};

function normalized(overrides: Record<string, unknown> = {}) {
  const result = normalizePremiumLineageEvent({ ...base, ...overrides });
  if (result.status !== 'ok') throw new Error(`unexpected quarantine: ${result.reason}`);
  return result.event;
}

function apply(current: PremiumLineageState | null, overrides: Record<string, unknown> = {}) {
  return applyPremiumLineageEvent(current, normalized(overrides));
}
...
describe('RevenueCat canonical premium lineage reducer', () => {
  it('is order-safe: old expiration cannot revoke newer renewal', () => {
    const renewal = apply(null, {
      id: 'evt-renew', type: 'RENEWAL', event_timestamp_ms: 3_000_000, expiration_at_ms: 5_000_000,
    });
    expect(renewal.status).toBe('applied');
    const expiration = apply(renewal.state, {
      id: 'evt-expire-old', type: 'EXPIRATION', event_timestamp_ms: 2_000_000, expiration_at_ms: 2_000_000,
    });
    expect(expiration).toMatchObject({ status: 'stale', state: { revoked: false, activeThroughMs: 5_000_000 } });
  });

  it('is order-safe: old renewal cannot resurrect a newer refund', () => {
    const active = apply(null, { id: 'evt-active', type: 'RENEWAL', event_timestamp_ms: 2_000_000 });
    const refunded = apply(active.state, { id: 'evt-refund', type: 'REFUND', event_timestamp_ms: 4_000_000 });
    const oldRenewal = apply(refunded.state, { id: 'evt-old', type: 'RENEWAL', event_timestamp_ms: 3_000_000 });
  ...
  it('uses semantic rank at the same millisecond so a refund wins regardless of opaque ids or delivery order', () => {
    const renewal = normalized({ id: 'zzz-renewal', type: 'RENEWAL', event_timestamp_ms: 3_000_000 });
    const refund = normalized({ id: 'aaa-refund', type: 'REFUND', event_timestamp_ms: 3_000_000 });
    const renewalThenRefund = applyPremiumLineageEvent(applyPremiumLineageEvent(null, renewal).state, refund).state;
    const refundThenRenewal = applyPremiumLineageEvent(applyPremiumLineageEvent(null, refund).state, renewal).state;
  ...
  it('keeps same-ms terminal transitions monotonic for lifetime and recurring lineages', () => {
    for (const product of ['premium_lifetime', 'premium_yearly']) {
      const purchase = apply(null, {
        ...
        product_id: product, event_timestamp_ms: 2_000_000,
        expiration_at_ms: product.includes('lifetime') ? undefined : 9_000_000,
      }).state;
      const terminalEvents = [
        normalized({ id: 'zzz-expiration', type: 'EXPIRATION', product_id: product, event_timestamp_ms: 3_000_000 }),
        normalized({ id: 'aaa-support', type: 'CANCELLATION', cancel_reason: 'CUSTOMER_SUPPORT', product_id: product, event_timestamp_ms: 3_000_000 }),
        normalized({ id: '000-refund', type: 'REFUND', product_id: product, event_timestamp_ms: 3_000_000 }),
      ];
      for (const order of [terminalEvents, [...terminalEvents].reverse()]) {
        const state = order.reduce((current, event) => applyPremiumLineageEvent(current, event).state, purchase);
  ...
  it('accepts exact event replay and rejects same-id different fingerprint without mutation', () => {
    ...
    expect(receiptReplayDecision({ eventId: event.eventId, fingerprint: event.fingerprint }, event))
      .toBe('duplicate');
    const conflict = normalized({ product_id: 'phraseman_premium_monthly' });
    expect(receiptReplayDecision({ eventId: event.eventId, fingerprint: event.fingerprint }, conflict))
      .toBe('conflict');
  });

  it('derives a canonical bounded lineage and quarantines missing durable identity/time', () => {
    const a = canonicalPremiumLineageHash(base);
    const b = canonicalPremiumLineageHash({
      ...base,
    ...
    for (const field of ['id', 'app_id', 'environment', 'store', 'original_transaction_id', 'event_timestamp_ms']) {
      expect(normalizePremiumLineageEvent({ ...base, [field]: undefined })).toMatchObject({ status: 'quarantine' });
    }
    expect(normalizePremiumLineageEvent({ ...base, id: 'x'.repeat(257) })).toMatchObject({ status: 'quarantine' });
  });

  it('treats PRODUCT_CHANGE as metadata-only and never as a grant', () => {
    const productChange = apply(null, {
      id: 'evt-product', type: 'PRODUCT_CHANGE', event_timestamp_ms: 3_000_000,
      product_id: 'phraseman_premium_monthly', expiration_at_ms: 9_000_000,
    });
    ...
    expect(aggregatePremiumLineages([productChange.state], legacy, NOW)).toMatchObject({
      status: 'ambiguous',
  ...
  it('keeps metadata and access ordering independent when PRODUCT_CHANGE arrives before an older activation', () => {
    const activation = normalized({ id: 'evt-renew-t2', type: 'RENEWAL', event_timestamp_ms: 2_000_000 });
    const metadata = normalized({
      id: 'evt-product-t3', type: 'PRODUCT_CHANGE', event_timestamp_ms: 3_000_000,
      product_id: 'phraseman_premium_monthly', expiration_at_ms: 9_000_000,
    });
    const activationThenMetadata = applyPremiumLineageEvent(applyPremiumLineageEvent(null, activation).state, metadata).state;
    const metadataThenActivation = applyPremiumLineageEvent(applyPremiumLineageEvent(null, metadata).state, activation).state;
  ...
  it.each([
    ...
    const activation = normalized({ id: 'activation-t2', type: 'RENEWAL', event_timestamp_ms: 2_000_000 });
    const metadata = normalized({ id: `metadata-${type}`, type, event_timestamp_ms: 3_000_000, ...extra });
    const first = applyPremiumLineageEvent(applyPremiumLineageEvent(null, activation).state, metadata).state;
    const second = applyPremiumLineageEvent(applyPremiumLineageEvent(null, metadata).state, activation).state;
    expect(first).toMatchObject({ revoked: false, activeThroughMs: 4_000_000 });
    expect(second).toMatchObject({ revoked: false, activeThroughMs: 4_000_000 });
  });

  it('converges same-ms future support cancellation and recurring expiration to revoked', () => {
    const purchase = apply(null, { id: 'purchase', type: 'RENEWAL', event_timestamp_ms: 2_000_000 }).state;
    const support = normalized({
      id: 'zzz-support', type: 'CANCELLATION', cancel_reason: 'CUSTOMER_SUPPORT',
      event_timestamp_ms: 3_000_000, expiration_at_ms: 3_500_000,
    });
    const expiration = normalized({
      id: 'aaa-expiration', type: 'EXPIRATION', event_timestamp_ms: 3_000_000, expiration_at_ms: 3_000_000,
    });
  ...
  it('chooses the tighter same-ms support expiry independent of opaque ids and order', () => {
    ...
    const loose = normalized({
      ...
      event_timestamp_ms: 3_000_000, expiration_at_ms: 3_900_000,
    });
    ...
    for (const order of [[loose, tight], [tight, loose]]) {
      const state = order.reduce((current, event) => applyPremiumLineageEvent(current, event).state, purchase);
      expect(state).toMatchObject({ revoked: false, activeThroughMs: 3_500_000 });
    }
  });
  ...
  it('does not let an older future-support bound shorten a newer renewal regardless of delivery order', () => {
    const support = normalized({
      ...
      event_timestamp_ms: 2_000_000, expiration_at_ms: 4_000_000,
    });
    const renewal = normalized({
      id: 'renewal-t3', type: 'RENEWAL', event_timestamp_ms: 3_000_000, expiration_at_ms: 5_000_000,
    });
    for (const order of [[support, renewal], [renewal, support]]) {
      const state = order.reduce<PremiumLineageState | null>(
        (current, event) => applyPremiumLineageEvent(current, event).state,
        null,
      );
      expect(state).toMatchObject({ revoked: false, activeThroughMs: 5_000_000 });
    }
  });
  ...
  it('never lets lifetime EXPIRATION grant or resurrect access', () => {
    const expiration = normalized({
      ...
      event_timestamp_ms: 4_000_000, expiration_at_ms: undefined,
    });
    expect(applyPremiumLineageEvent(null, expiration).state).toMatchObject({ revoked: true });
    const purchase = apply(null, {
      id: 'life-buy', type: 'NON_RENEWING_PURCHASE', product_id: 'premium_lifetime',
      event_timestamp_ms: 2_000_000, expiration_at_ms: undefined,
    }).state;
    const refund = apply(purchase, {
      id: 'life-refund', type: 'REFUND', product_id: 'premium_lifetime',
      event_timestamp_ms: 3_000_000, expiration_at_ms: undefined,
    }).state;
    expect(applyPremiumLineageEvent(refund, expiration).state).toMatchObject({ revoked: true });
  });

  it('applies cancellation reason only to the exact lineage', () => {
    const active = apply(null, { id: 'evt-active', type: 'RENEWAL', event_timestamp_ms: 2_000_000 });
    const unsubscribe = apply(active.state, {
      id: 'evt-unsub', type: 'CANCELLATION', cancel_reason: 'UNSUBSCRIBE', event_timestamp_ms: 2_500_000,
    });
    expect(unsubscribe.state).toMatchObject({ revoked: false, activeThroughMs: 4_000_000 });

    const support = apply(unsubscribe.state, {
      id: 'evt-support', type: 'CANCELLATION', cancel_reason: 'CUSTOMER_SUPPORT', event_timestamp_ms: 3_000_000,
      expiration_at_ms: 3_500_000,
    });
    expect(support.state).toMatchObject({ revoked: false, activeThroughMs: 3_500_000 });
    ...
    const lifetimeSupport = apply(lifetime.state, {
      ...
      cancel_reason: 'CUSTOMER_SUPPORT', event_timestamp_ms: 3_000_000, expiration_at_ms: undefined,
    });
  ...
  it('restores only the exact lineage on authoritative REFUND_REVERSED evidence and fences old reversal', () => {
    const purchase = apply(null, { id: 'buy', type: 'RENEWAL', event_timestamp_ms: 2_000_000 }).state;
    const refund = apply(purchase, { id: 'refund', type: 'REFUND', event_timestamp_ms: 3_000_000 }).state;
    const restored = apply(refund, {
      id: 'reversed', type: 'REFUND_REVERSED', event_timestamp_ms: 4_000_000, expiration_at_ms: 9_000_000,
    });
    expect(restored.state).toMatchObject({ revoked: false, activeThroughMs: 9_000_000 });
    const oldReversal = apply(refund, {
      id: 'old-reversed', type: 'REFUND_REVERSED', event_timestamp_ms: 2_500_000, expiration_at_ms: 9_000_000,
    });
  ...
  it.each([
    ...
    const purchase = apply(null, { id: 'same-ms-buy', type: 'RENEWAL', event_timestamp_ms: 2_000_000 }).state;
    const terminal = normalized({
      id: `same-ms-${terminalType}`, type: terminalType, event_timestamp_ms: 3_000_000,
      expiration_at_ms: terminalType === 'CANCELLATION' ? 3_000_000 : undefined,
      ...extra,
    });
    const reversal = normalized({
      id: 'same-ms-reversal', type: 'REFUND_REVERSED', event_timestamp_ms: 3_000_000,
      expiration_at_ms: 9_000_000,
    });
    for (const order of [[terminal, reversal], [reversal, terminal]]) {
      const state = order.reduce((current, event) => applyPremiumLineageEvent(current, event).state, purchase);
      expect(state).toMatchObject({ revoked: false, activeThroughMs: 9_000_000, lastEventType: 'REFUND_REVERSED' });
    }
  });
  ...
  it('refunds only one lineage and aggregates the other active lineage', () => {
    ...
    const second = apply(null, { id: 'evt-b', original_transaction_id: 'original-b', expiration_at_ms: 6_000_000 }).state;
    const firstRefunded = apply(first, {
      id: 'evt-a-refund', type: 'REFUND', original_transaction_id: 'original-a', event_timestamp_ms: 3_000_000,
    }).state;
    const aggregate = aggregatePremiumLineages([firstRefunded, second], {}, NOW);
  ...
  it.each([
    ...
    const purchase = apply(null, {
      ...
      product_id: productId, event_timestamp_ms: 2_000_000,
      expiration_at_ms: plan === 'lifetime' ? undefined : 9_000_000,
    }).state;
    const granted = aggregatePremiumLineages([purchase], {}, 3_000_000);
    expect(granted.progressPatch).toMatchObject({ premium_plan: plan, premium_rc_store: 'APP_STORE' });
    const terminal = apply(purchase, {
      id: `terminal-${plan}`, type: terminalType, product_id: productId, event_timestamp_ms: 4_000_000,
      expiration_at_ms: terminalType === 'EXPIRATION' ? 4_000_000 : undefined,
    }).state;
    const cleared = aggregatePremiumLineages([terminal], granted.progressPatch, 4_000_000);
  ...
  it('uses deterministic lifetime-first aggregate and preserves non-RC access fields', () => {
    const yearly = apply(null, { id: 'evt-year', expiration_at_ms: 9_000_000 }).state;
    const lifetime = apply(null, {
    ...
    const aggregate = aggregatePremiumLineages([yearly, lifetime], existing, NOW);
  ...
  it.each(['9000000', '0'])('migrates finite/lifetime legacy admin authority to VIP while projecting RC (expiry=%s)', (adminExpiry) => {
    ...
    const projected = aggregatePremiumLineages([rc], admin, NOW).progressPatch;
    ...
    const refunded = apply(rc, { id: 'refund-admin', type: 'REFUND', event_timestamp_ms: 3_000_000 }).state;
    expect(aggregatePremiumLineages([refunded], projected, NOW).progressPatch).toMatchObject({
  ...
  it('projects active RC when a legacy admin marker is already expired', () => {
    const rc = apply(null, { expiration_at_ms: 6_000_000 }).state;
    const projected = aggregatePremiumLineages([rc], {
    ...
    expect(isPremiumAccessActive(projected, NOW + 1)).toBe(true);
  });

  it('keeps canonical RC access after a migrated finite admin grant expires', () => {
    const rc = apply(null, { expiration_at_ms: 8_000_000 }).state;
    const projected = aggregatePremiumLineages([rc], {
      premium_plan: 'admin_grant', premium_expiry: String(NOW + 1_000), admin_premium_override: 'true',
      premium_admin_grant_at: '123456',
    }, NOW).progressPatch;

    expect(projected).toMatchObject({
      premium_plan: 'yearly', premium_expiry: '0', admin_premium_override: 'false',
      vip_active: 'true', vip_until: String(NOW + 1_000), premium_rc_expiry_ms: '8000000',
    });
    expect(isPremiumAccessActive(projected, NOW + 2_000)).toBe(true);
  });

  it('treats an expired Timestamp-like admin window as expired instead of lifetime', () => {
    const projected = aggregatePremiumLineages([apply(null).state], {
  ...
  it('preserves a stronger Timestamp-like VIP window over a finite admin grant', () => {
    const vipUntil = NOW + 10_000;
    const projected = aggregatePremiumLineages([apply(null).state], {
      premium_plan: 'admin_grant', premium_expiry: { seconds: (NOW + 1_000) / 1000 },
  ...
  it.each([
    ...
    const refunded = apply(rc, { id: `refund-${adminExpiry}`, type: 'REFUND', event_timestamp_ms: 3_000_000 }).state;
    const afterRefund = aggregatePremiumLineages([refunded], projected, 4_000_000).progressPatch;
    expect(isPremiumAccessActive(afterRefund, 4_000_000)).toBe(true);
  });
  ...
  it.each([
    [String(NOW - 1), 'false'],
    [String(NOW + 1_000), 'true'],
  ])('retires override-only legacy authority and projects RC (admin expiry=%s)', (adminExpiry, expectedVipActive) => {
    const projected = aggregatePremiumLineages([apply(null, { expiration_at_ms: 8_000_000 }).state], {
  ...
  it('uses yearly before monthly when activeThrough is equal, then lineage hash only as the final tie-break', () => {
    ...
    expect(aggregatePremiumLineages([monthly, yearly], {}, NOW).winnerLineageHash).toBe(yearly.lineageHash);
    expect(aggregatePremiumLineages([yearly, monthly], {}, NOW).winnerLineageHash).toBe(yearly.lineageHash);
  });

  it('quarantines a retired MAX receipt instead of projecting Premium', () => {
    expect(normalizePremiumLineageEvent({ ...base,
      id: 'evt-max', original_transaction_id: 'original-max',
      product_id: 'phraseman_max_monthly_v1', expiration_at_ms: 9_000_000,
    })).toMatchObject({ status: 'quarantine', reason: 'unmanaged_premium_product' });
  });
  ...
  it('fails closed on owner-candidate overflow instead of silently truncating identities', () => {
    ...
    expect(boundPremiumOwnerCandidates(Array.from({ length: 17 }, (_, index) => `owner-${index}`)))
  ...
  it('bounds aggregate input and only clears a strict legacy RC sentinel', () => {
    ...
    expect(aggregatePremiumLineages(Array.from({ length: 65 }, () => active), {}, NOW))
      .toMatchObject({ status: 'ambiguous' });
    const ambiguous = aggregatePremiumLineages([], { premium_plan: 'yearly' }, NOW);
    expect(ambiguous).toMatchObject({ status: 'ambiguous', progressPatch: { premium_plan: 'yearly', premium_rc_reconcile_needed: 'true' } });
    const strict = aggregatePremiumLineages([], {
      premium_plan: 'yearly',
  ...
  it('preserves the existing 72-hour RC expiry grace seam during billing issue metadata', () => {
    const active = apply(null, {
      id: 'renew', type: 'RENEWAL', event_timestamp_ms: 2_000_000, expiration_at_ms: 4_000_000,
    }).state;
    const billing = apply(active, {
      id: 'billing', type: 'BILLING_ISSUE', event_timestamp_ms: 4_500_000, expiration_at_ms: 4_000_000,
    }).state;
    const withinGrace = aggregatePremiumLineages([billing], {}, 4_000_000 + (71 * 60 * 60 * 1000));
