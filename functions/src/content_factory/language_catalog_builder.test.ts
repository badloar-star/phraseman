...
const release = {
  ...
  artifacts: Object.fromEntries(['lesson', 'quiz', 'flashcard', 'arena'].map((surface) => [surface, { releaseId: 'fr-ru-r1', studyTarget: 'fr', learnerSourceLocale: 'ru', surface, contentHash: hash, objectGeneration: 'g1', byteSize: 100, entryIndex: `${surface}/index.json` }])) as any,
};

describe('active language catalog builder', () => {
  it('expands one active canonical release into four exact target/source surfaces', () => {
    ...
    expect(entries.map((entry) => entry.surface).sort()).toEqual(['arena', 'flashcard', 'lesson', 'quiz']);
