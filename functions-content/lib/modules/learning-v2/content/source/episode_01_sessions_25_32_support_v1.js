...
exports.buildEpisode01Session25To32 = buildEpisode01Session25To32;
exports.assertEpisode01Sessions25To32Contract = assertEpisode01Sessions25To32Contract;
const LOCALES = ["ru", "uk", "es", "pt-BR", "vi", "id", "tr", "pl"];
const L = (select) => Object.fromEntries(LOCALES.map((locale) => [locale, select(locale)]));
const PHRASES = {
    25: [
        "We are ready.",
        "We are here.",
...
const normalizedTokens = (english) => english
    .replace(/[?.!]/gu, "")
    ...
    .split(/\s+/u)
    .filter(Boolean);
const surfaceTokens = (english) => english.replace(/[?.!]/gu, "").split(/\s+/u).filter(Boolean);
const tokenise = normalizedTokens;
const tokenCategory = (token) => /^(I|You|He|She|It|We|They)$/u.test(token)
    ? "pronoun"
        ...
        : token === "not"
            ...
            : /^(a|the)$/u.test(token)
                ...
                : /s$/u.test(token)
                    ? "plural"
                    : "meaning";
function realizeSemanticClass(locale, subject, predicate, negative, question) {
    const objectForms = {
    ...
    const roles = ["friends", "teachers", "students"];
    if ((subject === "We" || subject === "They") &&
        roles.includes(predicate)) {
        const role = {
...
function realizeEuropean(locale, subject, predicate, negative, question) {
    const gender = subject === "She"
        ? "f"
        : subject === "It"
            ? "n"
    ...
    if (!["ru", "uk", "es", "pt-BR", "pl"].includes(locale))
        return undefined;
    const localized = adjective[locale][predicate]?.[gender] ?? WORD[locale][predicate];
    const names = {
        ru: {
            I: "Я",
            You: "Ты",
    ...
    if (locale === "ru" || locale === "uk") {
        const text = `${names[locale][subject]} ${negative ? "не " : ""}${localized}`.trim();
...
function meaning(locale, english) {
    ...
    const predicate = tokens
        ...
        .join(" ");
    const semanticClass = realizeSemanticClass(locale, subject, predicate, negative, question);
    if (semanticClass)
        return semanticClass;
    const european = realizeEuropean(locale, subject, predicate, negative, question);
    if (european)
        return european;
    if (locale === "vi" || locale === "id") {
        ...
        const weather = subject === "It" && (predicate === "cold" || predicate === "warm");
    ...
    if (locale === "tr") {
        ...
        const lastVowel = [...stem].reverse().find((letter) => /[aeıioöuü]/u.test(letter)) ?? "ı";
        ...
        const plural = stem === "meşgul" ? "ler" : /[aıou]/u.test(lastVowel) ? "lar" : "ler";
        const vowelEnding = /[aeıioöuü]$/u.test(stem);
        const suffixedStem = (ending, buffer = false) => `${stem === "sıcak" && /^[ıiuü]/u.test(ending) ? "sıcağ" : stem}${buffer && vowelEnding ? "y" : ""}${ending}`;
        const positive = {
        ...
        const questionForm = {
            ...
            They: (() => {
                ...
                const finalVowel = [...pluralPredicate]
                    .reverse()
    ...
    const base = `${SUBJECT[locale][subject]} ${negative ? negativeWord(locale) : ""}${WORD[locale][predicate] ?? predicate}`.trim();
    return question ? questionPrefix(locale, base) : base;
}
function negativeWord(locale) {
    return {
        ru: "не ",
        uk: "не ",
        es: "no ",
...
function questionPrefix(locale, text) {
    return {
...
function quizPrompt(locale, native) {
    return {
...
function tokenGloss(locale, token) {
    ...
    if (lexical)
        return lexical;
    const pronoun = SUBJECT[locale][token.replace(/^./u, (letter) => letter.toUpperCase())];
    return shared[locale][lower] ?? pronoun ?? token;
}
function reason(locale, correct, alternative, phraseMeaning) {
...
function explanation(locale, english, native) {
    const question = english.endsWith("?");
    ...
    const copy = {
        ru: {
            statement: "Сначала назван участник, затем форма be связывает его с состоянием или местом; порядок нельзя переставлять.",
            negative: "Отрицание стоит после формы be и отменяет признак, не меняя самого участника.",
            question: "Вопрос начинается с формы are, а затем называет участника; такой порядок сразу делает фразу вопросом.",
            contraction: "Короткая форма сохраняет ту же связку are, но произносится одним блоком с местоимением.",
            combined: "isn’t или aren’t уже объединяет is/are и not в одной короткой форме; без этой части смысл был бы противоположным.",
        },
        uk: {
            statement: "Спершу названо учасника, потім форма be поєднує його зі станом або місцем; порядок не можна міняти.",
            negative: "Заперечення стоїть після форми be та скасовує ознаку, не міняючи учасника.",
            question: "Запитання починається з are, а далі називає учасника; такий порядок створює запитання.",
            contraction: "Коротка форма зберігає are, але вимовляється одним блоком із займенником.",
            combined: "isn’t або aren’t уже поєднує is/are і not в одній короткій формі; без неї зміст був би протилежним.",
        },
        es: {
            statement: "Primero aparece el participante y be lo une con estado o lugar; el orden no se intercambia.",
            negative: "La negación va después de be y cancela la cualidad sin cambiar al participante.",
            question: "La pregunta empieza con are y después nombra al participante; ese orden crea la pregunta.",
            contraction: "La forma breve conserva are, pero lo pronuncia unido al pronombre.",
            combined: "isn’t o aren’t ya une is/are y not en una sola forma breve; sin esa parte el sentido sería el contrario.",
        },
        "pt-BR": {
            statement: "Primeiro vem o participante e be o liga ao estado ou lugar; essa ordem não se troca.",
            negative: "A negação fica depois de be e cancela a característica sem mudar o participante.",
            question: "A pergunta começa com are e depois nomeia o participante; essa ordem cria a pergunta.",
            contraction: "A forma curta conserva are, mas o pronuncia unido ao pronome.",
            combined: "isn’t ou aren’t já une is/are e not numa única forma curta; sem essa parte, o sentido seria o oposto.",
        },
        vi: {
            statement: "Chủ thể xuất hiện trước, rồi be nối với trạng thái hoặc nơi chốn; không đổi trật tự này.",
            negative: "Phủ định đứng sau be và phủ nhận đặc điểm mà không đổi chủ thể.",
            question: "Câu hỏi mở đầu bằng are rồi mới nêu chủ thể; trật tự đó tạo câu hỏi.",
            contraction: "Dạng ngắn vẫn giữ are nhưng gắn với đại từ thành một cụm.",
            combined: "isn’t hoặc aren’t đã gộp is/are và not vào một dạng ngắn; bỏ phần đó sẽ đổi nghĩa.",
        },
        id: {
            statement: "Pelaku muncul lebih dahulu, lalu be menghubungkannya dengan keadaan atau tempat; urutan ini tidak ditukar.",
            negative: "Penyangkalan berada setelah be dan meniadakan sifat tanpa mengganti pelaku.",
            question: "Pertanyaan dimulai dengan are lalu menyebut pelaku; urutan itu membuat pertanyaan.",
            contraction: "Bentuk pendek tetap menyimpan are, tetapi menyatu dengan kata ganti.",
            combined: "isn’t atau aren’t sudah menyatukan is/are dan not dalam satu bentuk singkat; tanpa bagian itu maknanya berlawanan.",
        },
        tr: {
            statement: "Önce kişi gelir, sonra be onu durum veya yere bağlar; bu sıra değiştirilmez.",
            negative: "Olumsuzluk be sonrasında gelir ve kişiyi değiştirmeden özelliği reddeder.",
            question: "Soru are ile başlar, sonra kişiyi söyler; bu sıra soruyu kurar.",
            contraction: "Kısa biçim are anlamını korur ama zamirle tek blok söylenir.",
            combined: "isn’t ya da aren’t, is/are ile not öğelerini tek kısa biçimde birleştirir; bu bölüm olmadan anlam tersine döner.",
        },
        pl: {
            statement: "Najpierw występuje uczestnik, potem be łączy go ze stanem lub miejscem; tego szyku nie zmieniamy.",
            negative: "Przeczenie stoi po be i odwołuje cechę bez zmiany uczestnika.",
            question: "Pytanie zaczyna się od are, a potem podaje uczestnika; ten szyk tworzy pytanie.",
            contraction: "Skrót zachowuje are, ale wymawia je razem z zaimkiem.",
            combined: "isn’t albo aren’t łączy is/are i not w jednej krótkiej formie; bez tej części sens byłby przeciwny.",
        },
    };
    return `${native} ${copy[locale][shape]} ${{ ru: "Каждое слово занимает свою роль в готовой мысли.", uk: "Кожне слово має свою роль у готовій думці.", es: "Cada palabra ocupa su papel en la idea completa.", "pt-BR": "Cada palavra ocupa seu papel na ideia completa.", vi: "Mỗi từ đều có vai trò trong ý trọn vẹn.", id: "Setiap kata memiliki peran dalam gagasan utuh.", tr: "Her sözcüğün tamamlanmış düşüncede bir görevi vardır.", pl: "Każde słowo ma swoją rolę w gotowej myśli." }[locale]}`;
}
function phrase(ordinal, position, english) {
...
function introBody(locale, ordinal, kind, target) {
    ...
    return Object.fromEntries(LOCALES.map((locale) => {
        ...
        return [
            ...
            index < 0
                ...
                : [
                    { text: text.slice(0, index), semantic: "explanation" },
                    { text: target, semantic: "targetCorrect" },
                    {
...
function buildEpisode01Session25To32(ordinal) {
    const target = PHRASES[ordinal][0];
    const title = L((locale) => TITLES[locale][ordinal - 25]);
    const introTargets = PHRASES[ordinal].slice(0, 3);
    const introPages = ["concept", "formula", "trap"].map((kind, index) => {
        const pageTarget = introTargets[index];
        const [firstWrong, secondWrong] = wrongs(pageTarget);
        const pageBody = L((locale) => introBody(locale, ordinal, kind, pageTarget));
        const pageTitle = L((locale) => INTRO_FOCUS[locale][ordinal - 25][index].replace(/[.!?]+$/u, ""));
        return {
            kind,
            title: pageTitle,
            body: pageBody,
            ...
            question: {
                prompt: L((locale) => quizPrompt(locale, meaning(locale, pageTarget))),
                choices: [
                    L(() => pageTarget),
                    L(() => firstWrong),
                    L(() => secondWrong),
    ...
    return {
        ...
        introPages,
        phrases: PHRASES[ordinal].map((english, index) => phrase(ordinal, index, english)),
    };
}
function answerExplanation(locale, english) {
    const native = meaning(locale, english);
    return {
...
function assertEpisode01Sessions25To32Contract(sources) {
    const chapter = sources.filter((source) => source.requiredSessionOrdinal >= 25 &&
...
//# sourceMappingURL=episode_01_sessions_25_32_support_v1.js.map