...
describe('DEV center bottom sheet', () => {
  ...
  test('renders a deterministic typed registry that can grow by category', () => {
    ...
    expect(registry).toContain("id: 'compass'");
    ...
    expect(registry).toContain("action: 'preview-compass-review'");
    expect(registry).toContain("action: 'preview-compass-lesson'");
    expect(registry).toContain("action: 'preview-compass-weak-area'");
    ...
    // зачем 13.08.2026: добавлен раздел «Лига · итоги недели» (order 25) —
    // между компасом и Plus. Реестр расширен осознанно, не ослаблен.
    expect(ordered.map((section: { id: string }) => section.id)).toEqual(['level-previews', 'compass', 'league', 'subscription']);
