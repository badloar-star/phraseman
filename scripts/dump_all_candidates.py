...
out_dir = Path("c:/appsprojects/phraseman/scripts/all_candidates_dump")
...
for fp, edits in sorted(edits_per_file.items()):
    ...
    for i, (old, new, sess) in enumerate(edits):
        if old in content:
            ...
            status = "UNAPPLIED" if new not in content else "NEW_ALSO_PRESENT"
            ...
            print(f"[{idx}] {status}: {fp} (op #{i}, sess={sess})")
