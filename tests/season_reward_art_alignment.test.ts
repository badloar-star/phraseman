...
describe('картинка внутри слота имеет поля', () => {
  ...
  test('слот — согласованное число, а не случайно поехавшее значение', () => {
    // зачем 2026-08-04 (владелец: «иконки увеличить»): было 58, слот вырос
    // осознанно — вместе с ROW_HEIGHT (108→142) и полным пересчётом бюджета
    // высоты карточки, см. tests/season_pass_row_height_budget_contract.test.ts.
    // Guard теперь ловит СЛУЧАЙНОЕ отклонение от текущего согласованного
    // значения, а не запрещает менять его вовсе.
    expect(SEASON_REWARD_ART_SIZE).toBe(72);
  });
});
