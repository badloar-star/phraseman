...
## ⛔ РАЗДЕЛ MAX ЗАКОНСЕРВИРОВАН — НЕ ВОССТАНАВЛИВАТЬ БЕЗ КОМАНДЫ
...
следующих нейронок, что раздел закрыт и его не восстанавливать без специальной
команды».
...
дольше минуты продержались лишь 29. 37 из 63 аккаунтов сожгли пожизненный
пробник впустую. При этом `maxVoiceWatchdog` крутился каждые 10 минут
(~4320 запусков в месяц) плюс оплата OpenAI. Раздел не работал, но стоил денег.

**Что сделано:**
- клиентская пломба `MAX_SECTION_SEALED_BY_OWNER_2026_09_04` в
  `app/max_voice_flags.ts` — проверяется ПЕРВОЙ, выше env и «Пульта»;
- все 13 функций в `functions-max/index.ts` сняты с деплоя (закомментированы);
- плитка ушла с Главной, сетка сама встала на две плитки (Урок и Карточки);
- орбы (375 КБ) больше не считаются при старте Главной.

**Код НЕ удалён намеренно:** 161 файл связан с авторизацией, XP и синхронизацией,
а весит вместе с ассетами ~560 КБ. Вырезать = риск сломать приложение ради
полумегабайта. Ассеты нельзя убрать из бандла, пока жив код: Metro тянет их по
статическим `require`.

**⛔ Запрещено любой сессии (Claude, Codex, любой ИИ):** снимать пломбу, возвращать
экспорты, «чинить» упавший сторож `tests/max_section_sealed_contract.test.ts`.
Ни «починка сломанного импорта», ни «раз уж рядом правлю», ни включённый
`gate_ai_voice_call` в «Пульте» НЕ являются основанием. Раздел включается ТОЛЬКО
по прямой явной команде владельца — например «включи MAX обратно».

**Дев-выдача минут закрыта тем же решением.** Флаг `voiceMinuteDevGrantEnabled`
в `config/max_voice` снят в `false`, функция `voiceMinuteDevGrant` удалена с прода
и её экспорт закомментирован в `functions/src/index.ts`. Пока флаг стоял `true`,
ЛЮБОЙ вошедший мог начислить себе платные минуты бесплатно (до 600 мин на
аккаунт). Не возвращать: возврат экспорта заново создаёт функцию при деплое.

**Как вернуть (когда владелец скажет):** снять пломбу в `max_voice_flags.ts`,
раскомментировать экспорты в `functions-max/index.ts`, задеплоить кодбазу `max`,
удалить сторож. Перед возвратом — починить причину: 59% нулевых звонков.
...
2. **Целевая длина интро — 240–290 знаков** (уточнено 2026-09-04: автор
   стабильно писал 284–300, и каждую сессию приходилось стягивать руками —
   в 260 знаков слово, правило и шутка не помещаются). Прежнее правило
   «220–260 знаков» (владелец, 2026-09-03: «слишком
   длинные, есть риск переутомлять текстами; сократить примерно вдвое, но
   качественно, юмор конечно оставить»). Минимума НЕТ; прежний «минимум 300
   знаков» отменён — он заставлял лить воду. Сокращать надо за счёт разгона и
   повторов, а не за счёт образа и шутки: страница без шутки — дефект, даже
   если она короткая.
...
## ⛔ ФАБРИКА КУРСА: ЧЕМУ НАУЧИЛИ ОШИБКИ (2026-09-05/06)

Правила ниже выведены из реальных провалов на уроке 1. Действуют для любой
сессии, работающей с `content/learning-v2-course`.

### Судьи нестабильны — это измерено, а не предположение

Один и тот же текст, три прогона: `judge_reader` дал REVISE(2/3), REVISE(2/3),
BLOCK(1/3); `judge_nonsense` — REVISE, BLOCK, REVISE. **Из 12 претензий
повторилась 1, девять оказались разовым шумом.** Сессия 45 прошла 12 кругов
правки именно поэтому: чинился шум, а правки под шум рождали новые дефекты.

Решение владельца: **судить дважды, чинить только пересечение**. Реализовано
в `stageJudgeStable()`; второй прогон идёт только при не-PASS, поле
`confirmed_problems` в вердикте — то, что повторилось.

**Вердикт судьи — сигнал посмотреть, а не приговор.** Владелец 06.09
остановил массовую перепроверку словами «может не надо и лучше просто
продолжим писать? мне нравились» — и был прав: из 13 «подтверждённых»
претензий к сессии 4 настоящей оказалась одна.

### Но и «на глаз» отвергать замечание нельзя

В тот же день я дважды отверг ВЕРНОЕ замечание, посмотрев на текст
впечатлением. «Как ночи в новом городе» → `quiet`/`busy`/`tired`: я решил,
что `busy` «про день», а `The nights are busy` — живая фраза.

**Рабочее правило: замечание проверяется ПОДСТАНОВКОЙ, а не ощущением.**
Подставь спорный вариант в задание и прочитай вслух. Оба полюса —
слепо верить судьям и слепо им не верить — уже дали по кругу потерянной работы.

### Проверять надо ТО, ЧТО УВИДИТ ЧЕЛОВЕК, а не исходник

Дважды за сутки отчитался «исправлено», проверив не тот файл:
- правка `final.ru.md` без `build_release.mjs` — владелец видел в макете
  вчерашний текст (`at home` в сессии 51);
- статус сессии писался по ЧЕРНОВИКУ, не по финалу — сессии несли блоки,
  которых уже не было (s49 при пересуде дала PASS). Закрыто «судом по финалу».

После любой ручной правки: пересобрать релиз, пересобрать макет, проверить
**в сборке**. И переводы: ручная правка русского НЕ тянет за собой `final.uk.md`.

### Переход между уроками

`loadPlan` читает секцию `## Урок N`. Нумерация сессий в плане обязана быть
СКВОЗНОЙ по уроку (1..56), не заново в каждой главе. Арка пишется
`**Арка:**`, не `**Арка.**`. Все три поломки найдены при первом же переходе
к уроку 2 — до того путь никто не проходил.

### Урок ≠ сессия

Урок — крупная единица, их 32. Сессия — одна из 56 внутри урока. «42 урока»
звучит как 42/32 всего курса вместо 42/56 первого. Владелец поправил капсом.
...
- интро короче **300 знаков или менее 4 предложений** в любой из 8 локалей —
  ...
  ⚠️ Это описание СТАРОЙ системы (`modules/learning-v2/content/source`), которая
  уходит в архив. Для новой фабрики (`content/learning-v2-course`) действует
  правило выше: 240–290 знаков, минимума нет. При подключении фабрики к
  приложению порог 300 снимается вместе со старым гейтом;
