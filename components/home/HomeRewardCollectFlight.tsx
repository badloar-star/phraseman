...
/**
 * Размер летящей частицы.
 *
 * зачем 40, а не 22 (владелец, 2026-09-01: «ассеты частицы должны быть
 * крупными, а не мелкими»): прежние 22px совпадали с иконкой в счётчике, то
 * есть летящая награда была РОВНО такой же мелкой, как статичный элемент
 * шапки — и терялась на фоне. Летящий объект обязан быть заметно крупнее
 * цели: он главный герой сцены, а счётчик лишь пункт назначения.
 */
  ...
  const style = useAnimatedStyle(() => {
    ...
    return {
      ...
      transform: [
        ...
        // Приближаясь к счётчику, частица уменьшается — это «всасывание» в
        // цель. Но сжатие МЯГКОЕ: 1.1 → 0.5, а не → 0.4, иначе крупный ассет
        // успевает стать мелким уже к середине пути и вся прибавка размера
        // пропадает именно там, где на неё смотрят.
        { scale: 1.1 - 0.6 * p },
      ],
    };
  });
