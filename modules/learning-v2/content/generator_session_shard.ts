...
export type LearningV2SessionCardPurpose =
  (typeof LEARNING_V2_SESSION_CARD_PURPOSES)[number];
...
function assertExpected(
  ...
  if (
    !TOKEN_RE.test(expected.packageId) ||
...
export function learningV2GeneratedMeaningSourceHash(
  input: Readonly<{
    contentItemId: string;
    targetLanguage: string;
    targetText: string;
...
function validateLocalizedCopy(value: unknown, field: string): void {
  ...
  for (const locale of LEARNING_V2_INTERFACE_LOCALES)
    clean(value[locale], `learning_v2_session_shard_${field}_invalid`, 1_000);
}

function validateAudioScript(
  value: unknown,
  card: V2ContentItem,
  family: V2ActivityFamily,
  targetLanguage: string,
  ...
  if (
    input.contentItemId !== card.contentItemId ||
...
export function validateLearningV2GeneratedSessionShardV1(
  ...
  try {
    canonical = canonicalJsonV1(value);
  } catch {
  ...
  if (
    canonical.length > MAX_SESSION_SHARD_BYTES ||
    utf8ByteLengthV1(canonical) > MAX_SESSION_SHARD_BYTES
  ) {
    throw new Error('learning_v2_session_shard_size_invalid');
  }
  let input: Record<string, unknown>;
  try {
    input = JSON.parse(JSON.stringify(value)) as Record<string, unknown>;
  } catch {
    throw new Error('learning_v2_session_shard_json_invalid');
  }
  ...
  exactTuple(
    ...
    LEARNING_V2_INTERFACE_LOCALES,
    'learning_v2_session_shard_locales_invalid',
  );
  exactTuple(
    input.contentKinds,
    LEARNING_V2_REQUIRED_CONTENT_KINDS,
    'learning_v2_session_shard_content_kinds_invalid',
  );
  const intro = validateLearningV2GeneratedSessionIntro(
    input.intro as LearningV2GeneratedSessionIntro,
  );
  ...
  for (let index = 0; index < input.cards.length; index += 1) {
    ...
    if (!validated.ok)
      throw new Error(
        `learning_v2_session_shard_content_item_invalid:${validated.issues.join(',')}`,
      );
    ...
    if (
      ...
      !item.objectiveIds.includes(input.canDoOutcomeId as string) ||
      !item.compatibleFamilies.includes(family) ||
      contentIds.has(item.contentItemId)
    ) {
      throw new Error(
        'learning_v2_session_shard_content_item_identity_invalid',
      );
    }
    const previousContentIds = new Set(
      cards.map((previous) => previous.contentItem.contentItemId),
    );
    if (
      item.prerequisiteContentItemIds.some((id) => !previousContentIds.has(id))
    )
      throw new Error('learning_v2_session_shard_prerequisite_invalid');
    if (
      item.learnerMeanings.length !== LEARNING_V2_INTERFACE_LOCALES.length ||
...
export function learningV2GeneratedSessionShardFingerprint(
  ...
  return hashCanonicalBody(
    validateLearningV2GeneratedSessionShardV1(value, expected),
  );
}
