...
const phrase = (id) => ({ id: `p-${id}`, sourceText: `Source ${id}`, targetText: `Target ${id}` });
const validLesson = {
    lessonId: 1,
...
describe('language factory lesson QA contract', () => {
    it('accepts a complete 50-phrase lesson with applicable drills only', () => {
        expect((0, contracts_1.validateLessonArtifact)(validLesson)).toEqual({ ok: true, errors: [] });
    });
    it('rejects wrong phrase count, duplicates, and empty vocabulary', () => {
        const result = (0, contracts_1.validateLessonArtifact)({
            ...validLesson,
...
//# sourceMappingURL=qa_contract.test.js.map