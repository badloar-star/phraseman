...
import { ENABLE_DEV_TOOLS, ENABLE_DEV_STUDY_TARGET_LANG } from "../config";
import {
...
import {
  peekAppMessagesSnapshot,
  refreshAppMessagesSnapshotOnce,
} from "../app_messages";
import {
  selectSettingsMessageSlots,
  type SettingsMessageSlotSelection,
} from "../settings_message_slots";
...
export default function SettingsMain() {
  ...
  const chipBorderOff = isGradientLight
  ...
  const handleSettingsScroll = useCallback(
    (e: any) => {
      ...
      settingsScrollYRef.current = Math.max(
        0,
  ...
  useEffect(() => {
    ...
    AsyncStorage.getItem(HOME_FEATURE_TIPS_REPLAY_COUNT_KEY)
      .then((raw) => {
        if (cancelled) return;
        setHomeTipsReplayCount(
          clampHomeFeatureTipReplayCount(Number(raw ?? 0)),
        );
      })
      .catch(() => {});
    return () => {
  ...
  const homeTipsReplayAvailable =
    homeTipsReplayCount < HOME_FEATURE_TIPS_MAX_REPLAYS;
  const resetHomeFeatureTips = useCallback(() => {
    doHaptic();
    const nextReplayCount = clampHomeFeatureTipReplayCount(
      homeTipsReplayCount + 1,
    );
  ...
  useEffect(() => {
    if (settingsTabVisible && scrollRef.current) {
      scrollRef.current.scrollTo({
        y: settingsScrollYRef.current,
        animated: false,
      });
    }
  }, [settingsTabVisible]);

  const appSnapshot = useAppSnapshotSelector(
    (snapshot) => ({
  ...
  useEffect(() => {
    ...
    setSoundSettings({
      uiSounds:
        appSnapshot.settings.uiSounds ?? getUserSettingsSnapshot().uiSounds,
      voiceOut: appSnapshot.settings.voiceOut,
    });
  }, [appSnapshot.settings]);
  const updateSoundSetting = useCallback(
    (key: "uiSounds" | "voiceOut", value: boolean) => {
  ...
  const initialSettingsMessageOwnerRef = useRef(
    captureAccountGeneration().stableId ?? "",
  );
  const [settingsMessageOwner, setSettingsMessageOwner] = useState(
    initialSettingsMessageOwnerRef.current,
  );
  const [settingsMessageSelection, setSettingsMessageSelection] =
    useState<SettingsMessageSlotSelection>(() =>
  ...
  useEffect(() => {
    ...
    void refreshAppMessagesSnapshotOnce({ minIntervalMs: 30_000 })
      .then((snapshot) => {
        if (
          cancelled ||
          !account.stableId ||
        ...
        setSettingsMessageSelection((current) => {
          ...
          if (
            current.top?.id !== next.top?.id ||
            current.bottom?.id !== next.bottom?.id
          ) {
  ...
  const [codeEntryMode, setCodeEntryMode] = useState<"referral" | "promo">(
    "referral",
  );
  ...
  const [settingsReferralStateKey, setSettingsReferralStateKey] = useState<
    string | null
  >(() => settingsReferralAccountKey);
  const scopedSettingsReferralState = selectAccountScopedReferralState(
    settingsReferralAccountKey,
    {
  ...
  const settingsReferralRowVisible =
    settingsReferralSurface.marketingVisible || settingsReferralDrainVisible;
   ...
   * ре-рендера.
   */
  const [clearCacheState, setClearCacheStateRaw] = useState<
    "idle" | "running" | "done" | "failed"
  >("idle");
  const clearCacheStateRef = useRef<"idle" | "running" | "done" | "failed">(
    "idle",
  );
  const clearCacheResetTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  const setClearCacheState = useCallback(
    (next: "idle" | "running" | "done" | "failed") => {
   ...
   */
  const [switchAccountStage, setSwitchAccountStage] = useState<
    "idle" | "confirm" | "wiping"
  >("idle");
  ...
  const [hapticTap, setHapticTap] = useState(
    () => appSnapshot.settings?.tapHaptics ?? true,
  );
    ...
    Promise.all([
      ...
      .then(([pairs, vip]) => {
        if (
          ...
          return;
        const hydratedName = resolveHydratedProfileName(
          settingsHydrationStartedAt,
          pairs[0][1],
        );
  ...
  useEffect(() => {
    ...
    const onVipActivated = DeviceEventEmitter.addListener(
      "vip_activated",
      refreshVipUntil,
    );
    const onAccessChanged = DeviceEventEmitter.addListener(
      "premium_access_changed",
      refreshVipUntil,
    );
    const onIntroChanged = DeviceEventEmitter.addListener(
  ...
  const refreshLinkedAuth = useCallback((): Promise<void> => {
    ...
    const task = getLinkedAuthInfo()
      .then((info) => {
        if (
          settingsRuntimeActive &&
          generation === linkedAuthGenerationRef.current
        )
          setLinkedAuth(info);
      })
      .catch(() => {
      ...
      .finally(() => {
        if (
          ...
          setAuthReady(true);
        if (linkedAuthInFlightRef.current === task)
          linkedAuthInFlightRef.current = null;
      });
  ...
  const updateLocalNameReferences = useCallback(
    async (fromName: string, toName: string) => {
      try {
        ...
        if (lb) {
          ...
          const updated = arr.map((e: any) =>
            e.name === fromName ? { ...e, name: toName } : e,
          );
          await AsyncStorage.setItem("leaderboard", JSON.stringify(updated));
        }
      } catch (error) {
        DebugLogger.error(
          "settings.tsx:renameName:leaderboard",
          error,
          "warning",
        );
      }
      ...
      try {
        ...
        DebugLogger.error(
          "settings.tsx:renameName:weekLeaderboard",
          error,
          "warning",
        );
      }
      ...
      try {
        ...
        DebugLogger.error(
          "settings.tsx:renameName:leagueState",
          error,
          "warning",
        );
      }
      ...
      try {
        ...
        DebugLogger.error(
          "settings.tsx:renameName:leagueResultPending",
          error,
          "warning",
        );
      }
  ...
  const saveName = async () => {
    ...
    if (!trimmed) {
      alertOverName(
        L(
          "Введи имя",
          "Введіть ім\'я",
          "Escribe un nombre o apodo",
          "Digite um nome ou apelido",
          "Nhập tên hoặc biệt danh",
          "Masukkan nama atau nama panggilan",
          "Bir ad veya takma ad gir",
    ...
    void AsyncStorage.setItem("user_name", trimmed).catch((error) => {
      DebugLogger.error(
        "settings.tsx:renameName:localApplyOptimistic",
        error,
        "warning",
      );
    });
    void updateLocalNameReferences(oldName, trimmed).catch((error) => {
      DebugLogger.error(
        "settings.tsx:renameName:localReferencesOptimistic",
        error,
        "warning",
      );
    });
    ...
    const rollbackToOldName = () => {
      ...
      void AsyncStorage.setItem("user_name", oldName).catch((error) => {
        DebugLogger.error(
          "settings.tsx:renameName:rollbackStorage",
          error,
          "warning",
        );
      });
      void updateLocalNameReferences(trimmed, oldName).catch((error) => {
        DebugLogger.error(
          "settings.tsx:renameName:rollbackReferences",
          error,
          "warning",
        );
      });
    ...
    try {
      ...
      try {
        ...
        DebugLogger.error(
          "settings.tsx:renameName:reserveName",
          error,
          "warning",
        );
      ...
      if (reservation.status === "taken") {
        ...
        setNameChangeNotice(
          L(
            ...
            "Це ім\'я вже зайняте. Оберіть інше.",
            "Este nombre ya está en uso. Elige otro.",
            "Esse nome já está em uso. Escolha outro.",
            "Tên này đã được dùng. Hãy chọn tên khác.",
            "Nama ini sudah dipakai. Pilih yang lain.",
            "Bu ad zaten kullanılıyor. Başka bir ad seç.",
  ...
  const vipExpiryText =
    vipUntilMs > 0
      ...
      : L(
          `${tierName} без срока окончания`,
          `${tierName} без дати завершення`,
          `${tierName} has no end date`,
          `${tierName} sem data de término`,
          `${tierName} không có ngày kết thúc`,
          `${tierName} tanpa tanggal akhir`,
          `${tierName} bitiş tarihi yok`,
          `${tierName} bez daty zakończenia`,
        );
  ...
  const vipAccessTitle = (() => {
    switch (vipPlan) {
      ...
      case "referral":
        return L(
          "Подарок за приглашения",
          "Подарунок за запрошення",
          "Referral gift",
          "Presente por convite",
          "Quà mời bạn bè",
          "Hadiah undangan",
          "Davet hediyesi",
          "Prezent za zaproszenia",
        );
      case "survey_vip":
        return L(
          "Plus за опрос",
      ...
      default:
        return L(
          `Дополнительный ${tierName}-доступ`,
          `Додатковий ${tierName}-доступ`,
          `Extra ${tierName} access`,
          `Acesso ${tierName} extra`,
          `Quyền ${tierName} bổ sung`,
          `Akses ${tierName} tambahan`,
          `Ek ${tierName} erişimi`,
          `Dodatkowy dostęp ${tierName}`,
        );
    }
  })();
  ...
  if (
    isIntroFullAccess &&
    introFullAccessEndsAt &&
    introFullAccessEndsAt > Date.now()
  ) {
    plusAccessDetails.push({
      key: "intro",
      ...
      title: L(
        ...
        "Повний доступ на 3 дні",
        "Full access for 3 days",
        "Acesso completo por 3 dias",
        "Truy cập đầy đủ 3 ngày",
        "Akses penuh 3 hari",
        "3 gün tam erişim",
        "Pełny dostęp na 3 dni",
      ),
  ...
  const plusRowPress = () => {
    ...
    if (hasPremiumAccess) {
      ...
      router.push({
        ...
        params: { context, source: "settings_premium" },
      } as any);
    }
  };
  ...
  const confirmClearCache = () => {
    ...
    Alert.alert(
      ...
      [
        {
          text: L("Нет", "Ні", "No", "Não", "Không", "Tidak", "Hayır", "Nie"),
  ...
  return (
    <ScreenGradient>
      <BouncyWrap style={bouncyStyle}>
        <Reanimated.ScrollView
          ...
          contentContainerStyle={{
            paddingBottom: tabContentBottomPad,
        ...
        >
          ...
          <View
            style={{
              flexDirection: "row",
              alignItems: "center",
              paddingHorizontal: SETTINGS_GROUP_MARGIN,
              paddingTop: 12,
              paddingBottom: 8,
            }}
          >
          ...
          <SettingsGroup
            marginTop={8}
            surfaceColor={settingsPanelBg}
            borderColor={settingsBorder}
            dividerColor={settingsDivider}
          >
            <SettingsRow
        ...
        */}
          {false ? (
            ...
            >
              <SettingsCustomRow style={{ paddingVertical: 0 }}>
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    if (codeEntryMode === "promo") {
                      ...
                      router.push({
                        pathname: "/referrals",
                        params: { enter: "1", source: "settings" },
                      } as any);
                    }
                  }}
                  style={{
                ...
                >
                  <SettingsIconTile
                    icon={codeEntryMode === "promo" ? "ticket-outline" : "gift"}
                    color={codeEntryMode === "promo" ? "purple" : "pink"}
                  />
                  <Text
                    style={{
                      flex: 1,
                ...
                {false ? (
                  <View
                    style={{ flexDirection: "row", gap: 8, paddingBottom: 13 }}
                  >
              ...
              >
                <View style={{ flex: 1, minWidth: 0 }}>
                  ...
                  >
                    {settingsReferralSurface.softEnabled
                      ? L(
                          "Когда друг оформит Plus или Pro, ты получишь шанс выиграть Plus от 1 до 365 дней",
                          "Коли друг оформить Plus або Pro, ти отримаєш шанс виграти Plus від 1 до 365 днів",
                          "Cuando tu amigo compre Plus o Pro, tendrás la oportunidad de ganar Plus de 1 a 365 días",
                          "Quando seu amigo assinar Plus ou Pro, você terá a chance de ganhar Plus de 1 a 365 dias",
                          "Khi bạn bè mua Plus hoặc Pro, bạn có cơ hội thắng Plus từ 1 đến 365 ngày",
                          "Saat temanmu membeli Plus atau Pro, kamu berkesempatan memenangkan Plus 1–365 hari",
                          "Arkadaşın Plus veya Pro satın aldığında 1–365 gün Plus kazanma şansın olur",
                          "Gdy znajomy kupi Plus lub Pro, dostaniesz szansę wygrać Plus od 1 do 365 dni",
                        )
                      : L(
                          "У тебя остались шансы выиграть Plus — забери их до срока",
                          "У тебе залишилися шанси виграти Plus — забери їх до строку",
                          "Te quedan oportunidades de ganar Plus: úsalas antes del plazo",
                          "Você ainda tem chances de ganhar Plus — use-as antes do prazo",
                          "Bạn vẫn còn cơ hội thắng Plus — hãy dùng trước hạn",
                          "Kamu masih punya kesempatan memenangkan Plus — pakai sebelum batas waktu",
                          "Plus kazanma şansların duruyor — süresi dolmadan kullan",
                          "Masz jeszcze szanse wygrać Plus — wykorzystaj je przed terminem",
                        )}
                  </Text>
                </View>
                <Ionicons
          ...
          {studyTargetSourceLang && ENABLE_DEV_STUDY_TARGET_LANG && false && (
            <View
              style={{ paddingHorizontal: 20, paddingTop: 6, paddingBottom: 6 }}
            >
          ...
          <SettingsSectionTitle
            title={L(
              ...
              "Tampilan dan respons",
              "Görünüm ve geri bildirim",
              "Wygląd i reakcje",
            )}
          />
          <SettingsGroup
                ...
                {(["small", "medium", "large"] as const).map((sz) => (
                  ...
                  >
                    ...
                    <Text
                      style={{
                        fontSize: f.label,
                        color: fontSize === sz ? chipTextOn : t.textMuted,
                        marginTop: 4,
                        textAlign: "center",
                      }}
                    >
                ...
                <SettingsRow
                  ...
                  right={
                    <CustomSwitch
                      ...
                      onValueChange={(value) =>
                        updateSoundSetting("uiSounds", value)
                      }
                    />
                  }
                />
                <SettingsRow
                  ...
                  right={
                    <CustomSwitch
                      ...
                      onValueChange={(value) =>
                        updateSoundSetting("voiceOut", value)
                      }
                    />
                  }
                />
            ...
            {homeTipsReplayAvailable ? (
              <SettingsRow
                ...
                label={L(
                  ...
                  "Mostrar consejos de nuevo",
                  "Mostrar dicas de novo",
                  "Hiện lại mẹo",
                  "Tampilkan tips lagi",
                  "İpuçlarını yeniden göster",
                  "Pokaż wskazówki ponownie",
                )}
      ...
      >
        ...
        >
          ...
          >
            ...
            <Text
              style={{ color: t.textSecond, fontSize: f.body, lineHeight: 22 }}
            >
              {linkedAuth
                ? linkedAuth.provider === "apple"
                  ? "Apple"
            ...
            {!!linkedAuth?.email && (
              <Text
                style={{ color: t.textMuted, fontSize: f.sub, marginTop: 2 }}
              >
                {linkedAuth.email}
              </Text>
            )}
      ...
      >
        ...
        >
          ...
          >
            ...
            <Text
              style={{ color: t.textMuted, fontSize: f.sub, lineHeight: 20 }}
            >
              {L(
              ...
              <TouchableOpacity
                ...
                onPress={async () => {
                  ...
                  if (!res.ok && res.reason === "pending_shard_spend") {
                    Alert.alert(
                      ...
                      [
                        ...
                        {
                          text: L(
                            "Сменить без сохранения",
                            "Змінити без збереження",
                            "Cambiar sin guardar",
                            "Trocar sem salvar",
                            "Đổi mà không lưu",
                            "Ganti tanpa menyimpan",
                            "Kaydetmeden değiştir",
                  ...
                  if (!res.ok && res.reason === "sync_failed") {
                    ...
                    Alert.alert(
                      L(
                        ...
                        "Progresso não salvo",
                        "Chưa lưu tiến độ",
                        "Progres belum disimpan",
                        "İlerleme kaydedilmedi",
                        "Postęp nie został zapisany",
                      ),
                      L(
                  ...
                  if (!res.ok) {
                    showInfoAlert(
                      L(
                        "Выход не прошёл. Попробуй снова.",
                        "Не вдалося вийти",
                      ...
                      L(
                        ...
                        "Невідома помилка. Спробуй ще раз.",
                        "Error desconocido. Inténtalo de nuevo.",
                        "Erro desconhecido. Tente novamente.",
                        "Lỗi không xác định. Hãy thử lại.",
                        "Error tidak dikenal. Coba lagi.",
                        "Bilinmeyen hata. Tekrar dene.",
                  ...
                  if (!res.synced) {
                    showInfoAlert(
                      L(
                        "Можно выбрать аккаунт",
                        "Можна вибрати акаунт",
                        "Puedes elegir cuenta",
                        "Você pode escolher a conta",
                        "Bạn có thể chọn tài khoản",
                        "Kamu bisa memilih akun",
                        "Hesap seçebilirsin",
                        "Możesz wybrać konto",
                      ),
                      L(
      ...
      <Modal
        visible={switchAccountStage === "wiping"}
        transparent
        animationType="fade"
      >
        <View
          ...
          <Pressable onPress={(e) => e.stopPropagation()}>
            ...
            >
              ...
              <View style={{ flexDirection: "row", gap: 12 }}>
                ...
                >
                  <Text
                    style={{ color: t.textMuted, fontSize: f.body }}
                    numberOfLines={1}
                  >
                    {L(
                      "Отмена",
