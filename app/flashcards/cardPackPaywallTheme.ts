...
function m(
  a: CardPackPaywallTheme,
  p: Partial<CardPackPaywallTheme>,
): CardPackPaywallTheme {
  return { ...a, ...p };
}

/** Блок «Вартість / Стоимость» у темі ocean: темно-синій градієнт + світлий текст */
const OCEAN_DARK_PRICE_BLOCK: Pick<
  CardPackPaywallTheme,
  "priceGradient" | "priceBorder" | "priceTextOnCard"
> = {
...
function fadeStop(
  ...
  return [fadeRgba(pair[0], factor), fadeRgba(pair[1], factor)] as [
    string,
    string,
  ];
}
 ...
 */
function forLightShell(
  t: CardPackPaywallTheme,
  themeMode: ThemeMode,
): CardPackPaywallTheme {
  const g = 0.42;
  const edge = 0.42;
  ...
  return m(t, {
    outerGlow: [
      fadeRgba(t.outerGlow[0], g),
      fadeRgba(t.outerGlow[1], g * 0.94),
      fadeRgba(t.outerGlow[2], edge),
    ] as const,
  ...
  switch (cat) {
    ...
    case "exam": {
      ...
      return m(base, {
        ...
        priceBorder: fadeRgba("rgba(109,40,217,0.12)", 1),
      });
    }
    case "slang": {
    ...
    case "verbs": {
      ...
      return m(base, {
        outerGlow: g as [string, string, string],
        priceGradient: [
          fadeRgba("rgba(40,100,50,0.08)", 1),
          "rgba(0,0,0,0)",
        ] as [string, string],
      });
    }
...
export function getCommunityUgcPackPaywallTheme(
  ...
  const k =
    themeKey && ids.includes(String(themeKey))
      ? String(themeKey)
      : UGC_CARD_THEME_DEFAULT_ID;
  const patch =
    UGC_SHELL_PATCH[k] ?? UGC_SHELL_PATCH[UGC_CARD_THEME_DEFAULT_ID];
  let t = m(shell, patch);
