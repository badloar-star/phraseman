...
function scorePromptRegressionCase(item) {
    ...
    if (!texts.length && !questions.length && !item.semanticKeys?.length) {
        const productionErrors = item.stageKind === 'challenge_topic' ? (0, question_artifacts_1.validateTopicArtifact)({}, { kind: item.stageKind, cefr: item.expectedCefr }) : ['regression_fixture_shape_invalid'];
