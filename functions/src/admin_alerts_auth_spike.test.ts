import { isAuthFailureErrorDoc, nextAuthFailureSpikeState } from './admin_alerts';
...
describe('isAuthFailureErrorDoc', () => {
  ...
  it('ловит context=auth:signin_failure даже без feature', () => {
    ...
    expect(isAuthFailureErrorDoc({ feature: 'lessons', context: 'auth:signin_failure' })).toBe(true);
  });

  it('отсекает не-auth ошибки и мусор', () => {
    expect(isAuthFailureErrorDoc({ feature: 'payments' })).toBe(false);
    expect(isAuthFailureErrorDoc({ feature: 'authentication' })).toBe(false);
    expect(isAuthFailureErrorDoc({ context: 'auth:signin_success' })).toBe(false);
    expect(isAuthFailureErrorDoc({})).toBe(false);
    expect(isAuthFailureErrorDoc({ feature: null, context: undefined })).toBe(false);
  });
});

describe('nextAuthFailureSpikeState', () => {
  it('первое событие открывает окно без алерта', () => {
    const next = nextAuthFailureSpikeState({ window: undefined, lastAlertedAt: 0, now: 1000, threshold: 5 });
  ...
  it('инкрементит счётчик внутри окна и алертит на пороге', () => {
    const below = nextAuthFailureSpikeState({
    ...
    const atThreshold = nextAuthFailureSpikeState({
  ...
  it('считает stage внутри окна и капает карту на 8 ключей', () => {
    const first = nextAuthFailureSpikeState({
    ...
    for (let i = 0; i < 8; i += 1) full[`s${i}`] = 1;
    const capped = nextAuthFailureSpikeState({
