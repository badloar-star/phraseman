# Parse lesson 30 Relative Clauses markdown and emit TypeScript fragment for lessonCards[30]
import json
import pathlib
import re

# Format: **Правильно (RU):** ... (same as lesson 29)
...
def clean_secret_glue(s: str) -> str:
    s = s.strip()
    s = re.split(
        r"\.(?:Продолжаем|Завершаем|Переходим|Ми продовжуємо|Мы завершаем)\b",
        s,
        maxsplit=1,
        flags=re.I,
    )[0]
    s = re.split(r"\.Ниже представлены\b", s, maxsplit=1, flags=re.I)[0]
    s = re.sub(r"\s*---\s*$", "", s)
...
def main() -> None:
    p = pathlib.Path(__file__).resolve().parent / "lesson_30_user.md"
    if not p.exists():
        raise SystemExit("Place scripts/lesson_30_user.md")
    ...
    lines.append("  },")
    out = pathlib.Path(__file__).resolve().parent / "_lesson_30_insert.fragment.txt"
    out.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print("Wrote", out, "lines", len(lines))

    jpath = pathlib.Path(__file__).resolve().parent / "lesson_30_cards.json"
