...
const MAX_SUBJECT_WIDTH = 396;
const MAX_SUBJECT_HEIGHT = 390;
const PORTRAIT_BASELINE_Y = 410;
const PORTRAIT_OVERFLOW_CUTOFF_Y = OUTPUT_SIZE / 2;
...
function fitsPortraitOverlay(data) {
  for (let y = 0; y < OUTPUT_SIZE; y += 1) {
    for (let x = 0; x < OUTPUT_SIZE; x += 1) {
      if (data[(y * OUTPUT_SIZE + x) * 4 + 3] <= 10) continue;
      if (inPoly(SAFE_HEX, x + 0.5, y + 0.5)) continue;
      if (y < PORTRAIT_OVERFLOW_CUTOFF_Y) continue;
      return false;
    }
  }
  return true;
...
export async function normalizePortraitOverlay({ inputPath, outputPath, backgroundMode = 'saturated-matte' }) {
  ...
  for (let attempt = 0; attempt < 20; attempt += 1) {
    ...
    normalized = await sharp({
      ...
      .composite([{
        input: subject,
        left: Math.round((OUTPUT_SIZE - width) / 2),
        top: PORTRAIT_BASELINE_Y - height,
      }])
      .raw()
      .toBuffer();
    if (fitsPortraitOverlay(normalized)) break;
    normalized = undefined;
    fitScale *= 0.96;
  }
  if (!normalized) throw new Error('Could not keep the portrait base inside the lower hex frame');
...
async function main() {
  ...
  await normalizePortraitOverlay(options);
