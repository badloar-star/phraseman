...
function activitySection(snapshot) {
  ...
  return `<section class="card section" aria-labelledby="analytics-activity-title"><div class="card-header"><div><h2 id="analytics-activity-title">Активность приложения</h2><p>Компактная разбивка серверного журнала действий за период.</p></div><span class="badge ${sourceTone(snapshot?.sources?.app_activity?.state)}">${escapeHtml(sourceLabel(snapshot?.sources?.app_activity?.state))}</span></div><div class="card-body">${available && entries.length ? `<div class="table-scroll"><table><thead><tr><th>Действие</th><th>События</th></tr></thead><tbody>${entries.slice(0, 20).map(([key, value]) => `<tr><td>${escapeHtml(key)}</td><td>${escapeHtml(number(value))}</td></tr>`).join('')}</tbody></table></div>` : '<div class="analytics-empty">Нет доступных событий за период.</div>'}</div></section>`;
}
