...
> **Owner override 2026-09-19.** Каждая сессия, включая checkpoint/voice/recall,
> обязана вводить 1–5 новых lexical senses и новую ситуацию. Grammar-контракт
> остаётся прежним: новая operation либо явный review set с learning delta.
> Раздел `retrieval_only` ниже сохранён как история и
> больше не разрешает пустой `new_words`. Перед authoring каждой сессии весь
> маршрут читается заново; обязателен межсессионный `judge_progression`.
...
### `retrieval_only` — RETIRED решением владельца 2026-09-19

- пустой `newLexicalSenseIds` теперь даёт `HOLD`;
...
Число само по себе не оправдывает filler. Пустая сессия любого типа теперь
является машинным `HOLD`: автор обязан подобрать 1–5 частотных полезных
значений и новую ситуацию, естественно совместимые с текущей грамматикой. Owner map должен
