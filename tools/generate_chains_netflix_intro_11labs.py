#!/usr/bin/env python3
"""Generate the new Russian Netflix-doc Chains intro VO with ElevenLabs."""
...
OUT_DIR = Path("exports/chains/episode1/netflix_intro")
...
INTRO_TEXT = (
    ...
    "Сначала ты слышишь действие. Потом появляется причина. "
    "Затем время. Затем место. "
    "Каждая новая деталь цепляется за предыдущую, и длинная фраза перестает быть шумом. "
    "Смотри на экран, слушай, повторяй вслух. "
    "Сейчас мы будем собирать английские фразы по цепочке, пока смысл не станет цельным."
)
...
def elevenlabs_tts(api_key: str, text: str, path: Path) -> None:
    ...
    for attempt in range(1, 5):
        response = requests.post(url, headers=headers, data=json.dumps(payload, ensure_ascii=False).encode("utf-8"), timeout=180)
        if response.ok:
...
def fit_to_slot(src: Path, dst: Path) -> dict[str, float]:
    ...
    target_speech_sec = INTRO_DURATION_SEC - TAIL_SILENCE_SEC
    filters: list[str] = []
    speed_factor = 1.0
    ...
    return {"raw_duration_sec": duration, "speed_factor": speed_factor, "tail_silence_sec": TAIL_SILENCE_SEC}
...
def main() -> int:
    ...
    mp3 = OUT_DIR / "chains_ep01_intro_netflix_doc_ru_11labs.mp3"
    wav = OUT_DIR / "chains_ep01_intro_netflix_doc_ru_11labs_slot_41s10.wav"
    ...
    (OUT_DIR / "intro_11labs_manifest.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
