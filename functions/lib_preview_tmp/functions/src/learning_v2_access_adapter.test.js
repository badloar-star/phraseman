    ...
    async runTransaction(fn) {
        ...
        try {
            const tx = {
                get: async (key) => {
                    this.reads.push(key);
                    return { exists: this.values.has(key), data: this.values.get(key) };
                },
                create: (key, value) => {
...
describe('V2 access purchase transaction adapter', () => {
    ...
    ])('fails closed with ordered barrier reads and zero writes after a pre-read %s', async (_label, race, expectedError) => {
        const repo = new MemoryRepository();
        seed(repo);
        const raced = race();
        repo.values.set(raced.key, raced.value);
        await expect((0, learning_v2_access_adapter_1.finalizeV2AccessPurchase)(repo, policy, input))
    ...
    it('routes the account-bound quote below the canonical stable owner root', () => {
        expect((0, learning_v2_access_adapter_1.firestoreV2AccessPath)('learning-v2:access-quote:user-1:quote-1'))
    ...
    it('rejects stale account generation and insufficient balance before writes', async () => {
        const repo = new MemoryRepository();
        seed(repo);
        await expect((0, learning_v2_access_adapter_1.finalizeV2AccessPurchase)(repo, policy, { ...input, accountGeneration: 2 })).rejects.toThrow('account_generation_mismatch');
