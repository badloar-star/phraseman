...
const SOURCE_FILES = {
  ...
  reviewerAiDecisionTemplateV2: 'generated/fr/reviewer/reviewer_ai_decision_template_v2.jsonl',
  legacyDecisionTemplate: 'generated/fr/reviewer/french_review_decision_template.jsonl',
  reviewDecisionImportDryRunScript: 'scripts/gustav_french_review_decision_import_dry_run.ts',
  reviewDecisionContractScript: 'scripts/gustav_french_review_decision_contract_packet.ts',
  reviewerWorkflowV2Script: 'scripts/gustav_reviewer_workflow_v2_packet.ts',
  targetPackManifestV2Script: 'scripts/gustav_target_pack_manifest_v2_packet.ts',
} as const;
...
function scanAdminFile(repoRoot: string, filePath: string): AdminSurfaceFile {
  ...
  return {
    ...
    mentionsStudyTarget: /studytarget|study-target|targetlocale|target-locale/.test(lower),
    mentionsSourceLocale: /sourcelocale|source-locale|source locale/.test(lower),
    mentionsFrench: /french|francais|français|\bfr\b/.test(lower),
    mentionsCoursePack: /course[_ -]?pack|content pack|pack manifest|downloadable pack/.test(lower),
    mentionsReviewer: /reviewer|review queue|quality review|needs review/.test(lower),
    ...
    mentionsApply: /\bapply\b|applyboolpatch|applytextpatch|applynumberpatch/.test(lower),
...
function collectAdminSurfaces(repoRoot: string): AdminSurfaceFile[] {
  ...
  return [...candidates].sort((a, bValue) => a.localeCompare(bValue)).map((file) => scanAdminFile(repoRoot, file));
}

function withAny(files: AdminSurfaceFile[], predicate: (file: AdminSurfaceFile) => boolean): boolean {
  return files.some(predicate);
}

function pathsWhere(files: AdminSurfaceFile[], predicate: (file: AdminSurfaceFile) => boolean): string[] {
...
function buildProductionBlockers(evidence: ProductionBlockerEvidence): Contract['productionBlockerMap'] {
  return [
    ...
    },
    {
      ...
      area: 'activation',
      status: 'blocked',
      evidence: 'activationApproved remains false until all content, reviewer, server, runtime, rollback and apply gates pass.',
...
function buildContract(repoRoot: string, runDir: string): Contract {
  ...
  const adminV2Files = adminFiles.filter((file) => file.path.startsWith('admin/v2/')).length;
  const adminBiblePath = path.join(repoRoot, SOURCE_FILES.adminUiBible);
  const storagePacketPath = runPath(runDir, SOURCE_FILES.storageCloudTargetMapV2Packet);
  const storagePacket = fs.existsSync(storagePacketPath) ? object(readJson<unknown>(storagePacketPath)) : {};
  ...
  const reviewerJsonlFiles = reviewerFiles.filter((file) => file.endsWith('.jsonl'));
  const reviewerTsvFiles = reviewerFiles.filter((file) => file.endsWith('.tsv'));
  const targetPackManifestPath = runPath(runDir, SOURCE_FILES.targetPackManifestV2Draft);
  const runtimeContractPath = runPath(runDir, SOURCE_FILES.runtimeServerDeliveryContractV2);
  const runtimePacketPath = runPath(runDir, SOURCE_FILES.runtimeServerDeliveryContractV2Packet);
  ...
  const adminMentionsStudyTarget = withAny(adminFiles, (file) => file.mentionsStudyTarget);
  const adminMentionsSourceLocale = withAny(adminFiles, (file) => file.mentionsSourceLocale);
  const adminMentionsFrench = withAny(adminFiles, (file) => file.mentionsFrench);
  ...
  return {
    ...
    upstreamStorageCloudContract: {
      ...
      readyForAdminPackDeliverySurfaceV2: b(storageSummary, 'readyForAdminPackDeliverySurfaceV2'),
    ...
    adminSurfaceInventory: {
      ...
      adminIndexPresent: fs.existsSync(path.join(repoRoot, SOURCE_FILES.adminIndex)),
      adminV2Files,
      ...
      adminPackUploadCandidates: pathsWhere(adminFiles, (file) =>
        file.mentionsCoursePack || (file.mentionsUpload && file.mentionsImport) || file.path === SOURCE_FILES.adminPersonalTrainings,
      ),
      adminReviewerCandidates: pathsWhere(adminFiles, (file) => file.mentionsReviewer || file.mentionsAudit),
      adminImportCandidates: pathsWhere(adminFiles, (file) => file.mentionsImport),
      adminActivationCandidates: pathsWhere(adminFiles, (file) => file.mentionsActivation || file.mentionsApply),
      files: adminFiles,
    },
    reviewerArtifactInventory: {
      reviewerDir: rel(repoRoot, reviewerDir),
      ...
      reviewerWorkflowV2SchemaPresent: fs.existsSync(runPath(runDir, SOURCE_FILES.reviewerWorkflowV2Schema)),
      rowDecisionTemplateV2Present: fs.existsSync(runPath(runDir, SOURCE_FILES.reviewerDecisionTemplateV2)),
      aiDecisionTemplateV2Present: fs.existsSync(runPath(runDir, SOURCE_FILES.reviewerAiDecisionTemplateV2)),
      legacyDecisionTemplatePresent: fs.existsSync(runPath(runDir, SOURCE_FILES.legacyDecisionTemplate)),
      rowDecisionTemplateRows: countJsonlRows(runPath(runDir, SOURCE_FILES.reviewerDecisionTemplateV2)),
      aiDecisionTemplateRows: countJsonlRows(runPath(runDir, SOURCE_FILES.reviewerAiDecisionTemplateV2)),
      legacyDecisionTemplateRows: countJsonlRows(runPath(runDir, SOURCE_FILES.legacyDecisionTemplate)),
      dryRunFixtureFiles: walkFiles(path.join(reviewerDir, 'dry_run_fixtures')).length,
    },
    packArtifactInventory: {
...
function validateContract(contract: Contract): Finding[] {
  ...
  if (contract.schemaVersion !== 'gustav-admin-reviewer-delivery-surface-v2') {
    addFinding(findings, 'blocker', 'schema_version_invalid', 'Admin/reviewer delivery surface schema is invalid.');
  }
  if (contract.studyTarget !== 'fr' || contract.targetLocale !== 'fr') {
    addFinding(findings, 'blocker', 'target_not_fr', 'P12 is scoped to studyTarget=fr only.');
  }
  ...
  if (!contract.upstreamStorageCloudContract.readyForAdminPackDeliverySurfaceV2) {
    addFinding(findings, 'blocker', 'upstream_storage_cloud_not_ready', 'P11 storage/cloud map is not ready for P12.');
  }
  ...
  if (!admin.adminPreviewRollbackModelPresent) {
    addFinding(findings, 'blocker', 'admin_preview_rollback_model_missing', 'Admin surfaces must expose preview, audit/history and rollback patterns before pack delivery planning.');
  }
  if (admin.adminPackUploadCandidates.length < 1) {
    addFinding(findings, 'blocker', 'admin_pack_candidate_missing', 'No admin pack/content delivery candidate surface was mapped.');
  }
  ...
  if (!pack.targetPackManifestDraftPresent || !pack.runtimeServerContractPresent || !pack.storageCloudMapPresent) {
    addFinding(findings, 'blocker', 'pack_contract_artifacts_missing', 'Target pack manifest, runtime/server contract and storage/cloud map must all be present.');
  }
  if (pack.runtimeSlicesRequired < 12 || pack.runtimeCacheKeyContracts < 12) {
    addFinding(findings, 'blocker', 'runtime_slice_contract_incomplete', 'Runtime slice/cache-key contract coverage is too small.');
  }
  const approval = contract.deliveryApprovalContract;
  for (const required of ['studyTarget', 'sourceLocales', 'uiLocale', 'rollbackPlanSha256', 'activationApproved']) {
    if (!approval.requiredApprovalFields.includes(required)) {
      addFinding(findings, 'blocker', 'approval_field_missing', `Required approval field is missing: ${required}.`);
    }
  }
  const transitions = approval.disallowedTransitionsNow;
  for (const [key, value] of Object.entries(transitions)) {
    if (value !== false) {
      addFinding(findings, 'blocker', 'dangerous_transition_opened', `Dangerous transition must remain false in P12: ${key}.`);
    }
  }
  const isolation = contract.isolationGateContract;
  if (isolation.requiredAdminGates.length < 6 || isolation.requiredReviewerImportGates.length < 5) {
    addFinding(findings, 'blocker', 'isolation_gate_coverage_too_small', 'Admin/reviewer isolation gates are not broad enough.');
  }
  if (isolation.requiredServerPreviewGates.length < 4 || isolation.requiredRollbackGates.length < 4) {
    addFinding(findings, 'blocker', 'server_or_rollback_gate_coverage_too_small', 'Server preview and rollback gates are not broad enough.');
  }
  if (contract.productionBlockerMap.length < 9) {
    addFinding(findings, 'blocker', 'production_blocker_map_too_small', 'P12 must enumerate admin/reviewer/payload/server/upload/rollback/runtime/activation blockers.');
  }
  return findings;
}
...
function makeProbes(contract: Contract): Probe[] {
  const fixtures: { id: string; expectedAccept: boolean; mutate?: (draft: Contract) => void }[] = [
    { id: 'canonical_admin_reviewer_contract_accepts', expectedAccept: true },
    {
      ...
      mutate: (draft) => { (draft.deliveryApprovalContract.disallowedTransitionsNow as { serverUploadAllowed: boolean }).serverUploadAllowed = true; },
    },
    {
      id: 'reviewer_import_allowed_rejected',
      expectedAccept: false,
      mutate: (draft) => { (draft.deliveryApprovalContract.disallowedTransitionsNow as { reviewerDecisionImportAllowed: boolean }).reviewerDecisionImportAllowed = true; },
    },
    {
      id: 'activation_approved_rejected',
      expectedAccept: false,
      mutate: (draft) => { (draft.deliveryApprovalContract.disallowedTransitionsNow as { activationApproved: boolean }).activationApproved = true; },
    },
    {
      id: 'ready_for_apply_rejected',
      expectedAccept: false,
      mutate: (draft) => { (draft.deliveryApprovalContract.disallowedTransitionsNow as { readyForApply: boolean }).readyForApply = true; },
    },
    {
      id: 'study_target_approval_field_missing_rejected',
      expectedAccept: false,
      mutate: (draft) => { draft.deliveryApprovalContract.requiredApprovalFields = draft.deliveryApprovalContract.requiredApprovalFields.filter((field) => field !== 'studyTarget'); },
    },
    {
      id: 'source_locales_approval_field_missing_rejected',
      expectedAccept: false,
      mutate: (draft) => { draft.deliveryApprovalContract.requiredApprovalFields = draft.deliveryApprovalContract.requiredApprovalFields.filter((field) => field !== 'sourceLocales'); },
    },
    {
    ...
    },
    {
      ...
      expectedAccept: false,
      mutate: (draft) => { draft.upstreamStorageCloudContract.readyForAdminPackDeliverySurfaceV2 = false; },
    },
    {
      id: 'rollback_gate_missing_rejected',
      expectedAccept: false,
      mutate: (draft) => { draft.isolationGateContract.requiredRollbackGates = []; },
    },
  ];
...
function renderMarkdown(report: Report): string {
  ...
  for (const field of report.contract.deliveryApprovalContract.requiredApprovalFields) {
    lines.push(`- \`${field}\``);
  }
...
function main(): void {
  ...
  if (!runArg) {
    throw new Error('Usage: npx tsx scripts/gustav_admin_reviewer_delivery_surface_v2_packet.ts --run <run-dir> --target fr');
  }
  if (target !== 'fr') {
    throw new Error('P12 admin/reviewer delivery surface V2 is currently scoped to --target fr.');
  }
  ...
  const outContract = path.join(packDir, 'admin_reviewer_delivery_surface_v2.json');
  const outJson = path.join(auditsDir, 'admin_pack_delivery_surface_v2_packet.json');
  const outMd = path.join(auditsDir, 'admin_pack_delivery_surface_v2_packet.md');
  ...
  const transitions = contract.deliveryApprovalContract.disallowedTransitionsNow;
  ...
  const report: Report = {
    ...
    inputs: {
      ...
      adminV2Dir: SOURCE_FILES.adminV2Dir,
      adminPersonalTrainings: SOURCE_FILES.adminPersonalTrainings,
      storageCloudTargetMapV2Packet: rel(repoRoot, runPath(runDir, SOURCE_FILES.storageCloudTargetMapV2Packet)),
    ...
    summary: {
      ...
      upstreamStorageCloudReady: contract.upstreamStorageCloudContract.readyForAdminPackDeliverySurfaceV2,
      adminUiBiblePresent: contract.adminSurfaceInventory.adminUiBiblePresent,
      adminSurfaceFilesScanned: contract.adminSurfaceInventory.adminSurfaceFilesScanned,
      ...
      adminDedicatedFrenchPackConsolePresent: contract.adminSurfaceInventory.adminDedicatedFrenchPackConsolePresent,
      adminMentionsStudyTarget: contract.adminSurfaceInventory.adminMentionsStudyTarget,
      adminMentionsSourceLocale: contract.adminSurfaceInventory.adminMentionsSourceLocale,
      adminMentionsFrench: contract.adminSurfaceInventory.adminMentionsFrench,
      adminPreviewRollbackModelPresent: contract.adminSurfaceInventory.adminPreviewRollbackModelPresent,
      adminPackUploadCandidateFiles: contract.adminSurfaceInventory.adminPackUploadCandidates.length,
      adminReviewerCandidateFiles: contract.adminSurfaceInventory.adminReviewerCandidates.length,
      adminImportCandidateFiles: contract.adminSurfaceInventory.adminImportCandidates.length,
      adminActivationCandidateFiles: contract.adminSurfaceInventory.adminActivationCandidates.length,
      ...
      rowDecisionTemplateRows: contract.reviewerArtifactInventory.rowDecisionTemplateRows,
      aiDecisionTemplateRows: contract.reviewerArtifactInventory.aiDecisionTemplateRows,
      legacyDecisionTemplateRows: contract.reviewerArtifactInventory.legacyDecisionTemplateRows,
      reviewerWorkflowV2SchemaPresent: contract.reviewerArtifactInventory.reviewerWorkflowV2SchemaPresent,
      targetPackManifestDraftPresent: contract.packArtifactInventory.targetPackManifestDraftPresent,
      runtimeServerContractPresent: contract.packArtifactInventory.runtimeServerContractPresent,
      storageCloudMapPresent: contract.packArtifactInventory.storageCloudMapPresent,
      runtimeSlicesRequired: contract.packArtifactInventory.runtimeSlicesRequired,
      runtimeCacheKeyContracts: contract.packArtifactInventory.runtimeCacheKeyContracts,
      requiredApprovalFields: contract.deliveryApprovalContract.requiredApprovalFields.length,
      requiredAdminGates: contract.isolationGateContract.requiredAdminGates.length,
      requiredReviewerImportGates: contract.isolationGateContract.requiredReviewerImportGates.length,
      requiredServerPreviewGates: contract.isolationGateContract.requiredServerPreviewGates.length,
      requiredRollbackGates: contract.isolationGateContract.requiredRollbackGates.length,
  ...
  console.log(`GUSTAV admin/reviewer delivery surface V2 packet: ${report.status}`);
  console.log(`Admin surface files scanned: ${report.summary.adminSurfaceFilesScanned}`);
  console.log(`Reviewer artifacts: ${report.summary.reviewerArtifacts}`);
  console.log(`Row decision template rows: ${report.summary.rowDecisionTemplateRows}`);
  console.log(`AI decision template rows: ${report.summary.aiDecisionTemplateRows}`);
  console.log(`Required approval fields: ${report.summary.requiredApprovalFields}`);
  console.log(`Ready for Reviewer Decision Import V2 dry-run: ${report.summary.readyForReviewerDecisionImportV2DryRun ? 'yes' : 'no'}`);
