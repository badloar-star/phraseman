...
import { loadApprovedFlashcardPackIdea, loadFlashcardBatchForReview, loadFlashcardReplacementForReview, type FlashcardGroundingBucketLike } from './flashcard_grounding';
...
function stored(value: unknown, generation = '7') {
  ...
  return {
    ...
    bucket: { file: () => ({ getMetadata: async () => [{ generation }], download: async () => [bytes] }) } as FlashcardGroundingBucketLike,
  };
}
...
describe('verified flashcard grounding', () => {
  it('loads only an approved, generation-pinned, hash-verified pack idea', async () => {
    ...
    const stage = { artifactId: 'idea-artifact', kind: 'flashcard_pack_idea', state: 'approved', cefr: 'A2', objectPath: 'idea.json', contentHash: value.hash, objectGeneration: '7' } as const;
    await expect(loadApprovedFlashcardPackIdea(value.bucket, stage)).resolves.toMatchObject({ artifactId: 'idea-artifact', packIdea: { packId: 'travel-a2' } });
    await expect(loadApprovedFlashcardPackIdea(value.bucket, { ...stage, state: 'needs_review' })).rejects.toThrow('flashcard_pack_idea_not_approved');
    await expect(loadApprovedFlashcardPackIdea(value.bucket, { ...stage, contentHash: 'f'.repeat(64) })).rejects.toThrow('flashcard_pack_idea_content_hash_mismatch');
  });
  ...
  it('revalidates a rich exact-count batch before review or ledger mutation', async () => {
    ...
    await expect(loadFlashcardBatchForReview(value.bucket, { artifactId: 'batch-artifact', kind: 'flashcard_items', state: 'needs_review', count: 1, objectPath: 'batch.json', contentHash: value.hash, objectGeneration: '8', groundingReceipt }, { allowNeedsReview: true }))
  ...
  it('revalidates one immutable card replacement before ledger mutation', async () => {
    ...
    await expect(loadFlashcardReplacementForReview(value.bucket, { artifactId: 'replacement-artifact', kind: 'flashcard_item_replacement', state: 'needs_review', objectPath: 'replacement.json', contentHash: value.hash, objectGeneration: '9', groundingReceipt }, { allowNeedsReview: true }))
