...
import {
  ...
  GAVAN_WEEK1_DAY5_REVIEWER_EXPORT_PATH,
  validateGavanWeek1Day5ReviewerExport,
  writeGavanWeek1Day5ReviewerExport,
} from '../tools/personal_plan_gavan_week1_day5_reviewer_export';

const GENERATED_AT = '2026-06-02T23:30:00.000Z';
const BROKEN_ENCODING_RE = /[\u00d0\u00d1\u00c2\u00e2\ufffd]/;

function candidate() {
  return buildGavanWeek1Day5BlueprintCandidate(
    buildGavanWeek1ExpansionStandards({ generatedAt: GENERATED_AT }),
    { generatedAt: GENERATED_AT },
  );
}

describe('Gavan week 1 day 5 reviewer export', () => {
  beforeEach(() => {
    if (existsSync(GAVAN_WEEK1_DAY5_REVIEWER_EXPORT_PATH)) {
      rmSync(GAVAN_WEEK1_DAY5_REVIEWER_EXPORT_PATH, { force: true });
    }
  });

  afterAll(() => {
    writeGavanWeek1Day5ReviewerExport(candidate(), {
      generatedAt: GENERATED_AT,
      targetPath: GAVAN_WEEK1_DAY5_REVIEWER_EXPORT_PATH,
    });
  });

  it('exports every day 5 content unit and explanation card for manual review', () => {
    const blueprint = candidate();
    const reviewExport = buildGavanWeek1Day5ReviewerExport(blueprint, {
      generatedAt: GENERATED_AT,
    });

    expect(reviewExport.kind).toBe('gavan_week1_day5_reviewer_export');
    expect(reviewExport.dayId).toBe('gavan-week1-day5');
    expect(reviewExport.dayIndex).toBe(5);
    ...
    expect(validateGavanWeek1Day5ReviewerExport(reviewExport, blueprint)).toEqual({
      valid: true,
      issues: [],
    });
  });

  it('keeps day 5 media and pronunciation claims honest for a non-final blueprint', () => {
    const reviewExport = buildGavanWeek1Day5ReviewerExport(candidate(), {
      generatedAt: GENERATED_AT,
    });
  ...
  it('exports day 5 exercise blueprint coverage without pretending exercises are built', () => {
    const blueprint = candidate();
    const reviewExport = buildGavanWeek1Day5ReviewerExport(blueprint, {
      generatedAt: GENERATED_AT,
    });
    ...
    expect(reviewExport.exerciseCoverage).toEqual({
      requiredTypes: ['micro_dialogue', 'missing_word', 'active_recall', 'natural_choice'],
      presentTypes: ['micro_dialogue', 'missing_word', 'active_recall', 'natural_choice'],
      valid: true,
    });
  ...
  it('catches missing rows fake media claims forbidden anchors and missing exercise coverage', () => {
    ...
    const result = validateGavanWeek1Day5ReviewerExport(corrupted, blueprint);
  ...
  it('writes deterministic day 5 reviewer JSON only under temp or report roots', () => {
    const result = writeGavanWeek1Day5ReviewerExport(candidate(), {
      generatedAt: GENERATED_AT,
      targetPath: GAVAN_WEEK1_DAY5_REVIEWER_EXPORT_PATH,
    });
    ...
    expect(existsSync(GAVAN_WEEK1_DAY5_REVIEWER_EXPORT_PATH)).toBe(true);

    const serialized = readFileSync(GAVAN_WEEK1_DAY5_REVIEWER_EXPORT_PATH, 'utf8');
    expect(serialized).not.toMatch(BROKEN_ENCODING_RE);

    const parsed = JSON.parse(serialized);
    expect(parsed.kind).toBe('gavan_week1_day5_reviewer_export');
  ...
  it('rejects app tools tests and root config write targets', () => {
    const appResult = writeGavanWeek1Day5ReviewerExport(candidate(), {
      generatedAt: GENERATED_AT,
      targetPath: path.join(process.cwd(), 'app', 'gavan-week1-day5-reviewer-export.json'),
    });
    const toolsResult = writeGavanWeek1Day5ReviewerExport(candidate(), {
      generatedAt: GENERATED_AT,
      targetPath: path.join(process.cwd(), 'tools', 'gavan-week1-day5-reviewer-export.json'),
    });
    const testsResult = writeGavanWeek1Day5ReviewerExport(candidate(), {
      generatedAt: GENERATED_AT,
      targetPath: path.join(process.cwd(), 'tests', 'gavan-week1-day5-reviewer-export.json'),
    });
  ...
  it('does not import live catalog quiz UI storage audio scoring or navigation', () => {
    const source = readFileSync(
      path.join(process.cwd(), 'tools', 'personal_plan_gavan_week1_day5_reviewer_export.ts'),
      'utf8',
    );
