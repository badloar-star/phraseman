 ...
 */
const snapshotCanonicalJsonInput = (input, rootPath) => {
...
const validatePackageShape = (input) => {
    ...
    const episodeIssues = exactObjectIssues(root.episode, EPISODE_KEYS.filter((key) => key !== "checkpointContract"), ["checkpointContract"], "$.episode");
...
const validateContentGateReceiptBody = (input) => {
    ...
    if (input.schemaVersion !== "content-gate-receipt-body.v1" ||
        ...
        !["validationReceiptHash", "localizationReceiptSetHash", "reviewReceiptHash", "waiverSetHash"].every((key) => typeof input[key] === "string" && HASH_PATTERN.test(String(input[key]))) ||
...
const validateEpisodePolicyLimits = (episode, registry) => {
    ...
    if (episode.episodeKind === "ordinary") {
        ...
        if (typeof episode.estimatedMinutes !== "number" ||
