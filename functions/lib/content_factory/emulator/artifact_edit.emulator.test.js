...
test('concurrent edit replay creates one immutable revision, correction event and audit', async () => {
    ...
    const base = { stageId, requestId, kind: 'challenge_questions', scopeId: 'topic-1', studyTarget: 'en', sourceLocale: 'ru', cefr: 'A2', count: 10, revision: 1, artifactId: `artifact:${stageId}`, state: 'approved', objectPath: 'base.json', objectGeneration: '1', contentHash: 'a'.repeat(64), qaReceipt: { status: 'passed' }, groundingReceipt: { hash: 'g' }, judgeReceipt: { status: 'advisory_pass', contentHash: 'a'.repeat(64) }, judgeUpdatedAt: 123 };
