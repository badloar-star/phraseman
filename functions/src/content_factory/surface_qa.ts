...
export function runSurfaceQa(input: { artifact: GeneratedSurfaceArtifact; studyTarget: string; sourceLocale: string; sourcePhrases: readonly string[] }) {
  ...
  const semanticKeys = input.artifact.items.map((item) => ('front' in item ? `${item.front}\n${item.back}` : `${item.prompt}\n${item.answer}`).trim().toLowerCase());
  ...
  const contract = input.artifact.surface === 'flashcard'
    ? { promptLocale: input.studyTarget, answerLocale: input.sourceLocale }
    : { promptLocale: input.sourceLocale, answerLocale: input.studyTarget };
