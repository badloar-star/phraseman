...
describe("privacy-safe aggregate onboarding funnel contract", () => {
  it("records start once after onboarding state restoration and never gates it on analytics consent", () => {
    ...
    expect(onboarding).toMatch(
      /if \(!restored\) return;[\s\S]*?recordOnboardingFunnelStart\(\)/,
    );
    expect(onboarding).not.toMatch(
      /if \(analyticsAllowed\)[\s\S]{0,180}recordOnboardingFunnelStart/,
    );
  });
  ...
  it("records completion only inside the successful finish path after local completion is persisted", () => {
    ...
    const completion = onboarding.indexOf(
      "recordOnboardingFunnelCompletion(",
      finishStart,
    );
  ...
  it("counts the analytics decision for refusals too, not only for grants", () => {
    expect(onboarding).toContain(
      "recordOnboardingFunnelCompletion(analyticsAllowed ? 'granted' : 'denied')",
    );
  ...
  it("shows aggregate-only Started, Completed, and Conversion metrics with 7/28/90 and platform filters", () => {
    ...
    expect(admin).toContain(
      "httpsCallable(functionsUs, 'adminGetOnboardingFunnel')",
    );
  });
