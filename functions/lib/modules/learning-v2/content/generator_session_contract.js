...
function learningV2VoiceForSlot(input) {
    if (!Number.isSafeInteger(input.episodeOrdinal) || input.episodeOrdinal < 1 || input.episodeOrdinal > 32 ||
        !Number.isSafeInteger(input.slotOrdinal) || input.slotOrdinal < 1 || input.slotOrdinal > 10000) {
    ...
    if (!characterId) {
        return exports.LEARNING_V2_OPENAI_TTS_VOICES[(input.episodeOrdinal + input.slotOrdinal - 2) % exports.LEARNING_V2_OPENAI_TTS_VOICES.length];
    }
    const seed = (0, decision_registry_1.hashCanonicalBody)({ schemaVersion: 'learning-v2-character-voice-seed.v1', characterId });
...
function validateLearningV2GeneratedSessionIntro(input) {
    ...
    for (const block of input.blocks) {
        ...
        if (blockIds.has(id))
            throw new Error('learning_v2_generator_intro_block_duplicate');
        blockIds.add(id);
        (0, generator_course_contract_1.assertLearningV2LocalizedEnvelope)(block.titleByLocale, 'intro_block_title');
        (0, generator_course_contract_1.assertLearningV2LocalizedEnvelope)(block.bodyByLocale, 'intro_block_body');
    }
    const ids = new Set();
    const slots = new Set();
    for (const question of input.checkQuestions) {
        ...
        ids.add(id);
        if (![1, 2, 3].includes(question.requiredTaskSlot) || slots.has(question.requiredTaskSlot))
