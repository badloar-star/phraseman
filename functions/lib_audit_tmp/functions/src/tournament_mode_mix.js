...
exports.ROUND_NUMBERS = exports.MIX_TOTAL = void 0;
exports.defaultModeMix = defaultModeMix;
exports.defaultRoundMix = defaultRoundMix;
exports.normalizeModeMix = normalizeModeMix;
exports.normalizeRoundMix = normalizeRoundMix;
exports.adjustModeMix = adjustModeMix;
exports.mixToSlots = mixToSlots;
exports.isCustomMix = isCustomMix;
const tournament_pool_plan_1 = require("./tournament_pool_plan");
exports.MIX_TOTAL = 100;
exports.ROUND_NUMBERS = [1, 2, 3, 4];
/**
 ...
 */
function defaultModeMix(modes = tournament_pool_plan_1.TOURNAMENT_MODES) {
    const share = Math.floor(exports.MIX_TOTAL / modes.length);
    const mix = {};
    modes.forEach((mode) => { mix[mode] = share; });
    let remainder = exports.MIX_TOTAL - share * modes.length;
    for (let i = 0; remainder > 0 && modes.length > 0; i += 1, remainder -= 1) {
        mix[modes[i % modes.length]] += 1;
    }
    return mix;
}
function defaultRoundMix() {
    const rounds = {};
    for (const roundNo of exports.ROUND_NUMBERS)
        rounds[roundNo] = defaultModeMix();
    return { rounds };
}
/**
 ...
 */
function normalizeModeMix(raw, modes = tournament_pool_plan_1.TOURNAMENT_MODES) {
    ...
    for (const mode of modes) {
        const value = Math.max(0, Math.min(exports.MIX_TOTAL, Math.round(Number(source[mode] ?? 0)) || 0));
        cleaned[mode] = value;
        sum += value;
    }
    if (sum === 0)
        return defaultModeMix(modes);
    if (sum === exports.MIX_TOTAL)
        return cleaned;
    ...
    let scaledSum = 0;
    for (const mode of modes) {
        const value = Math.floor((cleaned[mode] * exports.MIX_TOTAL) / sum);
        scaled[mode] = value;
        scaledSum += value;
    }
    let remainder = exports.MIX_TOTAL - scaledSum;
...
function normalizeRoundMix(raw) {
    const source = (raw && typeof raw === 'object' && !Array.isArray(raw))
        ? raw.rounds
        : null;
 ...
 */
function adjustModeMix(current, changedMode, nextValue, modes = tournament_pool_plan_1.TOURNAMENT_MODES) {
    const target = Math.max(0, Math.min(exports.MIX_TOTAL, Math.round(nextValue)));
    const others = modes.filter((mode) => mode !== changedMode);
    if (others.length === 0)
        return { [changedMode]: exports.MIX_TOTAL };
    const rest = exports.MIX_TOTAL - target;
 ...
 */
function mixToSlots(mix, taskCount) {
    const modes = tournament_pool_plan_1.TOURNAMENT_MODES.filter((mode) => (mix[mode] ?? 0) > 0);
    if (modes.length === 0 || taskCount <= 0)
        return [];
    ...
    floors.forEach((entry) => {
        for (let i = 0; i < entry.floor; i += 1)
            slots.push(entry.mode);
    });
...
function isCustomMix(mix, modes = tournament_pool_plan_1.TOURNAMENT_MODES) {
    const base = defaultModeMix(modes);
    return modes.some((mode) => (mix[mode] ?? 0) !== base[mode]);
}
//# sourceMappingURL=tournament_mode_mix.js.map