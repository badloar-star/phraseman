import {
  resolveModeTemplateApprovalGateFromFirestore,
  type ModeTemplateReceiptDocumentReader,
} from "./mode_template_transition_reader";

const hash = "a".repeat(64);
const subject = { entityType: "mode_template" as const, entityId: "template-1", entityRevision: 1, entityFingerprint: hash };
...
const reader: ModeTemplateReceiptDocumentReader = {
...
describe("server-owned ModeTemplate receipt reader", () => {
  it("loads all receipt documents and accepts an exact gate", async () => {
    await expect(resolveModeTemplateApprovalGateFromFirestore(reader, subject, ids)).resolves.toEqual({ ok: true });
  });

  it("fails closed when any receipt is missing", async () => {
    await expect(
      resolveModeTemplateApprovalGateFromFirestore(reader, subject, { ...ids, reviewReceiptId: "missing" }),
    ).resolves.toEqual({ ok: false, reason: "mode_template_gate_receipt_missing" });
  });
  ...
  it("rejects a stale receipt subject before publish", async () => {
    ...
    await expect(
      resolveModeTemplateApprovalGateFromFirestore(reader, subject, { ...ids, reviewReceiptId: "review-stale" }),
    ).resolves.toEqual({ ok: false, reason: "mode_template_gate_subject_mismatch" });
  });

  it("rejects a forged gate hash", async () => {
    docs["content_studio_gate_receipts/gate-forged"] = {
      ...(docs["content_studio_gate_receipts/gate-1"] as object),
      validationReceiptHash: "f".repeat(64),
    };
    await expect(
      resolveModeTemplateApprovalGateFromFirestore(reader, subject, { ...ids, gateReceiptId: "gate-forged" }),
    ).resolves.toEqual({ ok: false, reason: "mode_template_gate_hash_or_subject_mismatch" });
  });
});
