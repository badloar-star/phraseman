...
export type PlanTaskDestination =
  ...
  | { type: 'mistake_practice'; length: 5 }
...
function buildPlanMistakesAddMoreTask(day: PlanDay): PlanDailyTask | null {
  ...
  return buildTask(
    ...
    'plan_mistakes',
    'Исправь ошибки',
    'Короткая адаптивная сессия по твоим реальным ошибкам.',
    5,
    [5, 10, 15, 20],
    { type: 'mistake_practice', length: 5 },
  );
...
function addMoreTasksAfterVisibleSlice(
  ...
  const supplemental: PlanDailyTask[] = [];
  if ((options.mistakePracticeReadyCount ?? 0) >= 5 && !queue.some((task) => task.kind === 'mistake_practice')) {
    const mistakesTask = buildPlanMistakesAddMoreTask(day);
    if (mistakesTask) supplemental.push(mistakesTask);
  }
  if (supplemental.length === 0) return queue;
...
function withOrderedTasks(planId: PersonalPlanId, day: PlanDay): PlanDay {
  return {
    ...day,
...
function generateDays(planId: PersonalPlanId, horizonWeeks: number, topics: string[], firstDays?: PlanDay | PlanDay[]): PlanDay[] {
  ...
  const days: PlanDay[] = firstDays ? (Array.isArray(firstDays) ? [...firstDays] : [firstDays]).map((day) =>
    bindAcceptedCandidateSource(planId, withOrderedTasks(planId, day))
  ) : [];
  for (let day = days.length + 1; day <= total; day += 1) {
    const topic = topics[(day - 2 + topics.length) % topics.length];
    days.push(withOrderedTasks(planId, makeGeneratedDay(planId, day, topic)));
  }
  return days;
}
