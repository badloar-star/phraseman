...
exports.validateArenaQuestionReplacementArtifact = validateArenaQuestionReplacementArtifact;
...
function validateArenaQuestionReplacementArtifact(artifact, expected) {
    const output = record(artifact);
    const result = record(output?.result);
    const grounding = record(expected.grounding);
    if (!output || output.stage !== 'arena_question_replacement' || !result)
        return ['arena_replacement_artifact_invalid'];
    if (!grounding || !record(grounding.topic) || !record(grounding.originalQuestion) || !text(grounding.replacementForQuestionId))
        return ['arena_replacement_grounding_required'];
    const item = record(result.item);
    const errors = [];
    if (text(result.replacementForQuestionId) !== text(grounding.replacementForQuestionId))
        errors.push('arena_replacement_identity_mismatch');
    if (text(item?.id) !== text(grounding.replacementForQuestionId))
        errors.push('arena_replacement_item_id_mismatch');
    ...
    if (semanticKey === arenaQuestionSemanticKey(grounding.originalQuestion) || (Array.isArray(grounding.previousQuestionKeys) && grounding.previousQuestionKeys.map(String).includes(semanticKey)))
        errors.push('arena_replacement_duplicate');
    return [...new Set(errors)];
}
