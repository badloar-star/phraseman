...
exports.accessBoostPolicyFromRegistry = exports.evaluateAccessBoostEligibility = void 0;
const positiveInteger = (value) => Number.isInteger(value) && value > 0;
const nonNegativeInteger = (value) => Number.isInteger(value) && value >= 0;
const safePositiveInteger = (value) => positiveInteger(value) && Number.isSafeInteger(value);
const validPolicy = (policy) => safePositiveInteger(policy.unitPriceShards) &&
...
const evaluateAccessBoostEligibility = (input, policy) => {
    ...
    if (!nonNegativeInteger(input.recoveryReviewImpressionCount) ||
        input.recoveryReviewImpressionCount < 1) {
        return { eligible: false, reason: 'review_not_shown' };
    }
    if (!input.serverQuoteAvailable)
    ...
    if (!nonNegativeInteger(input.purchasedForGate) ||
    ...
    if (input.purchasedForGate > input.purchasedForChapter ||
...
exports.evaluateAccessBoostEligibility = evaluateAccessBoostEligibility;
const accessBoostPolicyFromRegistry = (registry) => {
    const entry = registry.body.decisions['HYP-V2-006'];
    const settings = entry.settings;
    if (registry.record.ref.version !== registry.body.version ||
...
exports.accessBoostPolicyFromRegistry = accessBoostPolicyFromRegistry;
//# sourceMappingURL=access_boost.js.map