...
assert.match(html, /id:"lesson-select"/u);
...
const visibleSessions = buildLearningV2StaticOwnerReviewBundleV1().sessions;
for (const session of visibleSessions) {
  assert.match(html, new RegExp(`"sessionOrdinal":${session.sessionOrdinal}`, "u"));
}
