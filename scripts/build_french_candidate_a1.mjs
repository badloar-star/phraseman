...
const OUT_DIR = path.join(ROOT, 'content', 'language-test-pilots', 'fr', 'candidate-bank');
...
if (source.language !== 'fr' || source.level !== 'A1' || source.publishable !== false) throw new Error('French A1 source metadata is invalid');
if (!Array.isArray(source.items) || source.items.length !== 40) throw new Error(`Expected 40 authored French A1 items, found ${source.items?.length ?? 0}`);
for (const [skill, expected] of Object.entries(QUOTAS)) {
  const count = source.items.filter((item) => item.skill === skill).length;
  if (count !== expected) throw new Error(`Expected ${expected} French A1 ${skill} items, found ${count}`);
}
if (new Set(SOURCE_ORDER).size !== 40 || SOURCE_ORDER.some((index) => index < 0 || index >= 40)) throw new Error('French A1 source order must contain every source row exactly once');
if (new Set(source.items.map(({ stimulus }) => stimulus)).size !== 40) throw new Error('French A1 stimuli must be unique');
if (new Set(source.items.map(({ targetConstruct }) => targetConstruct)).size !== 40) throw new Error('French A1 target constructs must be unique');
source.items.forEach((item, index) => {
  if (!(item.skill in QUOTAS)) throw new Error(`French A1 item ${index + 1}: invalid skill`);
  if (!['multiple-choice', 'gap-fill'].includes(item.format)) throw new Error(`French A1 item ${index + 1}: invalid format`);
  if (!Array.isArray(item.distractors) || item.distractors.length !== 3) throw new Error(`French A1 item ${index + 1}: expected three distractors`);
  const options = [item.answer, ...item.distractors.map(({ text }) => text)].map((text) => text.normalize('NFKC').trim().toLocaleLowerCase('fr'));
  if (new Set(options).size !== 4) throw new Error(`French A1 item ${index + 1}: options must be unique`);
  for (const distractor of item.distractors) {
    if (!distractor.reasonRu || !/[\u0400-\u04ff]/u.test(distractor.reasonRu)) throw new Error(`French A1 item ${index + 1}: missing Russian distractor rationale`);
  }
});
...
const questions = SOURCE_ORDER.map((sourceIndex, index) => {
  ...
  return {
    id: `fr-a1-${String(index + 1).padStart(3, '0')}`,
...
if (WRITE) {
  ...
  console.log('Wrote isolated French A1 candidate bank (40 items) and manifest.');
} else {
  let stale = false;
  ...
  else console.log('French A1 candidate bank is up to date.');
}
