...
describe('personal plan runtime state', () => {
  ...
  it('keeps every plan day executable for every supported time choice while preserving week-one progression', () => {
    ...
    for (const plan of PERSONAL_PLAN_CATALOG) {
      for (const planDay of plan.days) {
        for (const minutes of [5, 10, 15, 20] as const) {
          if (tasksForMinutes(planDay, minutes).length === 0) {
            emptyChoices.push(`${plan.id}:day${planDay.dayIndex}:${minutes}m`);
          }
        }
      }
    }
    ...
    expect(tasksForMinutes(day, 5)).toHaveLength(1);
    expect(tasksForMinutes(day, 10)).toHaveLength(3);
    expect(tasksForMinutes(day, 15)).toHaveLength(4);
    expect(tasksForMinutes(day, 20)).toHaveLength(4);
    expect(allTasksForDay(day).length).toBeGreaterThan(tasksForMinutes(day, 20).length);
    expect(nextTaskAfterVisibleSlice(day, 5, 0)).toBeNull();
  ...
  it('keeps unfinished tasks first and does not advance the visible day until required work is complete', () => {
    ...
    expect(runtime.requiredTodayCount).toBe(day1Tasks.length);
  });
  ...
  it('builds a home snapshot from actual completion data instead of a hardcoded percent', () => {
    ...
    expect(snapshot.requiredTodayCount).toBe(tasksForMinutes(gavan.days[0], state.minutesPerDay).length);
    expect(snapshot.completedTodayCount).toBe(2);
    expect(snapshot.todayDone).toBe(false);
    expect(snapshot.dayProgressPct).toBe(50);
  ...
  it('separates visible day progress from the long route progress', () => {
    ...
    expect(snapshot.requiredTodayCount).toBe(tasksForMinutes(gavan.days[0], state.minutesPerDay).length);
    expect(snapshot.dayProgressPct).toBe(25);
