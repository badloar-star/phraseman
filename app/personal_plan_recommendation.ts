...
export function personalPlanSetupGoals(lang: Lang): (PersonalPlanSetupChoice & { id: PersonalPlanSetupGoal })[] {
  return [
    { id: 'series', icon: 'ear-outline', title: triLang(lang, { ru: 'Понимать кино и сериалы', uk: 'Розуміти кіно і серіали', es: 'Entender películas y series', 'pt-BR': 'Entender filmes e séries', vi: 'Hiểu phim và series', id: 'Memahami film dan serial', tr: 'Film ve dizileri anlamak', pl: 'Rozumieć filmy i seriale' }), subtitle: triLang(lang, { ru: 'Живая речь на слух — без субтитров', uk: 'Жива мова на слух — без субтитрів', es: 'Habla real de oído, sin subtítulos', 'pt-BR': 'Fala real de ouvido, sem legendas', vi: 'Nghe hiểu lời thoại thật — không phụ đề', id: 'Percakapan nyata dari pendengaran — tanpa subtitle', tr: 'Kulaktan gerçek konuşma — altyazısız', pl: 'Żywa mowa ze słuchu — bez napisów' }) },
    { id: 'everyday', icon: 'chatbubble-ellipses-outline', title: triLang(lang, { ru: 'Говорить в обычной жизни', uk: 'Говорити у звичайному житті', es: 'Hablar en la vida cotidiana', 'pt-BR': 'Falar no dia a dia', vi: 'Nói trong đời sống hàng ngày', id: 'Berbicara dalam kehidupan sehari-hari', tr: 'Günlük hayatta konuşmak', pl: 'Mówić w codziennym życiu' }), subtitle: triLang(lang, { ru: 'Отвечать в разговоре без ступора', uk: 'Відповідати в розмові без ступору', es: 'Responder en una conversación sin bloquearte', 'pt-BR': 'Responder numa conversa sem travar', vi: 'Trả lời trong hội thoại mà không bị đơ', id: 'Menjawab dalam percakapan tanpa macet', tr: 'Konuşmada donmadan cevap vermek', pl: 'Odpowiadać w rozmowie bez blokady' }) },
    { id: 'travel', icon: 'airplane-outline', title: triLang(lang, { ru: 'Путешествовать', uk: 'Подорожувати', es: 'Viajar', 'pt-BR': 'Viajar', vi: 'Đi du lịch', id: 'Bepergian', tr: 'Seyahat etmek', pl: 'Podróżować' }), subtitle: triLang(lang, { ru: 'Аэропорт, отель, кафе и дорога', uk: 'Аеропорт, готель, кафе і дорога', es: 'Aeropuerto, hotel, café y camino', 'pt-BR': 'Aeroporto, hotel, café e estrada', vi: 'Sân bay, khách sạn, quán cà phê và trên đường', id: 'Bandara, hotel, kafe, dan perjalanan', tr: 'Havalimanı, otel, kafe ve yol', pl: 'Lotnisko, hotel, kawiarnia i droga' }) },
    { id: 'words', icon: 'book-outline', title: triLang(lang, { ru: 'Знать нужные слова', uk: 'Знати потрібні слова', es: 'Conocer las palabras necesarias', 'pt-BR': 'Conhecer as palavras necessárias', vi: 'Biết những từ cần thiết', id: 'Mengetahui kata-kata yang dibutuhkan', tr: 'Gerekli kelimeleri bilmek', pl: 'Znać potrzebne słowa' }), subtitle: triLang(lang, { ru: 'Запас на каждый день — и сразу в речь', uk: 'Запас на кожен день — і одразу в мову', es: 'Vocabulario diario, listo para usar al hablar', 'pt-BR': 'Vocabulário diário, pronto para usar na fala', vi: 'Vốn từ mỗi ngày — và dùng ngay khi nói', id: 'Kosakata harian — langsung dipakai bicara', tr: 'Her gün için kelime dağarcığı — hemen konuşmaya', pl: 'Zapas na każdy dzień — od razu do mowy' }) },
    { id: 'mind', icon: 'school-outline', title: triLang(lang, { ru: 'Заниматься для себя', uk: 'Займатися для себе', es: 'Aprender para ti mismo', 'pt-BR': 'Aprender para si mesmo', vi: 'Học cho chính mình', id: 'Belajar untuk diri sendiri', tr: 'Kendin için çalışmak', pl: 'Uczyć się dla siebie' }), subtitle: triLang(lang, { ru: 'Спокойный темп и польза для ума', uk: 'Спокійний темп і користь для розуму', es: 'Ritmo tranquilo y beneficio para la mente', 'pt-BR': 'Ritmo tranquilo e benefício para a mente', vi: 'Nhịp độ thư thái và bổ ích cho trí óc', id: 'Ritme santai dan bermanfaat bagi pikiran', tr: 'Sakin tempo ve zihin için fayda', pl: 'Spokojne tempo i korzyść dla umysłu' }) },
  ];
}

export function personalPlanSetupLevels(lang: Lang): (PersonalPlanSetupChoice & { id: PersonalPlanSetupLevel })[] {
  return [
    { id: 'a0', icon: 'leaf-outline', title: triLang(lang, { ru: 'A0: почти с нуля', uk: 'A0: майже з нуля', es: 'A0: casi desde cero', 'pt-BR': 'A0: quase do zero', vi: 'A0: gần như từ đầu', id: 'A0: hampir dari nol', tr: 'A0: neredeyse sıfırdan', pl: 'A0: prawie od zera' }), subtitle: triLang(lang, { ru: 'Нужны самые базовые фразы', uk: 'Потрібні найбазовіші фрази', es: 'Necesito las frases más básicas', 'pt-BR': 'Preciso das frases mais básicas', vi: 'Cần những cụm từ cơ bản nhất', id: 'Butuh frasa paling dasar', tr: 'En temel ifadelere ihtiyacım var', pl: 'Potrzebuję najbardziej podstawowych zwrotów' }) },
    { id: 'a1', icon: 'book-outline', title: triLang(lang, { ru: 'A1: знаю базовые слова', uk: 'A1: знаю базові слова', es: 'A1: conozco palabras básicas', 'pt-BR': 'A1: conheço palavras básicas', vi: 'A1: biết các từ cơ bản', id: 'A1: tahu kata-kata dasar', tr: 'A1: temel kelimeleri biliyorum', pl: 'A1: znam podstawowe słowa' }), subtitle: triLang(lang, { ru: 'Хочу быстрее собирать фразы', uk: 'Хочу швидше складати фрази', es: 'Quiero formar frases más rápido', 'pt-BR': 'Quero formar frases mais rápido', vi: 'Muốn ghép câu nhanh hơn', id: 'Ingin lebih cepat menyusun frasa', tr: 'Cümleleri daha hızlı kurmak istiyorum', pl: 'Chcę szybciej układać zdania' }) },
    { id: 'a2', icon: 'chatbubble-ellipses-outline', title: triLang(lang, { ru: 'A2: понимаю, но не говорю', uk: 'A2: розумію, але не говорю', es: 'A2: entiendo pero no hablo', 'pt-BR': 'A2: entendo mas não falo', vi: 'A2: hiểu nhưng không nói được', id: 'A2: mengerti tapi tidak bisa bicara', tr: 'A2: anlıyorum ama konuşamıyorum', pl: 'A2: rozumiem, ale nie mówię' }), subtitle: triLang(lang, { ru: 'Нужна практика ответов', uk: 'Потрібна практика відповідей', es: 'Necesito practicar respuestas', 'pt-BR': 'Preciso praticar respostas', vi: 'Cần luyện tập trả lời', id: 'Butuh latihan menjawab', tr: 'Cevap verme pratiğine ihtiyacım var', pl: 'Potrzebuję ćwiczyć odpowiadanie' }) },
    { id: 'b1', icon: 'mic-outline', title: triLang(lang, { ru: 'B1: хочу увереннее', uk: 'B1: хочу впевненіше', es: 'B1: quiero más confianza', 'pt-BR': 'B1: quero mais confiança', vi: 'B1: muốn tự tin hơn', id: 'B1: ingin lebih percaya diri', tr: 'B1: daha kendinden emin olmak istiyorum', pl: 'B1: chcę pewniej' }), subtitle: triLang(lang, { ru: 'Нужен ритм и более живые задания', uk: 'Потрібен ритм і живіші завдання', es: 'Necesito ritmo y ejercicios más dinámicos', 'pt-BR': 'Preciso de ritmo e exercícios mais dinâmicos', vi: 'Cần nhịp độ và bài tập sống động hơn', id: 'Butuh ritme dan latihan yang lebih hidup', tr: 'Ritim ve daha canlı görevlere ihtiyacım var', pl: 'Potrzebuję rytmu i bardziej żywych zadań' }) },
  ];
}
