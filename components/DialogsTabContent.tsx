...
interface DialogsTabContentProps {
  /** Шапка, которую рендерит хозяин экрана НАД списком (внутри того же скролла). */
  headerSlot?: React.ReactNode;
  /** Доп. отступ снизу под плавающий таб-бар. */
  bottomPadding?: number;
  /** Доп. отступ сверху (safe-area), если хозяин не рендерит свою шапку. */
  topPadding?: number;
  /** Проброс скролла наружу (TopFade / коллапс таб-бара). */
  onScroll?: (event: NativeSyntheticEvent<NativeScrollEvent>) => void;
  /** Стрелять impression-аналитику показа списка (CTR-знаменатель). Дефолт true. */
  trackImpression?: boolean;
}

/**
 * Тело раздела «Диалоги» без собственного ScreenGradient/SafeArea/шапки-«назад».
 * Используется и как страница внутри вкладки «Уроки», и внутри тонкой обёртки
 * `ai_dialog_home.tsx` (для прямых переходов на маршрут /ai_dialog_home).
 */
