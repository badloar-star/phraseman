...
export const GAVAN_WEEK1_DAY7_REVIEWER_EXPORT_PATH = path.join(
  process.cwd(),
  '.codex-tmp',
  'personal-plans',
  'gavan-week1-day7-reviewer-export.json',
);
...
export type GavanWeek1Day7ReviewerStatus = 'needs_manual_review';

export type GavanWeek1Day7ReviewerContentUnitRow = {
  id: string;
  kind: 'content_unit';
  ...
  reviewStatus: GavanWeek1Day7ReviewerStatus;
};

export type GavanWeek1Day7ReviewerExplanationRow = {
  id: string;
...
export type GavanWeek1Day7ReviewerExerciseRow = {
  id: string;
...
export type GavanWeek1Day7ReviewerExport = {
  kind: 'gavan_week1_day7_reviewer_export';
  generatedAt: string;
  ...
  reviewStatus: GavanWeek1Day7ReviewerStatus;
  contentUnitRows: GavanWeek1Day7ReviewerContentUnitRow[];
  explanationRows: GavanWeek1Day7ReviewerExplanationRow[];
  exerciseRows: GavanWeek1Day7ReviewerExerciseRow[];
  exerciseCoverage: {
  ...
  mediaClaims: GavanWeek1Day7BlueprintCandidate['mediaClaims'];
  writePolicy: {
    dryRunOnly: true;
...
export type GavanWeek1Day7ReviewerExportOptions = {
  generatedAt: string;
};

export type GavanWeek1Day7ReviewerExportWriteOptions =
  GavanWeek1Day7ReviewerExportOptions & {
    targetPath: string;
  };

export type GavanWeek1Day7ReviewerExportIssueCode =
  | 'wrong_export_kind'
  | 'wrong_day_id'
...
export type GavanWeek1Day7ReviewerExportIssue = {
  code: GavanWeek1Day7ReviewerExportIssueCode;
  detail: string;
  target?: string;
};

export type GavanWeek1Day7ReviewerExportValidationResult = {
  valid: boolean;
  issues: GavanWeek1Day7ReviewerExportIssue[];
};

export type GavanWeek1Day7ReviewerExportWriteResult = {
  valid: boolean;
  issues: GavanWeek1Day7ReviewerExportIssue[];
  targetPath?: string;
  bytesWritten?: number;
  export?: GavanWeek1Day7ReviewerExport;
};
...
function issue(
  ...
  detail: string,
  target?: string,
): GavanWeek1Day7ReviewerExportIssue {
...
export function isGavanWeek1Day7ReviewerExportTargetAllowed(
  targetPath: string,
  cwd = process.cwd(),
): boolean {
...
function contentUnitRow(
  unit: GavanWeek1Day7ContentUnit,
...
function explanationRow(
  contentUnitId: string,
  card: GavanWeek1Day7ExplanationCard,
...
function exerciseRow(
  exercise: GavanWeek1Day7ExerciseBlueprint,
...
function visibleCopy(exportValue: GavanWeek1Day7ReviewerExport): string {
  return JSON.stringify({
    contentUnitRows: exportValue.contentUnitRows.map((row) => ({
      english: row.english,
      meaningRu: row.meaningRu,
      newWords: row.newWords,
      firstSeenConstructions: row.firstSeenConstructions,
    })),
    explanationRows: exportValue.explanationRows.map((row) => ({
      covers: row.covers,
      body: row.body,
    })),
    exerciseRows: exportValue.exerciseRows.map((row) => ({
...
function forbiddenAnchors(exportValue: GavanWeek1Day7ReviewerExport): string[] {
...
function presentExerciseTypes(
  rows: GavanWeek1Day7ReviewerExerciseRow[],
): GavanWeek1ExpansionExerciseType[] {
  return rows.map((row) => row.exerciseType);
}

export function buildGavanWeek1Day7ReviewerExport(
  candidate: GavanWeek1Day7BlueprintCandidate,
  options: GavanWeek1Day7ReviewerExportOptions,
  ...
  const exportValue: GavanWeek1Day7ReviewerExport = {
    kind: 'gavan_week1_day7_reviewer_export',
...
function everyRowNeedsReview(exportValue: GavanWeek1Day7ReviewerExport): boolean {
  return [
...
export function validateGavanWeek1Day7ReviewerExport(
  exportValue: unknown,
  candidate: GavanWeek1Day7BlueprintCandidate,
): GavanWeek1Day7ReviewerExportValidationResult {
  const value = exportValue as GavanWeek1Day7ReviewerExport;
  const issues: GavanWeek1Day7ReviewerExportIssue[] = [];

  if (value.kind !== 'gavan_week1_day7_reviewer_export') {
  ...
  if (value.dayId !== 'gavan-week1-day7' || value.dayIndex !== 7) {
    issues.push(issue('wrong_day_id', 'Reviewer export must describe Gavan week 1 day 7 only.'));
  }
  ...
  candidate.contentUnits.forEach((unit) => {
    if (!exportedContentIds.has(unit.id)) {
      issues.push(issue(
        'missing_content_unit_export',
        'Every day 7 content unit must appear in the reviewer export.',
        unit.id,
      ));
    }
  });
  ...
  candidate.contentUnits.flatMap((unit) => unit.explanationCards).forEach((card) => {
    if (!exportedExplanationIds.has(card.id)) {
      issues.push(issue(
        'missing_explanation_card_export',
        'Every day 7 explanation card must appear in the reviewer export.',
        card.id,
      ));
    }
  });
  ...
  candidate.exerciseBlueprints.forEach((exercise) => {
    if (!exportedExerciseIds.has(exercise.id)) {
      issues.push(issue(
        'missing_exercise_blueprint_export',
        'Every day 7 exercise blueprint must appear in the reviewer export.',
        exercise.id,
      ));
    }
  });
  ...
  if (
    ...
    issues.push(issue(
      'fake_final_audio_claim',
      'Reviewer export cannot claim generated final audio for day 7.',
    ));
  }
  ...
  if (
    ...
    issues.push(issue(
      'fake_final_pronunciation_claim',
      'Reviewer export cannot claim built pronunciation scoring for day 7.',
    ));
  }
...
export function serializeGavanWeek1Day7ReviewerExport(
  exportValue: GavanWeek1Day7ReviewerExport,
): string {
  return `${JSON.stringify(exportValue, null, 2)}\n`;
}

export function writeGavanWeek1Day7ReviewerExport(
  candidate: GavanWeek1Day7BlueprintCandidate,
  options: GavanWeek1Day7ReviewerExportWriteOptions,
): GavanWeek1Day7ReviewerExportWriteResult {
  const exportValue = buildGavanWeek1Day7ReviewerExport(candidate, options);
  const validation = validateGavanWeek1Day7ReviewerExport(exportValue, candidate);
  ...
  if (!isGavanWeek1Day7ReviewerExportTargetAllowed(resolvedTargetPath)) {
    return {
      valid: false,
      issues: [
        issue(
          'target_path_not_allowed',
          'Gavan week 1 day 7 reviewer export can only write under .codex-tmp or docs/reports.',
        ),
      ],
    };
  }

  const serialized = serializeGavanWeek1Day7ReviewerExport(exportValue);
