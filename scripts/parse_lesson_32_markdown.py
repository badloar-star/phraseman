# Parse lesson 32 markdown and emit TypeScript fragment for lessonCards[32]
import json
import pathlib
import re
...
def clean_secret_glue(s: str) -> str:
    s = s.strip()
    s = re.split(
        r"\.(?:Продолжаем|Завершаем|Переходим|Продолжаем наше|Продолжаем наше увлекательное|"
        r"Завершаем наш|Ми продовжуємо|Мы завершаем|Мы приступаем|Мы подошли|Завершаем наш учебный)\b",
        s,
        maxsplit=1,
        flags=re.I,
    )[0]
...
def main() -> None:
    p = ROOT / "lesson_32_user.md"
    if not p.exists():
        raise SystemExit("Place scripts/lesson_32_user.md")
    ...
    lines.append("  },")
    out = ROOT / "_lesson_32_insert.fragment.txt"
    out.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print("Wrote", out, "lines", len(lines))
    (ROOT / "lesson_32_cards.json").write_text(
