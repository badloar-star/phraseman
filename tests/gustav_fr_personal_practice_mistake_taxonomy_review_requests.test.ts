...
const AUDIT_PATH = path.join(ROOT, 'docs', 'gustav', 'generated', 'fr', 'personal_practice', 'fr_personal_practice_mistake_taxonomy_review_requests_audit_v1.json');
const REQUESTS_PATH = path.join(ROOT, 'docs', 'gustav', 'generated', 'fr', 'personal_practice', 'fr_personal_practice_mistake_taxonomy_review_requests_v1.jsonl');
const SCRIPT_PATH = path.join(ROOT, 'scripts', 'gustav_build_fr_personal_practice_mistake_taxonomy_review_requests.mjs');

function readJsonl(filePath: string) {
  return fs.readFileSync(filePath, 'utf8').trim().split(/\r?\n/).filter(Boolean).map((line) => JSON.parse(line));
}

describe('Gustav French personal practice mistake taxonomy review requests', () => {
  it('builds strict LLM review requests for all 56 mistake-taxonomy candidate rows', () => {
    ...
    expect(script).toContain('Do not approve if the English diagnosis id is reused as French mistake taxonomy');
    expect(script).toContain('Problem coach may explain in RU/UK but target answers and examples must remain French.');
    expect(script).toContain('llmApiCalledByThisScript: false');

    expect(audit.schemaVersion).toBe('gustav-fr-personal-practice-mistake-taxonomy-review-requests-audit-v1');
    ...
    expect(audit.outputs.futureDecisionsJsonl).toBe('docs/gustav/generated/fr/personal_practice/fr_personal_practice_mistake_taxonomy_review_decisions_v1.jsonl');

    expect(requests).toHaveLength(56);
    expect(requests[0]).toMatchObject({
      schemaVersion: 'gustav-fr-personal-practice-mistake-taxonomy-review-request-v1',
      requestId: 'fr.personal_practice.mistake_taxonomy.01.review_request.v1',
    ...
    for (const request of requests) {
      ...
      expect(request.candidate.feedbackContract.mustNotUseEnglishAsFallbackContent).toBe(true);
