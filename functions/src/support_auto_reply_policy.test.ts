...
describe('Ответ без утверждений о продукте', () => {
  ...
  describe('сквозное решение', () => {
    ...
    // модель ответила «обычно не занимает больше 48 часов» и «в течение
    // одного-двух дней». Такого срока никто не устанавливал — клиент
    // начнёт его отсчитывать и вернётся с претензией, когда он пройдёт.
    test.each([
      'Обычно это не занимает больше 48 часов, поэтому подождите немного и всё появится.',
      'Как правило, всё решается в течение одного или двух дней, просим немного подождать.',
      'This is usually resolved within 24 hours, so please wait a little longer for it.',
    ])('выдуманный срок не проходит: %s', (reply) => {
      expect(decide(reply).grounded).toBe(false);
    });

    test('честный ответ про очередь БЕЗ срока проходит', () => {
      // зачем: запрет на сроки не должен перекрыть правильную альтернативу —
      // именно её владелец и просил: «сейчас большая нагрузка, ответим
      // в порядке очереди».
      const honest = 'Понимаем, что хочется определённости — это нормально, когда речь о своих '
        + 'деньгах. Честно скажем: сейчас у нас большая нагрузка, и мы отвечаем в порядке очереди, '
        + 'поэтому назвать точный срок не возьмёмся, чтобы не подвести вас обещанием. Ваше '
        + 'обращение не потеряется — оно уже в очереди, и человек из команды напишет вам в это '
        + 'же письмо. Если появятся новые детали, допишите их в ответ.';
      expect(decide(honest).grounded).toBe(true);
    });
