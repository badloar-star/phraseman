...
function safeParseVerdict(text: string): Partial<TriageVerdict> | null {
  try {
    const j = JSON.parse(text);
    if (!j || typeof j !== 'object') return null;
    return j as Partial<TriageVerdict>;
  } catch {
    return null;
  }
}

function normalizeVerdict(raw: Partial<TriageVerdict> | null): TriageVerdict {
...
async function runTriage(systemPrompt: string, userPrompt: string): Promise<TriageVerdict | null> {
  try {
...
const CONTENT_SYSTEM =
  'Ты модератор приложения для изучения английского. Классифицируй жалобу пользователя на ошибку в контенте. ' +
...
export const triageOnErrorReport = onDocumentCreated(
  { document: 'error_reports/{id}', region: REGION, secrets: [OPENAI_API_KEY] },
  async (event) => {
    const snap = event.data;
    if (!snap) return;
    const d = snap.data() || {};
    if (d.triage) return; // already triaged
    const userPrompt = [
...
const USER_SYSTEM =
  'Ты модератор. Классифицируй жалобу одного пользователя на другого (ник/поведение). ' +
...
export const triageOnUserReport = onDocumentCreated(
  { document: 'user_reports/{id}', region: REGION, secrets: [OPENAI_API_KEY] },
  async (event) => {
    const snap = event.data;
    if (!snap) return;
    const d = snap.data() || {};
