...
export function residentIndexForSlot(groupId: string, slot: number): number {
  ...
  // Шаг 7 взаимно прост с числом слотов (50), поэтому внутри комнаты индексы
  // не повторяются — а значит не повторяются и имена.
  return (offset + slot * 7) % RESIDENT_SLOT_COUNT;
}
...
/**
 * Убирает жителей из карты участников — для мест, где считаются НАГРАДЫ.
 * Финализация недели и выбор короны сундука обязаны видеть только живых.
 */
