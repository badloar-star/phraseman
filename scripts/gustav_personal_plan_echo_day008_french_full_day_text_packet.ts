...
type GeneratedRow = {
  ...
  generatedBy: 'gustav_personal_plan_echo_day008_french_full_day_text_packet';
};

const PLAN_ID = 'echo' as const;
const DAY_INDEX = 8 as const;
const GENERATED_BY = 'gustav_personal_plan_echo_day008_french_full_day_text_packet' as const;

const EXPLANATIONS = [
  {
    phraseId: 'echo_d8_p1',
    title: '« In the morning » = le matin',
    rule: 'Pour les moments de la journée, morning, afternoon et evening, on utilise « in ».',
    why: '« In » indique que l’action se passe dans une période large de la journée.',
    commonMistake: 'Ne dis pas « at the morning ». La forme correcte est « in the morning ».',
  },
  {
    phraseId: 'echo_d8_p2',
    title: '« At night » = la nuit',
    rule: 'Night est un cas spécial : on dit « at night », pas « in the night ».',
    why: '« At night » est une expression fixe ; il faut l’apprendre comme un bloc.',
    commonMistake: 'Ne dis pas « in night ». Dis toujours « at night », sans article.',
  },
  {
    phraseId: 'echo_d8_p3',
    title: '« On Sundays » = le dimanche',
    rule: 'Devant les jours de la semaine, on utilise toujours « on » : on Monday, on Friday, on Sundays.',
    why: '« On » montre que l’on parle d’un jour précis, comme d’un point dans le calendrier.',
    commonMistake: 'Ne dis pas « in Sunday » ou « at Sunday ». Dis seulement « on Sunday ».',
  },
  {
    phraseId: 'echo_d8_p4',
    title: '« At seven o’clock » = à sept heures',
    rule: 'Devant une heure exacte, on utilise « at » : at six, at noon, at midnight.',
    why: '« At » indique un point exact dans le temps, pas une période.',
    commonMistake: 'Ne dis pas « in seven o’clock ». Devant l’heure, on utilise seulement « at ».',
  },
  {
    phraseId: 'echo_d8_p5',
    title: 'Deux « sur/dans » dans une seule phrase',
    rule: 'Dans une même phrase, il peut y avoir « on » pour le téléphone et « in » pour le matin.',
    why: '« On my phone » voit le téléphone comme un support. « In the morning » voit le matin comme une période.',
    commonMistake: 'Ne dis pas « in my phone ». En anglais, on dit « on my phone ».',
  },
  {
    phraseId: 'echo_d8_p6',
    title: '« Every evening » = tous les soirs',
    rule: 'Avec « every », on n’utilise pas de préposition : every morning, every day, every weekend.',
    why: '« Every » exprime déjà la régularité ; ajouter « in » serait inutile.',
    commonMistake: 'Ne dis pas « in every evening ». « Every » va directement avant le mot de temps.',
  },
] as const;
...
const FIELD_SPECS: FieldSpec[] = [
  {
    fieldPath: 'topic',
    fieldKind: 'day_topic',
    frenchText: 'Raconter quand les choses arrivent',
    source: (day) => day.topic,
  },
  {
    fieldPath: 'outcome',
    fieldKind: 'day_outcome',
    frenchText: 'Tu apprendras à dire à quel moment de la journée tu fais quelque chose : le matin, l’après-midi, le lundi, à cinq heures.',
    source: (day) => day.outcome,
  },
  {
    fieldPath: 'intro[0].title',
    fieldKind: 'intro_title',
    frenchText: 'Comment dire « à quelle heure » ou « quand »',
    source: (day) => day.intro?.[0]?.title,
  },
  {
    fieldPath: 'intro[0].body',
    fieldKind: 'intro_body',
    frenchText: 'Pour dire quand quelque chose se passe, on utilise de petits mots devant le moment. « In » pour les parties de la journée, comme le matin, l’après-midi ou le soir. « At » pour l’heure exacte ou des moments spéciaux. « On » pour les jours de la semaine.',
    source: (day) => day.intro?.[0]?.body,
  },
  {
    fieldPath: 'intro[0].examples[0].gloss',
    fieldKind: 'intro_example_gloss',
    frenchText: 'Je lis les nouvelles le matin.',
  ...
  },
  {
    ...
    fieldKind: 'intro_example_gloss',
    frenchText: 'Elle regarde la télé la nuit.',
  ...
  },
  {
    ...
    fieldKind: 'intro_title',
    frenchText: 'In / At / On — résumé rapide',
    source: (day) => day.intro?.[1]?.title,
  },
  {
    fieldPath: 'intro[1].body',
    fieldKind: 'intro_body',
    frenchText: 'In the morning / afternoon / evening sert aux parties de la journée. At night / at noon / at 7 o’clock sert à la nuit, midi et l’heure exacte. On Monday / on weekends sert aux jours et aux week-ends. Ces trois mots ne changent pas : retiens simplement avec quoi chacun va.',
    source: (day) => day.intro?.[1]?.body,
  },
  {
    fieldPath: 'intro[1].examples[0].gloss',
    fieldKind: 'intro_example_gloss',
    frenchText: 'Il écoute la radio le dimanche.',
  ...
  },
  {
    ...
    fieldKind: 'intro_example_gloss',
    frenchText: 'Nous regardons les nouvelles à sept heures.',
  ...
  },
  {
    ...
    fieldKind: 'intro_title',
    frenchText: 'Quand les gens lisent les nouvelles',
    source: (day) => day.intro?.[2]?.title,
  },
  {
    fieldPath: 'intro[2].body',
    fieldKind: 'intro_body',
    frenchText: 'Dans différents pays, les gens ont des habitudes différentes. Au Royaume-Uni, beaucoup de personnes écoutent les nouvelles à la radio le matin pendant le petit déjeuner. Aux États-Unis, les journaux télévisés du soir sont populaires, souvent à six heures ou à onze heures. C’est une bonne habitude d’apprendre l’anglais avec de vraies actualités.',
    source: (day) => day.intro?.[2]?.body,
  },
  {
    fieldPath: 'intro[2].examples[0].gloss',
    fieldKind: 'intro_example_gloss',
    frenchText: 'Je lis les nouvelles sur mon téléphone le matin.',
...
function rowIdFromPath(fieldPath: string): string {
  return `fr_echo_d008_${fieldPath.replace(/[^a-zA-Z0-9]+/g, '_').replace(/^_+|_+$/g, '').toLowerCase()}`;
}
...
function renderMarkdown(report: any): string {
  const lines = [
    '# GUSTAV Personal Plan Echo Day 008 French Full-Day Text Packet',
    '',
    `Run: \`${report.runId}\``,
    '',
...
async function main(): Promise<void> {
  ...
  if (!runArg) {
    throw new Error('Usage: npx tsx scripts/gustav_personal_plan_echo_day008_french_full_day_text_packet.ts --run <run-dir>');
  }
  ...
  const rowsPath = path.join(outDir, 'echo_day_008_full_day_fr_rows.jsonl');
  const reviewerQueuePath = path.join(outDir, 'echo_day_008_full_day_fr_reviewer_queue.tsv');
  const packetJsonPath = path.join(auditsDir, 'personal_plan_echo_day008_french_full_day_text_packet.json');
  const packetMdPath = path.join(auditsDir, 'personal_plan_echo_day008_french_full_day_text_packet.md');

  const findings: Finding[] = [];
  const day = await readEchoDay(repoRoot);
  if (!day) {
    addFinding(findings, 'blocker', 'echo_day_008_missing', 'Echo day 8 is missing from source content.', rel(repoRoot, sourcePath));
  }

  let existingPhraseMeaningRowsForDay = 0;
  if (!fs.existsSync(phraseMeaningRowsPath)) {
    addFinding(findings, 'blocker', 'phrase_meaning_rows_missing', 'Echo day 8 phrase meaning rows are missing.', rel(repoRoot, phraseMeaningRowsPath));
  } else {
    ...
    if (existingPhraseMeaningRowsForDay !== 6) {
      addFinding(findings, 'blocker', 'phrase_meaning_rows_day_008_incomplete', `Expected 6 reviewer-needed phrase meaning rows for Echo day 8 but found ${existingPhraseMeaningRowsForDay}.`, rel(repoRoot, phraseMeaningRowsPath));
    }
  }
  ...
  const report = {
    schemaVersion: 'gustav-personal-plan-echo-day008-french-full-day-text-packet-v0',
    ...
    fieldKindCounts,
    generationPolicy: [
      'This packet translates Echo day 8 full-day text fields except phrase meanings, which are already covered by the phrase-meaning layer.',
  ...
  console.log(`GUSTAV Echo day 008 French full-day text packet: ${report.status}`);
