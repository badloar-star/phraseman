...
## 7. ДОПОЛНЕНИЕ 2026-08-20 — ПОЛНЫЙ АУДИТ ХЕНДОВЕРА

По прямому указанию владельца проверена не только DEV-панель, а вся принятая механика.

- Исправлены несовместимые формы ответов level/chest callable и реплей старых receipt/claim docs.
- Клейм уровня теперь выдаёт все пропущенные вехи (L2→L5 = 85★), хранит стабильный
  requestId между сетевыми ретраями и не показывает уже забранную награду после рестарта.
- Статус недельного сундука восстанавливается из серверного claim doc; повторный клейм
  возвращает канонические поля награды; топ списка ограничен утверждёнными 100 друзьями.
- Реализована скидка 25% на подарки с L3 одновременно в UI и в атомарной серверной операции.
- Клиент читает те же remote-config ручки, что сервер (пороги уровней/сундука, cap/top-N,
  eligibility, boost multiplier); allowlist массивов сохранён в безопасном локальном кэше.
- XP-бонус подчиняется kill-switch; pair/snapshot/bonus/claim кэши изолированы по stable uid;
  недельный referral boost больше не остаётся активным из-за старого boolean-кэша.
- «Позвать» отражается оптимистично без кадра задержки; в карточке друга подключены
  существующая «Дуэль», «Подарок» и «Позвать»; в empty state добавлено приглашение.
- Карточка «Вместе» переведена на обязательный HybridSheetShell («Световод + Чекан»).
- Privacy Policy синхронизирована по каноническому legal-пайплайну.

Проверки: 120 клиентских unit/contract тестов + RNTL DEV-sheet, 99 functions-тестов,
functions TypeScript build/runtime parity, economy constitution и Firestore Rules — зелёные.
В прод `phraseman-ea0b3` точечно задеплоены и ACTIVE: `friendsTogetherClaimLevel`,
`friendsClaimWeeklyChest`, `friendSendGift`; `friendsNudge` и
`referralOnUserProgressUpdated` также ACTIVE; `friends_together_enabled=true`.

Не входит в принятую механику и сознательно не реализовано: отдельные косметические
награды L4/L5 из отложенной Фазы 3. Полный живой визуальный проход на эмуляторе всё ещё
требует тестовый аккаунт за auth/onboarding; логика и взаимодействия покрыты узкими тестами.
