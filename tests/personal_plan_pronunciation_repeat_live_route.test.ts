...
describe('personal plan pronunciation-repeat live route', () => {
  ...
  it('builds self-check pronunciation items without fake score promises', () => {
    ...
    expect(items[0]).toEqual(expect.objectContaining({
      ...
      explanation: expect.objectContaining({
        correctRu: expect.stringContaining('Фраза записана'),
    ...
    expect(visibleCopy).not.toMatch(/score|scoring|балл|процент|точн[а-я]+ оцен/i);
    expectCleanRussian(items[0]);
    expect(validatePersonalPlanPronunciationRepeatItem(items[0])).toEqual([]);
  });

  it('fails low-quality pronunciation copy before it reaches the renderer', () => {
    ...
    expect(validatePersonalPlanPronunciationRepeatItem({
      ...
      explanation: {
        ...item.explanation,
        correctRu: 'Получишь точную оценку произношения в процентах.',
      },
    })).toContainEqual(expect.objectContaining({
      code: 'fake_score_claim',
    }));

    expect(validatePersonalPlanPronunciationRepeatItem({
      ...item,
      completionLabel: 'ÐŸÐ¾Ð²Ñ‚Ð¾Ñ€Ð¸Ð»',
    })).toContainEqual(expect.objectContaining({
      code: 'mojibake_copy',
    }));
  });

  it('keeps the live pronunciation screen honest: record, self-play, then complete without fake score', () => {
    const source = fs.readFileSync(path.join(__dirname, '..', 'app', 'personal_plan_exercise.tsx'), 'utf8');

    expect(source).toContain('canCompletePlanPronunciationRecording({');
    expect(source).toContain('setUserPlayedRecording(false);');
    expect(source).toContain('setUserPlayedRecording(true);');
    expect(source).toContain('disabled={saving || !pronunciationRecording}');
    expect(source).toContain('payload: buildPlanPronunciationAttemptPayload({');
    expect(source).toContain("result: 'completed'");
    expect(source).toContain('title="Без оценки и давления"');
  });
  ...
  it('keeps pronunciation-repeat source copy clean, calm and honest', () => {
    ...
    expect(source).toContain('Скажи фразу ещё раз медленнее');
    expect(source).toContain('сначала смысл, потом скорость');
    expect(source).not.toMatch(/процент|точн[а-я]+ оцен/i);
  });
});
