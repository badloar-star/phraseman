...
exports.ADMIN_PLAN_ACTION_CODES = exports.ADMIN_PLAN_EFFECTS = exports.ADMIN_PLAN_KINDS = exports.ADMIN_PLAN_SCHEMA_VERSION = void 0;
exports.parseIdempotencyKey = parseIdempotencyKey;
exports.parsePlanId = parsePlanId;
exports.parseAdminPlanDraft = parseAdminPlanDraft;
exports.parseCreatePlanInput = parseCreatePlanInput;
exports.parseGetPlanInput = parseGetPlanInput;
exports.parseListPlansInput = parseListPlansInput;
const https_1 = require("firebase-functions/v2/https");
exports.ADMIN_PLAN_SCHEMA_VERSION = 1;
exports.ADMIN_PLAN_KINDS = ['reliability', 'retention', 'conversion', 'content_quality', 'support_quality', 'cost_control'];
exports.ADMIN_PLAN_EFFECTS = ['reduce_failures', 'improve_resilience', 'reduce_drop_off', 'improve_conversion', 'improve_quality', 'reduce_cost', 'improve_response_time'];
exports.ADMIN_PLAN_ACTION_CODES = ['investigate_metrics', 'reproduce_issue', 'prepare_change', 'verify_change', 'define_rollback', 'monitor_outcome'];
...
function actionCodes(value) {
    if (!Array.isArray(value) || value.length < 1 || value.length > 8)
        fail('actionCodes is invalid');
    const codes = value.map((item) => choice(item, exports.ADMIN_PLAN_ACTION_CODES, 'actionCode'));
    if (new Set(codes).size !== codes.length)
        fail('actionCodes is invalid');
    return Object.freeze(codes);
}
function buildAdminPlanDraft(input) {
    const planKind = choice(input.planKind, exports.ADMIN_PLAN_KINDS, 'planKind');
    const expectedEffect = choice(input.expectedEffect, exports.ADMIN_PLAN_EFFECTS, 'expectedEffect');
    const codes = actionCodes(input.actionCodes);
    return Object.freeze({
        schemaVersion: exports.ADMIN_PLAN_SCHEMA_VERSION,
        planKind,
...
function parsePlanId(value) {
    if (typeof value !== 'string' || !/^plan_[a-f0-9]{64}$/.test(value))
        fail('planId is invalid');
    return value;
}
function parseAdminPlanDraft(value) {
...
function parseCreatePlanInput(value) {
...
function parseGetPlanInput(value) {
...
function parseListPlansInput(value) {
