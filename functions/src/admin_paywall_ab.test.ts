...
function validPayload(overrides: DocData = {}): DocData {
  return {
    variants: {
...
async function callPublish(data: DocData, auth: unknown = ADMIN_AUTH) {
  const { adminPublishPaywallAb } = require('./admin_paywall_ab');
  return adminPublishPaywallAb({ auth, data });
}

async function callWorkspace(auth: unknown = ADMIN_AUTH) {
...
function buildDoc(input: unknown, before: DocData, actorUid = 'admin-1', nowMs = Date.now()) {
...
describe('buildPaywallAbDoc / paywallAbChanges', () => {
  ...
  it('produces a per-field old-to-new change list', () => {
    ...
    const changes: string[] = paywallAbChanges(before, after);
...
describe('adminGetPaywallAbWorkspace', () => {
  ...
  it('returns the current document and only its own history entries, newest first', async () => {
    ...
    expect(result.history.map((entry: DocData) => entry.id)).toEqual(['h-new', 'h-old']);
  });
