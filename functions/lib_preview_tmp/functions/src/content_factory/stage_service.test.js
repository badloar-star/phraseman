...
describe('independent generation stage planning', () => {
    ...
    it('requires exactly one approved Arena batch for a replacement', () => {
        const base = { requestId: 'r', kind: 'arena_question_replacement', studyTarget: 'en', sourceLocale: 'ru', cefr: 'A2', scopeId: 'arena.replace.a1', schemaVersion: 2, promptVersion: 'v2', qaPolicy: 'arena-studio-quality-v2', revision: 1, approvedPrerequisites: [{ kind: 'arena_questions', artifactId: 'batch', state: 'approved' }] };
        expect((0, stage_service_1.buildGenerationStagePlan)({ ...base, count: 1 }).unit.count).toBe(1);
        expect(() => (0, stage_service_1.buildGenerationStagePlan)({ ...base, count: 2 })).toThrow('stage_capability_count_unsupported');
    });
});
