...
describe('approved dependency catalog', () => {
  test('derives kinds and approved state from the consumer capability', () => {
    expect(parseDependencyCatalogRequest({ requestId: 'req-1', studyTarget: 'en', sourceLocale: 'ru', consumerKind: 'challenge_questions', limit: 25 })).toMatchObject({
  ...
  test('rejects arbitrary state/kind and tampered cursor', () => {
    expect(() => parseDependencyCatalogRequest({ requestId: 'req-1', studyTarget: 'en', sourceLocale: 'ru', consumerKind: 'challenge_questions', state: 'rejected' })).toThrow('dependency_catalog_invalid');
    expect(() => parseDependencyCatalogRequest({ requestId: 'req-1', studyTarget: 'en', sourceLocale: 'ru', consumerKind: 'challenge_questions', cursor: 'bad cursor!' })).toThrow('dependency_catalog_invalid');
  });
