...
import {
  ...
  GAVAN_WEEK1_DAY6_MATERIAL_EXPORT_PACKET_PATH,
  writeGavanWeek1Day6MaterialExportPacket,
...
describe('Gavan week 1 day 6 material quality export packet', () => {
  beforeEach(() => {
    if (existsSync(GAVAN_WEEK1_DAY6_MATERIAL_EXPORT_PACKET_PATH)) {
      rmSync(GAVAN_WEEK1_DAY6_MATERIAL_EXPORT_PACKET_PATH, { force: true });
    }
  });

  afterAll(() => {
    writeGavanWeek1Day6MaterialExportPacket(candidate(), seed(), day5Export(), {
      generatedAt: GENERATED_AT,
      targetPath: GAVAN_WEEK1_DAY6_MATERIAL_EXPORT_PACKET_PATH,
    });
  });

  it('builds a non-live reviewer export packet from the day 6 material candidate', () => {
  ...
  it('summarizes blocks phrases quick replies missing words repairs pronunciation recall and quiz intent', () => {
    const packet = buildGavanWeek1Day6MaterialExportPacket(candidate(), seed(), day5Export(), {
      generatedAt: GENERATED_AT,
    });
    ...
    expect(packet.reviewerSummary).toEqual(expect.objectContaining({
      ...
      repairItemCount: 4,
      pronunciationPlaceholderCount: 4,
  ...
  it('includes pass quality gates for the valid day 6 candidate', () => {
    const packet = buildGavanWeek1Day6MaterialExportPacket(candidate(), seed(), day5Export(), {
      generatedAt: GENERATED_AT,
    });
  ...
  it('includes a compact human-readable preview of day 6', () => {
    const sourceCandidate = candidate();
    const packet = buildGavanWeek1Day6MaterialExportPacket(sourceCandidate, seed(), day5Export(), {
      generatedAt: GENERATED_AT,
    });
    ...
    expect(packet.preview.blocks).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ type: 'pronunciation_shadow' }),
    ...
    expect(packet.preview.pronunciationPlaceholderLine).toContain('not built');
  ...
  it('blocks export quality when candidate gates fail instead of approving it', () => {
    ...
    const brokenCandidate = {
      ...
      quickReplyItems: [
        {
          ...sourceCandidate.quickReplyItems[0],
          options: [sourceCandidate.quickReplyItems[0].correctEnglish],
        },
      ],
      missingWordItems: [
        {
          ...sourceCandidate.missingWordItems[0],
          sentenceWithBlank: sourceCandidate.missingWordItems[0].targetEnglish,
          options: [sourceCandidate.missingWordItems[0].correctToken],
        },
      ],
      repairItems: [
        {
      ...
      pronunciationPlaceholders: [
        {
          ...sourceCandidate.pronunciationPlaceholders[0],
          scoringStatus: 'ready',
    ...
    const packet = buildGavanWeek1Day6MaterialExportPacket(brokenCandidate as any, seed(), day5Export(), {
      generatedAt: GENERATED_AT,
    });
    const failedGates = packet.qualityGates.filter((gate) => gate.status === 'fail');

    expect(packet.status).toBe('day6_material_export_blocked');
  ...
  it('writes deterministic export JSON only under temp or report roots', () => {
    const result = writeGavanWeek1Day6MaterialExportPacket(candidate(), seed(), day5Export(), {
      generatedAt: GENERATED_AT,
      targetPath: GAVAN_WEEK1_DAY6_MATERIAL_EXPORT_PACKET_PATH,
    });
    ...
    expect(existsSync(GAVAN_WEEK1_DAY6_MATERIAL_EXPORT_PACKET_PATH)).toBe(true);

    const parsed = JSON.parse(readFileSync(GAVAN_WEEK1_DAY6_MATERIAL_EXPORT_PACKET_PATH, 'utf8'));
    expect(parsed.kind).toBe('gavan_week1_day6_material_export_packet');
    expect(parsed.status).toBe('day6_material_export_not_live');
  });

  it('rejects app tools tests and root config write targets', () => {
    const appResult = writeGavanWeek1Day6MaterialExportPacket(candidate(), seed(), day5Export(), {
      generatedAt: GENERATED_AT,
      targetPath: path.join(process.cwd(), 'app', 'gavan-week1-day6-export.json'),
    });
    const toolsResult = writeGavanWeek1Day6MaterialExportPacket(candidate(), seed(), day5Export(), {
      generatedAt: GENERATED_AT,
      targetPath: path.join(process.cwd(), 'tools', 'gavan-week1-day6-export.json'),
    });
    const testsResult = writeGavanWeek1Day6MaterialExportPacket(candidate(), seed(), day5Export(), {
      generatedAt: GENERATED_AT,
      targetPath: path.join(process.cwd(), 'tests', 'gavan-week1-day6-export.json'),
    });
  ...
  it('does not import or mutate live app routes catalog quiz UI storage audio scoring or navigation', () => {
    const source = readFileSync(
      path.join(process.cwd(), 'tools', 'personal_plan_gavan_week1_day6_material_export_packet.ts'),
      'utf8',
    );
