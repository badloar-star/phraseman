import { retrieveTheoryExemplars, validateTheoryArtifact, validateTheoryExemplarRegistry } from './theory_generation';
...
describe('evidence-backed lesson theory', () => {
  ...
  it('validates and retrieves 2-4 relevant versioned English exemplars', () => {
    expect(validateTheoryExemplarRegistry(registry)).toEqual([]);
    const result = retrieveTheoryExemplars(registry, { objective: 'Ask for directions and understand instructions', approvedTargetPhrases: ['Could you show me the way?', 'Turn left here.'] });
  ...
  it('requires rule, examples, mistakes, mini-check and concrete evidence references', () => {
    ...
    expect(validateTheoryArtifact(artifact, { allowedPhraseIds: ['p1'], allowedExemplarFragmentIds: ['f1'] })).toEqual([]);
    artifact.result.rules[0].evidenceRefs = ['exemplar:unknown'];
    expect(validateTheoryArtifact(artifact, { allowedPhraseIds: ['p1'], allowedExemplarFragmentIds: ['f1'] })).toContain('theory_evidence_reference_invalid');
  });
  ...
  it('stops for human adjudication when retrieved exemplars conflict', () => {
    ...
    expect(retrieveTheoryExemplars(conflicting, { objective: 'directions', approvedTargetPhrases: ['Turn left.'] })).toMatchObject({ state: 'review_required', reason: 'exemplar_conflict', conflictingRuleKeys: ['imperative'] });
  });
  ...
  it('rejects empty theory prose even when evidence references are syntactically valid', () => {
    ...
    expect(validateTheoryArtifact(artifact, { allowedPhraseIds: ['p1'], allowedExemplarFragmentIds: [] })).toEqual(expect.arrayContaining(['theory_rules_content_invalid', 'theory_examples_content_invalid', 'theory_commonMistakes_content_invalid', 'theory_miniCheck_content_invalid']));
  });
});
