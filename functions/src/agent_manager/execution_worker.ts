...
import { SUPPORT_OPENAI_API_KEY } from '../support_inbox';
import { parseManagerTaskDraft, parseManagerTaskResult, type ManagerTaskResult } from './contracts';
import { createAnalyticsExecutionHandler } from './analytics_execution';
import { parseExecutionJob, type ExecutionJob, type ExecutionJobScope } from './execution_contracts';
import { createSupportExecutionHandler } from './support_execution';
...
export interface ExecutionWorkerDocument { readonly id: string; readonly data: Record<string, unknown>; }
export interface ExecutionWorkerTransaction {
  get(path: string): Promise<ExecutionWorkerDocument | null>;
...
export interface ExecutionWorkerRepository {
  get(path: string): Promise<ExecutionWorkerDocument | null>;
  listEligibleJobs(allowedScopes?: readonly ExecutionJobScope[]): Promise<readonly ExecutionWorkerDocument[]>;
  runTransaction<T>(body: (transaction: ExecutionWorkerTransaction) => Promise<T>): Promise<T>;
}

type WorkerTask = Readonly<{ taskId: string; revision: number; status: 'queued'; scope: ExecutionJobScope; }>;
export type ExecutionClaim = Readonly<{ jobId: string; job: ExecutionJob; leasedAtMs: number; leaseUntilMs: number; }>;
type Claim = ExecutionClaim;
export type ExecutionWorkerOutcome = 'succeeded' | 'cancelled' | 'failed' | 'skipped';
/** Return null to retain the bounded built-in handler for scopes the adapter does not own. */
export type ExecutionCompletion = Readonly<{ result: ManagerTaskResult }>;
export type ExecutionClaimHandler = (claim: ExecutionClaim, nowMs: number) => Promise<Exclude<ExecutionWorkerOutcome, 'skipped'> | ExecutionCompletion | null>;
...
function parseWorkerTask(value: unknown): WorkerTask | null {
  const task = object(value);
  if (!task || task.schemaVersion !== 1 || task.status !== 'queued' || task.result !== null) return null;
  const revision = integer(task.revision, 1);
  if (!revision || typeof task.assignedAgentId !== 'string' || typeof task.createdByUid !== 'string') return null;
  try {
    ...
    if (draft.allowedScope !== 'support_draft' && draft.allowedScope !== 'analysis_only' && draft.allowedScope !== 'report_triage') return null;
...
function isClaimable(job: ExecutionJob, nowMs: number): boolean {
  return job.state === 'queued' || (job.state === 'leased' && job.leaseUntilMs !== null && job.leaseUntilMs <= nowMs);
}

function terminalJobUpdate(state: 'cancelled' | 'failed', nowMs: number): Record<string, unknown> {
  return { state, leaseUntilMs: null, updatedAtMs: nowMs, finishedAtMs: nowMs, outputRef: null, outputHash: null };
}

function taskEvent(taskId: string, revision: number, fromStatus: 'queued' | 'in_progress', toStatus: 'in_progress' | 'needs_review', scope: ExecutionJobScope, nowMs: number): Record<string, unknown> {
  return {
...
function boundedResult(scope: ExecutionJobScope): ManagerTaskResult {
  if (scope === 'report_triage') {
    return parseManagerTaskResult({
...
function outputHashFor(job: ExecutionJob): string {
  return sha256(`agent-manager-review-v1:${job.scope}:${job.taskId}:r${job.taskRevision}`);
}

function matchesLiveClaim(currentJob: ExecutionJob, claim: Claim, nowMs: number): boolean {
...
async function claimJob(repository: ExecutionWorkerRepository, jobId: string, nowMs: number): Promise<Claim | null | 'cancelled' | 'failed'> {
  return repository.runTransaction(async (transaction) => {
    const document = await transaction.get(jobPath(jobId));
    if (!document) return null;
    let job: ExecutionJob;
    try { job = parseExecutionJob(document.data); } catch { return null; }
    ...
    const leased: ExecutionJob = Object.freeze({ ...job, state: 'leased', attempts: job.attempts + 1, leasedAtMs: nowMs, leaseUntilMs, updatedAtMs: nowMs });
...
async function completeClaim(repository: ExecutionWorkerRepository, claim: Claim, nowMs: number, completion?: ExecutionCompletion): Promise<Exclude<ExecutionWorkerOutcome, 'skipped'>> {
  return repository.runTransaction(async (transaction) => {
    const document = await transaction.get(jobPath(claim.jobId));
    if (!document) return 'cancelled';
    let currentJob: ExecutionJob;
    try { currentJob = parseExecutionJob(document.data); } catch { return 'cancelled'; }
    const completionNow = Date.now();
    if (!matchesLiveClaim(currentJob, claim, completionNow)) return 'cancelled';
    ...
    let result: ManagerTaskResult;
    try { result = parseManagerTaskResult(completion?.result ?? boundedResult(task.scope)); } catch {
...
export async function executeOneAgentManagerJob(repository: ExecutionWorkerRepository, jobId: string, nowMs: number, handler?: ExecutionClaimHandler): Promise<Readonly<{ jobId: string; outcome: ExecutionWorkerOutcome }>> {
  const claim = await claimJob(repository, jobId, nowMs);
  if (claim === null) return Object.freeze({ jobId, outcome: 'skipped' });
  if (claim === 'cancelled') return Object.freeze({ jobId, outcome: 'cancelled' });
  if (claim === 'failed') return Object.freeze({ jobId, outcome: 'failed' });
  const handled = handler ? await handler(claim, nowMs) : null;
  if (handled && typeof handled === 'object') return Object.freeze({ jobId, outcome: await completeClaim(repository, claim, nowMs, handled) });
  return Object.freeze({ jobId, outcome: handled ?? await completeClaim(repository, claim, nowMs) });
}

export async function executeEligibleAgentManagerJobs(repository: ExecutionWorkerRepository, nowMs: number, limit = MAX_BATCH, handler?: ExecutionClaimHandler, allowedScopes?: readonly ExecutionJobScope[]): Promise<Readonly<{ attempted: number; succeeded: number; cancelled: number; failed: number }>> {
  const jobs = (await repository.listEligibleJobs(allowedScopes))
    .filter((document) => {
      if (!allowedScopes) return true;
      try { return allowedScopes.includes(parseExecutionJob(document.data).scope); } catch { return false; }
    })
    .slice(0, Math.max(1, Math.min(MAX_BATCH, limit)));
  let succeeded = 0; let cancelled = 0; let failed = 0;
  for (const job of jobs) {
    const outcome = await executeOneAgentManagerJob(repository, job.id, nowMs, handler);
    if (outcome.outcome === 'succeeded') succeeded += 1;
    if (outcome.outcome === 'cancelled') cancelled += 1;
    if (outcome.outcome === 'failed') failed += 1;
  }
  return Object.freeze({ attempted: jobs.length, succeeded, cancelled, failed });
}

function firestoreRepository(db: FirebaseFirestore.Firestore): ExecutionWorkerRepository {
  return {
    get: async (path) => {
...
export const agentManagerRunAnalyticsWorker = onSchedule({ schedule: 'every 5 minutes', region: REGION, timeoutSeconds: 60, memory: '256MiB', maxInstances: 1 }, async () => {
  const db = admin.firestore();
  await executeEligibleAgentManagerJobs(firestoreRepository(db), Date.now(), 1, createAnalyticsExecutionHandler(db), ['analysis_only']);
});

export const agentManagerRunReportTriageWorker = onSchedule({ schedule: 'every 5 minutes', region: REGION, timeoutSeconds: 60, memory: '256MiB', maxInstances: 1 }, async () => {
...
export const agentManagerRunBoundedExecutionWorker = onSchedule({ schedule: 'every 5 minutes', region: REGION, timeoutSeconds: 60, memory: '256MiB', maxInstances: 1, secrets: [SUPPORT_OPENAI_API_KEY] }, async () => {
  const apiKey = String(SUPPORT_OPENAI_API_KEY.value() || '').trim();
  if (!apiKey) throw new Error('agent manager support draft key is unavailable');
  const db = admin.firestore();
  // One provider request maximum per invocation keeps the 60-second deadline honest.
  await executeEligibleAgentManagerJobs(firestoreRepository(db), Date.now(), 1, createSupportExecutionHandler(db, apiKey), ['support_draft']);
});
