...
const question_grounding_1 = require("./content_factory/question_grounding");
const question_batch_ledger_1 = require("./content_factory/question_batch_ledger");
const question_artifacts_1 = require("./content_factory/question_artifacts");
...
function resolveContentStageCount(kind, requestedCount, grounding = null) {
    ...
    if (kind === 'lesson_phrases')
        return 50;
    if (kind === 'challenge_questions')
        return 10;
    if (kind === 'challenge_question_replacement')
        return 1;
...
exports.adminRunContentStage = (0, https_1.onCall)({ region: REGION, enforceAppCheck: callable_options_1.ENFORCE_APP_CHECK, secrets: [exports.CONTENT_STAGE_OPENAI_API_KEY], timeoutSeconds: 300, memory: '1GiB' }, async (request) => {
    if (!request.auth?.token?.admin || !(0, roles_1.hasAdminRole)(request.auth.token.adminRole) || !(0, permissions_1.hasPermission)(request.auth.token.adminRole, 'content.draft.write'))
    ...
    try {
        ...
        if (stage.kind === 'challenge_questions') {
            ...
            const prerequisite = prerequisiteSnapshot.data() ?? {};
            const expectedKind = 'challenge_topic';
            if (prerequisite.kind !== expectedKind)
                throw new https_1.HttpsError('failed-precondition', 'studio_topic_prerequisite_kind_invalid');
            try {
                grounding = await (0, question_grounding_1.loadApprovedTopicGrounding)(admin.storage().bucket(), { stageId: prerequisiteSnapshot.id, artifactId: String(prerequisite.artifactId ?? ''), kind: expectedKind, state: String(prerequisite.state ?? ''), cefr: String(prerequisite.cefr ?? ''), objectPath: String(prerequisite.objectPath ?? ''), contentHash: String(prerequisite.contentHash ?? ''), objectGeneration: String(prerequisite.objectGeneration ?? '') });
            }
            catch (error) {
        ...
        if (stage.kind === 'challenge_question_replacement') {
            ...
            const expectedBatchKind = 'challenge_questions';
            ...
            const batch = await (0, question_grounding_1.loadQuestionBatchForReview)(admin.storage().bucket(), { stageId: batchSnapshot.id, artifactId: String(batchStage.artifactId ?? ''), kind: expectedBatchKind, state: String(batchStage.state ?? ''), objectPath: String(batchStage.objectPath ?? ''), contentHash: String(batchStage.contentHash ?? ''), objectGeneration: String(batchStage.objectGeneration ?? ''), groundingReceipt: batchStage.groundingReceipt });
            ...
            const originalKey = (0, question_artifacts_1.questionSemanticKey)(originalQuestion);
