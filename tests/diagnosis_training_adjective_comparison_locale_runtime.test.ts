...
describe('adjective comparison diagnosis training locale runtime', () => {
  it('keeps planned locale fallback explanations explicit and audit-safe', () => {
    ...
    expect(training!.analyticsEvents.recovery).toBe('diagnosis_training_adjective_comparison_recovery');
  });

  it('does not use the legacy telemetry fallback key in this training file', () => {
    const source = fs.readFileSync(path.join(ROOT, 'app', 'diagnosis_training_adjective_comparison.ts'), 'utf8');
