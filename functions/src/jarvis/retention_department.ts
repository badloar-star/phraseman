...
export function runRetentionDepartment(input: RunRetentionDepartmentInput): RunRetentionDepartmentResult {
  ...
  const finding = !trustworthy
    ? 'Не удалось прочитать активность пользователей — источник недоступен, удержание неизвестно.'
    : month === 0
      ? 'За последний месяц активных пользователей не было — удержание пока не о чем считать.'
      : !bigEnough
        ? `Активных за месяц ${month} — слишком мало, чтобы судить об удержании по долям.`
        : weak
          ? `Из ${month} заходивших за месяц на этой неделе вернулись только ${week} — это ${percent}%.`
          : `Из ${month} заходивших за месяц на этой неделе вернулись ${week} — это ${percent}%.`;
  ...
  const decision = buildDecision({
    ...
    finding,
    hypothesis: weak
      ? 'Вероятная причина — людям нечего делать после первых уроков либо нет повода вернуться.'
