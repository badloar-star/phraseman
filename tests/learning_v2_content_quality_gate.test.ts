...
type LocaleCopy = Readonly<Record<'ru' | 'uk' | 'es' | 'pt-BR' | 'vi' | 'id' | 'tr' | 'pl', string>>;
...
const LOCALE_COPY: LocaleCopy = Object.freeze({
  ...
  'pt-BR': 'Em português a pessoa já aparece dentro de estou. Em inglês é preciso dizer I am here. A forma I here fica quebrada porque falta o verbo que liga as duas partes. I am here é a construção completa e natural. Assim se descreve uma situação presente sem acrescentar outro verbo.',
...
function englishLocalized(value: string): LocalizedSource {
  return localized({ ru: value, uk: value, es: value, 'pt-BR': value, vi: value, id: value, tr: value, pl: value });
}

function candidate(): SessionSource {
  const page = (kind: 'concept' | 'formula' | 'trap', ordinal: number) => ({
    kind,
    title: fieldLocalized(`Живой заголовок ${ordinal} раскрывает одну мысль`, `one concrete idea ${ordinal}`),
    body: denseLocalized(LOCALE_COPY),
    question: {
      prompt: fieldLocalized(`Какая фраза звучит правильно ${ordinal}?`, `which phrase is correct ${ordinal}?`),
      choices: [
  ...
  const words = ['ready', 'here', 'fine', 'calm', 'busy', 'early', 'safe', 'sure', 'warm', 'cold', 'tired', 'happy', 'hungry', 'quiet', 'free'];
  return {
    ...
    summary: fieldLocalized('Живые фразы о состоянии и месте', 'living phrases about state and place'),
    learningGoal: fieldLocalized('Говорить о себе через I am', 'speak about yourself with I am'),
      ...
      words: [
        {
          ...
          category: 'pronoun',
          distractors: [
            { value: 'me', reasonCode: 'wrong_pronoun', why: 'Me является формой дополнения, а здесь нужно подлежащее I.' },
            { value: 'my', reasonCode: 'wrong_pronoun', why: 'My показывает принадлежность и не может заменить подлежащее I.' },
            { value: 'mine', reasonCode: 'wrong_pronoun', why: 'Mine заменяет принадлежащий предмет, а не говорящего человека.' },
          ],
        },
        {
          correct: 'am',
          category: 'to-be',
          distractors: [
            { value: 'is', reasonCode: 'agreement', why: 'Is сочетается с he, she или it, но не с подлежащим I.' },
            { value: 'are', reasonCode: 'agreement', why: 'Are сочетается с you, we или they, но не с подлежащим I.' },
            { value: 'be', reasonCode: 'infinitive', why: 'Be является начальной формой, а в готовой фразе с I нужна форма am.' },
          ],
        },
        {
          correct: word,
          category: 'adjective',
          distractors: [
            { value: `${word}s`, reasonCode: 'wrong_form', why: 'Окончание s здесь не нужно: признак после am не согласуется по числу.' },
            { value: `${word}ly`, reasonCode: 'wrong_form', why: 'Форма на ly обычно описывает способ действия, а здесь нужен признак состояния.' },
            { value: `very ${word}`, reasonCode: 'extra_word', why: 'Дополнительное very меняет исходную фразу и не требуется для ответа.' },
          ],
        },
      ],
...
function approvedReceipt(source: SessionSource): LearningV2ContentQualityReviewReceipt {
  return {
...
describe('Learning V2 strict content quality gate', () => {
  test('rejects thin template-like intro copy before review', () => {
    ...
    (source.introPages[0].body as { ru: string }).ru = 'I am here. Просто запомните эту формулу.';
    const report = evaluateLearningV2SessionContentQuality(source, approvedReceipt(source));
    expect(report.ok).toBe(false);
    expect(report.issues.map((issue: { code: string }) => issue.code)).toContain('intro_body_too_thin');
  });

  test('rejects weak phrase selection and shallow distractors', () => {
    const source = candidate();
    (source.phrases[0] as { english: string }).english = 'Me too';
    (source.phrases[0].words[0].distractors[0] as { why: string }).why = 'Неверно.';
    const report = evaluateLearningV2SessionContentQuality(source, approvedReceipt(source));
    expect(report.ok).toBe(false);
    expect(report.issues.map((issue: { code: string }) => issue.code)).toEqual(
      expect.arrayContaining(['phrase_not_standalone', 'distractor_reason_too_thin']),
    );
  });
  ...
  test('rejects missing, stale, self-approved, or incomplete review receipts', () => {
    ...
    (receipt.phraseSelection as { reviewerId: string }).reviewerId = receipt.introStyle.reviewerId;
    delete (receipt.localeAuthorship as unknown as Record<string, unknown>).tr;
    const codes = evaluateLearningV2SessionContentQuality(source, receipt).issues.map((issue: { code: string }) => issue.code);
  ...
  test('allows gold-style material only after every automatic and human gate passes', () => {
    const source = candidate();
    const receipt = approvedReceipt(source);
    expect(evaluateLearningV2SessionContentQuality(source, receipt)).toEqual({ ok: true, issues: [] });
    expect(() => assertLearningV2SessionContentQuality(source, receipt)).not.toThrow();
  });
});
