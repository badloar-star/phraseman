...
//
// Гипотеза G: decoy-эффект. МЕЖДУ карточками «Год» и «Месяц» показываем
// НЕВЫБИРАЕМУЮ полноразмерную карточку-якорь «6 месяцев» с ценой ≈ 85% от
// годовой — визуально она выглядит как обычный план, и рядом с ней годовой
// план выглядит заметно выгоднее. Якорь — не план: не нажимается, не попадает
// в selectPlan/покупку, дефолтный выбор остаётся годовым. Если число из цены
// стора не парсится — якорь скрыт.
// ════════════════════════════════════════════════════════════════════════════
...
export function PaywallGView({ entry }: { readonly entry: PaywallEntry }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      // некритично
      DebugLogger.error('paywall_g:tags', e instanceof Error ? e : new Error(String(e)), 'warning');
    }
      try {
      ...
      // некритично
      DebugLogger.error('paywall_g:m', e instanceof Error ? e : new Error(String(e)), 'warning');
    }
      try {
      ...
      // некритично
      DebugLogger.error('paywall_g:prof', e instanceof Error ? e : new Error(String(e)), 'warning');
    }
    })();
    ...
    try {
      ...
      // некритично
      DebugLogger.error('paywall_g:dayHash', e instanceof Error ? e : new Error(String(e)), 'warning');
    }
    return () => { dead = true; };
  }, [source, ctx, lang]);
  ...
  // Якорь-приманка: 85% от годовой цены в той же валюте; не парсится → скрыт.
  const decoyPrice = p.yearlyPrice ? computeDecoyPriceString(p.yearlyPrice) : null;
          ...
          >
            ...
            <PaywallEntrance index={2}>
              ...
              {p.offeringsFailed ? (
                ...
                /* Якорь-приманка «6 месяцев» — полноразмерная display-only
                   карточка МЕЖДУ «Годом» и «Месяцем» внутри PaywallPlanCards,
                   БЕЗ onSelect: она не план и не участвует в выборе/покупке.
                   Дефолт остаётся годовой. */
                <PaywallPlanCards
                  lang={lang as Lang}
                  chrome={chrome}
                  selected={p.selected}
...
export default function PaywallGRoute() {
  return <DirectPaywallRoute render={(entry) => <PaywallGView entry={entry} />} />;
}
