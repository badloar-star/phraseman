...
function checkCapabilityParity() {
  ...
  assert(ADMIN_CAPABILITY_REGISTRY.filter((item) => item.nativeRoute).length === 17, 'native capability count drifted from 17');
  assert(ADMIN_CAPABILITY_REGISTRY.filter((item) => !item.nativeRoute).length === 42, 'fallback capability count drifted from 42');
