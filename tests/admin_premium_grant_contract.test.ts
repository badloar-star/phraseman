...
describe("admin premium grant contract", () => {
  ...
  it("routes admin VIP grants through the protected callable with verified responses", () => {
    const grants = [
      between(
        "window.grantHelperPlus = async function",
        "window.editShards = async function",
      ),
      between(
        "async function grantAdminPremiumChoice",
        "window.revokeAdminVip = async function",
      ),
      between(
        "window.bulkGrantPremium = async function",
        "window.bulkBanUsers = async function",
      ),
    ];
  ...
  it("keeps forever and bulk grants as admin_vip, not annual Premium", () => {
    const timedGrant = between(
      "async function grantAdminPremiumChoice",
      "window.revokeAdminVip = async function",
    );
    const bulkGrant = between(
      "window.bulkGrantPremium = async function",
      "window.bulkBanUsers = async function",
    );
  ...
  it("does not allow new manual admin_grant writes through the Premium debug editor", () => {
    expect(html).toMatch(
      /allowed\s*=\s*new Set\(\['monthly', 'annual', 'yearly', 'null', ''\]\)/,
    );
    expect(html).toContain("Для админской выдачи используй Plus");
    expect(html).not.toContain(
      "new Set(['monthly', 'annual', 'yearly', 'admin_grant'",
    );
  });
  ...
  it("extends timed VIP from the current VIP expiry only, never from real Premium expiry", () => {
    ...
    expect(html).not.toMatch(
      /vip_until \|\| \(rawProg \|\| \{\}\)\.premium_expiry/,
    );
  });

  it("keeps a protected compatibility migration for legacy admin_grant documents", () => {
    const repair = between(
      "window.repairAdminGrantPremiumCompat = async function",
      "function appMessageClampDays",
    );
    expectNoDirectSensitiveWrite(repair);
    expect(repair).toContain(
      "where('progress.premium_plan', '==', 'admin_grant')",
    );
    ...
    expect(repair).toContain(
      "createAdminCommandId('premium_migration_request')",
    );
    expect(repair).toContain(
      "createAdminCommandId('premium_migration_operation')",
    );
