...
function PersonalPlanExerciseScreen() {
  ...
  const { lang } = useLang();
  // зачем: владелец 2026-08-23 — трата энергии за ОШИБКУ удалена целиком во всех
  // четырёх режимах задания (выбор, listen-build, recall, произношение).
  // Заряд платится один раз за вход в задание (эффект старта выше).
  ...
  const block = useMemo<PlanExerciseBlock>(() => ({
    ...
    recoveryPolicy: isPronunciationMode ? 'none' : 'capture_mistake',
  ...
  const capturePlanWrong = useCallback((input: {
    itemId: string;
    expected: string;
    selected: string;
    facet: 'meaning' | 'form' | 'word_order' | 'missing_token' | 'listening' | 'pronunciation';
    distractors?: readonly string[];
  }) => {
    if (studyTarget !== 'en' && studyTarget !== 'fr') return;
    void captureCurrentAccountObjectiveAttempt({
      attemptId: `personal-plan:${planInstanceId}:${planTaskId}:${input.itemId}:${input.selected}`,
      studyTarget,
      verdict: 'wrong',
      objective: true,
      content: {
        sourceKind: 'personal_plan',
        sourceId: `${planInstanceId}:${input.itemId}`,
        canonicalTarget: input.expected,
        lessonId: String(lessonId),
        tokens: input.expected.split(/\s+/),
        distractors: input.distractors,
      },
      facet: { kind: input.facet, expected: input.expected },
    }).catch(() => {});
  }, [lessonId, planInstanceId, planTaskId, studyTarget]);

  useEffect(() => {
    if (!item || !pronunciationScore || pronunciationScore.passed || !('targetText' in item)) return;
    capturePlanWrong({
      itemId: item.id,
      expected: item.targetText,
      selected: pronunciationScore.transcript || `score-${pronunciationScore.score}`,
      facet: 'pronunciation',
    });
  }, [capturePlanWrong, item, pronunciationScore]);
    ...
    if (attempt.shouldPersist) {
      if (!isCorrect) {
        capturePlanWrong({
          ...
          selected: answer,
          facet: isListeningMode ? 'listening' : isChoiceMode ? 'meaning' : 'missing_token',
          distractors: choiceOptions,
        });
      }
...
export default withPersonalPlanSunsetGuard(PersonalPlanExerciseScreen);
