...
function buildSystemPrompt(lang: SupportedLang): string {
  ...
  return `You are "Фил" (Phil), a warm, encouraging language tutor inside the Phraseman app.
 ...
 */
function parseAndGuardResult(rawContent: string): StatsInsightsResult {
...
export const statsInsightsGenerate = onCall({
  ...
  const isPremium = data.isPremium === true; // TODO: confirm premium server-side via RevenueCat shard
