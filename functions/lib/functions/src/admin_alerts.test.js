...
describe('formatContentReportAlert', () => {
    it('включает полный текст репорта: комментарий, контент, ответ юзера, экран, dataId, юзера', () => {
        const text = (0, admin_alerts_1.formatContentReportAlert)(baseReport);
    ...
    it('экранирует HTML в пользовательском тексте (parse_mode=HTML не должен ломаться)', () => {
        const text = (0, admin_alerts_1.formatContentReportAlert)({
            ...baseReport,
    ...
    it('не печатает строку «Ответ юзера» когда ответа нет', () => {
        const text = (0, admin_alerts_1.formatContentReportAlert)({ ...baseReport, userAnswer: '' });
        expect(text).not.toContain('Ответ юзера');
    });
    it('скрывает категорию free_text, но показывает кастомную', () => {
        expect((0, admin_alerts_1.formatContentReportAlert)(baseReport)).not.toContain('free_text');
        expect((0, admin_alerts_1.formatContentReportAlert)({ ...baseReport, category: 'wrong_translation' }))
            .toContain('wrong_translation');
    });
});
describe('formatContentReportAlertSafe', () => {
    it('обычный репорт проходит без обрезки', () => {
        const text = (0, admin_alerts_1.formatContentReportAlertSafe)(baseReport);
    ...
    it('гигантские поля ужимаются под лимит Telegram (4096)', () => {
        ...
        const text = (0, admin_alerts_1.formatContentReportAlertSafe)(huge);
        expect(text.length).toBeLessThanOrEqual(TELEGRAM_TEXT_LIMIT);
    ...
    it('держит лимит даже при патологическом экранировании (сплошные &)', () => {
        ...
        const text = (0, admin_alerts_1.formatContentReportAlertSafe)(nasty);
        expect(text.length).toBeLessThanOrEqual(TELEGRAM_TEXT_LIMIT);
    });
});
...
describe('isAuthFailureErrorDoc', () => {
    it('ловит auth-ошибки по feature и по context, не реагируя на прочие', () => {
        ...
        expect((0, admin_alerts_1.isAuthFailureErrorDoc)({ feature: 'lesson' })).toBe(false);
        expect((0, admin_alerts_1.isAuthFailureErrorDoc)({ context: 'payment:failed' })).toBe(false);
        expect((0, admin_alerts_1.isAuthFailureErrorDoc)({})).toBe(false);
        // Похожее, но не то: подстрока не считается совпадением.
        expect((0, admin_alerts_1.isAuthFailureErrorDoc)({ feature: 'oauth_provider' })).toBe(false);
    });
});
describe('nextAuthFailureSpikeState', () => {
    const base = { lastAlertedAt: 0, now: 1000000, threshold: 3 };
    it('первое событие открывает окно и НЕ алертит', () => {
        const res = (0, admin_alerts_1.nextAuthFailureSpikeState)({ ...base, window: undefined });
    ...
    it('алертит ровно при достижении порога, но не раньше', () => {
        ...
        const below = (0, admin_alerts_1.nextAuthFailureSpikeState)({ ...base, window: { since, count: 1 } });
        expect(below).toMatchObject({ shouldAlert: false, count: 2 });
        const atThreshold = (0, admin_alerts_1.nextAuthFailureSpikeState)({ ...base, window: { since, count: 2 } });
    ...
    it('сбрасывает окно старше часа — старые ошибки не копятся вечно', () => {
        ...
        const res = (0, admin_alerts_1.nextAuthFailureSpikeState)({ ...base, window: { since: stale, count: 999 } });
    ...
    it('считает stage и не даёт тегам раздувать документ', () => {
        const since = base.now - 60000;
        const counted = (0, admin_alerts_1.nextAuthFailureSpikeState)({
        ...
        for (let i = 0; i < 8; i += 1)
            full[`stage-${i}`] = 1;
        const overflow = (0, admin_alerts_1.nextAuthFailureSpikeState)({
        ...
        const existing = (0, admin_alerts_1.nextAuthFailureSpikeState)({
    ...
    it('переживает мусорное состояние из Firestore', () => {
        const res = (0, admin_alerts_1.nextAuthFailureSpikeState)({
            ...base,
...
//# sourceMappingURL=admin_alerts.test.js.map