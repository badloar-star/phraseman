...
const RULES = 'КОНТРАКТ: planId __PLAN__. РОВНО 6 фраз; id __PLAN___d<N>_p<K>; фраза 5-7 слов (естественность важнее длины — лимита нет, но не растягивай). У каждого слова РОВНО 5 дистракторов того же класса (POS строго из набора, НЕ выдумывать numeral/wh-word/auxiliary/phrase: число=determiner, where/how=adverb, there в there-is=existential, do-вспом=verb, инфинитив to=other, модал=modal, is/was/be=to-be), БЕЗ дублей, НЕ совпадают со словом. ПОЛИТИКА ДИСТРАКТОРОВ (опора правильности = показанный русский перевод фразы): для СМЫСЛОВЫХ слов (сущ/глагол/прилаг/наречия места-времени) дистракторы = ТО ЖЕ семантическое поле или антоним, ОДНОЗНАЧНО неверные против перевода (kitchen←bathroom/bedroom; open←close/lock; hot←cold/warm; today←tomorrow) — НЕ рандомные слова из чужих полей; для СЛУЖЕБНЫХ слов (артикли/be/do/can/местоимения/союзы — перевод их не различает) дистракторы ЛОМАЮТ грамматику (these+ед.число, an+согласная, has+I, May you в просьбе). ЗАПРЕЩЕНО: синонимы, неразличимые переводом (want/need, home/house, begin/start, could/can в просьбе); слова, видимые в этой же фразе; правильные ответы других слотов этой фразы. 6 vocab (каждое слово реально в english фраз В ТОЙ ЖЕ ФОРМЕ и УНИКАЛЬНО, example = точная фраза дня). 3 intro (how-экран с 2 examples). prerequisiteLessons НЕ содержит сам день и НЕ содержит уроки выше gate. constructions фразы — теги ТОЛЬКО из разрешённого набора для gate дня (не выше).';
