...
describe('quiz and challenge studio artifacts', () => {
    const topic = (stage) => ({ stage, result: { topicId: 'travel-basics', title: 'Travel basics', learningPromise: 'Choose natural phrases for common travel situations.', skillTags: ['travel', 'requests'], inclusions: ['transport', 'directions'], exclusions: ['rare aviation terms'], difficultyDistribution: { easy: 3, medium: 4, hard: 3 } } });
    it.each(['quiz_topic', 'challenge_topic'])('accepts an editable %s proposal', (stage) => {
        expect((0, quiz_challenge_artifacts_1.validateTopicArtifact)(topic(stage), { kind: stage, cefr: 'A2' })).toEqual([]);
    });
    const item = (index) => ({ id: `q-${index}`, prompt: `Как сказать вариант ${index}?`, choices: [`Correct ${index}`, `Wrong A ${index}`, `Wrong B ${index}`, `Wrong C ${index}`], correctIndex: 0, optionExplanations: [`Correct because ${index}.`, `Wrong form A ${index}.`, `Wrong meaning B ${index}.`, `Wrong grammar C ${index}.`], skillTag: index % 2 ? 'travel' : 'requests', difficulty: (index <= 3 ? 'easy' : index <= 7 ? 'medium' : 'hard'), sourcePhraseIds: [] });
    it.each(['quiz_questions', 'challenge_questions'])('accepts exactly ten fully explained %s', (stage) => {
        const grounding = { topic: topic(stage === 'quiz_questions' ? 'quiz_topic' : 'challenge_topic').result };
        expect((0, quiz_challenge_artifacts_1.validateQuestionBatchArtifact)({ stage, items: Array.from({ length: 10 }, (_, index) => item(index + 1)) }, { kind: stage, count: 10, grounding })).toEqual([]);
    });
    it('rejects duplicate choices, bad correct index, missing per-option explanations and repeated templates', () => {
        ...
        items[1] = { ...items[0], id: 'q-2', choices: ['Same', 'Same', 'Other', 'Last'], correctIndex: 4, optionExplanations: ['Only one'], skillTag: 'outside-topic' };
    ...
    it('accepts one independently grounded replacement and rejects a semantic clone', () => {
        ...
        const grounding = { replacementForQuestionId: 'q-1', originalQuestion: original, topic: topic('quiz_topic').result, previousQuestionKeys: [] };
        expect((0, quiz_challenge_artifacts_1.validateQuestionReplacementArtifact)({ stage: 'quiz_question_replacement', result: { replacementForQuestionId: 'q-1', item: replacement } }, { kind: 'quiz_question_replacement', grounding })).toEqual([]);
        expect((0, quiz_challenge_artifacts_1.validateQuestionReplacementArtifact)({ stage: 'quiz_question_replacement', result: { replacementForQuestionId: 'q-1', item: { ...original, id: 'clone' } } }, { kind: 'quiz_question_replacement', grounding })).toContain('question_replacement_duplicate');
    });
});
//# sourceMappingURL=quiz_challenge_artifacts.test.js.map