# Parse lesson 31 Complex Object markdown and emit TypeScript fragment for lessonCards[31]
import json
import pathlib
import re
...
def main() -> None:
    p = pathlib.Path(__file__).resolve().parent / "lesson_31_user.md"
    if not p.exists():
        raise SystemExit("Place scripts/lesson_31_user.md")
    ...
    lines.append("  },")
    out = pathlib.Path(__file__).resolve().parent / "_lesson_31_insert.fragment.txt"
    out.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print("Wrote", out, "lines", len(lines))

    jpath = pathlib.Path(__file__).resolve().parent / "lesson_31_cards.json"
