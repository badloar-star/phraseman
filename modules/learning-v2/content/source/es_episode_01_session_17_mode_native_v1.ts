...
// + СТАРТ ES §0/§10, открывает Главу 3 "Он, она, оно: предметы и ситуации"):
// испанская сессия 17 переписывается в mode-native формат, зеркаля код
// (не данные) сессий 10/11/13 — тот же класс сессии (kind: 'phrases', НЕТ
// новых слов). Все 15 фраз применяют es — третье лицо связки ser — к уже
// известным признакам (fácil/difícil/verdad/así/igual/importante/caro/
// bonito/bonita/rápido/rápida/único/única/verdadero), в явном контрасте с
// eres (сессия 9) и soy (сессия 1). Фразовые данные и 8-локальные разборы
// взяты из легаси без изменений (MODE_NATIVE_AUTHORING_CONTRACT.ru.md §7) —
// единственная причина отклонения легаси была отсутствие mode-native payload,
// а не качество фразовых данных.
//
...
// families (listen_choose/phrase_builder/context_gap_grammar/speed_match/
...
const phrases = ES_EPISODE_01_SESSION_17_PHRASES;
if (phrases.length !== 15) {
  throw new Error('es_session_17_mode_native_phrase_count_invalid');
}

const L = (source: LocalizedSource): LearningV2Localized<string> => expandLocalized(source);

// зачем эта функция (владелец, 2026-08-28, тот же класс бага сессий 5/6/8/9/
// 10/11): runtime-валидатор course_session_client_children_v1.ts принимает
// responseId только по ID_RE = /^[A-Za-z0-9][A-Za-z0-9._:-]{0,159}$/u — без
// диакритики. Фразы этой сессии переиспользуют reasonCode с диакритикой
// (fácil/difícil/único/verdadero/rápido), context_gap_grammar передаёт
// responseId напрямую в runtime.
...
// зачем 'en' вместо 'es' здесь (как в испанских сессиях 1-16): в испанском
...
function localizedPhraseField(
  ...
  if (!phrase?.localizedDetails) throw new Error(`es_session_17_phrase_details_missing:${phraseIndex}`);
  return Object.fromEntries(LEARNING_V2_INTERFACE_LOCALES.map((locale) => {
    ...
    if (!detail) throw new Error(`es_session_17_phrase_locale_missing:${phraseIndex}:${locale}`);
...
function localizedPhraseDistractorFeedback(
  ...
  return Object.fromEntries(LEARNING_V2_INTERFACE_LOCALES.map((locale) => {
    ...
    if (!reason) throw new Error(`es_session_17_phrase_distractor_missing:${phraseIndex}:${locale}:${value}`);
...
function phraseAudio(phraseIndex: number): LearningV2ModeAudioReferenceV1 {
  const phrase = phrases[phraseIndex]!;
  return authoredAudio(`es-e01-s17-phrase-${phraseIndex + 1}`, phrase.english);
}
function phraseSlowAudio(phraseIndex: number): LearningV2ModeAudioReferenceV1 {
  const phrase = phrases[phraseIndex]!;
  return authoredAudio(`es-e01-s17-phrase-${phraseIndex + 1}-slow`, phrase.english);
}
...
function contextGapLastWord(phraseIndex: number): LearningV2ModeNativePayloadV1 {
  ...
  return Object.freeze({
    ...
    testedDimension: `third_person_quality:${asciiId(phrase.id)}`,
    choiceFeedback: Object.freeze([
      ...
      ...lastWord.distractors.map((entry) => feedback(
        ...
        false,
        `third_person_quality:${asciiId(entry.reasonCode)}`,
...
// приём, что сессия 11): пять фраз с es, разбросанных по всей сессии
// (индексы 3/6/9/12/14) — широкий срез уже изученных es-оценок, а не
// смежный блок.
function speedMatchPhrases(pairIndices: readonly number[]): LearningV2ModeNativePayloadV1 {
  ...
  const ids = pairIndices.map((index) => `es-e01-s17-pair-${index + 1}`);
  return Object.freeze({
    family: 'speed_match',
...
// зачем именно эта раскладка 12 шагов (владелец, 2026-08-28, зеркало
...
export const ES_EPISODE_01_SESSION_17_MODE_NATIVE_PRACTICE_V1 = Object.freeze<readonly SessionModeNativePracticeSourceV1[]>([
...
if (ES_EPISODE_01_SESSION_17_MODE_NATIVE_PRACTICE_V1.length !== 12) {
  throw new Error('es_session_17_mode_native_practice_count_invalid');
}
