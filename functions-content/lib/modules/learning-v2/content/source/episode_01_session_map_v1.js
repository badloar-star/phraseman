...
exports.EPISODE_01_SESSION_MAP_V1 = exports.SESSION_KIND_FAMILIES = void 0;
exports.featuresTaughtBySession = featuresTaughtBySession;
/**
 ...
 */
exports.SESSION_KIND_FAMILIES = Object.freeze({
...
exports.EPISODE_01_SESSION_MAP_V1 = Object.freeze([
    ...
    // ── Глава 2. He / she / it + is ───────────────────────────────────────
    // Full B1 packet 01:09 opens the next prerequisite-safe agreement frame.
    { sessionOrdinal: 9, kind: 'words_then_phrases', title: 'He, she, it — is', teaches: ['copula_be'], builtOn: [1], recalls: [1, 4, 6, 7] },
    // Session 10 is a lexical extension of the already introduced I am / he, she, it is frames.
    { sessionOrdinal: 10, kind: 'words_then_phrases', title: 'Old, kind, funny — with is', teaches: [], builtOn: [1, 9], recalls: [1, 4, 6, 7, 9] },
    { sessionOrdinal: 11, kind: 'words_then_phrases', title: 'Smart, strong, quiet — with is', teaches: [], builtOn: [1, 9, 10], recalls: [1, 4, 6, 7, 9, 10] },
    { sessionOrdinal: 12, kind: 'words_then_phrases', title: 'Loud, friendly, helpful — with is', teaches: [], builtOn: [1, 9, 10, 11], recalls: [1, 4, 9, 10, 11] },
    { sessionOrdinal: 13, kind: 'words_then_phrases', title: 'Small, big, clean — with it is', teaches: [], builtOn: [1, 9, 10, 11, 12], recalls: [1, 9, 10, 11, 12] },
    ...
    { sessionOrdinal: 17, kind: 'words_then_phrases', title: 'You, we, they — are', teaches: ['second_person', 'plural_reference', 'affirmative_you_we_they'], builtOn: [9, 16], recalls: [9, 13, 14, 15, 16] },
    { sessionOrdinal: 18, kind: 'words_then_phrases', title: 'Wrong, early, lucky — affirmative retrieval', teaches: [], builtOn: [1, 9, 17], recalls: [1, 9, 13, 14, 15, 17] },
    { sessionOrdinal: 19, kind: 'words_then_phrases', title: 'Together, alone, nearby — affirmative contrast', teaches: [], builtOn: [1, 9, 17, 18], recalls: [1, 9, 13, 14, 15, 17, 18] },
    { sessionOrdinal: 20, kind: 'words_then_phrases', title: 'Lost, prepared, careful — guided application', teaches: [], builtOn: [1, 9, 17, 18, 19], recalls: [1, 9, 17, 18, 19] },
    { sessionOrdinal: 21, kind: 'words_then_phrases', title: 'Inside, outside, upstairs — diagnostic repair', teaches: [], builtOn: [1, 9, 17, 18, 19, 20], recalls: [1, 9, 17, 18, 19, 20] },
    { sessionOrdinal: 22, kind: 'words_then_phrases', title: 'Rich, poor, famous — listening retrieval', teaches: [], builtOn: [1, 9, 17, 18, 19, 20, 21], recalls: [1, 9, 17, 18, 19, 20, 21] },
    { sessionOrdinal: 23, kind: 'voice', title: 'You, we, they — вслух', teaches: [], builtOn: [17, 22], recalls: [17, 19, 20, 22] },
    ...
    { sessionOrdinal: 25, kind: 'words_then_phrases', title: 'I am, she is, they are', teaches: ['full_form_choice'], builtOn: [1, 9, 17, 24], recalls: [17, 19, 22, 23] },
    { sessionOrdinal: 26, kind: 'words_then_phrases', title: 'Bored, confused, worried', teaches: [], builtOn: [25], recalls: [17, 22, 25] },
    { sessionOrdinal: 27, kind: 'phrases', title: 'Awake, asleep, available', teaches: [], builtOn: [25, 26], recalls: [22, 25, 26] },
    { sessionOrdinal: 28, kind: 'phrases', title: 'Correct, certain, serious', teaches: [], builtOn: [25, 27], recalls: [20, 25, 27] },
    { sessionOrdinal: 29, kind: 'phrases', title: 'Local, foreign, online', teaches: [], builtOn: [25, 28], recalls: [21, 25, 28] },
    { sessionOrdinal: 30, kind: 'recall', title: 'Три факта из памяти', teaches: ['three_fact_retrieval'], builtOn: [29], recalls: [9, 17, 25, 29] },
    { sessionOrdinal: 31, kind: 'voice', title: 'Equal, similar, separate — вслух', teaches: [], builtOn: [30], recalls: [23, 29, 30] },
    { sessionOrdinal: 32, kind: 'checkpoint', title: 'Am, is, are — проверка', teaches: [], builtOn: [25, 26, 27, 28, 29, 30, 31], recalls: [25, 26, 27, 28, 29, 30, 31] },
    // ── Глава 5. Утвердительное you are ──────────────────────────────────
    // Только утверждение. Вопрос и отрицание остаются в эпизоде 2.
    { sessionOrdinal: 33, kind: 'words_then_phrases', title: 'I’m — короткая форма', teaches: ['contraction_im'], builtOn: [32], recalls: [25, 30, 32] },
...
function featuresTaughtBySession(sessionOrdinal) {
    ...
    for (const entry of exports.EPISODE_01_SESSION_MAP_V1) {
        if (entry.sessionOrdinal > sessionOrdinal)
            break;
...
//# sourceMappingURL=episode_01_session_map_v1.js.map