...
// + СТАРТ ES §0/§10, Rules §4.1, Глава 2 "Ты: вопрос"): испанская voice-
// сессия 15 переписывается в mode-native формат, зеркаля КОД (не данные)
// es_episode_01_session_07_mode_native_v1.ts — та же voice-механика. В
// отличие от word-first сессий voice-сессия НЕ вводит новых слов и не имеет
// newVocabulary — она применяет 15 уже утверждённых вопросительных фраз из
// сессий 10/12/14 (es_episode_01_session_15_phrases_v1.ts переиспользует их
// целиком по id через findPhrase(), полностью локализованные) в новых
// interaction-контекстах.
//
// зачем families здесь ТОЛЬКО listen_choose/listen_build_dictation/
// scripted_repeat_compare (владелец, 2026-08-27, ИМЕННО ТА ошибка, которую
// нашла и исправила сессия 7 в легаси voiceSteps() — 'sound_contrast' была
// снята с активного authoring решением владельца 2026-08-25,
// MODE_NATIVE_AUTHORING_CONTRACT.ru.md §2, "не может назначаться новой
// learner interaction"): этот файл НИКОГДА не использует sound_contrast —
// только шесть утверждённых families, здесь конкретно три голосовые.
const phrases = ES_EPISODE_01_SESSION_15_VOICE_PHRASES;
if (phrases.length !== 15) {
  throw new Error('es_session_15_mode_native_phrase_count_invalid');
}

const L = (source: LocalizedSource): LearningV2Localized<string> => expandLocalized(source);

// зачем эта функция (владелец, 2026-08-27, тот же класс бага сессий 5/6/8/9-14):
...
// зачем 'en' вместо 'es' здесь (как в испанских сессиях 1-14): в испанском
...
// Эти фразы унаследованы из сессий 10/12/14 через findPhrase() целиком,
// включая уже готовый localizedDetails.
function localizedPhraseField(
  phraseIndex: number,
  field: 'meaning' | 'explanation',
): LearningV2Localized<string> {
  const phrase = phrases[phraseIndex];
  if (!phrase?.localizedDetails) throw new Error(`es_session_15_phrase_details_missing:${phraseIndex}`);
  return Object.fromEntries(LEARNING_V2_INTERFACE_LOCALES.map((locale) => {
    ...
    if (!detail) throw new Error(`es_session_15_phrase_locale_missing:${phraseIndex}:${locale}`);
...
function localizedPhraseDistractorFeedback(
  ...
  return Object.fromEntries(LEARNING_V2_INTERFACE_LOCALES.map((locale) => {
    ...
    if (!reason) throw new Error(`es_session_15_phrase_distractor_missing:${phraseIndex}:${locale}:${value}`);
...
// зачем audio id namespace 's15' даже для переиспользованных фраз сессий
// 10/12/14 (владелец, 2026-08-27, тот же приём, что и сессия 7 для
// сессий 1-6): аудио этой сессии — самостоятельные logical targets, а не
// алиасы на чужие clip id.
function phraseAudio(phraseIndex: number): LearningV2ModeAudioReferenceV1 {
  const phrase = phrases[phraseIndex]!;
  return authoredAudio(`es-e01-s15-phrase-${phraseIndex + 1}`, phrase.english);
}
function phraseSlowAudio(phraseIndex: number): LearningV2ModeAudioReferenceV1 {
  const phrase = phrases[phraseIndex]!;
  return authoredAudio(`es-e01-s15-phrase-${phraseIndex + 1}-slow`, phrase.english);
}
...
// es_episode_01_session_07_mode_native_v1.ts по составу и весу семей): три
...
// оставшиеся 12 (3-14) получают практику здесь. Порядок: две поддерживающие
...
// вопроса вслух без опоры, ядро voice-сессии.
export const ES_EPISODE_01_SESSION_15_MODE_NATIVE_PRACTICE_V1 = Object.freeze<readonly SessionModeNativePracticeSourceV1[]>([
...
if (ES_EPISODE_01_SESSION_15_MODE_NATIVE_PRACTICE_V1.length !== 12) {
  throw new Error('es_session_15_mode_native_practice_count_invalid');
}
