...
describe('пары: сколько верных — столько звёзд', () => {
  ...
  // Владелец 2026-08-04: неверная попытка пары снимает одну звезду даже до
  // первой верной пары; итоговый счёт игрока отдельно ограничивается нулём.
  test('в поле пар каждый уникальный ошибочный выбор вычитает одну звезду', () => {
    ...
    expect(answer({ difficulty: 1, matchedPairs: 0, totalPairs: 6, penaltyStars: 1 }))
      .toBe(-1);
