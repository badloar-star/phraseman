 ...
 */
function PersonalPlanTheoryScreen() {
  ...
  const theoryStartsPaidTask = useMemo(() => {
    if (!startTaskId) return false;
    try {
      const plan = getPlanById(planId as PersonalPlanId);
      const task = plan.days.find((entry) => entry.dayIndex === dayIndex)
        ?.tasks.find((entry) => entry.id === startTaskId);
      return task ? personalPlanTaskStartsPaidExercise(task) : false;
    } catch {
      return false;
    }
  }, [dayIndex, planId, startTaskId]);
  ...
  return (
    <View style={styles.safe}>
      <LessonIntroScreens
        ...
        onBack={goBack}
        showEnergyCost={theoryStartsPaidTask}
      />
...
export default withPersonalPlanSunsetGuard(PersonalPlanTheoryScreen);
