...
Status: Done (этапы 1-4), деплой functions за владельцем
...
Четыре коммита: 295ebfa7a (вход, хаб, список, дневной лимит), 91b298699
(разбор ответа, цепочка, предгенерация), 77fe48807 (карточка ошибки),
13e40c950 + 54d419b79 (награды, финал, полка).

Что выяснилось по ходу и изменило план:
- Достижения за ошибки не просто отсутствовали в каталоге — событие
  `mistake_practice_progress` отсекается фильтром FOUNDATION_ACHIEVEMENT_EVENT_TYPES
  (achievements.ts:1707). Каталог v2 сознательно сужен до фундаментных, поэтому
  по решению владельца звания сделаны ВНУТРИ раздела, а общий фильтр не тронут.
- Журнал ошибок не хранит ответ ученика: блок «твои ответы» из макета карточки
  не реализован сознательно, причина записана в mistake_detail_model.ts.
- Разбор конкретного промаха нельзя предгенерировать (ключ кэша содержит ответ).
  Греем объяснение самой фразы для всей очереди — оно от ответа не зависит.
- Приз за цель недели 51 руна, а не 50: цена элемента в механизме рун
  фиксирована (3), приз обязан быть ей кратен.

Осталось владельцу: деплой функции `mistakeHubAdvice` (кодбаза default,
поштучно) и флаг `gate_mistake_practice_premium` в «Пульте» при желании снять
дневной лимит.

Чужие падения, НЕ связанные с задачей (были до неё): контракт
home_learning_cta (ждёт константу homeQuickTilePanelBg, которой нет ни в
рабочем дереве, ни в HEAD), ошибки типов в factory_native_course_v1.ts,
сторожа perf_freeze и layout_stability (арена, лига, Learning V2,
HomeRuneBalance).
