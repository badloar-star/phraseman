...
const CONCEPT_BODY = L({
  ru: 'Importante уже встречалось раньше как обычный признак. Теперь у него появляется противник: igual, «всё равно», уже знакомое по фразам вроде No es igual. Es importante говорит, что что-то имеет значение, Es igual говорит, что значения нет вовсе. Оба признака встают после связки es точно так же, как caro или bonito.',
  uk: 'Importante вже траплялося раніше як звичайна ознака. Тепер у нього з’являється суперник: igual, «все одно», вже знайоме за фразами на кшталт No es igual. Es importante каже, що щось має значення, Es igual каже, що значення немає взагалі. Обидві ознаки стоять після зв’язки es точно так само, як caro чи bonito.',
  es: 'Importante has already appeared before as an ordinary quality. Now it gets an opponent: igual, "all the same," already familiar from phrases like No es igual. Es importante says something matters, Es igual says nothing matters at all. Both qualities go after the linking word es exactly like caro or bonito.',
  'pt-BR': 'Importante já apareceu antes como uma qualidade comum. Agora ganha um oponente: igual, "dá no mesmo," já conhecido de frases como No es igual. Es importante diz que algo importa, Es igual diz que nada importa. As duas qualidades ficam depois da ligação es exatamente como caro ou bonito.',
  vi: 'Importante đã xuất hiện từ trước như một đặc điểm bình thường. Bây giờ nó có một đối thủ: igual, "sao cũng được," đã quen thuộc từ những câu như No es igual. Es importante nói rằng điều gì đó quan trọng, Es igual nói rằng không có gì quan trọng cả. Cả hai đặc điểm đều đứng sau từ nối es giống hệt như caro hay bonito.',
  id: 'Importante sudah pernah muncul sebagai sifat biasa. Sekarang ia mendapat lawan: igual, "sama saja," sudah dikenal dari frasa seperti No es igual. Es importante mengatakan sesuatu itu penting, Es igual mengatakan tidak ada yang penting sama sekali. Keduanya berada setelah es persis seperti caro atau bonito.',
  tr: 'Importante daha önce sıradan bir nitelik olarak ortaya çıkmıştı. Şimdi bir rakibi var: igual, "fark etmez," No es igual gibi ifadelerden tanıdık. Es importante bir şeyin önemli olduğunu söyler, Es igual hiçbir şeyin önemli olmadığını söyler. İkisi de tıpkı caro ya da bonito gibi es’ten sonra gelir.',
  pl: 'Importante już wcześniej się pojawiało jako zwykła cecha. Teraz zyskuje przeciwnika: igual, „wszystko jedno”, już znane z fraz takich jak No es igual. Es importante mówi, że coś ma znaczenie, Es igual mówi, że nic nie ma znaczenia. Obie cechy stoją po łączniku es dokładnie tak samo jak caro czy bonito.',
});

const FORMULA_BODY = L({
  ru: 'Формула та же, что и для caro/bonito: связка es (или no es) плюс признак. El libro es importante — книга значима. Но igual ведёт себя иначе: он не согласуется по роду вовсе, даже рядом с libro мужского рода — El libro es igual, а не El libro es iguala, потому что формы iguala просто не существует.',
  uk: 'Формула та сама, що й для caro/bonito: зв’язка es (або no es) плюс ознака. El libro es importante — книга значуща. Але igual поводиться інакше: він не узгоджується за родом узагалі, навіть біля libro чоловічого роду — El libro es igual, а не El libro es iguala, бо форми iguala просто не існує.',
  es: 'The formula is the same as for caro/bonito: the linking word es (or no es) plus the quality. El libro es importante — the book is significant. But igual behaves differently: it does not agree by gender at all — El libro es igual, not El libro es iguala, because the form iguala simply does not exist.',
  'pt-BR': 'A fórmula é a mesma de caro/bonito: a ligação es (ou no es) mais a qualidade. El libro es importante — o livro é importante. Mas igual se comporta de forma diferente: não concorda em gênero de jeito nenhum — El libro es igual, não El libro es iguala, porque a forma iguala simplesmente não existe.',
  vi: 'Công thức giống hệt như với caro/bonito: từ nối es (hoặc no es) cộng với đặc điểm. El libro es importante — cuốn sách quan trọng. Nhưng igual hoạt động khác: nó hoàn toàn không hòa hợp theo giống — El libro es igual, không phải El libro es iguala, vì dạng iguala đơn giản không tồn tại.',
  id: 'Rumusnya sama seperti caro/bonito: kata penghubung es (atau no es) ditambah sifat. El libro es importante — bukunya penting. Tapi igual berperilaku berbeda: sama sekali tidak sesuai gender — El libro es igual, bukan El libro es iguala, karena bentuk iguala sama sekali tidak ada.',
  tr: 'Formül, caro/bonito için olanla aynıdır: es (ya da no es) bağlacı artı nitelik. El libro es importante — kitap önemli. Ama igual farklı davranır: eril libro’nun yanında bile cinsiyete hiç uyum sağlamaz — El libro es igual, El libro es iguala değil, çünkü iguala biçimi basitçe yoktur.',
  pl: 'Formuła jest taka sama jak dla caro/bonito: łącznik es (lub no es) plus cecha. El libro es importante — książka jest ważna. Ale igual zachowuje się inaczej: w ogóle nie zgadza się co do rodzaju, nawet obok męskiego libro — El libro es igual, nie El libro es iguala, bo forma iguala po prostu nie istnieje.',
});

const TRAP_BODY = L({
  ru: 'Самая частая ошибка — согласовать igual по роду, как caro или bonito: El libro es iguala вместо El libro es igual. Igual не меняется никогда. Вторая ловушка: No es igual — это не то же самое, что No es importante — первое отрицает разницу между вариантами, второе отрицает саму значимость.',
  uk: 'Найчастіша помилка — узгодити igual за родом, як caro чи bonito: El libro es iguala замість El libro es igual. Igual не змінюється ніколи. Друга пастка: No es igual — це не те саме, що No es importante — перше заперечує різницю між варіантами, друге заперечує саму значущість.',
  es: 'The most common mistake is agreeing igual by gender, like caro or bonito: El libro es iguala instead of El libro es igual. Igual never changes. The second trap: No es igual is not the same as No es importante — the first negates a difference between options, the second negates significance itself.',
  'pt-BR': 'O erro mais comum é concordar igual em gênero, como caro ou bonito: El libro es iguala em vez de El libro es igual. Igual nunca muda. A segunda armadilha: No es igual não é o mesmo que No es importante — o primeiro nega uma diferença entre opções, o segundo nega a própria importância.',
  vi: 'Lỗi phổ biến nhất là hòa hợp igual theo giống, như caro hay bonito: El libro es iguala thay vì El libro es igual. Igual không bao giờ đổi. Cái bẫy thứ hai: No es igual không giống với No es importante — cái đầu phủ định sự khác biệt giữa các lựa chọn, cái sau phủ định chính tầm quan trọng.',
  id: 'Kesalahan umum adalah menyesuaikan igual menurut gender: El libro es iguala alih-alih El libro es igual. Igual tidak pernah berubah. Jebakan kedua: No es igual tidak sama dengan No es importante — yang pertama menegasikan perbedaan antar opsi, yang kedua menegasikan kepentingan itu sendiri.',
  tr: 'En yaygın hata, igual’i caro ya da bonito gibi cinsiyete göre uyumlu hale getirmektir: El libro es iguala yerine El libro es igual. Igual asla değişmez. İkinci tuzak: No es igual, No es importante ile aynı değildir — ilki seçenekler arasındaki farkı olumsuzlar, ikincisi önemin kendisini olumsuzlar.',
  pl: 'Najczęstszy błąd to dopasowanie igual pod względem rodzaju, jak caro czy bonito: El libro es iguala zamiast El libro es igual. Igual nigdy się nie zmienia. Druga pułapka: No es igual to nie to samo co No es importante — pierwsze zaprzecza różnicy między opcjami, drugie zaprzecza samej ważności.',
});
