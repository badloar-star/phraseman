...
const ITEM_STAGES = new Set(['lesson_phrases', 'lesson_vocabulary', 'lesson_irregular_verbs', 'lesson_prepositions', 'quiz_questions', 'challenge_questions', 'flashcard_items', 'arena_questions']);
...
for (const kind of ['quiz_topic', 'challenge_topic']) {
...
for (const kind of ['quiz_questions', 'challenge_questions']) {
...
for (const kind of ['quiz_question_replacement', 'challenge_question_replacement']) {
...
}));
{
    ...
    if (!previous)
        throw new Error('arena_questions_v4_prompt_missing');
