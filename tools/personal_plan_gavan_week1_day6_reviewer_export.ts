...
} from './personal_plan_gavan_week1_day6_blueprint_candidate';

export const GAVAN_WEEK1_DAY6_REVIEWER_EXPORT_PATH = path.join(
  process.cwd(),
  '.codex-tmp',
  'personal-plans',
...
export type GavanWeek1Day6ReviewerStatus = 'needs_manual_review';

export type GavanWeek1Day6ReviewerContentUnitRow = {
  id: string;
  kind: 'content_unit';
  ...
  reviewStatus: GavanWeek1Day6ReviewerStatus;
};

export type GavanWeek1Day6ReviewerExplanationRow = {
  id: string;
...
export type GavanWeek1Day6ReviewerExerciseRow = {
  id: string;
...
export type GavanWeek1Day6ReviewerExport = {
  kind: 'gavan_week1_day6_reviewer_export';
  generatedAt: string;
  ...
  reviewStatus: GavanWeek1Day6ReviewerStatus;
  contentUnitRows: GavanWeek1Day6ReviewerContentUnitRow[];
  explanationRows: GavanWeek1Day6ReviewerExplanationRow[];
  exerciseRows: GavanWeek1Day6ReviewerExerciseRow[];
  exerciseCoverage: {
  ...
  mediaClaims: GavanWeek1Day6BlueprintCandidate['mediaClaims'];
  writePolicy: {
    dryRunOnly: true;
...
export type GavanWeek1Day6ReviewerExportOptions = {
  generatedAt: string;
};

export type GavanWeek1Day6ReviewerExportWriteOptions =
  GavanWeek1Day6ReviewerExportOptions & {
    targetPath: string;
  };

export type GavanWeek1Day6ReviewerExportIssueCode =
  | 'wrong_export_kind'
  | 'wrong_day_id'
...
export type GavanWeek1Day6ReviewerExportIssue = {
  code: GavanWeek1Day6ReviewerExportIssueCode;
  detail: string;
  target?: string;
};

export type GavanWeek1Day6ReviewerExportValidationResult = {
  valid: boolean;
  issues: GavanWeek1Day6ReviewerExportIssue[];
};

export type GavanWeek1Day6ReviewerExportWriteResult = {
  valid: boolean;
  issues: GavanWeek1Day6ReviewerExportIssue[];
  targetPath?: string;
  bytesWritten?: number;
  export?: GavanWeek1Day6ReviewerExport;
};
...
function issue(
  ...
  detail: string,
  target?: string,
): GavanWeek1Day6ReviewerExportIssue {
...
export function isGavanWeek1Day6ReviewerExportTargetAllowed(
  targetPath: string,
  cwd = process.cwd(),
): boolean {
...
function contentUnitRow(
  unit: GavanWeek1Day6ContentUnit,
...
function explanationRow(
  contentUnitId: string,
  card: GavanWeek1Day6ExplanationCard,
...
function exerciseRow(
  exercise: GavanWeek1Day6ExerciseBlueprint,
...
function visibleCopy(exportValue: GavanWeek1Day6ReviewerExport): string {
...
function forbiddenAnchors(exportValue: GavanWeek1Day6ReviewerExport): string[] {
...
function presentExerciseTypes(
  rows: GavanWeek1Day6ReviewerExerciseRow[],
): GavanWeek1ExpansionExerciseType[] {
  return rows.map((row) => row.exerciseType);
}

export function buildGavanWeek1Day6ReviewerExport(
  candidate: GavanWeek1Day6BlueprintCandidate,
  options: GavanWeek1Day6ReviewerExportOptions,
  ...
  const exportValue: GavanWeek1Day6ReviewerExport = {
    kind: 'gavan_week1_day6_reviewer_export',
...
function everyRowNeedsReview(exportValue: GavanWeek1Day6ReviewerExport): boolean {
  return [
...
export function validateGavanWeek1Day6ReviewerExport(
  exportValue: unknown,
  candidate: GavanWeek1Day6BlueprintCandidate,
): GavanWeek1Day6ReviewerExportValidationResult {
  const value = exportValue as GavanWeek1Day6ReviewerExport;
  const issues: GavanWeek1Day6ReviewerExportIssue[] = [];

  if (value.kind !== 'gavan_week1_day6_reviewer_export') {
  ...
  if (value.dayId !== 'gavan-week1-day6' || value.dayIndex !== 6) {
    issues.push(issue('wrong_day_id', 'Reviewer export must describe Gavan week 1 day 6 only.'));
  }
  ...
  candidate.contentUnits.forEach((unit) => {
    if (!exportedContentIds.has(unit.id)) {
      issues.push(issue(
        'missing_content_unit_export',
        'Every day 6 content unit must appear in the reviewer export.',
        unit.id,
      ));
    }
  });
  ...
  candidate.contentUnits.flatMap((unit) => unit.explanationCards).forEach((card) => {
    if (!exportedExplanationIds.has(card.id)) {
      issues.push(issue(
        'missing_explanation_card_export',
        'Every day 6 explanation card must appear in the reviewer export.',
        card.id,
      ));
    }
  });
  ...
  candidate.exerciseBlueprints.forEach((exercise) => {
    if (!exportedExerciseIds.has(exercise.id)) {
      issues.push(issue(
        'missing_exercise_blueprint_export',
        'Every day 6 exercise blueprint must appear in the reviewer export.',
        exercise.id,
      ));
    }
  });
  ...
  if (
    ...
    issues.push(issue(
      'fake_final_audio_claim',
      'Reviewer export cannot claim generated final audio for day 6.',
    ));
  }
  ...
  if (
    ...
    issues.push(issue(
      'fake_final_pronunciation_claim',
      'Reviewer export cannot claim built pronunciation scoring for day 6.',
    ));
  }
...
export function serializeGavanWeek1Day6ReviewerExport(
  exportValue: GavanWeek1Day6ReviewerExport,
): string {
  return `${JSON.stringify(exportValue, null, 2)}\n`;
}

export function writeGavanWeek1Day6ReviewerExport(
  candidate: GavanWeek1Day6BlueprintCandidate,
  options: GavanWeek1Day6ReviewerExportWriteOptions,
): GavanWeek1Day6ReviewerExportWriteResult {
  const exportValue = buildGavanWeek1Day6ReviewerExport(candidate, options);
  const validation = validateGavanWeek1Day6ReviewerExport(exportValue, candidate);
  ...
  if (!isGavanWeek1Day6ReviewerExportTargetAllowed(resolvedTargetPath)) {
    return {
      valid: false,
      issues: [
        issue(
          'target_path_not_allowed',
          'Gavan week 1 day 6 reviewer export can only write under .codex-tmp or docs/reports.',
        ),
      ],
    };
  }

  const serialized = serializeGavanWeek1Day6ReviewerExport(exportValue);
