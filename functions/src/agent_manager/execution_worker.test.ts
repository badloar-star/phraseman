...
import { executeEligibleAgentManagerJobs, executeOneAgentManagerJob, type ExecutionWorkerDocument, type ExecutionWorkerRepository, type ExecutionWorkerTransaction } from './execution_worker';

class MemoryExecutionRepository implements ExecutionWorkerRepository {
  readonly documents = new Map<string, ExecutionWorkerDocument>();
  afterTransaction: ((transactionCount: number) => void) | null = null;
  private transactionCount = 0;
  async get(path: string): Promise<ExecutionWorkerDocument | null> { return this.documents.get(path) ?? null; }
  async listEligibleJobs(): Promise<readonly ExecutionWorkerDocument[]> {
    return [...this.documents.entries()]
      .filter(([path]) => path.startsWith('agent_manager_execution_jobs/'))
      .map(([, document]) => document);
  }
  async runTransaction<T>(body: (transaction: ExecutionWorkerTransaction) => Promise<T>): Promise<T> {
...
describe('agent-manager bounded execution worker', () => {
  test('moves an eligible job through a lease into a PII-free needs-review result', async () => {
    ...
    const result = await executeOneAgentManagerJob(repository, JOB_ID, NOW);
    ...
    expect(String((savedTask?.result as Record<string, unknown>)?.summary)).toMatch(/Внешняя отправка не выполнялась/);
  ...
  test('does not duplicate a succeeded result and honours a live lease', async () => {
    const repository = seeded();
    await executeOneAgentManagerJob(repository, JOB_ID, NOW);
    const replay = await executeOneAgentManagerJob(repository, JOB_ID, NOW + 1);
    ...
    expect(await executeOneAgentManagerJob(leased, JOB_ID, NOW)).toEqual({ jobId: JOB_ID, outcome: 'skipped' });
  ...
  test('does not complete when the claimed lease has expired before the completion transaction', async () => {
    ...
    expect(await executeOneAgentManagerJob(repository, JOB_ID, NOW)).toEqual({ jobId: JOB_ID, outcome: 'cancelled' });
  ...
  test('uses a separate aggregate-only review result for analysis jobs', async () => {
    ...
    expect(await executeOneAgentManagerJob(repository, JOB_ID, NOW)).toEqual({ jobId: JOB_ID, outcome: 'succeeded' });
    expect((repository.documents.get(`agent_manager_tasks/${TASK_ID}`)?.data.result as Record<string, unknown>)?.summary).toMatch(/агрегированная аналитическая заготовка/);
  });
  ...
  test('persists a handler-provided analytics brief through the same leased review boundary', async () => {
    ...
    const result = await executeOneAgentManagerJob(repository, JOB_ID, NOW, async (claim) => claim.job.scope === 'analysis_only'
  ...
  test('rejects an invalid handler result at the execution boundary', async () => {
    ...
    expect(await executeOneAgentManagerJob(repository, JOB_ID, NOW, async () => ({ result: { summary: 'contact person@example.com', outcome: 'needs_review' } } as never))).toEqual({ jobId: JOB_ID, outcome: 'failed' });
    expect(repository.documents.get(`agent_manager_tasks/${TASK_ID}`)?.data).toMatchObject({ status: 'queued', revision: 3, result: null });
  });
  ...
  test('does not accept a handler result after the real-time lease has expired', async () => {
    ...
    try {
      expect(await executeOneAgentManagerJob(repository, JOB_ID, NOW, async () => ({ result: { summary: 'Aggregate metrics require manual review.', outcome: 'needs_review' } }))).toEqual({ jobId: JOB_ID, outcome: 'cancelled' });
    } finally {
      nowSpy.mockRestore();
    }
  });
  ...
  test('reclaims an expired lease and processes only bounded candidate jobs', async () => {
    ...
    const result = await executeEligibleAgentManagerJobs(repository, NOW, 10);
  ...
  test('does not let an analytics worker claim a support job', async () => {
    ...
    const result = await executeEligibleAgentManagerJobs(repository, NOW, 10, undefined, ['analysis_only']);
