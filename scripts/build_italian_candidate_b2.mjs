...
const SOURCE = path.join(ROOT, 'content', 'language-test-pilots', 'it', 'candidate-bank', 'sources', 'B2.json');
const OUTPUT = path.join(ROOT, 'content', 'language-test-pilots', 'it', 'candidate-bank', 'B2.json');
...
if (source.language !== 'it' || source.level !== 'B2' || source.publishable !== false) throw new Error('Italian B2 source metadata is invalid');
if (!Array.isArray(source.items) || source.items.length !== 40) throw new Error(`Expected 40 authored Italian B2 items, found ${source.items?.length ?? 0}`);
for (const [skill, expected] of Object.entries(QUOTAS)) {
  const count = source.items.filter((item) => item.skill === skill).length;
  if (count !== expected) throw new Error(`Expected ${expected} Italian B2 ${skill} items, found ${count}`);
}
if (new Set(SOURCE_ORDER).size !== 40 || SOURCE_ORDER.some((index) => index < 0 || index >= 40)) throw new Error('Italian B2 source order must contain every source row exactly once');
if (new Set(source.items.map(({ stimulus }) => stimulus)).size !== 40) throw new Error('Italian B2 stimuli must be unique');
if (new Set(source.items.map(({ targetConstruct }) => targetConstruct)).size !== 40) throw new Error('Italian B2 target constructs must be unique');
source.items.forEach((item, index) => {
  if (!(item.skill in QUOTAS)) throw new Error(`Italian B2 item ${index + 1}: invalid skill`);
  if (!['multiple-choice', 'gap-fill'].includes(item.format)) throw new Error(`Italian B2 item ${index + 1}: invalid format`);
  if (!Array.isArray(item.distractors) || item.distractors.length !== 3) throw new Error(`Italian B2 item ${index + 1}: expected three distractors`);
  const options = [item.answer, ...item.distractors.map(({ text }) => text)].map((text) => text.normalize('NFKC').trim().toLocaleLowerCase('it'));
  if (new Set(options).size !== 4) throw new Error(`Italian B2 item ${index + 1}: options must be unique`);
  for (const distractor of item.distractors) {
    if (!distractor.reasonRu || !/[\u0400-\u04ff]/u.test(distractor.reasonRu)) throw new Error(`Italian B2 item ${index + 1}: missing Russian distractor rationale`);
  }
});

const cefrEn = (item) => ({
  grammar: `The B2 situation “${item.scenario}” requires reliable discourse-sensitive control of ${item.targetConstruct}.`,
  vocabulary: `The B2 context “${item.scenario}” requires precise register-aware discrimination of ${item.targetConstruct}.`,
  reading: `This B2 text requires supported inference, scope control, or evidence integration through ${item.targetConstruct}.`,
  pragmatics: `This B2 interaction requires coordinated stance, evidence, and outcome control through ${item.targetConstruct}.`,
}[item.skill]);
const cefrRu = (item) => ({
  grammar: `${item.canDoRu} Задание требует надёжного дискурсивно обусловленного контроля грамматики уровня B2.`,
  vocabulary: `${item.canDoRu} Задание требует точного различения лексики и регистра уровня B2.`,
  reading: `${item.canDoRu} Текст требует обоснованного вывода, контроля области или интеграции доказательств уровня B2.`,
...
const questions = SOURCE_ORDER.map((sourceIndex, index) => {
  ...
  return {
    id: `it-b2-${String(index + 1).padStart(3, '0')}`,
