    ...
    audio: {
      input: {
        ...
        turn_detection: {
          ...
          // зачем (владелец 2026-08-23, живой тест): «обрывает на полуслове,
          // долго ждёт после моей речи, может начать отвечать дважды». Все три
          // симптома давал самодельный обмен ходами на клиенте: он создавал
          // ответ сам и держал его, пока доигрывает аудио-буфер MAX, а страховочный
          // таймер транскрипта (1500 мс) успевал поставить ВТОРОЙ ответ на одну
          // реплику. Возвращаем штатную схему OpenAI Realtime: сервер сам знает
          // границу хода, сам отменяет свою речь при перебивании и физически не
          // создаёт два ответа на одно высказывание.
          create_response: true,
          // Перебивание включено намеренно: это делает разговор живым (ученик
          // может остановить учителя голосом). От эха громкой связи защищает
          // АУДИО-тракт, а не запрет отвечать: noise_reduction far_field чистит
          // буфер ДО VAD, плюс AEC в WebRTC. Лечим причину, а не следствие.
