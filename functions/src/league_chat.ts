...
function moderate(text: string): { status: 'clean' | 'review' | 'blocked'; categories: ModCategory[]; reasons: string[]; normalizedText: string } {
  const normalizedText = normalize(text);
  const compacted = normalizedText.replace(/\s+/g, '');
  const categories: ModCategory[] = [];
  const reasons: string[] = [];

  if (text.length > MAX_MESSAGE_LENGTH) { categories.push('length'); reasons.push('message_too_long'); }
  ...
  if (containsTerm(normalizedText, compacted, BLOCK_TERMS) || containsBlockedPattern(normalizedText, compacted)) { categories.push('profanity'); reasons.push('blocked_term'); }
  if (HATE_PATTERNS.some((re) => re.test(normalizedText))) { categories.push('hate'); reasons.push('hate_or_harassment_pattern'); }
  if (containsTerm(normalizedText, compacted, SEXUAL_TERMS)) { categories.push('sexual'); reasons.push('sexual_content'); }
  if (containsTerm(normalizedText, compacted, REVIEW_IDENTITY_TERMS)) { categories.push('identity'); reasons.push('protected_identity_context'); }
  if (/(?:\bты\b|\byou\b).{0,24}(?:туп|дебил|идиот|лох|stupid|idiot|dumb)/i.test(normalizedText)) { categories.push('insult'); reasons.push('direct_insult'); }
...
async function assertActiveChatUser(
...
): Promise<{ member: Record<string, unknown>; userData: Record<string, unknown> }> {
  ...
  if (chatBan.status === 'banned' || mutedUntil > Date.now()) {
    throw new HttpsError('permission-denied', 'chat_restricted');
  }
  ...
  if (
    leaderboard.groupId !== room.groupId ||
    ...
    throw new HttpsError('permission-denied', 'leaderboard_room_mismatch');
  }

  const member = group.members?.[stableUid];
  if (!member) throw new HttpsError('permission-denied', 'not_league_member');
  return { member, userData: userSnap?.data() || {} };
}

async function grantRoomReadAccess(
  db: FirebaseFirestore.Firestore,
  authUid: string,
  stableUid: string,
  room: { groupId: string; weekId: string; leagueId: number },
...
export const leagueChatSendMessage = onCall({ region: REGION, enforceAppCheck: ENFORCE_APP_CHECK }, async (request) => {
  ...
  if (now - lastSendAt < SEND_THROTTLE_MS) {
    throw new HttpsError('resource-exhausted', 'send_throttled');
  }
...
export const leagueChatReportMessage = onCall({ region: REGION, enforceAppCheck: ENFORCE_APP_CHECK }, async (request) => {
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (now - lastReportAt < REPORT_THROTTLE_MS) throw new HttpsError('resource-exhausted', 'report_throttled');
