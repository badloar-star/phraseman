...
describe("Season Pass per-theme backgrounds", () => {
  it("statically wires one unique full-screen asset for every ThemeMode without aliases", () => {
    ...
    expect(source).toContain(
      "satisfies Record<ThemeMode, ImageSourcePropType>",
    );

    for (const theme of THEME_MODES) {
      const requiredAsset = new RegExp(
        `require\\(["']\\.\\./assets/images/season/backgrounds/${theme}/background\\.webp["']\\)`,
      );
      expect(
        source.match(new RegExp(`^  ${theme}: require\\(`, "gm")),
      ).toHaveLength(1);
      expect(source).toMatch(requiredAsset);
      expect(
        fs.existsSync(
          path.join(
            ROOT,
            `assets/images/season/backgrounds/${theme}/background.webp`,
          ),
        ),
      ).toBe(true);
    }
    ...
    expect(source.match(/\/background\.webp["']\)/g)).toHaveLength(
      THEME_MODES.length,
    );
  });
  ...
  it("renders one fixed decorative background below the scrolling rewards", () => {
    const source = fs.readFileSync(
      path.join(ROOT, "app/season_pass.tsx"),
      "utf8",
    );
    ...
    const fixedBackground = source.indexOf(
