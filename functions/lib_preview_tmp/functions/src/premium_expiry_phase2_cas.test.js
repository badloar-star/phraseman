...
describe('premium expiry cron Phase 2 CAS safety (RED)', () => {
    ...
    it('uses canonical lineage/version CAS so a concurrent renewal wins over the stale sweep', () => {
        expect((0, premium_expiry_cron_1.samePremiumAuthority)({ premium_rc_active_lineage: 'old', premium_rc_expiry_ms: '100' }, { premium_rc_active_lineage: 'new', premium_rc_expiry_ms: '200' })).toBe(false);
        expect((0, premium_expiry_cron_1.samePremiumAuthority)({ premium_rc_active_lineage: 'same', premium_rc_expiry_ms: '200' }, { premium_rc_active_lineage: 'same', premium_rc_expiry_ms: '200' })).toBe(true);
        const activeSecondary = {
            lineageHash: 'lineage-b',
            plan: 'monthly',
            revoked: false,
            activeThroughMs: 3000000,
            productId: 'premium_monthly',
            store: 'APP_STORE',
            environment: 'PRODUCTION',
            lastEventType: 'RENEWAL',
            lastAccessEventTimeMs: 2000000,
            lastGrantEventTimeMs: 2000000,
        };
        expect((0, premium_expiry_cron_1.reconcilePremiumExpiry)({
            premium_plan: 'yearly',
            premium_expiry: '1000',
            premium_rc_product_id: 'old_yearly',
            premium_rc_store: 'APP_STORE',
            premium_rc_event_type: 'RENEWAL',
        }, [activeSecondary], 2500000)?.patch).toMatchObject({
        ...
        }, [{ ...activeSecondary, revoked: true, activeThroughMs: 1000 }], 2500000)?.patch)
