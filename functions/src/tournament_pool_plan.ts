 ...
 *
 * зачем ровно четыре: владелец 2026-07-27 — «не спеши перенимать задания из
 * Learning V2, я ещё не разобрался какие там нормальные, скажу сам». До его
 * отбора турнир играет только на проверенных режимах, пул которых уже наполнен.
 * Добавление режима = одна строка здесь: планировщик, готовность раундов и
 * заказ для генератора пересчитаются автоматически.
 *
 * Отдельно решено: аудио и диалоги в турнир НЕ идут. Таймер вопроса ~15 с, а
 * загрузка озвучки съедает 1-3 с у игроков с медленной сетью, играют часто без
 * наушников (транспорт, работа), и аудио нельзя «перечитать» — соревнование
 * начинало бы наказывать за условия, а не за знание языка.
 */
