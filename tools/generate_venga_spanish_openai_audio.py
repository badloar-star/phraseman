#!/usr/bin/env python3
"""Generate OpenAI TTS audio for the Venga RU->ES phrase pack."""
...
ROLE_CONFIG: dict[str, dict[str, str]] = {
    ...
    "es1": {
        "voice": "coral",
        "instructions": (
            "Speak Spanish slowly and clearly for an A1 learner. Warm, beautiful native neutral Latin American Spanish teacher voice. "
            "Correct Spanish stress and vowels. Say only the phrase, then stop cleanly with a tiny natural pause. No extra words."
        ),
    },
    "es2": {
        "voice": "cedar",
        "instructions": (
            "Speak Spanish at a calm medium lesson pace, clear and natural, not rushed. Native neutral Latin American pronunciation. "
            "Correct stress and rhythm. Say only the phrase, then stop cleanly with a tiny natural pause. No extra words."
        ),
    },
    "es3": {
        "voice": "sage",
        "instructions": (
            "Speak Spanish slowly and clearly, not faster than the previous voice. Gentle learner-friendly native neutral pronunciation. "
            "Correct stress and vowels. Say only the phrase, then stop cleanly with a tiny natural pause. No extra words."
        ),
    },
}
...
def digest_for(role: str, row: dict[str, Any]) -> str:
    text = str(row["ru"] if role == "ru" else row["es"])
    payload = {
        "role": role,
        "index": int(row["index"]),
        "text": text,
...
def main() -> int:
    ...
    parser.add_argument("--phrases", type=Path, default=Path("exports/venga-phrase-packs/ru-es-a1-vsscp/phrase_rows.json"))
    parser.add_argument("--out-dir", type=Path, default=Path("exports/venga-phrase-packs/ru-es-a1-vsscp/openai-audio"))
    ...
    for position, item in enumerate(manifest, start=1):
        ...
        if path.exists() and path.stat().st_size > 1024 and not args.overwrite:
            ...
            cached += 1
            continue
        print(f"[openai-tts-es] {position:03d}/{len(manifest):03d} {item['role']} {item['index']:03d}", flush=True)
    ...
    summary = {
        ...
        "roles": {role: sum(1 for item in manifest if item["role"] == role) for role in ["ru", "es1", "es2", "es3"]},
    }
