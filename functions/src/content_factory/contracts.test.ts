...
describe('language factory contracts', () => {
  ...
  it('accepts a complete target-language manifest', () => {
    expect(validatePackManifest(manifest)).toEqual({ ok: true, errors: [] });
  });

  it('rejects missing identity and publish metadata', () => {
    const result = validatePackManifest({ ...manifest, studyTarget: '', contentHash: '', reviewStatus: 'needs_review', activationStatus: 'published' });
