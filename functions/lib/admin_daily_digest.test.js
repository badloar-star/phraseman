...
const admin_daily_digest_1 = require("./admin_daily_digest");
...
describe('aggregateDigestFacts', () => {
    test('пустые источники → нули и пустой дайджест', () => {
        const facts = (0, admin_daily_digest_1.aggregateDigestFacts)(EMPTY_ROWS);
        ...
        expect((0, admin_daily_digest_1.isDigestEmpty)(facts)).toBe(true);
    });
    ...
    test('unknown-ключ для отсутствующих полей', () => {
        const facts = (0, admin_daily_digest_1.aggregateDigestFacts)({ ...EMPTY_ROWS, reports: [{ status: 'new' }] });
...
describe('buildDigestPrompt / utcDayKey', () => {
    ...
    test('utcDayKey — YYYY-MM-DD по UTC', () => {
        ...
        expect((0, admin_daily_digest_1.utcDayKey)(Date.UTC(2026, 0, 1, 0, 0, 0))).toBe('2026-01-01');
    });
});
//# sourceMappingURL=admin_daily_digest.test.js.map