...
async function saveOrReplay(
  ...
  bytes: Buffer,
  contentHash: string,
  // зачем (инцидент 2026-08-17): закрепления релиза Learning V2 сверяют пять
  // полей файла, включая contentType, и ждут «application/json; charset=utf-8».
  // Этот писатель ставил «application/json» без charset — четыре поля из пяти
  // совпадали, а пятое роняло чтение каталога с невнятным metadata_mismatch.
  // Параметр необязательный: пять остальных вызывающих сторон работают как
  // работали, менять их тип рискованно и незачем.
  ...
  try {
    await file.save(bytes, {
      ...
      metadata: { contentType, cacheControl: 'public,max-age=31536000,immutable', metadata: { contentHash } },
    });
  } catch (error) {
...
export async function writeImmutableObject(
  ...
  objectPath: string,
  payload: unknown,
  // зачем: см. saveOrReplay — закрепления релиза Learning V2 требуют charset
  // в contentType. По умолчанию поведение прежнее.
  contentType = 'application/json',
): Promise<ImmutableObjectReceipt> {
  ...
  const bytes = Buffer.from(serialized, 'utf8');
  const contentHash = createHash('sha256').update(serialized).digest('hex');
  const { metadata } = await saveOrReplay(file, bytes, contentHash, contentType);
  const objectGeneration = String(metadata.generation ?? '').trim();
  if (!objectGeneration) throw new Error('artifact_generation_missing');
