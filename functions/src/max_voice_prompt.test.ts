...
describe('tutor instructions', () => {
  ...
  it('префикс байт-в-байт стабилен при смене памяти/снимка (кэш инструкций)', () => {
    ...
    expect(TUTOR_TOOLS.map((t) => t.name)).toEqual(['start_scene', 'end_scene', 'mark_phrase_result', 'assign_homework', 'set_next_topic', 'set_language_preference', 'flag_safety', 'end_call']);
