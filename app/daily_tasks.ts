...
/** Number of cards still required for an unfinished recall objective. */
...
const recallTaskIsRuntimeEligible = (
  task: DailyTask,
  dueCount: number,
  row?: TaskProgress,
): boolean => {
...
/** Read eligibility context without reconciling or mutating persisted progress. */
const loadTodayProgressSnapshot = async (
  studyTarget?: RuntimeStudyTarget,
): Promise<TaskProgress[]> => {
  try {
    const raw = await AsyncStorage.getItem(dailyTasksProgressKey(getTodayKey(), studyTarget));
    return parseStoredTaskProgressStrict(raw);
  } catch {
    return [];
  }
};
...
const replaceRecallTasksWhenNoDueItems = async (
  ...
  const [dueCount, progress] = await Promise.all([
    ...
    loadTodayProgressSnapshot(studyTarget),
  ]);
  ...
  return tasks.map((task) => {
    ...
    if (recallTaskIsRuntimeEligible(task, dueCount, row)) return task;
