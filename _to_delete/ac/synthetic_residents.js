...
exports.RESIDENT_STREAK_MAX_RESETS_PER_MONTH = exports.RESIDENT_STREAK_MAX = exports.RESIDENT_SLOT_COUNT = exports.RESIDENT_POPULATION = exports.RESIDENT_MAX_LEVEL = exports.RESIDENT_IDLE_CHANCE = exports.RESIDENT_TICK_MAX_XP = exports.RESIDENT_TICK_MIN_XP = exports.RESIDENT_TICK_MS = exports.RESIDENT_EPOCH_MS = void 0;
exports.residentSignupMs = residentSignupMs;
exports.residentGeneration = residentGeneration;
exports.residentPace = residentPace;
exports.residentHash = residentHash;
exports.residentTickGain = residentTickGain;
exports.residentTicksElapsed = residentTicksElapsed;
exports.residentTotalXpAt = residentTotalXpAt;
exports.residentXpGainedBetween = residentXpGainedBetween;
exports.residentLevelFromXp = residentLevelFromXp;
exports.residentName = residentName;
exports.residentAvatar = residentAvatar;
exports.residentStreakAt = residentStreakAt;
exports.residentBaseLikes = residentBaseLikes;
exports.residentProfileAt = residentProfileAt;
 ...
 */
exports.RESIDENT_EPOCH_MS = Date.UTC(2026, 7, 4);
/**
...
function residentLifespanMs(index, generation) {
...
function residentFirstSignupMs(index) {
    ...
    return exports.RESIDENT_EPOCH_MS - Math.round(RESIDENT_SIGNUP_SPREAD_MS * Math.pow(draw, 1.6));
}
/**
 ...
 */
function residentSignupMs(index, nowMs = exports.RESIDENT_EPOCH_MS) {
 ...
 */
function residentGeneration(index, nowMs = exports.RESIDENT_EPOCH_MS) {
...
exports.RESIDENT_TICK_MIN_XP = 2;
exports.RESIDENT_TICK_MAX_XP = 134;
/**
...
exports.RESIDENT_POPULATION = tournament_reddit_bot_names_1.REDDIT_BOT_NAMES.length;
/** FNV-1a: стабильный хэш строки → uint32. Зеркало клиентского fnv1a. */
function residentHash(text) {
    let hash = 0x811c9dc5;
 ...
 */
function residentTickRandom(index, tick, stream) {
 ...
 */
function residentTickGain(index, tick) {
    if (residentTickRandom(index, tick, 'idle') < exports.RESIDENT_IDLE_CHANCE)
        return 0;
    const draw = residentTickRandom(index, tick, 'gain');
    const base = exports.RESIDENT_TICK_MIN_XP + (exports.RESIDENT_TICK_MAX_XP - exports.RESIDENT_TICK_MIN_XP) * draw;
 ...
 */
function residentTicksElapsed(nowMs, index = 0) {
    const start = residentSignupMs(index, nowMs);
    if (nowMs <= start)
        return 0;
    return Math.floor((nowMs - start) / exports.RESIDENT_TICK_MS);
}
/**
 ...
 */
function residentTotalXpAt(index, nowMs) {
    const { generation, signupMs } = residentGeneration(index, nowMs);
    if (nowMs <= signupMs)
        return 0;
    const ticks = Math.floor((nowMs - signupMs) / exports.RESIDENT_TICK_MS);
    let total = 0;
...
function residentXpGainedBetween(index, fromMs, toMs) {
    if (toMs <= fromMs)
        return 0;
    ...
    const fromTick = Math.max(0, Math.floor((fromMs - to.signupMs) / exports.RESIDENT_TICK_MS));
    const toTick = Math.max(0, Math.floor((toMs - to.signupMs) / exports.RESIDENT_TICK_MS));
    let total = 0;
...
function residentLevelFromXp(totalXp) {
    if (totalXp <= 0)
        return 1;
    let level = 1;
    while (level < exports.RESIDENT_MAX_LEVEL && totalXpForLevel(level + 1) <= totalXp) {
        level++;
    }
    return level;
}
...
exports.RESIDENT_SLOT_COUNT = Math.floor(exports.RESIDENT_POPULATION / RESIDENT_NAMES_PER_SLOT);
function residentName(index, nowMs = exports.RESIDENT_EPOCH_MS) {
    const { generation } = residentGeneration(index, nowMs);
    const slot = ((index % exports.RESIDENT_SLOT_COUNT) + exports.RESIDENT_SLOT_COUNT) % exports.RESIDENT_SLOT_COUNT;
    ...
    return tournament_reddit_bot_names_1.REDDIT_BOT_NAMES[slot * RESIDENT_NAMES_PER_SLOT + withinBlock];
}
/**
 ...
 */
function residentAvatar(level) {
    return String(Math.max(1, Math.min(exports.RESIDENT_MAX_LEVEL, level)));
}
 ...
 */
function residentResetDaysInMonth(index, monthIndex) {
    ...
    const count = seed % (exports.RESIDENT_STREAK_MAX_RESETS_PER_MONTH + 1);
    ...
    const bandSize = total / exports.RESIDENT_STREAK_MAX_RESETS_PER_MONTH;
    const days = [];
 ...
 */
function residentStreakAt(index, nowMs, signupMs) {
    ...
    const earliestDay = Math.max(signupDay, today - exports.RESIDENT_STREAK_MAX);
    let lastResetDay = -1;
    ...
    for (let month = fromMonth; month <= toMonth; month++) {
        for (const day of residentResetDaysInMonth(index, month)) {
            if (day >= earliestDay && day <= today && day > lastResetDay)
                lastResetDay = day;
        }
    }
    ...
    return Math.max(0, Math.min(exports.RESIDENT_STREAK_MAX, since));
}
/**
 ...
 */
function residentBaseLikes(totalXp) {
...
function residentProfileAt(index, nowMs) {
    const { generation, signupMs } = residentGeneration(index, nowMs);
