...
function parseGeneratedSurfaceArtifact(raw) {
    ...
    if ((surface !== 'quiz' && surface !== 'flashcard' && surface !== 'arena') || !Number.isInteger(artifact.lessonId) || Number(artifact.lessonId) < 1 || !Array.isArray(artifact.items) || artifact.items.length === 0)
        throw new Error('generated_surface_invalid');
