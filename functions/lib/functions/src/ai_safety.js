...
exports.SAFETY_SYSTEM_INSTRUCTION = void 0;
exports.evaluateSafety = evaluateSafety;
exports.moderateUserText = moderateUserText;
exports.recordSafetyFlag = recordSafetyFlag;
 ...
 */
exports.SAFETY_SYSTEM_INSTRUCTION = 'SAFETY: You are a friendly language-learning companion, not a therapist, doctor, or crisis service. ' +
 ...
 */
function evaluateSafety(userText) {
 ...
 */
async function moderateUserText(apiKey, userText) {
    const none = { flagged: false, category: null, matched: null };
 ...
 */
async function recordSafetyFlag(verdict, ctx) {
    if (!verdict.flagged || !verdict.category)
        return;
    ...
    try {
        ...
        try {
            ...
            if (cfg.types && cfg.types.safetyFlag === false)
                safetyTypeEnabled = false;
        }
        catch {
            /* нет конфига — оставляем default-on */
        }
        if (safetyTypeEnabled) {
            const msg = `🆘 <b>Safety flag</b> — ${escapeHtml(verdict.category)}\n` +
            ...
            await (0, admin_alerts_1.sendTelegramAlert)(admin_alerts_1.ADMIN_ALERT_BOT_TOKEN.value() || process.env.ADMIN_ALERT_BOT_TOKEN || '', msg, null);
        }
    }
    catch (error) {
...
//# sourceMappingURL=ai_safety.js.map