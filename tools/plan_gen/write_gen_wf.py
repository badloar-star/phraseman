...
const CBL = '__CBL__';
const STYLE = 'СТИЛЬ (жёстко): дружелюбный тренер на ты; живые разговорные фразы из ситуации дня. ЭМОДЗИ ЗАПРЕЩЕНЫ ВЕЗДЕ. НИКАКИХ грам.терминов в видимом тексте (глагол/связка/артикль/отрицание/Present Simple) — простыми словами. НИКАКИХ надуманных образов-действий. Объяснение = чистый смысл + мини-пример слово->со словом. Каждая часть rule/why/commonMistake <= 24 слова по-русски. ИСПАНСКИЙ: обязательны ударения (Podrás, estación, ¿Cuánto, Así, más, rápido) и открывающий ¿ во всех вопросах. ru/uk/es — реальные переводы, НЕ копия ru, согласованы между собой (если rule учит перевод слова, в meaning тот же перевод).';
const RULES = 'КОНТРАКТ: planId voyazh. РОВНО 6 фраз; id voyazh_d<N>_p<K>; каждая фраза 4-6 слов. У каждого слова РОВНО 5 дистракторов того же класса (POS строго из набора, НЕ выдумывать numeral/wh-word/auxiliary/phrase: число=determiner, where/how=adverb, there в there-is=existential, do-вспом=verb), БЕЗ дублей, НЕ совпадают со словом, и КРИТИЧНО — ни один дистрактор НЕ должен давать тоже правильную/естественную фразу (синонимы и близкие по смыслу убирать: seat/table, empty/free, those/these, start/open, get/have, want/need). 6 vocab (каждое слово реально в english фраз В ТОЙ ЖЕ ФОРМЕ, example = точная фраза дня). 3 intro (how-экран с 2 examples). prerequisiteLessons НЕ содержит сам день и НЕ содержит уроки выше gate. constructions фразы — теги ТОЛЬКО из разрешённого набора для gate дня (не выше).';

phase('Write')
const drafts = await pipeline(
  BATCH,
  (d) => agent(
    'Ты — методист-писатель английского для русскоязычных (uk/es) новичков. Напиши контент дня ' + d.dayIndex + ' плана voyazh (путешествия), приложение phraseman.\\n' +
    'ТЕМА: "' + d.topic + '". Уровень ' + d.level + '. Заметка: ' + d.note + '\\n' +
    'Длинная сюжетная арка реального путешествия (84 дня). День ' + d.dayIndex + ' строго из этой темы, продолжает историю.\\n' +
...
print('wrote gen workflow', len(js), 'bytes')
