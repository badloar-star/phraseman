...
### 20.1 Каталог объяснений после повторной ошибки

Каждое из 144 обязательных заданий хранит отдельную запись объяснения ошибки.
В корневом Content Studio эта запись фильтруется по interface locale, activity
family, episode, session и task. Для одной записи разрешено до восьми
неизменяемых вариантов со статусами `candidate`, `selected_for_preview` или
`rejected`; выбранным для preview может быть ровно один вариант.

Вариант обязан различать происхождение `owner_authored` и
`generator_candidate`, но это происхождение, служебный provenance и остальные
варианты никогда не входят в learner-проекцию. Приложение получает только
выбранный текст восьми локалей и стабильную ссылку на него. Первая учебная
ошибка не раскрывает объяснение; начиная со второй ошибки того же задания
runtime показывает выбранный локализованный текст. Сам факт выбора preview не
является публикацией или owner approval.

Генератор может предложить варианты только когда его вручную запускает
владелец. Codex не создаёт и не редактирует реальные объяснения E1–E32. Ни один
текст объяснения не может содержать ключ ответа, принятую фразу или иной
answer-revealing материал; это проверяется отдельным semantic QA, а не выводится
из формы JSON.
