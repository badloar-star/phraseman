...
describe('заголовки каталога уроков', () => {
  ...
  it('у КАЖДОГО урока есть сцена проверки', () => {
    // зачем (владелец 2026-08-31): 12 целей жили без единой сцены, и высшую
    // ступень освоения по ним нельзя было взять штатным путём —
    // applyGoalProgress требует выполненной сцены переноса. Сцены дописаны;
    // сторож не даёт добавить новую цель, забыв про проверку.
    const orphans = CAN_DO_GOALS.filter((g) => g.sceneIds.length === 0).map((g) => g.id);
