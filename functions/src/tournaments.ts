...
const TOURNAMENT_PRIVATE_STATE_COLLECTION = 'tournamentPrivateState';
  ...
  for (let index = 0; index < TOURNAMENT_ROUND_MODE_PLAN.length; index += 1) {
    ...
    for (const plannedMode of plannedModes) {
      const picked = selectRoundTasks({
        pool: pool.filter((task) => task.mode === plannedMode && !usedTaskIds.has(task.taskId)),
        roomId,
        roundNo,
...
function buildBotReservationPlan(input: {
  ...
  return {
    ...
    fillDeadlineAtMs: Math.max(input.gatherStartedAtMs, lastBotAtMs) + 1000,
  };
}
...
export async function tournamentJoinTransaction(
  ...
  return db.runTransaction(async (tx) => {
    ...
    const fillsLastSeat = room.state === 'lobby' && nextRoom.players.length === TOURNAMENT_ROOM_SIZE;
...
export async function tournamentFillRoomTransaction(
  ...
  return db.runTransaction(async (tx) => {
    ...
    const activationTasks = activationRound
      ...
      : [];
    // Once all sixteen seats are server-owned, there is no later event that can
    // safely start the game: bot joinAtMs values are presentation timestamps,
    // not Firestore writes. Start round one in this transaction with a fresh
    // authoritative schedule instead of leaving a full lobby at 00:00 until
    // the next one-minute scheduler tick.
...
export const tournamentStartNow = onCall(
  ...
  async (request) => {
    ...
    const recentRef = recentBotRosterRef(db);
    await db.runTransaction(async (tx) => {
      const recentSnap = await tx.get(recentRef);
      const botPlan = buildBotReservationPlan({
        profiles: resources.bots,
        roomId,
        count: TOURNAMENT_ROOM_SIZE - 1,
    ...
    // The transaction rechecks authentication, identity, ban state and the
    // immutable zero economy, but test admission is schedule-independent.
