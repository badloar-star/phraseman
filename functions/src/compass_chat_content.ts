...
export type CompassPostKind =
  | 'discussion'
  | 'language_fact'
  | 'mini_challenge'
  | 'poll'
  // Legacy values may still exist in old Firestore messages. New generation
  // never produces them.
  | 'word_of_day'
  | 'fact'
  | 'question'
...
export const LEAGUE_COMPASS_GENERATED_KINDS = [
  'discussion',
  'language_fact',
  'mini_challenge',
  'poll',
...
export type LeagueCompassGeneratedKind = (typeof LEAGUE_COMPASS_GENERATED_KINDS)[number];

const MAX_TEXT_CHARS = 420;
const MAX_POLL_LABEL_CHARS = 90;

const FALLBACK_POSTS: CompassPost[] = [
  {
    kind: 'discussion',
    systemType: 'generic',
    i18n: {
...
function at<T>(items: readonly T[], seed: number): T {
  const idx = ((seed % items.length) + items.length) % items.length;
  return items[idx]!;
}

function fallbackIndexForSeed(seed: number): number {
...
export function hasForbiddenDailyLabel(text: string): boolean {
  const lower = String(text || '').toLocaleLowerCase();
  return FORBIDDEN_DAILY_LABELS.some((needle) => lower.includes(needle));
}

export function hasForbiddenProgressSummaryClaim(text: string): boolean {
  const lower = String(text || '').toLocaleLowerCase();
...
function cleanText(value: unknown, maxChars: number): string {
  ...
  if (text.length < 12 || text.length > maxChars) return '';
  if (LINK_RE.test(text)) return '';
  if (hasForbiddenDailyLabel(text)) return '';
  if (hasForbiddenProgressSummaryClaim(text)) return '';
  return text;
}

function normalizeI18n(value: unknown, maxChars: number): CompassI18n | null {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
  const src = value as Record<string, unknown>;
  const out = {} as CompassI18n;
  for (const lang of COMPASS_CHAT_LANGS) {
...
function cleanKind(value: unknown): LeagueCompassGeneratedKind | null {
  const kind = String(value ?? '').trim();
  return (LEAGUE_COMPASS_GENERATED_KINDS as readonly string[]).includes(kind)
    ? (kind as LeagueCompassGeneratedKind)
    : null;
}
...
function normalizePollOptions(rawOptions: unknown): CompassPollOption[] | null {
  if (!Array.isArray(rawOptions) || rawOptions.length < 2 || rawOptions.length > 4) return null;
  const out: CompassPollOption[] = [];
...
export function normalizeGeneratedCompassPost(raw: unknown): CompassPost | null {
  ...
  const post: CompassPost = { kind, systemType: 'generic', i18n };
  if (kind === 'poll') {
...
export function buildLeagueCompassDailyPrompt(input: { dayKey: string; seed: number }): string {
  const formatHint = at(
    [
