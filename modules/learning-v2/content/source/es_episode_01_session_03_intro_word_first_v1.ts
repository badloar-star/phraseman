...
// зачем тело переписано короче прежнего варианта (владелец, 2026-08-27,
// повторный прогон гейта после ревью раздела «Библия текстов»): реальный
// гейт (learning_content_quality_gate_v1.ts) держит верхний потолок 320
// знаков / 4 предложения и БЕЗ нижнего минимума (MIN_BODY_CHARS объявлена, но
// нигде не используется — комментарий в гейте прямо это фиксирует). Прежний
// текст был раздут до ~470-480 знаков в расчёте на несуществующий нижний
// порог и валил intro_body_overloaded во всех восьми локалях; recognize-
// guidance слова bonito (см. es_episode_01_session_03_vocabulary_v1.ts) была
// раздута аналогично и валила guidance_overloaded. Обе правки — только сжатие
// текста, смысл и структура concept→formula→trap не изменились.
// зачем тело дословно повторяет формулировку правильного варианта ответа
// (владелец, 2026-08-27): intro_question_not_grounded требует, чтобы текст
// нижнего вопроса либо дословно встречался в теле, либо на 50%+ токенов
// совпадал с targetCorrect-разметкой bodyRuns. Короткая перефразировка
// («о чём идёт речь» вместо «что описывают») этот гейт не проходит.
const CONCEPT_BODY = L({
  ru: 'Bonito описывает мужской род, bonita — женский: одно и то же качество «красивый», разная концовка. Концовка признака зависит только от рода того, что описывают, — не от говорящего и не от времени.',
  uk: 'Bonito описує чоловічий рід, bonita — жіночий: та сама якість «красивий», інше закінчення. Закінчення ознаки залежить тільки від роду того, що описують, — не від мовця і не від часу.',
  es: 'Bonito describes the masculine, bonita the feminine: the same quality "pretty", a different ending. The ending depends only on the gender of what is being described — not on the speaker, not on time.',
  'pt-BR': 'Bonito descreve o masculino, bonita o feminino: a mesma qualidade "bonito", terminação diferente. A terminação depende só do gênero daquilo que se descreve — não de quem fala, nem da hora.',
  vi: 'Bonito mô tả giống đực, bonita mô tả giống cái: cùng một đặc điểm "đẹp", đuôi khác nhau. Đuôi chỉ phụ thuộc vào giống của thứ được mô tả — không phụ thuộc người nói, không phụ thuộc thời gian.',
  id: 'Bonito mendeskripsikan maskulin, bonita feminin: sifat yang sama "cantik", akhiran berbeda. Akhirannya hanya bergantung pada gender dari apa yang dideskripsikan — bukan penutur, bukan waktu.',
  tr: 'Bonito eril, bonita dişil olanı tanımlar: aynı "güzel" niteliği, farklı son ek. Son ek yalnızca tanımlanan şeyin cinsiyetine bağlıdır — konuşana ya da zamana değil.',
  pl: 'Bonito opisuje rodzaj męski, bonita żeński: ta sama cecha „ładny”, inna końcówka. Końcówka zależy tylko od rodzaju tego, co się opisuje — nie od mówiącego i nie od pory.',
});

const FORMULA_BODY = L({
  ru: 'Формула короткая: -o для мужского рода, -a для женского. Чтобы получить bonita из bonito, нужно заменить -o на -a — больше ничего не меняется. Связка es от рода не зависит и остаётся одинаковой в обоих случаях.',
  uk: 'Формула коротка: -o для чоловічого роду, -a для жіночого. Щоб отримати bonita з bonito, потрібно замінити -o на -a — більше нічого не змінюється. Зв’язка es від роду не залежить і лишається однаковою в обох випадках.',
  es: 'The formula is short: -o for masculine, -a for feminine. To get bonita from bonito, you need to change -o to -a — nothing else changes. The linking word es does not depend on gender and stays the same in both cases.',
  'pt-BR': 'A fórmula é curta: -o para masculino, -a para feminino. Para obter bonita a partir de bonito, é preciso trocar -o por -a — nada mais muda. A ligação es não depende do gênero e permanece igual nos dois casos.',
  vi: 'Công thức ngắn gọn: -o cho giống đực, -a cho giống cái. Để có bonita từ bonito, cần đổi -o thành -a — không gì khác thay đổi. Từ nối es không phụ thuộc vào giống và giữ nguyên trong cả hai trường hợp.',
  id: 'Rumusnya singkat: -o untuk maskulin, -a untuk feminin. Untuk mendapatkan bonita dari bonito, perlu mengganti -o menjadi -a — tidak ada yang lain berubah. Kata penghubung es tidak bergantung pada gender dan tetap sama di kedua kasus.',
  tr: 'Formül kısadır: eril için -o, dişil için -a. Bonito’dan bonita elde etmek için -o’yu -a ile değiştirmek gerekir — başka hiçbir şey değişmez. Es bağlacı cinsiyete bağlı değildir ve her iki durumda da aynı kalır.',
  pl: 'Formuła jest krótka: -o dla męskiego, -a dla żeńskiego. Aby uzyskać bonita z bonito, trzeba zamienić -o na -a — nic więcej się nie zmienia. Łącznik es nie zależy od rodzaju i pozostaje taki sam w obu przypadkach.',
});

const TRAP_BODY = L({
  ru: 'Fácil «лёгкий» никогда не меняется по роду — и про предмет мужского, и про предмет женского рода говорят одинаково fácil. Bonito и bonita меняются всегда: другой класс признаков, с явной концовкой -o/-a.',
  uk: 'Fácil «легкий» ніколи не змінюється за родом — про предмет і чоловічого, і жіночого роду кажуть однаково fácil. Bonito і bonita змінюються завжди: інший клас ознак, з явним закінченням -o/-a.',
  es: 'Fácil "easy" never changes by gender — both a masculine and a feminine noun are described the same way, fácil. Bonito and bonita always change: a different class of quality, with an explicit -o/-a ending.',
  'pt-BR': 'Fácil "fácil" nunca muda por gênero — um substantivo masculino e um feminino são descritos do mesmo jeito, fácil. Bonito e bonita sempre mudam: outra classe de qualidade, com terminação explícita -o/-a.',
  vi: 'Fácil "dễ" không bao giờ đổi theo giống — danh từ giống đực và giống cái đều được mô tả giống nhau, fácil. Bonito và bonita luôn thay đổi: một lớp đặc điểm khác, có đuôi -o/-a rõ ràng.',
  id: 'Fácil "mudah" tidak pernah berubah menurut gender — kata benda maskulin dan feminin dideskripsikan dengan cara sama, fácil. Bonito dan bonita selalu berubah: kelas sifat lain, dengan akhiran -o/-a yang jelas.',
  tr: 'Fácil "kolay" cinsiyete göre asla değişmez — eril ve dişil bir isim aynı şekilde, fácil olarak anlatılır. Bonito ve bonita her zaman değişir: açık -o/-a son ekine sahip farklı bir nitelik sınıfı.',
  pl: 'Fácil „łatwy” nigdy nie zmienia się przez rodzaj — rzeczownik męski i żeński opisuje się tak samo, fácil. Bonito i bonita zawsze się zmieniają: inna klasa cech, z wyraźną końcówką -o/-a.',
});
