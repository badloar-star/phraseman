...
exports.adminGenerateDailyDigest = void 0;
...
function aggregateDigestFacts(rows, windowHours = 24) {
    ...
    const sampleTexts = rows.cancels
        ...
        .filter((t) => t.length > 0)
        .slice(0, 5);
    const ideaItems = rows.ideas.slice(0, 8).map((i) => ({
 ...
 */
async function loadDigestSources(db, since, limitPer = 1000) {
    ...
    const loadWebsiteInbox = async () => {
        try {
            ...
            const snap = await db
                ...
                .limit(limitPer)
                .get();
            return snap.docs.map((d) => ({ topic: d.data().topic, message: d.data().message }));
        }
        catch (e) {
    ...
    const loadPromoRedemptions = async () => {
        try {
            const snap = await db
                ...
                .limit(limitPer)
                .get();
            return snap.docs.map((d) => ({ code: d.data().code || d.id }));
        }
        catch (e) {
 ...
 */
async function runAdminDailyDigest(apiKey, actorEmail, now = Date.now()) {
