...
export type ArenaBackgroundSearchMatched = Readonly<{
  ...
  acceptDeadlineAtMs?: number;
  /**
   * Матч не удалось прочитать на сервере.
   *
   * зачем отдельный признак, а не просто отсутствие срока (живой прогон
   * 2026-09-21): «срока нет» значит РАЗНОЕ. У старого документа живого матча
   * его просто не сохранили — там запасное окно уместно. А здесь сервер
   * ответил ошибкой на попытку описать матч: срок не «не пришёл», его нет
   * вовсе, и предлагать такой матч человеку нельзя. Отличить одно от другого
   * по пустому полю невозможно — поэтому признак явный.
   */
...
export class ArenaBackgroundSearch {
  ...
  private handleFound(match: ArenaBackgroundSearchMatched, origin: string): void {
    this.clearAllTimers();
    /**
     * Матч, который сервер не смог прочитать, человеку НЕ предлагаем.
     *
     * зачем (живой прогон на эмуляторе 2026-09-21): в логе стояло
     * `describe failed matchId=... arena_match_sealed_publication_invalid`, и
     * после этого находка всё равно доходила до тоста — с запасным окном 12 с,
     * отсчитанным от текущего момента. То есть кнопка «Принять» обещала матч,
     * про который уже известно, что он сломан. Тап стоил 25⚡ и заканчивался
     * словами «Этого матча больше нет» — дословной жалобой владельца.
     *
     * Молчанием это не заканчивается: поиск честно останавливается, экран
     * поиска уводит на хаб, человек не ждёт впустую.
     */
    if (match.describeFailed) {
      this.deps.log(`[ARENA-BGSEARCH] находка отброшена origin=${origin} `
        + `matchId=${match.matchId}: матч не читается на сервере, срок неизвестен`);
      this.stop('no_opponent');
      return;
    }
