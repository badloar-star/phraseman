...
describe('constellations/service_core — уровень вопросов по кольцу (A2, банк A1–B2)', () => {
    test('внешнее кольцо: A1 для низких рангов, A2 для высоких', () => {
        expect((0, service_core_1.levelForRing)('outer', 0)).toBe('A1');
        expect((0, service_core_1.levelForRing)('outer', 23)).toBe('A2');
    });
    test('среднее A2→B1, внутреннее B1→B2, Полярная всегда B2 (C1 в банке нет)', () => {
        expect((0, service_core_1.levelForRing)('middle', 0)).toBe('A2');
        expect((0, service_core_1.levelForRing)('middle', 23)).toBe('B1');
        expect((0, service_core_1.levelForRing)('inner', 0)).toBe('B1');
        expect((0, service_core_1.levelForRing)('inner', 23)).toBe('B2');
        expect((0, service_core_1.levelForRing)('polar', 0)).toBe('B2');
        expect((0, service_core_1.levelForRing)('polar', 23)).toBe('B2');
    });
});
describe('constellations/service_core — число вопросов на атаку (A2/A5/A6)', () => {
    test('кольца без Сияния: внешнее 1, среднее 2, внутреннее 2, Полярная 3', () => {
        const s = makeState();
        expect((0, service_core_1.attackQuestionSpec)(s, '3,-1', 0, CFG).count).toBe(1); // внешнее
        expect((0, service_core_1.attackQuestionSpec)(s, '2,0', 0, CFG).count).toBe(2); // среднее
        expect((0, service_core_1.attackQuestionSpec)(s, '1,0', 0, CFG).count).toBe(2); // внутреннее
        expect((0, service_core_1.attackQuestionSpec)(s, '0,0', 0, CFG).count).toBe(3); // Полярная
    });
    test('Сияние добавляет +1 за уровень, кап 5 (1.5 — центр даёт заметный скачок)', () => {
        const s = makeState();
        s.stars['2,0'] = { owner: 1, radiance: 2 };
        expect((0, service_core_1.attackQuestionSpec)(s, '2,0', 0, CFG).count).toBe(4); // 2+2 (среднее)
        s.stars['0,0'] = { owner: 1, radiance: 2 };
        expect((0, service_core_1.attackQuestionSpec)(s, '0,0', 0, CFG).count).toBe(5); // 3+2 = 5 (Полярная, кап 5)
    });
    test('атака родной звезды: 2 вопроса среднего уровня + Сияние', () => {
        const s = makeState();
        const spec = (0, service_core_1.attackQuestionSpec)(s, HOMES[1], 0, CFG);
        expect(spec.count).toBe(2);
        expect(spec.level).toBe((0, service_core_1.levelForRing)('middle', 0));
        expect(spec.isBossAssault).toBe(false);
        s.stars[HOMES[1]] = { owner: 1, radiance: 1 };
        expect((0, service_core_1.attackQuestionSpec)(s, HOMES[1], 0, CFG).count).toBe(3);
    });
    test('последнее ядро — босс-штурм: всегда 3 вопроса внутреннего уровня, Сияние игнорируется', () => {
        ...
        expect(spec.level).toBe((0, service_core_1.levelForRing)('inner', 0));
    });
    test('underdog-скидка (1.4): отстающий (≤3 звезды) атакует лидера — минус вопрос', () => {
        ...
        expect((0, service_core_1.attackQuestionSpec)(s, '0,0', 0, CFG, 0).count).toBe(2);
        // Без attackerSlot скидки нет — 3 вопроса.
        expect((0, service_core_1.attackQuestionSpec)(s, '0,0', 0, CFG).count).toBe(3);
    });
    test('underdog-скидка НЕ применяется, если атакующий не отстаёт', () => {
        const s = makeState();
        // Дать slot0 много звёзд → не отстающий.
        for (const k of ['2,0', '2,1', '3,-1', '2,-2'])
            s.stars[k] = { owner: 0, radiance: 0 };
        s.stars['0,0'] = { owner: 1, radiance: 0 };
        expect((0, service_core_1.attackQuestionSpec)(s, '0,0', 0, CFG, 0).count).toBe(3); // без скидки
    });
    test('минимум вопросов на атаку — всегда 1 (скидка не уводит в 0)', () => {
        ...
        for (const k of ['2,0', '1,0', '0,0', '1,-1'])
            s.stars[k] = { owner: 1, radiance: 0 };
        expect((0, service_core_1.attackQuestionSpec)(s, '3,-1', 0, CFG, 0).count).toBe(1); // 1-1=0 → min 1
    });
});
describe('constellations/service_core — сборка входа резолва из доков', () => {
    test('атака: correctAll только когда отвечены ВСЕ выданные вопросы верно', () => {
        const input = (0, service_core_1.buildRoundInput)([
            {
...
//# sourceMappingURL=service_core.test.js.map