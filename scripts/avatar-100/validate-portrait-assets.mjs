...
const PORTRAIT_OVERFLOW_CUTOFF_Y = SIZE / 2;
...
export async function checkFile(filePath) {
  ...
  for (let y = 0; y < info.height; y += 1) {
    for (let x = 0; x < info.width; x += 1) {
      ...
      visible += 1;
      if (!inPoly(SAFE_HEX, x + 0.5, y + 0.5)) {
        outsideSafe += 1;
        if (y >= PORTRAIT_OVERFLOW_CUTOFF_Y) lowerOutsideSafe += 1;
      }
  ...
  if (lowerOutsideSafe > 0) {
    errors.push(`${lowerOutsideSafe} visible pixels spill outside the lower hex frame`);
  }
  ...
  return {
    ...
    visible,
    outsideSafe,
    upperOutsideSafe: outsideSafe - lowerOutsideSafe,
    lowerOutsideSafe,
