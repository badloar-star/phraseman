...
describe('server source registry', () => {
    it('validates a versioned English blueprint and uses a safe doc id', () => {
        expect((0, source_registry_1.sourceRegistryDocId)(registry.blueprintId, registry.version)).toBe('english-core-32:v1');
        expect((0, source_registry_1.validateSourceRegistry)(registry)).toEqual({ ok: true, errors: [] });
    });
    it('rejects invented source evidence and non-English blueprint locale', () => {
        expect((0, source_registry_1.validateSourceRegistry)({ ...registry, blueprintLocale: 'fr' }).errors).toContain('blueprint_locale_must_be_en');
        expect((0, source_registry_1.validateSourceRegistry)({ ...registry, evidence: [{ ...registry.evidence[0], url: 'javascript:bad' }] }).errors).toContain('source_evidence_invalid');
    });
    it('reports the exact requested lessons missing from a 32-lesson registry', () => {
        ...
        expect((0, source_registry_1.inspectSourceRegistryCoverage)({ ...registry, lessons }, [32, 33])).toEqual({
            ok: false,
...
//# sourceMappingURL=source_registry.test.js.map