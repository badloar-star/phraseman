...
describe("live admin sensitive user operations", () => {
  it.each([
    ...
    "bulkGrantPremium",
    "bulkBanUsers",
  ])("%s has no direct sensitive Firestore write", (name) =>
    noDirectSensitiveWrite(windowHandler(name)),
  );
  ...
  it.each([
    ...
    [
      "repairAdminGrantPremiumCompat",
      "getAdminMigrateLegacyAdminPremiumCallable()",
    ],
  ...
  it.each([
    ...
    "%s uses existing protected commands with shared reason, unique ids and exact counts",
    (name, getter) => {
      const body = windowHandler(name);
      expect(body).toContain(getter);
      expect(body).toContain("reason");
