...
import {
  ...
  type LocalRunnerDocument,
  type LocalRunnerRepository,
  type LocalRunnerTransaction,
} from './local_runner_transport';

class MemoryLocalRunnerRepository implements LocalRunnerRepository {
  readonly documents = new Map<string, LocalRunnerDocument>();

  async get(path: string): Promise<LocalRunnerDocument | null> { return this.documents.get(path) ?? null; }
...
const random = (...values: string[]) => {
  const copy = [...values];
  return () => {
    const value = copy.shift();
    if (!value) throw new Error('test random exhausted');
    return value;
  };
};

function seedQueuedCodeTask(repository: MemoryLocalRunnerRepository, taskId = 'task-001', revision = 3) {
  ...
  repository.documents.set(`agent_manager_tasks/${taskId}`, { id: taskId, data: {
    ...
    assignedAgentId: 'developer', createdByUid: 'owner-001', createdAtMs: NOW - 1_000, updatedAtMs: NOW - 1_000,
  ...
  repository.documents.set(`agent_manager_execution_jobs/${jobId}`, { id: jobId, data: {
    ...
    createdAtMs: NOW - 1_000, updatedAtMs: NOW - 1_000, leasedAtMs: null, finishedAtMs: null, outputRef: null, outputHash: null,
  } });
  return jobId;
}

async function paired(repository: MemoryLocalRunnerRepository) {
  const pairing = await createLocalRunnerPairing(repository, { ownerUid: 'owner-001' }, NOW, random('pair-001', 'code-raw'));
  return exchangeLocalRunnerPairing(repository, { pairingId: pairing.pairingId, code: pairing.code }, NOW + 1, random('cap-001', 'cap-token-raw'));
}
...
describe('agent-manager local runner transport', () => {
  ...
  test('exchanges a valid pairing only once and stores only a capability hash', async () => {
    ...
    const capability = await exchangeLocalRunnerPairing(repository, { pairingId: pairing.pairingId, code: pairing.code }, NOW + 1, random('cap-001', 'cap-token-raw'));
    ...
    await expect(exchangeLocalRunnerPairing(repository, { pairingId: pairing.pairingId, code: pairing.code }, NOW + 2, random('cap-002', 'second-cap-token'))).rejects.toThrow('pairing code is unavailable');
  });

  test('does not exchange expired or revoked pairing codes', async () => {
    const repository = new MemoryLocalRunnerRepository();
    const pairing = await createLocalRunnerPairing(repository, { ownerUid: 'owner-001' }, NOW, random('pair-001', 'code-raw'));
    await expect(exchangeLocalRunnerPairing(repository, { pairingId: pairing.pairingId, code: pairing.code }, NOW + 10 * 60 * 1000, random('cap-001', 'cap-token-raw'))).rejects.toThrow('pairing code is unavailable');
  });
  ...
  test('claims exactly one queued code_prepare job with a capability-bound lease', async () => {
    ...
    const claim = await claimOneLocalRunnerJob(repository, capability, NOW + 2, random('lease-raw'));
  ...
  test('rejects non-code jobs and expired or revoked capabilities', async () => {
    ...
    await expect(claimOneLocalRunnerJob(repository, capability, NOW + 2, random('lease-raw'))).resolves.toBeNull();
    await revokeLocalRunnerCapability(repository, { ownerUid: 'owner-001', capabilityId: capability.capabilityId }, NOW + 3);
    await expect(claimOneLocalRunnerJob(repository, capability, NOW + 4, random('lease-two'))).rejects.toThrow('local runner capability is unavailable');
  });
  ...
  test('accepts only the live capability-bound lease and writes a bounded needs-review result', async () => {
    ...
    const capability = await paired(repository);
    const claim = await claimOneLocalRunnerJob(repository, capability, NOW + 2, random('lease-raw'));
    if (!claim) throw new Error('claim expected');

    const outcome = await submitLocalRunnerReview(repository, {
      capability,
      jobId,
    ...
    await expect(submitLocalRunnerReview(repository, { capability, jobId, leaseToken: claim.leaseToken, result: { summary: 'Repeat', outcome: 'needs_review' } }, NOW + 4)).rejects.toThrow('local runner lease is unavailable');
  });
});
