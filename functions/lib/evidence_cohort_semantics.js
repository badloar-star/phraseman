...
exports.resolveOrderedActivationMilestones = resolveOrderedActivationMilestones;
exports.summarizeFirstTouchCohorts = summarizeFirstTouchCohorts;
exports.summarizeActivationCohort = summarizeActivationCohort;
const DAY_MS = 24 * 60 * 60 * 1000;
const RETENTION_DAYS = [1, 7, 14, 30];
function firstAtOrAfter(values, threshold) {
    return values.filter((value) => Number.isFinite(value) && value >= threshold).sort((a, b) => a - b)[0] ?? null;
}
function resolveOrderedActivationMilestones(events) {
...
function summarizeFirstTouchCohorts(instances, reportThroughMs) {
    const summary = Object.fromEntries(RETENTION_DAYS.map((day) => [`d${day}`, { eligible: 0, exactReturned: 0, rollingReturned: 0 }]));
    ...
    for (const instance of instances) {
        ...
        if (distinctActiveDays <= 1)
            activeDayBuckets.one += 1;
        else if (distinctActiveDays <= 3)
            activeDayBuckets.twoToThree += 1;
        else if (distinctActiveDays <= 7)
            activeDayBuckets.fourToSeven += 1;
        else
            activeDayBuckets.eightPlus += 1;
        const offsets = activeDays.map((day) => Math.round((day - firstTouchDay) / DAY_MS));
        for (const retentionDay of RETENTION_DAYS) {
            if ((reportThroughDay - firstTouchDay) / DAY_MS < retentionDay)
                continue;
            const counts = summary[`d${retentionDay}`];
            counts.eligible += 1;
            if (offsets.includes(retentionDay))
                counts.exactReturned += 1;
            if (offsets.some((offset) => offset >= retentionDay))
                counts.rollingReturned += 1;
        }
    }
    return { validInstances, invalidInstances, summary, activeDayBuckets };
}
function summarizeActivationCohort(instances) {
...
//# sourceMappingURL=evidence_cohort_semantics.js.map