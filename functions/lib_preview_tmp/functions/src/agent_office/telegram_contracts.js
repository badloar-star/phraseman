...
function parseVerifiedTelegramApprovalUpdate(value) {
    ...
    (0, contracts_1.assertExactKeys)(input, ['verification', 'callbackNamespace', 'updateId', 'callbackQueryId', 'chatId', 'userId', 'commandText'], 'verified Telegram update');
    if (input.verification !== 'verified')
        throw new https_1.HttpsError('permission-denied', 'Telegram identity is not verified');
    if (input.callbackNamespace !== 'ao1' && input.callbackNamespace !== 'am1')
        invalid('verified Telegram update.callbackNamespace is invalid');
...
function parseTelegramCallbackUpdate(value) {
    ...
    if (!match)
        invalid('Telegram callback data is invalid');
    const verb = match[2] === 'a' ? 'authorize' : 'reject';
    return Object.freeze({
        verification: 'verified',
        callbackNamespace: match[1],
        updateId,
