...
## ⛔ ФУНДАМЕНТ: празднование не ставится в durable-очередь

**Правило владельца 2026-09-01. Действует ВСЕГДА, во ВСЕХ сессиях, для ЛЮБОЙ
новой фичи — не только для повышения уровня.**

Любое поздравление / анимация награды / «ура, получилось» проектируется по
четырём законам:

1. **Празднование НЕ durable.** В AsyncStorage сохраняется САМА награда
   (валюта, опыт, спин, предмет), а не «долг на показ». Пропущенная анимация не
   стоит ничего; непоказанная запись живёт вечно и однажды всплывает не вовремя.
2. **Поздравляем ФАКТОМ, а не записью.** Перед показом сверь запись с текущим
   состоянием человека и подрежь по факту. Показывать сохранённое «когда-то»
   значение поверх сегодняшнего — это и есть баг.
3. **Празднование живёт на СВОЁМ экране.** Не всплывает глобальным оверлеем
   поверх урока, арены, спина. Ждёт свой экран, где есть контекст (полоска,
   аватарка, счётчик), в который эффект встраивается естественно.
4. **Удаляешь показ — не унеси награду.** Всё, что начислялось «по кнопке в
   модалке», обязано переехать, а не исчезнуть: бонусы, апселлы, прогревы,
   аналитика. Проверь КАЖДЫЙ побочный эффект показа.

**Повод.** Полноэкранная модалка повышения уровня держала durable-очередь
непоказанных уровней, которая чистилась только по кнопке «Готово». Один
недожатый показ жил месяцами и всплывал на любом экране: человек на 40-м уровне
видел «Поздравляем, уровень 13». Жалобы пользователей.

Как сделано правильно — `app/home_level_up_celebration_queue.ts` (очередь в
памяти процесса, сверка цели с фактическим уровнем по опыту) и
`components/home/use_home_level_up_celebration.ts`. Сторож:
`tests/level_up_celebrated_on_home_contract.test.ts`. Сработал — возвращать
правило, а не удалять проверку.
