...
export const TOURNAMENT_TABLE_DISPLAY_MS = 6 * 1000;
...
export function planTournamentFinalization(
  ...
  for (let start = 0; start < standings.length;) {
    let end = start;
    while (end + 1 < standings.length && standings[end + 1].score === standings[start].score) end += 1;
    const place = start + 1;
...
/** General standings. Equal scores share one competition place (1, 1, 3). */
export function computePlacements(players: TournamentPlayer[]): {
  ...
  let previousForfeited: boolean | undefined;
  const placements = standings.map((player, index) => {
    const forfeited = player.forfeitedAtMs !== undefined;
    if (index === 0 || player.score !== previousScore || forfeited !== previousForfeited) sharedPlace = index + 1;
    previousScore = player.score;
    previousForfeited = forfeited;
