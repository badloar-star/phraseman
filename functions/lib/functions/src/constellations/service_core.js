...
exports.PERFECT_SPEED_FRACTION = void 0;
exports.levelForRing = levelForRing;
exports.attackQuestionSpec = attackQuestionSpec;
exports.buildRoundInput = buildRoundInput;
 ...
 */
function levelForRing(ring, matchRankIndex) {
    switch (ring) {
...
function starCountOf(state, slot) {
    let n = 0;
 ...
 */
function underdogDiscount(state, targetKey, attackerSlot, cfg) {
    if (attackerSlot === undefined)
        return 0;
    const target = state.stars[targetKey];
    if (!target || target.owner === null || target.owner === attackerSlot)
        return 0;
    if (starCountOf(state, attackerSlot) > cfg.underdog.maxStarsForDiscount)
        return 0;
    // Цель у лидера?
 ...
 */
function attackQuestionSpec(state, targetKey, matchRankIndex, cfg, attackerSlot) {
    ...
    const withDiscount = (n) => Math.max(1, n - discount);
    ...
    const ring = (0, hex_1.ringOf)((0, hex_1.parseHexKey)(targetKey));
...
exports.PERFECT_SPEED_FRACTION = 0.5;
/**
 ...
 */
function isFastEnough(answers, thresholdMs) {
    if (answers.length === 0)
        return false;
    const sum = answers.reduce((acc, a) => acc + a.timeMs, 0);
    return sum / answers.length < thresholdMs;
}
function duelAnswersFor(record, questionCount) {
    const out = [];
 ...
 */
function buildRoundInput(records, duels, cfg) {
    const bySlot = new Map();
    for (const r of records)
        bySlot.set(r.slot, r);
    const shields = records
        .filter((r) => r.shieldStarKey)
        .map((r) => ({ slot: r.slot, starKey: r.shieldStarKey }));
    ...
    const perfectSpeedMs = cfg.questionMaxSec * 1000 * exports.PERFECT_SPEED_FRACTION;
    ...
    const duelOutcomes = duels.map((duel) => {
        ...
        const score = (0, duel_1.scoreDuel)(a, b, mainQ);
        return {
...
//# sourceMappingURL=service_core.js.map