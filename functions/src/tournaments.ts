...
/**
 * Сколько времени сервер даёт на ОДНО задание раунда.
 *
 * зачем 2026-07-27 (аудит турниров): было 10 000 мс, а экран раунда рисует
 * SECONDS_PER_QUESTION = 15 (app/tournament_round.tsx). Сервер закрывал раунд
 * через 4×10 = 40 секунд, когда игрок по таймеру на экране был только на
 * третьем вопросе из четырёх: комнату уводило в таблицу, а ответы не успевали
 * уйти. Числа обязаны совпадать — держим 15 секунд в обоих местах.
 * Расхождение стережёт tests/tournament_timing_contract.test.ts.
 */
const DEFAULT_MAX_MS_PER_TASK = 15_000;
