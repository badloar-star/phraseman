...
if (!existsSync(absoluteEnvPath)) fail(`${relativeEnvPath} is missing`);

try {
  execFileSync('git', ['check-ignore', '--quiet', '--no-index', '--', relativeEnvPath], { stdio: 'ignore' });
} catch {
  fail(`${relativeEnvPath} is not ignored by Git`);
}

try {
  execFileSync('git', ['ls-files', '--error-unmatch', '--', relativeEnvPath], { stdio: 'ignore' });
