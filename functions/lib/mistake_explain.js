...
async function enforceDailyQuota(db, authUid, stableUid, isPremium) {
    const now = Date.now();
    ...
    return db.runTransaction(async (tx) => {
        ...
        if (used >= dailyCap) {
            throw new https_1.HttpsError('resource-exhausted', isPremium ? 'mistake_explain_premium_limit' : 'mistake_explain_free_limit');
        }
        tx.set(ref, {
            authUid,
            stableUid,
...
function buildMessages(payload) {
    ...
    return [
        {
            role: 'system',
            content: 'You are a careful Phraseman mistake coach. Explain only the learner\'s exact answer error. ' +
