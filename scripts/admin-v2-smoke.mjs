...
function checkCapabilityParity() {
  ...
  assert(ADMIN_CAPABILITY_REGISTRY.filter((item) => item.nativeRoute).length === 15, 'native capability count drifted from 15');
  assert(ADMIN_CAPABILITY_REGISTRY.filter((item) => !item.nativeRoute).length === 44, 'fallback capability count drifted from 44');
