...
exports.parseOwnerRepositoryWalletStateBlob = exports.materializeOwnerRepositoryWalletStateBlob = void 0;
...
const invalid = () => { throw new Error("owner_repository_wallet_blob_invalid"); };
...
const materializeOwnerRepositoryWalletStateBlob = (input) => {
    let state;
    try {
        state = (0, wallet_reducer_1.parseWalletState)(input);
    }
    catch {
        return invalid();
    }
    ...
    if ((0, decision_registry_1.utf8ByteLengthV1)(encoded) > MAX_BYTES)
        throw new Error("owner_repository_wallet_blob_overflow");
...
exports.materializeOwnerRepositoryWalletStateBlob = materializeOwnerRepositoryWalletStateBlob;
const parseOwnerRepositoryWalletStateBlob = (input) => {
    let request;
    let refValue;
    try {
    ...
    try {
        ...
        detachedEnvelope = (0, wallet_1.detachBoundedWalletJson)(parsed, "owner_repository_wallet_blob_invalid");
    ...
    if (!isRecord(detachedEnvelope) || !Reflect.ownKeys(detachedEnvelope).every((key) => typeof key === "string" && ENVELOPE_KEYS.includes(key)) ||
        ...
        detachedEnvelope.kind !== "wallet_state")
        return invalid();
    let rebuilt;
    try {
        rebuilt = (0, exports.materializeOwnerRepositoryWalletStateBlob)(detachedEnvelope.payload);
    }
    catch {
        return invalid();
    }
...
exports.parseOwnerRepositoryWalletStateBlob = parseOwnerRepositoryWalletStateBlob;
//# sourceMappingURL=owner_repository_wallet_blob.js.map