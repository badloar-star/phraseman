...
const html = fs.readFileSync(path.resolve(__dirname, '../admin/v2/legacy.html'), 'utf8');
...
const directWritePattern = /\b(?:addDoc|setDoc|updateDoc|deleteDoc|writeBatch|runTransaction)\s*\(/;
...
describe('live admin sensitive write handlers', () => {
  ...
  test.each(Object.entries(handlers))('%s delegates writes to verified callables', (_name, body) => {
  ...
  test('uses the exact protected write lanes for each domain', () => {
    expect(handlers.adminNavSaveToServer).toContain('getAdminPublishNavLayoutCallable');
    expect(handlers.deactivateVipSurveyMessages).toContain('getAdminDeactivateVipSurveyCampaignCallable');
    expect(handlers.sendVipSurveyCampaign).toContain('getAdminLaunchVipSurveyCampaignCallable');
    expect(handlers.saveAlertsConfig).toContain('getAdminPublishAlertsConfigCallable');
    expect(handlers.sendAlertTest).toContain('getAdminTestAlertsCallable');
    expect(handlers.saveAppMessage).toMatch(/getAdmin(?:Create|Update)AppMessageCallable/);
    expect(handlers.toggleAppMessageActive).toContain('getAdminSetAppMessageActiveCallable');
    expect(handlers.deleteAppMessage).toContain('getAdminDeleteAppMessageCallable');
    expect(handlers.cleanupExpiredAppMessages).toContain('getAdminCleanupExpiredAppMessagesCallable');
    expect(handlers.leagueResetPoints).toContain('getAdminResetLeaguePointsCallable');
  ...
  test('alert testing does not submit or persist client-side alert configuration', () => {
    ...
    expect(handlers.sendAlertTest).not.toMatch(/document\.getElementById\(['"]alerts-/);
    const callStart = handlers.sendAlertTest.indexOf('getAdminTestAlertsCallable()({');
    const callEnd = handlers.sendAlertTest.indexOf('});', callStart);
    const payload = handlers.sendAlertTest.slice(callStart, callEnd);
    expect(payload).not.toMatch(/chatId|enabled|types|spikeThreshold|spikePerHour/);
  });
});
