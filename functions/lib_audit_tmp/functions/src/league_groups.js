...
function clampLeagueId(value, fallback = 0) {
    return Math.max(0, Math.min(MAX_LEAGUE_ID, readInt(value, fallback)));
}
async function resolveAuthoritativeLeagueId(db, stableUid, weekId, leaderboard, nowMs = Date.now()) {
    const leaderboardLeagueId = clampLeagueId(leaderboard?.leagueId, 0);
    if (sanitizeString(leaderboard?.groupWeekId, 16) === weekId)
    ...
    const mondayStartMs = Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() - ((now.getUTCDay() + 6) % 7));
    ...
    if (!resultSnap?.exists) {
        if (finalizationPending) {
            throw new https_1.HttpsError('failed-precondition', 'league_finalization_pending');
        }
        return leaderboardLeagueId;
    }
...
function leagueFallbackName(stableUid) {
    const suffix = sanitizeString(stableUid, 180).slice(-4).toUpperCase() || 'USER';
    return `Player ${suffix}`;
}
async function leagueNameOwnerIsLive(db, uid) {
...
async function legacyLeagueNameHasOtherLiveOwner(db, stableUid, name, nameLower) {
    const queries = [
    ...
    for (const query of queries) {
        ...
        for (const doc of snap?.docs ?? []) {
            ...
            if (!ownerUid || ownerUid === stableUid)
                continue;
            if (await leagueNameOwnerIsLive(db, ownerUid))
                return true;
        }
    }
    return false;
...
async function resolveAuthoritativeLeagueMemberName(db, stableUid, userData, leaderboardData, requestedName) {
    ...
    for (const candidate of candidates) {
        ...
        if (idxSnap?.exists && idxData.identityHidden !== true) {
            if (indexOwner === stableUid)
                return sanitizeString(idxData.name, 48) || name;
            continue;
        }
...
function getAuthoritativeLeagueWeekPoints(userData, leaderboard, weekId, nowMs = Date.now()) {
    ...
    const leaderboardPoints = leaderboard?.weekKey === weekId
        ? Math.max(0, Math.min(1000000000, readInt(leaderboard.weekPoints, 0)))
        : 0;
    return Math.max(progressPoints, leaderboardPoints);
}
async function resolveAuthoritativeLeagueFields(db, stableUid, userData, authUid, weekId, leaderboard) {
    const progress = userData?.progress && typeof userData.progress === 'object'
        ? userData.progress
        : {};
...
async function findExistingGroupForUser(db, weekId, leagueId, stableUid) {
    ...
    while (true) {
        ...
        if (snap.docs.length < BROAD_GROUP_QUERY_LIMIT)
            break;
...
exports.leagueJoinOrUpdateGroup = (0, https_1.onCall)(callable_options_1.HOT_CALLABLE_OPTIONS, async (request) => {
    ...
    const rawMember = (request.data?.member || {});
    ...
    const authoritativeFields = await resolveAuthoritativeLeagueFields(db, stableUid, userSnap?.data(), authUid, weekId, lbSnap?.data());
...
exports.leagueUpdateMyMember = (0, https_1.onCall)(callable_options_1.HOT_CALLABLE_OPTIONS, async (request) => {
    ...
    const updates = {
        ...
        [`members.${stableUid}.name`]: await resolveAuthoritativeLeagueMemberName(db, stableUid, userData, lbSnap.data(), raw.name),
        updatedAt: Date.now(),
    };
    ...
    if (Object.prototype.hasOwnProperty.call(raw, 'profileCardLevel'))
        updates[`members.${stableUid}.profileCardLevel`] = Math.max(0, Math.min(1, readInt(raw.profileCardLevel, 0)));
    ...
    const authoritativeFields = await resolveAuthoritativeLeagueFields(db, stableUid, userData, authUid, groupWeekId, lbSnap.data());
...
async function activateLeagueGroupBoostForStableUid(db, stableUid) {
    await assertCanUseLeague(db, stableUid);
    const lbSnap = await db.collection('leaderboard').doc(stableUid).get();
    const groupId = String(lbSnap.data()?.groupId || '');
    const groupWeekId = String(lbSnap.data()?.groupWeekId || '');
    ...
    if (!groupId || groupWeekId !== getWeekId())
        throw new https_1.HttpsError('failed-precondition', 'no-current-group');
    const groupRef = db.collection('league_groups').doc(groupId);
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (!groupSnap.exists)
            throw new https_1.HttpsError('not-found', 'league-group-not-found');
        const groupData = groupSnap.data() || {};
        if (groupData.weekId !== groupWeekId || readInt(groupData.leagueId, 0) !== leagueId) {
            throw new https_1.HttpsError('permission-denied', 'room-mismatch');
        }
        const members = groupData.members && typeof groupData.members === 'object'
            ? groupData.members
            : {};
        const buyer = members[stableUid];
        if (!buyer || buyer.identityHidden === true)
            throw new https_1.HttpsError('permission-denied', 'not-group-member');
