...
export const adminProductAnalytics = onCall({
  ...
  const trueRetentionDays: Record<string, unknown>[] = [];
  const activeDayBuckets: Record<string, unknown>[] = [];
  const acquisitionChannels: Record<string, unknown>[] = [];
  ...
  for (const row of rows) {
    ...
    else if (row.row_kind === 'true_retention_day') trueRetentionDays.push(payload);
    else if (row.row_kind === 'true_retention_summary') trueRetentionSummary = payload;
  ...
  const value = {
    ...
    acquisition: {
      firebaseFirstTouch: {
        ...
        entity: 'app_instance',
        channels: acquisitionChannels.sort((a, b) => (
          Number(b.consented_first_touch_app_instances ?? 0) - Number(a.consented_first_touch_app_instances ?? 0)
        )),
      },
