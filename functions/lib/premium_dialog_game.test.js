...
describe('premium dialog game mechanics', () => {
    ...
    describe('gameBlock injection into scenario prompt', () => {
        ...
        it('low-patience temperament seeds a lower start mood than high', () => {
            const high = (0, premium_dialog_1.buildScenarioSystemPrompt)('A2', { objectives, temperament: { patience: 'high' } });
            const low = (0, premium_dialog_1.buildScenarioSystemPrompt)('A2', { objectives, temperament: { patience: 'low' } });
        ...
        it('warmth shifts the seed mood (H7: matches client temperamentStartMood)', () => {
            const warm = (0, premium_dialog_1.buildScenarioSystemPrompt)('A2', { objectives, temperament: { patience: 'high', warmth: 'warm' } });
            const cold = (0, premium_dialog_1.buildScenarioSystemPrompt)('A2', { objectives, temperament: { patience: 'high', warmth: 'cold' } });
        ...
        it('strips control chars from objective text (H10: prompt-injection guard)', () => {
            ...
            const prompt = (0, premium_dialog_1.buildScenarioSystemPrompt)('A2', { objectives: evil });
    ...
    describe('modelSupportsJsonObject (C1: game-mode gate)', () => {
        it('nano (default) does NOT support json_object → game mode off', () => {
            expect((0, openai_dialog_model_config_1.modelSupportsJsonObject)('gpt-4.1-nano')).toBe(false);
        });
        it('gpt-4o-mini and gpt-4.1 do support it', () => {
            expect((0, openai_dialog_model_config_1.modelSupportsJsonObject)('gpt-4o-mini')).toBe(true);
            expect((0, openai_dialog_model_config_1.modelSupportsJsonObject)('gpt-4.1')).toBe(true);
            expect((0, openai_dialog_model_config_1.modelSupportsJsonObject)('gpt-4.1-mini')).toBe(true);
        });
        it('unknown model → false (safe)', () => {
            expect((0, openai_dialog_model_config_1.modelSupportsJsonObject)('some-other-model')).toBe(false);
        });
    });
    describe('parseGameEnvelope — robust parsing + fallback', () => {
        it('parses a clean JSON envelope', () => {
            const out = (0, premium_dialog_1.parseGameEnvelope)('{"reply":"What size?","mood":80,"objectivesMet":["order_drink"],"outcome":"ongoing","characterReaction":"","coachTips":[]}');
            expect(out).not.toBeNull();
            expect(out.reply).toBe('What size?');
            expect(out.turnState.mood).toBe(80);
        });
        it('strips ```json fences', () => {
            const out = (0, premium_dialog_1.parseGameEnvelope)('```json\n{"reply":"Hi there"}\n```');
            expect(out).not.toBeNull();
            expect(out.reply).toBe('Hi there');
        });
        it('returns null on broken JSON with no extractable reply', () => {
            expect((0, premium_dialog_1.parseGameEnvelope)('not json at all')).toBeNull();
            expect((0, premium_dialog_1.parseGameEnvelope)('{"reply":')).toBeNull();
        });
        it('returns null when reply field is missing/empty', () => {
            expect((0, premium_dialog_1.parseGameEnvelope)('{"mood":50}')).toBeNull();
            expect((0, premium_dialog_1.parseGameEnvelope)('{"reply":""}')).toBeNull();
        });
        it('recovers reply from TRUNCATED JSON instead of leaking raw JSON (H1)', () => {
            const out = (0, premium_dialog_1.parseGameEnvelope)('{"reply":"Sure, what size would you like?","mood":80,"objecti');
            expect(out).not.toBeNull();
        ...
        it('clamps out-of-range mood at the server boundary (L1)', () => {
            const out = (0, premium_dialog_1.parseGameEnvelope)('{"reply":"Hi","mood":250}');
            expect(out.turnState.mood).toBe(100);
        });
        it('downgrades success to stalled when not all objectives met (H4)', () => {
            const out = (0, premium_dialog_1.parseGameEnvelope)('{"reply":"Bye","outcome":"success","objectivesMet":["order_drink"]}', ['order_drink', 'ask_price']);
            expect(out.turnState.outcome).toBe('stalled');
        });
        it('keeps success when ALL objectives met (H4)', () => {
            const out = (0, premium_dialog_1.parseGameEnvelope)('{"reply":"Bye","outcome":"success","objectivesMet":["order_drink","ask_price"]}', ['order_drink', 'ask_price']);
            expect(out.turnState.outcome).toBe('success');
        });
    });
});
//# sourceMappingURL=premium_dialog_game.test.js.map