(function youtubeAnalyticsModule(root, factory) {
  ...
  function renderQuality(snapshot) {
    ...
    return `<section class="card section youtube-quality" aria-labelledby="youtube-quality-title"><div class="card-header"><div><h2 id="youtube-quality-title">Что мы можем измерить</h2><p>Учитываются только анонимные экземпляры приложения, согласившиеся на аналитику. Процент согласия здесь вычислить нельзя. Переустановка приложения может создать новый анонимный экземпляр.</p></div><span class="badge ${quality.state === 'partial' ? 'warning' : quality.state === 'ready' ? 'success' : ''}">${quality.state === 'partial' ? 'Частичные данные' : quality.state === 'ready' ? 'Данные готовы' : 'Данных пока нет'}</span></div><div class="card-body"><p><strong>Свежесть:</strong> ${escapeHtml(dateTimeFromMicros(snapshot.dataThroughMicros))}. Firebase Analytics выгружается в BigQuery не в реальном времени.</p><p>Нажатия «открыть видео» и «открыть канал» фиксируются перед передачей ссылки операционной системе. Они не подтверждают фактический переход, просмотр или подписку.</p><p>Активное время считается только во время воспроизведения. Checkpoint обычно отправляется раз в 10 секунд; остановка приложения операционной системой может оставить небольшой непросчитанный хвост.</p><div class="youtube-quality-grid">${defects.map(([label, value]) => `<span><strong>${formatNumber(value)}</strong>${escapeHtml(label)}</span>`).join('')}</div><p class="hint">Валидацию прошли ${formatNumber(quality.acceptedEvents)} из ${formatNumber(quality.totalEvents)} событий (${formatPercent(quality.validationRatio)}).</p></div></section>`;
  }
