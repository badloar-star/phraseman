...
exports.TOURNAMENT_FALLBACK_NAME = exports.TOURNAMENT_HOT_STREAK_TIERS = exports.TOURNAMENT_BANK_RATE = exports.TOURNAMENT_SEASON_POINTS = exports.TOURNAMENT_PRIZES = exports.TOURNAMENT_TIEBREAK_WEIGHT = exports.TOURNAMENT_STAR_REST = exports.TOURNAMENT_STAR_SECOND = exports.TOURNAMENT_STAR_FIRST = exports.TOURNAMENT_STREAK_X2_THRESHOLD = exports.TOURNAMENT_STREAK_X15_THRESHOLD = exports.TOURNAMENT_MAX_SPEED_BONUS_RATIO = exports.TOURNAMENT_VOICE_BASE_MULTIPLIER = exports.TOURNAMENT_BASE_SCORE = void 0;
...
exports.TOURNAMENT_EARLY_ADVANCE_DELAY_MS = 2 * 1000;
...
exports.TOURNAMENT_ROUND_INTRO_MS = 3000;
/**
...
function tournamentEconomySnapshotForMode(raw, testMode) {
    ...
    return testMode
        ...
        : Object.freeze({ ...economy, entryGems: tournament_economy_1.DEFAULT_TOURNAMENT_ECONOMY.entryGems });
}
/**
...
function hasStrictSpeedMatchFieldContract(task) {
    ...
    if (rightOptions.length !== 6 || items.length !== 6
        || !boundedStringArray(rightOptions, {
            maxItems: exports.TOURNAMENT_TASK_LIMITS.maxTimeattackItems,
            maxItemBytes: exports.TOURNAMENT_TASK_LIMITS.optionBytes,
        }) || !rightOptions.every((value) => value.trim().split(/\s+/u).length >= 2))
        return false;
 ...
 */
exports.TOURNAMENT_TIEBREAK_WEIGHT = 0;
...
function scoreAnswer(input) {
    ...
    const stars = rank === 1
        ? exports.TOURNAMENT_STAR_FIRST
        : rank === 2
            ? exports.TOURNAMENT_STAR_SECOND
            : exports.TOURNAMENT_STAR_REST;
...
function applyTournamentSubmission(room, submission) {
    ...
    if (submission.answerRanksByPlayer) {
        for (const [resultPlayerId, storedResult] of Object.entries(rounds[roundIndex].results)) {
            ...
            if (!canonicalRanks || !storedResult.review?.length)
                continue;
            const canReconcile = storedResult.review.every((entry) => (!entry.correct || entry.starsAwarded !== undefined || resultPlayerId === submission.playerId));
            if (!canReconcile)
                continue;
...
function planTournamentFinalization(room, nowMs, economy = tournament_economy_1.DEFAULT_TOURNAMENT_ECONOMY) {
    ...
    const botPrizeGems = standings.reduce((total, player) => (player.isBot ? total + (payoutByPlayerId.get(player.id)?.gems ?? 0) : total), 0);
...
function selectTournamentExposureDeck(candidates, roomId, roundNo, count, excludedTaskIds) {
    if (candidates.length === 0)
        return null;
    const exposureTag = TOURNAMENT_EXPOSURE_TAGS.find((tag) => (candidates.every((task) => task.tags?.includes(tag))));
    if (!exposureTag)
        return null;
