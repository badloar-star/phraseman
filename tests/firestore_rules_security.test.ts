...
function rootMatchBlocks(source: string): ParsedMatchBlock[] {
  ...
  const databaseHeader =
    /\bmatch\s+\/databases\/\{database\}\/documents\s*\{/g.exec(executable);
  if (!databaseHeader || databaseHeader.index === undefined) return [];
  const databaseOpenBrace =
    databaseHeader.index + databaseHeader[0].lastIndexOf("{");
...
function identityRuleGuardViolations(source: string): string[] {
  ...
  const exactBlock = (pathPattern: string) =>
    blocks.filter((block) => block.path === pathPattern);
  ...
  if (catchAllBlocks.length !== 1) {
    ...
    if (!catchAll || catchAll.operations !== "read, write") {
      ...
      const conjuncts = catchAll.expression
        .split("&&")
        .map((term) => term.trim());
...
${indent}}`,
      ...
      expect(identityRuleGuardViolations(mutated)).toContain(
        "auth_links:exact_root_count",
      );
    },
  );
...
${indent}}`,
  ...
  test("users create rejects client identity poisoning, including browser admins", () => {
    const fields = rules.match(
      /function serverOwnedUserIdentityFields\(\) \{[\s\S]*?\n    \}/,
    );
    const guard = rules.match(
      /function newDocHasNoServerIdentityWrites\(\) \{[\s\S]*?\n    \}/,
    );
  ...
  test("users update and delete cannot change or remove server-owned identity", () => {
    const updateGuard = rules.match(
      /function hasNoServerIdentityWrites\(\) \{[\s\S]*?\n    \}/,
    );
    const deleteGuard = rules.match(
      /function userDocHasNoServerIdentity\(\) \{[\s\S]*?\n    \}/,
    );
    ...
    expect(updateGuard![0]).toContain(
      ".hasAny(serverOwnedUserIdentityFields())",
    );
    expect(deleteGuard![0]).toContain(
      ".hasAny(serverOwnedUserIdentityFields())",
    );
    expect(rules).toMatch(
  ...
  test("Arena runtime pool is client-readable but server-owned", () => {
    expect(rules).toMatch(
      /match \/arena_questions\/\{qId\} \{[\s\S]*?allow read:\s*if request\.auth != null;[\s\S]*?allow write:\s*if false;/,
    );
  });
  ...
  test("users collection is restricted to owner/admin, including stableId auth mapping", () => {
    ...
    expect(rules).toContain(
      "stableUserMatchesAuth(userId) || authLinkMapsToUser(userId)",
    );
    ...
    expect(rules).toMatch(
      /allow update:[^;]*accountDeletionNotPending\(userId\)/,
    );
    expect(rules).toMatch(
      /allow create:[^;]*accountDeletionNotPending\(userId\)/,
    );
  ...
  test("read of a non-existent user doc is allowed (so the 1.5.41 sign-in tx survives)", () => {
    ...
    expect(rules).not.toContain(
      "allow delete: if userDocOwnerMatchesAuth(userId) || userDocMissing(userId);",
    );
  ...
  const SERVER_OWNED_PROGRESS_KEYS = [
    ...
    ...(["A1", "A2", "B1", "B2", "C1", "C2", "final"] as const).flatMap(
      (level) =>
        (
          ["pct", "best_pct", "passed", "pass_count", "completed_at"] as const
        ).flatMap((field) => [
  ...
  test("cloud_sync filters representative server-owned progress keys blocked by rules", () => {
    ...
    expect(cloudSync).toMatch(
      /SERVER_OWNED_PROGRESS_KEYS[\s\S]*?'shard_survey_last_at_ms'/,
    );
    expect(
      cloudSync.slice(0, cloudSync.indexOf("SERVER_OWNED_PROGRESS_KEYS")),
  ...
  test("leaderboard writes are restricted to admin callables while owners can delete", () => {
    const leaderboardBlock = rules.match(
      /match \/leaderboard\/\{userId\} \{[\s\S]*?\n    \}/,
    );
    expect(leaderboardBlock).not.toBeNull();
    expect(leaderboardBlock![0]).toContain(
    ...
    expect(leaderboardBlock![0]).not.toContain(
  ...
  test("leaderboard_stats aggregate is client-readable but admin-write only", () => {
    const statsBlock = rules.match(
      /match \/leaderboard_stats\/\{docId\} \{[\s\S]*?\n    \}/,
    );
    expect(statsBlock).not.toBeNull();
    expect(statsBlock![0]).toContain("allow read: if true;");
    expect(statsBlock![0]).toContain(
      "allow create, update, delete: if isAdmin();",
    );
    expect(statsBlock![0]).not.toContain(
      "allow write: if request.auth != null;",
    );
  });

  test("card_packs marketplace exposes only published metadata to clients", () => {
    const cardPacksBlock = rules.match(
      /match \/card_packs\/\{packId\} \{[\s\S]*?\n    \}/,
    );
    ...
    expect(cardPacksBlock![0]).not.toContain(
      "allow write: if request.auth != null;",
    );
  });

  test("league_groups uses Cloud Functions for writes and keeps client reads only", () => {
    const leagueGroupsBlock = rules.match(
      /match \/league_groups\/\{groupId\} \{[\s\S]*?\n    \}/,
    );
    expect(leagueGroupsBlock).not.toBeNull();
    expect(leagueGroupsBlock![0]).toContain(
      "allow read: if request.auth != null;",
    );
    expect(leagueGroupsBlock![0]).toContain(
      "allow create, update, delete: if isAdmin();",
    );
    expect(leagueGroupsBlock![0]).not.toContain(
      "allow write: if request.auth != null;",
    );
    expect(leagueGroupsBlock![0]).not.toContain(
  ...
  test("admin catch-all remains admin-only and is followed by a terminal deny", () => {
    ...
    expect(catchAllBlock![0]).toMatch(
      /allow read, write: if isAdmin\(\)[\s\S]*?;/,
    );
  ...
  test("Learning V2 required-session authority and transaction state stay server-only", () => {
    for (const path of [
      ...
      "users/{uid}/v2_activity_released_completion_inbox/{docId}",
    ]) {
      const escaped = path.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
      const block = rules.match(
        new RegExp(`match /${escaped} \\{[\\s\\S]*?\\n    \\}`),
      );
    ...
    expect(catchAllBlock?.[0]).toContain(
  ...
  test("cloud sync strips every server-owned gift entitlement key", () => {
    ...
    const keySet = cloudSync.match(
      /const PREMIUM_PROGRESS_KEYS = new Set\(\[([\s\S]*?)\]\);/,
    );
  ...
  test("app diagnostics collections are server/admin-write only with admin read", () => {
    ...
    expect(rules).toMatch(
      /match \/app_errors\/\{docId\} \{[\s\S]*?allow create: if false;[\s\S]*?allow read, update, delete: if isAdmin\(\);/,
    );
    expect(rules).toMatch(
      /match \/app_activity\/\{docId\} \{[\s\S]*?allow create: if false;[\s\S]*?allow read, update, delete: if isAdmin\(\);/,
    );
  });

  test("website checkout admin page can save prices and manage web orders only as admin", () => {
    const checkoutBlock = rules.match(
      /match \/web_checkout\/\{docId\} \{[\s\S]*?\n    \}/,
    );
    expect(checkoutBlock).not.toBeNull();
    expect(checkoutBlock![0]).toContain(
      "allow read, create, update: if isAdmin();",
    );
    expect(checkoutBlock![0]).toContain("allow delete: if false;");
    expect(checkoutBlock![0]).not.toContain("request.auth != null");

    const orderBlock = rules.match(
  ...
  test("admin email contacts and campaigns are server-created with admin read/update", () => {
    const contactsBlock = rules.match(
      /match \/email_contacts\/\{docId\} \{[\s\S]*?\n    \}/,
    );
    expect(contactsBlock).not.toBeNull();
    expect(contactsBlock![0]).toContain(
      "allow read, update, delete: if isAdmin();",
    );
    expect(contactsBlock![0]).toContain("allow create: if false;");
    expect(contactsBlock![0]).not.toContain("request.auth != null");

    const campaignsBlock = rules.match(
      /match \/email_campaigns\/\{docId\} \{[\s\S]*?\n    \}/,
    );
    expect(campaignsBlock).not.toBeNull();
    expect(campaignsBlock![0]).toContain(
      "allow read, update, delete: if isAdmin();",
    );
  ...
  test("arena_rooms is decommissioned and denies all client access", () => {
    const arenaRoomsBlock = rules.match(
      /match \/arena_rooms\/\{roomId\} \{[\s\S]*?\n    \}/,
    );
  ...
  test("arena_questions runtime pool is decommissioned and denies all client access", () => {
    const arenaQuestionsBlock = rules.match(
      /match \/arena_questions\/\{qId\} \{[\s\S]*?\n    \}/,
    );
  ...
  test("arena_invites legacy flow is decommissioned and denies all client access", () => {
    const arenaInvitesBlock = rules.match(
      /match \/arena_invites\/\{inviteId\} \{[\s\S]*?\n    \}/,
    );
  ...
  test("Arena V2 exposes only owner projections and participant public matches", () => {
    expect(rules).toMatch(
      /arena_v2_config\/\{configId\} \{[\s\S]*?allow read, write: if false;/,
    );
    ...
    expect(rules).toContain(
      "arena_v2_matches/$(matchId)/arena_v2_members/$(request.auth.uid)",
    );
    ...
    expect(rules).toMatch(
      /arena_v2_match_private\/\{matchId\} \{[\s\S]*?allow read, write: if false;/,
    );
    expect(rules).toMatch(
      /arena_v2_queue_locks\/\{lockId\} \{[\s\S]*?allow read, write: if false;/,
    );
    expect(rules).toMatch(
      /arena_v2_pair_limits\/\{pairId\} \{[\s\S]*?allow read, write: if false;/,
    );
    expect(rules).toMatch(
  ...
  test("Arena Expansion seals task evidence and shared social state", () => {
    for (const collection of [
      ...
      expect(rules).toMatch(
        new RegExp(
          `${collection}/\\{[^}]+\\} \\{[\\s\\S]*?allow read, write: if false;`,
        ),
      );
    }
  });
  ...
  test("friend activity my_events keeps client reads but restricts writes to admins/server", () => {
    ...
    expect(rules).toMatch(
      /my_events\/\{eventId\} \{[\s\S]*?allow read: if request\.auth != null;/,
    );
    expect(rules).toMatch(
      /my_events\/\{eventId\} \{[\s\S]*?allow create, update, delete: if isAdmin\(\);/,
    );
    expect(rules).not.toMatch(
      /my_events\/\{eventId\} \{[\s\S]*?allow create, update, delete: if canonicalUserMatchesAuth\(userId\);/,
    );
  });
  ...
  test("friend activity likes are server-written with readable counters and owner-only daily state", () => {
    ...
    expect(rules).toMatch(
      /friend_activity_like_daily_limits\/\{dayId\} \{[\s\S]*?allow read: if canonicalUserMatchesAuth\(userId\);/,
    );
    expect(rules).toMatch(
      /friend_activity_like_daily_limits\/\{dayId\} \{[\s\S]*?allow create, update, delete: if isAdmin\(\);/,
    );
    expect(rules).toContain(
      "match /users/{userId}/friend_activity_likes_sent/{likeId} {",
    );
    expect(rules).toMatch(
      /friend_activity_likes_sent\/\{likeId\} \{[\s\S]*?allow read: if canonicalUserMatchesAuth\(userId\);/,
    );
    expect(rules).toMatch(
      /friend_activity_likes_sent\/\{likeId\} \{[\s\S]*?allow create, update, delete: if isAdmin\(\);/,
    );
    expect(rules).toMatch(
      /activity_like_stats\/\{docId\} \{[\s\S]*?allow read: if request\.auth != null;/,
    );
    expect(rules).toMatch(
      /activity_like_stats\/\{docId\} \{[\s\S]*?allow create, update, delete: if isAdmin\(\);/,
    );
  });
  ...
  test("app messages allow user-owned read state and per-message reactions", () => {
    ...
    expect(rules).toMatch(
      /app_message_states\/\{messageId\} \{[\s\S]*?allow read, create, update, delete: if userDocOwnerMatchesAuth\(userId\);/,
    );
    expect(rules).toMatch(
      /app_messages\/\{messageId\} \{[\s\S]*?allow read: if request\.auth != null;/,
    );
    expect(rules).toMatch(
      /reactions\/\{userId\} \{[\s\S]*?request\.resource\.data\.messageId == messageId/,
    );
    expect(rules).toMatch(
      /reactions\/\{userId\} \{[\s\S]*?request\.resource\.data\.reaction in \['like', 'dislike'\]/,
    );
    expect(rules).toContain("match /poll_votes/{userId} {");
    expect(rules).toMatch(
      /poll_votes\/\{userId\} \{[\s\S]*?allow create, update: if appMessagePollVoteOk\(messageId, userId\);/,
    );
    expect(rules).toMatch(
      /function appMessagePollVoteOk\(messageId, userId\) \{[\s\S]*?request\.resource\.data\.optionId in message\.poll\.optionIds/,
    );
  });
  ...
  test.each([
    ...
    "%s is server-only for every browser client, including admins",
    (collection) => {
  ...
  test("support catch-all helper covers every sealed support namespace without an oversized expression", () => {
    const helper =
      rules.match(
        /function isServerOwnedSupportRoot\(collection\) \{[\s\S]*?\n    \}/,
      )?.[0] ?? "";
    expect(helper).toContain("support_auto_reply_(reservations|counters)");
    expect(helper).toContain(
      "support_telegram_(reviews|edit_sessions|reply_jobs)",
    );
  ...
  test("support auto-reply config is readable by admins but writable only through Admin SDK callables", () => {
    ...
    expect(rules).toMatch(
      /function isServerOwnedSupportRoot\(collection\)[\s\S]*?collection == 'admin_config'/,
    );
  });
  ...
  test("support inbox correspondence is returned only by the permission-checked callable", () => {
    ...
    expect(rules).toMatch(
      /function isServerOwnedSupportRoot\(collection\)[\s\S]*?collection == 'support_inbox'/,
    );
  });
  ...
  test("account deletion auth marker keeps exact owner read and denies every browser write", () => {
    const blocks = exactRootMatchBlocks(
      "account_deletion_auth_markers/{authUid}",
    );
    expect(blocks).toHaveLength(1);
    expect(activeAllowLines(blocks[0])).toEqual([
  ...
  test("user_consents writers (age bracket, AI-explain, AI-dialog consent) all use the Admin SDK path", () => {
    ...
    const explainCallable = readFileSync(
      path.join(process.cwd(), "functions/src/record_ai_explain_consent.ts"),
      "utf8",
    );
    expect(explainCallable).toContain(
      "createRecordAiConsentCallable('aiExplainConsent')",
    );
    const dialogCallable = readFileSync(
      path.join(process.cwd(), "functions/src/record_ai_dialog_consent.ts"),
      "utf8",
    );
    expect(dialogCallable).toContain(
      "createRecordAiConsentCallable('aiDialogConsent')",
    );
  });

  test("auth_links server writes remain on the Admin SDK path", () => {
    const authIdentity = readFileSync(
      path.join(process.cwd(), "functions/src/auth_identity.ts"),
      "utf8",
    );
...
describe("firestore.rules friend system (Phase 1)", () => {
  ...
  test("friend_code_index rule allows authenticated read", () => {
    ...
    expect(rules).toMatch(
      /match \/friend_code_index\/\{code\} \{[\s\S]*?allow read: if request\.auth != null;/,
    );
  });

  test("friend_code_index writes are restricted to Cloud Functions/admin", () => {
    const friendCodeBlock = rules.match(
      /match \/friend_code_index\/\{code\} \{[\s\S]*?\n    \}/,
    );
    expect(friendCodeBlock).not.toBeNull();
    expect(friendCodeBlock![0]).toContain(
      "allow read: if request.auth != null;",
    );
    expect(friendCodeBlock![0]).toContain(
      "allow create, update, delete: if isAdmin();",
    );
    expect(friendCodeBlock![0]).not.toContain(
  ...
  test("friend_code_index write path is exported through callable function", () => {
    ...
    const friendCodes = readFileSync(
  ...
  test("friend_requests create requires senderUid to match the signed-in canonical user (anti-impersonation)", () => {
    ...
    expect(rules).toMatch(
      /friend_requests\/\{senderUid\}[\s\S]*?canonicalUserMatchesAuth\(senderUid\)/,
    );
  });

  test("friend_requests create checks banned_users (SEC-05)", () => {
    expect(rules).toMatch(
      /friend_requests\/\{senderUid\}[\s\S]*?!exists\(\/databases\/\$\(database\)\/documents\/banned_users\/\$\(senderUid\)\)/,
    );
  });

  test("friend_requests create requires status == pending", () => {
    expect(rules).toMatch(
      /friend_requests\/\{senderUid\}[\s\S]*?request\.resource\.data\.status == 'pending'/,
    );
  });

  test("friend_requests create permits only a bounded sender display name", () => {
    expect(rules).toMatch(
      /friend_requests\/\{senderUid\}[\s\S]*?fromName[\s\S]*?request\.resource\.data\.fromName is string[\s\S]*?request\.resource\.data\.fromName\.size\(\) <= 40/,
    );
  });

  test("friend_requests forbids self-targeting", () => {
    expect(rules).toMatch(
      /friend_requests\/\{senderUid\}[\s\S]*?targetUid != senderUid/,
    );
  });

  test("friend_requests update is field-restricted to status and updatedAt", () => {
    expect(rules).toMatch(
      /friend_requests\/\{senderUid\}[\s\S]*?affectedKeys\(\)[\s\S]*?\.hasOnly\(\['status', 'updatedAt'\]\)/,
    );
  });

  test("friend_requests update only permits accepted or declined statuses", () => {
    expect(rules).toMatch(
      /friend_requests\/\{senderUid\}[\s\S]*?request\.resource\.data\.status in \['accepted', 'declined'\]/,
    );
  });

  test("friend_requests delete restricted to target", () => {
    expect(rules).toMatch(
      /friend_requests\/\{senderUid\}[\s\S]*?allow delete: if canonicalUserMatchesAuth\(targetUid\) \|\| canonicalUserMatchesAuth\(senderUid\);/,
    );
  });
  ...
  test("friends subcollection rule exists with canonical owner create", () => {
    ...
    expect(rules).toMatch(
      /friends\/\{friendUid\}[\s\S]*?allow create: if request\.auth != null[\s\S]*?canonicalUserMatchesAuth\(ownerUid\)/,
    );
  });

  test("friends subcollection forbids self-friending", () => {
    expect(rules).toMatch(
      /friends\/\{friendUid\}[\s\S]*?ownerUid != friendUid/,
    );
  });

  test("friends subcollection forbids updates", () => {
    expect(rules).toMatch(
      /friends\/\{friendUid\}[\s\S]*?allow update: if false;/,
    );
  });
  ...
  test("friends subcollection delete allows ownerUid or friendUid (bidirectional)", () => {
    ...
    expect(rules).toMatch(
      /friends\/\{friendUid\}[\s\S]*?allow delete: if canonicalUserMatchesAuth\(ownerUid\) \|\| canonicalUserMatchesAuth\(friendUid\);/,
    );
  });
  ...
  test("shard operation and global reward claim receipts are server-only", () => {
    const receiptBlock = rules.match(
      /match \/shard_operation_receipts\/\{opId\} \{[\s\S]*?\n      \}/,
    );
    expect(receiptBlock).not.toBeNull();
    expect(receiptBlock![0]).toContain("allow read, write: if false;");

    const rewardClaimsBlock = rules.match(
      /match \/reward_claims\/\{claimId\} \{[\s\S]*?\n      \}/,
    );
    expect(rewardClaimsBlock).not.toBeNull();
    expect(rewardClaimsBlock![0]).toContain(
  ...
  test("gift perks reject client forge while shard inbox only permits seen fields", () => {
    ...
    const inbox = rules.match(
      /match \/shard_rewards\/\{rewardId\} \{[\s\S]*?\n      \}/,
    );
...
describe("firestore.rules coin exchange (coins → stars, 2026-07-20 plan §6)", () => {
  ...
  test("exchange state, history and trade ledger are server-only", () => {
    for (const name of [
      "economy",
      "economy_exchange_history",
      "coin_exchange_trades",
      "coin_migrations",
    ]) {
      const re = new RegExp(
        "match /" + name + "/\\{document=\\*\\*\\} \\{[\\s\\S]*?\\n    \\}",
      );
      const block = rules.match(re);
      expect(block).not.toBeNull();
      expect(block![0]).toContain("allow read, write: if false;");
      const catchAllStart = rules.indexOf(
        "match /{collection}/{document=**} {",
      );
  ...
  test("v2 star journal subcollection is server-only", () => {
    const block = rules.match(
      /match \/v2_star_journal\/\{entryId\} \{[\s\S]*?\n      \}/,
    );
    expect(block).not.toBeNull();
    expect(block![0]).toContain("allow read, write: if false;");
    const rewardReceipts = rules.match(
      /match \/v2_wallet_reward_receipts\/\{rewardId\} \{[\s\S]*?\n      \}/,
    );
    expect(rewardReceipts).not.toBeNull();
    expect(rewardReceipts![0]).toContain("allow read, write: if false;");
    const unlockReceipts = rules.match(
      /match \/v2_course_unlock_receipts\/\{unlockId\} \{[\s\S]*?\n      \}/,
    );
  ...
  test("star wallet and coin migration fields are covered by both shard write guards", () => {
    const guardBodies = rules.match(
      /function (?:hasNoShardWrites|newDocHasNoShardWrites)\(\) \{[\s\S]*?\n    \}/g,
    );
...
describe("firestore.rules Explain like I'm five (Phase 5)", () => {
  ...
  test("phrase_explanations: world-readable, content writes denied, admin may only DELETE (reset)", () => {
    const block = rules.match(
      /match \/phrase_explanations\/\{phraseHash\} \{[\s\S]*?\n    \}/,
    );
    expect(block).not.toBeNull();
  ...
  for (const collection of SERVER_ONLY_EXPLAIN_COLLECTIONS) {
    test(`${collection} is server-only: read AND write are denied (if false, not isAdmin())`, () => {
      const block = rules.match(
        new RegExp(`match /${collection}/\\{docId\\} \\{[\\s\\S]*?\\n    \\}`),
      );
      expect(block).not.toBeNull();
  ...
  test("explain_reports (счётчики): admin read+delete, никакой клиентской записи", () => {
    const block = rules.match(
      /match \/explain_reports\/\{docId\} \{[\s\S]*?\n    \}/,
    );
    expect(block).not.toBeNull();
  ...
  test("explain_report_entries (лента жалоб): admin read/update/delete, create только CF", () => {
    const block = rules.match(
      /match \/explain_report_entries\/\{docId\} \{[\s\S]*?\n    \}/,
    );
    expect(block).not.toBeNull();
...
describe("firestore.rules Compass why-now AI", () => {
  const rules = readFileSync(rulesPath, "utf8");
  const collections = [
    "compass_why_cache",
  ...
  test.each(collections)("%s remains callable/Admin-SDK only", (collection) => {
    const block = rules.match(
      new RegExp(`match /${collection}/\\{docId\\} \\{[\\s\\S]*?\\n    \\}`),
    );
    expect(block).not.toBeNull();
...
describe("RevenueCat premium lineage roots are server-only", () => {
  ...
  test("RevenueCat premium event receipts allow admin listing but deny browser-admin mutation", () => {
    const block = rules.match(
      /match \/revenuecat_premium_events\/\{eventId\} \{[\s\S]*?\n    \}/,
    );
    expect(block).not.toBeNull();
...
describe("firestore.rules YouTube catalog access contract", () => {
  ...
  test("signed-in clients read only the public pointer and active ready snapshot", () => {
    ...
    expect(rules).toMatch(
      /match \/youtube_catalog\/\{docId\} \{[\s\S]*?docId == 'public'[\s\S]*?request\.auth != null/,
    );
    expect(rules).toMatch(
      /docId in \['config', 'sync_state'\][\s\S]*?isAdmin\(\)/,
    );
    expect(rules).toMatch(
      /match \/youtube_catalog_snapshots\/\{version\} \{[\s\S]*?isActiveYoutubeSnapshot\(version\)[\s\S]*?resource\.data\.status == 'ready'/,
    );
    expect(rules).toMatch(
      /match \/\{document=\*\*\} \{[\s\S]*?isReadyYoutubeSnapshot\(version\)/,
    );
  });

  test("all browser writes and history access are denied, including browser admins", () => {
    const catalogBlock = rules.match(
      /match \/youtube_catalog\/\{docId\} \{[\s\S]*?\n    \}/,
    );
    expect(catalogBlock).not.toBeNull();
    expect(catalogBlock![0]).toContain(
      "allow create, update, delete: if false;",
    );
    expect(rules).toMatch(
      /match \/youtube_catalog_history\/\{document=\*\*\} \{\s*allow read, write: if false;\s*\}/,
    );
    const catchAll = rules.match(
      /match \/\{collection\}\/\{document=\*\*\} \{[\s\S]*?\n    \}/,
    );
    ...
    for (const root of [
      "youtube_catalog",
      "youtube_catalog_snapshots",
      "youtube_catalog_history",
    ]) {
