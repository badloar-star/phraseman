...
describe('parseProductAnalyticsPayload', () => {
    it('returns only parsed JSON records and never throws for invalid warehouse rows', () => {
        expect((0, admin_product_analytics_1.parseProductAnalyticsPayload)({ payload: '{"events":2}' })).toEqual({ events: 2 });
        expect((0, admin_product_analytics_1.parseProductAnalyticsPayload)({ payload: '{malformed' })).toBeNull();
        expect((0, admin_product_analytics_1.parseProductAnalyticsPayload)({ payload: '[{"events":2}]' })).toBeNull();
        expect((0, admin_product_analytics_1.parseProductAnalyticsPayload)({ payload: '"raw string"' })).toBeNull();
        expect((0, admin_product_analytics_1.parseProductAnalyticsPayload)({ payload: 'null' })).toBeNull();
        expect((0, admin_product_analytics_1.parseProductAnalyticsPayload)({})).toBeNull();
        expect((0, admin_product_analytics_1.parseProductAnalyticsPayload)({ payload: { events: 2 } }))
            .toBeNull();
    });
});
...
describe('bounded purchase-failure aggregate query', () => {
    it('queries only deduplicated purchase failures and returns at most ten governed groups', () => {
        const query = (0, admin_product_analytics_1.buildProductAnalyticsPurchaseFailureQuery)('`safe_dataset.events_*`');
        const source = fs_1.default.readFileSync(path_1.default.join(process.cwd(), 'src', 'admin_product_analytics.ts'), 'utf8');
