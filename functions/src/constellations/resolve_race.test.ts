...
type FakeRef = { path: string; collection: (n: string) => { doc: (id?: string) => FakeRef } };
function makeRef(path: string): FakeRef {
  return { path, collection: (n: string) => ({ doc: (id?: string) => makeRef(`${path}/${n}/${id ?? 'auto'}`) }) };
}

function buildDb() {
  return {
    collection: (name: string) => ({ doc: (id?: string) => makeRef(`${name}/${id ?? 'auto'}`) }),
    getAll: async (...refs: FakeRef[]) => refs.map((r) => snapFor(r.path)),
    runTransaction: async <T>(fn: (tx: {
      get: (r: FakeRef) => Promise<ReturnType<typeof snapFor>>;
      update: (r: FakeRef, d: DocData) => void;
      set: (r: FakeRef, d: DocData, o?: { merge?: boolean }) => void;
    }) => Promise<T>): Promise<T> => {
      const writes: Array<() => void> = [];
      const tx = {
        get: async (r: FakeRef) => snapFor(r.path),
        update: (r: FakeRef, d: DocData) => { writes.push(() => docs.set(r.path, { ...(docs.get(r.path) ?? {}), ...d })); },
        set: (r: FakeRef, d: DocData, o?: { merge?: boolean }) => {
...
jest.mock('firebase-functions/v2/https', () => ({ HttpsError: FakeHttpsError, onCall: (_o: unknown, h: unknown) => h }));
...
describe('resolveCurrentRound — гонка/re-read guard (7.3)', () => {
  ...
  test('второй конкурентный резолв выходит без изменений (phase уже не answer)', async () => {
    ...
    const afterFirst = docs.get(`constellation_matches/${MATCH_ID}`) as DocData;
    // Раунд перешёл: phase='choose', round инкрементнут.
    expect(afterFirst.phase).toBe('choose');
    const roundAfterFirst = afterFirst.round as number;
    ...
    await svc.resolveCurrentRound(MATCH_ID);
    const afterSecond = docs.get(`constellation_matches/${MATCH_ID}`) as DocData;
  ...
  test('резолв на матче не в фазе answer (choose) ничего не делает', async () => {
    ...
    await svc.resolveCurrentRound(MATCH_ID);
    const after = docs.get(`constellation_matches/${MATCH_ID}`) as DocData;
