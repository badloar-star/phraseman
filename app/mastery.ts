 ...
 *
 * ВАЖНО: повтор урока — БЕСПЛАТНЫЙ для всех (цена осколков = 0, см. MASTERY_REPLAY_COST_SHARDS).
 * Параметр `isPremium` НАМЕРЕННО не используется (`void isPremium`): это не баг и не дыра —
 * фича сознательно не гейтится премиумом. Не рекламируем «безлимит повторов» как платный перк
 * в достижимых пейволах (аудит #22, решение пользователя: mastery остаётся бесплатным).
 * Параметр оставлен в сигнатуре ради совместимости вызовов; убирать — отдельно.
 */
