...
function fakeDb() {
  ...
  const db = {
    ...
    runTransaction: async (work: (tx: FakeTx) => Promise<unknown>) =>
      work({
        async get(document) {
...
async function seedAndQueue(db: ReturnType<typeof fakeDb>) {
  const seeded = await handleAdminSeedV2E1DemoSource(
    { auth, data: {} },
    { db: db as never, now: () => "2026-07-25T10:00:00.000Z" },
  );
  const plan = await handleAdminCreateV2GenerationPlan(
    ...
    {
      db: db as never,
      resolveTemplate: async (value: unknown) => value,
...
describe("V2 E1 compilation worker", () => {
  it("seeds the demo source with a real canonical profile hash", async () => {
    ...
    const source = db.docs.get("content_v2_test_sources/ep-01") as Record<
      string,
      unknown
    >;
    expect(source).toBeDefined();
    expect(seeded.languageProfileRef.contentHash).toBe(
      hashCanonicalBody(source.languageProfile),
    );
  ...
  it("compiles the queued job into an artifact, succeeds stages and replays idempotently", async () => {
    ...
    const artifact = db.docs.get(
      "content_v2_test_compiled_units/ep-01",
    ) as Record<string, unknown>;
    expect(artifact).toBeDefined();
    expect(artifact.jobId).toBe(plan.jobId);
    const job = db.docs.get(
      `content_v2_generation_jobs/${plan.jobId}`,
    ) as Record<string, unknown>;
    expect(job.state).toBe("compiled");

    const stageStates = [...db.docs.entries()]
      .filter(([path]) => path.startsWith("content_factory_stages/"))
  ...
  it("fails closed on missing job, missing source and tampered profile", async () => {
    ...
    db.docs.set("content_v2_test_sources/ep-01", {
      ...source,
      languageProfile: {
        ...(source.languageProfile as Record<string, unknown>),
        targetLanguage: "de",
      },
    });
    await expect(
  ...
  it("runs a direct vertical-slice compilation without a queued job", async () => {
    const db = fakeDb();
    await handleAdminSeedV2E1DemoSource(
      { auth, data: {} },
      { db: db as never, now: () => "2026-07-25T11:00:00.000Z" },
    );
    ...
    const job = db.docs.get(
      "content_v2_test_generation_jobs/v2-e1-direct",
    ) as Record<string, unknown>;
