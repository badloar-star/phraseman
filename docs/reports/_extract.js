...
// Match a quoted string value following a key, allowing escaped quotes inside
function findValue(text, key, startIdx) {
  // Find key:'...'
...
for (const f of files) {
  ...
  // Split per top-level lesson section
  ...
  for (let li = 0; li < lessonSections.length; li++) {
    ...
    // Find each phrase by id occurrences
    const idRe = /id:\s*(?:'([^']+)'|(\d+))/g;
    let idm;
    const ids = [];
    ...
    for (let i = 0; i < ids.length; i++) {
      ...
      // Compute approximate line in original file
...
const filtered = phrases.filter(p => p.lesson >= 1 && p.lesson <= 27);
fs.writeFileSync('docs/reports/_phrases_extracted.json', JSON.stringify(filtered, null, 2));

const counts = {};
for (const p of filtered) counts[p.lesson] = (counts[p.lesson] || 0) + 1;
console.log('Total phrases L1-27:', filtered.length);
