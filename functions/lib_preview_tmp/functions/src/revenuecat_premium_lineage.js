...
exports.PREMIUM_RC_GRACE_MS = exports.MAX_OWNER_CANDIDATES = exports.MAX_OWNER_LINEAGES = void 0;
exports.chooseAuthoritativePremiumLineageState = chooseAuthoritativePremiumLineageState;
exports.canonicalPremiumLineageHash = canonicalPremiumLineageHash;
exports.normalizePremiumLineageEvent = normalizePremiumLineageEvent;
exports.receiptReplayDecision = receiptReplayDecision;
exports.boundPremiumOwnerCandidates = boundPremiumOwnerCandidates;
exports.applyPremiumLineageEvent = applyPremiumLineageEvent;
exports.aggregatePremiumLineages = aggregatePremiumLineages;
const crypto_1 = require("crypto");
const MAX_ID_LENGTH = 256;
exports.MAX_OWNER_LINEAGES = 64;
exports.MAX_OWNER_CANDIDATES = 16;
exports.PREMIUM_RC_GRACE_MS = 72 * 60 * 60 * 1000;
function chooseAuthoritativePremiumLineageState(left, right) {
    ...
    for (const field of ['lastAccessEventId', 'lastEventId']) {
        if (left[field] !== right[field])
            return left[field] > right[field] ? left : right;
    }
...
function bounded(raw, normalizeCase = false) {
...
function positiveSafeMs(raw) {
    const value = typeof raw === 'number' ? raw : Number(raw);
    return Number.isSafeInteger(value) && value > 0 ? value : null;
}
function eventPlan(productId) {
    const value = productId.toLowerCase();
    if (/lifetime|forever|one.?time|onetime|perpetual/.test(value))
        return 'lifetime';
    if (/year|yearly|annual|12.?month/.test(value))
        return 'yearly';
    return 'monthly';
}
function canonicalLineageParts(raw) {
    ...
    if (!appId || !environment || !store || !originalTransactionId)
        return null;
    return [appId, environment, store, originalTransactionId];
}
function canonicalPremiumLineageHash(raw) {
    const parts = canonicalLineageParts(raw);
    if (!parts)
        throw new Error('invalid_premium_lineage_identity');
    return sha256(parts.map((part) => `${part.length}:${part}`).join('|'));
}
function normalizePremiumLineageEvent(raw) {
    ...
    const parts = canonicalLineageParts(raw);
    if (!eventId)
        return { status: 'quarantine', reason: 'missing_or_invalid_event_id', rawFingerprint };
    if (!eventType)
        return { status: 'quarantine', reason: 'missing_or_invalid_event_type', rawFingerprint };
    if (eventTimeMs === null)
        return { status: 'quarantine', reason: 'missing_or_invalid_event_time', rawFingerprint };
    if (!parts)
        return { status: 'quarantine', reason: 'missing_or_invalid_lineage_identity', rawFingerprint };
...
function receiptReplayDecision(receipt, event) {
    return receipt.eventId === event.eventId && receipt.fingerprint === event.fingerprint
        ? 'duplicate'
        : 'conflict';
}
function boundPremiumOwnerCandidates(rawCandidates) {
    if (rawCandidates.length > exports.MAX_OWNER_CANDIDATES) {
...
function semanticEventRank(eventType, cancelReason = '') {
...
function accessTransitionRank(event, state) {
    if (event.eventType === 'REFUND_REVERSED')
        return 110;
    if (event.eventType === 'REFUND')
        return 100;
    if (event.eventType === 'EXPIRATION')
        return state.plan === 'lifetime' ? null : 90;
    if (event.eventType === 'CANCELLATION' && event.cancelReason === 'CUSTOMER_SUPPORT') {
        if (state.plan === 'lifetime' || event.expirationAtMs === null || event.expirationAtMs <= event.eventTimeMs)
            return 80;
        return null;
    }
    if (new Set([
...
function accessTransitionTieValue(event, rank) {
    if (rank === 50)
        return event.plan === 'lifetime'
            ? Number.MAX_SAFE_INTEGER
            : event.expirationAtMs ?? 0;
    return 0;
}
function isOlder(current, event) {
    ...
    if (eventAccessRank === null) {
        if (event.eventType === 'CANCELLATION' && event.cancelReason === 'CUSTOMER_SUPPORT'
            ...
            if (event.eventTimeMs !== supportTime)
                return event.eventTimeMs < supportTime;
        ...
        if (event.eventTimeMs !== metadataTime)
            return event.eventTimeMs < metadataTime;
    ...
    if (event.eventTimeMs !== accessTime)
        return event.eventTimeMs < accessTime;
    ...
    if (eventAccessRank !== currentRank)
        return eventAccessRank < currentRank;
    ...
    if (eventTie !== currentTie)
        return eventTie < currentTie;
    return event.eventId <= accessId;
}
function baseState(event) {
    const initial = {
...
function withMetadata(state, event) {
    ...
    const futureSupport = event.eventType === 'CANCELLATION'
        ...
        && state.plan !== 'lifetime';
    const supportIsNewer = futureSupport && (event.eventTimeMs >= state.lastAccessEventTimeMs
...
function recordGrantMetadata(state, event) {
    if (!isActivationEvent(event.eventType))
        return state;
...
function applyPremiumLineageEvent(current, event) {
...
function progressMs(raw) {
    if (raw == null || raw === '')
        return 0;
    if (typeof raw === 'object') {
        const value = raw;
        if (typeof value.toMillis === 'function')
            return progressMs(value.toMillis());
        if (typeof value.seconds === 'number')
            return progressMs(value.seconds * 1000);
    }
    const value = Number(raw);
    return Number.isSafeInteger(value) && value > 0 ? value : 0;
}
function legacyAdminMigrationPatch(progress, nowMs) {
    ...
    if (!hasLegacyAuthority && !(plan === 'admin_grant' && explicitRevoke))
        return {};
    const patch = { admin_premium_override: 'false' };
    if (explicitRevoke)
        return patch;
    ...
    const hasVipRepresentation = VIP_PROGRESS_KEYS.some((key) => Object.prototype.hasOwnProperty.call(progress, key));
    ...
    if (existingVipIsAtLeastAsStrong)
        return patch;
...
function aggregatePremiumLineages(lineages, existingProgress, nowMs) {
    const progressPatch = { ...existingProgress };
    if (!Number.isSafeInteger(nowMs) || nowMs <= 0 || lineages.length > exports.MAX_OWNER_LINEAGES) {
    ...
    active.sort((a, b) => {
        ...
        const planRank = (plan) => plan === 'yearly' ? 2 : plan === 'monthly' ? 1 : 3;
    ...
    const hasCanonicalAccessEvidence = lineages.some((lineage) => ((Number.isSafeInteger(lineage.lastAccessEventTimeMs) && lineage.lastAccessEventTimeMs > 0)
...
//# sourceMappingURL=revenuecat_premium_lineage.js.map