...
function guardLearnerFacingNote(key, note) {
    if (!note)
        return '';
    const lower = note.toLocaleLowerCase();
    const hasInternalBalancePhrase = /\d+\s*(?:\/\s*100\s*)?(?:балл|балла|баллов|points?|pts?|score)/i.test(note) ||
    ...
    if (key === 'balance' && hasInternalBalancePhrase)
        return '';
    return note;
}
// ── Callable ──────────────────────────────────────────────────────────────────
