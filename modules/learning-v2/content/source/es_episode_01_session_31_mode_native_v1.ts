...
// + СТАРТ ES §0/§10 + Rules §4.1, завершает Главу 4 "Мы и они" перед
// checkpoint-сессией 32): испанская voice-сессия 31 переписывается в
// mode-native формат, зеркаля КОД (не данные) сессий 7/15/23 — та же voice-
// механика. Voice-сессия НЕ вводит новых слов и не имеет newVocabulary —
// применяет 15 уже утверждённых фраз из сессий 25/26/27
// (es_episode_01_session_31_phrases_v1.ts переиспользует их целиком по id
// через findPhrase(), полностью локализованные). Тема озвучки — слитное
// произношение somos (SO-mos, два слога, без паузы перед признаком).
//
...
// которую нашли и исправили сессии 7/15/23 в легаси generic voiceSteps() —
...
// для этой сессии.
const phrases = ES_EPISODE_01_SESSION_31_VOICE_PHRASES;
if (phrases.length !== 15) {
  throw new Error('es_session_31_mode_native_phrase_count_invalid');
}

const L = (source: LocalizedSource): LearningV2Localized<string> => expandLocalized(source);

// зачем эта функция (владелец, 2026-08-28, тот же класс бага сессий 5/6/8-30):
...
// зачем 'en' вместо 'es' здесь (как в испанских сессиях 1-30): в испанском
...
// 25/26/27 через findPhrase() целиком, включая уже готовый localizedDetails.
function localizedPhraseField(
  phraseIndex: number,
  field: 'meaning' | 'explanation',
): LearningV2Localized<string> {
  const phrase = phrases[phraseIndex];
  if (!phrase?.localizedDetails) throw new Error(`es_session_31_phrase_details_missing:${phraseIndex}`);
  return Object.fromEntries(LEARNING_V2_INTERFACE_LOCALES.map((locale) => {
    ...
    if (!detail) throw new Error(`es_session_31_phrase_locale_missing:${phraseIndex}:${locale}`);
...
function localizedPhraseDistractorFeedback(
  ...
  return Object.fromEntries(LEARNING_V2_INTERFACE_LOCALES.map((locale) => {
    ...
    if (!reason) throw new Error(`es_session_31_phrase_distractor_missing:${phraseIndex}:${locale}:${value}`);
...
// зачем audio id namespace 's31' даже для переиспользованных фраз сессий
// 25/26/27 (владелец, 2026-08-28, тот же приём, что и сессии 7/15/23):
// аудио этой сессии — самостоятельные logical targets, а не алиасы на чужие
// clip id.
function phraseAudio(phraseIndex: number): LearningV2ModeAudioReferenceV1 {
  const phrase = phrases[phraseIndex]!;
  return authoredAudio(`es-e01-s31-phrase-${phraseIndex + 1}`, phrase.english);
}
function phraseSlowAudio(phraseIndex: number): LearningV2ModeAudioReferenceV1 {
  const phrase = phrases[phraseIndex]!;
  return authoredAudio(`es-e01-s31-phrase-${phraseIndex + 1}-slow`, phrase.english);
}
...
// es_episode_01_session_23_mode_native_v1.ts по составу и весу семей): три
...
// somos-фразы вслух без опоры, ядро voice-сессии.
export const ES_EPISODE_01_SESSION_31_MODE_NATIVE_PRACTICE_V1 = Object.freeze<readonly SessionModeNativePracticeSourceV1[]>([
...
if (ES_EPISODE_01_SESSION_31_MODE_NATIVE_PRACTICE_V1.length !== 12) {
  throw new Error('es_session_31_mode_native_practice_count_invalid');
}
