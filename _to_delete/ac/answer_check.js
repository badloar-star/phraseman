...
exports.ARENA_ANSWER_JOINER = void 0;
exports.arenaHash32 = arenaHash32;
exports.arenaCanonicalAnswerValue = arenaCanonicalAnswerValue;
exports.arenaAnswerFingerprint = arenaAnswerFingerprint;
exports.arenaAnswerIsCorrect = arenaAnswerIsCorrect;
exports.arenaPairIsCorrect = arenaPairIsCorrect;
/**
 ...
 * исходнике он записан невидимым литералом, поэтому здесь задан явным кодом.
 */
exports.ARENA_ANSWER_JOINER = '\u0001';
/** Канонический вид ответа: одинаковый на сервере и на клиенте. */
function arenaCanonicalAnswerValue(value) {
    if (Array.isArray(value)) {
        return value.map((item) => String(item).trim()).join(exports.ARENA_ANSWER_JOINER);
    }
    return String(value ?? '').trim();
}
function arenaAnswerFingerprint(saltId, taskId, answer) {
 ...
 */
function arenaAnswerIsCorrect(saltId, task, answer) {
    const expected = task.answerFingerprints;
    if (!expected.length)
        return false;
    const record = answer && typeof answer === 'object' ? answer : null;
    if (!record)
        return false;
    if (task.kind === 'choice' || task.kind === 'listen') {
        if (!Number.isInteger(record.selectedIndex))
            return false;
    ...
    if (!Array.isArray(selected) || selected.length !== expected.length)
        return false;
    return selected.every((value, index) => Number.isInteger(value) && expected[index] === arenaAnswerFingerprint(saltId, task.taskId, value));
}
/**
 * Верна ли одна пара. Пара проверяется своим отпечатком по номеру левой
 * карточки — ровно так же, как её проверит сервер.
 */
function arenaPairIsCorrect(saltId, task, pairIndex, selectedIndex) {
    const expected = task.answerFingerprints[pairIndex];
    if (!expected || !Number.isInteger(selectedIndex))
        return false;
