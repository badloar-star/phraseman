...
describe('Arena hub overflow', () => {
  it('contains the approved secondary destinations in a stable order', () => {
    const rows = arenaHubOverflowChoices('m-42', 'fr');
    ...
    expect(rows.find((row) => row.key === 'ranks')?.route).toEqual({
      pathname: '/arena_ranks',
      params: { studyTarget: 'fr' },
    });
    expect(rows.find((row) => row.key === 'review')?.route).toEqual({
      pathname: '/arena_review',
      params: { matchId: 'm-42', studyTarget: 'fr' },
    });
  });

  it('keeps latest review visible but disabled when no receipt is loaded', () => {
    const review = arenaHubOverflowChoices(null, 'en').find((row) => row.key === 'review');
  ...
  it('hides the star store entry while the catalogue is not ready', () => {
    const rows = arenaHubOverflowChoices('m-42', 'en');
  ...
  it('no longer exposes a separate Arena spin entry', () => {
    const keys = arenaHubOverflowChoices('m-42', 'en').map((row) => row.key as string);
