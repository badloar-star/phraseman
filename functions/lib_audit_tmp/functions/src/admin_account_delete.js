...
exports.adminQueueAccountDeletion = void 0;
exports.normalizeAdminAccountDeleteInput = normalizeAdminAccountDeleteInput;
exports.resolveAdminAccountDeleteIdentityEvidence = resolveAdminAccountDeleteIdentityEvidence;
exports.assertAdminAccountDeleteNotSelf = assertAdminAccountDeleteNotSelf;
exports.assertIdentityQueryNotTruncated = assertIdentityQueryNotTruncated;
exports.assertAdminAccountDeleteIdentityAnchors = assertAdminAccountDeleteIdentityAnchors;
exports.queuedDeletionJobMatchesIdentity = queuedDeletionJobMatchesIdentity;
...
function normalizeAdminAccountDeleteInput(data) {
    if (!isRecord(data))
        throw new https_1.HttpsError('invalid-argument', 'account deletion command required');
    ...
    if (!uid || !reason || !TOKEN_RE.test(requestId) || !TOKEN_RE.test(idempotencyKey)) {
        throw new https_1.HttpsError('invalid-argument', 'uid, reason, requestId and idempotencyKey are required');
    }
    return Object.freeze({ uid, reason, requestId, idempotencyKey });
}
function linkedAuthUid(user) {
    const linked = isRecord(user.linkedAuth) ? user.linkedAuth : {};
    return safeUid(linked.providerUid);
}
function canonicalStableEvidence(requestedUid, requestedUser, canonicalUser) {
    if (!requestedUser)
        throw new https_1.HttpsError('not-found', 'TARGET_USER_NOT_FOUND');
    if (requestedUser.identityHidden === true) {
        ...
        if (!canonicalStableId
            ...
            throw new https_1.HttpsError('failed-precondition', 'TARGET_CANONICAL_IDENTITY_UNRESOLVED');
        }
    ...
    if (conflictingCanonical && conflictingCanonical !== requestedUid) {
        throw new https_1.HttpsError('failed-precondition', 'TARGET_CANONICAL_IDENTITY_AMBIGUOUS');
    }
 ...
 */
function resolveAdminAccountDeleteIdentityEvidence(input) {
    const canonical = canonicalStableEvidence(input.requestedUid, input.requestedUser, input.canonicalUser);
    const authCandidates = new Set();
    const addAuthCandidate = (value) => {
        const uid = safeUid(value);
        if (uid)
            authCandidates.add(uid);
    };
    ...
    for (const link of input.authLinks) {
        ...
        if (linkedStableId !== canonical.stableUid) {
            throw new https_1.HttpsError('failed-precondition', 'TARGET_AUTH_LINK_CONFLICT');
        }
        addAuthCandidate(link.id);
        addAuthCandidate(link.data.providerUid);
    }
    if (authCandidates.size !== 1) {
        throw new https_1.HttpsError('failed-precondition', 'TARGET_AUTH_IDENTITY_AMBIGUOUS');
    }
    const authUid = [...authCandidates][0];
    if (safeUid(input.authRecordUid) !== authUid) {
        throw new https_1.HttpsError('failed-precondition', 'TARGET_AUTH_RECORD_UNRESOLVED');
    }
    if (input.authLinks.length === 0) {
        throw new https_1.HttpsError('failed-precondition', 'TARGET_AUTH_LINK_MISSING');
    }
    ...
    for (const owner of input.authOwners) {
        ...
        if (owner.data.identityHidden === true
            && safeUid(owner.data.canonicalStableId) === canonical.stableUid) {
            continue;
        }
        throw new https_1.HttpsError('failed-precondition', 'TARGET_AUTH_OWNER_AMBIGUOUS');
    }
    if (!canonicalOwnerFound) {
        throw new https_1.HttpsError('failed-precondition', 'TARGET_AUTH_OWNER_MISSING');
    }
    return Object.freeze({ stableUid: canonical.stableUid, authUid });
}
function assertAdminAccountDeleteNotSelf(input) {
    if (input.actorAuthUid === input.target.authUid
        || input.actorStableUids.includes(input.target.stableUid)) {
        throw new https_1.HttpsError('failed-precondition', 'ADMIN_CANNOT_DELETE_SELF');
    }
}
function assertIdentityQueryNotTruncated(sizes, limit) {
    if (!Number.isInteger(limit) || limit < 1 || sizes.some((size) => size >= limit)) {
        throw new https_1.HttpsError('failed-precondition', 'IDENTITY_EVIDENCE_LIMIT');
    }
}
function assertAdminAccountDeleteIdentityAnchors(input) {
    if (!input.requestedUser || !input.stableUser || input.stableUser.identityHidden === true) {
        throw new https_1.HttpsError('failed-precondition', 'TARGET_IDENTITY_ANCHOR_MISSING');
    }
    if (input.requestedUser.identityHidden === true) {
        if (safeUid(input.requestedUser.canonicalStableId) !== input.target.stableUid) {
            throw new https_1.HttpsError('failed-precondition', 'TARGET_CANONICAL_IDENTITY_CHANGED');
        }
    }
    ...
    if (userAuthUids.size !== 1 || !userAuthUids.has(input.target.authUid)) {
        throw new https_1.HttpsError('failed-precondition', 'TARGET_AUTH_IDENTITY_CHANGED');
    }
    if (!input.authLink) {
        throw new https_1.HttpsError('failed-precondition', 'TARGET_AUTH_LINK_MISSING');
    }
    ...
    if (linkedStableUid !== input.target.stableUid
        || (directLinkAuthUid && directLinkAuthUid !== input.target.authUid)) {
        throw new https_1.HttpsError('failed-precondition', 'TARGET_AUTH_LINK_CHANGED');
    }
}
function queuedDeletionJobMatchesIdentity(jobId, job, target) {
    if (jobId !== (0, account_delete_job_1.accountDeleteJobId)(target.authUid))
        return false;
    if (!['queued', 'running', 'completed', 'failed'].includes(text(job.status, 20)))
        return false;
    const authUidHash = (0, crypto_1.createHash)('sha256').update(target.authUid).digest('hex');
    const stableUidHash = (0, crypto_1.createHash)('sha256').update(target.stableUid).digest('hex');
    if (text(job.authUidHash, 64) !== authUidHash
...
function documentEvidence(snapshot) {
    return { id: snapshot.id, data: snapshot.data() ?? {} };
}
function uniqueDocumentEvidence(snapshots) {
    const byId = new Map();
    for (const snapshot of snapshots) {
        if (snapshot.exists)
            byId.set(snapshot.id, documentEvidence(snapshot));
    }
    return [...byId.values()];
}
async function resolveAdminAccountDeleteTarget(db, requestedUid) {
    ...
    const preliminaryAuthCandidates = new Set();
    const add = (value) => {
        const uid = safeUid(value);
        if (uid)
            preliminaryAuthCandidates.add(uid);
    };
    ...
    const [directLink, firebaseOwners, providerOwners, directAuthUser, authRecord] = await Promise.all([
        ...
        admin.auth().getUser(authUid).catch((error) => {
            const code = text(error?.code, 80);
            if (code === 'auth/user-not-found')
                return null;
            throw new https_1.HttpsError('unavailable', 'TARGET_AUTH_LOOKUP_FAILED');
        }),
    ]);
    ...
    ]).filter((owner) => (owner.id === stableEvidence.stableUid
...
async function actorStableUids(db, actorAuthUid) {
...
function replayOperation(operation, requestFingerprint, actorUid) {
    ...
    if (!stableUid || !authUid || !auditId) {
        throw new https_1.HttpsError('failed-precondition', 'ACCOUNT_DELETE_OPERATION_CORRUPT');
    }
    return {
...
function hardeningErrorCode(error) {
    const code = text(error?.code, 80);
    return /^auth\/[a-z0-9-]+$/.test(code) ? code : 'auth/hardening-failed';
}
async function hardenTargetAuth(identity) {
...
exports.adminQueueAccountDeletion = (0, https_1.onCall)(ADMIN_ACCOUNT_DELETE_OPTIONS, async (request) => {
    if (!request.app)
    ...
    if (!role || !(0, permissions_1.hasPermission)(role, 'users.delete')) {
        throw new https_1.HttpsError('permission-denied', 'USERS_DELETE_REQUIRED');
    }
    ...
    const prepared = await db.runTransaction(async (tx) => {
        ...
        if (operationSnapshot.exists) {
            ...
            const [requestedAnchor, stableAnchor, authLinkAnchor, queuedJob] = await Promise.all([
                ...
                tx.get(db.collection(account_delete_job_1.ACCOUNT_DELETE_JOBS).doc((0, account_delete_job_1.accountDeleteJobId)(replay.identity.authUid))),
            ]);
            try {
                ...
                if (!queuedJob.exists
                    || !queuedDeletionJobMatchesIdentity(queuedJob.id, queuedJob.data() ?? {}, replay.identity)) {
                    throw anchorError;
                }
            }
        ...
        if (!resolvedIdentity) {
            throw new https_1.HttpsError('aborted', 'TARGET_IDENTITY_RETRY_REQUIRED');
        }
    ...
    const queue = await (0, account_delete_job_1.enqueueAccountDeletionJob)(db, identity.authUid, identity.stableUid);
    ...
    return db.runTransaction(async (tx) => {
        const operationSnapshot = await tx.get(operationRef);
        if (!operationSnapshot.exists) {
            throw new https_1.HttpsError('aborted', 'ACCOUNT_DELETE_OPERATION_MISSING');
        }
        const replay = replayOperation(operationSnapshot.data() ?? {}, requestFingerprint, actorUid);
        if (replay.result?.ok === true)
...
//# sourceMappingURL=admin_account_delete.js.map