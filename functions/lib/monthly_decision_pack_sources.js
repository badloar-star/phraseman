...
async function loadDecisionPackAggregateInput(window, generatedAtMs) {
    const loadProductScope = async (startMs, endExclusiveMs, maximumBytesBilled) => {
        try {
            const result = await (0, admin_product_analytics_1.loadProductAnalyticsAggregateRows)({ startMs, endExclusiveMs, reportingTimezone: window.timezone, maximumBytesBilled });
