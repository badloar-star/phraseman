...
// + СТАРТ ES §0/§10): испанская сессия 19 переписывается в mode-native
// формат, зеркаля код (не данные) сессий 10/11/13/17 — тот же класс сессии
// (kind: 'phrases', НЕТ новых слов). Все 15 фраз держат ТОЛЬКО отрицание
// no + es (No es fácil, No es caro...) — завершение парадигмы отрицания
// связки ser по лицам (no soy сессия 2, no eres сессия 11, теперь no es).
// Фразовые данные и 8-локальные разборы взяты из легаси без изменений
// (MODE_NATIVE_AUTHORING_CONTRACT.ru.md §7).
//
...
// в сессиях 10/11/13/17): phraseSteps() уже использует только утверждённые
...
const phrases = ES_EPISODE_01_SESSION_19_PHRASES;
if (phrases.length !== 15) {
  throw new Error('es_session_19_mode_native_phrase_count_invalid');
}

const L = (source: LocalizedSource): LearningV2Localized<string> => expandLocalized(source);

// зачем эта функция (владелец, 2026-08-28, тот же класс бага сессий 5/6/8-18):
...
// Фразы этой сессии переиспользуют reasonCode с диакритикой (fácil/difícil/
// único/verdadero/rápido), context_gap_grammar передаёт responseId напрямую
// в runtime.
...
// зачем 'en' вместо 'es' здесь (как в испанских сессиях 1-18): в испанском
...
function localizedPhraseField(
  ...
  if (!phrase?.localizedDetails) throw new Error(`es_session_19_phrase_details_missing:${phraseIndex}`);
  return Object.fromEntries(LEARNING_V2_INTERFACE_LOCALES.map((locale) => {
    ...
    if (!detail) throw new Error(`es_session_19_phrase_locale_missing:${phraseIndex}:${locale}`);
...
function localizedPhraseDistractorFeedback(
  ...
  return Object.fromEntries(LEARNING_V2_INTERFACE_LOCALES.map((locale) => {
    ...
    if (!reason) throw new Error(`es_session_19_phrase_distractor_missing:${phraseIndex}:${locale}:${value}`);
...
function phraseAudio(phraseIndex: number): LearningV2ModeAudioReferenceV1 {
  const phrase = phrases[phraseIndex]!;
  return authoredAudio(`es-e01-s19-phrase-${phraseIndex + 1}`, phrase.english);
}
function phraseSlowAudio(phraseIndex: number): LearningV2ModeAudioReferenceV1 {
  const phrase = phrases[phraseIndex]!;
  return authoredAudio(`es-e01-s19-phrase-${phraseIndex + 1}-slow`, phrase.english);
}
...
function contextGapLastWord(phraseIndex: number): LearningV2ModeNativePayloadV1 {
  ...
  return Object.freeze({
    ...
    testedDimension: `negation_es:${asciiId(phrase.id)}`,
    choiceFeedback: Object.freeze([
      ...
      ...lastWord.distractors.map((entry) => feedback(
        ...
        false,
        `negation_es:${asciiId(entry.reasonCode)}`,
...
// приём, что сессии 11/17): пять фраз с no es, разбросанных по всей сессии
// (индексы 3/6/9/12/14) — широкий срез уже изученных отрицаний, а не
// смежный блок.
function speedMatchPhrases(pairIndices: readonly number[]): LearningV2ModeNativePayloadV1 {
  ...
  const ids = pairIndices.map((index) => `es-e01-s19-pair-${index + 1}`);
  return Object.freeze({
    family: 'speed_match',
...
// легаси generic phraseSteps(), как и в сессиях 10/11/13/17): те же 12
// families/purposes/индексов (3-14), реальный mode-native payload вместо
// generic-карточки для каждого шага.
export const ES_EPISODE_01_SESSION_19_MODE_NATIVE_PRACTICE_V1 = Object.freeze<readonly SessionModeNativePracticeSourceV1[]>([
...
if (ES_EPISODE_01_SESSION_19_MODE_NATIVE_PRACTICE_V1.length !== 12) {
  throw new Error('es_session_19_mode_native_practice_count_invalid');
}
