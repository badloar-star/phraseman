...
// Numeric energy: базовый шаг 6 минут; «на треть быстрее» даёт шаг 4 минуты.
// ОБЯЗАНО совпадать с клиентским
...
// цель сундука калибровалась под ОПЫТ (28 жителей давали ~159 000 XP в неделю,
// база 400 000 держала планку выше их вклада). После перевода лиги на руны
// (2026-08-26) шкала стала в разы мельче: замер по самой формуле жителей
// (residentTickGain, 168 тиков) даёт комнате из 28 человек ~55 000 РУН за
// неделю при медиане игрока 1 330. Прежние 400 000 стали недостижимы примерно
// в семь раз — сундук не открылся бы никогда.
//
// Затем фактическая неделя показала, что и база 100 000 остаётся недостижимой:
// первые 5 участников вместе набрали 20 500, и только 13 набрали больше 1 000.
// Владелец 2026-09-08 выбрал базу 50 000; шаг между лигами 10 000 не менялся.
const LEAGUE_CHEST_BASE_GOAL = 50_000;
const LEAGUE_CHEST_GOAL_STEP = 10_000;
