...
import { resolveImmutableSeasonRevision } from "./season_revision_resolver";
...
const makeDocs = () => {
  ...
  const record = { schemaVersion: "season-authoring-record.v1", draftId: "d1", seasonId: "s1", revision: 1, contentHash, revisionFingerprint: seasonRevisionFingerprint("d1", 1, contentHash), object: { objectPath: seasonRevisionObjectPath("d1", 1, contentHash), contentHash, objectGeneration: "11", byteSize: 128 }, provenance: {}, createdAt: "2026-07-17T00:00:00.000Z" };
...
describe("Storage-backed SeasonRevision resolver", () => {
  it("reads the pinned object and rejects a tampered object reader result", async () => {
    const docs = makeDocs();
    const documentReader = { read: async (path: string) => ({ exists: true, data: () => path.includes("lifecycle") ? docs.lifecycle : docs.record }) };
    const objectReader = { read: async () => ({ body: docs.body, contentHash: docs.record.contentHash, objectGeneration: "11", byteSize: 128 }) };
    await expect(resolveImmutableSeasonRevision({ revisionPath: "revision", lifecyclePath: "lifecycle", documentReader, objectReader })).resolves.toMatchObject({ body: docs.body, lifecycle: docs.lifecycle });
    await expect(resolveImmutableSeasonRevision({ revisionPath: "revision", lifecyclePath: "lifecycle", documentReader, objectReader: { read: async () => ({ body: { ...docs.body, seasonId: "tampered" }, contentHash: docs.record.contentHash, objectGeneration: "11", byteSize: 128 }) } })).rejects.toThrow("season_revision_object_invalid");
  });
  ...
  it.each(["missing-object", "hash-mismatch", "generation-mismatch", "byte-size-mismatch"])("propagates %s from the immutable reader", async (reason) => {
    ...
    await expect(resolveImmutableSeasonRevision({ revisionPath: "revision", lifecyclePath: "lifecycle", documentReader, objectReader })).rejects.toThrow(`immutable_object_${reason}`);
  });
  ...
  it.each([
    ...
    await expect(resolveImmutableSeasonRevision({ revisionPath: "revision", lifecyclePath: "lifecycle", documentReader, objectReader })).rejects.toThrow("season_revision_object_metadata_invalid");
  });
});
