...
describe('personal plan exercise: bare "Дальше" CTA on a correct answer', () => {
  it('renders the next button as a big 3D CTA (intro-style), not wrapped in a feedback plate', () => {
    // При верном ответе без тела/эха показываем ТОЛЬКО кнопку — большую объёмную,
    ...
    // Объёмная кнопка интро-стиля: DuoPressable с градиентом и нижней 3D-кромкой.
    expect(block).toContain('<DuoPressable');
    expect(block).toContain('gradientColors={[accent, accent');
    expect(block).toContain('edgeColor=');
