...
/** Ниже этого числа лайков набор «лучшим» не считается — иначе витрина врёт. */
const TOP_MIN_LIKES = 1;
...
/**
 * Отбор витрины: только наборы сообщества с реальными лайками, отсортированные
 * по лайкам ↓, затем по добавлениям ↓. Чистая функция — тестируется отдельно.
 */
export function pickTopCommunityPacks(
  ...
  return catalog
    .filter(
      (p) =>
        !!p?.isCommunityUgc
        && p.listingStatus !== 'local_only'
        && Math.max(0, Math.floor(p.likesCount ?? 0)) >= TOP_MIN_LIKES,
    )
    .sort((a, b) => {
