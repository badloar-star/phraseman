...
describe('live admin RevenueCat overview contract', () => {
  ...
  const server = fs.readFileSync(path.join(ROOT, 'functions/src/admin_revenuecat_overview.ts'), 'utf8');
  const exportsFile = fs.readFileSync(path.join(ROOT, 'functions/src/index.ts'), 'utf8');

  test('Paying now and On trial come from the protected RevenueCat API callable', () => {
    expect(html).toContain("httpsCallable(functionsUs, 'adminGetRevenueCatOverviewMetrics')");
    expect(html).toMatch(/an2Set\('an2-pay-total',[^;]*AN2_NUM\(_an2\.rcOverview\.activeSubscriptions\)/);
    expect(html).toMatch(/an2Set\('an2-pay-trial',[^;]*AN2_NUM\(_an2\.rcOverview\.activeTrials\)/);
  ...
  test('the secret stays server-side behind admin money.read permission', () => {
    ...
    expect(exportsFile).toContain("export { adminGetRevenueCatOverviewMetrics } from './admin_revenuecat_overview';");
  });
});
