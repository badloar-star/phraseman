...
describe('Gustav post-apply rollback guard contract V2 packet', () => {
  const source = fs.readFileSync(
    path.join(ROOT, 'scripts', 'gustav_post_apply_rollback_guard_contract_v2_packet.ts'),
    'utf8',
  );

  it('requires French server publish/upload/remote verification before post-apply guard can open', () => {
  ...
  it('keeps fixture probes for upload and remote verification regressions', () => {
