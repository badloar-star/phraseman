...
const KINDS = ['lesson_outline', 'lesson_phrases', 'lesson_vocabulary', 'lesson_irregular_verbs', 'lesson_prepositions', 'lesson_theory', 'challenge_topic', 'challenge_questions', 'challenge_question_replacement', 'flashcard_pack_idea', 'flashcard_items', 'flashcard_item_replacement'];
...
exports.adminReviewContentStage = (0, https_1.onCall)({ region: REGION, enforceAppCheck: callable_options_1.ENFORCE_APP_CHECK }, async (request) => {
    ...
    if (input.status === 'approved' && preliminary.kind === 'challenge_questions') {
        try {
            questionBatch = await (0, question_grounding_1.loadQuestionBatchForReview)(admin.storage().bucket(), { stageId: input.stageId, artifactId: String(preliminary.artifactId ?? ''), kind: preliminary.kind, state: String(preliminary.state ?? ''), objectPath: String(preliminary.objectPath ?? ''), contentHash: String(preliminary.contentHash ?? ''), objectGeneration: String(preliminary.objectGeneration ?? ''), groundingReceipt: preliminary.groundingReceipt }, { allowNeedsReview: true });
        }
        catch (error) {
    ...
    if (preliminary.kind === 'challenge_question_replacement') {
        try {
            questionReplacement = await (0, question_grounding_1.loadQuestionReplacementForReview)(admin.storage().bucket(), { artifactId: String(preliminary.artifactId ?? ''), kind: preliminary.kind, state: String(preliminary.state ?? ''), objectPath: String(preliminary.objectPath ?? ''), contentHash: String(preliminary.contentHash ?? ''), objectGeneration: String(preliminary.objectGeneration ?? ''), groundingReceipt: preliminary.groundingReceipt }, { allowNeedsReview: true });
        }
        catch (error) {
