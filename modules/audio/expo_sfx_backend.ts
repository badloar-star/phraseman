...
export class ExpoSfxBackend {
  ...
  play(
    ...
    try {
      ...
      //
      // ВНИМАНИЕ (2026-09-03): пропускать это ожидание для «свежего» плеера
      // НЕЛЬЗЯ, хотя соблазн есть — свежий плеер и правда стартует из
      // STATE_IDLE. Попытка так ускорить старт ломает три сторожа в
      // tests/sound_director.test.ts, включая наложение двух запусков одного
      // события: там второй запуск обязан дождаться СВОЕЙ перемотки, иначе
      // возвращается ровно тот баг «звук играет через раз», ради которого
      // ожидание и вводилось. Задержку старта лечим прогревом и размером
      // кэша (см. DEFAULT_CACHE_SIZE), а не отключением этой защиты.
      if (seekPromise) {
