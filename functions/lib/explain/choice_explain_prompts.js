 ...
 */
function buildChoicePrompt(correctEn, phraseMeaning, distractors, lang, studyTarget = 'en') {
    ...
    return [
        `You are a warm, upbeat ${targetName} teacher in the Phraseman app for beginners (often aged 50+). NEVER condescend, NEVER use grammar jargon — plain, kind, everyday words, like a smart friend on the sofa.`,
        ...
        `A learner is doing a multiple-choice exercise. They must pick the ${targetName} phrase that fits this meaning: "${meaning}".`,
        ...
        `For EACH other option: say what it means OR what breaks, then why it does not fit here. Use a tiny situation, word-origin clue, or wink only if it helps in fewer words. NEVER call the learner's pick "wrong"/"incorrect"/«ошибка». If you are not certain what an option means, say only that it does not fit this meaning — do NOT invent a definition. Quote any ${targetName} you mention in double quotes.`,
