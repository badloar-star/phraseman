#!/usr/bin/env python3
"""Restore readable Russian CTA/STA text materials for the French Cepicepi draft."""
...
REPORT = Path("exports/chains/cepicepi_french_ru_a1_20260606/sta_text_restore_report.json")
CYR_FONT = "C:/Windows/Fonts/arial.ttf"

STA_TEXTS: dict[tuple[int, int], str] = {
    (7, 0): "Мы создали приложение,\nчтобы помочь тебе\nдумать на французском.",
...
def qa(content: dict[str, Any]) -> dict[str, Any]:
    ...
    for track_idx, seg_idx in STA_TEXTS:
        ...
        material = texts.get(segment.get("material_id"))
        if not material:
            bad.append({"track": track_idx, "segment": seg_idx, "reason": "missing_material"})
            continue
        text = text_payload(material).get("text", "")
        if "?" in text or "Ð" in text or "Ñ" in text or "\ufffd" in text:
            bad.append({"track": track_idx, "segment": seg_idx, "reason": "bad_text_encoding", "text": text})
        for line in text.split("\n"):
            if len(line) > 34:
                bad.append({"track": track_idx, "segment": seg_idx, "reason": "line_too_long", "line": line, "text": text})
    try:
        if content["tracks"][2]["segments"][0].get("visible") is not False:
