...
describe('normalizePhrase + phraseHashFor — deterministic, dedup-friendly', () => {
  ...
  it('SAME phrase+lang in DIFFERENT study targets → DIFFERENT cache keys (DoD 5)', () => {
    ...
    const en = phraseHashFor('content', 'ru', 'en');
    const fr = phraseHashFor('content', 'ru', 'fr');
  ...
  it('the English key is unchanged: default target === explicit en === legacy 2-arg (DoD 4)', () => {
    ...
    expect(phraseHashFor('It sounds good', 'ru')).toBe(phraseHashFor('It sounds good', 'ru', 'en'));
  });
});
