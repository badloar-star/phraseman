...
export async function measureDailyJourneyRevealTarget(signal?: AbortSignal): Promise<DailyJourneyRevealTarget | null> {
  ...
  if (!measurer) {
    if (__DEV__) console.log(LOG, 'no measurer registered -> top-center fallback');
    return null;
  }
  ...
  const measured = measurer().catch((e: unknown) => {
    ...
    if (__DEV__) console.log(LOG, 'measurer threw:', e);
    return null;
  });
  ...
  if (!point || !Number.isFinite(point.x) || !Number.isFinite(point.y)) {
    if (__DEV__) console.log(LOG, 'measurer returned', point, '-> top-center fallback');
    return null;
  }
  return point;
}
