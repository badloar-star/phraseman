...
export type RemoteBoolKey =
  ...
  // Дефолт TRUE (kill-switch): фича едет с релизом, «Пульт» может экстренно
  // выключить её живьём. Раньше дефолт был FALSE — и раздел оставался пустым
  // у всех, включая владельца, пока кто-то не запишет флаг в базу руками.
  // Закрывать НОВУЮ фичу от самого себя — это не осторожность, а поломка.
  | 'gate_ai_text_tutor'
