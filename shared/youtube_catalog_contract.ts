...
export type YoutubeChannelConfig = {
  ...
  pinnedVideos?: Array<{ id: string; title?: string; url?: string }>;
  playlistOverrides: Record<string, { hidden?: boolean; order?: number; titleOverride?: string }>;
...
export type YoutubeCatalogConfig = {
  schemaVersion: 1;
  enabled: boolean;
...
export type YoutubeCatalogManifest = {
  schemaVersion: 1;
...
export type YoutubePlaylistSnapshot = {
  id: string;
  channelId: string;
  title: string;
  description: string;
...
export type YoutubePlaylistPage = { page: number; videoIds: string[] };
...
const VIDEO_STATES = new Set<YoutubeVideoState>(['video', 'upcoming', 'live', 'completed']);
...
function optionalText(value: unknown, code: string, max = 500): string | undefined {
...
function integer(value: unknown, code: string, min = 0): number {
  if (!Number.isSafeInteger(value) || Number(value) < min) fail(code);
  return Number(value);
}

function finiteNumber(value: unknown, code: string, min = 0): number {
  if (typeof value !== 'number' || !Number.isFinite(value) || value < min) fail(code);
  return value;
}
...
function optionalIsoDate(value: unknown, code: string): string | undefined {
  if (value == null || value === '') return undefined;
...
function youtubeUrl(value: unknown, code: string): string {
...
function stringArray(
  ...
  code: string,
  max: number,
  item: (value: string) => string = (entry) => entry,
): string[] {
  if (!Array.isArray(value)) fail(code);
  if (value.length > max) fail(`${code}_limit_exceeded`);
  const normalized = value.map((entry) => item(text(entry, code, 160)));
  if (new Set(normalized).size !== normalized.length) fail(`${code}_must_be_unique`);
  return normalized;
}

function languageTag(value: string): string {
  const normalized = value.replace(/_/g, '-').trim();
  if (!/^[A-Za-z]{2,3}(?:-[A-Za-z0-9]{2,8})*$/.test(normalized)) fail('language_tag_invalid');
  return normalized;
}

function internalId(value: unknown, code: string): string {
  const normalized = text(value, code, 80);
  if (!INTERNAL_ID_RE.test(normalized)) fail(code);
  return normalized;
}

function videoId(value: unknown, code: string): string {
  const normalized = text(value, code, 32);
  if (!VIDEO_ID_RE.test(normalized)) fail(code);
  return normalized;
}

function playlistId(value: unknown, code: string): string {
  const normalized = text(value, code, 100);
  if (!PLAYLIST_ID_RE.test(normalized)) fail(code);
  return normalized;
}

function boundedRecord(value: unknown, code: string, max: number): UnknownRecord {
  const normalized = record(value, code);
  if (Object.keys(normalized).length > max) fail(`${code}_limit_exceeded`);
  return normalized;
}
...
export function resolveYoutubeChannelId(
  ...
  if (manualChannelId && available.has(manualChannelId)) return manualChannelId;
  ...
  if (baseMatch && available.has(baseMatch)) return baseMatch;
  if (available.has(manifest.defaultChannelId)) return manifest.defaultChannelId;
...
function parseChannelConfig(value: unknown): YoutubeChannelConfig {
  ...
  let pinnedVideos: YoutubeChannelConfig['pinnedVideos'];
  if (source.pinnedVideos != null) {
    if (!Array.isArray(source.pinnedVideos)) fail('pinned_videos_invalid');
    if (source.pinnedVideos.length > YOUTUBE_CATALOG_LIMITS.videosPerChannel) fail('pinned_videos_limit_exceeded');
    ...
    if (new Set(pinnedVideos.map((entry) => entry.id)).size !== pinnedVideos.length) fail('pinned_video_id_must_be_unique');
  }

  const playlistSource = boundedRecord(
    source.playlistOverrides ?? {},
    'playlist_overrides_invalid',
    YOUTUBE_CATALOG_LIMITS.playlistsPerChannel,
  );
  ...
  const premiereSource = boundedRecord(
    source.premiereOverrides ?? {},
    'premiere_overrides_invalid',
    YOUTUBE_CATALOG_LIMITS.videosPerChannel,
  );
...
export function validateYoutubeCatalogConfig(value: unknown): YoutubeCatalogConfig {
  const source = record(value, 'youtube_catalog_config_invalid');
  if (source.schemaVersion !== 1) fail('youtube_catalog_schema_version_invalid');
  const enabled = booleanValue(source.enabled, 'youtube_catalog_enabled_invalid');
  if (!Array.isArray(source.channels)) fail('channels_invalid');
  if (source.channels.length > YOUTUBE_CATALOG_LIMITS.channels) fail('channels_limit_exceeded');
  const channels = source.channels.map(parseChannelConfig);
  const channelIds = channels.map((channel) => channel.id);
  if (new Set(channelIds).size !== channelIds.length) fail('channel_id_must_be_unique');
  const youtubeIds = channels.map((channel) => channel.youtubeChannelId);
  if (new Set(youtubeIds).size !== youtubeIds.length) fail('youtube_channel_id_must_be_unique');
  ...
  for (const [locale, rawChannelId] of Object.entries(defaultsSource)) {
    ...
    if (!channel) fail('locale_default_channel_missing');
    if (!channel.enabled) fail('locale_default_must_be_enabled');
...
function parseManifestChannel(value: unknown): YoutubeCatalogManifest['channels'][number] {
...
export function parseYoutubeCatalogManifest(value: unknown): YoutubeCatalogManifest {
  const source = record(value, 'youtube_catalog_manifest_invalid');
  if (source.schemaVersion !== 1) fail('manifest_schema_version_invalid');
  const activeVersion = text(source.activeVersion, 'manifest_active_version_invalid', 120);
  if (!VERSION_ID_RE.test(activeVersion)) fail('manifest_active_version_invalid');
  if (!Array.isArray(source.channels) || source.channels.length === 0) fail('manifest_channels_invalid');
  if (source.channels.length > YOUTUBE_CATALOG_LIMITS.channels) fail('manifest_channels_limit_exceeded');
  const channels = source.channels.map(parseManifestChannel);
  if (new Set(channels.map((channel) => channel.id)).size !== channels.length) fail('manifest_channel_id_must_be_unique');
  ...
  if (!available.has(defaultChannelId)) fail('manifest_default_channel_missing');
  ...
  for (const [locale, rawChannelId] of Object.entries(defaultsSource)) {
    ...
    if (!available.has(target)) fail('manifest_locale_default_channel_missing');
...
export function parseYoutubeChannelSnapshot(value: unknown): YoutubeChannelSnapshot {
  ...
  if (!CHANNEL_ID_RE.test(youtubeChannelId)) fail('snapshot_youtube_channel_id_invalid');
  ...
  const recentVideoIds = stringArray(
    source.recentVideoIds,
    'snapshot_recent_video_ids',
    YOUTUBE_CATALOG_LIMITS.videosPerChannel,
    (entry) => videoId(entry, 'snapshot_video_id_invalid'),
  );
  if (activeEventVideoId && !recentVideoIds.includes(activeEventVideoId)) fail('snapshot_active_event_missing');
  return {
    ...
    recentVideoIds,
    playlistIds: stringArray(
      source.playlistIds,
      'snapshot_playlist_ids',
      YOUTUBE_CATALOG_LIMITS.playlistsPerChannel,
      (entry) => playlistId(entry, 'snapshot_playlist_id_invalid'),
    ),
  };
}

export function parseYoutubeVideoSnapshot(value: unknown): YoutubeVideoSnapshot {
  const source = record(value, 'youtube_video_snapshot_invalid');
  const state = text(source.state, 'video_state_invalid', 16) as YoutubeVideoState;
  if (!VIDEO_STATES.has(state)) fail('video_state_invalid');
  ...
  if (state === 'upcoming' && !scheduledStartTime) fail('upcoming_video_schedule_required');
  if (state === 'live' && !actualStartTime) fail('live_video_actual_start_required');
  if (state === 'completed' && !actualEndTime) fail('completed_video_actual_end_required');
  ...
  return {
    ...
    playlistIds: stringArray(
      ...
      'video_playlist_ids',
      YOUTUBE_CATALOG_LIMITS.playlistsPerChannel,
      (entry) => playlistId(entry, 'video_playlist_id_invalid'),
    ),
  };
}

export function parseYoutubePlaylistSnapshot(value: unknown): YoutubePlaylistSnapshot {
  ...
  if (itemCount > YOUTUBE_CATALOG_LIMITS.playlistItems) fail('playlist_item_count_limit_exceeded');
  const pageCount = integer(source.pageCount, 'playlist_page_count_invalid');
  const maximumPages = Math.ceil(YOUTUBE_CATALOG_LIMITS.playlistItems / YOUTUBE_CATALOG_LIMITS.playlistPageSize);
  if (pageCount > maximumPages || pageCount !== Math.ceil(itemCount / YOUTUBE_CATALOG_LIMITS.playlistPageSize)) {
...
export function parseYoutubePlaylistPage(value: unknown): YoutubePlaylistPage {
  const source = record(value, 'youtube_playlist_page_invalid');
  if (!Array.isArray(source.videoIds)) fail('playlist_page_video_ids_invalid');
  if (source.videoIds.length > YOUTUBE_CATALOG_LIMITS.playlistPageSize) fail('playlist_page_limit_exceeded');
  return {
    page: integer(source.page, 'playlist_page_number_invalid'),
    videoIds: stringArray(
      source.videoIds,
      'playlist_page_video_ids',
      YOUTUBE_CATALOG_LIMITS.playlistPageSize,
      (entry) => videoId(entry, 'playlist_page_video_id_invalid'),
    ),
  };
}
