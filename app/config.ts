 ...
 *
 * РЕШЕНИЕ 2026-06-21: фича пока НЕ уходит в релиз — остаётся СТРОГО в dev. UI ещё сырой
 * (превью уровней визуально мало отличаются), поэтому в стор-сборке вся фича карточки
 * скрыта целиком: бейдж у имени в списках, вход с экрана аватара и кнопка в профиле —
 * ничего этого в проде нет. В dev/QA/TestFlight-dev работает как раньше.
 *
 * Гейт по тому же надёжному паттерну, что ENABLE_DEV_TOOLS ниже: голый __DEV__/DEV_MODE
 * в этом коде может быть true даже в стор-бандле, поэтому ОБЯЗАТЕЛЬНО `&& !IS_STORE_RELEASE`
 * (EXPO_PUBLIC_STORE_RELEASE=1) — это и есть жёсткая отсечка от релиза.
 *
 * Чтобы вернуть фичу в релиз позже — поставить здесь `true` (как было).
 */
export const ENABLE_PROFILE_CARD =
  ((typeof __DEV__ !== 'undefined' && __DEV__) || DEV_MODE || TESTFLIGHT_DEV_TOOLS) && !IS_STORE_RELEASE;
