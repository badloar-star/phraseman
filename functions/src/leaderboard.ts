...
export const nameGenerateAndReserve = onCall(HOT_CALLABLE_OPTIONS, async (request) => {
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (existingName && existingNameLower) {
      ...
      if (existingSnap.exists && existingSnap.data()?.uid === stableUid && !nameIndexDocIsHidden(existingSnap.data())) {
    ...
    tx.set(chosen.ref, {
      uid: stableUid, authUid, name: chosen.name, nameLower: chosen.nameLower,
      identityHidden: admin.firestore.FieldValue.delete(), updatedAt: now,
    }, { merge: true });
    tx.set(db.collection('leaderboard').doc(stableUid), {
      name: chosen.name, nameLower: chosen.nameLower, firebaseAuthUid: authUid, updatedAt: now,
    }, { merge: true });
    tx.set(userRef, {
      progress: {
    ...
    tx.set(db.collection('public_profiles').doc(stableUid), {
      uid: stableUid, name: chosen.name, nameLower: chosen.nameLower, updatedAt: now,
    }, { merge: true });
  });
