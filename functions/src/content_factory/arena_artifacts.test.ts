import { arenaQuestionSemanticKey, validateArenaQuestionBatchArtifact, validateArenaTopicArtifact } from './arena_artifacts';
...
const item = (index: number) => ({
  ...
  options: [`Correct ${index}`, `Wrong A ${index}`, `Wrong B ${index}`, `Wrong C ${index}`] as [string, string, string, string],
  correctIndex: index % 4, correct: '', rule: `Правило ${index}.`, skillTag: index % 2 ? 'directions' : 'services',
  difficulty: index <= 3 ? 'easy' : index <= 7 ? 'medium' : 'hard', expectedAnswerTimeMs: 6000 + index * 100, sourceReferences: [] as string[],
});
...
describe('Arena Studio artifacts', () => {
  it('validates a topic with explicit speed, limits, locales and fixed runtime policy', () => {
    expect(validateArenaTopicArtifact(topic, { cefr: 'A2', studyTarget: 'en', sourceLocale: 'ru' })).toEqual([]);
    expect(validateArenaTopicArtifact({ ...topic, result: { ...topic.result, runtimePolicy: { ...topic.result.runtimePolicy, questionTimeoutMs: 10000 } } }, { cefr: 'A2', studyTarget: 'en', sourceLocale: 'ru' })).toContain('arena_topic_runtime_timeout_must_be_40000');
    expect(validateArenaTopicArtifact({ ...topic, result: { ...topic.result, level: 'C1' } }, { cefr: 'C1', studyTarget: 'en', sourceLocale: 'ru' })).toContain('arena_topic_level_not_supported');
  });

  it('accepts an exact-ten fair batch with string/index answer identity', () => {
    expect(validateArenaQuestionBatchArtifact({ stage: 'arena_questions', items: items() }, { count: 10, grounding: { topic: topic.result, previousQuestionKeys: [] } })).toEqual([]);
  });
  ...
  it('rejects long text, answer mismatch, duplicate options, speed violations and cross-batch duplicates', () => {
    ...
    const errors = validateArenaQuestionBatchArtifact({ stage: 'arena_questions', items: invalid }, { count: 10, grounding: { topic: topic.result, previousQuestionKeys: [arenaQuestionSemanticKey(invalid[2])] } });
  ...
  it('fails closed when correct answer positions are unbalanced or repeat more than twice', () => {
    ...
    expect(validateArenaQuestionBatchArtifact({ stage: 'arena_questions', items: alignedUnbalanced }, { count: 10, grounding: { topic: topic.result, previousQuestionKeys: [] } })).toContain('arena_correct_position_distribution_invalid');

    const run = items();
    for (const index of [0, 1, 2]) run[index] = { ...run[index], correctIndex: 0, correct: run[index].options[0] };
    expect(validateArenaQuestionBatchArtifact({ stage: 'arena_questions', items: run }, { count: 10, grounding: { topic: topic.result, previousQuestionKeys: [] } })).toContain('arena_correct_position_run_too_long');
  });
});
