...
import {
  ...
  GAVAN_WEEK1_DAY6_MATERIAL_CANDIDATE_PATH,
  validateGavanWeek1Day6MaterialCandidate,
  writeGavanWeek1Day6MaterialCandidate,
} from '../tools/personal_plan_gavan_week1_day6_material_candidate';

const GENERATED_AT = '2026-06-03T23:00:00.000Z';
...
function day5Export() {
  const sourceSeed = seed();
  const sourceDay4Export = day4Export();
  const day5Candidate = buildGavanWeek1Day5MaterialCandidate(sourceSeed, sourceDay4Export, {
    generatedAt: GENERATED_AT,
  });

  return buildGavanWeek1Day5MaterialExportPacket(day5Candidate, sourceSeed, sourceDay4Export, {
    generatedAt: GENERATED_AT,
  });
}

function candidate() {
  return buildGavanWeek1Day6MaterialCandidate(seed(), day5Export(), {
    generatedAt: GENERATED_AT,
  });
}

describe('Gavan week 1 day 6 concrete material candidate', () => {
  beforeEach(() => {
    if (existsSync(GAVAN_WEEK1_DAY6_MATERIAL_CANDIDATE_PATH)) {
      rmSync(GAVAN_WEEK1_DAY6_MATERIAL_CANDIDATE_PATH, { force: true });
    }
  });

  afterAll(() => {
    writeGavanWeek1Day6MaterialCandidate(seed(), day5Export(), {
      generatedAt: GENERATED_AT,
      targetPath: GAVAN_WEEK1_DAY6_MATERIAL_CANDIDATE_PATH,
    });
  });

  it('builds a non-live day 6 material candidate from seed and day 5 export', () => {
  ...
  it('uses only the reset day 6 seed phrases', () => {
    const material = candidate();
  ...
  it('uses a varied rhythm instead of copying the day 5 exercise set', () => {
    const material = candidate();
    ...
    expect(material.exerciseBlocks.map((block) => block.exerciseType)).not.toEqual([
      'lesson_bridge',
      'phrase_build',
      'listening_choice',
      'natural_choice',
  ...
  it('builds quick replies missing-word items and repair tiles with safe counts', () => {
    const material = candidate();
    ...
    expect(material.missingWordItems).toHaveLength(4);
    material.missingWordItems.forEach((item) => {
      expect(item.options).toContain(item.correctToken);
      expect(new Set(item.options).size).toBe(item.options.length);
      expect(item.options).toHaveLength(4);
      expect(item.sentenceWithBlank).toContain('___');
    });
  ...
  it('keeps pronunciation as a placeholder without fake scoring', () => {
    const material = candidate();

    expect(material.pronunciationPlaceholders).toHaveLength(4);
    expect(material.pronunciationPlaceholders.every((item) => item.scoringStatus === 'not_built')).toBe(true);
    expect(material.pronunciationPlaceholders.every((item) => item.finalScoringReady === false)).toBe(true);
  });
  ...
  it('passes its own quality gate and catches corrupted material candidates', () => {
    ...
    expect(validateGavanWeek1Day6MaterialCandidate(material, seed(), day5Export())).toEqual({
      valid: true,
      issues: [],
    });
    ...
    const corrupted = {
      ...
      quickReplyItems: [
        {
          ...material.quickReplyItems[0],
          options: [material.quickReplyItems[0].correctEnglish],
        },
      ],
      missingWordItems: [
        {
          ...material.missingWordItems[0],
          sentenceWithBlank: material.missingWordItems[0].targetEnglish,
          options: [material.missingWordItems[0].correctToken],
        },
      ],
      repairItems: [
        {
          ...
          wordTiles: ['That'],
          distractorTiles: ['That'],
        },
      ],
      pronunciationPlaceholders: [
        {
          ...material.pronunciationPlaceholders[0],
          scoringStatus: 'ready',
      ...
      materialPhrases: [
        {
          ...
          english: 'My phone number works for me.',
    ...
    const result = validateGavanWeek1Day6MaterialCandidate(corrupted as any, seed(), day5Export());
    expect(result.valid).toBe(false);
    expect(result.issues).toEqual(expect.arrayContaining([
      expect.objectContaining({ code: 'wrong_day_id' }),
      expect.objectContaining({ code: 'quick_reply_option_count_mismatch' }),
      expect.objectContaining({ code: 'missing_word_option_count_mismatch' }),
      expect.objectContaining({ code: 'missing_word_blank_missing' }),
      expect.objectContaining({ code: 'repair_tile_count_mismatch' }),
      expect.objectContaining({ code: 'unsafe_distractor_tile' }),
      expect.objectContaining({ code: 'fake_pronunciation_placeholder_claim' }),
  ...
  it('writes deterministic material JSON only under temp or report roots', () => {
    const result = writeGavanWeek1Day6MaterialCandidate(seed(), day5Export(), {
      generatedAt: GENERATED_AT,
      targetPath: GAVAN_WEEK1_DAY6_MATERIAL_CANDIDATE_PATH,
    });
    ...
    expect(existsSync(GAVAN_WEEK1_DAY6_MATERIAL_CANDIDATE_PATH)).toBe(true);

    const parsed = JSON.parse(readFileSync(GAVAN_WEEK1_DAY6_MATERIAL_CANDIDATE_PATH, 'utf8'));
    expect(parsed.kind).toBe('gavan_week1_day6_material_candidate');
    expect(parsed.dayId).toBe('gavan-week1-day6');
  });

  it('rejects app tools tests and root config write targets', () => {
    const appResult = writeGavanWeek1Day6MaterialCandidate(seed(), day5Export(), {
      generatedAt: GENERATED_AT,
      targetPath: path.join(process.cwd(), 'app', 'gavan-week1-day6-material.json'),
    });
    const toolsResult = writeGavanWeek1Day6MaterialCandidate(seed(), day5Export(), {
      generatedAt: GENERATED_AT,
      targetPath: path.join(process.cwd(), 'tools', 'gavan-week1-day6-material.json'),
    });
    const testsResult = writeGavanWeek1Day6MaterialCandidate(seed(), day5Export(), {
      generatedAt: GENERATED_AT,
      targetPath: path.join(process.cwd(), 'tests', 'gavan-week1-day6-material.json'),
    });
  ...
  it('does not import or mutate live app routes catalog quiz UI storage audio scoring or navigation', () => {
    const source = readFileSync(
      path.join(process.cwd(), 'tools', 'personal_plan_gavan_week1_day6_material_candidate.ts'),
      'utf8',
    );
