...
exports.tournamentWeeklyBankInfo = exports.adminGetTournamentEconomy = exports.adminSetTournamentEconomy = exports.adminPayoutTournamentWeeklyBank = exports.tournamentWeeklyBankCron = void 0;
exports.previousWeekId = previousWeekId;
exports.payoutWeeklyBank = payoutWeeklyBank;
...
function previousWeekId(nowMs) {
    return (0, tournament_core_1.tournamentWeekId)(nowMs - 7 * 24 * 60 * 60 * 1000);
}
/**
 ...
 */
async function payoutWeeklyBank(db, weekId, nowMs) {
    ...
    const standingsSnap = await db.collection(tournament_core_1.TOURNAMENT_SEASONS_COLLECTION).doc(weekId)
    ...
    const economy = (0, tournament_economy_1.normalizeTournamentEconomy)(economySnap.data());
    const { payouts, carryOver } = (0, tournament_economy_1.weeklyBankPayouts)(bankGems, standings, economy);
    const nextWeek = (0, tournament_core_1.tournamentWeekId)(nowMs);
    const nextBankRef = nextWeek !== weekId
        ? db.collection(tournament_core_1.TOURNAMENT_BANK_COLLECTION).doc(nextWeek)
        : null;
    ...
    const applied = await db.runTransaction(async (tx) => {
        ...
        for (const payout of payouts) {
            ...
            tx.set((0, user_notifications_1.userNotificationRef)(db, payout.uid, `tournament_weekly_${weekId}`), (0, user_notifications_1.buildUserNotification)({
 ...
 */
exports.tournamentWeeklyBankCron = (0, scheduler_1.onSchedule)({ schedule: '10 0 * * 1', timeZone: 'UTC', region: REGION }, async () => {
...
exports.adminPayoutTournamentWeeklyBank = (0, https_1.onCall)({ region: REGION, enforceAppCheck: callable_options_1.ENFORCE_APP_CHECK, timeoutSeconds: 120 }, async (request) => {
 ...
 */
exports.adminSetTournamentEconomy = (0, https_1.onCall)({ region: REGION, enforceAppCheck: callable_options_1.ENFORCE_APP_CHECK }, async (request) => {
    if (!request.auth?.token?.admin)
        throw new https_1.HttpsError('permission-denied', 'Admin only');
    const economy = (0, tournament_economy_1.normalizeTournamentEconomy)(request.data);
    const db = admin.firestore();
    const nowMs = Date.now();
...
exports.adminGetTournamentEconomy = (0, https_1.onCall)({ region: REGION, enforceAppCheck: callable_options_1.ENFORCE_APP_CHECK }, async (request) => {
    if (!request.auth?.token?.admin)
        throw new https_1.HttpsError('permission-denied', 'Admin only');
    const snap = await admin.firestore()
        .collection(TOURNAMENT_SCHEDULE_COLLECTION).doc('economy').get();
    return { ok: true, economy: (0, tournament_economy_1.normalizeTournamentEconomy)(snap.data()) };
});
 ...
 */
exports.tournamentWeeklyBankInfo = (0, https_1.onCall)({ region: REGION, enforceAppCheck: callable_options_1.ENFORCE_APP_CHECK }, async (request) => {
    if (!request.auth?.uid)
        throw new https_1.HttpsError('unauthenticated', 'auth_required');
    const db = admin.firestore();
    const nowMs = Date.now();
    const currentWeek = (0, tournament_core_1.tournamentWeekId)(nowMs);
    ...
    const [currentSnap, lastSnap, economySnap] = await db.getAll(db.collection(tournament_core_1.TOURNAMENT_BANK_COLLECTION).doc(currentWeek), db.collection(tournament_core_1.TOURNAMENT_BANK_COLLECTION).doc(lastWeek), db.collection(TOURNAMENT_SCHEDULE_COLLECTION).doc('economy'));
    const economy = (0, tournament_economy_1.normalizeTournamentEconomy)(economySnap.data());
    ...
    return {
        ...
        lobbyOpenMs: tournament_core_1.TOURNAMENT_LOBBY_OPEN_MS,
        ...
        entryWindowMs: tournament_core_1.TOURNAMENT_ENTRY_WINDOW_MS,
...
//# sourceMappingURL=tournament_weekly_payout.js.map