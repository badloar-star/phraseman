...
describe('approved dependency catalog', () => {
    test('derives kinds and approved state from the consumer capability', () => {
        expect((0, dependency_catalog_1.parseDependencyCatalogRequest)({ requestId: 'req-1', studyTarget: 'en', sourceLocale: 'ru', consumerKind: 'quiz_questions', limit: 25 })).toMatchObject({
    ...
    test('rejects arbitrary state/kind and tampered cursor', () => {
        expect(() => (0, dependency_catalog_1.parseDependencyCatalogRequest)({ requestId: 'req-1', studyTarget: 'en', sourceLocale: 'ru', consumerKind: 'quiz_questions', state: 'rejected' })).toThrow('dependency_catalog_invalid');
        expect(() => (0, dependency_catalog_1.parseDependencyCatalogRequest)({ requestId: 'req-1', studyTarget: 'en', sourceLocale: 'ru', consumerKind: 'quiz_questions', cursor: 'bad cursor!' })).toThrow('dependency_catalog_invalid');
    });
