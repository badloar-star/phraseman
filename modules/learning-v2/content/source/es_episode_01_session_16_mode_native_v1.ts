...
// + СТАРТ ES §0/§10, Rules §4.1, Глава 2 "Ты: вопрос" финал): испанская
// checkpoint-сессия 16 переписывается в mode-native формат, зеркаля КОД (не
// данные) es_episode_01_session_08_mode_native_v1.ts — та же checkpoint-
// механика. Как и voice-сессия 15, checkpoint НЕ вводит новых слов и не
// имеет newVocabulary — переиспользует РОВНО тот же набор 15 фраз, что и
// сессия 15 (ES_EPISODE_01_SESSION_16_CHECKPOINT_PHRASES = алиас на
// ES_EPISODE_01_SESSION_15_VOICE_PHRASES). Разница между voice и checkpoint —
...
// (владелец, 2026-08-27, тот же вывод, что и в сессии 8): checkpointSteps()
...
const phrases = ES_EPISODE_01_SESSION_16_CHECKPOINT_PHRASES;
if (phrases.length !== 15) {
  throw new Error('es_session_16_mode_native_phrase_count_invalid');
}

const L = (source: LocalizedSource): LearningV2Localized<string> => expandLocalized(source);

// зачем эта функция (владелец, 2026-08-27, тот же класс бага сессий 5/6/8/9-15):
...
// зачем 'en' вместо 'es' здесь (как в испанских сессиях 1-15): в испанском
...
function localizedPhraseField(
  ...
  if (!phrase?.localizedDetails) throw new Error(`es_session_16_phrase_details_missing:${phraseIndex}`);
  return Object.fromEntries(LEARNING_V2_INTERFACE_LOCALES.map((locale) => {
    ...
    if (!detail) throw new Error(`es_session_16_phrase_locale_missing:${phraseIndex}:${locale}`);
...
function localizedPhraseDistractorFeedback(
  ...
  return Object.fromEntries(LEARNING_V2_INTERFACE_LOCALES.map((locale) => {
    ...
    if (!reason) throw new Error(`es_session_16_phrase_distractor_missing:${phraseIndex}:${locale}:${value}`);
...
function listenBuildPhrase(phraseIndex: number): LearningV2ModeNativePayloadV1 {
  ...
  return Object.freeze({
    ...
    referenceAudio: { audioTargetId: `es-e01-s16-phrase-${phraseIndex + 1}`, transcript: phrase.english },
    slowReferenceAudio: { audioTargetId: `es-e01-s16-phrase-${phraseIndex + 1}-slow`, transcript: phrase.english },
...
// (владелец, 2026-08-27, checkpoint-специфика, как и сессия 8): checkpoint
...
// (владелец, 2026-08-27, класс бага сессии 12): защита от коллизии, если
// два дистрактора одной позиции когда-либо получат одинаковый reasonCode.
...
// 2026-08-27, как и сессия 8): checkpoint — единственная сессия главы, где
// цель — проверить владение ЦЕЛОЙ главой сразу, без разбивки. pairGrid
// сопоставляет target испанский текст фразы с её точным переводом; ни одно
// новое слово не вводится — все 15 фраз уже честно изучены в сессиях 9-14 и
// повторены в сессии 15.
const SPEED_IDS = phrases.map((_phrase, index) => `es-e01-s16-pair-${index + 1}`);
...
// es_episode_01_session_08_mode_native_v1.ts по составу families и
...
// independent_check).
export const ES_EPISODE_01_SESSION_16_MODE_NATIVE_PRACTICE_V1 = Object.freeze<readonly SessionModeNativePracticeSourceV1[]>([
...
if (ES_EPISODE_01_SESSION_16_MODE_NATIVE_PRACTICE_V1.length !== 12) {
  throw new Error('es_session_16_mode_native_practice_count_invalid');
}
