...
describe('content factory unit runner request', () => {
    it('accepts one bounded canonical generation unit', () => {
        expect((0, content_factory_worker_1.parseGenerationUnitRequest)({ jobId: 'job-1', surface: 'lesson', lessonId: 3 })).toEqual({ jobId: 'job-1', surface: 'lesson', lessonId: 3 });
    });
    it('rejects unsupported or unsafe units', () => {
        expect(() => (0, content_factory_worker_1.parseGenerationUnitRequest)({ jobId: '../escape', surface: 'lesson', lessonId: 1 })).toThrow(https_1.HttpsError);
        ...
        expect(() => (0, content_factory_worker_1.parseGenerationUnitRequest)({ jobId: 'job', surface: 'quiz', lessonId: 1 })).toThrow(https_1.HttpsError);
        expect(() => (0, content_factory_worker_1.parseGenerationUnitRequest)({ jobId: 'job', surface: 'lesson', lessonId: 101 })).toThrow(https_1.HttpsError);
    });
