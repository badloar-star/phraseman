...
exports.SHARD_EARN_DAILY_TOTAL_MAX = exports.SHARD_EARN_DAILY_COUNTERS_COLLECTION = void 0;
exports.resolveShardEarnPolicy = resolveShardEarnPolicy;
exports.utcShardEarnDayKey = utcShardEarnDayKey;
exports.emptyShardEarnDailyCounter = emptyShardEarnDailyCounter;
exports.normalizeShardEarnDailyCounter = normalizeShardEarnDailyCounter;
exports.applyShardEarnBudget = applyShardEarnBudget;
exports.SHARD_EARN_DAILY_COUNTERS_COLLECTION = 'shard_earn_daily_counters';
// ═══════════════════════════════════════════════════════════════════════════
 ...
 */
exports.SHARD_EARN_DAILY_TOTAL_MAX = 1000;
/**
 ...
 */
const FIXED_REASON_CATALOG = Object.freeze({
...
function resolveShardEarnPolicy(reason, requestedAmount) {
    ...
    if (!Object.prototype.hasOwnProperty.call(FIXED_REASON_CATALOG, reason))
        return null;
    const entry = FIXED_REASON_CATALOG[reason];
    if (!entry || !entry.amounts.includes(requestedAmount))
        return null;
    return {
        reasonKey: reason,
...
function utcShardEarnDayKey(nowMs) {
    const date = new Date(nowMs);
    if (!Number.isFinite(date.getTime()))
        throw new Error('invalid_shard_earn_timestamp');
    return date.toISOString().slice(0, 10);
}
function readNonNegativeInteger(value) {
    return Number.isSafeInteger(value) && Number(value) >= 0 ? Number(value) : 0;
}
function emptyShardEarnDailyCounter(dayKey) {
    return { dayKey, totalEarned: 0, bySource: {} };
}
function normalizeShardEarnDailyCounter(dayKey, value) {
    ...
    const rawBySource = row.bySource && typeof row.bySource === 'object'
        ...
        ? row.bySource
        : {};
    const bySource = Object.fromEntries(Object.entries(rawBySource)
...
function applyShardEarnBudget(current, policy, receiptAlreadyExists) {
    ...
    if (nextSourceEarned > policy.perSourceDailyMax
        || nextTotalEarned > exports.SHARD_EARN_DAILY_TOTAL_MAX) {
...
//# sourceMappingURL=shard_reward_catalog.js.map