...
import { normalizePortraitOverlay } from './normalize-portrait-overlay.mjs';
...
function parseArgs(args) {
  ...
  if (!inputPath || !Number.isInteger(assetIndex) || assetIndex < 63 || assetIndex > 165
    ...
    throw new Error('Usage: node scripts/avatar-100/finalize-portrait-overlay.mjs --input <image> --index <63-165> --variant <black|white> --out-dir <directory>');
  }
  return {
...
export async function finalizePortraitOverlay({ inputPath, assetIndex, variant, outputDir }) {
...
async function main() {
  const result = await finalizePortraitOverlay(parseArgs(process.argv.slice(2)));
