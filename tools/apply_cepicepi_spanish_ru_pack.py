...
ROLE_CONFIG = {
    "es1": {
        "voice": "coral",
        "instructions": "Speak neutral Latin American Spanish beautifully and clearly for A1 learners. Warm, natural teacher voice. Say only the phrase.",
    },
    "ru": {
        "voice": "shimmer",
    ...
    "es2": {
        "voice": "nova",
        "instructions": "Speak neutral Latin American Spanish with a second pleasant voice, calm and clear for A1 learners. Say only the phrase.",
    },
}
...
def build_rows() -> list[dict[str, Any]]:
    ...
    for row in rows:
        row["pronunciation"] = spanish_hint(row["spanish"])
    (PACK / "spanish_chains_100_with_pronunciation.json").write_text(json.dumps(rows, ensure_ascii=False, indent=2), encoding="utf-8")
    return rows
    ...
    for idx, row in enumerate(rows):
        jobs.extend(
            [
                ...
                {"part": 1, "role": "es1", "text": row["spanish"], "track": 18, "seg": idx},
                {"part": 1, "role": "es2", "text": row["spanish"], "track": 19, "seg": idx},
                {"part": 2, "role": "ru", "text": row["russian"], "track": 17, "seg": 100 + idx},
                {"part": 2, "role": "es1", "text": row["spanish"], "track": 18, "seg": 100 + idx},
                {"part": 2, "role": "es2", "text": row["spanish"], "track": 19, "seg": 100 + idx},
                {"part": 3, "role": "es1", "text": row["spanish"], "track": 18, "seg": 200 + idx},
                {"part": 3, "role": "es2", "text": row["spanish"], "track": 19, "seg": 200 + idx},
            ]
        )
    generated_raw = 0
    fitted = []
...
def apply() -> dict[str, Any]:
    ...
    for idx, row in enumerate(rows):
        ...
        set_text(texts[content["tracks"][5]["segments"][idx]["material_id"]], wrap_text(row["spanish"], 40))
        set_text(texts[content["tracks"][3]["segments"][100 + idx]["material_id"]], wrap_text(row["spanish"], 40))
        ...
        set_text(texts[content["tracks"][5]["segments"][200 + idx]["material_id"]], wrap_text(row["spanish"], 40))
        text_changes += 8
    ...
    for idx, _row in enumerate(rows):
        for role, track, seg_idx in [
            ...
            local = localize(project, Path(item["path"]), "cepicepi_spanish_openai_audio")
    ...
    for part in range(3):
        for idx, row in enumerate(rows):
            ...
            local = localize(project, source, "cepicepi_spanish_phrase_backgrounds")
            mat["path"] = str(local)
            mat["media_path"] = str(local)
            mat["duration"] = int((seg.get("target_timerange") or {}).get("duration", 0))
    ...
    # Keep the known bad long overlay hidden.
    try:
...
def qa() -> dict[str, Any]:
    ...
    bg_next = [m for m in videos.values() if "cepicepi_spanish_phrase_backgrounds" in str(m.get("path") or m.get("media_path") or "")]
    audio_next = [m for m in audios.values() if "cepicepi_spanish_openai_audio" in str(m.get("path") or m.get("media_path") or "")]
    if len(bg_next) < 300:
        errors.append(f"Spanish background material count {len(bg_next)} < 300")
    if len(audio_next) < 800:
        errors.append(f"Spanish audio material count {len(audio_next)} < 800")
    report = {
        ...
        "media": {
            ...
            "spanish_background_materials": len(bg_next),
            "spanish_audio_materials": len(audio_next),
        },
        "text": {
