...
export function scorePromptRegressionCase(item: PromptRegressionCase) {
  ...
  if (texts.some((pair) => pair.source.trim().toLowerCase() === 'у вас есть это в меньшем размере?')) failures.add('source_calque');
  if (texts.some((pair) => /\b(i am agree|he go|she go)\b/i.test(pair.target))) failures.add('grammar_invalid');
  if (texts.some((pair) => /\b(i ready am|me help need|you map show)\b/i.test(pair.target))) failures.add('word_salad');
  if (questions.some((question) => question.difficulty === 'hard' && question.prompt.trim().split(/\s+/).length < 4)) failures.add('hard_too_weak');
  if (questions.some((question) => question.validAnswerIndexes.length !== 1)) failures.add('ambiguous_answer');
  if (questions.some((question) => !Number.isInteger(question.correctIndex) || question.options[question.correctIndex] !== question.correct || question.validAnswerIndexes[0] !== question.correctIndex)) failures.add('correct_index_mismatch');
...
export interface PromptRegressionCandidate { readonly kind: string; readonly fromVersion: string; readonly toVersion: string; readonly definitionHash: string }
export interface PromptCandidateCheck { readonly id: string; readonly passed: boolean; readonly failures: readonly string[] }
export function runPromptRegression(cases: readonly PromptRegressionCase[] = PROMPT_REGRESSION_CASES, candidate: PromptRegressionCandidate | null = null, candidateChecks: readonly PromptCandidateCheck[] = []) {
  const manifest = promptRegressionManifest(cases); const results = cases.map(scorePromptRegressionCase); let goldenAccepted = 0; let goldenRejected = 0; let redTeamRejected = 0; let redTeamAccepted = 0; let regressions = 0;
  results.forEach((result) => { if (result.expectation === 'golden_valid') result.accepted ? goldenAccepted++ : goldenRejected++; else result.accepted ? redTeamAccepted++ : redTeamRejected++; if (JSON.stringify(result.actualFailures) !== JSON.stringify(result.expectedFailures)) regressions++; });
  ...
  const summary = Object.freeze({ total: cases.length, goldenAccepted, goldenRejected, redTeamRejected, redTeamAccepted, regressions, candidateFailures }); const passed = goldenRejected === 0 && redTeamAccepted === 0 && regressions === 0 && candidateFailures === 0 && frozenCandidateChecks.every((check) => check.passed);
  const policyHash = regressionHash(QUALITY_REGRESSION_POLICY); const reportBody = { schemaVersion: 'prompt-regression-report-v2', manifestHash: manifest.hash, policyVersion: QUALITY_REGRESSION_POLICY.version, policyHash, candidate: candidate ? Object.freeze({ ...candidate }) : null, candidateChecks: frozenCandidateChecks, results, summary, passed }; return Object.freeze({ ...reportBody, reportHash: regressionHash(reportBody) });
}
