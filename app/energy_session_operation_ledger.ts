...
function normalizedProjection(projection: EnergySessionProjection, nowMs: number): EnergySessionProjection {
  ...
  if (projection.bonusEnergy > 0 && projection.bonusExpiresAt <= nowMs) {
    return { ...projection, bonusEnergy: 0, bonusExpiresAt: 0 };
  }
  return projection;
}
...
export function planEnergySessionDebit(
  ...
  const lastRecoveryTime = base > 0 && before.baseEnergy >= before.maxEnergy
    ? nowMs
  ...
  return {
    ...
    after: {
      ...
      bonusExpiresAt: before.bonusEnergy - bonus > 0 ? before.bonusExpiresAt : 0,
...
export function planEnergySessionRefund(
  ...
  const refundableBonus = original.split.bonus > 0 && original.projectionBefore.bonusExpiresAt > nowMs
    ? original.split.bonus
    : 0;
  ...
  return {
    ...
    bonusExpiresAt: refundableBonus > 0
      ? Math.max(current.bonusExpiresAt, original.projectionBefore.bonusExpiresAt)
      : current.bonusExpiresAt,
...
async function publishProjection(owner: string, token: AccountGenerationToken, projection: EnergySessionProjection): Promise<void> {
  ...
  if (projection.bonusEnergy > 0 && projection.bonusExpiresAt > Date.now()) {
    await AsyncStorage.setItem(bonusStorageKey, JSON.stringify({
      amount: projection.bonusEnergy,
      expiresAt: projection.bonusExpiresAt,
    }));
  } else {
...
async function finalizePrepared(
  ...
  const operationAlreadyProjected = Boolean(
    state
    && (
      state.revision > operation.revision
      || (state.revision === operation.revision
        && state.headOperationId === operation.operationId
        && durablePrepared === null)
    )
  );
  if (operationAlreadyProjected) {
    // A direct replay of the current journal head returns its durable result.
    // A superseded operation returns the caller's locked live projection so it
    // cannot roll back later refunds/debits or elapsed recovery.
    const current = state?.revision === operation.revision
      ? state.projection
      : normalizedProjection(liveProjection, Date.now());
  ...
  await AsyncStorage.removeItem(preparedKey(owner, operation.operationId));
  return { status: existing ? 'already-applied' : 'applied', operation, projection: operation.projectionAfter };
}
...
async function refundUnlocked(
  ...
  const operationId = `${originalOperationId.slice(0, 112)}:refund`;
  const reason = safeToken(reasonInput, 'entry_failed');
  ...
  const requestFingerprint = await fingerprint(publicRequest({
    ...
    reason, grant, reversesOperationId: originalOperationId,
  }));
  ...
  return finalizePrepared(prepared, token, before);
}
...
export async function recoverEnergySessionOperations(
  ...
  return withAccountTransitionLock(async () => withStorageLock(async () => {
    ...
    for (const key of allKeys.filter((item) => item.startsWith(preparedPrefix)).sort()) {
