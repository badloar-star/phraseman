...
Случайные «интересные слова» не добавляются ради количества. По прямому решению
владельца 2026-08-30 каждая не-контрольная session сочетает 1–5 полезных новых
lexical senses с повторением знакомых, а не бесконечно перекладывает
`ready/happy/sad`. Checkpoint-сессия вводит 0 новых senses и проверяет только
разблокированное. Voice, delayed retrieval, targeted repair и transfer сначала
дают новым словам word-first grounding вне score, поэтому не являются
исключением. Новая лексика продвигается по каждой главе, а важные слова
возвращаются в последующих sessions и lessons для закрепления.
