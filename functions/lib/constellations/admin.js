...
exports.constellationAdmin = void 0;
...
async function listActiveMatches(db) {
    ...
    for (const doc of snap.docs) {
        ...
        const bots = serverSnap?.data()?.bots?.length ?? 0;
        out.push({
            matchId: doc.id,
            round: m.round ?? 0,
...
exports.constellationAdmin = (0, https_1.onCall)({ region: REGION, enforceAppCheck: callable_options_1.ENFORCE_APP_CHECK, timeoutSeconds: 30 }, async (request) => {
    if (!request.auth?.token?.admin)
        throw new https_1.HttpsError('permission-denied', 'Admin only');
    const db = admin.firestore();
    const ref = db.collection(config_1.RUNTIME_CONFIG_COLLECTION).doc(config_1.RUNTIME_CONFIG_DOC);
    const data = (request.data ?? {});
    const action = asText(data.action, 20) || 'get';
    if (action === 'setConfig') {
        const patch = data.config;
        if (!patch || typeof patch !== 'object' || Array.isArray(patch)) {
            throw new https_1.HttpsError('invalid-argument', 'bad config patch');
        }
        ...
        (0, config_1.constellationConfigFromData)(merged);
        await ref.set({
            ...merged,
    ...
    const effective = (0, config_1.constellationConfigFromData)(snap.exists ? snap.data() : undefined);
    ...
    return {
        ok: true,
        config: effective,
        defaults: config_1.CONSTELLATION_DEFAULTS,
        stats,
        matches,
        cache,
        updatedAtMs: Number(snap.data()?.updatedAtMs ?? 0),
        updatedBy: asText(snap.data()?.updatedBy, 200),
    };
});
