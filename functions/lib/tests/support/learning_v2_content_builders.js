...
exports.buildEnglishProfile = buildEnglishProfile;
exports.buildE1ContentItems = buildE1ContentItems;
...
function buildEnglishProfile() {
    ...
    if (!result.ok)
        throw new Error(`english_profile_invalid: ${result.issues.join(',')}`);
    return result.value;
}
const E1_SEEDS = [
...
function buildE1ContentItems() {
    return E1_SEEDS.map((seed, index) => {
        ...
        if (!result.ok)
            throw new Error(`e1_item_invalid: ${result.issues.join(',')}`);
        return result.value;
    });
}
//# sourceMappingURL=learning_v2_content_builders.js.map