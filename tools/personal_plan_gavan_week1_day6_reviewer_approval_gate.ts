...
import type {
  GavanWeek1Day6ReviewerContentUnitRow,
  GavanWeek1Day6ReviewerExerciseRow,
  GavanWeek1Day6ReviewerExplanationRow,
  GavanWeek1Day6ReviewerExport,
} from './personal_plan_gavan_week1_day6_reviewer_export';

export const GAVAN_WEEK1_DAY6_APPROVED_REVIEWER_EXPORT_PATH = path.join(
  process.cwd(),
  '.codex-tmp',
  'personal-plans',
...
export type GavanWeek1Day6ReviewerApprovalRecord = {
  kind: 'gavan_week1_day6_reviewer_approval_record';
...
export type GavanWeek1Day6ReviewerApprovalInput = {
  kind: 'gavan_week1_day6_reviewer_approval_input';
  dayId: 'gavan-week1-day6';
  liveIntegration: false;
  approvals: GavanWeek1Day6ReviewerApprovalRecord[];
};

export type GavanWeek1Day6ReviewerApprovalInputOptions = {
  reviewerId: string;
  approvedAt: string;
};

export type GavanWeek1Day6ApprovedContentUnitRow =
  Omit<GavanWeek1Day6ReviewerContentUnitRow, 'reviewStatus'> & {
    reviewStatus: 'approved';
    approval: ApprovalMetadata;
  };

export type GavanWeek1Day6ApprovedExplanationRow =
  Omit<GavanWeek1Day6ReviewerExplanationRow, 'reviewStatus'> & {
    reviewStatus: 'approved';
    approval: ApprovalMetadata;
  };

export type GavanWeek1Day6ApprovedExerciseRow =
  Omit<GavanWeek1Day6ReviewerExerciseRow, 'reviewStatus'> & {
    reviewStatus: 'approved';
    approval: ApprovalMetadata;
  };

type ApprovedReviewerRow =
  | GavanWeek1Day6ApprovedContentUnitRow
  | GavanWeek1Day6ApprovedExplanationRow
  | GavanWeek1Day6ApprovedExerciseRow;

export type GavanWeek1Day6ReviewerApprovalSummary = {
  contentUnits: number;
...
export type GavanWeek1Day6ApprovedReviewerExport = {
  kind: 'gavan_week1_day6_approved_reviewer_export';
  generatedAt: string;
  ...
  dayIndex: 6;
  liveIntegration: false;
  contentUnitRows: GavanWeek1Day6ApprovedContentUnitRow[];
  explanationRows: GavanWeek1Day6ApprovedExplanationRow[];
  exerciseRows: GavanWeek1Day6ApprovedExerciseRow[];
  mediaClaims: GavanWeek1Day6ReviewerExport['mediaClaims'];
  exerciseCoverage: GavanWeek1Day6ReviewerExport['exerciseCoverage'];
  summary: GavanWeek1Day6ReviewerApprovalSummary;
};

export type GavanWeek1Day6ReviewerApprovalIssueCode =
...
export type GavanWeek1Day6ReviewerApprovalIssue = {
  code: GavanWeek1Day6ReviewerApprovalIssueCode;
  detail: string;
  rowId?: string;
};

export type GavanWeek1Day6ReviewerApprovalGateResult = {
  valid: boolean;
  issues: GavanWeek1Day6ReviewerApprovalIssue[];
  summary: GavanWeek1Day6ReviewerApprovalSummary;
};

export type GavanWeek1Day6ReviewerApprovalResult =
  GavanWeek1Day6ReviewerApprovalGateResult & {
    approvedExport?: GavanWeek1Day6ApprovedReviewerExport;
  };

export type GavanWeek1Day6ApprovedReviewerExportWriteOptions =
  GavanWeek1Day6ReviewerApprovalInputOptions & {
    generatedAt: string;
    targetPath: string;
  };

export type GavanWeek1Day6ApprovedReviewerExportWriteResult =
  GavanWeek1Day6ReviewerApprovalResult & {
    targetPath?: string;
    bytesWritten?: number;
  };
...
function issue(
  ...
  detail: string,
  rowId?: string,
): GavanWeek1Day6ReviewerApprovalIssue {
...
export function isGavanWeek1Day6ApprovedReviewerExportTargetAllowed(
  targetPath: string,
  cwd = process.cwd(),
): boolean {
...
function allReviewerRows(exported: GavanWeek1Day6ReviewerExport): ReviewerRow[] {
  return [
...
function allApprovedRows(exported: GavanWeek1Day6ApprovedReviewerExport): ApprovedReviewerRow[] {
  return [
    ...exported.contentUnitRows,
    ...exported.explanationRows,
    ...exported.exerciseRows,
...
export function checksumGavanWeek1Day6ReviewerRow(row: ReviewerRow | ApprovedReviewerRow): string {
  return fnv1a(`${row.kind}|${row.id}|${reviewerRowVisibleText(row)}`);
}

function emptySummary(): GavanWeek1Day6ReviewerApprovalSummary {
  return {
    contentUnits: 0,
    explanationCards: 0,
...
function summaryFromRecords(
  records: GavanWeek1Day6ReviewerApprovalRecord[],
): GavanWeek1Day6ReviewerApprovalSummary {
...
function summaryFromApprovedExport(
  exported: GavanWeek1Day6ApprovedReviewerExport,
): GavanWeek1Day6ReviewerApprovalSummary {
  return {
...
function reviewerExportIssues(exported: GavanWeek1Day6ReviewerExport): string[] {
  const issues: string[] = [];

  if (exported.dayId !== 'gavan-week1-day6' || exported.dayIndex !== 6) {
...
export function buildGavanWeek1Day6ReviewerApprovalInput(
  exported: GavanWeek1Day6ReviewerExport,
  options: GavanWeek1Day6ReviewerApprovalInputOptions,
): GavanWeek1Day6ReviewerApprovalInput {
  return {
    kind: 'gavan_week1_day6_reviewer_approval_input',
    ...
    approvals: allReviewerRows(exported).map((row) => ({
      ...
      textChecksum: checksumGavanWeek1Day6ReviewerRow(row),
    })),
  };
}
...
function approvalByRowId(
  approvals: GavanWeek1Day6ReviewerApprovalRecord[],
): Map<string, GavanWeek1Day6ReviewerApprovalRecord> {
  const byId = new Map<string, GavanWeek1Day6ReviewerApprovalRecord>();
...
export function validateGavanWeek1Day6ReviewerApprovalGate(
  exported: GavanWeek1Day6ReviewerExport,
  input: GavanWeek1Day6ReviewerApprovalInput,
): GavanWeek1Day6ReviewerApprovalGateResult {
  const issues: GavanWeek1Day6ReviewerApprovalIssue[] = [];
  const rows = allReviewerRows(exported);
  const rowById = new Map(rows.map((row) => [row.id, row]));
  const seenApprovalIds = new Set<string>();
  const validApprovalRecords: GavanWeek1Day6ReviewerApprovalRecord[] = [];
  const exportIssues = reviewerExportIssues(exported);

  if (input.kind !== 'gavan_week1_day6_reviewer_approval_input') {
    issues.push(issue('wrong_approval_input_kind', 'Approval input must use the expected kind.'));
  }

  if (input.dayId !== exported.dayId || input.dayId !== 'gavan-week1-day6') {
    issues.push(issue('wrong_day_id', 'Approval input must describe Gavan week 1 day 6.'));
  }
  ...
  for (const record of input.approvals) {
    ...
    if (record.textChecksum !== checksumGavanWeek1Day6ReviewerRow(row)) {
      issues.push(issue(
        'text_checksum_mismatch',
...
function approvalMetadata(record: GavanWeek1Day6ReviewerApprovalRecord): ApprovalMetadata {
  return {
...
function approveContentRow(
  row: GavanWeek1Day6ReviewerContentUnitRow,
  record: GavanWeek1Day6ReviewerApprovalRecord,
): GavanWeek1Day6ApprovedContentUnitRow {
  return {
    ...row,
    reviewStatus: 'approved',
...
function approveExplanationRow(
  row: GavanWeek1Day6ReviewerExplanationRow,
  record: GavanWeek1Day6ReviewerApprovalRecord,
): GavanWeek1Day6ApprovedExplanationRow {
  return {
    ...row,
...
function approveExerciseRow(
  row: GavanWeek1Day6ReviewerExerciseRow,
  record: GavanWeek1Day6ReviewerApprovalRecord,
): GavanWeek1Day6ApprovedExerciseRow {
  return {
    ...row,
...
export function approveGavanWeek1Day6ReviewerExport(
  exported: GavanWeek1Day6ReviewerExport,
  input: GavanWeek1Day6ReviewerApprovalInput,
  generatedAt = input.approvals[0]?.approvedAt ?? exported.generatedAt,
): GavanWeek1Day6ReviewerApprovalResult {
  const gate = validateGavanWeek1Day6ReviewerApprovalGate(exported, input);
  if (!gate.valid) {
    return gate;
  }

  const approvals = approvalByRowId(input.approvals);
  const approvedExport: GavanWeek1Day6ApprovedReviewerExport = {
    kind: 'gavan_week1_day6_approved_reviewer_export',
    generatedAt,
...
export function validateGavanWeek1Day6ApprovedReviewerExport(
  exported: GavanWeek1Day6ApprovedReviewerExport,
): GavanWeek1Day6ReviewerApprovalGateResult {
  const issues: GavanWeek1Day6ReviewerApprovalIssue[] = [];
  const rows = allApprovedRows(exported);

  if (exported.kind !== 'gavan_week1_day6_approved_reviewer_export') {
    issues.push(issue('wrong_approved_export_kind', 'Approved export must use the expected kind.'));
  }

  if (exported.dayId !== 'gavan-week1-day6' || exported.dayIndex !== 6) {
    issues.push(issue('wrong_day_id', 'Approved export must describe Gavan week 1 day 6.'));
  }
  ...
  for (const row of rows) {
    ...
    if (row.approval.textChecksum !== checksumGavanWeek1Day6ReviewerRow(row)) {
      issues.push(issue(
        'approved_export_checksum_mismatch',
...
export function serializeGavanWeek1Day6ApprovedReviewerExport(
  exported: GavanWeek1Day6ApprovedReviewerExport,
): string {
  return `${JSON.stringify(exported, null, 2)}\n`;
}

export function writeGavanWeek1Day6ApprovedReviewerExport(
  exported: GavanWeek1Day6ReviewerExport,
  options: GavanWeek1Day6ApprovedReviewerExportWriteOptions,
): GavanWeek1Day6ApprovedReviewerExportWriteResult {
  const resolvedTargetPath = path.resolve(options.targetPath);

  if (!isGavanWeek1Day6ApprovedReviewerExportTargetAllowed(resolvedTargetPath)) {
    return {
      valid: false,
      issues: [
        issue(
          'target_path_not_allowed',
          'Gavan week 1 day 6 approved reviewer export can only write under .codex-tmp or docs/reports.',
        ),
      ],
  ...
  const approval = approveGavanWeek1Day6ReviewerExport(exported, input, options.generatedAt);

  if (!approval.valid || !approval.approvedExport) {
    return approval;
  }

  const serialized = serializeGavanWeek1Day6ApprovedReviewerExport(approval.approvedExport);
