...
const vocabulary = ['alone'] as const;
const phrases = [
  'You are not alone', 'You are not tired', 'You are not busy', 'You are not ready',
  'You are not here', 'You are not fine', 'You are not happy', 'You are not sad',
] as const;

assert.equal(EPISODE_01_SESSION_10_SOURCE.requiredSessionOrdinal, 10);
assert.equal(EPISODE_01_SESSION_10_SOURCE.sessionKindOverride, 'phrases');
assert.equal(EPISODE_01_SESSION_10_SOURCE.distractorAuthorship, 'manual');
assert.deepEqual(EPISODE_01_SESSION_10_SOURCE.newVocabulary?.map((entry) => entry.target), vocabulary);
assert.deepEqual(EPISODE_01_SESSION_10_SOURCE.phrases.map((phrase) => phrase.english), phrases);
assert.deepEqual(EPISODE_01_SESSION_10_SOURCE.introPages.map((page) => page.kind), ['concept', 'formula', 'trap']);
...
for (const entry of EPISODE_01_SESSION_10_SOURCE.newVocabulary ?? []) {
...
for (const phrase of EPISODE_01_SESSION_10_SOURCE.phrases) {
  assert.match(phrase.english, /^You are not\b/u);
  assert.doesNotMatch(phrase.english, /\?/u);
...
assert.deepEqual(shard.cards.slice(3, 6).map((card) => card.contentItem.target.text), ['alone', 'alone', 'alone']);
const phraseTargets = shard.cards.slice(6).map((card) => card.contentItem.target.text);
assert.deepEqual(phraseTargets.slice(0, phrases.length), phrases);
assert.ok(phraseTargets.every((target) => (phrases as readonly string[]).includes(target)));
...
for (const locale of locales) {
  const learner = buildSessionChildBodiesFromShard(shard, locale, 'lesson-01:session:10').learner;
  assert.equal(learner.interactionProfile, 'rapid');
  assert.equal(learner.interactions.length, 17);
}

const report = evaluateLearningV2SessionContentQuality(EPISODE_01_SESSION_10_SOURCE);
...
process.stdout.write('LEARNING V2 SESSION 10 WORD-FIRST GATE: PASS\n');
