...
const explain_judge_1 = require("./explain_judge");
const explain_prompts_1 = require("./explain_prompts");
const API_KEY = 'sk-test';
function chatReply(text, promptTokens = 12, completionTokens = 6) {
...
describe('heuristicPreFilter — language-aware, runs before the judge', () => {
    it('does NOT reject a valid Russian (Cyrillic) explanation for lang=ru', () => {
        expect((0, explain_judge_1.heuristicPreFilter)('Это значит пожелать удачи перед важным делом.', 'ru')).toBeNull();
    });
    it('rejects an English explanation when lang=ru (wrong script)', () => {
        expect((0, explain_judge_1.heuristicPreFilter)('This means to wish someone good luck before something.', 'ru')).toBe('non_target_language');
    });
    it('rejects empty and too-short text', () => {
        expect((0, explain_judge_1.heuristicPreFilter)('   ', 'ru')).toBe('empty');
        expect((0, explain_judge_1.heuristicPreFilter)('ок', 'ru')).toBe('too_short');
    });
});
describe('judgeExplanation — heuristic short-circuit (0 judge tokens)', () => {
    it('rejects wrong-script WITHOUT calling the provider', async () => {
        const verdict = await (0, explain_judge_1.judgeExplanation)({
...
describe('judgeExplanation — prompt-injection resistance', () => {
    it('a verdict reason outside the enum collapses to incoherent (no echo leaks through)', async () => {
        ...
        expect(verdict.ok).toBe(false);
        expect(explain_prompts_1.JUDGE_REASONS).toContain(verdict.reason);
        expect(verdict.reason).toBe('incoherent');
...
//# sourceMappingURL=explain_judge.test.js.map