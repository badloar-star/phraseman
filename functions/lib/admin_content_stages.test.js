...
describe('admin independent content stage callables', () => {
    ...
    it('accepts only a bounded Arena replacement identity backed by an approved Arena batch', () => {
        const input = (0, admin_content_stages_1.parseContentStageCreateRequest)({ requestId: 'request-arena', kind: 'arena_question_replacement', studyTarget: 'en', sourceLocale: 'ru', cefr: 'A2', objective: 'Replace one question', scopeId: 'city-a2.replace.a1', count: 1, revision: 1, prerequisiteStageIds: ['batch-stage'], replacementForQuestionId: 'a1' });
        const plan = (0, admin_content_stages_1.stagePlanFromStoredPrerequisites)(input, [{ stageId: 'batch-stage', requestId: 'request-arena', kind: 'arena_questions', artifactId: 'batch-artifact', state: 'approved', studyTarget: 'en', sourceLocale: 'ru', scopeId: 'city-a2.replace.a1' }]);
        ...
        expect(() => (0, admin_content_stages_1.parseContentStageCreateRequest)({ ...input, count: 2 })).toThrow('stage_capability_count_unsupported');
    });
