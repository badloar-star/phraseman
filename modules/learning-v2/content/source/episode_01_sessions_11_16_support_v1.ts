...
> = {
  ru: {
    ...
    meaning: "Значение",
    explain:
      "Эту фразу говорят в живой ситуации. Порядок слов показывает, кто и в какой форме связан с признаком или местом.",
  ...
  uk: {
    ...
    meaning: "Значення",
    explain:
      "Цю фразу кажуть у живій ситуації. Порядок слів показує, хто і якою формою пов’язаний з ознакою або місцем.",
  ...
  es: {
    ...
    meaning: "Significado",
    explain:
      "Esta frase se usa en una situación real. El orden muestra quién se une a una cualidad o a un lugar.",
  ...
  "pt-BR": {
    ...
    meaning: "Significado",
    explain:
      "Esta frase aparece em uma situação real. A ordem mostra quem se liga a uma característica ou lugar.",
  ...
  vi: {
    ...
    meaning: "Nghĩa",
    explain:
      "Câu này dùng trong tình huống thực. Trật tự từ cho biết ai gắn với đặc điểm hoặc địa điểm.",
  ...
  id: {
    ...
    meaning: "Arti",
    explain:
      "Kalimat ini dipakai dalam situasi nyata. Urutan kata menunjukkan siapa yang terhubung dengan sifat atau tempat.",
  ...
  tr: {
    ...
    meaning: "Anlam",
    explain:
      "Bu cümle gerçek bir durumda kullanılır. Sözcük sırası kimin bir özellik ya da yerle bağlandığını gösterir.",
  ...
  pl: {
    ...
    meaning: "Znaczenie",
    explain:
      "Tego zdania używa się w prawdziwej sytuacji. Szyk pokazuje, kto łączy się z cechą lub miejscem.",
...
const localized = (value: (locale: Locale) => string): LocalizedSource =>
  Object.fromEntries(
...
function renderMeaning(locale: Locale, english: string): string {
  ...
  const subject =
    clean.startsWith("Am I") || clean.startsWith("I am") ? "I" : "you";
  const negative = /\bnot\b/.test(clean);
  const complement = clean.replace(
    /^(?:Are you|Am I|You’re|You are|I am)(?: not)?\s*/,
    "",
  );
...
function turkishQuestionEnding(
  complement: string,
  firstPerson: boolean,
): string {
  const vowel =
    [...complement].reverse().find((char) => "aeıioöuü".includes(char)) ?? "i";
...
function phraseExplanation(locale: Locale, english: string): string {
  ...
  const copy: Record<Locale, Record<string, string>> = {
    ru: {
      question:
        "Это вопрос: связка стоит перед человеком, поэтому собеседник слышит просьбу уточнить.",
      negative:
        "Это отрицание: not идёт после связки, поэтому смысл не меняется на утверждение.",
      location:
        "Это готовое дополнение места: его говорят целиком, чтобы назвать, где находится человек.",
      state:
        "Это естественное описание состояния: связка соединяет человека и его признак.",
    },
    uk: {
      question:
        "Це запитання: зв’язка стоїть перед людиною, тому співрозмовник чує прохання уточнити.",
      negative:
        "Це заперечення: not стоїть після зв’язки, тому зміст не стає твердженням.",
      location:
        "Це готовий додаток місця: його вимовляють цілком, щоб назвати, де є людина.",
      state: "Це природний опис стану: зв’язка поєднує людину та її ознаку.",
    },
    es: {
      question:
        "Es una pregunta: be va antes de la persona y se oye que quieres confirmar algo.",
      negative:
        "Es una negación: not va después de be y no convierte la frase en una afirmación.",
      location:
        "Es un complemento de lugar completo: se dice entero para indicar dónde está la persona.",
      state:
        "Es una descripción natural de un estado: be une a la persona con su cualidad.",
    },
    "pt-BR": {
      question:
        "É uma pergunta: be vem antes da pessoa e deixa claro que você quer confirmar algo.",
      negative:
        "É uma negação: not vem depois de be e não transforma a frase em afirmação.",
      location:
        "É um complemento de lugar completo: ele é dito inteiro para indicar onde a pessoa está.",
      state:
        "É uma descrição natural de um estado: be liga a pessoa à sua característica.",
    },
    vi: {
      question:
        "Đây là câu hỏi: be đứng trước người nên người nghe biết bạn đang muốn xác nhận.",
      negative:
        "Đây là câu phủ định: not đứng sau be nên câu không thành lời khẳng định.",
      location:
        "Đây là cụm địa điểm hoàn chỉnh: hãy nói cả cụm để chỉ người đó ở đâu.",
      state:
        "Đây là cách mô tả trạng thái tự nhiên: be nối người với đặc điểm của họ.",
    },
    id: {
      question:
        "Ini pertanyaan: be berada sebelum orangnya sehingga lawan bicara tahu kamu ingin memastikan.",
      negative:
        "Ini penyangkalan: not berada setelah be sehingga kalimat tidak menjadi pernyataan.",
      location:
        "Ini pelengkap tempat yang utuh: ucapkan seluruhnya untuk menyatakan lokasi orang itu.",
      state:
        "Ini gambaran keadaan yang alami: be menghubungkan orang dengan sifatnya.",
    },
    tr: {
      question:
        "Bu bir sorudur: be kişiden önce gelir ve bir şeyi doğrulamak istediğiniz duyulur.",
      negative:
        "Bu bir olumsuzluktur: not be sonrasında gelir, bu yüzden cümle bildirim olmaz.",
      location:
        "Bu tam bir yer tamamlayıcısıdır: kişinin nerede olduğunu söylemek için bütünüyle kullanılır.",
      state: "Bu doğal bir durum açıklamasıdır: be kişiyi özelliğine bağlar.",
    },
    pl: {
      question:
        "To pytanie: be stoi przed osobą, więc rozmówca słyszy, że chcesz coś potwierdzić.",
      negative:
        "To przeczenie: not stoi po be, dlatego zdanie nie staje się twierdzeniem.",
      location:
        "To pełne określenie miejsca: wypowiada się je w całości, aby wskazać, gdzie jest osoba.",
...
const TOPIC: Record<number, Record<Locale, string>> = {
  11: {
    ...
    "pt-BR":
      "Aqui você pergunta à outra pessoa com Are you, por isso be vem primeiro.",
  ...
  14: {
    ...
    "pt-BR":
      "Aqui o lugar é dito como bloco completo: em casa, na aula ou no ônibus.",
  ...
  15: {
    ...
    "pt-BR":
      "Aqui você fala frases conhecidas em voz alta, sem acrescentar regra nova.",
  ...
  16: {
    ...
    "pt-BR":
      "Aqui você junta I e you conhecidos, perguntas, contração e lugares sem forma nova.",
...
export function assertAuthoredSessionContract(
  ...
  source.phrases.forEach((item) => {
    ...
    LOCALES.forEach((locale) =>
      expect(item.localizedDetails?.[locale]).toBeDefined(),
    );
  });
  if (taught)
    expect(source.phrases.some((item) => item.features.includes(taught))).toBe(
      true,
    );
