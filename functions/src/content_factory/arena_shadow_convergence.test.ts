import { buildArenaShadowComparison, buildArenaShadowFailureComparison, convergenceReceiptDocumentId, groupArenaConvergenceReceipts, summarizeArenaConvergenceReceipts } from './arena_shadow_convergence';
import { createHash } from 'node:crypto';
import { generateSurfaceUnit, type GenerationProvider } from './generation_provider';
...
describe('Arena shadow convergence', () => {
  it('projects the accepted legacy artifact without another provider result and builds an idempotent bounded receipt', () => {
    const receipt = buildArenaShadowComparison({ unitId: 'job:arena:9', jobId: 'job', studyTarget: 'en', learnerSourceLocale: 'ru', lessonId: 9, attempt: 1, legacyArtifact: artifact, legacyArtifactHash: 'a'.repeat(64), qaOutcome: 'pass', evidenceIds: ['blueprint:e1'], configRevision: 4 });
    ...
    expect(receipt.documentId).toBe(convergenceReceiptDocumentId('job:arena:9', 'a'.repeat(64), 'arena-parity-v1', 4, 1));
  ...
  it('fails shadow eligibility without mutating authoritative outcome when evidence is missing', () => {
    const receipt = buildArenaShadowComparison({ unitId: 'u', jobId: 'j', studyTarget: 'en', learnerSourceLocale: 'ru', lessonId: 9, attempt: 1, legacyArtifact: artifact, legacyArtifactHash: 'a'.repeat(64), qaOutcome: 'pass', evidenceIds: [], configRevision: 4 });
  ...
  it('aggregates only bounded complete receipts and exposes GO blockers', () => {
    ...
    const metrics = summarizeArenaConvergenceReceipts(receipts, { limit: 100, expectedComparatorVersion: 'arena-parity-v1', expectedConfigRevision: 4, expectedShadowUnitCount: 5, isPartial: true });
    ...
    expect(() => summarizeArenaConvergenceReceipts(receipts, { limit: 501, expectedComparatorVersion: 'arena-parity-v1', expectedConfigRevision: 4 })).toThrow('surface_convergence_metrics_limit_invalid');
  });
  ...
  it('counts the latest receipt for each Arena unit instead of inflating evidence with retries', () => {
    ...
    const metrics = summarizeArenaConvergenceReceipts(receipts, { limit: 100, expectedComparatorVersion: 'arena-parity-v1', expectedConfigRevision: 4, expectedShadowUnitCount: 1 });
  ...
  it('does not treat failed generations as successful semantic evidence', () => {
    const failures = Array.from({ length: 200 }, (_, index) => buildArenaShadowFailureComparison({ unitId: `failed-${index}`, jobId: `job-${index % 20}`, studyTarget: 'en', learnerSourceLocale: 'ru', lessonId: (index % 32) + 1, attempt: 1, configRevision: 4, error: { category: 'provider_timeout', retryable: true } }));
    const metrics = summarizeArenaConvergenceReceipts(failures, { limit: 500, expectedComparatorVersion: 'arena-parity-v1', expectedConfigRevision: 4, expectedShadowUnitCount: 200, expectedLocalePairs: ['en:ru'] });
  ...
  it('separates receipt identity across revisions and attempts while keeping one run idempotent', () => {
    ...
    expect(convergenceReceiptDocumentId('unit', hash, 'arena-parity-v1', 4, 1)).toBe(convergenceReceiptDocumentId('unit', hash, 'arena-parity-v1', 4, 1));
    expect(convergenceReceiptDocumentId('unit', hash, 'arena-parity-v1', 4, 1)).not.toBe(convergenceReceiptDocumentId('unit', hash, 'arena-parity-v1', 4, 2));
    expect(convergenceReceiptDocumentId('unit', hash, 'arena-parity-v1', 4, 1)).not.toBe(convergenceReceiptDocumentId('unit', hash, 'arena-parity-v1', 5, 1));
  });

  it('keeps bounded evidence history grouped by comparator and configuration revision', () => {
    const groups = groupArenaConvergenceReceipts([
  ...
  it('uses exactly one fake-provider request for generation plus shadow comparison', async () => {
    let calls = 0;
    const provider: GenerationProvider = { generate: async () => { calls += 1; return JSON.stringify(artifact); }, getProviderRequestCount: () => calls };
    const generated = await generateSurfaceUnit({ provider, model: 'fake', surface: 'arena', studyTarget: 'en', sourceLocale: 'ru', lessonId: 9, topic: 'requests', sourcePhrases: ['I am ready'] });
    const hash = createHash('sha256').update(JSON.stringify(generated.artifact)).digest('hex');
    const receipt = buildArenaShadowComparison({ unitId: 'u1', jobId: 'j1', studyTarget: 'en', learnerSourceLocale: 'ru', lessonId: 9, attempt: 1, legacyArtifact: generated.artifact, legacyArtifactHash: hash, qaOutcome: generated.qa.status, evidenceIds: ['e1'], configRevision: 4 });
    expect(calls).toBe(1);
  ...
  it('records equivalent terminal failure semantics without requiring an artifact', () => {
    const receipt = buildArenaShadowFailureComparison({ unitId: 'u-failed', jobId: 'j-failed', studyTarget: 'en', learnerSourceLocale: 'ru', lessonId: 9, attempt: 2, configRevision: 4, error: { category: 'provider_timeout', retryable: true } });
