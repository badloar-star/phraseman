...
test('cursor pages have no gaps or duplicates and filters run before limit', async () => {
    ...
    let cursor = '';
    do {
        const input = (0, admin_content_stages_1.parseContentStageListRequest)({ requestId, kind: 'quiz_topic', studyTarget: 'fr', sourceLocale: 'ru', limit: 40, cursor });
...
test('approved dependency pages never leak wrong state, request or language', async () => {
    ...
    let cursor = '';
    do {
        const input = (0, dependency_catalog_1.parseDependencyCatalogRequest)({ requestId, studyTarget: 'en', sourceLocale: 'ru', consumerKind: 'quiz_questions', limit: 17, cursor });
