...
type GeneratedRow = {
  ...
  generatedBy: 'gustav_personal_plan_echo_day007_french_full_day_text_packet';
};

const PLAN_ID = 'echo' as const;
const DAY_INDEX = 7 as const;
const GENERATED_BY = 'gustav_personal_plan_echo_day007_french_full_day_text_packet' as const;

const EXPLANATIONS = [
  {
    phraseId: 'echo_d7_p1',
    title: 'have = « j’ai »',
    rule: 'I have signifie « j’ai ». Ensuite, tu nommes la chose.',
    why: 'En anglais, on dit directement I have pour parler de ce que l’on possède.',
    commonMistake: 'Ne dis pas « I am have ». Dis seulement « I have ».',
  },
  {
    phraseId: 'echo_d7_p2',
    title: 'she has = « elle a »',
    rule: 'Quand tu parles d’une femme, dis she has, pas she have.',
    why: 'Après he/she, on ajoute -s au verbe. C’est une règle fixe.',
    commonMistake: 'Ne dis pas « she have ». Dis seulement « she has ».',
  },
  {
    phraseId: 'echo_d7_p3',
    title: 'Parler de ce que tu as à la maison',
    rule: 'At home signifie « à la maison ». Ajoute-le à la fin de la phrase pour préciser le lieu.',
    why: 'At home rend le lieu clair. C’est souvent utile dans une conversation.',
    commonMistake: 'Ne dis pas « in home ». Dis seulement « at home ».',
  },
  {
    phraseId: 'echo_d7_p4',
    title: 'he has = « il a »',
    rule: 'Après he, dis toujours has. Après I, dis toujours have. Mémorise ce duo.',
    why: 'C’est la seule différence. Le reste de la phrase se construit de la même façon.',
    commonMistake: 'Ne dis pas « he have ». Dis seulement « he has ».',
  },
  {
    phraseId: 'echo_d7_p5',
    title: 'we have = « nous avons »',
    rule: 'We have signifie « nous avons ». Après we, on utilise have, pas has.',
    why: 'Has va seulement après he/she/it. Après les autres sujets, on utilise have.',
    commonMistake: 'Ne dis pas « we has ». Après we, dis toujours have.',
  },
  {
    phraseId: 'echo_d7_p6',
    title: 'they have = « ils ont »',
    rule: 'They have signifie « ils ont ». Tu parles d’un groupe de personnes.',
    why: 'C’est une phrase utile pour parler de famille, d’amis ou de collègues.',
    commonMistake: 'Ne dis pas « they has ». Après they, dis toujours have.',
  },
] as const;
...
const FIELD_SPECS: FieldSpec[] = [
  {
    fieldPath: 'topic',
    fieldKind: 'day_topic',
    frenchText: 'Dire ce que tu as',
    source: (day) => day.topic,
  },
  {
    fieldPath: 'outcome',
    fieldKind: 'day_outcome',
    frenchText: 'Tu peux dire en anglais ce que tu as : un journal, une émission préférée, une bonne radio.',
    source: (day) => day.outcome,
  },
  {
    fieldPath: 'intro[0].title',
    fieldKind: 'intro_title',
    frenchText: 'Comment dire « j’ai »',
    source: (day) => day.intro?.[0]?.title,
  },
  {
    fieldPath: 'intro[0].body',
    fieldKind: 'intro_body',
    frenchText: 'En anglais, « j’ai » se dit simplement avec le mot have. Dis « I have », puis ce que tu as. Rien de compliqué.',
    source: (day) => day.intro?.[0]?.body,
  },
  {
    fieldPath: 'intro[0].examples[0].gloss',
    fieldKind: 'intro_example_gloss',
    frenchText: 'J’ai un journal.',
  ...
  },
  {
    ...
    fieldKind: 'intro_example_gloss',
    frenchText: 'Elle a une émission préférée.',
  ...
  },
  {
    ...
    fieldKind: 'intro_title',
    frenchText: 'I have ou she has ?',
    source: (day) => day.intro?.[1]?.title,
  },
  {
    fieldPath: 'intro[1].body',
    fieldKind: 'intro_body',
    frenchText: 'Quand tu parles de toi, dis I have. Quand tu parles d’une autre personne, he ou she, dis he has ou she has. On ajoute seulement une lettre.',
    source: (day) => day.intro?.[1]?.body,
  },
  {
    fieldPath: 'intro[1].examples[0].gloss',
    fieldKind: 'intro_example_gloss',
    frenchText: 'J’ai une bonne radio.',
  ...
  },
  {
    ...
    fieldKind: 'intro_example_gloss',
    frenchText: 'Il a une chaîne d’information.',
  ...
  },
  {
    ...
    fieldKind: 'intro_title',
    frenchText: 'Le mot a avant la chose',
    source: (day) => day.intro?.[2]?.title,
  },
  {
    fieldPath: 'intro[2].body',
    fieldKind: 'intro_body',
    frenchText: 'Après have, il y a presque toujours le petit mot a. Souviens-toi : I have a newspaper, I have a radio. Cela fait partie de la phrase.',
    source: (day) => day.intro?.[2]?.body,
  },
  {
    fieldPath: 'intro[2].examples[0].gloss',
    fieldKind: 'intro_example_gloss',
    frenchText: 'J’ai un journal préféré.',
  ...
  },
  {
    ...
    fieldKind: 'intro_example_gloss',
    frenchText: 'Elle a une émission du matin.',
...
function rowIdFromPath(fieldPath: string): string {
  return `fr_echo_d007_${fieldPath.replace(/[^a-zA-Z0-9]+/g, '_').replace(/^_+|_+$/g, '').toLowerCase()}`;
}
...
function renderMarkdown(report: any): string {
  const lines = [
    '# GUSTAV Personal Plan Echo Day 007 French Full-Day Text Packet',
    '',
    `Run: \`${report.runId}\``,
    '',
...
async function main(): Promise<void> {
  ...
  if (!runArg) {
    throw new Error('Usage: npx tsx scripts/gustav_personal_plan_echo_day007_french_full_day_text_packet.ts --run <run-dir>');
  }
  ...
  const rowsPath = path.join(outDir, 'echo_day_007_full_day_fr_rows.jsonl');
  const reviewerQueuePath = path.join(outDir, 'echo_day_007_full_day_fr_reviewer_queue.tsv');
  const packetJsonPath = path.join(auditsDir, 'personal_plan_echo_day007_french_full_day_text_packet.json');
  const packetMdPath = path.join(auditsDir, 'personal_plan_echo_day007_french_full_day_text_packet.md');

  const findings: Finding[] = [];
  const day = await readEchoDay(repoRoot);
  if (!day) {
    addFinding(findings, 'blocker', 'echo_day_007_missing', 'Echo day 7 is missing from source content.', rel(repoRoot, sourcePath));
  }

  let existingPhraseMeaningRowsForDay = 0;
  if (!fs.existsSync(phraseMeaningRowsPath)) {
    addFinding(findings, 'blocker', 'phrase_meaning_rows_missing', 'Echo day 7 phrase meaning rows are missing.', rel(repoRoot, phraseMeaningRowsPath));
  } else {
    ...
    if (existingPhraseMeaningRowsForDay !== 6) {
      addFinding(findings, 'blocker', 'phrase_meaning_rows_day_007_incomplete', `Expected 6 reviewer-needed phrase meaning rows for Echo day 7 but found ${existingPhraseMeaningRowsForDay}.`, rel(repoRoot, phraseMeaningRowsPath));
    }
  }
  ...
  const report = {
    schemaVersion: 'gustav-personal-plan-echo-day007-french-full-day-text-packet-v0',
    ...
    fieldKindCounts,
    generationPolicy: [
      'This packet translates Echo day 7 full-day text fields except phrase meanings, which are already covered by the phrase-meaning layer.',
  ...
  console.log(`GUSTAV Echo day 007 French full-day text packet: ${report.status}`);
