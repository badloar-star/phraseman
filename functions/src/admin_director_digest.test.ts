...
import {
  getDirectorDigestResponse,
  type DirectorDigestDependencies,
  type DirectorDigestTrendsResponse,
} from './admin_director_digest';

const NOW_MS = Date.parse('2026-07-19T12:34:56.789Z');

function auth(role: 'owner' | 'admin' | 'analyst' = 'owner') {
  return {
    uid: `${role}-uid`,
...
function trendsResponse(
  overrides: Partial<DirectorDigestTrendsResponse> = {},
): DirectorDigestTrendsResponse {
  return {
    state: 'ready',
    sources: [
      {
...
function dependencies(
  response: DirectorDigestTrendsResponse = trendsResponse(),
): DirectorDigestDependencies & { loadTrends: jest.Mock } {
  return {
    nowMs: () => NOW_MS,
...
describe('admin director digest', () => {
  test('exports an App Check protected narrative callable without old digest runtime or writes', () => {
    const source = readFileSync(join(__dirname, 'admin_director_digest.ts'), 'utf8');
  ...
  test.each([1, 3, 7, 28, 90])(
    'accepts the supported %i-day period and requests a trusted previous-period comparison',
    async (rangeDays) => {
      const deps = dependencies();
      const result = await getDirectorDigestResponse({ rangeDays }, auth(), deps);
  ...
  test.each([
    ...
    const deps = dependencies();
    await expect(getDirectorDigestResponse(data, auth(), deps)).rejects.toMatchObject({
  ...
  test('requires authentication before reading sources', async () => {
    const deps = dependencies();
    await expect(getDirectorDigestResponse({ rangeDays: 7 }, null, deps)).rejects.toMatchObject({
  ...
  test.each([
    ...
    const deps = dependencies();
    await expect(getDirectorDigestResponse({ rangeDays: 7 }, caller, deps)).rejects.toMatchObject({
      code: 'permission-denied',
    } satisfies Partial<HttpsError>);
    expect(deps.loadTrends).not.toHaveBeenCalled();
  });

  test.each(['owner', 'admin'] as const)('allows the %s role with briefing.read', async (role) => {
    const deps = dependencies();
    await expect(getDirectorDigestResponse({ rangeDays: 7 }, auth(role), deps)).resolves.toMatchObject({
      state: 'ready',
    });
  });

  test('uses exact UTC calendar boundaries for current and previous periods', async () => {
    const result = await getDirectorDigestResponse({ rangeDays: 3 }, auth(), dependencies());
  ...
  test('marks incomplete sources explicitly and never invents a zero or trend', async () => {
    ...
    const result = await getDirectorDigestResponse({ rangeDays: 1 }, auth(), dependencies(response));
  ...
  test('fails closed when a required source-health record is missing', async () => {
    ...
    const result = await getDirectorDigestResponse({ rangeDays: 3 }, auth(), dependencies(response));
  ...
  test('returns the validated V2 narrative generated only from the sanitized aggregate prompt', async () => {
    const deps = {
      ...dependencies(),
      generateNarrative: jest.fn(async (prompt: { system: string; user: string }) => ({
        text: JSON.stringify({
    ...
    const result = await getDirectorDigestResponse(
      { rangeDays: 3 },
      auth(),
      deps as unknown as DirectorDigestDependencies,
    ) as unknown as {
      schemaVersion: number;
      narrativeState: string;
      narrative: {
        ownerMonologue: string;
        executiveSummary: string;
        productManager: Array<{ fact: string; comparison: string }>;
      };
    };
    ...
    expect(`${prompt.system}\n${prompt.user}`).not.toMatch(
      /PRIVATE_UID|PRIVATE_BODY|PRIVATE_EMAIL|private source prose/,
    );
  });
  ...
  test('keeps the aggregate response available with a deterministic narrative when generation fails', async () => {
    ...
    const result = await getDirectorDigestResponse(
      ...
      narrativeState: string;
      narrative: { ownerMonologue: string; executiveSummary: string; productManager: unknown[] };
      metrics: unknown[];
    };
  ...
  test('does not spend on narrative generation when all trusted sources are unavailable', async () => {
    ...
    const result = await getDirectorDigestResponse(
      ...
      auth(),
      deps as unknown as DirectorDigestDependencies,
    ) as unknown as { state: string; narrativeState: string };
