...
exports.tournamentRoundReview = exports.tournamentStartNow = exports.tournamentClaimReward = exports.tournamentAdvanceRound = exports.tournamentAdvanceRooms = exports.tournamentFinalize = exports.tournamentSubmitAnswers = exports.tournamentSubmitTaskAnswer = exports.tournamentSubmitSpeedMatchAttempt = exports.tournamentFillBots = exports.tournamentForfeit = exports.tournamentLeave = exports.tournamentJoin = exports.tournamentCreateRooms = exports.TOURNAMENT_PRIVATE_EVIDENCE_RETENTION_MS = exports.TOURNAMENT_REVIEW_RETENTION_MS = void 0;
exports.tournamentTestModeReleaseEnabled = tournamentTestModeReleaseEnabled;
exports.tournamentStartNowInternalErrorLog = tournamentStartNowInternalErrorLog;
exports.speedMatchAttemptDocId = speedMatchAttemptDocId;
exports.tournamentTaskAnswerReceiptDocId = tournamentTaskAnswerReceiptDocId;
exports.tournamentTaskRankCounterDocId = tournamentTaskRankCounterDocId;
exports.parseTournamentTaskDocument = parseTournamentTaskDocument;
...
exports.validBotProfile = validBotProfile;
exports.sameTournamentPoolBarrierToken = sameTournamentPoolBarrierToken;
exports.createTournamentResourcePoolCacheLoader = createTournamentResourcePoolCacheLoader;
exports.loadTournamentStartNowPrerequisites = loadTournamentStartNowPrerequisites;
exports.parseReadyTournamentPoolToken = parseReadyTournamentPoolToken;
exports.tournamentExposureBucketId = tournamentExposureBucketId;
exports.loadTournamentTaskSlicesForToken = loadTournamentTaskSlicesForToken;
exports.assertTournamentPoolCommitAllowed = assertTournamentPoolCommitAllowed;
exports.createTournamentTaskSecretsInTransaction = createTournamentTaskSecretsInTransaction;
exports.buildTournamentRounds = buildTournamentRounds;
exports.planTournamentStartNowJoinedRoom = planTournamentStartNowJoinedRoom;
exports.tournamentJoinTransaction = tournamentJoinTransaction;
exports.tournamentLeaveTransaction = tournamentLeaveTransaction;
exports.tournamentForfeitTransaction = tournamentForfeitTransaction;
...
exports.tournamentSpeedMatchAttemptTransaction = tournamentSpeedMatchAttemptTransaction;
exports.tournamentSubmitTaskAnswerTransaction = tournamentSubmitTaskAnswerTransaction;
...
exports.catchUpRoomAtDeadline = catchUpRoomAtDeadline;
exports.processDueTournamentRooms = processDueTournamentRooms;
exports.cleanupExpiredTournamentReviewEvidence = cleanupExpiredTournamentReviewEvidence;
...
exports.TOURNAMENT_REVIEW_RETENTION_MS = 24 * 60 * 60 * 1000;
exports.TOURNAMENT_PRIVATE_EVIDENCE_RETENTION_MS = 7 * 24 * 60 * 60 * 1000;
/**
 ...
 */
function tournamentTestModeReleaseEnabled(env = process.env) {
    return env.PHRASEMAN_TOURNAMENT_TEST_MODE_RELEASE === '1';
}
function tournamentStartNowInternalErrorLog(error) {
...
function speedMatchAttemptDocId(roundNo, taskId, stableUid) {
    return `${SPEED_MATCH_ATTEMPT_PREFIX}:${roundNo}:${taskId}:${stableUid}`;
}
function tournamentTaskAnswerReceiptDocId(roundNo, taskId, stableUid) {
 ...
 */
function tournamentTaskRankCounterDocId(roundNo, taskId) {
    return `${TASK_RANK_COUNTER_PREFIX}:${roundNo}:${taskId}`;
}
function readTournamentTaskRankCounter(data) {
    if (!data || data.kind !== TASK_RANK_COUNTER_KIND)
        return undefined;
    return { correctCount: Math.max(0, readInt(data.correctCount, 0)) };
}
function readTournamentTaskAnswerReceipt(data, expected) {
    if (!data)
        return undefined;
    ...
    if (data.kind !== 'tournament_task_answer_v1'
        ...
        throw new https_1.HttpsError('failed-precondition', 'task_answer_receipt_invalid');
    }
...
function tournamentReceiptRefsForRound(roomRef, round, playerIds) {
    return playerIds.flatMap((playerId) => round.taskIds.map((taskId) => ({
        playerId,
        taskId,
        ref: roomRef.collection(tournament_core_1.TOURNAMENT_TASK_SECRETS_SUBCOLLECTION)
            .doc(tournamentTaskAnswerReceiptDocId(round.roundNo, taskId, playerId)),
    })));
}
function tournamentReceiptIsTimely(round, receipt, fallbackDeadlineAtMs) {
    try {
        const scheduleEnforced = (0, tournament_core_1.assertTournamentAnswersWithinTaskSchedule)(round, [receipt.taskId], receipt.receivedAtMs);
        return scheduleEnforced || fallbackDeadlineAtMs === undefined
            || receipt.receivedAtMs <= fallbackDeadlineAtMs;
    }
    catch (error) {
        const message = error instanceof Error ? error.message : '';
        if (message === 'task_not_started' || message === 'task_deadline_elapsed')
            return false;
        throw error;
    }
}
function canonicalTournamentReceiptRanks(round, tasks, playerIds, receipts, fallbackDeadlineAtMs) {
    const taskMap = new Map(tasks.map((task) => [task.taskId, task]));
    const receiptByPlayerTask = new Map();
    const candidatesByTask = new Map();
    for (const receipt of receipts) {
        if (!playerIds.includes(receipt.playerId)
            || !tournamentReceiptIsTimely(round, receipt, fallbackDeadlineAtMs))
            continue;
        const task = taskMap.get(receipt.taskId);
        if (!task)
            throw new https_1.HttpsError('failed-precondition', 'round_tasks_unavailable');
        const verified = (0, tournament_core_1.verifyTournamentAnswer)(task, receipt.answer);
    ...
    for (const playerId of playerIds) {
        ...
        for (const review of result.review ?? []) {
            if (!review.correct || receiptByPlayerTask.has(`${playerId}\u0000${review.taskId}`))
                continue;
    ...
    for (const [taskId, candidates] of candidatesByTask) {
        ...
        candidates.forEach((candidate, index) => {
            var _a;
            (ranks[_a = candidate.playerId] ?? (ranks[_a] = {}))[taskId] = index + 1;
        });
    }
    return ranks;
}
function readSpeedMatchProgress(data) {
    if (!data || data.kind !== 'speed_match_attempt_v1')
        return undefined;
    return {
        matchedIndexes: Array.isArray(data.matchedIndexes) ? data.matchedIndexes.map((value) => readInt(value, -1)) : [],
        triedIndexes: Array.isArray(data.triedIndexes)
            ? data.triedIndexes.map((row) => Array.isArray(row) ? row.map((value) => readInt(value, -1)) : [])
            : [],
 ...
 */
function lobbyDeadlineAtMs(data, room, nowMs) {
    ...
    return gatherStartedAtMs + tournament_core_1.TOURNAMENT_ROOM_GATHER_MS + tournament_core_1.TOURNAMENT_BOT_FILL_WINDOW_MS;
}
function tournamentPlayersHaveArrived(players, nowMs) {
...
function tournamentProfileHint(value) {
...
function readRoom(snap) {
    ...
    return {
        ...
        economySnapshot: data.economySnapshot
            ? (0, tournament_economy_1.normalizeTournamentEconomy)(data.economySnapshot)
            : undefined,
        potGems: Math.max(0, readGemBalance(data.potGems)),
        lobbyEvents: Array.isArray(data.lobbyEvents)
            ? data.lobbyEvents.slice(0, tournament_core_1.TOURNAMENT_ROOM_SIZE)
            : [],
        ...
        players: Array.isArray(data.players) ? data.players.map((raw) => {
...
function parseTournamentTaskDocument(taskId, data) {
    const parsedExplanation = data.explanation && typeof data.explanation === 'object'
        && !Array.isArray(data.explanation)
        ? {
            ruleNote: sanitizeString(data.explanation.ruleNote, 600),
            example: sanitizeString(data.explanation.example, 600),
...
function parseTask(snap) {
    if (!snap.exists)
        return null;
    return parseTournamentTaskDocument(snap.id, (snap.data() || {}));
}
...
function validBotProfile(id, data) {
    if (data.isBot !== true || !(0, tournament_reddit_bot_names_1.isAcceptedTournamentBotSeedVersion)(data.seedVersion))
        return null;
    ...
    const profileIndex = tournament_reddit_bot_names_1.TOURNAMENT_REDDIT_BOT_PROFILE_IDS.indexOf(id);
    if (!id || profileIndex < 0 || !name || !(0, tournament_reddit_bot_names_1.isSafeTournamentBotName)(name) || !storedAvatar || !color
    ...
    const visual = (0, tournament_core_1.generateBotProfile)(profileIndex, tournament_reddit_bot_names_1.TOURNAMENT_REDDIT_BOT_PERSONA_SEED);
    return {
        botId: id,
        name,
...
const TOURNAMENT_BUCKETED_EXPOSURE_BUCKET_DWELL_DAYS = Object.freeze({
    guess_phrase: 14,
    fill_gap: 14,
    find_oddity: 20,
    translate_build: 19,
...
const TOURNAMENT_BUCKETED_EXPOSURE_EPOCH_DAY = Math.floor(Date.parse('2026-08-01T00:00:00.000Z') / 86400000);
function sameTournamentPoolBarrierToken(left, right) {
...
const TOURNAMENT_RESOURCE_POOL_CACHE_TTL_MS = 60000;
function createTournamentResourcePoolCacheLoader(options) {
    ...
    return async () => {
        ...
        if (inFlight && sameTournamentPoolBarrierToken(inFlight.token, token))
        ...
        const promise = options.loadForToken(token).then(async (pool) => {
            if (pool.taskPoolGeneration !== token.generation || pool.taskPoolRevision !== token.revision) {
                throw new https_1.HttpsError('aborted', 'tournament_pool_generation_changed');
            }
...
async function loadTournamentStartNowPrerequisites(options) {
    ...
    const reads = Promise.allSettled([configPromise, resourcesPromise, economyPromise]);
    ...
    if (economy.status === 'rejected')
        throw economy.reason;
    return {
        config: config.value,
        stableUid,
...
function tournamentPoolBarrierRef(db) {
    return db.collection(tournament_core_1.TOURNAMENT_PRIVATE_STATE_COLLECTION).doc(tournament_core_1.TOURNAMENT_POOL_BARRIER_DOC);
}
function parseReadyTournamentPoolToken(data) {
    if (!data || data.kind !== tournament_core_1.TOURNAMENT_POOL_BARRIER_KIND) {
        throw new https_1.HttpsError('failed-precondition', 'tournament_pool_barrier_unavailable');
    }
    if (data.state === 'migrating') {
        throw new https_1.HttpsError('failed-precondition', 'tournament_pool_migrating');
    }
    ...
    if (data.state !== 'ready' || generation.length === 0 || generation.length > 160
        ...
        throw new https_1.HttpsError('failed-precondition', 'tournament_pool_barrier_invalid');
    }
    if (!TOURNAMENT_BUCKETED_POOL_GENERATIONS.has(generation))
    ...
    const validCounts = rawCounts && typeof rawCounts === 'object' && !Array.isArray(rawCounts)
        && tournament_pool_plan_1.TOURNAMENT_MODES.every((mode) => (Number.isSafeInteger(rawCounts[mode])
            && rawCounts[mode] === TOURNAMENT_BUCKETED_EXPOSURE_BUCKET_COUNTS[mode]))
        && Object.keys(rawCounts).length === tournament_pool_plan_1.TOURNAMENT_MODES.length;
    ...
    return {
        generation,
        revision,
        exposureBucketCounts: Object.freeze(Object.fromEntries(tournament_pool_plan_1.TOURNAMENT_MODES.map((mode) => [mode, rawCounts[mode]]))),
        exposureLayoutHash,
    };
}
const readyTournamentPoolToken = parseReadyTournamentPoolToken;
function tournamentExposureBucketId(token, mode, dayOrdinal) {
    const count = token.exposureBucketCounts?.[mode];
    if (!Number.isSafeInteger(count) || !count || count < 1) {
        throw new https_1.HttpsError('failed-precondition', 'tournament_pool_exposure_layout_required');
    }
...
async function loadTournamentTaskSlicesForToken(options) {
    const rowsByMode = await Promise.all(tournament_pool_plan_1.TOURNAMENT_MODES.map(async (mode) => {
        ...
        if (rows.length > TASKS_PER_MODE_SLICE || rows.some((row) => (row.mode !== mode
            || row.poolVersion !== options.token.generation
            || row.exposureBucket !== bucket
            || row.verified !== true
            || row.source !== 'ai'))) {
            throw new https_1.HttpsError('failed-precondition', 'tournament_pool_exposure_task_mismatch');
        }
        return rows;
    }));
    return rowsByMode.flatMap((rows) => [...rows]);
}
async function assertTournamentPoolCommitAllowed(tx, db, expectedToken) {
    ...
    if (!expectedToken || typeof expectedToken === 'string'
        ...
        throw new https_1.HttpsError('failed-precondition', 'tournament_pool_token_required');
    }
...
function createTournamentTaskSecretsInTransaction(tx, roomRef, tasks) {
    for (const task of tasks) {
        tx.create(roomRef.collection(tournament_core_1.TOURNAMENT_TASK_SECRETS_SUBCOLLECTION).doc(task.taskId), task);
    }
}
async function loadResourcePoolFromFirestore(db, token) {
    ...
    const [botSnaps, taskRows] = await Promise.all([
        db.getAll(...tournament_reddit_bot_names_1.TOURNAMENT_REDDIT_BOT_PROFILE_IDS.map((botId) => (db.collection(tournament_core_1.BOT_PROFILES_COLLECTION).doc(botId)))),
        loadTournamentTaskSlicesForToken({
            token,
            dayOrdinal,
            readLegacyMode: async (mode, limit) => {
                const snap = await db.collection(tournament_core_1.TOURNAMENT_TASKS_COLLECTION)
                    .where('verified', '==', true)
                    .where('source', '==', 'ai')
                    .where('mode', '==', mode)
    ...
    const tasks = taskRows
        .map((row) => parseTournamentTaskDocument(String(row.taskId ?? ''), row))
        .filter((task) => !!task && (0, tournament_core_1.validateTournamentTaskForNewRoom)(task).ok);
    return {
        bots,
        tasks,
...
async function loadCuratedForRoom(db, roomId, pool) {
    ...
    if (missing.length > 0) {
        ...
        for (const taskSnap of snaps) {
            ...
            if (task && taskSnap.get('source') === 'ai' && (0, tournament_core_1.validateTournamentTaskForNewRoom)(task).ok)
...
function buildTournamentRounds(roomId, pool, curatedRounds) {
    const poolMap = new Map(pool.map((task) => [task.taskId, task]));
    const usedTaskIds = new Set();
    const rounds = [];
    for (let index = 0; index < tournament_core_1.TOURNAMENT_ROUND_MODE_PLAN.length; index += 1) {
        ...
        for (const plannedMode of plannedModes) {
            ...
            const picked = (0, tournament_core_1.selectRoundTasks)({
...
function publicTournamentPlayer(player) {
    const { isBot: _isBot, botWinRate: _botWinRate, resultPlace: rawResultPlace, rewardGems: rawRewardGems, ...publicPlayer } = player;
...
function recentBotRosterRef(db) {
    return db.collection(tournament_core_1.TOURNAMENT_PRIVATE_STATE_COLLECTION).doc(RECENT_BOT_ROSTER_DOC);
}
function readRecentBotProfileIds(data) {
    if (data?.kind !== 'recent_tournament_bot_roster_v1' || !Array.isArray(data.botProfileIds))
        return [];
    return Array.from(new Set(data.botProfileIds
        .map((value) => sanitizeString(value, 160))
        .filter((value) => !!value))).slice(0, tournament_core_1.TOURNAMENT_ROOM_SIZE);
}
function botArrivalPotEvents(players, startingPotGems, botEntryGems) {
    let potGemsAfter = Math.max(0, Math.trunc(startingPotGems));
    return players.slice().sort((left, right) => ((left.joinAtMs ?? 0) - (right.joinAtMs ?? 0) || left.id.localeCompare(right.id))).map((player) => {
...
function buildBotReservationPlan(input) {
    const selected = (0, tournament_core_1.selectTournamentBotProfiles)({
    ...
    const fillEndsAtMs = input.gatherStartedAtMs
        + tournament_core_1.TOURNAMENT_ROOM_GATHER_MS
        + tournament_core_1.TOURNAMENT_BOT_FILL_WINDOW_MS;
    const joinTimes = (0, tournament_core_1.planBotJoinTimes)({
        seed: input.roomId,
        botCount: selected.length,
        fromMs: input.gatherStartedAtMs,
        startsAtMs: fillEndsAtMs,
    });
    const players = selected.map((bot, index) => ({
        id: `p_${(0, tournament_core_1.tournamentHash32)(`${input.roomId}:${bot.botId}`).toString(36)}`,
        isBot: true,
        name: bot.name,
...
function recentBotRosterData(roomId, profileIds, nowMs) {
    return {
        kind: 'recent_tournament_bot_roster_v1',
        roomId,
        botProfileIds: profileIds.slice(0, tournament_core_1.TOURNAMENT_ROOM_SIZE),
        updatedAtMs: nowMs,
    };
}
function hydratePrivateBotMetadata(room, data) {
    ...
    if (!data && room.players.every((player) => typeof player.isBot === 'boolean'))
        return room;
    ...
    for (const entry of entries) {
        ...
        if (!playerId || !roomPlayerIds.has(playerId) || botRates.has(playerId)
            || !Number.isFinite(winRate) || winRate < 0.15 || winRate > 0.85) {
            throw new https_1.HttpsError('failed-precondition', 'bot_simulation_metadata_invalid');
        }
...
function publicTournamentRounds(rounds) {
...
function roundsWithActivatedPublicTasks(rounds, roundNo, tasks, stateStartedAtMs, 
...
roomId) {
    const publicTasks = tasks.map((task) => (0, tournament_core_1.toPublicTournamentTask)(task, roomId));
    const { taskSchedule } = (0, tournament_core_1.tournamentRoundTimingWindow)(tasks, stateStartedAtMs);
    if (publicTasks.some((task) => task === null))
        throw new https_1.HttpsError('failed-precondition', 'round_tasks_unavailable');
    return publicTournamentRounds(rounds.map((round) => {
        if (round.roundNo === roundNo) {
            return { ...round, tasks: publicTasks, taskSchedule };
        }
        if (round.roundNo < roundNo)
            return round;
...
function planTournamentStartNowJoinedRoom(input) {
    const { room, authUid, stableUid, user, publicProfile, leaderboardProfile, config, currentEconomy, selectedTasks, nowMs, } = input;
    if (!input.testModeReleaseEnabled) {
        throw new https_1.HttpsError('failed-precondition', 'tournament_testing_disabled');
    }
    if ((0, tournament_core_1.tournamentRoomAdmissionMode)(room, config) !== 'test') {
        throw new https_1.HttpsError('failed-precondition', 'tournament_testing_disabled');
    }
    const economy = (0, tournament_core_1.tournamentEconomySnapshotForMode)(currentEconomy, true);
    if (economy.entryGems !== 0 || economy.botEntryGems !== 0) {
        throw new https_1.HttpsError('failed-precondition', 'tournament_test_economy_invalid');
    }
    const gemsBefore = readGemBalance(user.shards);
    const playerProfile = (0, tournament_core_1.resolveTournamentPlayerProfile)(publicProfile, leaderboardProfile, user, tournamentProfileHint(input.profileHint));
    const player = {
        id: stableUid,
        isBot: false,
        ...playerProfile,
        ...
        entry: {
            ...
            weekId: (0, tournament_core_1.tournamentWeekId)(room.startsAt),
        },
    };
    let joinPlan;
    try {
        joinPlan = (0, tournament_core_1.planTournamentHumanLobbyJoin)(room, player, authUid, nowMs);
    }
    catch (error) {
    ...
    const fillsLastSeat = room.state === 'lobby'
        && nextRoom.players.length === tournament_core_1.TOURNAMENT_ROOM_SIZE
        && joinPlan.startImmediately;
    let activatedRounds;
    let activationTiming;
    if (fillsLastSeat) {
        ...
        activationTiming = (0, tournament_core_1.tournamentRoundTimingWindow)(firstRoundTasks, nowMs);
        activatedRounds = roundsWithActivatedPublicTasks(nextRoom.rounds, 1, firstRoundTasks, nowMs, room.roomId);
    }
    const botMetadata = joinPlan.displacedBotPlayerId
        ? {
            ...input.botMetadata,
            expectedBotCount: input.botMetadata.bots.length - 1,
            bots: input.botMetadata.bots.filter((entry) => entry.playerId !== joinPlan.displacedBotPlayerId),
        }
        : input.botMetadata;
...
exports.tournamentCreateRooms = (0, scheduler_1.onSchedule)({ schedule: '*/5 * * * *', timeZone: 'UTC', region: 'europe-west1', maxInstances: 1 }, async () => {
    const db = admin.firestore();
    const [config, resources, economySnap] = await Promise.all([
        loadScheduleConfig(db),
        loadResourcePool(db),
        db.collection(tournament_core_1.TOURNAMENT_SCHEDULE_COLLECTION).doc('economy').get(),
    ]);
    const economySnapshot = (0, tournament_core_1.tournamentEconomySnapshotForMode)(economySnap.data(), false);
    ...
    for (const slot of config.slots) {
        ...
        await db.runTransaction(async (tx) => {
            ...
            if (existing.exists)
                return;
            const taskPoolGeneration = await assertTournamentPoolCommitAllowed(tx, db, {
...
async function tournamentJoinTransaction(db, input) {
    ...
    const economyRef = db.collection(tournament_core_1.TOURNAMENT_SCHEDULE_COLLECTION).doc('economy');
    const authLinkRef = db.collection('auth_links').doc(authUid);
    const bannedRef = db.collection('banned_users').doc(stableUid);
    const botMetadataRef = roomRef.collection(tournament_core_1.TOURNAMENT_TASK_SECRETS_SUBCOLLECTION)
    ...
    return db.runTransaction(async (tx) => {
        const [roomSnap, userSnap, economySnap, configSnap, authLinkSnap, bannedSnap, leaderboardSnap, publicProfileSnap, botMetadataSnap] = await tx.getAll(roomRef, userRef, economyRef, configRef, authLinkRef, bannedRef, leaderboardRef, publicProfileRef, botMetadataRef);
        if (!roomSnap.exists)
        ...
        const playerProfile = (0, tournament_core_1.resolveTournamentPlayerProfile)(publicProfile, profile, user, tournamentProfileHint(input.profileHint));
        const existingPlayer = room.players.find((player) => !player.isBot && player.id === stableUid);
        if (existingPlayer) {
            const replayRoom = (0, tournament_core_1.applyTournamentJoin)(room, { ...existingPlayer, ...playerProfile, avatar: playerProfile.avatar || '🙂' }, authUid, nowMs);
            if (replayRoom !== room) {
                tx.set(roomRef, {
        ...
        const admissionMode = (0, tournament_core_1.tournamentRoomAdmissionMode)(room, config);
        if (!admissionMode) {
            throw new https_1.HttpsError('failed-precondition', (0, tournament_core_1.isTournamentTestRoom)(room) ? 'tournament_testing_disabled' : 'tournament_config_disabled');
        }
        ...
        const roomEconomySnapshot = (0, tournament_economy_1.normalizeTournamentEconomy)(room.economySnapshot ?? economySnap.data());
        const economy = (0, tournament_core_1.tournamentEconomySnapshotForMode)(roomEconomySnapshot, admissionMode === 'test');
        ...
        const slotKey = `${room.slotId}_${(0, tournament_core_1.dateKeyInTimezone)(room.startsAt, roomTimezone)}`;
        ...
        if (admissionMode === 'scheduled' && sanitizeString(user.tournament_last_slot_key, 200) === slotKey) {
            throw new https_1.HttpsError('failed-precondition', 'slot_already_played');
        }
        const gemsBefore = readGemBalance(user.shards);
        if (gemsBefore < entryGems) {
            throw new https_1.HttpsError('failed-precondition', 'not_enough_gems');
        }
        ...
        if (fillsLastSeat) {
            ...
            const taskSnaps = await tx.getAll(...firstRound.taskIds.map((taskId) => (roomRef.collection(tournament_core_1.TOURNAMENT_TASK_SECRETS_SUBCOLLECTION).doc(taskId))));
            const firstRoundTasks = taskSnaps.map(parseTask);
            if (firstRoundTasks.some((task) => task === null)) {
                throw new https_1.HttpsError('failed-precondition', 'round_tasks_unavailable');
            }
            activationTiming = (0, tournament_core_1.tournamentRoundTimingWindow)(firstRoundTasks, nowMs);
            activatedRounds = roundsWithActivatedPublicTasks(nextRoom.rounds, 1, firstRoundTasks, nowMs, roomId);
        }
...
async function tournamentLeaveTransaction(db, input) {
    const { authUid, stableUid, roomId } = input;
    const nowMs = input.nowMs ?? Date.now();
    const roomRef = db.collection(tournament_core_1.TOURNAMENT_ROOMS_COLLECTION).doc(roomId);
    const userRef = db.collection('users').doc(stableUid);
    const receiptRef = userRef.collection(tournament_core_1.TOURNAMENT_RECEIPTS_SUBCOLLECTION).doc(`leave_${roomId}`);
    const authLinkRef = db.collection('auth_links').doc(authUid);
    const bannedRef = db.collection('banned_users').doc(stableUid);
    const botMetadataRef = roomRef.collection(tournament_core_1.TOURNAMENT_TASK_SECRETS_SUBCOLLECTION)
        .doc(BOT_SIMULATION_METADATA_DOC);
    return db.runTransaction(async (tx) => {
        const [roomSnap, userSnap, receiptSnap, authLinkSnap, bannedSnap, botMetadataSnap] = await tx.getAll(roomRef, userRef, receiptRef, authLinkRef, bannedRef, botMetadataRef);
        if (!roomSnap.exists)
            throw new https_1.HttpsError('not-found', 'room_not_found');
        assertTransactionalTournamentAccess(authUid, stableUid, authLinkSnap, userSnap, bannedSnap);
        if (receiptSnap.exists) {
            ...
            if (receipt.kind !== 'tournament_lobby_leave_v1' || receipt.roomId !== roomId
                ...
                throw new https_1.HttpsError('failed-precondition', 'tournament_leave_receipt_invalid');
            }
        ...
        if ((room.state !== 'scheduled' && room.state !== 'lobby')
            ...
            throw new https_1.HttpsError('failed-precondition', 'room_not_leaveable');
        }
        const player = room.players.find((candidate) => !candidate.isBot && candidate.id === stableUid);
        if (!player)
            throw new https_1.HttpsError('failed-precondition', 'not_in_room');
        ...
        const testMode = (0, tournament_core_1.isTournamentTestRoom)(room);
        const frozenEconomy = (0, tournament_core_1.tournamentEconomySnapshotForMode)(room.economySnapshot, testMode);
...
async function tournamentForfeitTransaction(db, input) {
    if (input.confirmed !== true) {
        throw new https_1.HttpsError('failed-precondition', 'forfeit_confirmation_required');
    }
    const { authUid, stableUid, roomId } = input;
    const nowMs = input.nowMs ?? Date.now();
    const roomRef = db.collection(tournament_core_1.TOURNAMENT_ROOMS_COLLECTION).doc(roomId);
    const userRef = db.collection('users').doc(stableUid);
    const receiptRef = userRef.collection(tournament_core_1.TOURNAMENT_RECEIPTS_SUBCOLLECTION).doc(`forfeit_${roomId}`);
    const authLinkRef = db.collection('auth_links').doc(authUid);
    const bannedRef = db.collection('banned_users').doc(stableUid);
    const botMetadataRef = roomRef.collection(tournament_core_1.TOURNAMENT_TASK_SECRETS_SUBCOLLECTION)
        .doc(BOT_SIMULATION_METADATA_DOC);
    return db.runTransaction(async (tx) => {
        ...
        if (receiptSnap.exists) {
            ...
            if (receipt.kind !== 'tournament_active_forfeit_v1' || receipt.roomId !== roomId
                ...
                throw new https_1.HttpsError('failed-precondition', 'tournament_forfeit_receipt_invalid');
            }
            return { ok: true, alreadyForfeited: true, roomId, refundedGems: 0, potGems };
        }
        const room = hydratePrivateBotMetadata(readRoom(roomSnap), botMetadataSnap.data());
        if (!/^(round[1-4]|table[1-3])$/.test(room.state)) {
            throw new https_1.HttpsError('failed-precondition', 'room_not_forfeitable');
        }
        ...
        if (room.players[playerIndex].forfeitedAtMs !== undefined) {
            throw new https_1.HttpsError('failed-precondition', 'tournament_forfeit_receipt_missing');
        }
        const players = room.players.map((player, index) => index === playerIndex ? {
            ...player,
            forfeitedAtMs: nowMs,
            forfeitState: room.state,
            streak: 0,
        } : player);
        const rounds = room.rounds.map((round) => {
            if (round.results?.[stableUid])
                return round;
            return {
                ...round,
                results: {
        ...
        const stateDeadlineAtMs = allRealResolved
            ? Math.min(room.stateDeadlineAtMs ?? Number.MAX_SAFE_INTEGER, nowMs + tournament_core_1.TOURNAMENT_EARLY_ADVANCE_DELAY_MS)
 ...
 */
async function createTournamentShardRoom(db, input) {
    ...
    if (shard <= 0) {
        ...
        if (!slot)
            return false;
        startsAt = (0, tournament_core_1.slotStartMs)(dateKey, slot.localTime, slot.timezone);
        baseTicketsRequired = slot.ticketsRequired;
    }
    else {
        // Базовая комната слота — источник правды по времени старта и цене входа.
        const baseRef = db.collection(tournament_core_1.TOURNAMENT_ROOMS_COLLECTION)
            .doc((0, tournament_core_1.tournamentRoomId)(slotId, timezone, dateKey, 0));
    ...
    if (startsAt <= 0 || nowMs >= startsAt)
        return false;
    const roomId = (0, tournament_core_1.tournamentRoomId)(slotId, timezone, dateKey, shard);
    const roomRef = db.collection(tournament_core_1.TOURNAMENT_ROOMS_COLLECTION).doc(roomId);
    ...
    const created = await db.runTransaction(async (tx) => {
        ...
        const botPlan = buildBotReservationPlan({
            ...
            roomId,
            count: tournament_core_1.TOURNAMENT_ROOM_SIZE - 1,
            gatherStartedAtMs: nowMs,
            recentBotIds: readRecentBotProfileIds(recentSnap.data()),
        });
        if (botPlan.players.length !== tournament_core_1.TOURNAMENT_ROOM_SIZE - 1) {
            throw new https_1.HttpsError('failed-precondition', 'not_enough_bots');
        }
        tx.create(roomRef, {
            economySnapshot,
            roomId,
        ...
        tx.create(roomRef.collection(tournament_core_1.TOURNAMENT_TASK_SECRETS_SUBCOLLECTION).doc(BOT_SIMULATION_METADATA_DOC), botPlan.metadata);
 ...
 */
async function ensureRoomPlayable(db, roomId) {
    const roomRef = db.collection(tournament_core_1.TOURNAMENT_ROOMS_COLLECTION).doc(roomId);
    const snap = await roomRef.get();
    if (!snap.exists)
        return; // Нет комнаты — её создаст ветка шардов ниже.
    const data = snap.data() || {};
    const state = sanitizeString(data.state, 32);
    // Наполняем только каркас: идущий/готовый турнир не трогаем.
    if (state !== 'scheduled')
        return;
    if (data.ready === true && Array.isArray(data.rounds) && data.rounds.length === 4)
        return;
    const startsAt = readInt(data.startsAt, 0);
    const nowMs = Date.now();
    if (startsAt <= 0 || nowMs >= startsAt)
        return; // Вход всё равно закрыт по времени.
    const resources = await loadResourcePool(db);
    const curated = await loadCuratedForRoom(db, roomId, resources.tasks);
    const roomPool = curated ? [...resources.tasks, ...curated.extraTasks] : resources.tasks;
    const rounds = buildRounds(roomId, roomPool, curated?.rounds);
    if (!rounds)
        throw new https_1.HttpsError('failed-precondition', 'no_published_ai_tasks');
    if (resources.bots.length < tournament_core_1.TOURNAMENT_ROOM_SIZE - 1) {
        throw new https_1.HttpsError('failed-precondition', 'not_enough_bots');
    }
    ...
    const filled = await db.runTransaction(async (tx) => {
        ...
        if (sanitizeString(current.state, 32) !== 'scheduled')
            return null;
        ...
        const botPlan = buildBotReservationPlan({
            profiles: resources.bots,
            roomId,
            count: Math.max(0, tournament_core_1.TOURNAMENT_ROOM_SIZE - currentPlayers.length),
            gatherStartedAtMs: nowMs,
            recentBotIds: readRecentBotProfileIds(recentSnap.data()),
            colorOffset: currentPlayers.length,
        });
        if (botPlan.players.length !== Math.max(0, tournament_core_1.TOURNAMENT_ROOM_SIZE - currentPlayers.length)) {
            throw new https_1.HttpsError('failed-precondition', 'not_enough_bots');
        }
        const taskPoolGeneration = await assertTournamentPoolCommitAllowed(tx, db, {
            generation: resources.taskPoolGeneration,
...
exports.tournamentJoin = (0, https_1.onCall)({ ...callable_options_1.HOT_CALLABLE_OPTIONS, enforceAppCheck: false }, async (request) => {
    if (!request.auth?.uid)
    ...
    const logJoinFailure = (error, where) => {
    ...
    let lastError = null;
    for (let shard = parsed.shard; shard < tournament_core_1.TOURNAMENT_MAX_SHARDS_PER_SLOT; shard += 1) {
        const shardRoomId = (0, tournament_core_1.tournamentRoomId)(parsed.slotId, parsed.timezone, parsed.dateKey, shard);
        try {
            return await tournamentJoinTransaction(db, { ...joinIdentity, roomId: shardRoomId });
        }
        catch (error) {
            const code = error instanceof https_1.HttpsError ? error.code : '';
            const message = error instanceof Error ? error.message : '';
            ...
            if (!isFull && !isMissing) {
                logJoinFailure(error, `shard${shard}`);
                throw error;
            }
            lastError = error;
            if (isMissing) {
    ...
    throw lastError instanceof https_1.HttpsError
        ? lastError
        : new https_1.HttpsError('resource-exhausted', 'all_shards_full');
});
exports.tournamentLeave = (0, https_1.onCall)(callable_options_1.HOT_CALLABLE_OPTIONS, async (request) => {
    if (!request.auth?.uid)
        throw new https_1.HttpsError('unauthenticated', 'auth_required');
    const roomId = sanitizeString(request.data?.roomId, 160);
    if (!roomId)
...
exports.tournamentForfeit = (0, https_1.onCall)(callable_options_1.HOT_CALLABLE_OPTIONS, async (request) => {
    if (!request.auth?.uid)
        throw new https_1.HttpsError('unauthenticated', 'auth_required');
    const roomId = sanitizeString(request.data?.roomId, 160);
    if (!roomId)
...
function cancellationTaskSecretRefs(roomRef, room, discoveredRefs = []) {
    ...
    if (authoredTaskIds.length > tournament_core_1.TOURNAMENT_TASK_LIMITS.maxTaskSecrets) {
...
async function cancelRoomInTransaction(db, tx, roomRef, room, reason, nowMs, discoveredSecretRefs = []) {
    ...
    for (const refund of plan.refunds) {
        ...
        const creditGems = (0, tournament_core_1.cancellationCreditGems)(refund);
        const userPatch = {};
        if (creditGems > 0) {
...
async function tournamentCancelTransaction(db, roomTarget, reason, requestedNowMs, expected) {
    ...
    const secretSnap = await roomRef.collection(tournament_core_1.TOURNAMENT_TASK_SECRETS_SUBCOLLECTION)
        .limit(tournament_core_1.TOURNAMENT_TASK_LIMITS.maxTaskSecrets + 2).get();
    const authoredSecretCount = secretSnap.docs
        .filter((doc) => doc.id !== BOT_SIMULATION_METADATA_DOC).length;
    if (authoredSecretCount > tournament_core_1.TOURNAMENT_TASK_LIMITS.maxTaskSecrets) {
        throw new https_1.HttpsError('failed-precondition', 'task_secret_cleanup_bounds_exceeded');
    }
    ...
    return db.runTransaction(async (tx) => {
        ...
        if (publicRoom.state === tournament_core_1.TOURNAMENT_STATE_CANCELLED)
            return true;
...
async function tournamentFillRoomTransaction(db, roomRef, resources, dependencies = {}) {
    ...
    return db.runTransaction(async (tx) => {
        ...
        if (nowMs < gatherStartedAtMs + tournament_core_1.TOURNAMENT_ROOM_GATHER_MS)
            return 'skip';
        ...
        const picked = (0, tournament_core_1.selectTournamentBotProfiles)({
            profiles: resources.bots,
        ...
        const botJoinTimes = (0, tournament_core_1.planBotJoinTimes)({
            seed: room.roomId,
            botCount: picked.length,
            fromMs: gatherStartedAtMs,
            startsAtMs: gatherStartedAtMs + tournament_core_1.TOURNAMENT_ROOM_GATHER_MS + tournament_core_1.TOURNAMENT_BOT_FILL_WINDOW_MS,
        });
        ...
        const frozenEconomy = (0, tournament_core_1.tournamentEconomySnapshotForMode)(room.economySnapshot, (0, tournament_core_1.isTournamentTestRoom)(room));
        ...
        const activationTasks = activationRound
            ? activationRound.taskIds.map((taskId) => taskMap.get(taskId)).filter((task) => !!task)
            : [];
        ...
        const startsRoundImmediately = room.state === 'lobby'
            && filledPlayers.length === tournament_core_1.TOURNAMENT_ROOM_SIZE
        ...
        const activationTiming = startsRoundImmediately
            ? (0, tournament_core_1.tournamentRoundTimingWindow)(activationTasks, nowMs)
            : undefined;
...
async function processTournamentFillRooms(options) {
    ...
    let filled = 0;
    let failed = 0;
    const cursorRef = db.collection(tournament_core_1.TOURNAMENT_SCHEDULE_COLLECTION).doc(FILL_CURSOR_DOC);
...
exports.tournamentFillBots = (0, scheduler_1.onSchedule)({ schedule: '* * * * *', timeZone: 'UTC', region: 'europe-west1', maxInstances: 1 }, async () => {
    const db = admin.firestore();
    await processTournamentFillRooms({ db, nowMs: Date.now() });
});
// ── Submit: the transaction recomputes from its fresh room snapshot. ───────────
function parseTournamentSubmissionIdempotencyKey(value) {
    if (value === undefined || value === null || value === '')
        return undefined;
    if (typeof value !== 'string' || !/^[A-Za-z0-9:_-]{8,160}$/.test(value)) {
        throw new https_1.HttpsError('invalid-argument', 'submission_idempotency_key_invalid');
    }
    return value;
}
function tournamentSubmissionIdempotencyKeyHash(key) {
    return key ? (0, node_crypto_1.createHash)('sha256').update(key).digest('hex') : undefined;
}
async function tournamentSpeedMatchAttemptTransaction(db, input) {
    const receivedAtMs = input.receivedAtMs ?? Date.now();
    const roomRef = db.collection(tournament_core_1.TOURNAMENT_ROOMS_COLLECTION).doc(input.roomId);
    const taskRef = roomRef.collection(tournament_core_1.TOURNAMENT_TASK_SECRETS_SUBCOLLECTION).doc(input.taskId);
    const progressRef = roomRef.collection(tournament_core_1.TOURNAMENT_TASK_SECRETS_SUBCOLLECTION)
    ...
    return db.runTransaction(async (tx) => {
        const [roomSnap, taskSnap, progressSnap, authLinkSnap, userSnap, bannedSnap] = await tx.getAll(roomRef, taskRef, progressRef, authLinkRef, userRef, bannedRef);
        if (!roomSnap.exists)
            throw new https_1.HttpsError('not-found', 'room_not_found');
        assertTransactionalTournamentAccess(input.authUid, input.stableUid, authLinkSnap, userSnap, bannedSnap);
        const room = readRoom(roomSnap);
        const activeState = (0, tournament_core_1.roundStateFor)(input.roundNo);
        ...
        const withinFollowingState = !!activeState && replacingTimedOut
            && room.state === (0, tournament_core_1.stateAfterTournamentDeadline)(activeState);
        if (!activeState || (room.state !== activeState && !withinFollowingState)
            || !round?.taskIds.includes(input.taskId)) {
            throw new https_1.HttpsError('failed-precondition', 'round_not_active');
        }
        try {
            const scheduleEnforced = (0, tournament_core_1.assertTournamentAnswersWithinTaskSchedule)(round, [input.taskId], receivedAtMs);
            if (!scheduleEnforced && room.stateDeadlineAtMs && receivedAtMs > room.stateDeadlineAtMs) {
                throw new Error('round_deadline_elapsed');
            }
        }
        catch (error) {
            throw new https_1.HttpsError('failed-precondition', error instanceof Error ? error.message : 'task_deadline_elapsed');
        }
        ...
        if (!task || task.mode !== 'speed_match') {
            throw new https_1.HttpsError('failed-precondition', 'speed_match_task_unavailable');
        }
        let outcome;
        try {
            outcome = (0, tournament_core_1.applySpeedMatchAttempt)(task, readSpeedMatchProgress(progressSnap.data()), input.pairIndex, input.selectedIndex);
        }
        catch {
            throw new https_1.HttpsError('invalid-argument', 'speed_match_attempt_invalid');
        }
        tx.set(progressRef, {
...
async function tournamentSubmitTaskAnswerTransaction(db, input) {
    const receivedAtMs = input.receivedAtMs ?? Date.now();
    const idempotencyKey = parseTournamentSubmissionIdempotencyKey(input.idempotencyKey);
    if (!idempotencyKey)
        throw new https_1.HttpsError('invalid-argument', 'submission_idempotency_key_required');
    ...
    return db.runTransaction(async (tx) => {
        ...
        const attemptRefs = round.taskIds.map((taskId) => roomRef
            .collection(tournament_core_1.TOURNAMENT_TASK_SECRETS_SUBCOLLECTION)
        ...
        const snapshots = await tx.getAll(...secretRefs, ...attemptRefs, ...receiptDescriptors.map((entry) => entry.ref), rankCounterRef, ...accessRefs);
        ...
        if (input.authUid) {
            assertTransactionalTournamentAccess(input.authUid, input.stableUid, snapshots[accessOffset], snapshots[accessOffset + 1], snapshots[accessOffset + 2]);
        }
        ...
        const existing = parsedReceipts.find((receipt) => (receipt.playerId === input.stableUid && receipt.taskId === input.taskId));
        if (existing && existing.idempotencyKeyHash !== idempotencyKeyHash) {
            throw new https_1.HttpsError('already-exists', 'task_answer_idempotency_key_mismatch');
        }
        const activeState = (0, tournament_core_1.roundStateFor)(input.roundNo);
        ...
        const answer = existing?.answer ?? (task.mode === 'speed_match'
            ...
            : input.answer);
        const correct = existing?.correct ?? (0, tournament_core_1.verifyTournamentAnswer)(task, answer);
        const currentReceipt = existing ?? {
        ...
        const timelyPlayerReceipts = allReceipts.filter((receipt) => (receipt.playerId === input.stableUid
        ...
        const storedRankCounter = readTournamentTaskRankCounter(snapshots[rankCounterOffset].data());
        ...
        for (const receipt of allReceipts) {
            if (receipt.playerId !== input.stableUid || !receipt.correct)
                continue;
            const rank = receipt.taskId === input.taskId ? counterRank : receipt.answerRank;
            if (rank !== undefined)
                myAnswerRanks[receipt.taskId] = rank;
        }
        const answerRanksByPlayer = { [input.stableUid]: myAnswerRanks };
        if (!existing) {
            tx.create(receiptDescriptors.find((entry) => (entry.playerId === input.stableUid && entry.taskId === input.taskId)).ref, {
        ...
        if (hasCompleteReceiptSet) {
            const speedMatchProgress = {};
            round.taskIds.forEach((taskId, index) => {
                const taskProgress = readSpeedMatchProgress(snapshots[attemptOffset + index].data());
                if (taskProgress)
                    speedMatchProgress[taskId] = taskProgress;
            });
            const taskReceivedAtMs = Object.fromEntries(round.taskIds.map((taskId) => [taskId, receiptByTask.get(taskId).receivedAtMs]));
            ...
            try {
                applied = (0, tournament_core_1.applyTournamentSubmission)(room, {
                    ...
                    taskReceivedAtMs,
                    maxMsPerTask: Math.max(1, Math.ceil((0, tournament_core_1.tournamentRoundDurationMs)(tasks) / Math.max(1, tasks.length))),
                    speedMatchProgress,
                    answerRanksByPlayer,
                });
            }
            ...
            if (!applied.replay) {
                const earlyAdvanceDeadlineAtMs = applied.allRealSubmitted && room.state === activeState
                    ? Math.min(room.stateDeadlineAtMs ?? Number.MAX_SAFE_INTEGER, finalizedAtMs + tournament_core_1.TOURNAMENT_EARLY_ADVANCE_DELAY_MS)
                    : undefined;
                tx.set(roomRef, {
        ...
        const earnedStars = (0, tournament_core_1.scoreAnswer)({
        ...
        const selectedIndex = answer && typeof answer === 'object'
            ? readInt(answer.selectedIndex, -1)
            : -1;
        ...
        const correctIndex = Number.isInteger(task.payload.correctIndex)
            ? Number(task.payload.correctIndex)
            : undefined;
        return {
            ok: true,
            replay: !!existing,
...
async function tournamentSubmitTransaction(db, input) {
    ...
    return db.runTransaction(async (tx) => {
        ...
        const receiptDescriptors = tournamentReceiptRefsForRound(roomRef, initialRound, initialRealPlayerIds);
        const accessRefs = input.authUid ? [
            db.collection('auth_links').doc(input.authUid),
            db.collection('users').doc(stableUid),
            db.collection('banned_users').doc(stableUid),
        ] : [];
        const snapshots = await tx.getAll(roomRef, ...secretRefs, ...attemptRefs, ...receiptDescriptors.map((entry) => entry.ref), ...accessRefs);
        const roomSnap = snapshots[0];
        if (!roomSnap.exists)
            throw new https_1.HttpsError('not-found', 'room_not_found');
        if (input.authUid) {
            const accessOffset = 1 + secretRefs.length + attemptRefs.length + receiptDescriptors.length;
            assertTransactionalTournamentAccess(input.authUid, stableUid, snapshots[accessOffset], snapshots[accessOffset + 1], snapshots[accessOffset + 2]);
        }
        const room = readRoom(roomSnap);
        ...
        const receipts = initialRound.taskIds.map((taskId) => allReceipts.find((receipt) => (receipt.playerId === stableUid && receipt.taskId === taskId)));
        ...
        const answers = initialRound.taskIds.flatMap((taskId, index) => {
            ...
            if (receipt)
                return [{ taskId, answer: receipt.answer }];
        ...
        if (existing && !existingTimedOut) {
            try {
                const finalTaskId = round.taskIds[round.taskIds.length - 1];
                (0, tournament_core_1.assertTournamentAnswersWithinTaskSchedule)(round, [finalTaskId], receivedAtMs);
            }
            catch (error) {
                throw new https_1.HttpsError('failed-precondition', error instanceof Error ? error.message : 'task_deadline_elapsed');
            }
            if (idempotencyKeyHash && existing.submissionIdempotencyKeyHash
                ...
                throw new https_1.HttpsError('already-exists', 'submission_idempotency_key_mismatch');
            }
        ...
        if (!sameTaskIdSnapshot(realPlayerIds, initialRealPlayerIds)) {
            throw new https_1.HttpsError('aborted', 'room_players_changed_retry');
        }
        ...
        snapshots.slice(1 + secretRefs.length, 1 + secretRefs.length + attemptRefs.length)
            ...
            if (progress)
                speedMatchProgress[initialRound.taskIds[index]] = progress;
        });
        const syntheticReceipts = answers.flatMap((entry) => {
            if (receipts.some((receipt) => receipt?.taskId === entry.taskId))
                return [];
            ...
            return [{
                    ...
                    playerId: stableUid,
                    receivedAtMs,
                    idempotencyKeyHash: idempotencyKeyHash ?? (0, node_crypto_1.createHash)('sha256')
                        .update(`legacy:${roomId}:${roundNo}:${stableUid}:${entry.taskId}:${receivedAtMs}`)
                        .digest('hex'),
                    answer: entry.answer,
                    correct: (0, tournament_core_1.verifyTournamentAnswer)(task, entry.answer),
                }];
        });
        const answerRanksByPlayer = canonicalTournamentReceiptRanks(round, tasks, realPlayerIds, [...allReceipts, ...syntheticReceipts], room.stateDeadlineAtMs);
        let applied;
        try {
            const roundDurationMs = (0, tournament_core_1.tournamentRoundDurationMs)(tasks);
            applied = (0, tournament_core_1.applyTournamentSubmission)(room, {
                playerId: stableUid,
                roundNo,
                answers,
        ...
        const earlyAdvanceDeadlineAtMs = applied.allRealSubmitted
            ? Math.min(room.stateDeadlineAtMs ?? Number.MAX_SAFE_INTEGER, receivedAtMs + tournament_core_1.TOURNAMENT_EARLY_ADVANCE_DELAY_MS)
            : undefined;
        tx.set(roomRef, {
            players: applied.room.players.map(publicTournamentPlayer),
            rounds: publicTournamentRounds(applied.room.rounds),
...
exports.tournamentSubmitSpeedMatchAttempt = (0, https_1.onCall)(callable_options_1.HOT_CALLABLE_OPTIONS, async (request) => {
    if (!request.auth?.uid)
        throw new https_1.HttpsError('unauthenticated', 'auth_required');
    const db = admin.firestore();
...
exports.tournamentSubmitTaskAnswer = (0, https_1.onCall)(callable_options_1.HOT_CALLABLE_OPTIONS, async (request) => {
    if (!request.auth?.uid)
        throw new https_1.HttpsError('unauthenticated', 'auth_required');
    const roomId = sanitizeString(request.data?.roomId, 160);
    const taskId = sanitizeString(request.data?.taskId, 160);
    ...
    if (!roomId || !taskId || roundNo < 1 || roundNo > 4
        ...
        throw new https_1.HttpsError('invalid-argument', 'task_answer_required');
    }
    const db = admin.firestore();
    const stableUid = await resolveStableUid(db, request.auth.uid);
...
async function tournamentFinalizeTransaction(db, roomId, requestedNowMs) {
    ...
    return db.runTransaction(async (tx) => {
        ...
        for (const effect of plan.playerEffects) {
            ...
            const seasonName = (0, tournament_core_1.resolveSeasonEntryName)(roomPlayer?.name, season.name);
            ...
            const profileXp = Math.max(0, readInt(user.progress?.total_xp, 0));
        ...
        tx.set(roomRef, {
            ...
            featureGates: (0, tournament_core_1.tournamentFeatureGates)(),
            expireAt: admin.firestore.Timestamp.fromMillis(plan.room.privateEvidenceRetentionUntilMs ?? nowMs + tournament_core_1.TOURNAMENT_ROOM_TTL_MS),
            updatedAt: nowMs,
        }, { merge: true });
        return { ok: true };
    });
...
async function advanceRoomAtDeadline(db, roomRef, dependencies = {}) {
    ...
    return db.runTransaction(async (tx) => {
        ...
        const snapshots = await tx.getAll(roomRef, botMetadataRef, ...deadlineReceiptDescriptors.map((entry) => entry.ref), ...deadlineAttemptDescriptors.map((entry) => entry.ref), ...accessRefs);
        ...
        if (dependencies.requesterAuthUid && dependencies.requesterStableUid) {
            assertTransactionalTournamentAccess(dependencies.requesterAuthUid, dependencies.requesterStableUid, snapshots[accessOffset], snapshots[accessOffset + 1], snapshots[accessOffset + 2]);
        }
        ...
        if (room.state === 'lobby') {
            ...
            if (!tournamentPlayersHaveArrived(room.players, nowMs))
                return 'waiting';
            const roundTiming = (0, tournament_core_1.tournamentRoundTimingWindow)(activationTasks, nowMs);
            tx.set(roomRef, {
                state: 'round1',
                stateStartedAtMs: nowMs,
                introEndsAtMs: roundTiming.introEndsAtMs,
                stateDeadlineAtMs: roundTiming.stateDeadlineAtMs,
                rounds: roundsWithActivatedPublicTasks(room.rounds, activationRound.roundNo, activationTasks, nowMs, room.roomId),
                lifecycleRetryAtMs: admin.firestore.FieldValue.delete(),
                version: room.version + 1,
                updatedAt: nowMs,
            }, { merge: true });
        ...
        if (/^round[1-4]$/.test(room.state)) {
            ...
            if (!currentActiveRound || !activeRound
                || !sameTaskIdSnapshot(realPlayerIds, activeRealPlayerIds))
                return 'stale';
            const deadlineRound = currentActiveRound;
            let receiptSubmissions;
            try {
                ...
                const answerRanksByPlayer = canonicalTournamentReceiptRanks(deadlineRound, tasks, realPlayerIds, allReceipts, transitionAtMs);
                receiptSubmissions = Object.fromEntries(realPlayerIds.flatMap((playerId) => {
                    const timelyReceipts = allReceipts.filter((receipt) => (receipt.playerId === playerId
                        && tournamentReceiptIsTimely(deadlineRound, receipt, transitionAtMs)));
                    if (timelyReceipts.length === 0)
                        return [];
                    const speedMatchProgress = {};
                    deadlineAttemptDescriptors.forEach((descriptor, index) => {
                        if (descriptor.playerId !== playerId)
                            return;
            ...
            let completed;
            try {
                completed = (0, tournament_core_1.completeTournamentRoundAtDeadline)(room, tasks, transitionAtMs, receiptSubmissions);
            }
            catch {
                return 'cancel_resources';
            }
            const completedStateDuration = (0, tournament_core_1.stateDeadlineDurationMs)(completed.room.state, 0);
        ...
        if (room.state === 'rewards') {
            ...
            const reviewRetentionUntilMs = room.reviewRetentionUntilMs
                ?? finalizedAtMs + exports.TOURNAMENT_REVIEW_RETENTION_MS;
            const privateEvidenceRetentionUntilMs = room.privateEvidenceRetentionUntilMs
                ?? finalizedAtMs + exports.TOURNAMENT_PRIVATE_EVIDENCE_RETENTION_MS;
            const evidenceExpireAt = admin.firestore.Timestamp.fromMillis(privateEvidenceRetentionUntilMs);
        ...
        const roundTiming = nextRound
            ? (0, tournament_core_1.tournamentRoundTimingWindow)(activationTasks, nowMs)
            : undefined;
        const duration = (0, tournament_core_1.stateDeadlineDurationMs)(nextState, 0);
        tx.set(roomRef, {
            state: nextState,
            stateStartedAtMs: nowMs,
            ...
            ...(nextRound ? {
                rounds: roundsWithActivatedPublicTasks(room.rounds, nextRound.roundNo, activationTasks, nowMs, room.roomId),
            } : {}),
            lifecycleRetryAtMs: admin.firestore.FieldValue.delete(),
            version: room.version + 1,
            updatedAt: nowMs,
...
async function catchUpRoomAtDeadline(db, roomRef, dependencies = {}) {
    ...
    for (let transition = 0; transition < ACTIVE_DEADLINE_STATES.length + 2; transition += 1) {
        ...
        if (settled === 'cancelled')
            return settled;
        if (settled !== 'advanced')
            return progressed ? 'advanced' : settled;
        progressed = true;
        const current = await roomRef.get();
        if (!current.exists)
            return 'advanced';
        const data = current.data() || {};
        const state = sanitizeString(data.state, 20);
 ...
 */
async function cleanupExpiredTournamentReviewEvidence(db, options = {}) {
    const nowMs = options.nowMs ?? Date.now();
    const limit = Math.max(1, Math.min(400, Math.trunc(options.limit ?? 400)));
    const expired = await db.collectionGroup(tournament_core_1.TOURNAMENT_TASK_SECRETS_SUBCOLLECTION)
    ...
    for (const secret of expired.docs)
        batch.delete(secret.ref);
    await batch.commit();
    return { scanned: expired.size, deleted: expired.size };
}
exports.tournamentAdvanceRooms = (0, scheduler_1.onSchedule)({ schedule: '* * * * *', timeZone: 'UTC', region: 'europe-west1', maxInstances: 1 }, async () => {
    const db = admin.firestore();
    const nowMs = Date.now();
 ...
 */
exports.tournamentAdvanceRound = (0, https_1.onCall)(
...
{ ...callable_options_1.HOT_CALLABLE_OPTIONS, enforceAppCheck: false, minInstances: 1 }, async (request) => {
    if (!request.auth?.uid)
        throw new https_1.HttpsError('unauthenticated', 'auth_required');
    const roomId = sanitizeString(request.data?.roomId, 160);
...
exports.tournamentStartNow = (0, https_1.onCall)({
    ...
    try {
        ...
        const db = admin.firestore();
        const nowMs = Date.now();
        const { config, stableUid, resources, economy: economySnap } = await loadTournamentStartNowPrerequisites({
            ...
            loadEconomy: () => db.collection(tournament_core_1.TOURNAMENT_SCHEDULE_COLLECTION).doc('economy').get(),
        });
        const economySnapshot = (0, tournament_core_1.tournamentEconomySnapshotForMode)(economySnap.data(), true);
        ...
        const startsAt = nowMs + tournament_core_1.TOURNAMENT_ROOM_GATHER_MS + tournament_core_1.TOURNAMENT_BOT_FILL_WINDOW_MS;
        const dateKey = (0, tournament_core_1.dateKeyInTimezone)(nowMs, slot.timezone);
        // Своя метка времени в slotId: комната не конфликтует с комнатой расписания
        // и не занимает игроку его штатный слот.
        const roomId = (0, tournament_core_1.tournamentRoomId)(`now-${slot.slotId}-${nowMs}`, slot.timezone, dateKey);
        ...
        const recentRef = recentBotRosterRef(db);
        const userRef = db.collection('users').doc(stableUid);
        const configRef = db.collection(tournament_core_1.TOURNAMENT_SCHEDULE_COLLECTION)
            .doc(tournament_core_1.TOURNAMENT_SCHEDULE_CONFIG_DOC);
        const economyRef = db.collection(tournament_core_1.TOURNAMENT_SCHEDULE_COLLECTION).doc('economy');
        const authLinkRef = db.collection('auth_links').doc(authUid);
        const bannedRef = db.collection('banned_users').doc(stableUid);
        const leaderboardRef = db.collection('leaderboard').doc(stableUid);
        ...
        const joined = await db.runTransaction(async (tx) => {
            const [recentSnap, userSnap, currentEconomySnap, currentConfigSnap, authLinkSnap, bannedSnap, leaderboardSnap, publicProfileSnap] = await tx.getAll(recentRef, userRef, economyRef, configRef, authLinkRef, bannedRef, leaderboardRef, publicProfileRef);
            assertTransactionalTournamentAccess(authUid, stableUid, authLinkSnap, userSnap, bannedSnap);
            const taskPoolGeneration = await assertTournamentPoolCommitAllowed(tx, db, {
                generation: resources.taskPoolGeneration,
                revision: resources.taskPoolRevision,
            ...
            const joinedPlan = planTournamentStartNowJoinedRoom({
                ...
                config: currentConfigSnap.exists
                    ? (0, tournament_core_1.normalizeTournamentSchedule)(currentConfigSnap.data())
                    : (0, tournament_core_1.normalizeTournamentSchedule)(null),
            ...
            tx.create(roomRef.collection(tournament_core_1.TOURNAMENT_TASK_SECRETS_SUBCOLLECTION).doc(BOT_SIMULATION_METADATA_DOC), joinedPlan.botMetadata);
            tx.set(recentRef, recentBotRosterData(roomId, botPlan.selectedProfileIds, nowMs));
        ...
        if (!(error instanceof https_1.HttpsError)) {
            console.error('[tournaments] tournamentStartNow internal failure', tournamentStartNowInternalErrorLog(error));
        }
        throw error;
    }
});
 ...
 */
exports.tournamentRoundReview = (0, https_1.onCall)(callable_options_1.HOT_CALLABLE_OPTIONS, async (request) => {
    const uid = request.auth?.uid;
    if (!uid)
        throw new https_1.HttpsError('unauthenticated', 'auth_required');
    const roomId = sanitizeString(request.data?.roomId, 140);
    if (!roomId)
        throw new https_1.HttpsError('invalid-argument', 'room_required');
    const db = admin.firestore();
    const roomRef = db.collection(tournament_core_1.TOURNAMENT_ROOMS_COLLECTION).doc(roomId);
    ...
    if (!['final', 'results', 'rewards', 'closed'].includes(room.state)) {
        throw new https_1.HttpsError('failed-precondition', 'tournament_not_finished');
    }
    if (room.state === 'rewards' || room.state === 'closed') {
        const legacyFinalizedAtMs = room.finalizedAtMs ?? room.stateStartedAtMs;
        const reviewRetentionUntilMs = room.reviewRetentionUntilMs
            ?? (legacyFinalizedAtMs ? legacyFinalizedAtMs + exports.TOURNAMENT_REVIEW_RETENTION_MS : 0);
        if (reviewRetentionUntilMs <= 0 || Date.now() >= reviewRetentionUntilMs) {
            throw new https_1.HttpsError('failed-precondition', 'tournament_review_expired');
        }
    }
    ...
    if (!player)
        throw new https_1.HttpsError('permission-denied', 'not_a_participant');
    ...
    const secretSnaps = await db.getAll(...uniqueIds.map((taskId) => roomRef.collection(tournament_core_1.TOURNAMENT_TASK_SECRETS_SUBCOLLECTION).doc(taskId)));
    const secrets = new Map();
    for (const snap of secretSnaps) {
        if (snap.exists)
            secrets.set(snap.id, snap.data() || {});
    }
    if (secrets.size !== uniqueIds.length) {
        throw new https_1.HttpsError('failed-precondition', 'tournament_review_evidence_unavailable');
    }
    const items = reviewed.map((entry) => {
        ...
        const reviewExplanation = (raw) => {
            if (!raw || typeof raw !== 'object')
                return null;
            const wrongOptionReasons = Array.isArray(raw.wrongOptionReasons)
                ? raw.wrongOptionReasons.map((reason) => sanitizeString(reason, 600))
                : undefined;
            return {
                ruleNote: sanitizeString(raw.ruleNote, 600),
                example: sanitizeString(raw.example, 600),
        ...
        const aggregateItems = task?.mode === 'time_attack'
            ...
            ? payload.items.map((rawItem, index) => {
                const part = rawItem && typeof rawItem === 'object' ? rawItem : {};
