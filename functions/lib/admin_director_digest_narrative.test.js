...
describe('director digest V2 narrative', () => {
    test('builds a versioned aggregate-only prompt with the current DigestNarrative schema', () => {
        ...
        const built = (0, admin_director_digest_narrative_1.buildDirectorDigestNarrativePrompt)(unsafe);
    ...
    test('accepts only known metric ids and rebuilds facts and comparisons on the server', () => {
        const parsed = (0, admin_director_digest_narrative_1.parseDirectorDigestNarrative)(JSON.stringify({
        ...
        expect(parsed.narrative.sourceWarnings).toContain('Воронка предложения Plus: данные неполные.');
    });
    ...
    test('returns a deterministic facts-only narrative for invalid model output', () => {
        const parsed = (0, admin_director_digest_narrative_1.parseDirectorDigestNarrative)('not json', input());
        expect(parsed.state).toBe('fallback');
    ...
    test('rejects an incomplete generated narrative and never turns unavailable into zero', () => {
        ...
        const refundInsight = parsed.narrative.productManager.find((item) => item.metricIds.includes('store.refund.v1'));
    ...
    test('uses only aggregate metric statements in deterministic user voice', () => {
        const parsed = (0, admin_director_digest_narrative_1.parseDirectorDigestNarrative)('invalid', input());
...
//# sourceMappingURL=admin_director_digest_narrative.test.js.map