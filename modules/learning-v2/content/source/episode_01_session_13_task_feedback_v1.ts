...
const LISTEN_FEEDBACK: Readonly<Record<string, LocalizedSource>> =
  Object.freeze({
    "You’re busy.\u0000You’re tired.": L({
      ...
      "pt-BR":
        "You’re tired. fala de cansaço e falta de energia, mas uma pessoa cansada pode ter tempo livre. Aqui se ouve busy, que indica ocupação, então a frase exata é You’re busy.",
    ...
    "You’re busy.\u0000You’re happy.": L({
      ...
      "pt-BR":
        "You’re happy. descreve felicidade e não indica se a pessoa tem tempo. Aqui se fala das tarefas e se ouve busy, portanto a opção correta é You’re busy.",
    ...
    "You’re calm.\u0000You’re busy.": L({
      ...
      "pt-BR":
        "You’re busy. indica que alguém tem muitas tarefas. Estar ocupado não impede estar calmo; aqui se ouve calm e se avalia o estado interior, então a resposta é You’re calm.",
    ...
    "You’re calm.\u0000You’re tired.": L({
      ...
      "pt-BR":
        "You’re tired. refere-se ao cansaço físico ou mental. O cansaço não determina a tensão interior; aqui se ouve calm, «calmo», então a resposta é You’re calm.",
...
export function episode01Session13ListenFeedbackV1(
  locale: Locale,
  correct: string,
  wrong: string,
): string | undefined {
...
const YOUR_FEEDBACK: Readonly<Record<Locale, (target: string) => string>> =
  Object.freeze({
    ru: (target) =>
      `Your звучит так же, как You’re, но это притяжательное слово: после него нужна вещь, например your book. В «${target}» после начала идёт состояние или место, поэтому требуется You’re = you are.`,
    uk: (target) =>
      `Your звучить так само, як You’re, але це присвійне слово: після нього потрібна річ, наприклад your book. У «${target}» після початку йде стан або місце, тому потрібне You’re = you are.`,
    es: (target) =>
      `Your suena igual que You’re, pero es posesivo y necesita una cosa después, como en your book. En «${target}» sigue un estado o lugar, así que corresponde You’re = you are.`,
    "pt-BR": (target) =>
      `Your tem o mesmo som de You’re, mas é possessivo e precisa de uma coisa depois, como em your book. Em «${target}» vem um estado ou lugar, então é necessário You’re = you are.`,
    vi: (target) =>
      `Your phát âm giống You’re nhưng là từ sở hữu và phải có tên một vật phía sau, như your book. Trong “${target}”, phần sau là trạng thái hoặc nơi chốn nên cần You’re = you are.`,
    id: (target) =>
      `Your terdengar sama dengan You’re, tetapi menyatakan kepemilikan dan harus diikuti benda, seperti your book. Dalam “${target}” sesudahnya ada keadaan atau tempat, jadi bentuknya You’re = you are.`,
    tr: (target) =>
      `Your, You’re ile aynı duyulur; ancak sahiplik bildirir ve ardından your book gibi bir nesne ister. «${target}» içinde ardından durum ya da yer geldiği için You’re = you are gerekir.`,
    pl: (target) =>
      `Your brzmi tak samo jak You’re, ale oznacza przynależność i wymaga potem rzeczy, jak w your book. W „${target}” dalej jest stan albo miejsce, więc potrzebne jest You’re = you are.`,
  });

const YOU_FEEDBACK: Readonly<Record<Locale, (target: string) => string>> =
  Object.freeze({
    ru: (target) =>
      `You правильно называет собеседника, но само по себе не связывает его с признаком или местом. В «${target}» нужна связка are; сокращение You’re сохраняет её, а вариант с одним You оставляет связку пропущенной.`,
    uk: (target) =>
      `You правильно називає співрозмовника, але саме не пов’язує його з ознакою чи місцем. У «${target}» потрібна зв’язка are; скорочення You’re зберігає її, а варіант з одним You лишає зв’язку пропущеною.`,
    es: (target) =>
      `You identifica correctamente al interlocutor, pero por sí solo no lo une con una cualidad o lugar. En «${target}» hace falta are; You’re la conserva y el You aislado deja el enlace ausente.`,
    "pt-BR": (target) =>
      `You identifica corretamente o interlocutor, mas sozinho não o liga a uma característica ou lugar. Em «${target}» é preciso are; You’re preserva essa ligação e o You isolado a deixa ausente.`,
    vi: (target) =>
      `You gọi đúng người nghe nhưng một mình nó không nối người ấy với đặc điểm hoặc nơi chốn. Trong “${target}” cần are; You’re giữ từ nối này, còn chỉ có You thì câu bị thiếu liên kết.`,
    id: (target) =>
      `You menyebut lawan bicara dengan benar, tetapi sendirian tidak menghubungkannya dengan sifat atau tempat. Dalam “${target}” diperlukan are; You’re menyimpannya, sedangkan You saja menghilangkan penghubung.`,
    tr: (target) =>
      `You doğru kişiyi gösterir, fakat tek başına onu özellik ya da yerle bağlamaz. «${target}» içinde are bağı gerekir; You’re bu bağı korur, yalnız You ise bağı eksik bırakır.`,
    pl: (target) =>
      `You poprawnie wskazuje rozmówcę, lecz samo nie łączy go z cechą ani miejscem. W „${target}” potrzebne jest are; You’re je zachowuje, a samo You pozostawia brak łącznika.`,
  });

export function episode01Session13FormFeedbackV1(
  locale: Locale,
  target: string,
  wrong: string,
): string | undefined {
  if (!/^You’re\b/u.test(target)) return undefined;
  if (wrong === "Your") return YOUR_FEEDBACK[locale](target);
  if (wrong === "You") return YOU_FEEDBACK[locale](target);
  return undefined;
}
