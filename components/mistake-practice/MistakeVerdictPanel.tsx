...
const copyFor = (lang: Lang) => triLang(lang, {
  ru: { right: 'Верно', rightFree: 'Верно, и без подсказок', missing: 'Не хватило одного слова', order: 'Слова верные, порядок — нет', wrong: 'Нужно поправить', mine: 'ТЫ', correctLabel: 'ВЕРНО', why: 'Почему так', simpler: 'Объяснить проще', day: 'День', fixed: 'Исправлено', oneMore: 'Ещё один верный день в другом режиме — и ошибка уйдёт навсегда.', done: 'Эта ошибка исправлена навсегда.', stop: 'Повторим эту фразу завтра: три попытки подряд утомляют, а не учат.', thinking: 'Разбираю твой ответ' },
  uk: { right: 'Правильно', rightFree: 'Правильно, і без підказок', missing: 'Забракло одного слова', order: 'Слова правильні, порядок — ні', wrong: 'Потрібно виправити', mine: 'ТИ', correctLabel: 'ПРАВИЛЬНО', why: 'Чому так', simpler: 'Пояснити простіше', day: 'День', fixed: 'Виправлено', oneMore: 'Ще один правильний день в іншому режимі — і помилка зникне назавжди.', done: 'Цю помилку виправлено назавжди.', stop: 'Повторимо цю фразу завтра: три спроби поспіль втомлюють, а не вчать.', thinking: 'Розбираю твою відповідь' },
  en: { right: 'Correct', rightFree: 'Correct, with no hints', missing: 'One word was missing', order: 'Right words, wrong order', wrong: 'Needs fixing', mine: 'YOU', correctLabel: 'CORRECT', why: 'Why', simpler: 'Explain more simply', day: 'Day', fixed: 'Fixed', oneMore: 'One more correct day in a different mode and this mistake is gone for good.', done: 'This mistake is fixed for good.', stop: "We'll come back to this phrase tomorrow: three tries in a row tire you out rather than teach.", thinking: 'Looking at your answer' },
  es: { right: 'Correcto', rightFree: 'Correcto, y sin pistas', missing: 'Faltó una palabra', order: 'Palabras correctas, orden no', wrong: 'Hay que corregirlo', mine: 'TÚ', correctLabel: 'CORRECTO', why: 'Por qué', simpler: 'Explicar más simple', day: 'Día', fixed: 'Corregido', oneMore: 'Un día correcto más en otro modo y este error desaparece para siempre.', done: 'Este error está corregido para siempre.', stop: 'Volveremos a esta frase mañana: tres intentos seguidos cansan, no enseñan.', thinking: 'Analizando tu respuesta' },
  'pt-BR': { right: 'Correto', rightFree: 'Correto, e sem dicas', missing: 'Faltou uma palavra', order: 'Palavras certas, ordem errada', wrong: 'Precisa corrigir', mine: 'VOCÊ', correctLabel: 'CORRETO', why: 'Por que', simpler: 'Explicar mais simples', day: 'Dia', fixed: 'Corrigido', oneMore: 'Mais um dia correto em outro modo e este erro some para sempre.', done: 'Este erro está corrigido para sempre.', stop: 'Voltamos a esta frase amanhã: três tentativas seguidas cansam, não ensinam.', thinking: 'Analisando sua resposta' },
  vi: { right: 'Đúng', rightFree: 'Đúng, không cần gợi ý', missing: 'Thiếu một từ', order: 'Đúng từ, sai trật tự', wrong: 'Cần sửa lại', mine: 'BẠN', correctLabel: 'ĐÚNG', why: 'Vì sao', simpler: 'Giải thích đơn giản hơn', day: 'Ngày', fixed: 'Đã sửa', oneMore: 'Thêm một ngày đúng ở chế độ khác là lỗi này biến mất hẳn.', done: 'Lỗi này đã được sửa hẳn.', stop: 'Ta sẽ quay lại câu này vào ngày mai: ba lần liên tiếp làm mệt chứ không giúp học.', thinking: 'Đang xem câu trả lời của bạn' },
  id: { right: 'Benar', rightFree: 'Benar, tanpa petunjuk', missing: 'Kurang satu kata', order: 'Kata benar, urutan salah', wrong: 'Perlu diperbaiki', mine: 'KAMU', correctLabel: 'BENAR', why: 'Kenapa', simpler: 'Jelaskan lebih sederhana', day: 'Hari', fixed: 'Diperbaiki', oneMore: 'Satu hari benar lagi di mode lain dan kesalahan ini hilang selamanya.', done: 'Kesalahan ini sudah diperbaiki selamanya.', stop: 'Kita ulang frasa ini besok: tiga kali berturut-turut melelahkan, bukan mengajarkan.', thinking: 'Melihat jawabanmu' },
  tr: { right: 'Doğru', rightFree: 'Doğru, hem de ipucusuz', missing: 'Bir kelime eksikti', order: 'Kelimeler doğru, sıra yanlış', wrong: 'Düzeltmek gerek', mine: 'SEN', correctLabel: 'DOĞRU', why: 'Neden', simpler: 'Daha basit anlat', day: 'Gün', fixed: 'Düzeltildi', oneMore: 'Başka bir modda bir doğru gün daha ve bu hata tamamen gider.', done: 'Bu hata kalıcı olarak düzeltildi.', stop: 'Bu ifadeye yarın döneriz: üst üste üç deneme öğretmez, yorar.', thinking: 'Cevabına bakıyorum' },
  pl: { right: 'Dobrze', rightFree: 'Dobrze, i bez podpowiedzi', missing: 'Zabrakło jednego słowa', order: 'Słowa dobre, kolejność nie', wrong: 'Trzeba poprawić', mine: 'TY', correctLabel: 'POPRAWNIE', why: 'Dlaczego', simpler: 'Wyjaśnij prościej', day: 'Dzień', fixed: 'Poprawione', oneMore: 'Jeszcze jeden dobry dzień w innym trybie i ten błąd zniknie na zawsze.', done: 'Ten błąd jest poprawiony na zawsze.', stop: 'Wrócimy do tej frazy jutro: trzy próby pod rząd męczą, a nie uczą.', thinking: 'Analizuję twoją odpowiedź' },
});
      ...
      {chain ? (
        <View style={styles.chain}>
          ...
          <Text style={[styles.chainText, { color: t.textPrimary, fontSize: f.sub }]}>
            {chain.corrected ? copy.done : copy.oneMore}
          </Text>
        </View>
      ) : null}
