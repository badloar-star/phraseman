 ...
 *               сразу, плитка в «Подарках» при успехе НЕ создаётся.
 * `inventory` — энергия, расходники с длительностью и косметика: ждут тапа
 *               владельца в разделе «Подарки», как и раньше.
 *
 * Важно про сбой: канал НЕ управляет тем, что показывает инвентарь.
 * `loadPendingLevelGiftInventory` отбирает occurrence по `claimed !== true`
 * независимо от канала, поэтому не доставшийся instant-приз (упало хранилище,
 * сменился аккаунт) останется в журнале и появится плиткой в «Подарках».
 * Награда не теряется ни в одном сценарии — это страховка, а не недосмотр.
 */
