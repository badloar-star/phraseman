...
import { readImmutableCanonicalObject } from "../content_studio/immutable_object_reader";
import {
...
import {
  V2_VOICE_PROFILE_REPOSITORY_NAMESPACE_V1,
...
export const V2_OWNER_GENERATOR_SETUP_CATALOG_SCHEMA_V1 =
  "v2-owner-generator-setup-catalog.v1" as const;
...
export interface V2OwnerGeneratorSetupCatalogDependenciesV1 {
  readonly authenticateOwner?: typeof requireV2ConfiguredRootOwnerV1;
  readonly listDocuments: (
    collection: string,
    maximum: number,
  ) => Promise<readonly Readonly<{ id: string; data: JsonRecord }>[]>;
  readonly readDocument: (path: string) => Promise<JsonRecord | null>;
  readonly readCanonicalObject: (input: {
...
function identity(value: JsonRecord, idKey: "profileId" | "templateId") {
...
function objectPin(value: JsonRecord) {
...
async function languageEntries(
  targetLanguage: string,
  dependencies: V2OwnerGeneratorSetupCatalogDependenciesV1,
) {
  ...
  for (const row of rows) {
    try {
      ...
      if (
        `${LANGUAGE_LIFECYCLE}/${row.id}` !==
        v2LanguageProfileLifecycleDocumentPathV1(
          lifecycleIdentity.id,
          lifecycleIdentity.version,
        )
      )
        throw new Error("path_invalid");
      const recordPath = v2LanguageProfileVersionDocumentPathV1(
        lifecycleIdentity.id,
        lifecycleIdentity.version,
      );
      ...
      if (
        ...
        pin.objectPath !==
          v2LanguageProfileObjectPathV1(
            storedIdentity.id,
            storedIdentity.version,
            storedIdentity.contentHash,
          )
      )
        throw new Error("pin_invalid");
      const body = await dependencies.readCanonicalObject(pin);
      const validated = validateV2LanguageProfile(body);
      if (
        !validated.ok ||
...
async function templateEntries(
  ...
  for (const row of rows) {
    try {
      const lifecycleValidation = validateModeTemplateLifecycleHead(row.data);
      if (
      ...
      const stored = await dependencies.readDocument(
        `content_mode_template_versions/${lifecycleIdentity.id}__v${lifecycleIdentity.version}`,
      );
      if (
        !stored ||
        !exactKeys(stored, [
      ...
      matchingIdentity(lifecycleIdentity, storedIdentity);
      const pin = objectPin(stored);
      const expectedPath = `content-studio/mode-templates/${sha256Utf8(storedIdentity.id)}/v${storedIdentity.version}/${storedIdentity.contentHash}.json`;
      if (
        ...
        throw new Error("pin_invalid");
      const body = await dependencies.readCanonicalObject(pin);
      const validated = validateModeTemplateArtifactBody(body);
      if (
        !validated.ok ||
  ...
  for (const row of rows) {
    try {
      const lifecycle = parseV2VoiceProfileRepositoryLifecycleV1(
        canonicalJsonV1(row.data),
      );
      if (
        lifecycle.profileKind !== profileKind ||
        `${collection}/${row.id}` !==
          v2VoiceProfileRepositoryLifecycleDocumentPathV1(
            profileKind,
            lifecycle.profileId,
        ...
        throw new Error("lifecycle_invalid");
      const recordRaw = await dependencies.readDocument(
        v2VoiceProfileRepositoryRecordDocumentPathV1(
          profileKind,
          lifecycle.profileId,
          lifecycle.version,
      ...
      if (!recordRaw) throw new Error("record_missing");
      const stored = parseV2VoiceProfileRepositoryRecordV1(
        canonicalJsonV1(recordRaw),
      );
      if (
        stored.profileId !== lifecycle.profileId ||
      ...
      if (profileKind === "speech_profile") {
        const parsed = parseV2SpeechProfileBodyV1(canonicalJsonV1(body));
        if (
          parsed.profileId !== stored.profileId ||
        ...
        const parsed = parseV2VoiceGenerationProfileBodyV1(
          canonicalJsonV1(body),
        );
        if (
          parsed.profileId !== stored.profileId ||
          parsed.version !== stored.version
...
async function decisionEntries(
  ...
  for (const row of rows) {
    try {
      const validatedRecord = validateDecisionRegistryRecord(row.data);
      if (!validatedRecord.ok) throw new Error("record_invalid");
      ...
      const validated = validateDecisionRegistry({
        body,
        record: recordValue,
      });
...
function compareRef(left: object, right: object): number {
  const a = canonicalJsonV1(left);
  const b = canonicalJsonV1(right);
  return a < b ? -1 : a > b ? 1 : 0;
}

export async function readV2OwnerGeneratorSetupCatalogV1(
  request: {
    readonly auth?: {
      readonly uid?: string;
  ...
  if (
    ...
    parseV2ExactLanguageTagV1(request.data.targetLanguage) === null
  )
    throw new Error("v2_owner_generator_setup_request_invalid");
  const targetLanguage = request.data.targetLanguage;
  const [language, speech, generation, templates, decisions] =
    await Promise.all([
  ...
  return Object.freeze({
    ok: true as const,
    schemaVersion: V2_OWNER_GENERATOR_SETUP_CATALOG_SCHEMA_V1,
    targetLanguage,
    ...
    catalogUse: "setup_selection_only_server_revalidates_on_use" as const,
    generationAuthority: "none" as const,
    publicationAuthority: "none" as const,
    releaseAuthority: false as const,
  });
}

function firebaseDependencies(): V2OwnerGeneratorSetupCatalogDependenciesV1 {
  const db = admin.firestore();
  const bucket = admin.storage().bucket();
  return Object.freeze({
    async listDocuments(collection: string, maximum: number) {
    ...
    async readCanonicalObject(
      input: Parameters<
        V2OwnerGeneratorSetupCatalogDependenciesV1["readCanonicalObject"]
      >[0],
    ) {
      ...
      return (
        await readImmutableCanonicalObject(file, {
...
export const adminGetV2OwnerGeneratorSetupCatalog = onCall(
  { region: "us-central1", enforceAppCheck: ENFORCE_APP_CHECK_ADMIN },
  async (request) => {
    try {
      return await readV2OwnerGeneratorSetupCatalogV1(
        request,
      ...
      throw new HttpsError(
        "failed-precondition",
        /^v2_owner_generator_setup_[a-z0-9_]+$/u.test(message)
          ? message
