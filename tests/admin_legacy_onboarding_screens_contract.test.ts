...
// зачем 16.08.2026: тест читал admin/legacy.html — ЗАМОРОЖЕННЫЙ файл, который на
// боевую не публикуется (hosting отдаёт папку admin/v2). Из-за этого сторож охранял
// мёртвую копию: правки рабочей админки он не видел, а расхождение каталога с
// приложением пропускал. Читаем ровно тот файл, которым пользуется владелец.
...
describe("Admin legacy onboarding screens section", () => {
  ...
  it("lists exactly the app catalog, in the same order", () => {
    expect(adminCatalogIds()).toEqual(
  ...
  it("keeps the legally mandatory step permanently enabled", () => {
    expect(legacy).toContain(
      `const ONBOARDING_MANDATORY_STEP = '${MANDATORY_ONBOARDING_STEP}'`,
    );
    expect(legacy).toContain(
      "const mandatory = id === ONBOARDING_MANDATORY_STEP",
    );
  ...
  it("publishes through the revision-safe shared writer, after an explicit preview", () => {
    ...
    expect(legacy).toContain(
      "window.saveControlPanelTexts({ [ONBOARDING_ENABLED_STEPS_KEY]",
    );
