...
import {
  ...
  GAVAN_WEEK1_DAY7_BLUEPRINT_CANDIDATE_PATH,
  validateGavanWeek1Day7BlueprintCandidate,
  writeGavanWeek1Day7BlueprintCandidate,
} from '../tools/personal_plan_gavan_week1_day7_blueprint_candidate';

const GENERATED_AT = '2026-06-03T01:10:00.000Z';
...
describe('Gavan week 1 day 7 blueprint candidate', () => {
  beforeEach(() => {
    if (existsSync(GAVAN_WEEK1_DAY7_BLUEPRINT_CANDIDATE_PATH)) {
      rmSync(GAVAN_WEEK1_DAY7_BLUEPRINT_CANDIDATE_PATH, { force: true });
    }
  });

  afterAll(() => {
    writeGavanWeek1Day7BlueprintCandidate(standards(), {
      generatedAt: GENERATED_AT,
      targetPath: GAVAN_WEEK1_DAY7_BLUEPRINT_CANDIDATE_PATH,
    });
  });

  it('builds a non-live day 7 only blueprint candidate from the week standards', () => {
  ...
  it('keeps day 7 broad socially safe and free of private or narrow anchors', () => {
    const candidate = buildGavanWeek1Day7BlueprintCandidate(standards(), {
      generatedAt: GENERATED_AT,
    });
    const serializedContent = JSON.stringify(candidate.contentUnits.map((unit) => ({
      english: unit.english,
      meaningRu: unit.meaningRu,
      newWords: unit.newWords,
      firstSeenConstructions: unit.firstSeenConstructions,
      explanationBodies: unit.explanationCards.map((card) => card.body),
    }))).toLowerCase();
  ...
  it('uses practical day 7 phrase candidates for week review and a simple next action', () => {
    const candidate = buildGavanWeek1Day7BlueprintCandidate(standards(), {
      generatedAt: GENERATED_AT,
    });
  ...
  it('includes week-review exercise variety without claiming final exercises', () => {
    const candidate = buildGavanWeek1Day7BlueprintCandidate(standards(), {
      generatedAt: GENERATED_AT,
    });
    const exerciseTypes = candidate.exerciseBlueprints.map((exercise) => exercise.exerciseType);
    const purposes = candidate.exerciseBlueprints.map((exercise) => exercise.purpose).join(' ');
    ...
    expect(new Set(exerciseTypes).size).toBeGreaterThanOrEqual(4);
    expect(purposes.toLowerCase()).toContain('week review');
  ...
  it('passes its own quality gate and catches corrupted candidates', () => {
    ...
    expect(validateGavanWeek1Day7BlueprintCandidate(candidate, standards())).toEqual({
      valid: true,
      issues: [],
    });
    ...
    const result = validateGavanWeek1Day7BlueprintCandidate(corrupted, standards());
  ...
  it('writes deterministic day 7 candidate JSON only under temp or report roots', () => {
    const result = writeGavanWeek1Day7BlueprintCandidate(standards(), {
      generatedAt: GENERATED_AT,
      targetPath: GAVAN_WEEK1_DAY7_BLUEPRINT_CANDIDATE_PATH,
    });
    ...
    expect(existsSync(GAVAN_WEEK1_DAY7_BLUEPRINT_CANDIDATE_PATH)).toBe(true);

    const serialized = readFileSync(GAVAN_WEEK1_DAY7_BLUEPRINT_CANDIDATE_PATH, 'utf8');
    expect(serialized).not.toMatch(BROKEN_ENCODING_RE);

    const parsed = JSON.parse(serialized);
    expect(parsed.kind).toBe('gavan_week1_day7_blueprint_candidate');
  ...
  it('rejects app tools tests and root config write targets', () => {
    const appResult = writeGavanWeek1Day7BlueprintCandidate(standards(), {
      generatedAt: GENERATED_AT,
      targetPath: path.join(process.cwd(), 'app', 'gavan-week1-day7-blueprint.json'),
    });
    const toolsResult = writeGavanWeek1Day7BlueprintCandidate(standards(), {
      generatedAt: GENERATED_AT,
      targetPath: path.join(process.cwd(), 'tools', 'gavan-week1-day7-blueprint.json'),
    });
    const testsResult = writeGavanWeek1Day7BlueprintCandidate(standards(), {
      generatedAt: GENERATED_AT,
      targetPath: path.join(process.cwd(), 'tests', 'gavan-week1-day7-blueprint.json'),
    });
  ...
  it('does not import live catalog quiz UI storage audio scoring or navigation', () => {
    const source = readFileSync(
      path.join(process.cwd(), 'tools', 'personal_plan_gavan_week1_day7_blueprint_candidate.ts'),
      'utf8',
    );
