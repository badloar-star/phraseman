...
describe('server-authoritative stage capability matrix', () => {
    ...
    test('rejects unsupported levels, locale pairs, counts and prerequisite shapes', () => {
        expect(() => (0, stage_capabilities_1.assertStageCapabilityRequest)({ kind: 'arena_topic', count: 1, cefr: 'C1', studyTarget: 'en', sourceLocale: 'ru', prerequisiteKinds: [] })).toThrow('stage_capability_cefr_unsupported');
        ...
        expect(() => (0, stage_capabilities_1.assertStageCapabilityRequest)({ kind: 'quiz_questions', count: 10, cefr: 'A2', studyTarget: 'en', sourceLocale: 'ru', prerequisiteKinds: [] })).toThrow('stage_capability_prerequisites_invalid');
    });
    ...
    test('exposes language and exact prerequisite cardinality to Admin', () => {
        ...
        expect((0, stage_capabilities_1.stageCapability)('quiz_questions').prerequisiteCardinality).toEqual({ min: 1, max: 1 });
    });
});
