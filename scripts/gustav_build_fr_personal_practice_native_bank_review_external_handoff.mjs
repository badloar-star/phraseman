...
const OUT_JSON = path.join(OUT_DIR, 'fr_personal_practice_native_bank_review_external_handoff_v1.json');
const OUT_MD = path.join(OUT_DIR, 'fr_personal_practice_native_bank_review_external_handoff_v1.md');
const OUT_PS1 = path.join(OUT_DIR, 'fr_personal_practice_native_bank_review_external_handoff_run_next_batch.ps1');
...
function makePowerShell(batches) {
  ...
  return [
    ...
    'Write-Host "Running French personal-practice native-bank review batch $BatchNumber (start-index $startIndex, limit $limit, estimated cost `$${expectedCost})"',
    'node scripts/gustav_execute_fr_personal_practice_native_bank_review_batch.mjs --start-index $startIndex --limit $limit --execute --validate-after',
...
function main() {
  ...
  const handoff = {
    schemaVersion: 'gustav-fr-personal-practice-native-bank-review-external-handoff-v1',
    generatedAt,
    status: blockers.length > 0 ? 'BLOCK' : batches.length > 0 ? 'HOLD_EXTERNAL_NATIVE_BANK_REVIEW_HANDOFF_READY' : 'PASS_NO_PENDING_NATIVE_BANK_REVIEW_BATCHES',
    studyTarget: 'fr',
    ...
    batches: batches.map((batch) => ({
      ...batch,
      externalPowerShellCommand: `powershell -ExecutionPolicy Bypass -File docs\\gustav\\generated\\fr\\personal_practice\\fr_personal_practice_native_bank_review_external_handoff_run_next_batch.ps1 -BatchNumber ${batch.batchNumber}`,
    })),
    blockers,
    safety: {
  ...
  fs.writeFileSync(OUT_MD, `# French Personal Practice Native Bank External Handoff\n\nStatus: ${handoff.status}\n\nPending rows: ${pending.length}\n\nActivation approved: false\n`, 'utf8');
