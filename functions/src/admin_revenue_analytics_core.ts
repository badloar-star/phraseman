import type { FinancialCoverage } from './revenuecat_financial_normalization';
...
export interface ServerRevenueRow {
  eventId?: unknown;
  eventType?: unknown;
  environment?: unknown;
...
function normalizeRows(rows: ServerRevenueRow[], fromMs?: number): { rows: NormalizedRow[]; maxAt: number | null } {
  const seen = new Set<string>();
  const out: NormalizedRow[] = [];
  let maxAt: number | null = null;
  rows.forEach((row) => {
    if (text(row.environment).toUpperCase() === 'SANDBOX') return;
    const eventId = text(row.eventId);
    if (eventId && seen.has(eventId)) return;
    if (eventId) seen.add(eventId);
    const at = timestamp(row.eventTimestampMs) ?? timestamp(row.createdAtMs);
    if (at == null) return;
    if (fromMs != null && at < fromMs) return;
...
export function aggregateServerRevenueAnalytics(
  inputRows: ServerRevenueRow[],
  options: { watermarkMs?: number; fromMs?: number; truncated?: boolean } = {},
) {
  ...
  const paidChains = new Set(positiveRows.map((row) => row.chain).filter((chain): chain is string => Boolean(chain)));

  const chainRows = new Map<string, NormalizedRow[]>();
  rows.forEach((row) => {
    if (!row.chain) return;
  ...
  chainRows.forEach((chain) => {
    ...
    if (chain.some((row) => row.at >= trial.at
      ...
      && ((row.gross ?? 0) > 0 || row.trialConversion === true))) convertedTrialChains += 1;
  });

  const chainOrigins = new Map<string, NormalizedRow>();
  chainRows.forEach((chain, chainId) => {
    const origin = chain.find((row) => ORIGIN_EVENT_TYPES.has(row.type));
    if (origin) chainOrigins.set(chainId, origin);
  });
  const leftTruncatedChains = [...chainRows.entries()].filter(([chainId, chain]) =>
    !chainOrigins.has(chainId) && chain.some((row) => MONEY_EVENT_TYPES.has(row.type))).length;
  ...
  const monthlyRenewal = [1, 2, 3].map((monthOffset) => {
    ...
    monthlyFirstPaid.forEach((first) => {
      ...
      if (watermarkMs == null || watermarkMs < targetEnd) return;
      eligibleChains += 1;
      ...
      if (chain.some((row) => row.cadence === 'monthly'
        ...
        && POSITIVE_EVENT_TYPES.has(row.type) && (row.gross ?? 0) > 0)) renewedChains += 1;
    });
    return {
  ...
  const ltv = [30, 60, 90].map((windowDays) => {
    ...
    chainRows.forEach((chain) => {
      ...
      if (!firstPaid || watermarkMs == null) return;
      const end = firstPaid.at + windowDays * DAY_MS;
      if (watermarkMs < end) return;
      maturePaidChains += 1;
      signedGross += chain
  ...
  chainRows.forEach((chain) => {
    if (!chain.some((row) => ORIGIN_EVENT_TYPES.has(row.type))) return;
    const expiration = [...chain].reverse().find((row) => row.type === 'EXPIRATION');
    if (!expiration || chain.some((row) => row.at > expiration.at && row.type === 'RENEWAL' && (row.gross ?? 0) > 0)) return;
    realizedChurnChains += 1;
    if (expiration.expirationReason === 'UNSUBSCRIBE' || expiration.expirationReason === 'VOLUNTARY') voluntaryChurnChains += 1;
    if (expiration.expirationReason.includes('BILLING')) billingErrorChurnChains += 1;
  });
