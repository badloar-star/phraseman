...
describe('Admin v2 Free / Plus access workflow', () => {
  ...
  test('ports the old control-panel premium gates into a guarded native workflow', () => {
    ...
    expect(core).toContain('Бесплатный / Plus доступ и уроки');
  ...
  test('uses preview, reason, existing server publish and stale-preview guard', () => {
    ...
    expect(core).toContain('Укажите причину изменения Бесплатный / Plus доступа.');
    expect(core).toContain("source: 'premium-access'");
    expect(core).toContain('current = buildPremiumAccessPreview()');
    expect(core).toContain('настройки бесплатного и Plus-доступа меняют монетизацию и доступ к урокам');
  ...
  test('does not surface Arena access controls in V2 while preserving legacy configuration', () => {
    expect(core).not.toContain('gate_arena_premium');
    expect(core).not.toContain('arena_daily_max');
  ...
  test('updates the control-panel map from legacy write module to guarded workflow', () => {
    expect(core).toContain("primaryLabel: 'Открыть доступ Бесплатный / Plus'");
    expect(core).toContain("risk: 'Защищённая публикация'");
