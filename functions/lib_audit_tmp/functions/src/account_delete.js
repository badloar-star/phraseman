...
exports.resolveAccountDeleteIdentityClosure = resolveAccountDeleteIdentityClosure;
...
async function resolveAccountDeleteIdentityClosure(db, stableUid, authUid) {
    ...
    const add = (raw) => {
        ...
        if (identities.size >= MAX_ACCOUNT_DELETE_IDENTITIES) {
            throw new https_1.HttpsError('resource-exhausted', 'account_delete_identity_limit');
        }
        identities.add(id);
        queue.push(id);
    };
    ...
    for (let cursor = 0; cursor < queue.length; cursor += 1) {
        ...
        if (directOwnerMap.exists)
            add(directOwnerMap.data()?.canonicalStableId);
...
async function persistPermanentDeletionDenials(db, identities) {
    const batch = db.batch();
    for (const identity of identities) {
        batch.set(db.collection(account_delete_job_1.ACCOUNT_DELETE_PERMANENT_DENIALS).doc((0, account_delete_job_1.accountDeletePermanentDenialId)(identity)), {
...
async function deleteMergeIdentityRecords(db, identities, ctx, stats) {
    for (const identity of identities) {
        await deleteDocTree(ctx, db.collection('account_identity_owner_map').doc(identity));
        await deleteQuery(db.collection('account_merge_outbox').where('winnerStableId', '==', identity), ctx, stats);
        await deleteQuery(db.collection('account_merge_outbox').where('loserStableId', '==', identity), ctx, stats);
    }
}
...
async function executeAccountDeletion(db, stableUid, authUid) {
    ...
    try {
        ...
        const emails = [...new Set((await Promise.all(identities.map((identity) => getEmailsForDeletion(db, authUid, identity)))).flat())];
        ...
        await runDeleteStage(ctx, stats, 'direct_docs', async () => {
            for (const identity of identities)
                await deleteDirectDocs(db, identity, authUid, ctx);
        });
        await runDeleteStage(ctx, stats, 'arena_sessions_and_match_history', async () => {
            for (const identity of identities)
                await deleteArenaSessionsAndMatchHistory(db, identity, authUid, ctx, stats);
        });
        await runDeleteStage(ctx, stats, 'arena_season_entries', async () => {
            for (const identity of identities)
                await deleteArenaSeasonEntries(db, identity, authUid, ctx, stats);
        });
        await runDeleteStage(ctx, stats, 'field_matches', async () => {
            for (const identity of identities)
                await deleteFieldMatches(db, identity, authUid, ctx, stats);
        });
        await runDeleteStage(ctx, stats, 'collection_group_matches', async () => {
            for (const identity of identities)
                await deleteCollectionGroupMatches(db, identity, authUid, ctx, stats);
        });
        ...
        await runDeleteStage(ctx, stats, 'league_groups', async () => {
            for (const identity of identities)
                await removeFromLeagueGroups(db, identity, stats);
        });
        await runDeleteStage(ctx, stats, 'arena_club_events', async () => {
            for (const identity of identities)
                await removeFromArenaClubEvents(db, identity, stats);
        });
        await runDeleteStage(ctx, stats, 'activity_like_stats', async () => {
            for (const identity of identities)
                await anonymizeActivityLikeStats(db, identity, stats);
        });
        await runDeleteStage(ctx, stats, 'friend_gift_daily_limits', async () => {
            for (const identity of identities)
                await removeFromFriendGiftDailyLimits(db, identity, stats);
        });
        await runDeleteStage(ctx, stats, 'merge_identity_records', () => (deleteMergeIdentityRecords(db, identities, ctx, stats)));
