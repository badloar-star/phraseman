...
exports.buildEpisode01Session33To40 = buildEpisode01Session33To40;
exports.assertEpisode01Sessions33To40Contract = assertEpisode01Sessions33To40Contract;
const episode_01_sessions_33_40_intro_data_v1_1 = require("./episode_01_sessions_33_40_intro_data_v1");
const episode_01_sessions_33_40_token_semantics_v1_1 = require("./episode_01_sessions_33_40_token_semantics_v1");
const LOCALES = [
    "ru",
    "uk",
    "es",
...
const tokens = (english) => english.replace(/[?.!]/gu, "").split(/\s+/u).filter(Boolean);
function genderedPossessive(locale, owner, noun) {
    const feminineByLocale = {
        ru: new Set(["mother", "sister", "bag", "cup"]),
        uk: new Set(["mother", "sister", "book", "bag", "cup"]),
        "pt-BR": new Set(["mother", "sister", "bag", "cup", "key"]),
        pl: new Set(["mother", "sister", "book", "bag"]),
    };
    if (locale === "ru") {
        if (owner === "his")
            return "его";
    ...
    return { my: "my", your: "your", his: "his", her: "her" }[owner];
}
const BANK = {
...
const alternatives = (word) => (BANK[category(word)] ?? BANK.object)
    .filter((item) => item.toLowerCase() !== word.toLowerCase())
    .slice(0, 5);
const category = (word) => /^(what|where|who|how)$/iu.test(word)
    ? "question-word"
...
const nativeMeaning = (locale, english) => {
    const familyIdentity = english.match(/^(He|She|This|That) is (my|your|his|her) (mother|father|sister|brother)\.$/u);
    if (familyIdentity) {
    ...
    const familyIdentityQuestion = english.match(/^Is (this|that) (my|your|his|her) (mother|father|sister|brother)\?$/u);
    ...
    const who = english.match(/^Who is (he|she|this|that|my|your|his|her)(?: (mother|father|sister|brother))?\?$/u);
    if (who) {
    ...
    const locationQuestion = english.match(/^Are (he|she|you|we|they) (here|home)\?$/u) ??
    ...
    const demonstrativeWhere = english.match(/^Where is (this|that) (book|bag|cup|key)\?$/u);
    ...
    const possessiveWhere = english.match(/^Where is (my|your|his|her) (mother|father|sister|brother|book|bag|cup|key)\?$/u);
    if (possessiveWhere) {
    ...
    const demonstrativeQuestion = english.match(/^Is (this|that) (?:(a) |(my|your|his|her) )?(book|bag|cup|key)\?$/iu);
    ...
    const stateStatement = english.match(/^(I|You|He|She|They) (?:am|are|is) (fine|okay|ready|calm|happy|busy)\.$/u);
    if (stateStatement) {
    ...
    const stateQuestion = english.match(/^(?:Is (he|she) |Are (you|they) )(tired|happy|ready|calm|busy)\?$/u);
    if (stateQuestion) {
    ...
    const possessiveState = english.match(/^(My|Your|His|Her) (mother|father|sister|brother|book|bag|cup|key) is (here|home|ready|calm)\.$/u);
    if (possessiveState) {
    ...
    const identity = english.match(/^(This|That) is (?:(a) |(my|your|his|her) )?(book|bag|cup|key)\.$/u);
    if (identity) {
        ...
        const p = possessive
            ? ["ru", "uk", "pt-BR", "pl"].includes(locale)
                ? genderedPossessive(locale, possessive, noun)
...
function phraseWordPrompt(locale, meaning) {
    const prompts = {
...
function semanticDistractorReason(locale, correct, alternative, meaning) {
    const correctGloss = (0, episode_01_sessions_33_40_token_semantics_v1_1.episode01TokenGloss)(locale, correct);
    const alternativeGloss = (0, episode_01_sessions_33_40_token_semantics_v1_1.episode01TokenGloss)(locale, alternative);
    const reasons = {
...
function authoredPhraseExplanation(locale, english, meaning) {
    ...
    const possessive = phraseTokens.find((token) => /^(my|your|his|her)$/iu.test(token));
    const firstGloss = (0, episode_01_sessions_33_40_token_semantics_v1_1.episode01TokenGloss)(locale, first);
    const possessiveGloss = possessive
        ? (0, episode_01_sessions_33_40_token_semantics_v1_1.episode01TokenGloss)(locale, possessive)
        : "";
...
function buildPhraseStudyDetails(english) {
    const phraseTokens = tokens(english);
    return Object.fromEntries(LOCALES.map((locale) => {
        const meaning = nativeMeaning(locale, english);
        const makeDistractors = (correct) => alternatives(correct).map((value) => ({
            value,
    ...
    return {
        ...
        words: tokens(english).map((correct) => ({
            ...
            distractors: alternatives(correct).map((value) => ({
                ...
                why: semanticDistractorReason("ru", correct, value, localized.ru.meaning),
            })),
        })),
    };
}
    ...
    return Object.fromEntries(LOCALES.map((locale) => {
        ...
        return [
            ...
            at < 0
                ...
                : [
                    { text: text.slice(0, at), semantic: "explanation" },
...
function localizeEnglishChoice(value) {
    return {
        ru: value,
        uk: value,
        es: value,
...
function authoredIntroPage(page, ordinal) {
...
function buildEpisode01Session33To40(ordinal) {
    const authored = episode_01_sessions_33_40_intro_data_v1_1.AUTHORED_INTROS_33_TO_40[ordinal];
    if (!authored) {
    ...
    return {
        ...
        introPages,
        phrases: PHRASES[ordinal].map((english, index) => phrase(ordinal, index, english, FEATURES[ordinal])),
    };
}
function assertEpisode01Sessions33To40Contract(sources) {
    const chapter = sources.filter((source) => source.requiredSessionOrdinal >= 33 &&
    ...
    chapter.forEach((source) => {
        ...
        source.phrases.forEach((item) => {
            ...
            expect(Object.keys(item.localizedDetails ?? {}).sort()).toEqual([...LOCALES].sort());
        ...
        source.introPages.forEach((page) => LOCALES.forEach((locale) => {
            ...
            expect(page.question.choices[page.question.correctChoiceIndex][locale]).not.toMatch(/\b(?:do|does)\b/iu);
        }));
    });
    expect(chapter[0].phrases.some((item) => item.features.includes("demonstrative"))).toBe(true);
    expect(chapter[4].phrases.some((item) => item.features.includes("possessive_your"))).toBe(true);
    expect(chapter[5].phrases.some((item) => item.features.includes("possessive_his_her"))).toBe(true);
}
//# sourceMappingURL=episode_01_sessions_33_40_support_v1.js.map