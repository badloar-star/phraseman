...
exports.adminDeactivateVipSurveyCampaign = exports.adminLaunchVipSurveyCampaign = exports.adminCleanupExpiredAppMessages = exports.adminDeleteAppMessage = exports.adminUpdateAppMessage = exports.adminSetAppMessageActive = exports.adminCreateAppMessage = exports.adminListAppMessages = exports.adminSendPersonalAppMessage = void 0;
...
exports.assertBoundedActiveVipSurveyCount = assertBoundedActiveVipSurveyCount;
exports.normalizeVipSurveyCampaignInput = normalizeVipSurveyCampaignInput;
exports.normalizeDeactivateVipSurveyInput = normalizeDeactivateVipSurveyInput;
...
function assertCompleteSettingsTranslations(translations, kind, pollOptionCount) {
    for (const suffix of LANGUAGES.filter((item) => item !== 'Ru')) {
        ...
        const hasPollCopy = kind !== 'poll' || (Boolean(text(value.pollQuestion, 300))
            && pollOptions.length === pollOptionCount
            && pollOptions.every((option) => Boolean(text(option, 160))));
        if (!hasCopy || !hasPollCopy) {
            throw new https_1.HttpsError('invalid-argument', `settings_translations_required:${key}`);
        }
    }
}
function normalizeAppMessageCreateInput(data, actorEmail, nowMs = Date.now()) {
    ...
    if (deliverySurface === 'settings' && !['all', 'free', 'premium'].includes(audienceRaw)) {
        throw new https_1.HttpsError('invalid-argument', 'settings_audience_required');
    }
    ...
    const settingsSlot = deliverySurface === 'settings' && (data.settingsSlot === 'top' || data.settingsSlot === 'bottom')
        ? data.settingsSlot
        : null;
    if (deliverySurface === 'settings' && !settingsSlot)
        throw new https_1.HttpsError('invalid-argument', 'Settings slot must be top or bottom');
    const controlPercent = Math.max(0, Math.min(50, Math.floor(Number(data.controlPercent ?? 0))));
    if (!Number.isFinite(controlPercent))
        throw new https_1.HttpsError('invalid-argument', 'controlPercent invalid');
    ...
    if (kind === 'poll') {
        ...
        if (!questionRu || rawRuOptions.length < 2 || rawRuOptions.length > maxPollOptions) {
            throw new https_1.HttpsError('invalid-argument', `Poll requires a RU question and 2-${maxPollOptions} options`);
        }
...
function normalizeAppMessageUpdateInput(data) {
    ...
    if (typeof data.active !== 'boolean')
        throw new https_1.HttpsError('invalid-argument', 'active boolean required');
...
function assertBoundedActiveVipSurveyCount(count) {
    if (!Number.isSafeInteger(count) || count < 0)
        throw new https_1.HttpsError('internal', 'active_vip_survey_count_invalid');
    if (count > 50)
        throw new https_1.HttpsError('resource-exhausted', 'more_than_50_active_vip_surveys_require_offline_cleanup');
}
function normalizeVipSurveyCampaignInput(data, actorEmail, nowMs = Date.now()) {
    if (!isRecord(data))
        throw new https_1.HttpsError('invalid-argument', 'request object required');
    const command = requiredCommandFields(data);
    const translations = isRecord(data.translations) ? data.translations : {};
    ...
    if (!titleRu || !messageRu)
        throw new https_1.HttpsError('invalid-argument', 'RU title and body are required');
    if (data.audience !== 'free')
        throw new https_1.HttpsError('invalid-argument', 'VIP survey audience must be free');
    ...
    if (!Array.isArray(data.targetAppVersions) || data.targetAppVersions.length < 1 || data.targetAppVersions.length > 10) {
        throw new https_1.HttpsError('invalid-argument', 'targetAppVersions required');
    }
    ...
    if (targetAppVersions.some((value) => !/^[A-Za-z0-9._+-]+$/.test(value)) || new Set(targetAppVersions).size !== targetAppVersions.length) {
        throw new https_1.HttpsError('invalid-argument', 'targetAppVersions invalid');
    }
    if (!isRecord(data.survey))
        throw new https_1.HttpsError('invalid-argument', 'survey required');
    const surveyId = text(data.survey.surveyId, 80);
    const rewardDays = Number(data.survey.rewardDays);
    if (surveyId !== vip_survey_contract_1.VIP_SURVEY_ID || rewardDays !== vip_survey_contract_1.VIP_SURVEY_REWARD_DAYS) {
        throw new https_1.HttpsError('invalid-argument', 'VIP survey contract mismatch');
    }
    const nowIso = new Date(nowMs).toISOString();
    const expiresAtMs = nowMs + ttlDays * 24 * 60 * 60 * 1000;
    const document = {
        active: true, kind: 'vip_survey', audience: 'free', targetAppVersions, priority, ttlDays,
        vipSurvey: { surveyId: vip_survey_contract_1.VIP_SURVEY_ID, rewardDays: vip_survey_contract_1.VIP_SURVEY_REWARD_DAYS }, createdAt: nowIso, createdAtMs: nowMs,
    ...
    const requestFingerprint = JSON.stringify({ translations, audience: 'free', priority, ttlDays, targetAppVersions, survey: { surveyId: vip_survey_contract_1.VIP_SURVEY_ID, rewardDays: vip_survey_contract_1.VIP_SURVEY_REWARD_DAYS } });
    return Object.freeze({ document: Object.freeze(document), requestFingerprint, ...command });
}
function normalizeDeactivateVipSurveyInput(data) {
    if (!isRecord(data))
        throw new https_1.HttpsError('invalid-argument', 'request object required');
    const command = requiredCommandFields(data);
...
function roleFor(token) {
    ...
    return (0, roles_1.hasAdminRole)(role) ? role : 'owner';
}
...
exports.adminSendPersonalAppMessage = (0, https_1.onCall)(callable_options_1.ADMIN_SENSITIVE_WRITE_OPTIONS, async (request) => {
...
exports.adminCreateAppMessage = (0, https_1.onCall)(callable_options_1.ADMIN_SENSITIVE_WRITE_OPTIONS, async (request) => {
    (0, callable_options_1.requireAdminAppCheck)(request);
...
exports.adminSetAppMessageActive = (0, https_1.onCall)(callable_options_1.ADMIN_SENSITIVE_WRITE_OPTIONS, async (request) => {
    (0, callable_options_1.requireAdminAppCheck)(request);
    const role = assertPermission(request, 'campaigns.write');
    const actorUid = request.auth.uid;
    const actorEmail = text(request.auth.token.email, 320) || actorUid;
    ...
    return db.runTransaction(async (tx) => {
        ...
        const [settingsSlotSnapshot, activeInSlot] = settingsSlotRef
            ? await Promise.all([
                ...
                input.active ? tx.get(db.collection('app_messages')
                    .where('deliverySurface', '==', 'settings')
                    .where('settingsSlot', '==', settingsSlot)
                    .where('active', '==', true)) : Promise.resolve(null),
            ])
            : [null, null];
        if (activeInSlot) {
            const archivedAtMs = Date.now();
...
exports.adminUpdateAppMessage = (0, https_1.onCall)(callable_options_1.ADMIN_SENSITIVE_WRITE_OPTIONS, async (request) => {
    (0, callable_options_1.requireAdminAppCheck)(request);
    const role = assertPermission(request, 'campaigns.write');
    const actorUid = request.auth.uid;
    const actorEmail = text(request.auth.token.email, 320) || actorUid;
    ...
    const claimed = await db.runTransaction(async (tx) => {
        ...
        if (before.deliverySurface === 'settings' && before.kind === 'poll' && structureChanged) {
            throw new https_1.HttpsError('failed-precondition', 'settings_poll_requires_new_campaign');
        }
...
exports.adminDeleteAppMessage = (0, https_1.onCall)(callable_options_1.ADMIN_SENSITIVE_WRITE_OPTIONS, async (request) => {
    (0, callable_options_1.requireAdminAppCheck)(request);
    const role = assertPermission(request, 'campaigns.write');
    const actorUid = request.auth.uid;
...
exports.adminCleanupExpiredAppMessages = (0, https_1.onCall)(callable_options_1.ADMIN_SENSITIVE_WRITE_OPTIONS, async (request) => {
    (0, callable_options_1.requireAdminAppCheck)(request);
    const role = assertPermission(request, 'campaigns.write');
    const actorUid = request.auth.uid;
...
exports.adminLaunchVipSurveyCampaign = (0, https_1.onCall)(callable_options_1.ADMIN_SENSITIVE_WRITE_OPTIONS, async (request) => {
    (0, callable_options_1.requireAdminAppCheck)(request);
    const role = assertPermission(request, 'campaigns.write');
    const actorUid = request.auth.uid;
    const actorEmail = text(request.auth.token.email, 320) || actorUid;
...
exports.adminDeactivateVipSurveyCampaign = (0, https_1.onCall)(callable_options_1.ADMIN_SENSITIVE_WRITE_OPTIONS, async (request) => {
    (0, callable_options_1.requireAdminAppCheck)(request);
    const role = assertPermission(request, 'campaigns.write');
    const actorUid = request.auth.uid;
    const actorEmail = text(request.auth.token.email, 320) || actorUid;
