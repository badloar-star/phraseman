...
## ⛔ LEARNING V2 AUTHORING: СНАЧАЛА ПАПКА CODEX НА РАБОЧЕМ СТОЛЕ (владелец, 2026-09-19)

При любом написании, продолжении, изменении, проверке или показе learner-facing
сессий Learning V2 исполнитель обязан **до любых действий** прочитать:

1. `C:\Users\badlo\OneDrive\Desktop\Codex\00_ПРОЧТИ_ПЕРВЫМ.txt`;
2. `C:\Users\badlo\OneDrive\Desktop\Codex\00_ГЛАВНОЕ_ТРЕБОВАНИЕ.md`;
3. `C:\Users\badlo\OneDrive\Desktop\Codex\НАЧНИ_ОТСЮДА.md`;
4. актуальные документы фабрики в `content/learning-v2-course`: сначала
   `ГЛАВНОЕ_ТРЕБОВАНИЕ.md`, `КОНСТИТУЦИЯ.md`, `МЕТОД_РАБОТЫ.md`, затем три
   эталона `exemplars/en/l01_s01.ru.md`–`l01_s03.ru.md` и все файлы
   `judgements/owner/`;
5. применимый план курса и карточку слова.

Папка Desktop Codex — стартовый пакет и обязательный источник порядка работы;
редактировать её нельзя. Рабочая копия фабрики находится только в репозитории.
`docs/v2/СТАРТ В2.md` и связанные runtime/contracts читаются дополнительно,
когда задача затрагивает приложение, gates или native-проекцию, но не заменяют
фабричную Конституцию, эталоны и дословные owner-вердикты при написании текста.
После compaction, restart, handoff и перед каждой новой сессией этот маршрут
проходится заново. Пропуск любого пункта означает `HOLD`: новую сессию нельзя
выпускать или добавлять в макет.

Перед learner-facing authoring каждой новой сессии обязателен отдельный свежий
`PREAUTHORING GUARDIAN`. Это независимый read-only агент: он полностью проходит
маршрут выше, сверяет точную строку плана, Cambridge, все предыдущие
learner-facing сессии урока, новые слова, новую ситуацию и каждое применимое
требование. Автору запрещено выдавать или заполнять эту квитанцию самому.
Guardian возвращает `preauthoring.json`, привязанный к текущим хэшам документов
и предыдущего контента; только после его PASS фабрика может запустить
`draft`, `edit` или `full`. После compaction/restart/handoff старый ответ агента
не считается чтением: запускается новый guardian и выпускается свежая
квитанция. Любой BLOCK/HOLD, неполный чек-лист или незакрытая предыдущая сессия
останавливает authoring до исправления; root/автор не вправе переопределить
вердикт.
...
- `beginAccountDeletion()` keeps the UI-fast local handoff: it durably writes the guard, invalidates the old generation, drains premium/restore/cloud writers, and returns success only after one registered operation-scoped flight proves phone-state lineage retirement, outbox cleanup, account wipe, AsyncStorage clear, and strict readback. Any throw, timeout, unreadable storage, residue, or failed drain keeps `prepared`, blocks ordinary onboarding/new identity, and every retry joins the same still-running destructive flight so late work cannot cross into a fresh generation. The `local_data_cleared` CAS is also bounded; a late forward-only write remains restartable. Server closure/Auth release continue in `completion`. The server root-fences auth/stable deletion, discovers the alias closure under that fence, then atomically persists the sorted frozen closure/hash/version/cutoff, every member denial, the worker job, and a `closure_committed` receipt. The worker consumes only that frozen closure. Mutable identity indexes (`auth_links`, name and friend-code indexes) must be transactionally reread before deletion and deleted only while their current owner still belongs to the frozen closure; never delete a stale query snapshot after it has been repointed. The server then idempotently deletes the old Firebase Auth UID (`user-not-found` is success), and only then publishes `credential_safe`. Never advance past `local_data_cleared` from a dispatch-only or lost response.
...
- The first Google/Apple tap after the delete handoff must join/converge the pending deletion **before opening the provider picker**, wait for authoritative `credential_safe`, and then continue the same `signInWithProvider()` call with exactly one picker. The same external credential may create a brand-new empty profile immediately; it must never restore old data, progress, premium, avatar, aura, or the retired stable/auth identity. A response-loss/restart/network path stays fail-closed with the durable guard and may return `account_delete_pending`, but a healthy path must not require a second tap or picker.
