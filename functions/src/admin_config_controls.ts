...
function command(data: RecordValue) {
...
function boundedKeys(value: unknown, field: string): readonly string[] {
  if (!Array.isArray(value) || value.length > 100) throw new HttpsError('invalid-argument', `${field} must contain at most 100 items`);
  const values = value.map((item) => text(item, 200));
  if (values.some((item) => !item) || new Set(values).size !== values.length) {
    throw new HttpsError('invalid-argument', `${field} must contain unique non-empty keys`);
  }
  return Object.freeze(values);
}

export function normalizeAdminNavLayoutInput(data: unknown) {
  ...
  if (pinned.some((item) => hidden.includes(item))) throw new HttpsError('invalid-argument', 'pinned and hidden must be disjoint');
...
export function normalizeAdminAlertsConfigInput(data: unknown) {
  if (!isRecord(data)) throw new HttpsError('invalid-argument', 'request object required');
  const required = command(data);
  if (typeof data.enabled !== 'boolean' || !isRecord(data.types)) throw new HttpsError('invalid-argument', 'enabled and types are required');
...
export function normalizeAdminAlertsTestInput(data: unknown) {
  if (!isRecord(data)) throw new HttpsError('invalid-argument', 'request object required');
  if ('chatId' in data || 'enabled' in data || 'types' in data || 'spikeThreshold' in data || 'spikePerHour' in data) {
    throw new HttpsError('invalid-argument', 'alert test uses saved server configuration only');
  }
  const required = command(data);
...
export function requireAdminConfigActor(request: { app?: unknown; auth?: { uid: string; token?: RecordValue } }): { actorUid: string; role: AdminRole } {
...
export function assertAdminConfigReplay(operation: RecordValue, actorUid: string, fingerprint: string): void {
  if (operation.actorUid !== actorUid) throw new HttpsError('permission-denied', 'admin operation belongs to another actor');
  if (operation.requestFingerprint !== fingerprint) throw new HttpsError('already-exists', 'idempotencyKey reused for another payload');
}

export function assertExpectedAdminConfigRevision(currentRevision: unknown, expectedRevision: number): number {
  const revision = Number(currentRevision ?? 0);
  if (!Number.isSafeInteger(revision) || revision < 0 || revision !== expectedRevision) {
    throw new HttpsError('aborted', `stale_admin_config_revision:${Number.isFinite(revision) ? revision : 'invalid'}`);
  }
  return revision;
}

async function publishConfig(
  request: { app?: unknown; auth?: { uid: string; token?: RecordValue } },
  input: ReturnType<typeof normalizeAdminNavLayoutInput> | ReturnType<typeof normalizeAdminAlertsConfigInput>,
  documentId: 'nav_layout' | 'alerts',
  action: string,
) {
...
export const adminGetAdminConfigWorkspace = onCall(ADMIN_SENSITIVE_WRITE_OPTIONS, async (request) => {
...
export const adminPublishNavLayout = onCall(ADMIN_SENSITIVE_WRITE_OPTIONS, async (request) => {
...
export const adminPublishAlertsConfig = onCall(ADMIN_SENSITIVE_WRITE_OPTIONS, async (request) => {
...
export const adminTestAlerts = onCall(ADMIN_SENSITIVE_WRITE_OPTIONS, async (request) => {
  ...
  return db.runTransaction(async (tx) => {
    ...
    if (!configSnap.exists) throw new HttpsError('failed-precondition', 'alerts_config_not_saved');
    const before = configSnap.data() ?? {};
    const revision = assertExpectedAdminConfigRevision(before.revision, input.expectedRevision);
    if (before.enabled !== true) throw new HttpsError('failed-precondition', 'alerts_must_be_enabled_for_test');
    if (!text(before.chatId, 160)) throw new HttpsError('failed-precondition', 'alerts_chat_id_not_saved');
    const nowMs = Date.now();
    const nextRevision = revision + 1;
