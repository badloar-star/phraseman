...
const localized = <T>(
  factory: (locale: (typeof LEARNING_V2_INTERFACE_LOCALES)[number]) => T,
): LearningV2Localized<T> =>
  Object.fromEntries(
    LEARNING_V2_INTERFACE_LOCALES.map((locale) => [locale, factory(locale)]),
...
const introQuestion = (ordinal: 1 | 2 | 3) => ({
  ...
  explanationByLocale: localized(
    (locale) => `Explanation ${ordinal} ${locale}`,
  ),
});
...
function makeSessionShard() {
  ...
  const intro = {
    ...
    summaryByLocale: localized(
      (locale) => `Absolute-zero lesson summary ${locale}`,
    ),
    learningGoalByLocale: localized((locale) => `Say who you are ${locale}`),
    pages: ([1, 2, 3] as const).map((ordinal) => ({
      pageOrdinal: ordinal,
      ...
      bodyByLocale: localized(
        (locale) => `Full explanation ${ordinal} ${locale}`,
      ),
  ...
  const cards = Array.from({ length: LEARNING_V2_SESSION_CARD_PURPOSES.length }, (_, index) => {
    ...
    const contentItem = {
      ...
      rejectedAnswers: [
        { value: `Wrong phrase ${slot}`, reasonCode: "meaning_mismatch" },
      ],
    ...
    return {
      ...
      contentItem,
      instructionByLocale: localized(
        (locale) => `Instruction ${slot} ${locale}`,
      ),
      hintByLocale: localized((locale) => `Hint ${slot} ${locale}`),
      successMessageByLocale: localized(
        (locale) => `Success ${slot} ${locale}`,
      ),
      retryMessageByLocale: localized((locale) => `Retry ${slot} ${locale}`),
      errorExplanationByLocale: localized(
        (locale) => `Why this answer is wrong ${slot} ${locale}`,
      ),
      accessibilityLabelByLocale: localized(
        (locale) => `Accessible task ${slot} ${locale}`,
      ),
...
describe("Learning V2 all-locale generated session shard", () => {
  test("accepts exactly one complete 15-card session across all eight interface locales", () => {
    ...
    expect(learningV2GeneratedSessionShardFingerprint(shard, expected)).toMatch(
      /^[a-f0-9]{64}$/,
    );
  });
  ...
  test("rejects the historical 7-9 card demo shape", () => {
    ...
    expect(() =>
      validateLearningV2GeneratedSessionShardV1(
        { ...shard, cards: shard.cards.slice(0, 9) },
        expected,
      ),
  ...
  test("rejects a missing interface language and an intro question detached from its star slot", () => {
    ...
    const brokenLocale = {
      ...shard,
      cards: [
        { ...shard.cards[0], hintByLocale: missingPolish },
        ...shard.cards.slice(1),
      ],
    };
    expect(() =>
      validateLearningV2GeneratedSessionShardV1(brokenLocale, expected),
    ).toThrow("learning_v2_generator_hintByLocale_locales_invalid");
    const detachedIntro = {
      ...shard,
      cards: [
        { ...shard.cards[0], introQuestionId: "intro-q-2" },
        ...shard.cards.slice(1),
      ],
    };
    expect(() =>
      validateLearningV2GeneratedSessionShardV1(detachedIntro, expected),
  ...
  test("rejects a wrong canonical family and a forged localized meaning", () => {
    ...
    const wrongFamily = {
      ...shard,
      cards: [
        { ...shard.cards[0], family: "phrase_builder" },
        ...shard.cards.slice(1),
      ],
    };
    expect(() =>
      validateLearningV2GeneratedSessionShardV1(wrongFamily, expected),
      ...
      cards: [
        {
          ...
          contentItem: {
            ...
            learnerMeanings: [
              {
                ...shard.cards[0].contentItem.learnerMeanings[0],
                value: "Forged meaning",
              },
              ...shard.cards[0].contentItem.learnerMeanings.slice(1),
            ],
          },
        },
        ...shard.cards.slice(1),
    ...
    expect(() =>
      validateLearningV2GeneratedSessionShardV1(forgedMeaning, expected),
  ...
  test("requires an exact audio script only for listening and speaking families", () => {
    const shard = makeSessionShard();
    const audioIndex = shard.cards.findIndex(
      (card) => card.audioScript !== null,
    );
    const missingAudio = {
      ...shard,
      cards: shard.cards.map((card, index) =>
        index === audioIndex ? { ...card, audioScript: null } : card,
      ),
    };
    expect(() =>
      validateLearningV2GeneratedSessionShardV1(missingAudio, expected),
    ).toThrow("learning_v2_session_shard_audio_required");
    const silentIndex = shard.cards.findIndex(
      (card) => card.audioScript === null,
    );
    ...
    expect(() =>
      validateLearningV2GeneratedSessionShardV1(unexpectedAudio, expected),
