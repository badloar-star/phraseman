...
exports.synthesizeBotProfiles = synthesizeBotProfiles;
exports.botAnswerPlan = botAnswerPlan;
exports.botFocusBoost = botFocusBoost;
exports.botStyle = botStyle;
exports.chooseBotTarget = chooseBotTarget;
 ...
 */
function synthesizeBotProfiles(count, rankIndex, botsCfg, rand) {
    ...
    for (let i = 0; i < count; i += 1) {
        ...
        const accuracy = Math.min(botsCfg.accuracyMax, Math.max(botsCfg.accuracyMin, baseAccuracy + noise));
 ...
 */
function botAnswerPlan(profile, questionCount, rand, focusBoost = 0) {
 ...
 */
function botFocusBoost(state, slot, targetKey) {
...
function botStyle(uid) {
    const styles = ['centrist', 'expander', 'aggressor'];
    let h = 0;
    for (let i = 0; i < uid.length; i += 1)
        h = (h * 31 + uid.charCodeAt(i)) >>> 0;
    return styles[h % styles.length];
}
function chooseBotTarget(state, slot, rand, style = 'expander') {
    const targets = (0, engine_1.legalTargets)(state, slot);
    if (targets.length === 0)
        return null;
    ...
    const finisher = targets.find((key) => state.players.some((p) => p.status === 'alive' && p.slot !== slot && p.homeStarKey === key && p.cores === 1));
    if (finisher)
        return finisher;
    ...
    if (me && me.status === 'alive' && me.cores <= 1) {
        const homeNeighbors = new Set((0, hex_1.neighborsInMap)((0, hex_1.parseHexKey)(me.homeStarKey)).map((n) => (0, hex_1.hexKey)(n)));
    ...
    for (const star of Object.values(state.stars)) {
        if (star.owner !== null)
            starCounts.set(star.owner, (starCounts.get(star.owner) ?? 0) + 1);
    }
    let leaderSlot = null;
    let leaderCount = -1;
    for (const [s, c] of starCounts)
        if (c > leaderCount) {
            leaderCount = c;
            leaderSlot = s;
        }
    ...
    const ringWeight = { outer: 0.4, middle: 0.8, inner: 1.2, polar: 1.6 };
    const styleWeights = {
    ...
    const weighted = targets.map((key) => {
        ...
        const ambition = ringWeight[(0, hex_1.ringOf)((0, hex_1.parseHexKey)(key))] * styleWeights.ambitionMul;
        return {
            key,
