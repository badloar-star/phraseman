...
ROLE_CONFIG: dict[str, dict[str, str]] = {
    ...
    "fr1": {
        "voice": "coral",
        "instructions": (
            "Speak French slowly and clearly for an A1 learner. Warm, beautiful native French teacher voice. "
            "Natural France French pronunciation, correct liaison where natural. "
            "Say only the phrase, then stop cleanly with a tiny natural pause. No extra words."
        ),
    },
    "fr2": {
        "voice": "cedar",
        "instructions": (
            "Speak French at a calm medium lesson pace, clear and natural, not rushed. "
            "Native France French pronunciation with correct stress and liaison. "
            "Say only the phrase, then stop cleanly with a tiny natural pause. No extra words."
        ),
    },
    "fr3": {
        "voice": "sage",
        "instructions": (
            "Speak French slowly and clearly, not faster than the previous voice. "
            "Gentle learner-friendly native France French pronunciation. "
            "Say only the phrase, then stop cleanly with a tiny natural pause. No extra words."
        ),
    },
}
...
def digest_for(role: str, row: dict[str, Any]) -> str:
    text = str(row["ru"] if role == "ru" else row["fr"])
    payload = {
        "role": role,
        "index": int(row["index"]),
        "text": text,
...
def main() -> int:
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("exports/venga-phrase-packs/ru-fr-a1-vsscp/openai-audio"))
    ...
    for position, item in enumerate(manifest, start=1):
        ...
        if path.exists() and path.stat().st_size > 1024 and not args.overwrite:
            ...
            cached += 1
            continue
        print(f"[openai-tts-fr] {position:03d}/{len(manifest):03d} {item['role']} {item['index']:03d}", flush=True)
    ...
    summary = {
        ...
        "roles": {role: sum(1 for item in manifest if item["role"] == role) for role in ["ru", "fr1", "fr2", "fr3"]},
    }
