 ...
 * «ты пришёл по приглашению → пройди 1 урок → получишь 7 дней полного доступа».
 *
 * Зачем: раньше реферал-ссылка молча открывала главную, и новичок НЕ узнавал, что
 * за первый пройденный урок ему откроется 7 дней доступа (награда — referee VIP,
 * functions/src/referral.ts: REFEREE_VIP_DAYS = 7). Награда выдаётся автоматически
 * сервером после реального прохождения урока 1, но мотивации «пройди урок» в точке
 * входа не было. Эта модалка показывает её один раз.
 *
...
/**
 * «Приглашённый уже получил свои 7 дней» — ставится клиентом, когда из облака
 * прилетел признак reward (см. markRefereeWelcomeRewarded). Если получил —
 * приветствие «пройди урок ради 7 дней» больше не нужно.
 */
