...
const episode_lifecycle_transition_repository_1 = require("./episode_lifecycle_transition_repository");
const ref = { draftId: "d1", episodeId: "e1", revision: 1, revisionFingerprint: "a".repeat(64), contentHash: "b".repeat(64), ordinal: 1, chapterId: "c1", approvalStatus: "draft" };
const head = (status) => ({ schemaVersion: "episode-lifecycle.v1", draftId: "d1", episodeId: "e1", revision: 1, revisionFingerprint: ref.revisionFingerprint, status, changedBy: "author", changedAt: "2026-01-01T00:00:00.000Z", lifecycleRevision: 1 });
function store(initial, pinned = false) {
    let current = initial;
    ...
    return {
        ...
        async compareAndSetLifecycle(_ref, expected, next) { if (!current || current.lifecycleRevision !== expected)
            throw new Error("cas"); current = next; },
        async appendAudit() { },
...
describe("Episode lifecycle transitions", () => {
    it("submits an uninitialized revision into needs_review", async () => {
        const repo = new episode_lifecycle_transition_repository_1.EpisodeLifecycleTransitionRepository(store(undefined), { actorId: "author" });
    ...
    it("moves needs_review to changes_requested and replays idempotently", async () => {
        const repo = new episode_lifecycle_transition_repository_1.EpisodeLifecycleTransitionRepository(store(head("needs_review")), { actorId: "reviewer" });
    ...
    it("blocks archive while a season pin is active", async () => {
        const repo = new episode_lifecycle_transition_repository_1.EpisodeLifecycleTransitionRepository(store(head("approved"), true), { actorId: "reviewer" });
...
//# sourceMappingURL=episode_lifecycle_transition_repository.test.js.map