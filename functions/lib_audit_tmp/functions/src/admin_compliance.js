...
exports.adminMarkSafetyFlagsHandled = exports.adminListSafetyFlags = exports.adminGetComplianceOverview = exports.SAFETY_CATEGORIES = void 0;
exports.deriveComplianceCounts = deriveComplianceCounts;
exports.normalizeSafetyListInput = normalizeSafetyListInput;
exports.safetyFlagMatchesFilter = safetyFlagMatchesFilter;
exports.normalizeSafetyMarkInput = normalizeSafetyMarkInput;
...
exports.SAFETY_CATEGORIES = Object.freeze([
...
function requireAdmin(request, permission) {
    if (!request.app)
        throw new https_1.HttpsError('failed-precondition', 'App Check required');
...
function deriveComplianceCounts(input) {
    ...
    return Object.freeze({
        ...
        jurisdiction: { status: 'unavailable_not_collected' },
        complete: true,
    });
}
function normalizeCursor(value) {
    if (value == null || value === '')
        return null;
    if (!record(value) || (value.phase !== 'dated' && value.phase !== 'legacy'))
        throw new https_1.HttpsError('invalid-argument', 'Invalid cursor');
    const id = value.id == null && value.phase === 'legacy' ? null : requiredText(value.id, 'cursor id', 180);
    if (id && !SAFE_ID.test(id))
        throw new https_1.HttpsError('invalid-argument', 'Invalid cursor id');
    if (value.phase === 'legacy')
        return Object.freeze({ phase: 'legacy', id });
    const createdAtMs = Number(value.createdAtMs);
    if (!Number.isFinite(createdAtMs) || createdAtMs < 0 || !id)
        throw new https_1.HttpsError('invalid-argument', 'Invalid dated cursor');
    return Object.freeze({ phase: 'dated', createdAtMs, id });
}
function normalizeSafetyListInput(value) {
    const data = record(value) ? value : {};
    const handled = data.handled == null ? 'open' : data.handled;
    if (handled !== 'open' && handled !== 'handled' && handled !== 'all')
        throw new https_1.HttpsError('invalid-argument', 'Invalid handled filter');
    const category = data.category == null || data.category === '' ? null : requiredText(data.category, 'category', 40);
    if (category && !exports.SAFETY_CATEGORIES.includes(category))
        throw new https_1.HttpsError('invalid-argument', 'Invalid category');
    const requestedLimit = data.limit == null ? 50 : Number(data.limit);
    if (!Number.isInteger(requestedLimit) || requestedLimit < 1 || requestedLimit > MAX_PAGE)
        throw new https_1.HttpsError('invalid-argument', 'limit must be 1..100');
    const cursor = normalizeCursor(data.cursor);
    return Object.freeze({ handled, category, limit: requestedLimit, cursor });
}
function safetyFlagMatchesFilter(row, filter) {
...
function normalizeSafetyMarkInput(value) {
    if (!record(value))
        throw new https_1.HttpsError('invalid-argument', 'Safety command required');
    if (!Array.isArray(value.ids) || value.ids.length < 1 || value.ids.length > MAX_PAGE)
        throw new https_1.HttpsError('invalid-argument', 'ids must contain 1..100 values');
    const ids = [...new Set(value.ids.map((id) => requiredText(id, 'id', 180)))];
    if (ids.some((id) => !SAFE_ID.test(id)))
        throw new https_1.HttpsError('invalid-argument', 'Invalid safety id');
    ...
    if (!SAFE_ID.test(requestId) || !SAFE_ID.test(idempotencyKey))
        throw new https_1.HttpsError('invalid-argument', 'Invalid request token');
    return Object.freeze({ ids: Object.freeze(ids), reason, requestId, idempotencyKey });
}
async function count(query) {
    return (await query.count().get()).data().count;
}
async function complianceOverview(db) {
    const consents = db.collection(CONSENTS);
    const safety = db.collection(SAFETY);
    const minor = (bracket) => safety.where('ageBracket', '==', bracket);
    const [consentTotal, adult, teenSafe, under13, analyticsGranted, analyticsDenied, safetyTotal, safetyHandled, teenTotal, under13Total, teenHandled, under13Handled] = await Promise.all([
...
exports.adminGetComplianceOverview = (0, https_1.onCall)({
    region: REGION, enforceAppCheck: callable_options_1.ENFORCE_APP_CHECK, timeoutSeconds: 30, memory: '256MiB',
}, async (request) => {
    requireAdmin(request, 'diagnostics.read');
    return complianceOverview(admin.firestore());
});
async function exactFilteredTotal(db, filter) {
    let base = db.collection(SAFETY);
    if (filter.category)
...
exports.adminListSafetyFlags = (0, https_1.onCall)({
    region: REGION, enforceAppCheck: callable_options_1.ENFORCE_APP_CHECK, timeoutSeconds: 30, memory: '256MiB',
}, async (request) => {
    requireAdmin(request, 'diagnostics.read');
    ...
    let cursor = filter.cursor ?? { phase: 'dated', createdAtMs: Number.MAX_SAFE_INTEGER, id: '\uf8ff' };
    let scanned = 0;
    let exhausted = false;
    const rows = [];
    while (rows.length < filter.limit && scanned < MAX_SCAN && !exhausted) {
        ...
        if (cursor.phase === 'dated') {
            ...
            if (cursor.createdAtMs !== Number.MAX_SAFE_INTEGER)
                query = query.startAfter(cursor.createdAtMs, cursor.id);
        }
        else {
        ...
        for (const doc of snapshot.docs) {
            ...
            if ((cursor.phase === 'dated' ? isDated : !isDated) && safetyFlagMatchesFilter(data, filter))
                rows.push({ id: doc.id, ...data });
            if (rows.length >= filter.limit || scanned >= MAX_SCAN)
                break;
        }
...
exports.adminMarkSafetyFlagsHandled = (0, https_1.onCall)({
    region: REGION, enforceAppCheck: callable_options_1.ENFORCE_APP_CHECK_SENSITIVE, timeoutSeconds: 30, memory: '256MiB',
}, async (request) => {
    const { actorUid, role } = requireAdmin(request, 'diagnostics.status.write');
    const input = normalizeSafetyMarkInput(request.data);
    const db = admin.firestore();
    const fingerprint = (0, crypto_1.createHash)('sha256').update(JSON.stringify({ ids: [...input.ids].sort(), reason: input.reason, requestId: input.requestId })).digest('hex');
    const operationId = `safety_handle_${(0, crypto_1.createHash)('sha256').update(input.idempotencyKey).digest('hex')}`;
    return db.runTransaction(async (tx) => {
        ...
        if (operation.exists) {
            ...
            if (previous.requestFingerprint !== fingerprint || previous.actorUid !== actorUid)
                throw new https_1.HttpsError('already-exists', 'Idempotency key conflict');
        ...
        snapshots.forEach((snapshot, index) => {
            if (!updated.includes(input.ids[index]))
                return;
        ...
        const audit = (0, audit_contract_1.createAuditRecord)({ action: 'safety_flags.mark_handled', actorUid, role,
...
//# sourceMappingURL=admin_compliance.js.map