...
interface WeeklyReviewPreflightDependencies {
  ...
  resolvePremium: (db: FirebaseFirestore.Firestore, stableUid: string, authUid: string) => Promise<boolean>;
  rejectFree: () => never;
}
...
async function runWeeklyReviewPreflight(
  ...
  dependencies: WeeklyReviewPreflightDependencies = {
    ...
    resolvePremium: (db, stableUid, authUid) => resolvePremiumAccess(db, stableUid, Date.now(), authUid),
